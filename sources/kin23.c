...
double kinematic_23(double Pcm, int i3, double M45, double cs1, double cs2,
                    double fi, REAL *pmass, REAL *pvect) {
  int i;
  ...
  if (pvect[0] + pvect[4] <= M45 + pmass[i3])
    return 0;
  ...
  return 3.8937966E8 * pcm1 * pcm2 /
         (512 * M_PI * M_PI * M_PI * M_PI * (pvect[0] + pvect[4]) *
          (pvect[0] + pvect[4]));
  return r;
}
...
static double dIdcs2(double cs1) {
  cs1_ = cs1;
  return gauss(Idfi, -1., 1., Np);
}
static double dIdM(double M) {
  NCALL++;
  M_ = M;
  return gauss(dIdcs2, -1., 1., Np);
}
...
double cs23(numout *cc, int nsub, double Pcm, int ii3, int *err) {
  ...
  Mmax = sqrt(Pcm * Pcm + pmass[0] * pmass[0]) +
         sqrt(Pcm * Pcm + pmass[1] * pmass[1]) - pmass[i3];
  ...
  if (s == NULL) {
    ...
    if (err_) {
      if (err)
        *err = err_;
      printf("error in simpson kin23.c line 189\n");
    }
    return r;
  }
  ...
  //  displayFunc(dIdM_w_mu_mu, 0., uMax,"dIdM_w_mu_mu");
  {
    ...
    if (err_) {
      if (err)
        *err = err_;
      printf("error in simpson cs23.c line 206\n");
    }
    return r;
  }
}
...
static double func_23(double *x, double wgt) {
  ...
  REAL pvect[20];
  double factor;
  factor = kinematic_23(Pcm_, i3, Mmin + x[0] * (Mmax - Mmin), 2 * (x[1] - 0.5),
                        2 * (x[2] - 0.5), M_PI * x[3], pmass, pvect) /
           Pcm_;
  if (factor == 0)
    return 0;
  return factor * 2 * (Mmax - Mmin) * 4 * M_PI *
         (cc23_->interface->sqme)(nsub_, GG, pvect, NULL, &err);
}

double cs23Vegas(numout *cc, int nsub, double Pcm, int ii3, int nSess1,
                 int nCall1, int nSess2, int nCall2, double *dcs, double *chi2,
                 int *err) {
  ...
  Pcm_ = Pcm;
  cc23_ = cc;
  if (cc->interface->nin != 2 || cc->interface->nout != 3) {
    *err = -1;
    return 0;
  }
  i3 = ii3;
    ...
    if (nSess[k] && nCall[k]) {
      ...
      *dcs = 1 / sqrt(s0);
      if (nSess[k] <= 1)
        ...
        *chi2 = (s2 - s1 * s1 / s0) / (nSess[k] - 1);
    }
  vegas_finish(vegPtr);
  return rVal;
}

double kinematic_24(double Pcm, int i3, int i4, double M1, double M2,
                    double xcos, double xcos1, double xcos2, double fi1,
                    double fi2, REAL *pmass, REAL *P) {
  ...
  if (sqrtS <= M1 + M2)
    return 0;
  if (sqrtS <= pmass[2] + pmass[3] + pmass[4] + pmass[5])
    return 0;

  pcm = decayPcm(sqrtS, M1, M2);

  // printf("decayPcm(%E %E %E) ? %E\n", sqrtS,M1,M2, 1-
  // sqrt(M1*M1+pcm*pcm)/sqrtS- sqrt(M2*M2+pcm*pcm)/sqrtS);

  p1cm = decayPcm(M1, pmass[i3], pmass[i4]);
  p2cm = decayPcm(M2, pmass[i5], pmass[i6]);

  P[i3 * 4 + 0] = sqrt(pmass[i3] * pmass[i3] + p1cm * p1cm);
  P[i4 * 4 + 0] = sqrt(pmass[i4] * pmass[i4] + p1cm * p1cm);
  P[i3 * 4 + 2] = p1cm * sqrt(1 - xcos1 * xcos1);
  P[i3 * 4 + 1] = sin(fi1) * P[i3 * 4 + 2];
  P[i4 * 4 + 1] = -P[i3 * 4 + 1];
  P[i3 * 4 + 2] *= cos(fi1);
  P[i4 * 4 + 2] = -P[i3 * 4 + 2];
  P[i3 * 4 + 3] = p1cm * xcos1;
  ...
  P[i5 * 4 + 0] = sqrt(pmass[i5] * pmass[i5] + p2cm * p2cm);
  P[i6 * 4 + 0] = sqrt(pmass[i6] * pmass[i6] + p2cm * p2cm);
  P[i5 * 4 + 2] = p2cm * sqrt(1 - xcos2 * xcos2);
  P[i5 * 4 + 1] = sin(fi2) * P[i5 * 4 + 2];
  P[i6 * 4 + 1] = -P[i5 * 4 + 1];
  P[i5 * 4 + 2] *= cos(fi2);
  P[i6 * 4 + 2] = -P[i5 * 4 + 2];
  P[i5 * 4 + 3] = -p2cm * xcos2;
  ...
  // printf("dM1=%E %E %E %E \n", 1-(P[i3*4+0]+P[i4*4+0])/M1, P[i3*4+1]+
  // P[i4*4+1], P[i3*4+2]+ P[i4*4+2],P[i3*4+3]+ P[i4*4+3]); printf("dM2=%E %E %E
  // %E\n",  1-(P[i5*4+0]+P[i6*4+0])/M2, P[i5*4+1]+ P[i6*4+1], P[i5*4+2]+
  // P[i6*4+2],P[i5*4+3]+ P[i6*4+3]);
  ...
  return pcm * p1cm * p2cm * 3.8937966E8 / pow(2 * M_PI, 12 - 5) /
         pow(2, 4 + 2) / sqrtS / sqrtS;
}
...
static double func_24(double *x, double wgt) {
  ...
  sqrtS = sqrt(Pcm_ * Pcm_ + pmass[0] * pmass[0]) +
          sqrt(Pcm_ * Pcm_ + pmass[1] * pmass[1]);
  M1 = (1 - x[0]) * (pmass[i3] + pmass[i4]) +
       x[0] * (sqrtS - pmass[i5] - pmass[i6]);
  factor = sqrtS - pmass[i3] - pmass[i4] - pmass[i5] - pmass[i6];
  M2 = (1 - x[1]) * (pmass[i5] + pmass[i6]) + x[1] * (sqrtS - M1);
  factor *= sqrtS - M1 - pmass[i5] - pmass[i6];

  factor *= 2 * 2 * 2 * (2 * M_PI) * (2 * M_PI) *
            kinematic_24(Pcm_, i3, i4, M1, M2, 1 - 2 * x[2], 1 - 2 * x[3],
                         1 - 2 * x[4], 2 * M_PI * x[5], 2 * M_PI * x[6], pmass,
                         pvect);
...
double cs24Vegas(numout *cc, int nsub, double Pcm, int ii3, int ii4, int nSess1,
                 int nCall1, int nSess2, int nCall2, double *dcs, double *chi2,
                 int *err) {
  ...
  Pcm_ = Pcm;
  cc24_ = cc;
  if (cc->interface->nin != 2 || cc->interface->nout != 4) {
    *err = -1;
    return 0;
  }
  ...
  i3 = ii3;
  i4 = ii4;
  for (i5 = 2; i5 == i3 || i5 == i4; i5++)
    continue;
  for (i6 = 2; i6 == i3 || i6 == i4 || i6 == i5; i6++)
    continue;
  ...
  for (i = 0; i < 6; i++)
    cc->interface->pinf(nsub, 1 + i, pmass + i, NULL);
  nsub_ = nsub;
  sqrtS = sqrt(Pcm * Pcm + pmass[0] * pmass[0]) +
          sqrt(Pcm * Pcm + pmass[1] * pmass[1]);
  *err = 0;
  ...
  C = 3.8937966E8 * pow(2 * M_PI, 4) / (4 * Pcm * sqrtS * sqrtS) /
