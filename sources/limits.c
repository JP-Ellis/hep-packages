...
int Zinvisible(void) {
  ...
  zname = pdg2name(23);
  if (!zname) {
    printf("Z boson  is absent\n");
    return 0;
  }
  width = pWidth(zname, &L);
  for (i = 0, br = 0; i < 2; i++)
    if (name[i] && 2 * mass[i] < pMass(zname)) {
      char channel[40];
  ...
  if (br * 1000 * width > 0.5) {
    printf(" partial width Z->DM,DM is %.2EMeV,  more than 0.5 MeV. See "
           "1401.2447\n",
           br * 1000 * width);
    return 1;
  } else
  ...
  for (m = 0; m < 2; m++) // cycle over two DM sectors
  {
    ...
    for (n = 1;; n++) // looking for next DM
    {
      ...
      if (brZ > 0) {
        ...
        if (aCDM)
          sprintf(process + strlen(process), ",%s", aCDM);
        sprintf(process + strlen(process), "{%s", CDM_);
        aCDM_ = pdg2name(-pNum(CDM_));
        if (aCDM_ && strcmp(aCDM_, CDM_) == 0)
          aCDM_ = NULL;
        if (aCDM_)
          sprintf(process + strlen(process), ",%s", aCDM_);
        // printf("process=%s\n",process);
        sprintf(lib, "ee_");
        pname2lib(CDM, buff);
        strcat(lib, buff);
