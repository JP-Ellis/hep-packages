...
static double s_integrandT_(double sqrtS) {
  ...
  bess = sqrt(2 * x1 * x2 / y / M_PI) * exp(-(y - x1 - x2)) * K1pol(1 / y) /
         (K2pol(1 / x1) * K2pol(1 / x2));
...
static double u_integrand_(double u) {
  ...
  if (sqrtS <= M1 + M2 || sqrtS <= pmass[2] + pmass[3])
    return 0;
...
static double vsigma23integrandT(double *x, double w) {
  ...
  if (sqrtS <= sqrtSmin || sqrtS >= sqrtSmax)
    return 0;
  ...
  r = kinematic_23(pcmIn, i3, M45, 2 * x[2] - 1, 2 * x[3] - 1, M_PI * x[4],
                   pmass, pvect) *
      8 * M_PI * (M45_max - M45_min); //  /pcmIn
  if (r == 0)
    return 0;
...
static double vsigma23integrand0(double *x, double w) {
  double r, sqrtS = M1 + M2, M45_min = pmass[i4] + pmass[i5],
            M45_max = M1 + M2 - pmass[i3];
  int err;
  double GG;
  REAL pvect[20];

  r = kinematic_23(0., i3, M45_min + x[0] * (M45_max - M45_min), 0.5,
                   2 * x[1] - 1, M_PI * x[2], pmass, pvect) *
      8 * M_PI * (M45_max - M45_min);
  if (r == 0)
    return 0;
  GG = sqrt(4 * M_PI * alphaQCD(sqrtS));
  r *= CI->sqme(1, GG, pvect, NULL, &err);
...
static double vsigma24integrandT(double *x, double w) {
  ...
  double M34_min = pmass[i3] + pmass[i4],
         M34_max = sqrtSmax - pmass[i5] - pmass[i6];
  double M56_min = pmass[i5] + pmass[i6],
         M56_max = sqrtSmax - pmass[i3] - pmass[i4];
  int err;
  double r, x1, x2, y, z, bess, Rm;
  double GG;
  REAL pvect[24];
  ...
  r = kinematic_24(pcmIn, i3, i4, M34, M56, 2 * x[3] - 1, 2 * x[4] - 1,
                   2 * x[5] - 1, 2 * M_PI * x[6], 2 * M_PI * x[7], pmass,
                   pvect) *
      (M34_max - M34_min) * (M56_max - M56_min) * 2 * 4 * M_PI * 4 *
      M_PI; //  /pcmIn
  GG = sqrt(4 * M_PI * alphaQCD(sqrtS));
  r *= CI->sqme(1, GG, pvect, NULL, &err);
  bess = sqrt(2 * x1 * x2 / y / M_PI) * exp(-(y - x1 - x2)) * K1pol(1 / y) /
...
static double vsigma24integrand0(double *x, double w) {
  ...
  double M34_min = pmass[i3] + pmass[i4],
         M34_max = M1 + M2 - pmass[i5] - pmass[i6],
         M56_min = pmass[i5] + pmass[i6],
         M56_max = M1 + M2 - pmass[i3] - pmass[i4];
  int err;
  double GG;
  REAL pvect[24];
  ...
  r = kinematic_24(0., i3, i4, M34, M56, 0.5, 2 * x[2] - 1, 2 * x[3] - 1,
                   2 * M_PI * x[4], 2 * M_PI * x[5], pmass, pvect) *
...
double vSigmaCC(double T, numout *cc, int mode) {
  ...
  if (Qaddress && CI->nout == 2) {
    oldQ = *Qaddress;
    for (i = 0; i < 2; i++)
      pname[i] = CI->pinf(1, i + 1, pmass + i, pdg + i);
  ...
  if (mode) {
    if (pname[0][0] != '~' || pname[1][0] != '~')
      return 0;
    if (T == 0 && (M1 < Mcdm || M2 < Mcdm))
      return 0;
    dmOut = 0;
    for (i = 2; i < 2 + CI->nout; i++)
      if (pname[i][0] == '~')
        dmOut++;
    if (dmOut == 2)
      return 0;
  ...
  n0 = 0;
  if (CI->nout > 2)
    for (n = 1; (s = CI->den_info(1, n, &m, &w, NULL)); n++) {
      double d = sing2(s, CI->nout, CI->va[m], CI->va[w]);
      if (!isfinite(d)) {
        printf("non-integrable pole\n");
        return 0;
      }
      if (d > dMax) {
        dMax = d;
  ...
  case 3: {
    ...
    if (T == 0)
      a = vegas_chain(3, vsigma23integrand0, 2000, 1., 0.03, &dI, NULL);
    else
      a = vegas_chain(5, vsigma23integrandT, 2000, 1., 0.03, &dI, NULL);
    break;
  }
  case 4:
    if (n0) {
    ...
    if (T == 0)
      a = vegas_chain(6, vsigma24integrand0, 4000, 1., 0.03, &dI, NULL);
    else
      a = vegas_chain(8, vsigma24integrandT, 5000, 1., 0.03, &dI, NULL);
  ...
  if (mode) {
    ...
    double s = 0, k = M1 * M1 * M2 * M2 / sqrt(M1 * M2) * K2pol(T / M1) *
                      K2pol(T / M2);
    for (i = 0; i < nModelParticles; i++)
      if (ModelPrtcls[i].name[0] == '~') {
        ...
        if (strcmp(p0, ModelPrtcls[i].name) == 0) {
          c0 = ModelPrtcls[i].aname;
          k *= dim;
        } else if (strcmp(p0, ModelPrtcls[i].aname) == 0) {
          c0 = ModelPrtcls[i].name;
          k *= dim;
        }
        if (strcmp(p1, ModelPrtcls[i].name) == 0) {
          c1 = ModelPrtcls[i].aname;
          k *= dim;
        } else if (strcmp(p1, ModelPrtcls[i].aname) == 0) {
          c1 = ModelPrtcls[i].name;
          k *= dim;
        }
