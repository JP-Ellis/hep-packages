...
int LilithMO(char *fname) {
  ...
  for (k = 0; k < WWA->nTerms; k++)
    if (strcmp(WWA->SymbVert[k], "p1.m1*m3.m2") == 0)
      break;
  double EE = coeff[k];
  ...
  for (k = 0; k < WWZ->nTerms; k++)
    if (strcmp(WWZ->SymbVert[k], "p1.m1*m3.m2") == 0)
      break;
  double SW = sin(atan(EE / coeff[k]));
  double vev = 2 * MW * SW / EE;

  //  fprintf(f,"# SM basic parameters: ee=%E  MW=%.3E alphaEM^{-1}=%.3E
  ...
  for (i = 0; i < nModelParticles; i++)
    if (ModelPrtcls[i].name[0] != '~' && ModelPrtcls[i].spin2 == 0 &&
      ...
      if (strcmp(ModelPrtcls[i].name, ModelPrtcls[i].aname) == 0 && m > 123 &&
          m < 128)
  ...
  for (i = 0; i < nHiggs; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (c[1] == 0)
        fprintf(f, "       <C to=\"%s\" >  %E </C>\n", fn[j], c[0]);
      else {
        if (c[0])
          fprintf(f, "       <C to=\"%s\" part=\"re\"> %E </C>\n", fn[j], c[0]);
        if (c[1])
          fprintf(f, "       <C to=\"%s\" part=\"im\"> %E </C>\n", fn[j], c[1]);
      }
    }
    ...
    for (l = L; l; l = l->next) {
      ...
      if (nd == 2) {
        ...
        if (id[0] == 22 && id[1] == 22)
          BrAA = br;
        if ((abs(id[0]) == idDM1 && abs(id[1]) == idDM1) ||
            (abs(id[0]) == idDM2 && abs(id[1]) == idDM2))
          BrDM += br;
      }
      for (k = 0; k < nd; k++)
        if (!isSMP(id[k])) {
