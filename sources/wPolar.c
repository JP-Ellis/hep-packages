...
int vPolar(int out1, int out2, int out3, double *left, double *right,
           double *lng) {
  ...
  for (i = 0; i < nModelParticles; i++) {
    if (ModelPrtcls[i].NPDG == out1) {
      Wp = ModelPrtcls[i].name;
      sprintf(n1, "p%d", i + 1);
    }
    if (ModelPrtcls[i].NPDG == -out1) {
      Wp = ModelPrtcls[i].aname;
      sprintf(n1, "m%d", i + 1);
    }

    if (ModelPrtcls[i].NPDG == out2) {
      el = ModelPrtcls[i].name;
      sprintf(n2, "p%d", i + 1);
    }
    if (ModelPrtcls[i].NPDG == -out2) {
      el = ModelPrtcls[i].aname;
      sprintf(n2, "m%d", i + 1);
    }

    if (ModelPrtcls[i].NPDG == out3) {
      Ne = ModelPrtcls[i].name;
      sprintf(n3, "p%d", i + 1);
    }
    if (ModelPrtcls[i].NPDG == -out3) {
      Ne = ModelPrtcls[i].aname;
      sprintf(n3, "m%d", i + 1);
    }
    ...
    if (ModelPrtcls[i].name[0] == '~') {
      ...
      if (mass < massMin || massMin < 0) {
        o1 = ModelPrtcls[i].name;
        O1 = ModelPrtcls[i].aname;
        massMin = mass;
        oId = i + 1;
      }
    }
  ...
  for (i = 0; i < 5; i++)
    cc->interface->pinf(1, i + 1, m + i, code + i);
  for (i = 0; i < 20; i++)
    pvect[i] = 0;
  ...
  md = m[ie] - m[in];
  pcm2 =
      sqrt((m[iW] * m[iW] - ms * ms) * (m[iW] * m[iW] - md * md)) / (2 * m[iW]);
