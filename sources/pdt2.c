...
int setPDT(char *name) {
  ...
  for (cpdt = allPDT; cpdt; cpdt = cpdt->next)
    if (cpdt->beamP == 2212 && strcmp(name, cpdt->name) == 0) {
      ...
      for (i = 0; i < 8; i++) {
        int j;
        for (j = 0; cpdt->partons[j]; j++)
          if (cpdt->partons[j] == pdg[i]) {
...
static pdtStr *convStrFunAux(int pc1, int pc2) {
  ...
  {
    int nq = 20;
    double q_grid[20] = {1.30000E+00, 1.53106E+00, 1.83098E+00, 2.22659E+00,
                         2.75766E+00, 3.48439E+00, 4.50000E+00, 6.12359E+00,
                         8.60159E+00, 1.25127E+01, 1.89184E+01, 2.98475E+01,
                         4.93546E+01, 8.59491E+01, 1.58477E+02, 3.11214E+02,
                         6.55142E+02, 1.48904E+03, 3.68299E+03, 1.00000E+04};
    int nx = 96;
    double x_grid[96] = {
        5.E-7,       1.00000E-06, 1.28121E-06, 1.64152E-06, 2.10317E-06,
        2.69463E-06, 3.45242E-06, 4.42329E-06, 5.66715E-06, 7.26076E-06,
        9.30241E-06, 1.19180E-05, 1.52689E-05, 1.95617E-05, 2.50609E-05,
        3.21053E-05, 4.11287E-05, 5.26863E-05, 6.74889E-05, 8.64459E-05,
        1.10720E-04, 1.41800E-04, 1.81585E-04, 2.32503E-04, 2.97652E-04,
        3.80981E-04, 4.87518E-04, 6.26039E-04, 8.00452E-04, 1.02297E-03,
        1.30657E-03, 1.66759E-03, 2.12729E-03, 2.71054E-03, 3.44865E-03,
        4.37927E-03, 5.54908E-03, 7.01192E-03, 8.83064E-03, 1.10763E-02,
        1.38266E-02, 1.71641E-02, 2.11717E-02, 2.59364E-02, 3.15062E-02,
        3.79623E-02, 4.53425E-02, 5.36750E-02, 6.29705E-02, 7.32221E-02,
        8.44039E-02, 9.64793E-02, 1.09332E-01, 1.23067E-01, 1.37507E-01,
        1.52639E-01, 1.68416E-01, 1.84794E-01, 2.01731E-01, 2.19016E-01,
        2.36948E-01, 2.55242E-01, 2.73927E-01, 2.92954E-01, 3.12340E-01,
        3.32036E-01, 3.52019E-01, 3.72282E-01, 3.92772E-01, 4.13533E-01,
        4.34326E-01, 4.55495E-01, 4.76836E-01, 4.98342E-01, 5.20006E-01,
        5.41818E-01, 5.63773E-01, 5.85861E-01, 6.08077E-01, 6.30459E-01,
        6.52800E-01, 6.75387E-01, 6.98063E-01, 7.20830E-01, 7.43683E-01,
        7.66623E-01, 7.89636E-01, 8.12791E-01, 8.35940E-01, 8.59175E-01,
        8.82485E-01, 9.05866E-01, 9.29311E-01, 9.52817E-01, 9.76387E-01,
        1.00000E+00};
    ...
    if (D->interpolation == int_cteq6) {
      ...
      for (i = 0; i < D->nx; i++)
        D->x_grid_aux[i] = pow(D->x_grid[i], 0.3);

      D->q_grid_aux = malloc(nq * sizeof(double));
      for (i = 0; i < D->nq; i++)
        D->q_grid_aux[i] = log(D->lq_grid[i] - log(0.22));
    }
    ...
    D->q_max = 10000;
    D->pow0 = D->pow1 =
        0; /* factor x^pow *(1-x)^pow1 applied after interpolation */
  ...
  for (iq = 0; iq < D->nq; iq++) {
    ...
    for (ix = 0; ix < D->nx; ix++) {
      x0_ = D->x_grid[ix];
      D->strfun[D->nx * iq + ix] =
          simpson(conv_integrand, 0., -log(x0_), 1.E-3, NULL);
    }
  }
  cData[nc] = D;
  if (nPROCSS > 1)
...
double alpha_2(double q) {
  return parton_alpha(q);
} /* it needs only  to avoid external link with functions from  num.a  */
...
static double x_integrand(double x) {
  if (x == 0)
    ...
    return x * parton_distr(pc1_, x, q_);
}
