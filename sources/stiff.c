...
static double *a = NULL, *ysav = NULL, *yerr = NULL, *dydx = NULL, *dfdx = NULL,
              *dfdy = NULL;
static int *indx;
...
static double *dysav = NULL, *g1 = NULL, *g2 = NULL, *g3 = NULL, *g4 = NULL,
              *del = NULL, *ytemp = NULL;
...
static int stiff_1(double y[], int n, double *x, double htry, double eps,
  ...
  for (jtry = 1; jtry <= MAXTRY; jtry++) {
    ...
    *x = xsav + h;
    if (*x == xsav) {
      printf("stepsize not significant in stiff\n");
      return 1;
    }
    errmax = 0.0;
    if (yscal)
...
int stiff(int first, double xstart, double xend, int n, double *y,
...
static int stifbs_1(int first, double y[], int nv, double *xx, double htry,
    ...
    for (k = 0; k < kmax; k++) {
      ...
      if (k != 0) {
        ...
        if (yscal)
          ...
          for (i = 0; i < nv; i++)
            errmax = FMAX(errmax, fabs(yerr[i]));
        errmax /= eps;
        km = k - 1;
...
int stifbs(int first, double xstart, double xend, int nv, double *y,
           double *yscal, double eps, double *htry,
           void (*derivs)(double, double *, double *, double, double *,
                          double *)) {
  double x = xstart;
...
void derivs(double x, double *y, double *f, double h, double *dfdx,
            double *dfdy) {
  int i, n = 3;
