  ...
  for (;;) {
    ...
    *x = xsav + h;
    ...
    if (errmax <= 1.0) {
      *hdid = h;
      *hnext =
          ((errmax > ERRCON ? SAFETY * h * exp(PGROW * log(errmax)) : 4 * h));
      break;
    }
    {
...
int odeint(double *ystart, int nvar, double x1, double x2, double eps,
  ...
  double **allAlloc[9] = {NULL, NULL,   NULL,  &dym,  &dyt,
                          &yt,  &dysav, &ysav, &ytemp};
  allAlloc[0] = &yscal;
  allAlloc[1] = &y;
  allAlloc[2] = &dydx;
  for (i = 0; i < 9; i++)
    *allAlloc[i] = (double *)malloc(nvar * sizeof(double));
  ...
  for (nstp = 1; nstp <= MAXSTP; nstp++) {
    ...
    if ((x + h - x2) * (x + h - x1) > 0.0)
      h = x2 - x;
    if (rkqc(y, dydx, nvar, &x, h, eps, yscal, &hdid, &hnext, derivs))
      break;
...
double K2pol(double x) {
  if (x < 0.1)
    return 1 + 1.875 * x * (1 + 0.4375 * x * (1 - 0.375 * x));
  else
    return bessK2(1 / x) * exp(1 / x) * sqrt(2 / M_PI / x);
}

double K1pol(double x) {
  if (x < 0.1)
    return 1 + 0.375 * x * (1 - 0.3125 * x * (1 + 0.875 * x));
  else
    return bessK1(1 / x) * exp(1 / x) * sqrt(2 / M_PI / x);
}
...
int buildInterpolation(double (*Fun)(double), double x1, double x2, double eps,
                       double delt, int *N_, double **xa_, double **ya_) {
  int i, cnt, N, k;
  double *xa, *ya, dx0;
  dx0 = fabs(x2 - x1) * delt;
  N = 5;
  ...
  for (cnt = 1; cnt;) {
    ...
    for (i = 0; i < N; i++) {
      ...
      if (i < N - 1 && fabs(xa[i + 1] - xa[i]) < dx0)
        continue;
      else if (i > 0 && fabs(xa[i] - xa[i - 1]) < dx0)
        continue;

      del(i, &N, xa, ya);
      yy = polint3(x, N, xa, ya);
      ins(i, x, y, &N, xa, ya);
      if ((eps > 0 && fabs(yy - y) > eps) ||
          (eps < 0 && fabs(yy - y) > -eps * (y))) {
        cnt = 1;
...
*/
{
  ...
  if (x > mu - 1.E-5)
    return 1 - exp(-mu);
  for (k = 0, C0 = 0, kf = 1; k <= mu / x; k++, kf *= k) {
    C0 += pow(k * x - mu, k) * exp(-k * x) * (1 + k / (mu - k * x)) / kf;
  }
  return C0;
}
...
static double amotry(double *p, double *y, int ndim, double (*f)(double *),
                     int ilo, double fac) {
  int i, j;
...
double amoeba(double *pp, double *dp, int ndim, double (*f)(double *),
  ...
  for (i = 0; i < ndim; i++) {
    pp[i] = p[i + ihi * ndim];
    dp[i] = p[i + ilo * ndim] - p[i + ihi * ndim];
  }
  ysave = y[ihi];
  free(y);
  free(p);
...
#define MAXSTEP 15
double vegas_chain(int ndim, double (*Integrand)(double *, double), int N0,
                   double Nfact, double eps, double *dI, void (*clean)(void)) {
...
double ch2pval(int nexp, double ch2obs) {
  ...
  return simpson(ch2pva_int, 0, exp(-alpha * ch2obs / 2), 1.E-3,
                 NULL); /// exp(lgamma(nexp/2.))/alpha;
}
