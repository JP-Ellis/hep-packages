...
static double sigma23(double PcmIn) {
  ...
  if (AUX[nsub22].w[l_ - 2]) {
    ...
    if (wV1 * wV2 > 0)
      r *= decayPcmW(sqrtS, MV1, MV2, wV1, wV2, 0) /
           decayPcmW(sqrtS, MV1, MV2, wV1, 0, 0);
  }
...
static double sigma(double PcmIn) {
  ...
  if (AUX[nsub22].nTab > 0 &&
      PcmIn <= AUX[nsub22].pcmTab[AUX[nsub22].nTab - 1]) {
    if (PcmIn < AUX[nsub22].pcmTab[0])
      r = 0;
    else {
      r = exp(polint3(PcmIn, AUX[nsub22].nTab, AUX[nsub22].pcmTab,
                      AUX[nsub22].csTab)) /
          PcmIn;
    }
  } else {
    if (kin22(PcmIn, pmass))
      return 0.;
    if (gaussInt)
      ...
      if (err) {
        do_err = do_err | err;
        printf("error in simpson omega.c line 130\n");
      }
    }
    if (r < 0) {
      neg_cs_flag = 1;
    ...
    if ((VZdecay || VWdecay) && (AUX[nsub22].w[0] || AUX[nsub22].w[1])) {
      double f;
      double sqrtS = sqrt(PcmIn * PcmIn + pmass[0] * pmass[0]) +
                     sqrt(PcmIn * PcmIn + pmass[1] * pmass[1]);
      if (sqrtS - pmass[2] - pmass[3] <
          15 * (AUX[nsub22].w[0] + AUX[nsub22].w[1]))
        f = decayPcmW(sqrtS, pmass[2], pmass[3], AUX[nsub22].w[0],
                      AUX[nsub22].w[1], 5) /
            decayPcm(sqrtS, pmass[2], pmass[3]);
      else {
        f = 1;
        if (AUX[nsub22].w[0])
          f -= AUX[nsub22].w[0] / pmass[2] / M_PI;
        if (AUX[nsub22].w[1])
          f -= AUX[nsub22].w[1] / pmass[3] / M_PI;
      }
      r *= f;
    }
  }
...
static double geffDM(double T) {
  ...
  for (l = 0; l < NC; l++)
    if (!feebleParticles[oddPpos[sort[l]]]) {
      int k = sort[l];
      double A = Mcdm / T * (inMass[k] - Mcdm) / Mcdm;
      if (A > 15 || Mcdm + inMass[k] > MassCut)
        return sum;
      sum += inG[k] * pow(inMass[k] / Mcdm, 1.5) * exp(-A) *
             K2pol(1 / (Mcdm / T + A));
    }
  return sum;
}
...
static double weight_integrand(double s3) {
  ...
  geff = gEff(T);
  gf = geffDM(T);
  return K1pol(T / (Mcdm * y_pass)) *
         exp((1 / T - 1 / T_) * (2 - y_pass) * Mcdm) * sqrt(Mcdm / T) * heff /
         sqrt(geff) / (gf * gf * s3);
}
...
static double weight(double y) {
  ...
  y_pass = y;
  {
    ...
    if (err) {
      do_err = do_err | err;
      printf("error in simpson omega.c line 195\n");
    }
  }
  if (inBuff < 1000) {
    weightBuff_x[inBuff] = y;
    weightBuff_y[inBuff++] = w;
  }
  return w;
}
...
static double s_integrand(double u) {
  ...
  if (ms >= sqrtS)
    return 0;
  md = M1 - M2;
  PcmIn = sqrt((sqrtS - ms) * (sqrtS + ms) * (sqrtS - md) * (sqrtS + md)) /
          (2 * sqrtS);
  sv_tot = sigma(PcmIn);
  res0 = sqrt(2 * y / M_PI) * y * (PcmIn * PcmIn / (Mcdm * Mcdm)) * sv_tot * 6 *
         (1 - u) * z * z;
  if (exi) {
    return res0 * weight(sqrtS / Mcdm);
  } else
    return res0 * K1pol(T_ / sqrtS) * sqrt(Mcdm / T_);
}
...
static vGridStr crossVGrids(vGridStr *grid1, vGridStr *grid2) {
  ...
  while (i1 <= grid1->n && i2 <= grid2->n) {
    ...
    int m = (grid1->pow[i1 - 1] > grid2->pow[i2 - 1] ? grid1->pow[i1 - 1]
                                                     : grid2->pow[i2 - 1]);

    if (grid1->v[i1] < grid2->v[i2]) {
      grid.v[n] = grid1->v[i1++];
    } else {
      grid.v[n] = grid2->v[i2];
      if (grid1->v[i1] == grid2->v[i2])
  ...
  grid.n = n - 1;
  for (i = 0; i < grid.n; i++)
    if (grid.v[i + 1] - grid.v[i] > 0.4 && grid.pow[i] < 4)
      grid.pow[i] = 4;
  return grid;
}
...
static int testSubprocesses(void) {
  ...
  if (first) {
    ...
    for (i = 0, j = 0; i < Nodd; i++) {
      ...
      for (int k = 0; k < nModelParticles; k++)
        if (strcmp(inP[j], ModelPrtcls[k].name) == 0 ||
            strcmp(inP[j], ModelPrtcls[k].aname) == 0) {
          oddPpos[j] = k;
          break;
        }
    ...
    for (k1 = 0; k1 < NC; k1++)
      for (k2 = 0; k2 < NC; k2++)
        if (inC0[k1 * NC + k2] == -1) {
          ...
          if (inC0[kk1 * NC + kk2] == -1) {
            inC0[kk1 * NC + kk2] = 0;
            inC0[k1 * NC + k2]++;
          }
          if (inC0[kk2 * NC + kk1] == -1) {
            inC0[kk2 * NC + kk1] = 0;
            inC0[k1 * NC + k2]++;
          }
        }
    ...
    for (i = 0, j = 0; i < Nodd; i++) {
      ...
      if (!inMassAddress[j]) {
        ...
        printf(" Model is not self-consistent:\n "
               " Mass identifier '%s' for particle '%s' is absent  among "
               "parameetrs\n",
    ...
    for (Qaddress = NULL, i = 0; i < nModelVars; i++)
      if (strcmp(varNames[i], "Q") == 0)
        Qaddress = varValues + i;
  }
  ...
  if (Nodd == 0) {
    printf("No odd particles in the model\n");
    return -1;
  }
  ...
  if (CDM1 && CDM2)
    for (i = 0; i < Nodd; i++)
      if (Z4ch(OddPrtcls[i].name) != Z4ch(OddPrtcls[i].aname)) {
        if (Mcdm1 > Mcdm2) {
          Mcdm1 = Mcdm2;
          CDM1 = CDM2;
        }
  ...
  if (CDM1) {
    for (i = 0; i < Nodd; i++)
      if (OddPrtcls[i].name == CDM1) {
        aCDM1 = OddPrtcls[i].aname;
        break;
      }
  } else
    aCDM1 = NULL;
  if (CDM2) {
    for (i = 0; i < Nodd; i++)
      if (OddPrtcls[i].name == CDM2) {
        aCDM2 = OddPrtcls[i].aname;
        break;
      }
  } else
    aCDM2 = NULL;
  ...
  for (k1 = 0; k1 < NC; k1++)
    for (k2 = 0; k2 < NC; k2++) {
      if (code22_0[k1 * NC + k2])
        code22_0[k1 * NC + k2]->init = 0;
      if (code22_1[k1 * NC + k2])
        code22_1[k1 * NC + k2]->init = 0;
      if (code22_2[k1 * NC + k2])
        code22_2[k1 * NC + k2]->init = 0;
    }
  cleanDecayTable();
...
int toFeebleList(char *name) {
  ...
  trim(name_);
  for (i = 0; i < nModelParticles; i++)
    if (strcmp(name_, ModelPrtcls[i].name) == 0 ||
        strcmp(name_, ModelPrtcls[i].aname) == 0) {
      feebleParticles[i] = 1;
      return 0;
    }
...
int sortOddParticles(char *lsp) {
  ...
  if (!modelNum) {
    ...
    L = strlen(WORK);
    modelDir = malloc(L + 15);
    sprintf(modelDir, "%s/models", WORK);
    modelNum = 1;

    calchepDir = malloc(strlen(rootDir) + 1);
    strcpy(calchepDir, rootDir);
    uname(&buff);
    ...
    libDir = malloc(L + 15);
    sprintf(libDir, "%s/so_generated", WORK);
  }
  ...
  if (err) {
    if (lsp) {
      if (err > 0) {
        ...
        printf("can not calculate parameter %s\n", varNames[err]);
      } else
        strcpy(lsp, "Nodd=0");
...
char *EvenParticles(void) {
  ...
  if (out)
    return out;
  for (i = 0, len = 0; i < nModelParticles; i++)
...
static int new_code(int k1, int k2, int ch) {
  ...
  switch (ch) {
  case 0:
    sprintf(process, "%s,%s->AllEven,1*x{%s", inP[k1], inP[k2],
            EvenParticles());
    break;
  case 1:
    sprintf(process, "%s,%s->AllOdd1,AllOdd1{%s", inP[k1], inP[k2],
            OddParticles(1));
    strcat(lib, "_1");
    break;
  case -1:
    sprintf(process, "%s,%s->AllOdd2,AllOdd2{%s", inP[k1], inP[k2],
            OddParticles(2));
    strcat(lib, "_2");
    break;
  case 2:
    sprintf(process, "%s,%s->AllOdd1,AllOdd2{%s{%s", inP[k1], inP[k2],
            OddParticles(1), OddParticles(2));
    strcat(lib, "_12");
    break;
  }
  ...
  if (cc) {
    ...
    *(cc->interface->twidth) = 1;
    switch (ch) {
    case 0:
      code22_0[k1 * NC + k2] = cc;
      break;
    case 1:
    case -1:
      code22_1[k1 * NC + k2] = cc;
      break;
    case 2:
      code22_2[k1 * NC + k2] = cc;
      break;
    }
    ...
    switch (ch) {
    case 0:
      code22Aux0[k1 * NC + k2] = prc;
      break;
    case 1:
    case -1:
      code22Aux1[k1 * NC + k2] = prc;
      break;
    case 2:
      code22Aux2[k1 * NC + k2] = prc;
      break;
    }
    for (n = 0; n <= nprc; n++) {
      prc[n].w[0] = 0;
...
double vs1120F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1120T));
}
double vs2200F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs2200T));
}
double vs1100F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1100T));
}
double vs1210F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1210T));
}
double vs1122F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1122T));
}
double vs2211F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs2211T));
}

double vs1110F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1110T));
}
double vs2220F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs2220T));
}
double vs1112F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1112T));
}
double vs1222F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1222T));
}
double vs1220F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1220T));
}
double vs2210F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs2210T));
}
double vs2221F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs2221T));
}
double vs1211F(double T) {
  return 3.8937966E8 * exp(polint3(T, Ntab, Ttab, vs1211T));
}
...
static double aRate(double X, int average, int Fast, double *alpha,
                    aChannel **wPrc, int *NPrc) {
  double Sum = 0.;
  double Sum1 = 0;
  int i, l1, l2;
  int nPrc = 0;
  ...
  int nPrcTot = 0;
  if (MassCutOut < Mcdm * (2 + 10 / X))
    MassCutOut = Mcdm * (2 + 10 / X);
  WIDTH_FOR_OMEGA = 1;
  ...
  for (l1 = 0; l1 < NC; l1++)
    if (!feebleParticles[oddPpos[sort[l1]]] &&
        Mcdm + inMass[sort[l1]] < MassCut)
      for (l2 = 0; l2 < NC; l2++)
        if (!feebleParticles[oddPpos[sort[l2]]] &&
            inMass[sort[l1]] + inMass[sort[l2]] < MassCut) {
          double Sumkk = 0.;
          double Sum1kk = 0;
          double x[2], f[2];
          double factor;
          ...
          if (!code22_0[k1 * NC + k2]->init) {
            ...
            if (passParameters(code22_0[k1 * NC + k2])) {
              FError = 1;
              WIDTH_FOR_OMEGA = 0;
              return -1;
            }
          ...
          if (wPrc) {
            ...
            *wPrc =
                (aChannel *)realloc(*wPrc, sizeof(aChannel) * (nPrcTot + 1));
          }
          ...
          Msmall =
              M1 > M2 ? M1 - Mcdm * (1 - sWidth) : M2 - Mcdm * (1 - sWidth);
          Mlarge =
              M1 > M2 ? M2 + Mcdm * (1 - sWidth) : M1 + Mcdm * (1 - sWidth);

          v_min = m2v(MassCutOut);
          if (v_min < 1E-200)
            v_min = 1E-200;

          factor = inC0[k1 * NC + k2] * inG[k1] * inG[k2] *
                   exp(-(M1 + M2 - 2 * Mcdm) / T_);
          ...
          for (nsub22 = 1; nsub22 <= CI->nprc; nsub22++, nPrc++) {
            ...
            double K = 0;
            for (i = 0; i < 4; i++)
              pname[i] = CI->pinf(nsub22, i + 1, pmass + i, pdg + i);
            if (wPrc) {
              (*wPrc)[nPrc].weight = 0;
              for (i = 0; i < 4; i++)
                (*wPrc)[nPrc].prtcl[i] = pname[i];
              (*wPrc)[nPrc].prtcl[4] = NULL;
            }

            if (pmass[0] < Mcdm / 2 || pmass[1] < Mcdm / 2)
              continue;
            ...
            if (VZdecay || VWdecay) {
              ...
              if (!AUX[nsub22].virt) {
                ...
                if (c_a) {
                  for (l = 2; l < 4; l++)
                    if ((pdg[l] == 23 && VZdecay > 1) ||
                        (abs(pdg[l]) == 24 && VWdecay > 1))
                      vd[l] = 1;
                } else
                  for (l = 2; l < 4; l++) {
                    if ((pdg[l] == 23 && VZdecay) ||
                        (abs(pdg[l]) == 24 && VWdecay))
                      vd[l] = 1;
                  }
                ...
                if (l < 4) {
                  ...
                  if (abs(pdg[l_]) > 16 && pmass[l_] > 2)
                    AUX[nsub22].w[l_ - 2] = pWidth(pname[l_], NULL);
                  if (AUX[nsub22].w[l_ - 2] < 0.1)
                    AUX[nsub22].w[l_ - 2] = 0;
                } else
                  AUX[nsub22].virt = -1;
              }
              ...
              if (AUX[nsub22].virt > 0) {
                l = AUX[nsub22].virt;
                l_ = 5 - l;
                if ((pmass[0] + pmass[1] < smin + 4 * AUX[nsub22].w[l - 2]) &&
                    (pmass[l_] < MassCutOut)) {
                  if (AUX[nsub22].cc23)
                    ...
                    int i3W;
                    AUX[nsub22].cc23 =
                        xVtoxll(2, 2, pname, pdg, l, &wV1, &brV1);
                    if (pdg[l] == pdg[l_])
                      brV1 *= 2;
                    AUX[nsub22].br = brV1;
                    cc23 = AUX[nsub22].cc23;
                    if (cc23) {
                      ...
                      *(cc23->interface->gswidth) = 1;
                      for (i3W = 2; i3W < 5; i3W++)
                        if (strcmp(
                                cc23->interface->pinf(1, i3W + 1, NULL, NULL),
                                pname[l_]) == 0)
                          break;
                      AUX[nsub22].i3 = i3W;
                      PcmMax = decayPcm(pmass[2] + pmass[3] +
                                            10 * AUX[nsub22].w[l - 2],
                                        pmass[0], pmass[1]);
                      if (pmass[0] + pmass[1] >= pmass[l_])
                        Pcm0 = Mcdm * 0.01;
                      else
                        Pcm0 = 1.01 * decayPcm(pmass[l_], pmass[0], pmass[1]);
                      buildInterpolation(sigma23, Pcm0, PcmMax, 0.02, 1.E-5,
                                         &(AUX[nsub22].nTab),
                                         &(AUX[nsub22].pcmTab),
                                         &(AUX[nsub22].csTab));
                      /*
                         ...
                         loadINTER(AUX[nsub22].nTab,
                         AUX[nsub22].pcmTab,AUX[nsub22].csTab);
            ...
            *(CI->gswidth) = 0;
            ...
            if (smin > pmass[0] + pmass[1]) {
              if ((pmass[0] != M1 || pmass[1] != M2) &&
                  (pmass[0] != M2 || pmass[1] != M1)) {
                double ms = pmass[0] + pmass[1];
                double md = pmass[0] - pmass[1];
                double Pcm = sqrt((smin - ms) * (smin + ms) * (smin - md) *
                                  (smin + md)) /
                             (2 * smin);
            ...
            if (pmass[0] + pmass[1] > smin)
              smin = pmass[0] + pmass[1];
            v_max = m2v(smin);
          ...
          repeat:
            ...
            if (Fast == 0) {
              ...
              if (err) {
                do_err = do_err | err;
                printf("error in simpson omega.c line 1004\n");
              }
            } else {
              int isPole = 0;
              char *s;
              ...
              for (n = 1; (s = code22_0[k1 * NC + k2]->interface->den_info(
                               nsub22, n, &m, &w, NULL));
                   n++)
                if (m && w && strcmp(s, "\1\2") == 0) {
                  ...
                  if (mass < MassCutOut &&
                      mass + 8 * width > pmass[0] + pmass[1] &&
                      mass + 8 * width > smin) {
                    if ((pmass[0] != M1 || pmass[1] != M2) &&
                        (pmass[0] != M2 || pmass[1] != M1)) {
                      double ms = pmass[0] + pmass[1];
                      double md = pmass[0] - pmass[1];
                      double Pcm = sqrt((mass - ms) * (mass + ms) *
                                        (mass - md) * (mass + md)) /
                                   (2 * mass);
                      mass =
                          sqrt(M1 * M1 + Pcm * Pcm) + sqrt(M2 * M2 + Pcm * Pcm);
                    }
                    vgrid1 = makeVGrid(mass, width);
                    if (isPole)
                      vgrid = crossVGrids(&vgrid, &vgrid1);
                    else
                      vgrid = vgrid1;
                    isPole++;
                  }
              ...
              if (cc23) {
                ...
                if (mass - width > M1 + M2) { // double u=m2u(mass-1*width);
                  /*                if(u<0.96 && u>0.4)
                                  {
                                    grid1.n=2;
                                    grid1.ul[0]=0;
                                    ...
                                    grid1.pow[0]=3;
                                    grid1.ur[1]=1;
                                    if(u<0.6) grid1.pow[1]=5; else
                     grid1.pow[1]=3; if(isPole) grid=crossGrids(&grid,&grid1);
                     else grid=grid1; isPole++;
                                  }
                  */
                }
              }
              ...
              // for(i=0;i<grid.n;i++) printf(" (%E %E)
              // ",grid.ul[i],grid.ur[i]); printf("\n");
              ...
              */
              for (i = 0; i < vgrid.n; i++) {
                double da;
                if (Fast > 0)
                  da = gauss(s_integrand, vgrid.v[i], vgrid.v[i + 1],
                             vgrid.pow[i]);
                else {
                  int err;
                  da = simpson(s_integrand, vgrid.v[i], vgrid.v[i + 1], eps,
                               &err);
                  if (err) {
                    do_err = do_err || err;
                    printf("error in simpson omega.c line 1066\n");
                  }
                }
                a += da;
              }
            ...
            // pname[0],pname[1],pname[2],pname[3],
            // pMass(pname[0]),M1,pMass(pname[1]),M2);

            for (kk = 2; kk < 4; kk++)
              if (pmass[kk] > 2 * Mcdm && pname[kk][0] != '~') {
                txtList LL;
                double BrSm = 1;
                pWidth(pname[kk], &LL);
                for (; LL; LL = LL->next) {
            ...
            if (pname[2][0] == '~' || pname[3][0] == '~') {
              a /= 2;
              Sum1kk += a;
            }
            Sumkk += a;
            if (wPrc)
              (*wPrc)[nPrc].weight = a * factor;
          }
          Sum += factor * Sumkk;
          Sum1 += factor * Sum1kk;
  ...
  if (wPrc) {
    //    for(i=0; i<nPrc;i++) printf("wPrc  %s %s -> %s %s
    ...
    for (i = 0; i < nPrc; i++)
      (*wPrc)[i].weight /= Sum;
    for (i = 0; i < nPrc - 1;) {
      if ((*wPrc)[i].weight >= (*wPrc)[i + 1].weight)
        i++;
      else {
        aChannel buff;
        buff = (*wPrc)[i + 1];
...
double Yeq(double T) {
  ...
  double X = Mcdm / T;
  heff = hEff(T);
  return (45 / (4 * M_PI * M_PI * M_PI * M_PI)) * X * X * geffDM(T) *
         sqrt(M_PI / (2 * X)) * exp(-X) / heff;
  //   double res=
  //   (45/(4*M_PI*M_PI*M_PI*M_PI))*X*X*geffDM(T)*sqrt(M_PI/(2*X))*exp(-X)/heff;

  // printf("Yeq(%E) X=%E heff=%E MassCut=%E  geffDM=%E res=%e \n",T,X, heff,
  // MassCut, geffDM(T),res);
...
struct {
  ...
  double xtop;
  int pow, size;
} vSigmaGrid = {NULL, NULL, 0, 0, 0};

static void checkSgridUpdate(void) {
  if (vSigmaGrid.pow == vSigmaGrid.size) {
    vSigmaGrid.size += 20;
    vSigmaGrid.data =
        (double *)realloc(vSigmaGrid.data, sizeof(double) * vSigmaGrid.size);
    vSigmaGrid.alpha =
        (double *)realloc(vSigmaGrid.alpha, sizeof(double) * vSigmaGrid.size);
  }
}
...
static double vSigmaI(double T, double Beps, int fast, double *alpha_) {
  ...
  while (n + 2 > vSigmaGrid.pow - 1) {
    ...
    checkSgridUpdate();
    MassCut = Mcdm * (2 - log(Beps) / XX);
    vSigmaGrid.data[vSigmaGrid.pow] =
        aRate(XX, 0, fast, &alpha, NULL, NULL) + ZeroCS;
    vSigmaGrid.alpha[vSigmaGrid.pow] = alpha;
    vSigmaGrid.pow++;
  }

  {
    double X0, X1, X2, X3, sigmav0, sigmav1, sigmav2, sigmav3, alpha0, alpha1,
        alpha2, alpha3;
    i = log(X / vSigmaGrid.xtop) / log(XSTEP);
    if (i < 0)
      i = 0;
    if (i > vSigmaGrid.pow - 2)
      i = vSigmaGrid.pow - 2;
    X0 = vSigmaGrid.xtop * pow(XSTEP, n - 1);
    X1 = X0 * XSTEP;
    X2 = X1 * XSTEP;
    X3 = X2 * XSTEP;

    sigmav0 = log(vSigmaGrid.data[n - 1]);
    alpha0 = vSigmaGrid.alpha[n - 1];
    sigmav1 = log(vSigmaGrid.data[n]);
    alpha1 = vSigmaGrid.alpha[n];
    sigmav2 = log(vSigmaGrid.data[n + 1]);
    alpha2 = vSigmaGrid.alpha[n + 1];
    sigmav3 = log(vSigmaGrid.data[n + 2]);
    alpha3 = vSigmaGrid.alpha[n + 2];
    X = log(X);
    X0 = log(X0);
    X1 = log(X1);
    X2 = log(X2);
    ...
    if (alpha_) {
      if (alpha1 == 0)
        ...
        *alpha_ = alpha0 * (X - X1) * (X - X2) * (X - X3) / (X0 - X1) /
                      (X0 - X2) / (X0 - X3) +
                  alpha1 * (X - X0) * (X - X2) * (X - X3) / (X1 - X0) /
                      (X1 - X2) / (X1 - X3) +
                  alpha2 * (X - X0) * (X - X1) * (X - X3) / (X2 - X0) /
                      (X2 - X1) / (X2 - X3) +
                  alpha3 * (X - X0) * (X - X1) * (X - X2) / (X3 - X0) /
                      (X3 - X1) / (X3 - X2);
        if (*alpha_ < 0)
          *alpha_ = 0;
      }
    }
    double res = exp(sigmav0 * (X - X1) * (X - X2) * (X - X3) / (X0 - X1) /
                         (X0 - X2) / (X0 - X3) +
                     sigmav1 * (X - X0) * (X - X2) * (X - X3) / (X1 - X0) /
                         (X1 - X2) / (X1 - X3) +
                     sigmav2 * (X - X0) * (X - X1) * (X - X3) / (X2 - X0) /
                         (X2 - X1) / (X2 - X3) +
                     sigmav3 * (X - X0) * (X - X1) * (X - X2) / (X3 - X0) /
                         (X3 - X1) / (X3 - X2)) -
                 ZeroCS;
    if (res < 0)
      res = 0;
...
static double dY(double s3, double Beps, double fast) {
  ...
  d = 0.001 * s3;
  dlnYds3 = (log(Yeq(T_s3(s3 + d))) - log(Yeq(T_s3(s3 - d)))) / (2 * d); // ???
  ...
  res = dlnYds3 /
        (pow(2 * M_PI * M_PI / 45. * heff, 0.66666666) /
         sqrt(8 * M_PI / 3. * M_PI * M_PI / 30. * geff) * vSig * MPlank *
         (1 - alpha / 2) * sqrt(1 + epsY * epsY)) /
        Yeq(T);
  res = fabs(res);
  if (res > 10)
    return 10;
...
static double darkOmega1(double *Xf, double Z1, double dZ1, int Fast,
                         double Beps) {
  double X = *Xf;
  ...
  for (;;) {
    ...
    if (fabs(dCC1) < dCCX) {
      *Xf = X1;
      MassCut = Mcdm * (2 - log(Beps) / X1);
      return Yeq(Mcdm / X1) * sqrt(1 + CCX + dCC1);
    }
    if (fabs(dCC2) < dCCX || fabs(X1 - X2) < 0.0001 * X1) {
      *Xf = X2;
      MassCut = Mcdm * (2 - log(Beps) / X2);
      return Yeq(Mcdm / X2) * sqrt(1 + CCX + dCC2);
    }
    X = 0.5 * (X1 + X2);
...
static void XderivLn(double s3, double *Y, double *dYdx) {
  ...
  //  if(y<yeq) *dYdx=0; else
  {
    ...
    // printf("T=%E alpha=%E\n", Mcdm/x, alpha);
    *dYdx = MPlank * pow(2 * M_PI * M_PI / 45. * heff, 0.666666666666) /
            sqrt(8 * M_PI / 3. * M_PI * M_PI / 30. * geff)
            //    *sqrt_gStar*sqrt(M_PI/45)
            * vSig * (y * y - (1 - alpha) * yeq * yeq - alpha * y * yeq) *
            sqrt(1 + epsY * epsY);
...
double darkOmegaFO(double *Xf_, int Fast, double Beps) {
  ...
  double iColl = ((Mcdm / Xf) * sqrt(M_PI / 45) * MPlank *
                  aRate(Xf, 1, Fast, NULL, NULL, NULL));
  ...
  if (FError)
    return -1;
  return 2.742E8 * Mcdm /
         (1 / Yf + iColl); /* 2.828-old 2.755-new 2.742 next-new */
}
...
double darkOmega(double *Xf, int Fast, double Beps, int *err) {
  ...
  Mcdm = -1;
  for (i = 0; i < NC; i++)
    if (!feebleParticles[oddPpos[i]] && (Mcdm < 0 || Mcdm > inMass[i]))
      Mcdm = inMass[i];
  ...
  if (Yt < 0 || FError) {
    ...
    if (err)
      *err = 1;
      ...
      printf("Temperature of thermal  equilibrium is too large\n");
    return -1;
  }
  ...
  for (i = 0;; i++) {
    ...
    if (Yt * Yt >= Z2 * Z2 * (alpha * Yt * yeq + (1 - alpha) * yeq * yeq) ||
        Yt < fabs(deltaY * 1E-15))
      break;

    s3_t = s3_T(Mcdm / Xt);
    s3_2 = s3_T(Mcdm / X2);
    //    if(odeint(&y,1 ,Mcdm/Xt , Mcdm/X2 , 1.E-3, (Mcdm/Xt-Mcdm/X2 )/2,
    //    &XderivLn)){ printf("problem in solving diff.equation\n"); return -1;}
    ...
    for (i = 1; i < Ntab; i++) {
      ...
      if (dY2 < 0) {
        for (;;) {
          ...
          al = dY2 / (dY2 - dY1);
          Tx = al * T1 + (1 - al) * T2,
          /*Yx=al*Y1+(1-al)*Y2,*/ Yx = polint3(Tx, Ntab, Ttab, Ytab),
          dYx = Zf * Yeq(Tx) - Yx;
          if (fabs(dYx) < 0.01 * Yx) {
            *Xf = Mcdm / Tx;
            break;
  ...
  double iColl = ((Mcdm / Xt) * sqrt(M_PI / 45) * MPlank *
                  aRate(Xt, 1, Fast, NULL, NULL, NULL));
  Mcdm = Mcdm_mem;
  if (FError) {
    if (err)
      *err = 8;
  ...
  if (deltaY == 0) {
    dmAsymm = 0;
    return 2.742E8 * Mcdm /
           (1 / Yt + iColl); /* 2.828-old 2.755-new,2.742 -newnew */
  } else {
    double a, f, z0, Y0;
    a = fabs(deltaY);
    if (Yt < a * 1.E-5)
      ...
      f = (sqrt(Yt * Yt + a * a) - a) / (sqrt(Yt * Yt + a * a) + a);
    f *= exp(-2 * a * iColl);
    z0 = sqrt(f) * 2 * a / (1 - f);
    Y0 = sqrt(z0 * z0 + a * a);
    dmAsymm = deltaY / Y0;
...
double printChannels(double Xf, double cut, double Beps, int prcn, FILE *f) {
  ...
  Sum = aRate(Xf, 1, 1, NULL, &omegaCh, &nPrc) * (Mcdm / Xf) * sqrt(M_PI / 45) *
        MPlank / (2.742E8 * Mcdm_mem);
  if (Sum == 0 || FError) {
    return -1;
  }
  if (nform < 0)
  ...
  if (f) {
    ...
    fprintf(f, "# Channels which contribute to 1/(omega) more than %G%%.\n",
            100 * cut);
    if (prcn)
      fprintf(f, "# Relative contributions in %% are displayed\n");
    else
      fprintf(f, "# Absolute contributions are displayed\n");
    for (i = 0, s = 0; i < nPrc; i++)
      if (fabs(omegaCh[i].weight) >= cut) {
        s += omegaCh[i].weight;
        if (prcn) {
          if (cut < 0.000001)
            fprintf(f, "  %.1E%% ", 100 * omegaCh[i].weight);
          else
            fprintf(f, "  %*.*f%% ", nform + 3, nform, 100 * omegaCh[i].weight);
        } else
          fprintf(f, "  %.1E ", Sum * omegaCh[i].weight);
        for (j = 0; j < 4; j++) {
...
static int strcmp_(char *n1, char *n2) {
  if (n1[0] == '*' && n1[1] == 0)
    return 0;
  return strcmp(n1, n2);
}

double oneChannel(double Xf, double Beps, char *n1, char *n2, char *n3,
                  char *n4) {
  int j, nPrc;
  aChannel *wPrc;
  double Sum, res;
  ...
  MassCut = Mcdm * (2 - log(Beps) / Xf);
  Sum = aRate(Xf, 1, 1, NULL, &wPrc, &nPrc) * (Mcdm / Xf) * sqrt(M_PI / 45) *
        MPlank / (2.742E8 * Mcdm);
  if (FError) {
    return -1;
  }
  ...
  for (res = 0, j = 0; j < nPrc; j++)
    if (((strcmp_(n1, wPrc[j].prtcl[0]) == 0 &&
          strcmp_(n2, wPrc[j].prtcl[1]) == 0) ||
         (strcmp_(n2, wPrc[j].prtcl[0]) == 0 &&
          strcmp_(n1, wPrc[j].prtcl[1]) == 0)) &&
        ((strcmp_(n3, wPrc[j].prtcl[2]) == 0 &&
          strcmp_(n4, wPrc[j].prtcl[3]) == 0) ||
         (strcmp_(n4, wPrc[j].prtcl[2]) == 0 &&
          strcmp_(n3, wPrc[j].prtcl[3]) == 0))) {
      res += wPrc[j].weight;
    }
...
static void XderivLnExt(double s3, double *Y, double *dYdx) {
  ...
  if (vSig == 0) {
    *dYdx = 0;
    ...
    alpha = vSig1 / (vSig0 + vSig1);

  heff = hEff(T);
  geff = gEff(T);

  *dYdx = MPlank * pow(2 * M_PI * M_PI / 45. * heff, 0.666666666666) /
          sqrt(8 * M_PI / 3. * M_PI * M_PI / 30. * geff) * vSig *
          (y * y - (1 - alpha) * yeq * yeq - alpha * y * yeq) *
          sqrt(1 + epsY * epsY);
}
...
double darkOmegaExt(double *Xf, double (*f0)(double), double (*f1)(double)) {
  ...
  if (f0)
    ...
    vSigmaStat0 = vSigmaZero;
  if (f1)
    vSigmaStat1 = f1;
  else
    vSigmaStat1 = vSigmaZero;
  ...
  for (i = 0;; i++) {
    ...
    if (Tend < 1.E-3 || Yt < fabs(deltaY * 1E-5))
      break;
    Tbeg = Tend;
    Tend /= 1.2;
    s3_t = s3_T(Tbeg);
    s3_2 = s3_T(Tend);
    if (odeint(&Yt, 1, s3_t, s3_2, 1.E-3, (s3_2 - s3_t) / 2, &XderivLnExt)) {
      printf("problem in solving diff.equation\n");
      return -1;
    }
    if (!isfinite(Yt) || FError)
      return -1;
    if (Ntab >= Nt) {
      Nt += 20;
      Ytab = realloc(Ytab, sizeof(double) * Nt);
  ...
  for (l = 0; l < NC; l++)
    if (!feebleParticles[oddPpos[sort[l]]]) {
      ...
      if (Z4ch(inP[k]) == 1) {
        ...
        t = T / M;
        if (t < 0.1)
          bsk2 = K2pol(t) * exp((Mcdm1 - M) / T) * sqrt(M_PI * t / 2);
        else
          bsk2 = bessK2(1 / t) * exp(Mcdm1 / T);
        sum += inG[k] * M * M * bsk2;
      }
    }
  return sum;
  ...
  for (l = 0; l < NC; l++)
    if (!feebleParticles[oddPpos[sort[l]]]) {
      ...
      if (Z4ch(inP[k]) == 2) {
        ...
        t = T / M;
        if (t < 0.1)
          bsk2 = K2pol(t) * exp(-1 / t + Mcdm2 / T) * sqrt(M_PI * t / 2);
        else
          bsk2 = bessK2(1 / t) * exp(Mcdm2 / T);
        sum += inG[k] * M * M * bsk2;
      }
    }
  return sum;
...
static double s_integrandT(double sqrtS) {
  ...
  t = T_ / sqrtS;
  if (t < 0.1)
    bsk1 = K1pol(t) * exp(-1 / t + McdmSum / T_) * sqrt(M_PI * t / 2);
  else
    bsk1 = bessK1(sqrtS / T_) * exp(McdmSum / T_);
...
static int aRate4(double T, double *vs1100_, double *vs2200_, double *vs1110_,
                  double *vs1120_, double *vs1210_, double *vs1220_,
                  double *vs2210_, double *vs2220_, double *vs1112_,
                  double *vs1122_, double *vs1222_, double *vs1211_,
                  double *vs2211_, double *vs2221_) {
  int i, l1, l2, N12;
  char *pname[5];
  double X = Mcdm / T;

  double Msmall, Mlarge;
  double vs1100 = 0, vs2200 = 0, vs1110 = 0, vs2220 = 0, vs1120 = 0, vs1210 = 0,
         vs1122 = 0, vs2211 = 0, vs1112 = 0, vs1222 = 0, vs1220 = 0, vs2210 = 0,
         vs2221 = 0, vs1211 = 0;
      ...
      if (!feebleParticles[oddPpos[sort[l1]]]) {
        ...
        for (l2 = 0; l2 < NC; l2++)
          if (!feebleParticles[oddPpos[sort[l2]]]) {
            ...
            case 1:
              if (Mcdm1 > Mcdm2) {
                if (Z4ch(inP[k1]) == 2 && Z4ch(inP[k2]) == 2)
                  continue;
                if (code22_1[k1 * NC + k2] == NULL)
                  new_code(k1, k2, -1);
                code = code22_1[k1 * NC + k2];
              } else {
                if (Z4ch(inP[k1]) == 1 && Z4ch(inP[k2]) == 1)
                  continue;
                if (code22_1[k1 * NC + k2] == NULL)
                  new_code(k1, k2, 1);
                code = code22_1[k1 * NC + k2];
            ...
            case 2:
              if (Mcdm1 > Mcdm2) {
                if (Z4ch(inP[k1]) == 1 && Z4ch(inP[k2]) == 1) {
                  if (code22_2[k1 * NC + k2] == NULL)
                    new_code(k1, k2, 2);
                  code = code22_2[k1 * NC + k2];
                } else
                  continue;
              } else {
                if (Z4ch(inP[k1]) == 2 && Z4ch(inP[k2]) == 2) {
            ...
            if (!code->init) {
              ...
              for (i = 1; i <= cdi->nvar; i++)
                if (code->link[i])
                  cdi->va[i] = *(code->link[i]);
              if (cdi->calcFunc() > 0) {
                FError = 1;
                WIDTH_FOR_OMEGA = 0;
                return -1;
            ...
            case 0:
              ...
              break;
            case 1:
              AUX = code22Aux1[k1 * NC + k2];
              break;
            case 2:
              AUX = code22Aux2[k1 * NC + k2];
              break;
            }
            // printf("N12=%d   AUX=%p\n",N12,AUX);
            ...
            for (nsub22 = 1; nsub22 <= CI->nprc; nsub22++) {
              ...
              if (pmass[0] == 0)
                continue; // for the case of absence of process
              for (i = 0; i < 4; i++)
                z4[i] = Z4ch(pname[i]);
              smin = pmass[2] + pmass[3];
              cc23 = NULL;
                      ...
                      if (cc23) {
                        ...
                        *(cc23->interface->gswidth) = 0;
                        ...
                        if (pmass[0] + pmass[1] >= pmass[l_])
                          ...
                          Pcm0 = 1.01 * decayPcm(pmass[l_], pmass[0], pmass[1]);
                        buildInterpolation(sigma23, Pcm0, PcmMax, 0.02, 1E-5,
                                           &(AUX[nsub22].nTab),
                                           &(AUX[nsub22].pcmTab),
                                           &(AUX[nsub22].csTab));
                        /*
            ...
            repeat:
              ...
              if (Fast_ == 0) {
                int err;
                a = simpson(v_integrand, v_min, v_max, eps, &err);
                if (err) {
                  do_err = do_err | err;
                  printf("error in simpson omega.c line 2093\n");
                }
              } else {
                int isPole = 0;
                char *s;
                ...
                for (n = 1;
                     (s = code->interface->den_info(nsub22, n, &m, &w, NULL));
                     n++)
                  if (m && w && strcmp(s, "\1\2") == 0) {
                    ...
                    if (mass < MassCutOut &&
                      ...
                      if ((pmass[0] != M1 || pmass[1] != M2) &&
                        ...
                        double Pcm = sqrt((mass - ms) * (mass + ms) *
                                          (mass - md) * (mass + md)) /
                                     (2 * mass);
                        mass = sqrt(M1 * M1 + Pcm * Pcm) +
                               sqrt(M2 * M2 + Pcm * Pcm);
                      }
                      if (Fast_ == 1)
                        vgrid1 = makeVGrid(mass, width);
                      else
                        vgrid1 = makeVGrid2(mass, width);
                      if (isPole)
                        vgrid = crossVGrids(&vgrid, &vgrid1);
                      else
                        vgrid = vgrid1;
                ...
                for (i = 0; i < vgrid.n; i++) {
                  if (Fast_ <= 0) {
                    int err;
                    da = simpson(v_integrand, vgrid.v[i], vgrid.v[i + 1], eps,
                                 &err);
                    if (err) {
                      do_err = do_err | err;
                      printf("error in simpson omega.c line 2142\n");
                    }
                  } else
                    da = gauss(v_integrand, vgrid.v[i], vgrid.v[i + 1],
                               vgrid.pow[i]);
                  a += da;
                }
              }
              ...
              {
                double br[2][5] = {0};
                // br[i][0]-SM, br[i][1]- cdm1, br[i][2]-cmd2, bri[i][3]-2*cdm1,
                // br[i][4]-2*cdm2
                int l1, l2, kk;
                for (kk = 2; kk < 4; kk++) {
                  br[kk - 2][z4[kk]] = 1;
                  if (pmass[kk] > 2 * Mcdm && z4[kk] != 1 &&
                      abs(pdg[kk]) != 24 && pdg[kk] != 23) {
                    txtList LL;
                    pWidth(pname[kk], &LL);
                    for (; LL; LL = LL->next) {
                      double b;
                      ...
                      } else if (strstr(proc, "~~") == 0 && strstr(proc, "~")) {
                        br[kk - 2][2] -= b;
                        br[kk - 2][3] += b;
                      }
                    }
                ...
                for (l1 = 0; l1 < 5; l1++)
                  for (l2 = 0; l2 < 5; l2++) {
                    ...
                    if (b) {
                      if (l1 < 3 && l2 < 3)
                        NN = z4[0] + 3 * z4[1] + 9 * l1 + 27 * l2;
                      else if ((l1 == 0 && l2 == 3) || (l2 == 0 && l1 == 3))
                        NN = z4[0] + 3 * z4[1] + 9 * 1 + 27 * 1;
                      else if ((l1 == 0 && l2 == 4) || (l2 == 0 && l1 == 4))
                        NN = z4[0] + 3 * z4[1] + 9 * 2 + 27 * 2;
                      else if ((l1 == 1 && l2 == 3) || (l2 == 1 && l1 == 3))
                        NN = 0; //  111
                      else if ((l1 == 1 && l2 == 4) || (l2 == 1 && l1 == 4))
                        NN = 0; //  122
                      else if ((l1 == 2 && l2 == 3) || (l2 == 2 && l1 == 3))
                        NN = 0; //  211
                      else if ((l1 == 2 && l2 == 4) || (l2 == 2 && l1 == 4))
                        NN = 0; //  222
                      else if ((l1 == 3 && l2 == 3))
                        NN = 0; //  1111
                      else if ((l1 == 3 && l2 == 4) || (l2 == 3 && l1 == 4))
                        NN = 0; //  1122
                      else if ((l1 == 4 && l2 == 4))
                        NN = 0; //  2222
                      ...
                      case 2 + 3 + 9 + 27:
                        ...
                        //          default:    printf("unexpected type of
                        //          process: %d %d => %d
                        //          %d\n",z4[0],z4[1],z4[2],z4[3]);
                      }
                    }
  ...
  }
  {
    ...
    *vs2210_ = vs2210 / (g2 * g2);
    ...
    if (CDM1 && CDM2) {
      ...
      *vs1122_ = *vs2211_ = *vs1112_ = *vs1211_ = *vs1222_ = *vs2221_ = 0;
  }
  WIDTH_FOR_OMEGA = 0;
...
static int DMEQ0(double T, double vs1100, double vs2200, double vs1110,
                 double vs1120, double vs1210, double vs1220, double vs2210,
                 double vs2220, double vs1112, double vs1122, double vs1222,
                 double vs1211, double vs2211, double vs2221, double *C,
                 double *L, double *Q) {
  ...
  /*
    if(Mcdm1 >Mcdm2)z1122= vs1122*(y1*y1-y2*y2*y1_y2_Q(T)); else
    z1122=-vs2211*(y2*y2-y1*y1*y2_y1_Q(T)); dYdT[0]=vs1100*(y1*y1-y1_*y1_)+
    vs1120*(y1*y1-y2*y1q_y2)
    + z1122; dYdT[1]=vs2200*(y2*y2-y2_*y2_)- 0.5*vs1120*(y1*y1-y2*y1q_y2) -
    z1122 + 0.5*vs1210*y1*(y2-y2_) ;
  */

  L[0] = y1 * (2 * (vs1100 + vs1120 + vs1122) + 0.5 * (vs1110 + vs1112)) +
         0.5 * y2 * (vs1220 + vs1222) + 0.5 * y2_ * vs2210; // A1_1
  if (!isfinite(L[0])) {
    printf("y1=%e y2=%e y2_=%e\n", y1, y2, y2_);
  }
  L[3] = y2 * (2 * (vs2200 + vs2210 + vs2211) + 0.5 * (vs2220 + vs2221)) +
         0.5 * y1 * (vs1210 + vs1211) + 0.5 * y1_ * vs1120; // A2_2

  L[1] = -0.5 * y1 * vs1222 - y2 * 2 * vs2211 - 0.5 * y1 * vs1211 -
         y1_ * vs1120 - y2 * vs2210; // A1_2
  // printf(" y1*y1*vs1122/(y2*y2*vs2211)=%E  y2*2*vs2211=%E
  // y1_*vs1120=%E\n",y1*y1*vs1122/(y2*y2*vs2211)  ,    y2*2*vs2211,y1_*vs1120);
  L[2] = -0.5 * y2 * vs1211 - y1 * 2 * vs1122 - 0.5 * y2 * vs1222 -
         y2_ * vs2210 - y1 * vs1120; // A2_1

  // printf("L[2]=%e = %e + %e  vs2210=%E   \n", L[2], - y1_*2*vs1122,
  // -vs2210*y2q_y1, vs2210);
  ...
  if (CDM1)
    C[0] = (Yeq1(T * 1.01) - Yeq1(T / 1.01)) / (2 * log(1.01) * T);
  else
    C[0] = 0;
  if (CDM2)
    C[1] = (Yeq2(T * 1.01) - Yeq2(T / 1.01)) / (2 * log(1.01) * T);
  else
    C[1] = 0;
...
static int DMEQ(double T, double *C, double *L, double *Q) {
  double vs1100, vs2200, vs1110, vs2220, vs1120, vs1122, vs1210, vs2211, vs1112,
      vs1222, vs1220, vs2210, vs2221, vs1211;
  aRate4(T, &vs1100, &vs2200, &vs1110, &vs1120, &vs1210, &vs1220, &vs2210,
         &vs2220, &vs1112, &vs1122, &vs1222, &vs1211, &vs2211, &vs2221);
  return DMEQ0(T, vs1100, vs2200, vs1110, vs1120, vs1210, vs1220, vs2210,
               vs2220, vs1112, vs1122, vs1222, vs1211, vs2211, vs2221, C, L, Q);
}
...
static int dYstart(double T, double *dy, double *Lmin, double *Lmax) {
  ...
  if (dy) {
    ...
    //    C[0],C[1],L[0],L[1],L[2],L[3], dy[0],dy[1]);
  }
  {
    double s, d;
    s = 0.5 * (L[0] + L[3]);
...
static void TderivZ4(double T, double *Y, double *dYdT) {
  ...
  dYdT[0] = -C[0] + L[0] * dy1 + L[1] * dy2 + Q[0] * dy1 * dy1 +
            Q[1] * dy1 * dy2 + Q[2] * dy2 * dy2;
  dYdT[1] = -C[1] + L[2] * dy1 + L[3] * dy2 + Q[3] * dy1 * dy1 +
            Q[4] * dy1 * dy2 + Q[5] * dy2 * dy2;
}

static void stiffDerives(double T, double *Y, double *f, double h, double *dfdx,
                         double *dfdy) {
  double C[2], L[4], Q[6], dy1, dy2;
  double dT = -0.001 * T;
  int n = 2;
  ...
  f[0] = -C[0] + L[0] * dy1 + L[1] * dy2 + Q[0] * dy1 * dy1 + Q[1] * dy1 * dy2 +
         Q[2] * dy2 * dy2;
  f[1] = -C[1] + L[2] * dy1 + L[3] * dy2 + Q[3] * dy1 * dy1 + Q[4] * dy1 * dy2 +
         Q[5] * dy2 * dy2;
  ...
  if (dfdx) {
    DMEQ(T + dT, C, L, Q);
    dfdx[0] = -C[0] + L[0] * dy1 + L[1] * dy2 + Q[0] * dy1 * dy1 +
              Q[1] * dy1 * dy2 + Q[2] * dy2 * dy2;
    dfdx[1] = -C[1] + L[2] * dy1 + L[3] * dy2 + Q[3] * dy1 * dy1 +
              Q[4] * dy1 * dy2 + Q[5] * dy2 * dy2;
    dfdx[0] -= f[0];
    dfdx[0] /= dT;
    dfdx[1] -= f[1];
...
static void TabDmEq(double step, int show) {
  ...
  double *vs[14] = {vs1100T, vs1120T, vs1122T, vs1210T, vs2200T,
                    vs1110T, vs2220T, vs2211T, vs1112T, vs1222T,
                    vs1220T, vs2210T, vs2221T, vs1211T};

  for (T = Tstart, i = 0; i < N; i++) {
    Ttab[i] = T;
    Y1T[i] = 0;
    Y2T[i] = 0;
    aRate4(T, vs1100T + i, vs2200T + i, vs1110T + i, vs1120T + i, vs1210T + i,
           vs1220T + i, vs2210T + i, vs2220T + i, vs1112T + i, vs1122T + i,
           vs1222T + i, vs1211T + i, vs2211T + i, vs2221T + i);
    //      { double dT=T/100;
    //        double heff_=0.5*(hEff(T+dT)-hEff(T-dT))/dT;
  ...
  if (show) {
    if (CDM1 && CDM2) {
      ...
      if (Mcdm1 < Mcdm2)
        displayFunc("vs2211", vs2211F, Tend, Tstart, "T", 1);
      else
        displayFunc("vs1122", vs1122F, Tend, Tstart, "T", 1);
    }
    if (CDM2)
      displayFunc("vs2200", vs2200F, Tend, Tstart, "T", 1);
    if (CDM1)
      displayFunc("vs1100", vs1100F, Tend, Tstart, "T", 1);
  }
}
...
static int DMEQtab(double T, double *C, double *L, double *Q) {
  ...
  // printf("vs1100=%E,vs1120=%E, vs1122=%E, vs1210=%E, vs2200=%E,
...
static void TderivZ4tab2(double T, double *Y, double *dYdT) {
  ...
  dYdT[0] = -C[1] + L[2] * dy1 + L[3] * dy2 + Q[3] * dy1 * dy1 +
            Q[4] * dy1 * dy2 + Q[5] * dy2 * dy2;
}
...
double darkOmega2(double fast, double Beps0) {
  ...
  char *CDM1_mem = CDM1, *CDM2_mem = CDM2, *aCDM1_mem = aCDM1,
       *aCDM2_mem = aCDM2;
  ...
  for (i = 0; i < NC; i++)
    if (Z4ch(inP[i]) == 1 &&
        (!feebleParticles[oddPpos[i]] && (Mcdm1 == 0 || Mcdm1 > inMass[i]))) {
      Mcdm1 = inMass[i];
      if (Mcdm1 > 0) {
        CDM1 = inP[i];
  ...
  for (i = 0; i < NC; i++)
    if (Z4ch(inP[i]) == 2 &&
        (!feebleParticles[oddPpos[i]] && (Mcdm2 == 0 || Mcdm2 > inMass[i]))) {
      Mcdm2 = inMass[i];
      if (Mcdm2 > 0) {
        CDM2 = inP[i];
  ...
  if (!CDM1) {
    while (isfinite(Y[1]) && fabs(Y[1]) < 0.005 * Yeq2(Tstart)) {
      Tstart /= 1.05;
      dYstart(Tstart, Y, &Lmin, &Lmax);
    }
    ...
    while (isfinite(Y[0]) && fabs(Y[0]) < 0.005 * Yeq1(Tstart)) {
      Tstart /= 1.05;
      dYstart(Tstart, Y, &Lmin, &Lmax);
    }
  ...
  if (!CDM1) {
    for (i = 1, err = 0, T = Tstart; T > Tend && !err; i++) {
      ...
      if (T2 < Tend)
        T2 = Tend;
      if (info)
        printf(" CMD2 : T1=%.2E Y=%.2E ", T, Y[1]);
      err = odeint(Y + 1, 1, T, T2, 1.E-3, (T - T2), &TderivZ4tab2);
      if (err) {
        printf(" error in odeint\n");
        return -1;
      }
      if (info)
        printf(" CMD2 : T2=%.2E Y=%.2E\n", T2, Y[1]);
      Y1T[i] = 0;
      Y2T[i] = Y[1];
      T = T2;
    }
    ...
    fracCDM2 = (Y[1] + Yeq2(Tend)) * Mcdm2 /
               ((Y[0] + Yeq1(Tend)) * Mcdm1 + (Y[1] + Yeq2(Tend)) * Mcdm2);
    return (Y[0] + Yeq1(Tend)) * 2.742E8 * Mcdm1 +
           (Y[1] + Yeq2(Tend)) * 2.742E8 * Mcdm2;
  }
}
