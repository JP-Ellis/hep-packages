...
extern void plot_1(double xMin, double xMax, int dim, double *f, double *ff,
...
void displayPlotN(char *title, char *xName, double xMin, double xMax,
                  int lScale, int N, char **Y, int *Dim, double **f,
                  double **ff) {
  int pid;
  ...
  if (pid == 0) {
    ...
    if (err) {
      printf(
          "Can not display plot because micromegas is compiled without X11\n");
      exit(0);
    }
...
int displayPlot(char *title, char *xName, double xMin, double xMax, int lScale,
                int N, ...) {
  int i, j;
  double **f;
  double **ff;
  ...
  if (lScale != 0 && lScale != 1) {
    printf(
        " 5-th parameter  is not 0/for linear scale/ or 1/for log scale/.\n");
    return 1;
  }
  ...
  for (i = 0; i < N; i++) {
    ...
    for (j = 0; j < 50; j++)
      if (Y[i][j] == 0)
        break;
    if (j == 50) {
      printf("Parameter %d is not a text string or too long. Curve title "
    ...
    if (Dim[i] < 0 || Dim[i] > 300) {
      printf("Parameter %d =%d should present number of points for curve of "
             "number of bins for "
             "histogram.\n"
             "Values larger than 300 are forbidden. Zero input reserved for "
             "presentation of  "
             "functions\n",
             4 + N * 4, Dim[i]);
      free(Dim);
    ...
    if (Dim[i]) {
      ...
      for (j = 0; j < Dim[i]; j++) {
        double x;
        if (lScale)
          x = xMin * pow(xMax / xMin, (j + 0.5) / Dim[i]);
        else
          x = xMin + (j + 0.5) / Dim[i] * (xMax - xMin);
        if (A) {
...
void killPlots(void) {
  ...
  if (!newPID)
    return;
  for (i = 0; i < newPID; i++)
    if (waitpid(pidList[i], NULL, WNOHANG) == 0)
      break;
    else
      pidList[i] = 0;
