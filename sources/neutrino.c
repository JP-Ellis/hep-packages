...
{ double mu=M_cdm/MA;
 ...
 return - T/(MA*mu*mu)*(  mu*(-aP*exp(-aM*aM) - aM*exp(-aP*aP))/sqrt(M_PI)
                         +(mu/2 -mu*aP*aM - muP*muM)*( erf(aP)-erf(-aM)) +
muP*muP*( erf(bP)-erf(-bM))*exp(-M_cdm/(2*T)*(v*v-w*w)));
...
static double r3Integrand(double r3) {
  ...
  if (w2rate <= 0)
    return 0;
  rhoGeVcm3 =
      polint1(r / Rcm, nTab, rTab, rhoTab) / mp_g; // in proton mass units
  mfrac = polint1(r / Rcm, nTab, rTab, aFraction);
  if (FFalpha == 0)
    return rhoGeVcm3 * mfrac * w2rate;
  else
    return rhoGeVcm3 * mfrac * (1 + muX) *
           (exp(-v_stat * v_stat * FFalpha) -
            exp(-(v_stat * v_stat + vescQ) * FFalpha / (1 + muX))) /
           FFalpha;
}
...
static double vIntegrand(double v) {
  ...
  if (phiv > phiTab[0])
    return 0;
  if (phiv <= phiTab[nTab - 1])
    Rmax = Rcm;
  else
    Rmax = Rcm * polint1(phiv, nTab, phiTab, rTab);
  return u * (4 * M_PI / 3.) *
         simpson(r3Integrand, 0, Rmax * Rmax * Rmax, 1.E-4, NULL) * Vlight *
         Vlight * 1.E-1; // in cm
}
...
static void fillGravPotential(void) {
  ...
  for (; i < nTab; i++) {
    odeint(x, 2, Rcm * rTab[i - 1], Rcm * rTab[i], 1.E-3,
           Rcm * (rTab[i] - rTab[i - 1]) / 3, derivePhi);
    phiTab[i] = x[1];
  }
  phi0 = x[0] * Gconst * 100 / Rcm;
  // printf("VescSurface=%E\n", sqrt(2*phi0)*Vlight);
  for (i = 0; i < nTab; i++)
    phiTab[i] = phiTab[nTab - 1] - phiTab[i] + phi0;
  // printf("VescCenter=%E\n", sqrt(2*phiTab[0])*Vlight);
}

#define SUNPOINTS 1268
static double rSun[SUNPOINTS], tSun[SUNPOINTS], rhoSun[SUNPOINTS],
    Hsun[SUNPOINTS], He4sun[SUNPOINTS], He3sun[SUNPOINTS], C12sun[SUNPOINTS],
    N14sun[SUNPOINTS], O16sun[SUNPOINTS], phiSun[SUNPOINTS];

#define EARTHPOINTS 101

static double OEarth[EARTHPOINTS], NaEarth[EARTHPOINTS], MgEarth[EARTHPOINTS],
    AlEarth[EARTHPOINTS], SiEarth[EARTHPOINTS], PEarth[EARTHPOINTS],
    SEarth[EARTHPOINTS], CaEarth[EARTHPOINTS], CrEarth[EARTHPOINTS],
    FeEarth[EARTHPOINTS], NiEarth[EARTHPOINTS];

static double rEarth[EARTHPOINTS], tEarth[EARTHPOINTS], rhoEarth[EARTHPOINTS],
    phiEarth[EARTHPOINTS];
...
static int readSunData(void) {
  ...
  if (!rdOK) {
    ...
    for (i = 0; i < SUNPOINTS; i++) {
      fscanf(f, "%*lf %lf %lf %lf %*lf %*lf %lf %lf %lf %lf %lf %lf ", rSun + i,
             tSun + i, rhoSun + i, Hsun + i, He4sun + i, He3sun + i, C12sun + i,
             N14sun + i, O16sun + i);
      //    printf("%E %E %E %E %E %E %E %E %E\n", rsun[i], tsun[i], rhosun[i],
      //    Hsun[i],He4sun[i], He3sun[i], C12sun[i],N14sun[i],O16sun[i]);
      //      printf("frac= %E\n",(1-( Hsun[i]+He4sun[i]+ He3sun[i]+
      //      C12sun[i]+N14sun[i]+O16sun[i]))/O16sun[i] );
    }
    fclose(f);
  }
  rTab = rSun;
...
static double captureSun(double (*vfv)(double), double M_cdm, double pA0,
                         double nA0, double pA5, double nA5) {
  /*                 H    He      C     N     O    Ne     Mg    Si    S     Fe
   * Na    Al   Cl Ar Ca   Cr   Ni */
  int A[17] = {1,  4,  12, 14, 16, 20, 24, 28, 32,
               56, 23, 27, 35, 40, 40, 52, 59};
  int Z[17] = {1, 2, 6, 7, 8, 10, 12, 14, 16, 26, 11, 13, 17, 18, 20, 24, 28};
  double P10[17] = {12,   10.93, 8.39, 7.78, 8.66, 7.84, 7.53, 7.51, 7.14,
                    7.45, 6.17,  6.37, 5.50, 6.18, 6.31, 5.64, 6.23};
  //  double P10[17] ={
  //  12,  10.93,  8.39, 7.78, 8.66, 8.08, 7.58, 7.55, 7.33, 7.50
  //  , 6.33, 6.47, 5.50, 6.40, 6.36, 5.64, 6.25};
  ...
  for (sumI = 0, i = 0; i < 17; i++) {
    ...
    if (i < 4) {
      ...
      aFraction = O16sun;
      fr = A[i] / ((double)A[4]) * pow(10., P10[i] - P10[4]);
    }
    muX = (M_cdm - MA) * (M_cdm - MA) / (4 * M_cdm * MA);
    vmaxC = (Vesc + Vrot) / (1.E-3 * Vlight);
    vmaxQ = vmaxC * vmaxC;
    if (vmaxQ * muX > 2 * phiSun[0])
      vmaxQ = 2 * phiTab[0] / muX;

    if (i == 0)
      FFalpha = 0;
    else
      FFalpha =
          M_cdm * MA * pow((0.91 * pow(MA, 0.3333333) + 0.3) * GeVfm, 2) / 3;
    cI = fr * FF / A[i] * si *
         simpson(vIntegrand, 0, 1.E-3 * Vlight * sqrt(vmaxQ), 1.E-3, NULL);
    sumI += cI;
    // printf("C%d = %E  \n",A[i],rhoDM/M_cdm*cI);
  }
...
static double sigmaSun(double M_cdm, double pA0, double nA0, double pA5,
                       double nA5, double R3) {
  ...
  for (sumI = 0, i = 0; i < 5; i++) {
    ...
    if (i == 0)
      si += csSDp;
    /*
        if(i<4) {aFraction=sunFractions[i];fr=1;}  else { aFraction=O16sun;
       fr=A[i]/((double)A[4])*pow(10., P10[i]-P10[4]);}
  ...
  if (!rdOK) {
    ...
    for (i = 0; i < EARTHPOINTS; i++) {
      fscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
             rEarth + i, tEarth + i, rhoEarth + i, OEarth + i, NaEarth + i,
             MgEarth + i, AlEarth + i, SiEarth + i, PEarth + i, SEarth + i,
             CaEarth + i, CrEarth + i, FeEarth + i, NiEarth + i);
      //    printf("%E %E %E %E %E %E %E %E %E\n", rsun[i], tsun[i], rhosun[i],
      //    Hsun[i],He4sun[i], He3sun[i], C12sun[i],N14sun[i],O16sun[i]);
      //      printf("frac= %E\n",(1-( Hsun[i]+He4sun[i]+ He3sun[i]+
      //      C12sun[i]+N14sun[i]+O16sun[i]))/O16sun[i] );
...
static double captureEarth(double (*vfv)(double), double M_cdm, double pA0,
                           double nA0, double pA5, double nA5) {
  ...
  double sumI, mu;
  double *earthFractions[11] = {OEarth,  NaEarth, MgEarth, AlEarth,
                                SiEarth, PEarth,  SEarth,  CaEarth,
                                CrEarth, FeEarth, NiEarth};
  int i;
...
static double sigmaEarth(double M_cdm, double pA0, double nA0, double pA5,
                         double nA5, double R3) {
...
double captureAux(double (*vfv)(double), int forSun, double M_cdm, double csIp,
                  double csIn, double csDp, double csDn) {
  double pA0, nA0, pA5, nA5;
  double MN = 0.939;
  double Mr = MN * M_cdm / (MN + M_cdm);
  double sCoeff = 2 * sqrt(3.8937966E8 / M_PI) * Mr;

  pA0 = sqrt(fabs(csIp)) / sCoeff;
  if (csIp < 0)
    pA0 *= -1;
  nA0 = sqrt(fabs(csIn)) / sCoeff;
  if (csIn < 0)
    nA0 *= -1;
  pA5 = sqrt(fabs(csDp / 3)) / sCoeff;
  if (csDp < 0)
    pA5 *= -1;
  nA5 = sqrt(fabs(csDn / 3)) / sCoeff;
  if (csDn < 0)
    nA5 *= -1;

  if (forSun)
    return captureSun(vfv, M_cdm, pA0, nA0, pA5, nA5);
  else
    return captureEarth(vfv, M_cdm, pA0, nA0, pA5, nA5);
}
...
static nuTableStr *readNuTable(FILE *F) {
  ...
  for (int ix = 0;;) {
    ...
    if (2 != fscanf(F, " %lf %lf ", &m, &x))
      break;
    // printf("m=%e x=%e\n",m,x);
    if (!T->m) {
      T->m = malloc(sizeof(double));
      T->nM = 1;
      T->m[0] = m;
      // printf("T->m[0]=%E\n", T->m[0]);
  ...
  /*
    { int i,j,k;
      ...
      { printf(" %.2E %.2E | ", T->m[i],T->x[j]);
        for(k=0;k<T->nCh;k++) printf("
    %.2E(%.2E)",T->data[id++],T->data[k+T->nCh*(j+T->nX*i)] ); printf("\n");
      }
    }
  */
  return T;
...
static int nuTabInterpolation(nuTableStr *T, double Mass, int pdgN, int pol,
                              int smooth, double *tab) {
  int i;
  ...
  int iCh, iM;
  for (iCh = 0; iCh < T->nCh; iCh++)
    if (T->ch[iCh] == abs(pdgN))
      break;
    ...
    iM = T->nM - 2;
  {
    ...
    if (iM > 0 && iM < T->nM - 2) {
      ...
      /*
        a[0]=           (mm-m[1])*(mm-m[2])*(mm-m[3])/
        (m[0]-m[1])/(m[0]-m[2])/(m[0]-m[3]); a[1]= (mm-m[0])*
        (mm-m[2])*(mm-m[3])/(m[1]-m[0])/            (m[1]-m[2])/(m[1]-m[3]);
        a[2]= (mm-m[0])*(mm-m[1])*          (mm-m[3])/(m[2]-m[0])/(m[2]-m[1])/
        (m[2]-m[3]); a[3]= (mm-m[0])*(mm-m[1])*(mm-m[2])
        /(m[3]-m[0])/(m[3]-m[1])/(m[3]-m[2]);
      */
    } else if (iM == 0) {
      j1 = 1;
      j2 = 4;
    ...
    if (pol == 0 && poleData)
      for (i = 0; i < T->nX; i++) {
        for (int j = j1; j < j2; j++)
          T->buff[i] +=
              a[j] *
              (T->data[iCh + T->nCh * (i + T->nX * (iM + j - 1))] +
               T->data[1 + iCh + T->nCh * (i + T->nX * (iM + j - 1))]) /
              2;
      }
    else {
      if (pol == 1)
      ...
      for (i = 0; i < T->nX; i++)
        for (int j = j1; j < j2; j++)
          T->buff[i] +=
              a[j] * T->data[iCh + T->nCh * (i + T->nX * (iM + j - 1))];
    }
  }
  ...
  for (i = 1; i < NZ; i++) {
    double x = exp(Zi(i));
    tab[i] = smooth ? x * polint3(x, T->nX, T->x, T->buff)
                    : x * polint1(x, T->nX, T->x, T->buff);
  }

  if (Mass < T->m[0] || Mass > T->m[T->nM - 1])
    err++;

  return err;
}

int basicNuSpectra(int forSun, double Mass, int pdgN, int pol, double *nu,
                   double *nuB) {
  static nuTableStr *allTab[3][2][2] = {{{NULL, NULL}, {NULL, NULL}},
                                        {{NULL, NULL}, {NULL, NULL}},
                                        {{NULL, NULL}, {NULL, NULL}}};
  ...
  case 0:
    if (forSun) {
      ...
      printf("PPPC4DM$nu does not contain spectra of neutrinos produced in the "
             "center of Earth.\n"
  ...
  for (outN = 0; outN < 2; outN++)
    if (!allTab[pkgN][forSun][outN]) {
      ...
      sprintf(fname, "%s/sources/data_nu/%s_%s_%s.txt", micrO, source[forSun],
              yeald[outN], packages[pkgN]);
  ...
  err = 0;
  if (nu)
    err = nuTabInterpolation(allTab[pkgN][forSun][1], Mass, pdgN, pol,
                             pkgN != 2, nu);
  if (nuB)
    err = nuTabInterpolation(allTab[pkgN][forSun][0], Mass, pdgN, pol,
                             pkgN != 2, nuB);
  return err;
}
...
static void getSpectrum(int forSun, double M, double m1, double m2, char *n1,
                        char *n2, int N1, int N2, int outP, double *tab) {
  int i, k;
  char *nn[2];
  int pdg[2];
  double mm[2], E[2], p2;
  ...
  if (N1 + N2 == 0 || (N1 == 21 && N2 == 21) || (N1 == 22 && N2 == 22) ||
      (N1 == 23 && N2 == 23) || (N1 == 25 && N2 == 25)) {
    if (outP > 0) {
      if (basicNuSpectra(forSun, M / 2, N1, 0, tab, NULL) == 0)
        return;
    } else {
      if (basicNuSpectra(forSun, M / 2, N1, 0, NULL, tab) == 0)
        return;
    }
  }
  ...
  for (k = 0; k < 2; k++) {
    ...
    double tabAux[NZ];
    int err;
    if (abs(pdg[k]) == 11 || abs(pdg[k]) == 13 || abs(pdg[k]) == 22)
      continue;
    if (outP > 0)
      err = basicNuSpectra(forSun, E[k], pdg[k], 0, tabAux, NULL);
    else
      err = basicNuSpectra(forSun, E[k], pdg[k], 0, NULL, tabAux);
    if (err == 0) {
      double kf;
      dY = log(M / E[k] / 2);
      switch (abs(pdg[k])) {
      ...
      if (mm[k] == 0) {
        fprintf(stderr, "Can not hadronize BSM zero mass %s\n", nn[k]);
        continue;
      }
      ...
      if (!d2Proc) {
        fprintf(stderr, "Can not find decay modes for  mass %s\n", nn[k]);
        continue;
      }
      ...
      for (l = 1; l <= ntot; l++) {
        ...
        if (wP > 0) {
          ...
          getSpectrum(forSun, E[k], m[1], m[2], n[1], n[2], N2, N3, outP,
                      tab_p);
          for (i = 1; i < NZ; i++)
            tabAux[i] += wP * tab_p[i];
          w += wP;
      ...
      if (w == 0) {
        if (abs(pdg[k]) != abs(pNum(CDM)))
          fprintf(stderr, "Can't find decays for  %s\n", nn[k]);
        continue;
      }
...
static double calcSpectrum0(char *name1, char *name2, int forSun,
                            double *Spectranu, double *SpectraNu,
                            double *alpha) {
  int i, k;
  ...
  // Warning!!   in should be done in the same manner as annihilation libraries
  // for Omega
  ...
  for (k = 0; k < ntot; k++) {
    ...
    for (i = 0; i < 4; i++)
      N[i] = libPtr->interface->pinf(k + 1, i + 1, m + i, pdg + i);
    cc23 = NULL;
    v_cs[k] = 0;
    if (VZdecay || VWdecay) {
      int nVV;
      int vd[4] = {0, 0, 0, 0};
      for (l = 2; l < 4; l++)
        if ((pdg[l] == 23 && VZdecay) || (abs(pdg[l]) == 24 && VWdecay))
          vd[l] = 1;
    ...
    if (cc23) {
      ...
      for (i3W = 2; i3W < 5; i3W++)
        if (strcmp(cc23->interface->pinf(1, i3W + 1, NULL, NULL), N[l_]) == 0)
          break;
      ...
      if (pdg[l_] == 23 || abs(pdg[l_]) == 24) {
        ...
        r *= decayPcmW(M1 + M2, m[l], m[l_], wV, wV2, 0) /
             decayPcmW(M1 + M2, m[l], m[l_], wV, 0, 0);
        if (pdg[l] == pdg[l_])
          r /= 2;
      }
      v_cs[k] = r;
  ...
  for (k = 0; k < ntot; k++)
    if (v_cs[k] > 0) {
      ...
      for (l = 0; l < 2; l++)
        pdg[l] = qNumbers(N[2 + l], NULL, NULL, NULL);
      if (N[2][0] == '~' || N[3][0] == '~')
        vcsSum1 += v_cs[k];
#ifdef PRINT
      {
...
#endif
      getSpectrum(forSun, pMass(name1) + pMass(name2), m[2], m[3], N[2], N[3],
                  pdg[0], pdg[1], 1, tab2);
      for (i = 1; i < NZ; i++)
        Spectranu[i] += tab2[i] * v_cs[k] / vcsSum;
      getSpectrum(forSun, pMass(name1) + pMass(name2), m[2], m[3], N[2], N[3],
                  pdg[0], pdg[1], -1, tab2);
      for (i = 1; i < NZ; i++)
        SpectraNu[i] += tab2[i] * v_cs[k] / vcsSum;
...
static void deriv1(double t, double *n, double *dn) {
  dn[0] = C[0] - An[0] * n[0] * n[0] - Ev[0] * n[0];

  // printf(" C=%E An=%E Ev=%E\n", C[0], An[0]*n[0]*n[0], Ev[0]*n[0]);
}

static void deriv2(double t, double *n, double *dn) {
  dn[0] = C[0] + 0.5 * Alpha * An[1] * n[1] * n[1] - An[1] * n[0] * n[0] -
          An[0] * n[0] * n[1] - Ev[0] * n[0];
  dn[1] = C[1] + 0.5 * Alpha * An[1] * n[0] * n[0] - An[1] * n[1] * n[1] -
          An[0] * n[0] * n[1] - Ev[1] * n[1];
}
...
int neutrinoFlux(double (*fvf)(double), int forSun, double *nu, double *Nu) {
  ...
  if (CDM1 && CDM2) {
    printf(" The 'neutrinoFlux' code is still not upgrated for 2DM case\n");
    return 1;
  }
  if (CDM1)
    CDM = CDM1;
  ...
  Cr0 = forSun ? captureSun(fvf, M_cdm, pA0[0], nA0[0], pA5[0], nA5[0])
               : captureEarth(fvf, M_cdm, pA0[0], nA0[0], pA5[0], nA5[0]);

  if (pA0[0] == pA0[1] && nA0[0] == nA0[1] && pA5[0] == pA5[1] &&
      nA5[0] == nA5[1])
    Cr1 = Cr0;
  else
    Cr1 = forSun ? captureSun(fvf, M_cdm, pA0[1], nA0[1], pA5[1], nA5[1])
                 : captureEarth(fvf, M_cdm, pA0[1], nA0[1], pA5[1], nA5[1]);
  ...
  if (forSun)
    ...
    R = 6378.1;                           /* Distance to Sun/Earth in [km] */
  ...
  vcs0 = calcSpectrum0(name, aname, forSun, nu, Nu, NULL);
  {
    ...
    if (forSun)
      S = sigmaSun(M_cdm, pA0[0], nA0[0], pA5[0], nA5[0], r095 * r095 * r095);
    else
      S = sigmaEarth(M_cdm, pA0[0], nA0[0], pA5[0], nA5[0], r095 * r095 * r095);
    ...
    if (strcmp(name, aname)) {
      if (forSun)
        S = sigmaSun(M_cdm, pA0[1], nA0[1], pA5[1], nA5[1], r095);
      else
        S = sigmaEarth(M_cdm, pA0[1], nA0[1], pA5[1], nA5[1], r095);
  ...
  if (strcmp(name, aname)) {
    ...
    { /* symbolic solution for large Etime */
      ...
      beta = Cr0 / Cr1;
      x = (beta - 1 +
           sqrt((beta - 1) * (beta - 1) + 4 * beta * alpha * alpha)) /
          (2 * alpha);
    ...
    // 0.5*Cr0*pow(tanh(Etime*sqrt(Cr0*vcs0/Veff)),2)*Prop);
    for (i = 1; i < NZ; i++) {
      nu[i] *= rf;
      Nu[i] *= rf;
    }
...
static double inte_grand(double x) {
  return polint1(sqrt(1 + x * x - 2 * x * cosFi_stat), EARTHPOINTS, rEarth,
                 rhoEarth);
}
...
void makeTabl(void) {
  ...
  for (i = 0; i <= 20; i++) {
    cosFi_stat = i / 20.;
    printf("cos=%E rho=%E\n", cosFi_stat,
           2 * RE * simpson(inte_grand, 0, cosFi_stat, 1.E-3, NULL));
  }
}
...
double nuAttenuation(int nu, double cs, double E) // 1/pb
{
  ...
  double cos[21] = {0.000000E+00, 5.000000E-02, 1.000000E-01, 1.500000E-01,
                    ...
                    1.000000E+00};
  double rho[21] = {0.000000E+00, 1.775227E+08, 3.755063E+08, 6.110139E+08,
                    8.372967E+08, 1.056885E+09, 1.293592E+09, 1.538506E+09,
                    1.841714E+09, 2.167974E+09, 2.553811E+09, 2.913635E+09,
                    3.278042E+09, 3.656525E+09, 4.047799E+09, 4.461401E+09,
                    4.899167E+09, 6.159285E+09, 7.876339E+09, 9.341818E+09,
                    1.096080E+10};
  ...
  if (nu > 0)
    sigma = C * 0.5 * E * ((0.15 + 0.25) + (0.04 + 0.06) / 3); // sigma[cm^2]
  else
    sigma = C * 0.5 * E * ((0.04 + 0.06) + (0.15 + 0.25) / 3);
...
double sigmaNu(int nu, double E) //  pb
{
  ...
  if (nu > 0)
    return C * 0.5 * E * ((0.15 + 0.25) + (0.04 + 0.06) / 3);
  else
    return C * 0.5 * E * ((0.04 + 0.06) + (0.15 + 0.25) / 3);
}
#endif
...
static double dSigmadE_nu2mu(int inNu, double Enu,
                             double Emu) /*result in   1/cm^2/GeV */
{
  double GF = 1.16637E-5; /* GeV^(-2) */
  double GeVcm = 0.50677E14;
...
static double integrandX(double x) {
  ...
  //   double q= (0.10566 /* muon mass*/)/65865.4 /* (muon_life_time)*c in cm
  //   */)/alpha_stat;
  double q = (0.10566 /* muon mass*/) /
             (6665865.4 /* (muon_life_time)*c in cm */) / alpha_stat;
  double Emu_prim = (Emu_stat + g) * ex - g;
  double Psurv =
      pow(Emu_stat * (Emu_prim + g) / (Emu_prim * (Emu_stat + g)), q);
...
static double integrandEnuUpward_sigma(double e) {
  ...
  sigma *= sigma;
  return sigma * E * E * nuSpectrum(E) *
         simpson(integrandX, 0, xMax, 1.E-4, NULL);
}
    ...
    if (Emu_stat > 0.01) {
      for (k = 0; k < 2; k++)
        if (Sp[k]) {
          ...
          SpN_stat = Sp[k];
          mu[i] +=
              simpson(integrandEnuUpward, log(Emu_stat), log(M), 1.E-3, NULL);
          if (sigma)
            sigma[i] += simpson(integrandEnuUpward_sigma, 1 / M, 1 / Emu_stat,
                                1.E-4, NULL);
        }
      if (sigma) {
        sigma_mem = sigma[i] = sqrt(sigma[i] / mu[i]);
        sigma[i] *= Emu_stat;
      }
      mu[i] *= rho * NA * Emu_stat;
    } else {
      if (sigma)
        sigma[i] = sigma_mem * Emu_stat;
    }
  }
}
    ...
    if (Emu_stat > 0.01) {
      for (k = 0; k < 2; k++)
        if (Sp[k]) {
          inNu = 1 - 2 * k;
          SpN_stat = Sp[k];
          mu[i] += simpson(integrandEnuUpward_I, Emu_stat, M, 1.E-4, NULL) /
                   (alpha_stat + beta_stat * Emu_stat);
        }
      mu[i] *= rho * NA * Emu_stat;
    }
  }
...
static double integrandEnuContained(double e) {
  ...
  if (e == 0)
    return 0;
  E = 1 / e;
  return E * E * nuSpectrum(E) * dSigmadE_nu2mu(inNu, E, Emu_stat);
}
  ...
  for (i = 2; i < NZ; i++) {
    ...
    if (Emu_stat > 0.01)
      for (k = 0; k < 2; k++) {
        ...
        if (Sp[k]) {
          SpN_stat = Sp[k];
          mu[i] += simpson(integrandEnuContained, 1 / M_cdm, 1 / Emu_stat,
                           1.E-4, NULL);
        }
      }
...
double ATMdNudE(double E) // for NuBar *1.35/1.95
{
  ...
  return 1.95E17 * pow(E, -gamma - 1) *
         (a / (1 + b * E * cosFi_stat) + c / (1 + e * E * cosFi_stat));
}
...
double ATMmuonUpward(double cosFi, double E) {
  ...
  for (k = 0; k < 2; k++)
    for (l = 0; l < 2; l++) {
      ...
      inPr = -1 + 2 * l;
      mu += (k == 0 ? 1 : 1.35 / 1.95) *
            simpson(integrandEnuUpward, log(E), log(1E6), 1.E-3, NULL) *
            Nrate[l];
    }
  return mu * rho * NA;
}
...
double ATMmuonContained(double cosFi, double E, double rho) {
  ...
  for (k = 0; k < 2; k++)
    for (l = 0; l < 2; l++) {
      ...
      inPr = -1 + 2 * l;
      mu += (k == 0 ? 1 : 1.35 / 1.95) *
            simpson(integrandEnuContained, 0, 1 / E, 1.E-4, NULL) * Nrate[l];
    }
...
static int readATMnuTab(void) {
  ...
  for (i = 0; i < 31; i++) {
    ...
    for (j = 0; j < 10; j++)
      fscanf(f, " %lf", atmNuBar->data[i] + j);
    fscanf(f, " %lf", &norm);
    for (j = 0; j < 10; j++)
      atmNuBar->data[i][j] *= norm;
  }
  fclose(f);
  return 0;
}
...
double atmNuFlux(int nu, double cs, double E) {
  double alpha;
  double cstab[10] = {0.05, 0.15, 0.25, 0.35, 0.45,
                      0.55, 0.65, 0.75, 0.85, 0.95};
  atmDataStr *data;
  int i;

  if (E < 10 || E > 1E4)
    printf("ATM neutrino flux: Energy %E out of table range\n", E);

  if ((!atmNu || !atmNuBar) && readATMnuTab()) {
    printf(" Can not read data file for atmospheric neutrino\n");
    return -1;
  }
  if (nu >= 0)
    data = atmNu;
  ...
  alpha =
      (log(data->E[i + 1]) - log(E)) / (log(data->E[i + 1]) - log(data->E[i]));

  return 1.E6 * 365 * 24 * 60 * 60 *
         pow(polint1(cs, 10, cstab, data->data[i]), alpha) *
         pow(polint1(cs, 10, cstab, data->data[i + 1]), 1 - alpha);
}
