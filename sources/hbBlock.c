...
int hbBlocksMO(char *fname, int *nHiggsCh) {
  ...
  char *G = pdg2name(21);
  if (!G) {
    printf("Gluon  is absent in the model");
    return -1;
  }
  char *A = pdg2name(22);
  if (!A) {
    printf("Photon is absent in the model");
    return -1;
  }
  char *Z = pdg2name(23);
  if (!Z) {
    printf("Z is absent in the model");
    return -1;
  }
  char *Wp = pdg2name(24);
  if (!Wp) {
    printf("W+ is absent in the model");
    return -1;
  }
  char *Wm = antiParticle(Wp);
  double MW = pMass(Wp);

  lVert *WWA = getVertex(Wp, Wm, A, NULL);
  if (!WWA) {
    printf(" %s %s %s is absent\n", Wp, Wm, A);
    return -2;
  }
  getNumCoeff(WWA, coeff);
  // for(k=0;k<WWA->nTerms;k++) printf(" %s  ", WWA->SymbVert[k]); printf("\n");
  for (k = 0; k < WWA->nTerms; k++)
    if (strcmp(WWA->SymbVert[k], "m2.m1*p2.m3") == 0)
      break;
  double EE = coeff[k];
  ...
  lVert *WWZ = getVertex(Wp, Wm, Z, NULL);
  if (!WWZ) {
    printf(" %s %s %s is absent\n", Wp, Wm, Z);
    return -2;
  }
  getNumCoeff(WWZ, coeff);
  for (k = 0; k < WWZ->nTerms; k++)
    if (strcmp(WWZ->SymbVert[k], "m2.m1*p2.m3") == 0)
      break;
  ...
  fprintf(
      f,
      "# SM basic parameters: MW=%.3E alphaEM^{-1}=%.3E SW^2=%.3E  VEV=%.3E \n",
  ...
  for (i = 0; i < nModelParticles; i++)
    if (/*ModelPrtcls[i].name[0]!='~' &&*/ ModelPrtcls[i].spin2 == 0 &&
      ...
      for (j = 0; j < 6; j++)
        if (abs(ModelPrtcls[i].NPDG) == hList[j])
          break;

      if (j == 6) {
        if (ModelPrtcls[i].name[0] != '~')
          printf(" HB-Block Warning:  Colorless Scalar even particle  %s "
                 "(pdg=%d) is out of "
                 "HiggsBounds list\n",
  ...
  int DMList[7] = {1000022, 1000012, 1000014, 1000016,
  ...
  if (CDM1) {
    ...
    for (int i = 0; i < 7; i++)
      if (ndm == DMList[i])
        break;
    if (i == 7)
      printf(" HB-Block Warning: DM %s is out of HiggsBounds list\n", CDM1);
  }
  if (CDM2) {
  ...
  for (i = 0; i < nHch; i++)
    fprintf(f, " %d  %E # %s\n", pNum(chHiggs[i]), pMass(chHiggs[i]),
            chHiggs[i]);
  ...
  for (i = 0; i < nHiggs; i++)
    for (j = 0; j < 3; j++) {
      char *fe = pdg2name(ferm[j]);
      if (!fe)
        continue;
      char *Fe = antiParticle(fe);
      if (!Fe)
        continue;
      double mf = pMass(fe);
      if (mf == 0)
      ...
      if (hff) {
        ...
        for (k = 0; k < hff->nTerms; k++) {
          if (strcmp(hff->SymbVert[k], "1") == 0)
            c[0] = -coeff[k] * vev / mf;
          else if (strcmp(hff->SymbVert[k], "G5*i") == 0)
            c[1] = -coeff[k] * vev / mf;
        }
      }
      fprintf(f, " %12.4E  %12.4E  3  %3d  %3d  %3d  # %s %s %s\n", c[0], c[1],
              pNum(Higgs[i]), ferm[j], ferm[j], Higgs[i], Fe, fe);
    }
  ...
  for (i = 0; i < nHiggs; i++)
    for (j = 0; j < 2; j++) {
      char *ve = pdg2name(vbm[j]);
      if (!ve)
        continue;
      char *Ve = antiParticle(ve);
      if (!Ve)
        continue;
      ...
      if (hvv) {
        ...
        for (k = 0; k < hvv->nTerms; k++)
          if (strcmp(hvv->SymbVert[k], "m2.m1") == 0)
            c = 0.5 * coeff[k] * vev / pMass(ve) / pMass(ve);
      }
      fprintf(f, " %12.4E  3  %3d  %3d  %3d  # %s %s %s\n", c, pNum(Higgs[i]),
              vbm[j], vbm[j], Higgs[i], Ve, ve);
    }

  for (i = 0; i < nHiggs; i++)
    for (j = i + 1; j < nHiggs; j++) {
      char *ve = pdg2name(23);
      if (!ve)
        continue;
      ...
      if (hhv) {
        ...
        for (k = 0; k < hhv->nTerms; k++)
          if (strcmp(hhv->SymbVert[k], "p1.m3*i") == 0)
            c = coeff[k] * vev / pMass(ve);
      }
      fprintf(f, " %12.4E  3  %3d  %3d  %3d  # %s %s %s\n", c, pNum(Higgs[i]),
              pNum(Higgs[j]), 23, Higgs[i], Higgs[j], ve);
    }
  ...
  for (i = 0; i < nHiggs; i++)
    for (j = 0; j < nHch; j++) {
      ...
      if (hhv) {
        ...
        for (k = 0; k < hhv->nTerms; k++)
          if (strcmp(hhv->SymbVert[k], "p1.m3*i") == 0)
            c[0] = coeff[k] * vev / pMass(ve);
          else if (strcmp(hhv->SymbVert[k], "p1.m3") == 0)
            c[1] = coeff[k] * vev / pMass(ve);
      }
      fprintf(f, " %12.4E  3  %3d  %3d  %3d  # %s %s %s\n",
              (double)sqrt(c[0] * c[0] + c[1] * c[1]), pNum(Higgs[i]),
              pNum(chHiggs[j]), pNum(ve), Higgs[i], chHiggs[j], ve);
    }
  ...
  for (i = 0; i < nHiggs; i++) {
    ...
    fprintf(f, " %12.4E  3  %3d  %3d  %3d  # %s %s %s\n",
            lamQGG[i] / LGGSM / LGGSM, pNum(Higgs[i]), pNum(G), pNum(G),
            Higgs[i], G, G);
    fprintf(f, " %12.4E  3  %3d  %3d  %3d  # %s %s %s\n",
            lamQAA[i] / LAASM / LAASM, pNum(Higgs[i]), pNum(A), pNum(A),
            Higgs[i], A, A);
  }
  ...
  if (nHiggsCh)
    *nHiggsCh = nHch;
  if (changeVirtual) {
    VZdecay = VZdecay_save;
    VWdecay = VWdecay_save;
    cleanDecayTable();
  }
