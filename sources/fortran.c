...
void antiparticle_(char *aname, char *name, int alen, int len) {
  ...
  for (i = strlen(aname); i < alen; i++)
    aname[i] = ' ';
}

double darkomega_(double *Xf, int *Fast, double *Beps, int *err) {
  return darkOmega(Xf, *Fast, *Beps, err);
}
double darkomegafo_(double *Xf, int *fast, double *Beps) {
  return darkOmegaFO(Xf, *fast, *Beps);
}
...
double onechannel_(double *Xf, double *Beps, char *fn1, char *fn2, char *fn3,
                   char *fn4, int len1, int len2, int len3, int len4) {
...
static int channels(aChannel *Chann, int *i, double *w, int *pdg, char *txt,
                    int len) {
  int k, j;
  if (!Chann || *i < 1)
    return 0;
  ...
  *w = Chann[k].weight;
  ...
  if (Chann[k].prtcl[4]) {
    ...
    pdg[j] = 0;
  for (j = strlen(txt); j < len; j++)
    txt[j] = ' ';

  return 1;
}

int omegach_(int *i, double *w, int *pdg, char *txt, int len) {
  return channels(omegaCh, i, w, pdg, txt, len);
}

int vsigmatch_(int *i, double *w, int *pdg, char *txt, int len) {
  return channels(vSigmaTCh, i, w, pdg, txt, len);
}

int vsigmach_(int *i, double *w, int *pdg, char *txt, int len) {
  return channels(vSigmaCh, i, w, pdg, txt, len);
}
...
void setvvdecay_(int *vwdecay, int *vzdecay) {
  VWdecay = *vwdecay;
  VZdecay = *vzdecay;
  cleanDecayTable();
}

int neutrinoflux_(double (*fDv)(double *), int *forSun, double *nu,
                  double *Nu) {
  if (fDv == maxwell_)
    return neutrinoFlux(Maxwell, *forSun, nu, Nu);
  else {
    _fDv = fDv;
...
void muoncontained_(double *nu, double *Nu, double *rho, double *mu) {
  muonContained(nu, Nu, *rho, mu);
}

double captureaux_(double (*fDv)(double *), int *forSun, double *Mass,
