...
int init22_par(par_22 *arg, numout *cc, int nsub) {
  ...
  if (nsub > ntot || nin != 2 || nout != 2)
    return 2;
  for (i = 0; i < 4; i++)
    arg->pdg[i] = arg->cc->interface->pinfAux(arg->nsub, 1 + i, arg->spin2 + i,
                                              NULL, NULL, (arg->ndf) + i);
  //   printf("arg->ndf = %d %d %d %d\n", arg->ndf[0], arg->ndf[1], arg->ndf[2],
  //   arg->ndf[3]);
  arg->T = 0;
  arg->err = 0;
  return 0;
...
void mass22_par(par_22 *arg, double T) {
  ...
  char *p[5];
  for (i = 0; i < 4; i++)
    p[i] = cc->interface->pinf(nsub, 1 + i, arg->pmass + i, NULL);
  ...
  if (arg->sqrtSmin < arg->pmass[2] + arg->pmass[3])
    arg->sqrtSmin = arg->pmass[2] + arg->pmass[3];
}
...
static double poleSqme(double x0, double x1, double x2, double delta, double f0,
                       double f1, double f2) {
  if (f2 - f0 <= 0) {
    return f0 * x0;
  }
  double r = (f1 - f0) / (f2 - f0), z1 = (x0 + delta) / (x1 + delta),
         z2 = (x0 + delta) / (x2 + delta);
  double p, a, b;
  if (r >= (1 / z1 - 1) / (1 / z2 - 1))
    p = -1;
  else if (r <= (z1 * z1 - 1) / (z2 * z2 - 1))
    p = 2;
  else {
    double p1 = -1, p2 = 2;
    double a1 = (pow(z1, p1) - 1) / (pow(z2, p1) - 1) - r,
           a2 = (pow(z1, p2) - 1) / (pow(z2, p2) - 1) - r;
    if (a1 > a2) {
      a = a1;
      a1 = a2;
      a2 = a;
  ...
  // printf("dI=%E a=%E b=%e p=%e\n", a*x0+b/(1-p)*(pow(x0+delta,1-p) -
  // pow(delta,1-p)),a,b,p);
  return a * x0 + b / (1 - p) * (pow(x0 + delta, 1 - p) - pow(delta, 1 - p));
}

// cross section [pb]=
// sqmeInt*3.8937966E8*arg.PcmOut/(32.0*M_PI*arg.PcmIn*sqrtS*sqrtS))
...
double sqmeInt(par_22 *arg, double eps) {
  ...
  if (arg->s13 != 0 && arg->e13 < smallDelta) {
    ...
    for (;;) {
      ...
      //?       if(arg->err&ErrC && arg->err&ErrD) { arg->err=err_tmp|ErrP;
      //return 0;}
      if (arg->err & ErrD) {
        x0 *= 2;
        continue;
  ...
  #ifdef ERROR_PLOT
      char txt[100];
      sprintf(txt,"Process %s,%s -> %s %s",
  arg->cc->interface->pinf(1,1,NULL,NULL),
