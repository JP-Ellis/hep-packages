...
static int create2_th_model(char *disp, char *pname, char *ProcNameSI,
                            char *ProcNameSD) {
  char *command;
  char buff[100];
  int line, i, k, err;
  ...
  if (pInvolved[0].num == 0) {
    printf("Direct detection module can not work because the model contains a "
           "particle\n");
  ...
  for (i = 1; i < K; i++)
    if (pInvolved[i].num >= -6 && pInvolved[i].num <= 6 &&
        strcmp(pInvolved[i].mass, "0") == 0) {
      printf("\n Error! Direct detection module can not work because the model "
             "contains\n");
  ...
  sprintf(command,
          "cd %s;  %s/bin/s_calchep -blind "
          "\"{[[{[{\\19\\24{{MDD__\\091\\09" //   new  parameter MDD__ for mass
                                             //   of auxilarry particles
  ...
  sprintf(command,
          "cd %s;  %s/bin/s_calchep -blind \"[[{[[{"
          "\\19%d{\\04\\19\\${{}}0\";",
          disp, calchepDir, pInvolved[0].line);
  err = system(command);
  if (err) {
    free(command);
    return 3;
  ...
  sprintf(command,
          "cd %s;  %s/bin/s_calchep -blind \"[[{[[{"
          "{_S0_\\09_S0_\\09_S0_\\090\\090\\09MDD__\\090\\091\\09*\\09_S0_\\09_"
          "S0_\\09" // new
                    // point-like
                    // scalar _S0_
          "{_V5_\\09_V5_\\09_V5_\\090\\092\\09MDD__\\090\\091\\09*\\09_V5_\\09_"
          "V5_\\09" // new
                    // point-like
                    // vector _V5_
  ...
  sprintf(command, "cd %s; %s/bin/s_calchep -blind \"[[{[[[{", disp,
          calchepDir);
  for (i = 1; i < K; i++) {
    ...
    if (pInvolved[i].spin2 == 1) {
      sprintf(command + strlen(command),
              "{%s\\09_S0_\\09\\09MDD__\\091\\09" //   (F f _S0_)   MDD__
                                                  //   interaction for fermions
              "{%s\\09_V5_\\09\\09-MDD__\\09G5*G(m3)\\09" //   (F f _V5_)
                                                          //   -MDD__*G5*G(m3)
                                                          //   interaction
              ,
              aP_P, aP_P);
    } else if (pInvolved[i].spin2 == 0) {
      sprintf(command + strlen(command), "{%s\\09_S0_\\09\\09MDD__\\091\\09",
              aP_P); // (s S _S0_) MDD__   interaction for scalars
    }
  }
  ...
  {
    sprintf(buff, "_S0_\\09\\092*MDD__*2*%s\\091\\09",
            pInvolved[0].mass); //  (DM  dm _S0_)*2*MDD__*Mdm  interaction
    nAuxVerts = 1;
    auxVert[0] = buff;
  } else if (pInvolved[0].spin2 == 1) // for spinor DM:
  {
    nAuxVerts = 2;
    auxVert[0] = "_S0_\\09\\092*MDD__\\091\\09"; // (DM  dm _S0_)*2*MDD__
    auxVert[1] =
        "_V5_\\09\\092*MDD__\\09G5*G(m3)\\09"; // (DM  dm _V5_)*2*MDD__*G5*G(m3)
  } else if (pInvolved[0].spin2 == 2)          // for vector DM:
  {
    nAuxVerts = 2;
    sprintf(buff, "_S0_\\09\\092*MDD__*2*%s\\09m1.m2\\09",
            pInvolved[0].mass); //(DM dm _S0_)*2*MDD__*Mdm*m1.m2
    auxVert[0] = buff;
    auxVert[1] =
        "_V5_\\09\\092*i*MDD__*sqrt6D\\09eps(m1,m2,(p1-p2),m3)\\09"; //(DM dm
                                                                     //_V5_)*2*i*MDD__*sqrt(6)*eps(m1,m2,(p1-p2),m3)
  }
  ...
  strcpy(ProcNameSD, ProcNameSI);
  {
    ...
    for (i = 1, lquarks[0] = 0, allcol[0] = 0; i < K; i++) {
      ...
      if (forlQ)
        sprintf(lquarks + strlen(lquarks), "%s,", pInvolved[i].name);
      if (strcmp(pInvolved[i].name, pInvolved[i].aname)) {
        sprintf(allcol + strlen(allcol), "%s,", pInvolved[i].aname);
        if (forlQ)
          sprintf(lquarks + strlen(lquarks), "%s,", pInvolved[i].aname);
      }
    }

    if (strlen(lquarks))
      lquarks[strlen(lquarks) - 1] = 0;
    if (strlen(allcol))
      allcol[strlen(allcol) - 1] = 0;
    if (allcol[0])
      strcat(ProcNameSI, allcol);
    else
      ProcNameSI[0] = 0;
    if (lquarks[0])
      strcat(ProcNameSD, lquarks);
    else
      ProcNameSD[0] = 0;
...
static int getAuxCodesForDD(char *pname, numout **ccSI, numout **ccSD) {
  ...
  if (ccSI)
    *ccSI = getMEcode(0, 1, "", NULL, NULL, libnameSI);
  if (ccSD)
    *ccSD = getMEcode(0, 1, "", NULL, NULL, libnameSD);
  if ((ccSI && (!*ccSI)) || (ccSD && (!*ccSD))) {
    ...
    if (err)
      return err;
    for (line = 0; line < nModelParticles; line++)
      if (ModelPrtcls[line].NPDG == 22) {
        A = ModelPrtcls[line].name;
        break;
      }
  }
  ...
  if (ccSI && !*ccSI && ProcNameSI[0]) {
    ...
    if (A)
      sprintf(exclude + strlen(exclude), ",%s", A);
...
void calcScalarFF(double muDmd, double msDmd, double sigmaPiN, double sigma0) {
  ...
  double y = 1 - sigma0 / sigmaPiN, z = 1.49,
         alpha = (2 * z - (z - 1) * y) / (2 + (z - 1) * y);
...
void calcScalarQuarkFF(double muDmd, double msDmd, double sigmaPiN,
                       double sigmaS) {
  double Mp = 0.9383, Mn = 0.9396;
...
static double Lambda(double M, double m1, double m2) {
  ...
  //  else
  if (D >= 0)
    return log((Mm + sD) / (Mm - sD)) / sD;
  else
    return 2 * atan(sD / Mm) / sD;
}
...
double LintIk(int II, double MSQ, double MQ, double MNE) {
  ...
  if (del > 0)
    LAM = 2. * atan(sqrt(del) / SPPM) / sqrt(del);
  if (del < 0)
    LAM = log((SPPM + sqrt(-del)) / (SPPM - sqrt(-del))) / sqrt(-del);

  switch (II) {
  case 1:
    CMD = 1. / del *
          (R2 / 3 - 2 / 3. * R3 - 5 / 3. + (2 * msq2 - 2 / 3. * mne2) * LAM);
    break;
  case 2:
    CMD = (log(msq2 / mq2) - SPMM * LAM) / 2. / mne2 / mne2 +
          (((mq2 * mq2 - mq2 * msq2) / mne2 - 7 / 3. * mq2 +
            2 / 3. * (mne2 - msq2)) *
               LAM +
           R2 / 3 + R1 + 2 / 3.) /
              del;
  ...
  case 3:
    ...
    break;
  case 4:
    CMD = ((log(msq2 / mq2) - SPMM * LAM) / 2 / mne2 - 1 / msq2 -
           mq2 * SPMM / del * LAM) /
              mne2 / mne2

          + (mq2 / mne2 / mne2 - SQ(1 - mq2 / mne2) / msq2 + 0.5 / mne2 +
             3 * mq2 / del *
                 (1 + R1 + (-R1 * mq2 - 2 * mq2 - msq2 + mne2) * LAM)) /
                del;
    break;
  case 5:
    CMD = (log(msq2 / mq2) - SPMM * LAM) / (2 * mne2 * mne2) -
          (LAM * (2 * (msq2 - mne2) + 3 * mq2 + R1 * mq2) - 3 - R1) / del;
    break;
  default:
    CMD = 0.;
  }
...
static double FeScLoop(double sgn, double mq, double msq, double mne) {
  ...
  return 1.5 * mq *
         (mq * (LintIk(1, msq, mq, mne) -
          ...
          - sgn * mne *
                (LintIk(2, msq, mq, mne) - 1. / 3. * LintIk(5, msq, mq, mne) -
                 2. / 3. * mne * mne * LintIk(4, msq, mq, mne)));
}

//  Vectors  Dark Matter  1502.02244

static double FpVectA(double M, double m1, double m2) {
  double D = Delta(M, m1, m2), L = Lambda(M, m1, m2), Mq = M * M, m1q = m1 * m1,
         m2q = m2 * m2;

  return (D * (Mq * (m2q - m1q) + m1q * (m1q + 5 * m2q)) -
          6 * m1q * m2q * ((m2q - m1q) * (m2q - m1q) - Mq * (m1q + 3 * m2q))) /
             (6 * D * D * Mq) -
         log(m1q / m2q) * m1q / (12 * Mq * Mq) +
         (D * D * (m2q + m1q - Mq) +
          2 * m2q * D *
              (5 * m2q * m2q + 20 * m1q * m2q - m1q * m1q +
               Mq * (9 * m2q + m1q)) +
          12 * m2q * m2q *
              (Mq * (m2q * m2q + 10 * m1q * m2q + 5 * m1q * m1q) -
               (m2q - m1q) * (m2q - m1q) * (m2q + 3 * m1q))) *
             m1q * L / (12 * D * D * Mq * Mq);
}
...
static double FmVectA(double M, double m1, double m2) {
  double D = Delta(M, m1, m2), L = Lambda(M, m1, m2), Mq = M * M, m1q = m1 * m1,
         m2q = m2 * m2;
  return -m2 *
             (D * (2 * m2q + m1q - 2 * Mq) + 6 * m1q * m2q * (m2q - m1q - Mq)) /
             (6 * m1 * D * D) +
         m1 * m2q * m2 * (D + m1q * (m2q - m1q + Mq)) * L / (D * D);
}

static double FpVectC(double M, double m1, double m2) {
  double D = Delta(M, m1, m2), L = Lambda(M, m1, m2), Mq = M * M, m1q = m1 * m1,
         m2q = m2 * m2;
  return -(2 * Mq * Mq - 3 * (m1q + m2q) * Mq + (m1q - m2q) * (m1q - m2q)) /
             (6 * D * Mq) +
         log(m1q / m2q) * (m1q - m2q) / (12 * Mq * Mq) +
         L *
             (D * (Mq - m1q - m2q) * (m1q + m2q) +
              4 * m1q * m2q *
                  ((m1q - m2q) * (m1q - m2q) - 2 * Mq * (m1q + m2q))) /
             (12 * D * Mq * Mq);
}

static double FpVect(double M, double m1, double m2) {
  return FpVectA(M, m1, m2) + FpVectA(M, m2, m1) + FpVectC(M, m1, m2);
}

static double FmVect(double M, double m1, double m2) {
  return FmVectA(M, m1, m2) + FmVectA(M, m2, m1);
}

static double VectFermLoop(double sign, double m1, double m2, double M) {
  return -3 * m1 / m2 *
         (FmVect(M, m1, m2) -
          sign * (FpVect(m1, m2, M) * m2 + FmVect(M, m1, m2) * m1) / M);
}
...
static double FpScalA(double M, double m1, double m2) {
  ...
  return -L * m1q * m2q * m2q * (Mq + m1q - m2q) / (D * D) -
         ((-Mq + m1q + 2 * m2q) * D + 6 * m1q * m2q * (Mq - m1q + m2q)) /
             (6 * D * D);
}
...
static double FmScalA(double M, double m1, double m2) {
  ...
  return L * m1 * m2q * m2 * (D + m1q * (Mq - m1q + m2q)) / (D * D) -
         m2 *
             ((-2 * Mq + m1q + 2 * m2q) * D -
              6 * m1q * m2q * (Mq + m1q - m2q)) /
             (6 * m1 * D * D);
}
...
static double FpScal(double M, double m1, double m2) {
  return FpScalA(M, m1, m2) + FpScalA(M, m2, m1) + FpScalC(M, m1, m2);
}
static double FmScal(double M, double m1, double m2) {
  return FmScalA(M, m1, m2) + FmScalA(M, m2, m1) + FmScalC(M, m1, m2);
}

static double ScalFermLoop(double sign, double m1, double m2, double M) {
  return -3 * m1 / m2 *
         (FmScal(M, m1, m2) +
          sign * (FpScal(M, m1, m2) * m2 - FmScal(M, m1, m2) * m1) / M);
}
...
static double twist2_subtractionFF__(double sgn, double mq, double msq,
                                     double mne) {
  double D = (msq * msq - mne * mne - mq * mq);
  double D2 = D * D - 4 * mq * mq * mne * mne;
...
int nucleonAmplitudes(char *WIMP, double *pA0, double *pA5, double *nA0,
                      double *nA5) {
  double wimpMass;
  ...
  for (wimpN = 0; wimpN < Nodd; wimpN++)
    if (strcmp(OddPrtcls[wimpN].name, WIMP) == 0 ||
        strcmp(OddPrtcls[wimpN].aname, WIMP) == 0)
      break;

  if (OddPrtcls[wimpN].spin2)
    getAuxCodesForDD(WIMP, &ccSI, &ccSD);
  else {
    getAuxCodesForDD(WIMP, &ccSI, NULL);
    ccSD = NULL;
  }
  ...
  for (II = 0; II < 2; II++) {
    ...
    for (n = 1; n <= ntot; n++) {
      ...
      for (l = 0; l < 4; l++)
        names[l] = cc->interface->pinf(n, l + 1, masses + l, pdg + l);
      if (strcmp(names[0], names[2]))
        continue;
      ...
      if (II == 0 && DDLflag && aQnum > 3) {
        if (names[0][0] != '~') {
          if (spin2 == 1) // even  color  fermion  particle
            switch (spin2Wimp) {
            case 0:
              loopFF__ = ScalFermLoop;
              break;
            case 1:
              loopFF__ = FeScLoop;
              break;
            case 2:
              loopFF__ = VectFermLoop;
              break;
            default:
              loopFF__ = NULL;
              break;
          ...
          if ((spin2 == 0 && spin2Wimp == 1) ||
              (spin2 == 1 && (spin2Wimp == 0 || spin2Wimp == 2)))
            loopFF__ = zeroloopFactor;
          else
            loopFF__ = NULL;
        }
      } else
      ...
      if (II == 0 && loopFF__ == NULL &&
          spin2 == 1) // twist-2 trace  subtraction from SD amplitude
      {
        int k;
        double diff;
      ...
      if (spin2 == 1)
        ampl = cs0 / (128 * masses[0] * masses[0] * masses[1] * masses[1]);
      else if (spin2 == 0)
        ampl = cs0 / (32 * masses[1] * masses[1]);
      else
        ampl = 0;
      ...
      {
        if (aQnum <=
            3) { /* here (ampl/3) because of normalization of SD amplitude */
        ...
        if (aQnum > 3) {
          ...
          case -3:
            ...
            case 0:
              ...
              pA0[0] +=
                  ampl * qcdNLO * wGluP * (1. / 108.) * MN / qMass / qMass / 2;
              nA0[0] +=
                  ampl * qcdNLO * wGluN * (1. / 108.) * MN / qMass / qMass / 2;
              break;
            case 1:
        ...
        if (aQnum < 6 && Twist2On) /* twist-2  SI amplitude  */
        {
          ...
          for (k = 0; k < 3; k++) {
            ...
            if (aQnum == 1) {
              pdfQnum = 2;
              gn +=
                  (*cc->interface->sqme)(n, GG, pvect, NULL, &err_code) * d4[k];
            } else if (pdfQnum == 2) {
              pdfQnum = 1;
              gn +=
                  (*cc->interface->sqme)(n, GG, pvect, NULL, &err_code) * d4[k];
...
static double FermiNDP(double r) {
  return sin(p_ * r) * r / (1 + exp((r - R_) / a_));
}
...
double FermiFF(int A, double Qfermi) {
  ...
  a_ = A_;
  p_ = Qfermi;
  return simpson(FermiNDP, 0., R_ + 5, 1.E-4, NULL) /
         simpson(FermiND0, 0., R_ + 5, 1.E-4, NULL) / p_;
}
...
static double nucleusRecoil_stat(
    double M_cdm, double (*vfv)(double), int A, int Z, double J,

    void (*Sxx)(double, double *, double *, double *),
    //      double(*S00)(double),double(*S01)(double),double(*S11)(double),

    double css, double csv00, double csv01, double csv11, double *dNdE) {
  ...
  for (i = 0, sum = 0; i < REDIM; i++) {
    ...
    dNdE[i] *=
        1 / E0 * 1.E5 * vC * vC * (rhoDM / M_cdm) * lDay * (Kg / MA) * 1.E-6;
    if (i == 0)
      sum += 1E6 * E * sqrt(eStep) * dNdE[i];
    else if (i == REDIM - 1)
      sum += 1E6 * E / sqrt(eStep) * dNdE[i];
    else
      sum += 1E6 * E * (sqrt(eStep) + 1 / sqrt(eStep)) * dNdE[i];
  }
  return sum;
}
...
static double nucleusRecoil0_stat(double (*vfv)(double), int A, int Z, double J,
                                  double Sp, double Sn, double css,
                                  double csv00, double csv01, double csv11,
                                  double *dNdE) {
  if (J) {
    double A3 = pow(A, 1. / 3.);
    RS_ = 1.7 * A3 - 0.28 -
          0.78 * (A3 - 3.8 + sqrt((A3 - 3.8) * (A3 - 3.8) + 0.2));

    S00_0 = (Sp + Sn) * (Sp + Sn) * (2 * J + 1) * (J + 1) / (4 * M_PI * J);
    S11_0 = (Sp - Sn) * (Sp - Sn) * (2 * J + 1) * (J + 1) / (4 * M_PI * J);
    S01_0 = 2 * (Sp + Sn) * (Sp - Sn) * (2 * J + 1) * (J + 1) / (4 * M_PI * J);
  }
  return nucleusRecoil_stat(Mcdm, vfv, A, Z, J, Sxx_, css, csv00, csv01, csv11,
                            dNdE);
}

static double nucleusRecoil1(char *WINP, double (*vfv)(double), int A, int Z,
                             double J,
                             void (*Sxx)(double, double *, double *, double *),
                             double *dNdE) {
  ...
  return nucleusRecoil_stat(M, vfv, A, Z, J, Sxx, css, csv00, csv01, csv11,
                            dNdE);
}

double nucleusRecoil(double (*vfv)(double), int A, int Z, double J,
                     void (*Sxx)(double, double *, double *, double *),
                     double *dNdE) {
  int i;
  double NfracCDM2;
  if (CDM1 && !CDM2)
    return nucleusRecoil1(CDM1, vfv, A, Z, J, Sxx, dNdE);
  if (!CDM1 && CDM2)
    return nucleusRecoil1(CDM2, vfv, A, Z, J, Sxx, dNdE);
  if (CDM1 && CDM2) {
    double dNdE1[REDIM];
    double r1 = 0, r2 = 0;
    if (fracCDM2 != 1)
      r1 = nucleusRecoil1(CDM1, vfv, A, Z, J, Sxx, dNdE1);
    if (fracCDM2 != 0)
      r2 = nucleusRecoil1(CDM2, vfv, A, Z, J, Sxx, dNdE);
    if (fracCDM2 == 1) {
      return r2;
    }
    if (fracCDM2 == 0) {
    ...
    for (i = 0; i < NZ; i++)
      dNdE[i] = (1 - NfracCDM2) * dNdE1[i] + NfracCDM2 * dNdE[i];
    return r1 * (NfracCDM2 - 1) + r2 * NfracCDM2;
  }
}

double nucleusRecoilAux(double (*vfv)(double), int A, int Z, double J,
                        void (*Sxx)(double, double *, double *, double *),
  ...
  LmbdP = sqrt(fabs(siP)) / sCoeff;
  if (siP < 0)
    LmbdP *= -1;
  XiP = sqrt(fabs(sdP) / 3) / sCoeff;
  if (sdP < 0)
    XiP *= -1;
  LmbdN = sqrt(fabs(siN)) / sCoeff;
  if (siN < 0)
    LmbdN *= -1;
  XiN = sqrt(fabs(sdN) / 3) / sCoeff;
  if (sdN < 0)
    XiN *= -1;
  ...
  return nucleusRecoil_stat(Mcdm, vfv, A, Z, J, Sxx, css, csv00, csv01, csv11,
                            dNdE);
}

double nucleusRecoil0Aux(double (*vfv)(double), int A, int Z, double J,
                         double Sp, double Sn, double siP, double siN,
                         double sdP, double sdN, double *dNdE) {
  double AS, AVplus, AVminus, css, csv00, csv01, csv11;
  double LmbdP, XiP, LmbdN, XiN;
  double MN = 0.939;
  double Mr = MN * Mcdm / (MN + Mcdm);
  ...
  return nucleusRecoil0_stat(vfv, A, Z, J, Sp, Sn, css, csv00, csv01, csv11,
                             dNdE);
}

double nucleusRecoil0(double (*vfv)(double), int A, int Z, double J, double Sp,
                      double Sn, double *dNdE) {
  if (J) {
    double A3 = pow(A, 1. / 3.);
    RS_ = 1.7 * A3 - 0.28 -
          0.78 * (A3 - 3.8 + sqrt((A3 - 3.8) * (A3 - 3.8) + 0.2));
...
double cutRecoilResult(double *tab, double E1, double E2) // ????
{
  ...
  if (E2 > eStep * (REDIM - 1)) {
    printf(
        "cutRecoilResult:  Maximal bound %.3E is larger than data limit %.3E\n",
        E2, eStep * (REDIM - 1));
    E2 = eStep * (REDIM - 1);
  }
  if (E1 < 0)
  ...
  for (i = i1, sum = 0; i <= i2; i++)
    sum += tab[i];
  sum -= (tab[i1] + tab[i2]) / 2;
  dx = i1 - E1 / eStep;
  if (dx > 0)
    sum += tab[i1] * dx + (tab[i1 - 1] - tab[i1]) * dx * dx / 2;
  dx = E2 / eStep - i2;
  if (dx)
    sum += tab[i2] * dx + (tab[i2 + 1] - tab[i2]) * dx * dx / 2;
...
void SxxF19(double q, double *S00, double *S01, double *S11) {
  ...
  *S00 = ((2 * J + 1) / 16 / M_PI) * 2.610 * exp(-u) *
         (p01 * p01 * (1 + be0) + p21 * p21 * (1 + be2) - 2 * be02 * p01 * p21);
  *S11 = ((2 * J + 1) / 16 / M_PI) * 2.807 * exp(-u) *
         (q01 * q01 * (1 + be0) + q21 * q21 * (1 + be2) - 2 * be02 * q01 * q21);
  *S01 = ((2 * J + 1) / 8 / M_PI) * 2.707 * exp(-u) *
         (p01 * q01 * (1 + be0) + p21 * q21 * (1 + be2) -
...
void SxxSi29(double q, double *S00, double *S01, double *S11) {
  ...
  double q01 = Q01(u), q21 = Q21(u);
  double be0 = beta0(u, upi), be2 = 2 * be0, be02 = -sqrt(2.) * be0;
  *S00 = ((2 * J + 1) / 16 / M_PI) * 0.208 * exp(-u) *
         (p01 * p01 * (1 + be0) + p21 * p21 * (1 + be2) - 2 * be02 * p01 * p21);
  *S11 = ((2 * J + 1) / 16 / M_PI) * 0.220 * exp(-u) *
         (q01 * q01 * (1 + be0) + q21 * q21 * (1 + be2) - 2 * be02 * q01 * q21);
  *S01 = ((2 * J + 1) / 8 / M_PI) * (-sqrt(0.208 * 0.220)) * exp(-u) *
         (p01 * q01 * (1 + be0) + p21 * q21 * (1 + be2) -
          be02 * (p01 * q21 + p21 * q01));
}
...
{ double q_fm=q;
  ...
  return
((2*J+1)/8/M_PI)*(-sqrt(0.208*0.220))*exp(-u)*(p01*q01*(1+be0)+p21*q21*(1+be2)
  -be02*(p01*q21+p21*q01));
}
*/
...
void SxxNa23A(double q, double *S00, double *S01, double *S11) {
  ...
  *S00 = ((2 * J + 1) / 16 / M_PI) * (0.478) * exp(-u) *
         (p01 * p01 + p21 * p21 + p23 * p23 + p43 * p43);
  *S01 = ((2 * J + 1) / 8 / M_PI) * (0.406) * exp(-u) *
         (p01 * q01 + p21 * q21 + p23 * q23 + p43 * q43);
  *S11 = ((2 * J + 1) / 16 / M_PI) * (0.346) * exp(-u) *
         (q01 * q01 + q21 * q21 + q23 * q23 + q43 * q43);
}
...
static double SSxxYYzz(double q, double *data, int nData, int A, int expKey,
                       double yN, double y_max) {
  int i;
  double q_fm = q,
         /*  y=0.25*pow((double)A,1./3.)*q_fm*q_fm, y_i,res;*/
      y = 0.25 * pow(A, 1. / 3.) * (41.467 / (45 - 25 / pow(A, 1. / 3.))) *
          q_fm * q_fm,
         y_i, res;

  if (y > y_max)
    return 0.;
  for (i = 1, res = data[0], y_i = 1; i < nData; i++) {
    y_i *= y;
    res += y_i * data[i];
  }
  res += yN / (1 + y);
...
void SxxGe73(double q, double *S00, double *S01, double *S11) {
  double data00[7] = {0.1606, -1.1052, 3.2320, -4.9245,
                      4.1229, -1.8016, 0.3211};
  double data01[7] = {-0.271006, 2.018922, -6.226466, 9.860608,
                      -8.502157, 3.800620, -0.689352};
  double data11[7] = {0.1164, -0.9228, 2.9753, -4.8709,
                      4.3099, -1.9661, 0.3624};
...
{
  double
data[7]={-0.271006,2.018922,-6.226466,9.860608,-8.502157,3.800620,-0.689352};
  return SSxxYYzz(q,data,7,73,0,0.,1.3);
}
*/
...
void SxxNb93(double q, double *S00, double *S01, double *S11) {
  ...
  double s11[9] = {0.14,  0.085,  0.053, 0.034, 0.02,
                   0.012, 0.0052, 0.003, 0.0025};
  double x = q * 0.197327;
  x *= x;
  if (x > 0.03) {
    *S00 = 0;
...
void SxxXe131B(double q, double *S00, double *S01, double *S11) {
  double q2[11] = {0,     0.0025, 0.005, 0.01, 0.015, 0.02,
                   0.025, 0.03,   0.04,  0.05, 0.06};
  double s00[11] = {0.04,   0.0215, 0.014, 0.01,   0.009, 0.008,
                    0.0075, 0.0066, 0.005, 0.0035, 0.0017};
  double s01[11] = {-0.056, -0.028, -0.019, -0.013, -0.01, -0.009,
                    -0.008, -0.007, -0.005, -0.003, -0.001};
  double s11[11] = {0.020,  0.009,  0.006,  0.004,  0.003, 0.0027,
                    0.0025, 0.0023, 0.0019, 0.0015, 0.001};
  double x = q * 0.197327;
  x *= x;
  if (x > 0.06) {
    *S00 = 0;
...
void SxxXe131Me(double q, double *S00, double *S01,
                double *S11) // Menendez et al.;  1208.1094
{
  double data00[10] = {0.0417889,  -0.111171,   0.171966,   -0.133219,
                       0.0633805,  -0.0178388,  0.00282476, -2.31681E-4,
                       7.78223E-6, -4.49287E-10};
  double data11[10] = {0.022446,   -0.0733931, 0.110509,   -0.0868752,
                       0.0405399,  -0.0113544, 0.00187572, -1.75285E-4,
                       8.40043E-6, -1.53632E-7};
  double data01[10] = {-0.0608808,  0.181473,  -0.272533,  0.211776,
                       -0.0985956,  0.027438,  -0.0044424, 3.97619E-4,
                       -1.74758E-5, 2.55979E-7};

  double b = 2.2905; // fm
  double u = q * q * b * b / 2;

  *S00 = data00[0], *S01 = data01[0];
  *S11 = data11[0];
  double un = u;
  for (int i = 1; i < 10; i++) {
    *S00 += un * data00[i];
...
double S00Xe131B(double q)
{
 ...
 double
s00[11]={0.04,0.0215,0.014,0.01,0.009,0.008,0.0075,0.0066,0.005,0.0035,0.0017};
 double x=q*0.197327;
 x*=x;
 if(x>0.06) return 0;
...
double S11Xe131B(double q)
{
 double q2[11]= {0,0.0025,0.005,0.01,0.015,0.02,0.025,0.03,0.04,0.05,0.06};
 double
s11[11]={0.020,0.009,0.006,0.004,0.003,0.0027,0.0025,0.0023,0.0019,0.0015,0.001};
 double x=q*0.197327;
 x*=x;
 if(x>0.06) return 0;
...
double S01Xe131B(double q)
{
 double q2[11]= {0,0.0025,0.005,0.01,0.015,0.02,0.025,0.03,0.04,0.05,0.06};
 double s01[11]={
-0.056,-0.028,-0.019,-0.013,-0.01,-0.009,-0.008,-0.007,-0.005,-0.003,-0.001 };
 double x=q*0.197327;
 x*=x;
 if(x>0.06) return 0;
...
void SxxTe125(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.03971,  -0.19610, 0.47265,  -0.65023, 0.54193,
                      -0.26456, 0.07489,  -0.01146, 0.00075};
  double data01[9] = {-0.07894, 0.42738,  -1.09331, 1.55324, -1.28933,
                      0.61844,  -0.16964, 0.02481,  -0.00152};
  double data11[9] = {0.03922,  -0.22938, 0.62215,  -0.92253, 0.78465,
                      -0.38245, 0.1057,   -0.01542, 0.00093};
...
{
 double data[9]={0.03971,-0.19610, 0.47265,-0.65023, 0.54193,-0.26456,
0.07489,-0.01146,0.00075}; return SSxxYYzz(q,data,9,125,1,0.,10.);
}
double S11Te125(double q)
{
 double data[9]={
0.03922,-0.22938,0.62215,-0.92253,0.78465,-0.38245,0.1057,-0.01542,0.00093 };
 return SSxxYYzz(q,data,9,125,1,0.,10.);
}
double S01Te125(double q)
{
 double data[9]={-0.07894, 0.42738,-1.09331, 1.55324,-1.28933, 0.61844,-0.16964,
0.02481,-0.00152 }; return SSxxYYzz(q,data,9,125,1,0.,10.);
}
*/
...
void SxxI127(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.0983,  -0.4891, 1.1402,  -1.4717, 1.1717,
                      -0.5646, 0.1583,  -0.0239, 0.0015};
  double data01[9] = {0.1199,  -0.6184, 1.5089,  -2.0737, 1.7731,
                      -0.9036, 0.2600,  -0.0387, 0.0024};
  double data11[9] = {0.0366,  -0.1950, 0.5049,  -0.7475, 0.7043,
                      -0.3930, 0.1219,  -0.0192, 0.0012};
...
{
 double
data[9]={0.0983,-0.4891,1.1402,-1.4717,1.1717,-0.5646,0.1583,-0.0239,0.0015};
 return SSxxYYzz(q,data,9,127,1,0.,10.);
}
double S11I127(double q)
{
 double
data[9]={0.0366,-0.1950,0.5049,-0.7475,0.7043,-0.3930,0.1219,-0.0192,0.0012};
 return SSxxYYzz(q,data,9,127,1,0.,10.);
}
double S01I127(double q)
{
 double
data[9]={0.1199,-0.6184,1.5089,-2.0737,1.7731,-0.9036,0.2600,-0.0387,0.0024};
 return SSxxYYzz(q,data,9,127,1,0.,10.);
}
*/
...
void SxxXe129(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.07132,  -0.34478, 0.75590,  -0.93345, 0.69006,
                      -0.30248, 0.07653,  -0.01032, 0.00057};
  double data01[9] = {-0.12166, 0.64435,  -1.52732, 2.02061, -1.57689,
                      0.72398,  -0.19040, 0.02638,  -0.00149};
  double data11[9] = {-2.05825, 1.80756,  -1.27746, 0.65459, -0.22197,
                      0.04546,  -0.00427, -0.00014, 0.00004};
...
void SxxXe129M(double q, double *S00, double *S01, double *S11) {
  double data00[10] = {0.054731,   -0.146897,  0.182479,   -0.128112,
                       0.0539978,  -0.0133335, 0.00190579, -1.48373E-4,
                       5.11732E-6, -2.06597E-8};
  double data11[10] = {0.02933,    -0.0905396, 0.122783,   -0.0912046,
                       0.0401076,  -0.010598,  0.00168737, -1.56768E-4,
                       7.69202E-6, -1.48874E-7};
  double data01[10] = {-0.0796645,  0.231997,  -0.304198,   0.222024,
                       -0.096693,   0.0251835, -0.00392356, 3.53343E-4,
                       -1.65058E-5, 2.88576E-7};
...
{
 double data[9]={0.07132,-0.34478, 0.75590,-0.93345, 0.69006,-0.30248,
0.07653,-0.01032, 0.00057}; return SSxxYYzz(q,data,9,129,1,0.,10.);
}
double S11Xe129(double q)
{
 double data[9]={-2.05825, 1.80756,-1.27746, 0.65459,-0.22197,
0.04546,-0.00427,-0.00014, 0.00004}; return
SSxxYYzz(q,data,9,129,1,2.11016,10.);
}
double S01Xe129(double q)
{
 double data[9]={-0.12166, 0.64435,-1.52732, 2.02061,-1.57689, 0.72398,-0.19040,
0.02638,-0.00149}; return SSxxYYzz(q,data,9,129,1,0.,10.);
}
...
void SxxXe131(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.02964,  -0.13343, 0.37799,  -0.57961, 0.57890,
                      -0.34556, 0.11595,  -0.02012, 0.00142};
  double data01[9] = {-0.05455, 0.27176,  -0.72302, 1.05450, -0.97133,
                      0.53842,  -0.16899, 0.02742,  -0.00181};
  double data11[9] = {0.02510,  -0.13772, 0.36661,  -0.53851, 0.49255,
                      -0.26990, 0.08369,  -0.01340, 0.00087};
...
{
 double data[9]={ 0.02964,-0.13343, 0.37799,-0.57961, 0.57890,-0.34556,
0.11595,-0.02012, 0.00142}; return SSxxYYzz(q,data,9,131,1,0.,10.);
}
double S11Xe131(double q)
{
 double data[9]={ 0.02510,-0.13772,0.36661,-0.53851, 0.49255,-0.26990,
0.08369,-0.01340, 0.00087}; return SSxxYYzz(q,data,9,131,1,0.,10.);
}
double S01Xe131(double q)
{
 double data[9]={-0.05455, 0.27176,-0.72302, 1.05450,-0.97133, 0.53842,-0.16899,
0.02742,-0.00181}; return SSxxYYzz(q,data,9,131,1,0.,10.);
}
*/
...
void SxxTe125A(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.04960,  -0.24777, 0.54766,  -0.66553, 0.47462,
                      -0.19944, 0.04819,  -0.00616, 0.00032};
  double data01[9] = {-0.09939, 0.54303,  -1.28816, 1.67206, -1.26883,
                      0.56728,  -0.14545, 0.01959,  -0.00107};
  double data11[9] = {-1.92941, 1.68075,  -1.16336, 0.58650, -0.20730,
                      0.05141,  -0.00870, 0.00087,  0.00004};
  *S00 = SSxxYYzz(q, data00, 9, 125, 1, 0., 10.);
  *S01 = SSxxYYzz(q, data01, 9, 125, 1, 0., 10.);
...
{double
data[9]={0.04960,-0.24777,0.54766,-0.66553,0.47462,-0.19944,0.04819,-0.00616,0.00032};
 return SSxxYYzz(q,data,9,125,1,0.,10.);
}
double S11Te125A(double q)
{
 double data[9]={
-1.92941,1.68075,-1.16336,0.58650,-0.20730,0.05141,-0.00870,0.00087,0.00004};
 return SSxxYYzz(q,data,9,125,1,1.97923,10.);
}
double S01Te125A(double q)
{
 double
data[9]={-0.09939,0.54303,-1.28816,1.67206,-1.26883,0.56728,-0.14545,0.01959,-0.00107};
 return SSxxYYzz(q,data,9,125,1,0.,10.);
}
*/
/*============================ I127 =======================*/
...
void SxxI127A(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.1166,  -0.5721, 1.3380,  -1.7252, 1.3774,
                      -0.6700, 0.1905,  -0.0292, 0.0019};
  double data01[9] = {0.1621,  -0.8363, 2.0594,  -2.8319, 2.3973,
                      -1.2121, 0.3486,  -0.0522, 0.0032};
  double data11[9] = {0.0563,  -0.3038, 0.7948,  -1.1703, 1.0637,
                      -0.5713, 0.1722,  -0.0266, 0.0017};
  *S00 = SSxxYYzz(q, data00, 9, 127, 1, 0., 10.);
  *S01 = SSxxYYzz(q, data01, 9, 127, 1, 0., 10.);
  *S11 = SSxxYYzz(q, data11, 9, 127, 1, 0., 10.);
}
...
{
 double
data[9]={0.1166,-0.5721,1.3380,-1.7252,1.3774,-0.6700,0.1905,-0.0292,0.0019};
 return SSxxYYzz(q,data,9,127,1,0.,10.);
}
double S11I127A(double q)
{
 double
data[9]={0.0563,-0.3038,0.7948,-1.1703,1.0637,-0.5713,0.1722,-0.0266,0.0017};
 return SSxxYYzz(q,data,9,127,1,0.,10.);
}
double S01I127A(double q)
{
 double
data[9]={0.1621,-0.8363,2.0594,-2.8319,2.3973,-1.2121,0.3486,-0.0522,0.0032};
 return SSxxYYzz(q,data,9,127,1,0.,10.);
}
*/
...
void SxxXe129A(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.04649,  -0.22551, 0.49905,  -0.62244, 0.46361,
                      -0.20375, 0.05109,  -0.00671, 0.00036};
  double data01[9] = {-0.08538, 0.45343,  -1.06546, 1.38670, -1.05940,
                      0.47576,  -0.12208, 0.01643,  -0.00089};
  double data11[9] = {-1.28214, 1.09276, -0.71295, 0.31489, -0.08351,
                      0.01059,  0.00023, -0.00024, 0.00002};
...
{
 double
data[9]={0.04649,-0.22551,0.49905,-0.62244,0.46361,-0.20375,0.05109,-0.00671,0.00036};
 return SSxxYYzz(q,data,9,129,1,0.,10.);
}
double S11Xe129A(double q)
{
 double
data[9]={-1.28214,1.09276,-0.71295,0.31489,-0.08351,0.01059,0.00023,-0.00024,0.00002};
 return SSxxYYzz(q,data,9,129,1,1.32136,10.);
}
double S01Xe129A(double q)
{
 double
data[9]={-0.08538,0.45343,-1.06546,1.38670,-1.05940,0.47576,-0.12208,0.01643,-0.00089};
 return SSxxYYzz(q,data,9,129,1,0.,10.);
}
*/
...
void SxxXe131A(double q, double *S00, double *S01, double *S11) {
  double data00[9] = {0.02773,  -0.12449, 0.32829,  -0.48140, 0.47565,
                      -0.28518, 0.09682,  -0.01710, 0.00124};
  double data01[9] = {-0.04978, 0.24725,  -0.63231, 0.89642, -0.81645,
                      0.45235,  -0.14267, 0.02335,  -0.00156};
  double data11[9] = {0.02234,  -0.12206, 0.31949,  -0.46695, 0.42877,
                      -0.23679, 0.07408,  -0.01197, 0.00079};
...
{
 double
data[9]={0.02773,-0.12449,0.32829,-0.48140,0.47565,-0.28518,0.09682,-0.01710,0.00124};
 return SSxxYYzz(q,data,9,131,1,0.,10.);
}
double S11Xe131A(double q)
{
 double
data[9]={0.02234,-0.12206,0.31949,-0.46695,0.42877,-0.23679,0.07408,-0.01197,0.00079};
 return SSxxYYzz(q,data,9,131,1,0.,10.);
}
double S01Xe131A(double q)
{
 double
data[9]={-0.04978,0.24725,-0.63231,0.89642,-0.81645,0.45235,-0.14267,0.02335,-0.00156};
 return SSxxYYzz(q,data,9,131,1,0.,10.);
}
*/
...
static double _fPb_integrand(double x) {
  ...
  return exp(-u / 2) *
         (1. - u * (5. / 3. -
                    u * (17. / 15. -
                         u * (31. / 105. - u * (9. / 280. - u * 1. / 840.)))));
}

void SxxPb207(double q, double *S00, double *S01, double *S11) {
  _u_ = 0.5 * pow(207., 1. / 3.) * (q) * (q);
  *S00 = ((2 * 0.5 + 1) / 16 / M_PI) * 0.305 *
         simpson(_fPb_integrand, 0., 1., 1.E-4, NULL);
