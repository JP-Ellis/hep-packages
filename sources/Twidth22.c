...
double tWidth21(char *name, double T) {
  ...
  if (cc) {
    ...
    for (nsub = 1; nsub <= ntot; nsub++) {
      ...
      double pRestOut =
          sqrt((m[pIn] * m[pIn] - ms * ms) * (m[pIn] * m[pIn] - md * md)) /
          (2 * m[pIn]);
      ...
      if ((spin2[0] & 1) != (spin2[pIn] & 1))
        width *= -1;
      double tf = m[pIn] * m[pIn] / m[0] / m[0];
      if (m[0] != 0) {
        if (m[pIn] / T > 20)
          tf *= exp(-m[pIn] / T + m[0] / T) * sqrt(m[0] / m[pIn]);
        else {
          if (m[pIn] / T < 0.01)
            tf /= m[pIn] / T;
          else
  ...
  if (cc) {
    ...
    for (nsub = 1; nsub <= ntot; nsub++) {
      ...
      width = totcoef * (cc->interface->sqme)(nsub, GG, pvect, NULL, &err_code);
      double tf =
          m[0] * m[0] / m[1] / m[1] * bessK1(m[0] / T) / bessK1(m[1] / T);
      if (p[1] == p[2])
        tf *= 2;
      //       printf("width(%s -> %s,%s)=%e  tWidth=%E  \n", p[2],p[0],p[1],
      //       width*ndf[0]*ndf[1]/(double)ndf[2], width*tf);
