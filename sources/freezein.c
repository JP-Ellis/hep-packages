...
typedef struct {
  double m1, m2, m3, s1, s2, s3, pcm, e2, e3, e2_, e3_, gamma;
} K1to2_struct;

//=====================  SECTION:  Function for Statistics.    Stat2,  K1. K2,
//gammaLor
...
static double K1to2_int(double x, void *par_) {
  ...
  if (E2m > eps)
    ...
    D2m = (1 + s2) - s2 * E2m * (1 - E2m);
  if (E3m > eps)
    D3m = 1 + s3 * expl(-E3m);
  else
    D3m = (1 + s3) - s3 * E3m * (1 - E3m);
  if (E2p > eps)
    D2p = 1 + s2 * expl(-E2p);
  else
    D2p = (1 + s2) - s2 * E2p * (1 - E2p);
  if (E3p > eps)
    D3p = 1 + s3 * expl(-E3p);
  else
    D3p = (1 + s3) - s3 * E3p * (1 - E3p);
...
double K1to2(double m1, double m2, double m3, double s1, double s2, double s3) {
  ...
  if (s2 * s3 <= 0 || s2 > 0)
    ...
    delta23 = 1 / s2 / s3 - exp(-m1);
  else
    delta23 = (1 - s2 * s3) / s2 / s3 + m1;
  ...
  if (delta23 > 0.1)
    ...
    gamma23 = 1;
    for (int i = 0; i < 5; i++)
      gamma23 = log(gamma23 * delta23) / log(0.05);
  }

  double gamma123;
  if (delta1 > 0.1 || delta23 > 0.1)
    gamma123 = 0;
  else {
  ...
  if (err) {
    printf("Problem in K1to2(%e, %e, %e, %E, %E, %E) err=%d File  freezein.c, "
           "line 134)\n",
...
double Stat2(double P, double M, double m1, double m2, double eta1,
             double eta2) {
#ifdef NOSTATISTICS
  return 1;
#else
  ...
  double ex_1 = eta1 * exp(-cht * e1), ex_2 = eta2 * exp(-cht * e2),
         ex_p = exp(-sht * pcm);

  double f;
  if (pcm * sht < 1E-5 * (m1 + m2) * cht)
    f = 1 + ex_1 / (1 - ex_1) + ex_2 / (1 - ex_2);
  else
    f = 1 + log((1 - ex_1 * ex_p) * (1 - ex_2 * ex_p) / (1 - ex_1 / ex_p) /
                (1 - ex_2 / ex_p)) /
                (2 * pcm * sht);
...
static double K2_int(double y, void *arg) {
  ...
  double x = log(y);
  double J = 1 / y;
  return J * x * x / (exp(sqrt(x * x + mu * mu)) - s) *
         pow(mu / sqrt(x * x + mu * mu), n);
}
...
static double K2prime(double mu, double s) {
  ...
  if (err) {
    printf("Warning from Simpson (freezein.c line 193) code=%d. Precison is "
           "not reached\n",
           err);
#ifdef ERROR_PLOT
...
static double gammaLor(double mu, double s) {
  ...
  if ((s <= 1 && s >= -1) || mu > 20) {
    ...
    if (s <= -1)
      s = -1;
    double fb[21] = {1,          1.1927E+00, 1.7945E+00, 2.7716E+00, 4.1010E+00,
                     5.7833E+00, 7.8218E+00, 1.0219E+01, 1.2977E+01, 1.6097E+01,
                     1.9579E+01, 2.3425E+01, 2.7635E+01, 3.2209E+01, 3.7147E+01,
                     4.2450E+01, 4.8118E+01, 5.4150E+01, 6.0548E+01, 6.7310E+01,
                     7.4438E+01};
    double fm[21] = {1,          1.1927E+00, 1.8143E+00, 2.9269E+00, 4.5580E+00,
                     6.7126E+00, 9.3876E+00, 1.2579E+01, 1.6282E+01, 2.0494E+01,
                     2.5214E+01, 3.0439E+01, 3.6168E+01, 4.2400E+01, 4.9136E+01,
                     5.6373E+01, 6.4112E+01, 7.2352E+01, 8.1094E+01, 9.0337E+01,
                     1.0008E+02};
    double ff[21] = {1,          1.1927E+00, 1.8322E+00, 3.0401E+00, 4.8384E+00,
                     7.2169E+00, 1.0166E+01, 1.3679E+01, 1.7751E+01, 2.2380E+01,
                     2.7563E+01, 3.3298E+01, 3.9585E+01, 4.6422E+01, 5.3810E+01,
                     6.1748E+01, 7.0235E+01, 7.9271E+01, 8.8856E+01, 9.8990E+01,
                     1.0967E+02};
    double x0[21] = {0,          1.2500E-01, 5.0000E-01, 1.1250E+00, 2.0000E+00,
                     3.1250E+00, 4.5000E+00, 6.1250E+00, 8.0000E+00, 1.0125E+01,
                     1.2500E+01, 1.5125E+01, 1.8000E+01, 2.1125E+01, 2.4500E+01,
                     2.8125E+01, 3.2000E+01, 3.6125E+01, 4.0500E+01, 4.5125E+01,
                     5.0000E+01};

    double be[3];
    if (t > 50) {
      be[0] = fb[20] + 1.461824 * (t - 50);
      be[1] = fm[20] + 2 * (t - 50);
      be[2] = ff[20] + 2.191793 * (t - 50);
    } else {
      be[0] = polint3(t, 21, x0, fb);
      be[1] = polint3(t, 21, x0, fm);
      be[2] = polint3(t, 21, x0, ff);
    }

    return 0.5 * (1 + s) * s * be[0] + (1 - s) * (1 + s) * be[1] +
           0.5 * s * (s - 1) * be[2];
  }
...
static double gammaLor_(double mu) {
  ...
  if (err) {
    printf("Warning from Simpson. Precison is not reached (freezein.c line 235 "
           "code=%d)\n",
           err);
#ifdef ERROR_PLOT
    displayPlot("Error in simpson", "x", 0, 1, 0, 1, "K2_int", 0, K2_int, arg);
  ...
  if (err) {
    printf("Warning from Simpson. Precison is not reached (freezein.c line 245 "
           "code=%d)\n",
           err);
#ifdef ERROR_PLOT
    displayPlot("Error in simpson", "x", 0, 1, 0, 1, "K2_int", 0, K2_int, arg);
...
static double decayIntegrand(double lnT, void *arg_) {
  ...
  double H = sqrt(8 * M_PI / 3. * M_PI * M_PI / 30. * gEff(T)) * T * T /
             MPlank; // Hubble
...
double decayAbundance(double TR, double M, double w, int Ndf, double eta,
                      int plot) {
  double Tmin = M / 100;
  double arg[2] = {eta, M};
  int err;
  ...
  if (plot) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      arr[i] = decayIntegrand(log(T), &arg) * w * Ndf * fabs(eta) * M * M /
               (M_PI * M_PI) * log(10);
    }
    char txt[50];
  ...
  double res =
      w * Ndf * fabs(eta) * M * M / (M_PI * M_PI) *
      simpson_arg(decayIntegrand, &arg, log(Tmin), log(TR), 0.001, &err);
  if (err) {
    printf("Warning from Simpson. Precison is not reached (freezein.c line 287 "
           "code=%d)\n",
           err);
#ifdef ERROR_PLOT
    displayPlot("Error in simpson", "x", log(Tmin), log(TR), 0, 1,
                "decayIntegrand", 0, decayIntegrand, &arg);
    exit(0);
#endif
  }
  return res;
...
{
  double mu = M / T;
  if (eta && log(fabs(eta) * 1E4) < mu)
    eta = 0;
  if (mu < 25 && fabs(eta) > 0)
    return mu * mu / ((2 * M_PI * M_PI) * (2 * M_PI * M_PI / 45) * hEff(T)) *
           K2prime(mu, eta);
  else
    return mu * mu / ((2 * M_PI * M_PI) * (2 * M_PI * M_PI / 45) * hEff(T)) *
           K2pol(1 / mu) * exp(-mu) * sqrt(M_PI / 2 / mu);
}
...
static double getEta(double Y, double T, double M, int s) {
  ...
  if (eta < exp(mu) * 1E-3)
    return s * eta;
  for (int i = 0; i < 3; i++) {
    if (s * eta > exp(mu) * 0.999)
      eta = exp(mu) * 0.999;
    eta = Y / Yeq_1(T, M, s * eta);
  }
  if (s * eta > exp(mu)) {
...
static int getDecBrKE(double T, txtList L, double eta, double *brTot,
                      double *brBath, double *brSig1, double *brSig2,
                      int *nFiCh) {
  *brBath = 0;
  *brTot = 0;
  *brSig1 = 0;
  *brSig2 = 0;
  ...
  for (txtList l = L; l; l = l->next) {
    char name[4][20];
    double br;
    if (4 < sscanf(l->txt, "%lf %s -> %[^,], %[^,], %[^,]", &br, name[0],
    ...
    //     if(mu[0]<25)  printf("mu=%E s[1]=%d,s[2]=%d eta=%e
    //     K=%E\n",mu[0],s[1],s[2], eta,
    //     K1to2(mu[0],mu[1],mu[2],eta,s[1],s[2])/K1to2(mu[0],0,0,eta,0,0));
    if ((s[1] || s[2]) && mu[0] < 25)
      br *= K1to2(mu[0], mu[1], mu[2], eta, s[1], s[2]) /
            K1to2(mu[0], 0, 0, eta, 0, 0);
    *brTot += br;
    if (s[1] && s[2])
      *brBath += br;
    {
      ...
      if ((*brSig1 + *brSig2) > 0 && nFiCh) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          if (n > 0)
            omegaFiCh[nFiCh_].prtcl[i] = ModelPrtcls[n - 1].name;
          else
            omegaFiCh[nFiCh_].prtcl[i] = ModelPrtcls[-n - 1].aname;
        }
        for (int i = 3; i < 5; i++)
          omegaFiCh[nFiCh_].prtcl[i] = NULL;
        nFiCh_++;
  ...
  if (nFiCh) {
    double s = 0;
    for (int i = *nFiCh; i < nFiCh_; i++)
      s += omegaFiCh[i].weight;
    for (int i = *nFiCh; i < nFiCh_; i++)
      omegaFiCh[i].weight /= s;
    *nFiCh = nFiCh_;
  }
  return 0;
}
...
static double derEq0(double lnT, void *vi) {
  ...
  if (*i == 1)
    ...
    return medWidth * brSig2 * Y / gammaLor(medMass / T, medEta) * dtdLnT;
}

static int getDecBr(double T, double P, txtList L, double *brTot,
                    double *brBath, double *brSig1, double *brSig2) {
  *brBath = 0;
  *brTot = 0;
  *brSig1 = 0;
...
static void der3(double lnT, double *y, double *dy) {
  ...
  dy[0] = -medMass / E * medWidth *
          (brBath * exp(-E / T) * (1 + medEta * f) - brTot * f) * dlnTdt;
...
static double derEq(double lnT, void *ai) {
  ...
  double dlnTdt = (1 + hEffLnDiff(T) / 3) / Hubble(T);
  if (*i_ == 1)
    return -medMass / E * medWidth * brSig1 * feq * dlnTdt;
  else
    return -medMass / E * medWidth * brSig2 * feq * dlnTdt;
}

static double darkOmegaFiDecayNotKE(double TR, char *pname, int nStep,
                                    double *Ym, double *Y1, double *Y2) {
  int N = 100;
  double f[100][3];
  ...
  for (int k = 0; k < nStep; k++) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      double df[2];
      derEst(Tnext, df);
      if (fabs(df[0]) * log(step) <= 100)
        odeint(f[i], 3, log(T), log(Tnext), 1E-3, 1, der3);
      else {
        int n;
        n = 1;
        f[i][1] += gauss_arg(derEq, &n, log(T), log(Tnext), 3);
        n = 2;
        f[i][2] += gauss_arg(derEq, &n, log(T), log(Tnext), 3);
        f[i][0] = df[1];
      }
    }
  ...
  */
  ...
  double coeff = medNDF * 2 / (2 * M_PI) / (2 * M_PI) * C / N / hEff(T0) / T0 /
                 T0 / (2 * M_PI * M_PI / 45);
  if (Ym)
    for (int k = 0; k <= nStep; k++) {
      Ym[k] *= coeff;
...
double darkOmegaFiDecay(double TR, char *pname, int KE, int plot) {
  if (TR < 10 * Tzero) {
    printf("Reheating temperature is  too small\n");
    return 0;
  }
  int tabPos = pTabPos(pname);
  if (tabPos == 0) {
    printf(" '%s' unknown particle\n", pname);
    return 0;
  }
  tabPos = abs(tabPos) - 1;
  medNDF = (ModelPrtcls[tabPos].cdim) * (ModelPrtcls[tabPos].spin2 + 1);
  if (strcmp(ModelPrtcls[tabPos].name, ModelPrtcls[tabPos].aname))
    medNDF *= 2; // particle +antiparticle

  medEta = ModelPrtcls[tabPos].spin2 & 1 ? -1 : 1;

  int VZdecayMem = VZdecay, VWdecayMem = VWdecay;
  VZdecay = 0;
  VWdecay = 0;
  ...
  double step = exp(log(TR / T0) / nStep);
  double omega;
  if (isFeeble(pname) && !KE)
    omega = darkOmegaFiDecayNotKE(TR, pname, nStep, Ym, Y1, Y2);
  else {
    double eta;
    double T = TR;
    int s0, nT;
    ...
    for (nT = tGridDim - 2, T = TR; nT >= 0; nT--) {
      ...
      double H = sqrt(8 * M_PI / 3. * M_PI * M_PI / 30. * gEff(Tnext)) * Tnext *
                 Tnext / MPlank;                         // Huble
      ...
      getDecBrKE(Tnext, medDecayList, eta, &brTot, &brBath, &brSig1, &brSig2,
                 NULL);
      ...
      if (s0) {
        ...
        n = 1;
        y[1] += gauss_arg(derEq0, &n, log(Tnext), log(T), 3);
        n = 2;
        y[2] += gauss_arg(derEq0, &n, log(Tnext), log(T), 3);
      } else {
        double a = log(step) * medWidth * brTot * dtdLnT /
                   gammaLor(medMass / Tnext, eta);
        if (a > 100) // Y=Yeq up to beta factor;
        {
          ...
          if (etaOn)
            eta = getEta(y[0], Tnext, medMass, 1);
        }
      }
      if (plot) {
        Ym[nT] = y[0];
  ...
  if (plot) {
    char mess[100];
    if (!isFeeble(pname))
      sprintf(mess, "Thermal bath %s, mass=%.2E, width=%.2E", pname,
              pMass(pname), pWidth(pname, NULL));
    else {
      sprintf(mess, "Feeble %s,  mass=%.2E, width=%.2E", pname, pMass(pname),
              pWidth(pname, NULL));
      if (KE)
        strcat(mess, " [kinetic equilibrium]");
      else
        strcat(mess, " [ No kin. equilibrium]");
    }
    int y1ok = 0, y2ok = 0;
    for (int i = 0; i < tGridDim; i++) {
      if (Y1[i])
    ...
    if (y1ok)
      for (int i = 0; i < tGridDim; i++)
        Y1[i] *= Mcdm1 * EntropyNow / RhoCrit100;
    if (y2ok)
      for (int i = 0; i < tGridDim; i++)
        Y2[i] *= Mcdm2 * EntropyNow / RhoCrit100;
    ...
    if (y1ok && y2ok)
      displayPlot(mess, "T", T1, T2, 1, 2, "Omg1", tGridDim, Y1, NULL, "Omg2",
                  tGridDim, Y2, NULL);
    else if (y1ok)
      displayPlot(mess, "T", T1, T2, 1, 1, "Omg1", tGridDim, Y1, NULL);
    else if (y2ok)
      displayPlot(mess, "T", T1, T2, 1, 1, "Omg2", tGridDim, Y2, NULL);
...
static void getGauss(int *nG, double **xG, double **fG) {
  ...
  static double X4[4] = {6.943185E-02, 3.300095E-01, 6.699905E-01,
                         9.305682E-01};
  static double F4[4] = {1.739274E-01, 3.260726E-01, 3.260726E-01,
                         1.739274E-01};
...
static void cleanMediatorArr(void) {
  for (int i = 0; i < nMed; i++) {
    if (mediatorArr[i].wBath)
      free(mediatorArr[i].wBath);
    if (mediatorArr[i].wSig)
      free(mediatorArr[i].wSig);
    if (mediatorArr[i].wEff)
      free(mediatorArr[i].wEff);
    if (mediatorArr[i].mass)
      free(mediatorArr[i].mass);
    if (mediatorArr[i].wIwD)
      free(mediatorArr[i].wIwD);
    if (mediatorArr[i].eta)
      free(mediatorArr[i].eta);
    if (mediatorArr[i].fKK)
      free(mediatorArr[i].fKK);
  }
  free(mediatorArr);
  mediatorArr = NULL;
  nMed = 0;
...
static void fillMediatorArr(double Tr, par_22 *arg) {
  ...
  for (int n = 1;; n++) {
    ...
    if (s[0] == 1 && s[1] == 2 && m && w) {
      ...
      mediatorArr[nMed].wBath = mediatorArr[nMed].wEff = mediatorArr[nMed].eta =
          mediatorArr[nMed].mass = mediatorArr[nMed].wIwD = NULL;
      mediatorArr[nMed].fKK = NULL;
      mediatorArr[nMed].wSig = NULL;

      mediatorArr[nMed].widthPos = w; // position in local 2->2 code
      for (int i = 0; i < nModelVars + nModelFunc; i++)
        if (strcmp(varNames[i], arg->cc->interface->varName[m]) == 0) {
          m = i;
          break;
        }
  ...
  for (nT = tGridDim - 1, T = Tr; nT >= 0; nT--, T /= step) {
    ...
    for (int n = 0; n < nMed; n++)
      if (mediatorArr[n].my == -1) {
        txtList L;
        double Mm = varValues[mediatorArr[n].massPos];
        mediatorArr[n].mass[nT] = Mm;
        double wI = 0, wD = 0, eta, eta_;
        if (nT == tGridDim - 1) {
          if (isFeeble(mediatorArr[n].name))
            eta_ = 0;
          else if (mediatorArr[n].spin2 & 1)
            eta_ = -1;
          else
            eta_ = 1;
        } else
          eta_ = mediatorArr[n].eta[nT + 1];
        ...
        for (int k = (nT == tGridDim - 1) ? 3 : 1; k; k--) {
          if (w > 0)
            for (txtList l = L; l; l = l->next) {
              ...
              int sig = 0;
              if ((pdgD[1] == pdg[0] && pdgD[2] == pdg[1]) ||
                  (pdgD[1] == pdg[1] && pdgD[2] == pdg[0]))
                wI = w * br;
              else if ((pdgD[1] == pdg[2] && pdgD[2] == pdg[3]) ||
                       (pdgD[1] == pdg[3] && pdgD[2] == pdg[2])) {
                wD = w * br;
                sig = 1;
              }
              if ((s[1] || s[2]) && mu[0] < 25)
          ...
          if (wBath > 0) {
            double mu = Mm / T;
            double alpha = wBath / Hubble(T) / gammaLor(mu, eta) * log(2),
                   eAlpha;
            double beta = wBath_ / wBath;

            // printf("T=%.2E wBath=%.2E H=%.2E massPos=%d    Mm=%.2E    mu=%.2E
            // eta=%E gammaLor=%.2E  alpha=%.2E \n", T, wBath, Hubble(T),
            // mediatorArr[n].massPos,  Mm, mu, eta,gammaLor(mu ,eta),alpha);
            if (alpha > 20)
              eAlpha = 0;
            else
              eAlpha = exp(-alpha);
            ...
            if (mu < 0.1)
              eta = beta * (1 - eAlpha) + fabs(eta_) * eAlpha;
            else if (mu < 10)
              eta = beta * (1 - eAlpha) + fabs(eta_) * eAlpha *
                                              bessK2(mu / step) / bessK2(mu) /
                                              step / step;
            else {
              double delta = T / Mm - alpha;
              if (delta < -20)
                eta = beta * (1 - eAlpha);
              eta = beta * (1 - eAlpha) + fabs(eta_) * exp(delta) * M_SQRT2 *
                                              K2pol(step / mu) / K2pol(1 / mu) /
                                              step / step;
            }
            if (mediatorArr[n].spin2 & 1)
              eta *= -1;
          }
        }
        ...
        // printf("T=%E eta=%E wSig=%E wBath=%E \n", T, mediatorArr[n].eta[nT],
        // mediatorArr[n].wSig[nT],mediatorArr[n].wBath[nT]);
        //        mediatorArr[n].my=1;
      }
  }
  ...
  for (nT = tGridDim - 1; nT >= 0; nT--, T /= 2)
    for (int n = 0; n < nMed; n++)
      if (mediatorArr[n].my) {
        double Mm = mediatorArr[n].mass[nT];

        if (T < Mm / 25)
          mediatorArr[n].fKK[nT] = 1;
        else {
          double mu[3];
          mu[0] = Mm / T;
          mu[1] = pMass(arg->cc->interface->pinf(1, 1, NULL, NULL)) / T;
          mu[2] = pMass(arg->cc->interface->pinf(1, 2, NULL, NULL)) / T;
          mediatorArr[n].fKK[nT] =
              K1to2(mu[0], mu[1], mu[2], 0, arg->eta[0], arg->eta[1]) /
              K1to2(mu[0], mu[1], mu[2], mediatorArr[n].eta[nT], arg->eta[0],
                    arg->eta[1]);
        }
        ...
        double I0 = 0.5 * wEff * T * T / Hubble(T) /
                    gammaLor(mediatorArr[n].mass[nT] / T, eta);
        ...
        for (;;) {
          ...
          for (i = 0; i < ng; i++) {
            ...
            if (lTi < ltGrid[0])
              bD += cG[i] * mediatorArr[n].wSig[0] / mediatorArr[n].wBath[0];
            else
              bD += cG[i] *
                    polint3(lTi, tGridDim, ltGrid, mediatorArr[n].wSig) /
                    polint3(lTi, tGridDim, ltGrid, mediatorArr[n].wBath);
          }
          double wD = mediatorArr[n].wSig[nT] / bD;

          // printf("wD=%E wEff=%E  bD=%E\n", wD, wEff,bD);
          if (fabs(wD - wEff) < 1.E-3 * wD) {
            wEff = wD;
            break;
          }
          wEff = wD;
          for (i = 0; i < ng; i++) {
            ...
            for (j = 0, I0 = 0; j < ng; j++) {
              ...
              // printf("i=%d j=%d zc=%E z[i]=%E P[j]=%E Tij=%E
              // eta=%E\n",i,j,zc,z[i], P[j], Tj,eta);
              I0 += 0.5 * cG[j] * wBath * Tj * Tj / Hubble(Tj) /
                    gammaLor(Mm / Tj, eta);
            }
            // printf("I0=%e\n", I0);
            z[i] = zc + L[i] / I0;
...
static void printIntervals(void) {
  ...
  for (int i = 0; i < nIntervals; i++)
    printf("[%e,%e] ", intervals[2 * i], intervals[2 * i + 1]);
  printf("\n");
}
...
static void delInterval(double x1, double x2) {
  ...
  int i1, i2;
  for (i1 = 0; i1 < 2 * nIntervals; i1++)
    if (x1 <= intervals[i1])
      break;
  i1--;
  for (i2 = 0; i2 < 2 * nIntervals; i2++)
    if (x2 < intervals[i2])
      break;
  i2--;
  if (i1 == i2) {
    if (i1 & 1 == 1)
      return; // this interval is already excluded
    if (nIntervals == maxIntervals) {
      maxIntervals++;
      intervals = realloc(intervals, 2 * maxIntervals * sizeof(double));
    }
    for (int k = 2 * nIntervals + 1; k >= i1 + 3; k--)
  ...
  if (!(i2 & 1))
    ...
    i2++;
  if (i2 - i1 > 1) {
    for (int k = i1 + 1; k < 2 * nIntervals - 2; k++)
      intervals[k] = intervals[k - i1 + i2 - 1];
    nIntervals -= (i2 - i1) / 2;
  }
}
...
static double sIntegrand_y(double y, void *arg_) {
  ...
  if (Qaddress) {
    *Qaddress = sqrtS;
    ...
    for (int n = 0; n < nMed; n++)
      arg->cc->interface->va[mediatorArr[n].widthPos] = mediatorArr[n].Twidth;
    mass22_par(arg, T);
  }

  if (kin22_par(arg, sqrtS, sqrt(4 * M_PI * parton_alpha(sqrtS)))) {
    printf("kin22 problem\n");
    return 0;
  }
  //  if(arg->err) return 0;
  err = 0;
  ...
  double res = sqme_Int / (32 * M_PI * sqrtS) * T /
               (8 * M_PI * M_PI * M_PI * M_PI) * arg->PcmIn * arg->PcmOut *
               K1to2(sqrtS / T, arg->pmass[0] / T, arg->pmass[1] / T, 0,
                     arg->eta[0], arg->eta[1]) *
               2 * sqrtS * Tx / y;

  if ((arg->eta[2] || arg->eta[3]) && sqrtS / T < 25)
    res *= K1to2(sqrtS / T, arg->pmass[2] / T, arg->pmass[3] / T, 0,
                 arg->eta[2], arg->eta[3]) /
...
static double lnTIntegrand(double lnT, void *arg_) {
  ...
  nIntervals = 1;
  intervals[0] = 0;
  intervals[1] = exp(-sqrtSmin * 1.0001 / Tx);
  // printIntervals();
  // printf("first Print\n");
  double sum = 0;
  ...
  for (int n = 0; n < nMed; n++) {
    ...
    if (sqrtSmin <= Mm) {
      ...
      // if(Mm/T <25)   printf("eta=%E
      // K=%e\n",eta,K1to2(Mm/T,mu1,mu2,0,arg->eta[0],arg->eta[1])/K1to2(Mm/T,mu1,mu2,eta,arg->eta[0],arg->eta[1]));
      //      if(Mm/T <25)
      //      wEff*=K1to2(Mm/T,mu1,mu2,0,arg->eta[0],arg->eta[1])/K1to2(Mm/T,mu1,mu2,eta,arg->eta[0],arg->eta[1]);
...
#endif
      {
        ...
        if ((arg->eta[2] || arg->eta[3]) && Mm / T < 25)
          c = K1to2(Mm / T, arg->pmass[2] / T, arg->pmass[3] / T, 0,
                    arg->eta[2], arg->eta[3]) /
              K1to2(Mm / T, arg->pmass[2] / T, arg->pmass[3] / T, 0, 0, 0);
        sum += c * (mediatorArr[n].spin2 + 1) * mediatorArr[n].cdim * T * Mm *
               Mm * wIwD *
               K1to2(Mm / T, mu1, mu2, 0, arg->eta[0], arg->eta[1]) /
               ((2 * M_PI * M_PI) * wEff);
      }

    } else
      mediatorArr[n].Twidth =
          polint3(lnT, tGridDim, ltGrid, mediatorArr[n].wEff);
  }
  // printf("sum=%e\n",sum);
  double NdfIn = arg->ndf[0] * arg->ndf[1];
  if (arg->pdg[0] == arg->pdg[1])
    NdfIn /= 2;
  sum /= NdfIn;
  ...
  for (int i = 0; i < nIntervals; i++) {
    ...
    sum += simpson_arg(sIntegrand_y, arg, intervals[2 * i],
                       intervals[2 * i + 1], 1E-3, &err);
    if (err) {
      arg->err = arg->err | (2 * 8 * err);
#ifdef ERROR_PLOT
      {
        printf("Warning from Simpson. Precision is not reached (freezein.c "
               "line 1222 code=%d  "
               "arg->err=%d ErrIS=%d )\n",
               err, arg->err, ErrIS);
        printf("i=%d interval: %e %e  \n", i, intervals[2 * i],
               intervals[2 * i + 1]);
        displayPlot("s_Integrand_y", "y", intervals[2 * i],
                    intervals[2 * i + 1], 0, 1, "dI/dy", 0, sIntegrand_y, arg);
        exit(0);
      }
#endif
    }
...
static double lnsFntegrandForVegas(double lns) {
  ...
  if (Qaddress) {
    *Qaddress = sqrtS;
    ...
    for (int n = 0; n < nMed; n++)
      arg_stat.cc->interface->va[mediatorArr[n].widthPos] =
          mediatorArr[n].Twidth;
  }
  mass22_par(&arg_stat, T);

  if (kin22_par(&arg_stat, sqrtS, sqrt(4 * M_PI * parton_alpha(sqrtS))))
    return 0;
  if (arg_stat.err == 4) {
    tPoleDetected = 1;
    return 0;
...
static double sIntegralForVegas(double T, double sqS1, double sqS2) {
  ...
  int err;
  nIntervals = 1;
  intervals[0] = log(sqS1);
  intervals[1] = log(sqS2);
  double sum = 0;
  ...
  for (int n = 0; n < nMed; n++) {
    ...
    //     if(Mm/T <25)
    //     wEff*=K1to2(Mm/T,mu1,mu2,0,arg_stat.eta[0],arg_stat.eta[1])/K1to2(Mm/T,mu1,mu2,eta,arg_stat.eta[0],arg_stat.eta[1]);
...
#endif
    {
      double d0 = 100 * wEff / Mm;
      if (e0 > d0)
        d0 = e0;

      if (sqS1 < Mm * (1 + d0) && sqS2 > Mm * (1 - d0))
        delInterval(log(Mm) - d0, log(Mm) + d0);
      if (sqS1 < Mm && sqS2 > Mm) {
        double P = (arg_stat.E - Mm) * (arg_stat.E + Mm);
        if (P > 0) {
          P = sqrt(P);
          sum +=
              P * (mediatorArr[n].spin2 + 1) * Mm * wIwD * 64 * M_PI * M_PI *
              M_PI / wEff *
              Stat2(P / T, Mm / T, arg_stat.pmass[0] / T, arg_stat.pmass[1] / T,
                    arg_stat.eta[0], arg_stat.eta[1]) *
              Stat2(P / T, Mm / T, arg_stat.pmass[2] / T, arg_stat.pmass[3] / T,
                    arg_stat.eta[2], arg_stat.eta[3]);
        }
      }
    }
  }
  ...
  if (arg_stat.pdg[0] == arg_stat.pdg[1])
    NdfIn /= 2;
  sum /= NdfIn;
  ...
  for (int i = 0; i < nIntervals; i++) {
    int err;
    sum += simpson(lnsFntegrandForVegas, intervals[2 * i], intervals[2 * i + 1],
                   1E-2, &err);
    if (err) {
      printf("Warning from simpson. Precison is not reached (freezein.c line "
             "1327 code=%d [%e,%e])\n",
             err, intervals[2 * i], intervals[2 * i + 1]);
#ifdef ERROR_PLOT
      displayPlot(" Error code gauss345", "x", intervals[2 * i],
                  intervals[2 * i + 1], 0, 1, "lnsFntegrandForVegas", 0,
                  lnsFntegrandForVegas, NULL);
      exit(0);
#endif
    }
  }
...
static double vegas22FIintegrand(double *x, double w) {
  ...
  double T = exp(lnT);
  double J = (log(TR_stat) - log(Tmin_stat)) * (1 + hEffLnDiff(T) / 3) /
             Hubble(T); //  dT/H/T  = J* dx[0]
  double E = sqrtSmin - T * log(x[1]);
  J *= T * exp(-sqrtSmin / T); // dEexp(-E/T) = J*dx[1]
#ifndef ONSHELL
  double cs = 2 * (x[2] - 0.5), sn = sqrt(1 - cs * cs),
         fi = M_PI * 2 * (x[3] - 1);
#endif
  arg_stat.T = T;
#ifdef NOSTATISTICS
...
static int printFi22Error(FILE *f, int err) {
  switch (err) {
  case 1:
    fprintf(f, "process  is absent\n");
    break;
  case 2:
    fprintf(f, "2->2 type process is expected\n");
    break;
  case 3:
    fprintf(f, "can not calculate local parameters\n");
    break;
  case 4:
    fprintf(f, "reheating temperature is too small\n");
    break;
  case 5:
    fprintf(f, "there are incoming feeble particles\n");
    break;
  case 6:
    fprintf(f, "there is no odd  feeble particles among outgoing ones\n");
    break;

  case 7:
    fprintf(f, "Lost of precision in  temperature integrand\n");
    break;
  case 8:
    fprintf(f, "Pole in temperature integrand\n");
    break;
  case 9:
    fprintf(f, "NaN in temperature intergrand\n");
    break;
  case 10:
    fprintf(f, "Lost of precision in  sqrt(s) integrand\n");
    break;
  case 11:
    fprintf(f, "Pole in sqrt(s) integrand\n");
    break;
  case 12:
    fprintf(f, "NaN  in sqrt(s) intergrand\n");
    break;
  case 13:
    fprintf(f, "Lost of precision in angle integrand\n");
    break;
  case 14:
    fprintf(f, "Pole in angle  integrand\n");
    break;
  case 15:
    fprintf(f, "NaN  in angle  intergrand\n");
    break;
  case 16:
    fprintf(f, "lost of precision caused by diagramm cancelation\n");
  }
}
...
double darkOmegaFi22(double Tr, char *Proc, int vegas, int plot, int *err_) {
  ...
  for (Taddress = NULL, i = 0; i < nModelVars; i++)
    if (strcmp(varNames[i], "T") == 0) {
      Taddress = varValues + i;
      break;
    }
  if (err_)
    *err_ = 0;
  ...
  err = init22_par(&arg, cc,
                   1); // process   is absent || 2->2 type process is expected
  if (!err) {
    if (Taddress) {
      *Taddress = 0;
      calcMainFunc();
    }
    if (passParameters(cc))
      err = 3; // Can not calculate local parameters for process
  }
  if (!err) {
    if (Tr <= 10 * Tzero)
      err = 4;
      ...
      if (Tmin < (arg.pmass[0] + arg.pmass[1]) / 15)
        Tmin = (arg.pmass[0] + arg.pmass[1]) / 15;
      if (Tmin > Tr / 10)
        Tmin = Tr / 10;
      if (Tmin < Tzero)
        Tmin = Tzero;
  ...
  if (!err) {
    for (int i = 0; i < 4; i++)
      if (isFeeble(cc->interface->pinf(1, 1 + i, NULL, NULL)))
        arg.eta[i] = 0;
      else
        arg.eta[i] = 1 - 2 * (arg.spin2[i] & 1);
    if (arg.eta[0] == 0 || arg.eta[1] == 0)
      err = 5; //  There are incoming feeble particles
    for (int i = 2; i < 4; i++)
      if (arg.eta[i] == 0 &&
          cc->interface->pinf(1, 1 + i, NULL, NULL)[0] == '~') {
        if (cc->interface->pinf(1, 1 + i, NULL, NULL)[1] == '~')
          MDM += Mcdm2;
        else
          MDM += Mcdm1;
      }
    if (MDM == 0)
      err = 6; // There is no odd  feeble particles among outgoing ones
  }
  ...
  if (!err) {
    *(cc->interface->BWrange) = 1000; //!!!
    ...
    int neutral[4];
    for (i = 0; i < 4; i++)
      cc->interface->pinfAux(1, 1 + i, NULL, NULL, neutral + i, NULL);
    if (arg.pdg[0] + arg.pdg[1] != 0 && (!neutral[0] || !neutral[1]))
      NConj = 2;
    if (arg.pdg[2] + arg.pdg[3] != 0 && (!neutral[2] || !neutral[3]))
      NConj = 2;
    ...
    if (arg.pdg[0] == arg.pdg[1])
      Cin = 0.5;
    totOmegaCoeff =
        Cin * arg.ndf[0] * arg.ndf[1] * NConj * MDM * EntropyNow / RhoCrit100;
  }
  ...
  if (!err) {
    if (vegas) {
      ...
      if (nMed > 0) {
        ...
        for (int n = 0; n < nMed; n++)
          for (int k = 0; k < tGridDim; k++)
            if (mediatorArr[n].mass[k] < newSqrtSmin)
              newSqrtSmin = mediatorArr[n].mass[k];
      ...
      nPROCSS = nPROCSS_;
      if (arg_stat.err & Errt)
        err = 7; // pole in t/u channel
    } else {
      if (maxIntervals == 0) {
      ...
      if (plot) {
        ...
        if (arg.err == 0) {
          ...
          arr[99] = 0;
          for (int i = 98; i >= 0; i--) {
            double T1 = Tmin * pow(Tr / Tmin, (i + 0.5) / 100),
                   T2 = Tmin * pow(Tr / Tmin, (i + 1.5) / 100);
            arr[i] = arr[i + 1] +
                     gauss_arg(lnTIntegrand, &arg, log(T1), log(T2), 3) *
                         totOmegaCoeff;
          }
          char txt[60];
      ...
      arg.err = 0;
      omega = totOmegaCoeff *
              simpson_arg(lnTIntegrand, &arg, log(Tmin), log(Tr), 0.001, &err);
      if (err)
        arg.err = arg.err | (2 * 8 * 8 * err);
      if (arg.err & 2 * 8 * 8 * 4)
        err = 7;
      else if (arg.err & 2 * 8 * 8 * 2)
        err = 8;
      else if (arg.err & 2 * 8 * 8 * 1)
        err = 9;
      else if (arg.err & 2 * 8 * 4)
        err = 10;
      else if (arg.err & 2 * 8 * 2)
        err = 11;
      else if (arg.err & 2 * 8 * 1)
        err = 12;
      else if (arg.err & 2 * 4)
        err = 13;
      else if (arg.err & 2 * 2)
...
double darkOmegaFi(double TR, int *err) {
  ...
  for (int n = 0; n < nModelParticles; n++)
    if (isFeeble(ModelPrtcls[n].name) && ModelPrtcls[n].name[0] == '~') {
      addPrtcl(&feebleDm, n);
      double m = pMass(ModelPrtcls[n].name);
      if (minFeebleMass < 0)
        minFeebleMass = m;
      else if (m < minFeebleMass)
        minFeebleMass = m;
    }
  ...
  *err = 0;
  ...
  for (int n = 0; n < nModelParticles; n++)
    if (!isFeeble(ModelPrtcls[n].name)) {
      double m = pMass(ModelPrtcls[n].name);
      if (m > minFeebleMass) {
        double brTot, brBath, brSig1, brSig2;
        txtList L;
        ...
        if (brBath == 0 && brSig1 + brSig2 > 0) {
          ...
          omega += dOmega;
          if (dOmega)
            for (int i = nFiCh_; i < nFiCh; i++)
              omegaFiCh[i].weight *= dOmega;
          else
            nFiCh = nFiCh_;
        } else {
          addPrtcl(&bath, n);
  ...
  if (!strlen(feebleDm) || !strlen(bath))
    return 0;
  ...
  sprintf(command,
          "cd %s;"
          " %s/bin/s_calchep -blind "
          "\"{{allBath,allBath->feeblDm,1*x{%s{%s{{{[[{0\" >/dev/null;"
  ...
  if (f0) {
    ...
    while (4 ==
      ...
      for (i = 0; i < 4; i++) {
        n[i] = pTabPos(p[i]);
        if (strcmp(ModelPrtcls[abs(n[i]) - 1].name,
                   ModelPrtcls[abs(n[i]) - 1].aname) == 0)
          n_[i] = n[i];
        else
          n_[i] = -n[i];
      }
      ...
      if (dOmega || err) {
        for (int i = 0; i < 4; i++)
          if (n[i] > 0)
            omegaFiCh[nFiCh].prtcl[i] = ModelPrtcls[n[i] - 1].name;
          else
            omegaFiCh[nFiCh].prtcl[i] = ModelPrtcls[-n[i] - 1].aname;
...
void printChannelsFi(double cut, int prcn, FILE *f) {
  if (omegaFiCh && omegaFiCh[0].prtcl[0]) {
    ...
    for (int i = 0; omegaFiCh[i].prtcl[0]; i++)
      if (omegaFiCh[i].weight / omega > cut || omegaFiCh[i].err) {
        if (prcn)
          fprintf(f, " %6.2f%%  ", 100 * omegaFiCh[i].weight / omega);
        else
          fprintf(f, " %.3E  ", omegaFiCh[i].weight / omega);
        char proc[40];
        sprintf(proc, "%s", omegaFiCh[i].prtcl[0]);
        if (omegaFiCh[i].prtcl[3] == NULL)
          sprintf(proc + strlen(proc), " -> %s, %s", omegaFiCh[i].prtcl[1],
                  omegaFiCh[i].prtcl[2]);
        else
          sprintf(proc + strlen(proc), ", %s ->  %s,%s ", omegaFiCh[i].prtcl[1],
                  omegaFiCh[i].prtcl[2], omegaFiCh[i].prtcl[3]);
        fprintf(f, "%-30.30s ", proc);
        if (omegaFiCh[i].err) {
          fprintf(f, "Warning %d:", omegaFiCh[i].err);
          printFi22Error(f, omegaFiCh[i].err);
        } else
          fprintf(f, "\n");
      }
  }
  ...
  *omg2 = 0;
  ...
  for (int i = 0; omegaFiCh[i].prtcl[0]; i++) {
    ...
    if (omegaFiCh[i].prtcl[3] == NULL)
      ...
      n = 2;
    for (int k = n; k < n + 2; k++)
      if (omegaFiCh[i].prtcl[k][0] == '~') {
        if (omegaFiCh[i].prtcl[k][1] == '~')
          d2 += 1;
        else
          d1 += 1;
      }
