...
#define V0                                                                     \
  (Vrot / 299792. * 1.5957691) /* 2*sqrt(2/PI).  10^(-6)*Mslp < Width */
...
double Zi(int i) {
  return log(1.E-7) * pow((double)(i - 1) / (double)(NZ), 1.5);
}
...
double vcs22(numout *cc, int nsub, int *err) {
  ...
  *(cc->interface->gswidth) = 0;
  // printf("Mb4c=%E\n", findValW("Mb"));
  for (i = 0; i < 4; i++)
    cc->interface->pinf(nsub, 1 + i, pmass + i, NULL);
  *err = 0;
  if (pmass[0] + pmass[1] <= pmass[2] + pmass[3])
    return 0;
  for (i = 0; i < 16; i++)
    pvect[i] = 0;
  // printf("Mb4d=%E\n", findValW("Mb"));
  ...
  for (i = 0; i < 2; i++)
    pvect[4 * i] = pmass[i];
  for (i = 2; i < 4; i++)
    pvect[4 * i] = sqrt(pmass[i] * pmass[i] + pcm * pcm);
  pvect[8 + 3] = pcm;
  pvect[12 + 3] = -pcm;
  r = cc->interface->sqme(nsub, GG, pvect, NULL, err);
  return 3.8937966E8 * r * pcm /
         (16 * M_PI * pmass[0] * pmass[1] * (pmass[0] + pmass[1]));
}
...
static int iA, ix, iX;
static numout *cc23;
static double X0 = 0.01, dSigmadE_x0, Egamma, X1 = 0.8, dSigmadE_x1,
              dSigmadE_x1_e;
...
static double dSigmadCos23(double csfi) {
  ...
  */
  err_code = 0;
  r = (cc23->interface->sqme)(1, GG, pvect, NULL, &err_code) * pcm1 * pcm2 /
      (128 * M * pmass[0] * pmass[0] * M_PI * M_PI * M_PI);
  if (r < 0)
    r = 0;
  if (err_code)
    return 0;
...
static double dSigmadFi23(double fi) {
  if (fi == 0 || fi == M_PI)
    return 0;
  return dSigmadCos23(cos(fi)) * sin(fi);
}
...
static double dSigmadE(double E) {
  ...
  /*
     ...
     else { printf("dSigmadCos23\n"); //displayFunc("dSigmadCos23",dSigmadCos23,
     -1 , 1,0,"dSigmadCos23"); r= simpson(dSigmadCos23,-0.9,0.9,1.E-4,NULL);
     printf("ok\n");
  */
...
static double dSigmadERest(double E) {
  ...
  if (E + sqrt(E * E + ms * ms) >= 1.999999 * Mcdm0)
    return 0;
  res = dSigmadE(E);
  if (res == 0.)
    return 0;
  ...
  //  if( !(addGamma(code[iX]) && addGamma(code[ix])))
  {
    ...
    for (l = 0; l < 2; l++) {
      ...
      if (!addGamma(code[nn[l]])) {
        ...
        subtract += log((1 + kappa * csmax) / (1 - kappa * csmax)) *
                        (1 - x + x * x / 2 - eps * eps / 2) -
                    kappa * (1 - x) * csmax;
      }
      ...
      norm +=
          log((1 + csmax * kappa) / (1 - csmax * kappa)) * (1 - eps * eps / 2) -
          kappa * csmax;
    }
...
static double dSigmadEe(double E) {
  ...
  xe = Xe = E / Mcdm0;
  if (xe > x2 + 1.E-4) {
    return ((x2 - xe) * dSigmadEe(x1 * Mcdm0) +
            (xe - x1) * dSigmadEe(x2 * Mcdm0)) /
           (x2 - x1);
  }
  if (Xe > X0) {
    ...
    */
    ...
    if (csMax > 1)
      csMax = 1;
    csMin += 0.001; // !!!!!!!! Problem in  case of zero mass of electron.
                    // Reason is not clear
    int err;
    r = simpson(FEfi, acos(csMax), acos(csMin), 1.E-4, NULL);
    // if(err) displayPlot("dSigmadEe",
    // "fi",acos(csMax),acos(csMin),0,1,"FEfi",0,FEfi,NULL);
    return r;
  } else
    return 0;
}
...
static double Spectra22A(char *name1, char *name2, double **Spectra,
                         txtList plusA) {
  double vcs = 0;
  int i, l;
  txtList cRec;
  ...
  if (Spectra[0] == NULL && Spectra[1] == NULL)
    return 0;
  ...
  for (cRec = plusA; cRec; cRec = cRec->next) {
    ...
    *(cc23->interface->gswidth) = 0;
    ...
    for (i = 0; i < 5; i++)
      N[i] = cc23->interface->pinf(1, i + 1, pmass + i, code + i);
    ...
    if (Spectra[0] && dSigmadE(X1 * Mcdm0) > X1CUT * dSigmadE_x1) {
      double x2Sum;
      dSigmadE_x0 = 3 * X0 *
                    (dSigmadE(X0 * Mcdm0) + dSigmadE(3 * X0 * Mcdm0) -
                     2 * dSigmadE(2 * X0 * Mcdm0));
#ifdef DISPLAY_SPECTRA
      {
        char buff[100];
      ...
      }
      {
        ...
        vSigmaCh[nAnCh].weight = x2Sum;
        {
          int j;
          for (j = 0; j < 5; j++)
            vSigmaCh[nAnCh].prtcl[j] = N[j];
        }
        nAnCh++;
      }
    }
    if (Spectra[1] && abs(code[iX]) == 11 &&
        code[ix] + code[iX] == 0 /*&& pmass[ix]==0*/ && // !!! was blocked for
                                                        // Me!=0. Reason not
                                                        // clear.
        code[0] == code[1] && spin2Dm == 1 &&
        dSigmadEe(X1 * Mcdm0) > X1CUT * dSigmadE_x1_e) {
#ifdef DISPLAY_SPECTRA
...
static int readSpectra(void) {
  ...
  FILE *f;
  char *buff;
  char *fnames[Nin] = {"gg.dat",   "dd.dat", "uu.dat",   "ss.dat",
                       "cc.dat",   "bb.dat", "tt.dat",   "ee.dat",
                       "mm.dat",   "ll.dat", "zz.dat",   "zz_t.dat",
                       "zz_l.dat", "ww.dat", "ww_t.dat", "ww_l.dat"};
  ...
  for (n = 0; n < Nin; n++) {
    ...
    for (i = 0; i < 6; i++) {
      ...
      for (k = 0; k < NEn; k++) {
        for (l = 0; l < NZ; l++)
          if (1 != fscanf(f, "%f", phidiff[n][i][k] + l))
            break;
        fscanf(f, "%*s");
        for (; l < NZ; l++)
          phidiff[n][i][k][l] = 0;
  ...
  for (n = 0; n < Nin; n++)
    for (k = 0; k < NEn; k++) {
      double mi[NEn] = {2,   5,   10,  25,   50,   80.3, 85,   91.2, 92,  95,
                        100, 110, 120, 125,  130,  140,  150,  176,  200, 250,
                        350, 500, 750, 1000, 1500, 2000, 3000, 5000};
      ...
      for (n = 0; n < Nin; n++)
        for (k = 0; k < NEn; k++) {
          ...
          for (l = 0; l < 6; l++)
            if (phidiff[n][l][k]) {
              for (i = 0; i < NZ; i++) {
                ...
                double dx;
                if (i == 0)
                  dx = -0.5 * log(1.E-7) * pow((double)(1.) / NZ, 1.5);
                else
                  dx = 0.5 * log(1.E-7) *
                       (pow((double)(i - 1) / NZ, 1.5) -
                        pow((double)(i + 1) / NZ, 1.5));
                if (l == 2)
                  xx += 1 / mi[k];
          ...
          if (sum && (sum > 2.1 || sum < 1.9))
            printf("Channel = %s Energy=%.1E  %.2e + %.2e + %.2e + %.2e + %.2e "
                   "+ %.2e = %.2E\n",
...
double zInterp(double zz, double *tab) {
  ...
  if (tab[j0] > 0 && tab[j0 + 1] > 0)
    r = pow(tab[j0], (1 - dz)) * pow(tab[j0 + 1], dz);
  else
...
static void mInterp(double Nmass, int CHin, int CHout, double *tab) {
  //  float
  //  mi[NEn]={10,25,50,80.3,91.2,100,150,176,200,250,350,500,750,1000,1500,2000,3000,5000};
  double mi[NEn] = {2,   5,   10,  25,   50,   80.3, 85,   91.2, 92,  95,
                    100, 110, 120, 125,  130,  140,  150,  176,  200, 250,
                    350, 500, 750, 1000, 1500, 2000, 3000, 5000};
  int l, i0;
  ...
  case 17:
    if (i0 < 5) {
      i0 = 6;
      for (l = 1; l < NZ; l++)
        tab[l] = phidiff[CHin][CHout][i0][l - 1];
      tab[0] = Nmass;
      return;
    }
  case 10:
  ...
  if (i0 == NEn - 1)
    ...
    c1 = (Nmass * Nmass - mi[i0] * mi[i0]) /
         (mi[i0 + 1] * mi[i0 + 1] - mi[i0] * mi[i0]);
    c0 = 1 - c1;
    for (l = 1; l < NZ; l++)
      tab[l] = c0 * p0[l - 1] + c1 * p1[l - 1];
  }
  tab[0] = Nmass;
}
...
double spectrInfo(double Emin, double *tab, double *Etot) {
  if (Emin >= tab[0]) {
    ...
    if (Xmin < 1.22E-7)
      Xmin = 1.22E-7;
    zmin = log(Xmin);
    for (i1 = Iz(zmin); i1 > 1 && tab[i1] == 0; i1--)
      continue;
    if (i1 < NZ - 1)
      i1++;
    if (zmin < Zi(i1))
    ...
    tabStat = tab;
    if (Etot)
      *Etot = tab[0] * simpson(FUNE, zmin, zmax, 1.E-4, NULL);
...
double spectrInt(double Emin, double Emax, double *tab) {
  ...
  if (Emin < M * exp(Zi(NZ - 1)))
    zmin = Zi(NZ - 1);
  else if (Emin >= M)
    zmin = 0;
  else
    zmin = log(Emin / M);
  if (Emax < M * exp(Zi(NZ - 1)))
    zmax = Zi(NZ - 1);
  else if (Emax >= M)
    zmax = 0;
  else
...
void boost(double Y, double M0, double mx, double *tab) {
  ...
  for (l = 1; l < NZ; l++) {
    ...
    if (e1 >= e2)
      ...
      tab_out[l] = e * simpson(FUNB, e1, e2, 1.E-3, NULL) / 2 / shY;
...
int basicSpectra(double Mass, int pdgN, int outN, double *tab) {
  ...
  if (inP != 25)
    ...
    double Mh = 125;
    double tabV[NZ], Y;
    double br_b = 0.60, br_l = 0.06, br_W = 0.21, br_Z = 0.03, br_G = 0.08,
           br_c = 0.017, br_A = 0.003;
    int i;
    for (i = 1; i < NZ; i++)
      tab[i] = 0;
    mInterp(Mh / 2, 5, outN, tabV);
    for (i = 1; i < NZ; i++)
      tab[i] += 2 * br_b * tabV[i];
    mInterp(Mh / 2, 9, outN, tabV);
    for (i = 1; i < NZ; i++)
      tab[i] += 2 * br_l * tabV[i];
    mInterp(Mh / 2, 10, outN, tabV);
    for (i = 1; i < NZ; i++)
      tab[i] += 2 * br_Z * tabV[i];
    mInterp(Mh / 2, 13, outN, tabV);
    for (i = 1; i < NZ; i++)
      tab[i] += 2 * br_W * tabV[i];
    mInterp(Mh / 2, 0, outN, tabV);
    for (i = 1; i < NZ; i++)
      tab[i] += 2 * br_G * tabV[i];
    mInterp(Mh / 2, 4, outN, tabV);
    for (i = 1; i < NZ; i++)
      tab[i] += 2 * br_c * tabV[i];
    tab[0] = tabV[0];
    if (outN == 0)
      tab[1] += br_A * 8 / (Zi(1) - Zi(2));
    Y = acosh(Mass / Mh);
...
static void getSpectrum(int wPol, double M, double m1, double m2, char *n1,
  ...
  if (inP >= 0) {
    if (wPol && (inP == 10 || inP == 13)) {
      ...
      if (inP == 10) {
        ...
        /*        if(PrintOn && outP==0 ) printf(" %.0f%% transverse Z
         * produced\n",100*fabs(left+right));*/
      } else {
        err = vPolar(24, 11, -12, &left, &right, &lng);
        /*        if(PrintOn && outP==0 ) printf(" %.0f%% transverse W
         * produced\n",100*fabs(left+right));*/
      }
      if (err)
        mInterp(M / 2, inP, outP, tab);
      else {
        double tabL[NZ];
        mInterp(M / 2, inP + 1, outP, tab);
        mInterp(M / 2, inP + 2, outP, tabL);
        for (i = 1; i < NZ; i++)
          tab[i] = (1 - lng) * tab[i] + lng * tabL[i];
      }
    } else
      mInterp(M / 2, inP, outP, tab);
    ...
    for (k = 0; k < 2; k++) {
      ...
      if (inP >= 0) {
        ...
        for (i = 0; i < NZ; i++)
          tabAux[i] = 0;
        if (mm[k] == 0) {
          fprintf(stderr, "Can not hadronize zero mass %s\n", nn[k]);
          continue;
        }
        ...
        if (w == 0) {
          if (!((CDM1 &&
                 (strcmp(CDM1, nn[k]) == 0 || strcmp(aCDM1, nn[k]) == 0)) ||
                (CDM2 &&
                 (strcmp(CDM2, nn[k]) == 0 || strcmp(aCDM2, nn[k]) == 0))))
...
void sumSpectra(double *A, double *B, double *S) {
  ...
  if (A[0] == B[0]) {
    ...
    for (s[0] = A[0], i = 1; i < NZ; i++) {
      z = Zi(i) + log(A[0] / B[0]);
      if (z <= 0)
        s[i] += zInterp(z, B);
    }
  else
    for (s[0] = B[0], i = 1; i < NZ; i++) {
      z = Zi(i) + log(B[0] / A[0]);
      if (z <= 0)
        s[i] += zInterp(z, A);
    }
...
static double calcSpectrum0(char *name1, char *name2, int key, double **Spectra,
                            txtList *plusA) {
  int i, k, l;
  double vcsSum = 0;
  int ntot, err;
  ...
  for (k = 0; k < ntot; k++) {
    ...
    if (cc23) {
          ...
          break;
      {
        ...
        if (err)
          printf("error in simpson spectra.c line 902\n");
      }
      if (pdg[l_] == 23 || abs(pdg[l_]) == 24) {
        double wV2;

        wV2 = pWidth(N[l_], NULL);
        r *= decayPcmW(2 * Mcdm0, m[l], m[l_], wV, wV2, 0) /
             decayPcmW(2 * Mcdm0, m[l], m[l_], wV, 0., 0);
        if (pdg[l] == pdg[l_])
          r /= 2;
      }
      v_cs[k] = r;
      ...
      err = 0;
#ifdef V0
      v_cs[k] = V0 * cs22(libPtr, k + 1, V0 * m[0] * m[1] / (m[0] + m[1]), -1.,
                          1., &err);
#else
  ...
  for (k = 0; k < ntot; k++)
    if (v_cs[k] >= 0) {
      ...
      for (l = 0; l < 2; l++)
        pdg[l] = qNumbers(N[2 + l], spin2 + l, charge3 + l, NULL);

      if (Spectra[0] && plusA && (charge3[0] || charge3[1]) &&
          (m[2] + m[3] < m[0] + m[1])) {
        double m1 = m[2], m2 = m[3], Eg = X1 * Mcdm0;
        double kappa = 4 * Mcdm0 * (Mcdm0 - Eg), ms = m1 + m2, md = m1 - m2;
        if (ms * ms < kappa) {
          double dp = (Mcdm0 - Eg / 2) *
                      sqrt((1 - ms * ms / kappa) * (1 - md * md / kappa));
          double p1 = Eg / 2 * (1 + ms * md / kappa) - dp,
                 E1 = sqrt(p1 * p1 + m1 * m1);
          double p2 = Eg / 2 * (1 - ms * md / kappa) - dp,
                 E2 = sqrt(p2 * p2 + m2 * m2);
          ...
          */
          for (n = 1; (s = libPtr->interface->den_info(k + 1, n, &m, &w, NULL));
               n++) {
            double mass = fabs(libPtr->interface->va[m]);
            if ((strcmp(s, "\1\3") == 0 || strcmp(s, "\1\4") == 0) && m &&
                m_min > mass)
              m_min = mass;
          }

          if (m_min * m_min - Q1 <
                  QCUT * Mcdm0 * Mcdm0 * abs(charge3[0]) / 3. ||
              m_min * m_min - Q2 <
                  QCUT * Mcdm0 * Mcdm0 * abs(charge3[1]) / 3. ||
              (abs(pdg[0]) == 24 && abs(pdg[1]) == 24 && Mcdm0 > 500 &&
               v_cs[k] > 1.E-3 * vcsSum)) {
            txtList new22A = malloc(sizeof(txtListStr));
            new22A->next = *plusA;
            new22A->txt = malloc(50);
            *plusA = new22A;
            sprintf(new22A->txt, "%s,%s->%s,%s,%s", N[0], N[1], N[2], N[3],
                    photonName);
            PlusAok = 1;
          }
        }
      ...
      if (v_cs[k] > 0) {
        ...
        for (l = 0; l < 6; l++)
          if (Spectra[l]) {
            getSpectrum(key & 1, m[0] + m[1], m[2], m[3], N[2], N[3], N3, N4, l,
                        tab2);
            for (i = 1; i < NZ; i++)
              Spectra[l][i] += tab2[i] * v_cs[k];
          }

        if (Spectra[0] && charge3[0]) {
          for (l = 0; l < 2; l++)
            if (addGamma(pdg[l]) && m[2 + l] != 0.)
              for (i = 1; i < NZ; i++) {
                double x = exp(Zi(i));
                if (2 * Mcdm0 * sqrt(1 - x) > m[2] + m[3]) {
                  ...
                  if (m[2 + l] / pcm > 1.E-2)
                    ...
                    one_kappa = 0.5 * m[2 + l] * m[2 + l] / pcm / pcm;
                  f = (1. / 137.) * charge3[l] * charge3[l] / 9 / M_PI *
                      log((1 + kappa) / one_kappa);

                  if (spin2[l] & 1)
                    Spectra[0][i] += f * (1 - x * (1 - x * 0.5)) * v_cs[k];
                  else
                    Spectra[0][i] += f * (1 - x) * v_cs[k];
                }
              }
        }
      }
...
double calcSpectrum(int key, double *Sg, double *Se, double *Sp, double *Sne,
                    double *Snm, double *Snl, int *errcode) {
  int n, i, j, l, err;
  double vcs;
  char lop[20];
  ...
  err = readSpectra();
  if (err) {
    printf("calcSpectrum: Can not read data files for spectra\n");
    if (errcode)
      *errcode = -1;
    return 0;
  }
  ...
  if (WINPS[0]) {
    ...
    if (strcmp(ModelPrtcls[n - 1].name, ModelPrtcls[n - 1].aname))
      WINPS[1] = ModelPrtcls[n - 1].aname;
  }

  if (WINPS[2]) {
    int n = pTabPos(WINPS[2]);
    if (strcmp(ModelPrtcls[n - 1].name, ModelPrtcls[n - 1].aname))
      WINPS[3] = ModelPrtcls[n - 1].aname;
  }

  double w1[4] = {0.5 * (1 - NfracCDM2) * (1 + dmAsymm),
                  0.5 * (1 - NfracCDM2) * (1 - dmAsymm),
                  0.5 * NfracCDM2 * (1 + dmAsymm),
                  0.5 * NfracCDM2 * (1 - dmAsymm)};
  if (!WINPS[1]) {
    w1[0] += w1[1];
    w1[1] = 0;
  ...
  for (i = 0; i < 4; i++)
    for (j = 0; j < 4; j++) {
      ...
      if (weight[i][j]) {
        ...
        if (ModelPrtcls[ni - 1].spin2 == 0 && ModelPrtcls[nj - 1].spin2 == 0)
          checkGam[i][j] = 1;
        if (WINPS[i] == WINPS[j] && ModelPrtcls[nj - 1].spin2 == 1)
          checkGam[i][j] = 1;
      }
    }
  ...
  Spectra[0] = Sg;
  Spectra[1] = Se;
  Spectra[2] = Sp, Spectra[3] = Sne, Spectra[4] = Snm;
  Spectra[5] = Snl;
  for (l = 0; l < 6; l++)
    if (Spectra[l])
      Spectra_[l] = buff[l];
  ...
  for (i = 0; i < 4; i++)
    for (j = i; j < 4; j++)
      if (weight[i][j]) {
        ...
        err = findVal("Q", &scale);
        if (err == 0 && scale != 2 * Mcdm0) {
          assignValW("Q", 2 * Mcdm0);
          calcMainFunc();
        }
        ...
        {
          double vcsij =
              c * calcSpectrum0(WINPS[i], WINPS[j], key0, Spectra_, plusAptr);
          if (i < 2 && j < 2)
            chStat[0] += vcsij;
          else if (i > 1 && j > 1)
            chStat[2] += vcsij;
        ...
        for (k = nAnCh0; k < nAnCh; k++)
          vSigmaCh[k].weight *= c;
        if (plusA) {
          nAnCh0 = nAnCh;
          if (Spectra[0])
            dSigmadE_x1 = zInterp(log(X1), Spectra_[0]) / (X1 * Mcdm0);
          if (Spectra[1])
            dSigmadE_x1_e = zInterp(log(X1), Spectra_[1]) / (X1 * Mcdm0);
  ...
  for (j = 0; j < 5; j++)
    vSigmaCh[nAnCh].prtcl[j] = NULL;
  for (i = 0; i < nAnCh - 1;) {
    if (vSigmaCh[i].weight >= vSigmaCh[i + 1].weight)
      i++;
    else {
      aChannel buff;
      buff = vSigmaCh[i + 1];
      vSigmaCh[i + 1] = vSigmaCh[i];
      vSigmaCh[i] = buff;
      if (i)
        i--;
      else
  ...
  if (PrintOn) {
    ...
    for (i = 0; vSigmaCh[i].weight > 1.E-4; i++) {
      sprintf(txt, "%s,%s -> %s %s ", vSigmaCh[i].prtcl[0],
              vSigmaCh[i].prtcl[1], vSigmaCh[i].prtcl[2], vSigmaCh[i].prtcl[3]);
      if (vSigmaCh[i].prtcl[4])
        strcat(txt, vSigmaCh[i].prtcl[4]);
      printf("  %-20.20s  %.2E\n", txt, vSigmaCh[i].weight);
    }
    //    printf("chanStat: 11 -> %.2E  12-> %.2E
    //    22->%.2E\n",    2.9979E-26*chStat[0],2.9979E-26*chStat[1],2.9979E-26*chStat[2]);
  }

  if (Mcdm0 < 2)
    printf("WARNING! Spectra obtained at Mcdm=2GeV are used !\n");
