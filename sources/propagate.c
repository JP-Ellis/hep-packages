...
void setProfileZhao(double Halpha_n, double Hbeta_n, double Hgam_n,
                    double Ha_n) {
  Ha = Ha_n;
  Halpha = Halpha_n;
  Hgam = Hgam_n;
...
#define h                                                                      \
  .1 /* Disk half-thickness (kpc): this parameter should not be modified */
...
static double N_rho(void) {
  ...
  if (CDM1 == NULL && CDM2 == NULL)
    ...
    N = rhoDM * (fracCDM2 / Mcdm2 + (1 - fracCDM2) / Mcdm1);
...
static double psiIntegrand(double psi) {
  ...
  if (cs_ > 0) {
    double xmin;
    if (rHaloMin / Rsun > sn_)
      xmin = sqrt((rHaloMin / Rsun - sn_) * (rHaloMin / Rsun + sn_));
    else
      xmin = 0;
...
static double fiIntegrand(double fi) {
  dfi_ = fi;
  return sin(fi) * simpson(psiIntegrand, -M_PI / 2, M_PI / 2, 1.E-4, NULL) /
         M_PI;
}
...
double HaloFactor(double fi, double dfi) {
  ...
  if (dfi <= 0) {
    ...
    res = simpson(fiIntegrand, 0, dfi, 1.E-4, NULL) / (1 - cos(dfi));
...
void gammaFluxTab(double fi, double dfi, double sigmaV, double *Sp,
                  double *Sobs) {
  int i;
...
void gammaFluxTabGC(double l, double b, double dl, double db, double sigmaV,
                    double *Sp, double *Sobs) {
  int i;
  double hf, Norm = N_rho();
  if (vcsMode)
    Norm *= Norm / 2;
...
static double green_v(double Kt, double zS) {
  if (zS >= L_dif)
    return 0;
  if (L_dif * L_dif / (4 * Kt) >= 1)
    return propagator_image(Kt, zS); /* [kpc^{-1}] */
  else
    return propagator_quantic(Kt, zS); /* [kpc^{-1}] */
}

static double azimuthInt(double k) {
  /* return 1/pi*int_0^pi exp(- k sin^2(fi/2)) dfi == exp(-k/2)*bessI0(k/2)  */

  double X[18] = {0.000000E+00, 3.125000E-02, 6.250000E-02, 1.250000E-01,
                  1.875000E-01, 2.500000E-01, 2.812500E-01, 3.125000E-01,
                  3.750000E-01, 4.062500E-01, 4.375000E-01, 5.000000E-01,
                  5.625000E-01, 6.250000E-01, 7.500000E-01, 8.750000E-01,
                  9.375000E-01, 1.000000E+00};

  double Y[18] = {3.544908E+00, 3.660214E+00, 3.785458E+00, 4.072489E+00,
                  4.424998E+00, 4.880735E+00, 5.174304E+00, 5.534618E+00,
                  6.449089E+00, 6.942868E+00, 7.404071E+00, 8.105752E+00,
                  8.444493E+00, 8.465206E+00, 7.930969E+00, 7.108068E+00,
                  6.687196E+00, 6.283185E+00};
...
static double zIntegrand(double z) {
  ...
  if (rMin <= 0.0)
    rMin = 0.0;
  if (rMin * rMin < rHaloMin * rHaloMin - z * z)
    rMin = sqrt(rHaloMin * rHaloMin - z * z);
  if (rMax > Rdisk)
    rMax = Rdisk;
...
static double integral_cal_In(double K0_tau) {
  if (K0_tau < 0.001) {
    double prQ = haloProfile(Rsun);
    if (vcsMode)
      prQ *= prQ + rhoClumpEff_(Rsun) / rhoDM;
    return prQ / 2;
  }
  Kt_ = K0_tau;
...
static double rIntegrand(double r) {
  ...
  if (r >= rHaloMin)
    ...
    zMin = sqrt(rHaloMin * rHaloMin - r * r);
...
void posiFluxTab(double Emin, double sigmav, double *tab, double *tabOut) {
  ...
  buildInterpolation(integral_cal_In, 0., dKt(tab[0], Emin), -Eps, Delt, &N_,
                     &xa_, &ya_);
  ...
  for (i = 1; i < NZ; i++) {
    ...
    if (Eobs < Emin * 0.9)
      ...
      buff[i] = (flu / Eobs) * simpson(SpectIntegrand, Eobs, tab[0], Eps, NULL);
  }

  for (i = 1; i < NZ; i++)
    tabOut[i] = 2 * rho0 * buff[i];
  tabOut[0] = tab[0];
  free(xa_);
  free(ya_);
}
...
static double sigma_inelastic_NOANN_pbarH_TAN_and_NG(double EK_pbar) {
  if (EK_pbar <= 0.0)
    ...
    double result = 661. * (1. + 0.0115 * pow(EK_pbar, -0.774) -
                            0.948 * pow(EK_pbar, 0.0151));
    return (sigma_inelastic_pbarH_TAN_and_NG(EK_pbar) - result);
  } else
    return sigma_inelastic_pH_TAN_and_NG(EK_pbar);
}
...
static double sigma_inelastic_ANN_pbarH_TAN_and_NG(double T_pbar) {
  ...
  if (T_pbar <= 0.0)
    ...
    result = (sigma_inelastic_pbarH_TAN_and_NG(T_pbar) -
              sigma_inelastic_NOANN_pbarH_TAN_and_NG(T_pbar));
  if (result < 0)
    result = 0.;
  return result;
}
...
static double pbar_PropagatorInfiniteR(double rSrc, double zSrc, double ek) {
  ...
  for (n = 0;; n++) {
    ...
    //      if(n<=lastK) knL=kn[n]; else
    {
      ...
      if (kd) {
        ...
        if (kd > 0)
          ...
          nn = n;
        for (i = 0; i < 10; ++i)
          knL = nn * M_PI - atan(2. * knL / (kd * L_dif));
      }
      //          kn[n]=knL;
      //          lastK=n;
    }
    ...
    propagator +=
        dPropagator * sin(knL * (L_dif - zSrc) / L_dif) * sin(knL) / cn;
    if (fabs(dPropagator) <= 0.1 * Eps * fabs(propagator))
      break;
  }
...
static double rhoQ_2(double r) {
  double prQ = haloProfile(r);
  if (vcsMode)
    prQ *= prQ + rhoClumpEff_(r) / rhoDM;
  return r * r * prQ;
}
static double rhoQ_3(double r) {
  double prQ = haloProfile(r);
  if (vcsMode)
    prQ *= prQ + rhoClumpEff_(r) / rhoDM;
  return r * r * r * prQ;
...
static double thetaIntegrandP(double uTheta) {
  ...
  double prQ;
  double d =
      sqrt(z_ * z_ + (Rsun - r_) * (Rsun - r_) + 4 * r_ * Rsun * sinTh * sinTh);
  if (d >= Rdisk)
    return 0;
  prQ = haloProfile(d);
  if (vcsMode)
    prQ *= prQ + rhoClumpEff_(d) / rhoDM;
...
static double rIntegrandP(double r) {
  ...
  if (RQ <= 0)
    return 0;
  if (drQ > 0)
    thetaMin = 2 * asin(sqrt(drQ / (4 * Rsun * r)));
  else
    thetaMin = 0;
  ...
  if (sinMax >= 1)
    ...
    thetaMax = 2 * asin(sinMax);

  r_ = r;
  prQ = haloProfile(rHalo1);
  if (vcsMode)
    prQ *= prQ + rhoClumpEff_(rHalo1) / rhoDM;

  fluxDM_r_z = 2 * 2 *
                   simpson(thetaIntegrandP, sqrt(thetaMin), sqrt(thetaMax),
                           0.1 * Eps, NULL) +
               2 * thetaMin * prQ;
...
static double zIntegrandP(double u) {
  ...
  z_ = z;
  r1 = Leff / 25;
  p1 = rIntegrandP(r1) * sqrt(r1 * r1 + z * z) / r1;
  r2 = Leff / 20;
  p2 = rIntegrandP(r2) * sqrt(r2 * r2 + z * z) / r2;

  a = (p1 * r2 * r2 - p2 * r1 * r1) / (r2 * r2 - r1 * r1);
  b = (p1 - p2) / (r1 * r1 - r2 * r2);

  Ismall = (a - b * z * z) * (sqrt(r1 * r1 + z * z) - z) +
           b / 3 * (pow(r1 * r1 + z * z, 3. / 2.) - z * z * z);
...
static double pbarPropRate(double ek) {
  ...
  case 0:
    ...
    break;
  case 1:
    Gtot = (DensityH * sigma_inelastic_ANN_pbarH_TAN_and_NG(ek) +
            DensityHe * sigma_inelastic_ANN_pbarHe_TAN_and_NG(ek));
    break;

  default:
    Gtot = 0.8 * sigma_inelastic_pbarH_TAN_and_NG(ek) *
           (DensityH + DensityHe * 2.519842);
  }
  ...
  i2 = simpson(rhoQ_2, rHaloMin, rHalo1, 1.E-2, NULL) -
       rHalo1 * rhoQ_2(rHalo1) / 3;
  i3 = simpson(rhoQ_3, rHaloMin, rHalo1, 1.E-2, NULL) -
       rHalo1 * rhoQ_3(rHalo1) / 4;
...
void pbarFluxTab(double Emin, double sigmav, double *tab, double *tabOut) {
  ...
  double rho0;
  Mcdm0 = tab[0];
  buildInterpolation(logPbarRate, log(Emin / Mcdm0), log(0.9), 0.01, Delt, &N,
                     &Egrid, &Fgrid);
  /*printf("Npbar=%d\n",N);*/
  ...
  for (i = 0; i < NZ; i++) {
    ...
    if (E < Emin * 0.9)
      ...
      tab2[i] = sigmav * exp(polint3(z, N, Egrid, Fgrid)) * zInterp(z, tab);
  }
  for (i = 1; i < NZ; i++)
    tabOut[i] = rho0 * tab2[i];
  tabOut[0] = tab[0];
  free(Egrid);
  free(Fgrid);
}
...
double pBarBackgroundFlux(double E) /* arXiv:astro-ph/0609522v3 */
{
  ...
  if (E < 11) {
    ...
    s = D[0] * pow(x, D[1]);
  if (E < 11)
    return 1.E-4 * exp(s); /* m^2 => cm^2 */
  else
    return 8.621348E-01 * 1.E-4 * exp(s);
}

void pBarBackgroundTab(double Emax, double *pBarTab) {
  int i;
  for (i = 1; i < NZ; i++) {
    double E = Emax * exp(Zi(i));
    pBarTab[i] = E * pBarBackgroundFlux(E);
  }
  pBarTab[0] = Emax;
}