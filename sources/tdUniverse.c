...
static double h1eff_int(double x, void *par_) {
  ...
  if (par->eta == -1 && e < 1E-8)
    ...
    den = exp(e) + par->eta;
  return (par->mu * par->mu + 4. / 3. * p * p) * p * p / den / x / M_PI / M_PI /
         e;
}
...
double h1eff(double mu, int eta) {
  ...
  par.eta = eta;
  par.mu = mu;
  return simpson_arg(h1eff_int, &par, 0, 1, 1E-3, NULL) /
         (2 * M_PI * M_PI / 45);
}

double Hubble(double T) {
  return sqrt(8 * M_PI / 3. * M_PI * M_PI / 30. * gEff(T)) * T * T / MPlank;
}
...
int loadHeffGeff(char *fname) {
  ...
  for (i = 0; i < nRec; i++)
    s3_[i] = t_[i] * pow(2 * M_PI * M_PI / 45. * heff_[i], 0.3333333333333333);
  heff_ln_diff_ = malloc(nRec * sizeof(double));
  polintDiff(nRec, t_, heff_, heff_ln_diff_);
  for (i = 0; i < nRec; i++)
    heff_ln_diff_[i] *= t_[i] / heff_[i];
  Tdim = nRec;
  return nRec;
}
