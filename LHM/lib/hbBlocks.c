...
int hbBlocksMDL(char *fname, int *nHch) {
  ...
  fprintf(f, " %12.4E  3    25    24    24 # higgs-W-W \n",
          SQR((1. + findValW("del") * findValW("del") / 12.) *
              (1. - findValW("del") * findValW("del") * findValW("vh") *
                        findValW("vh") / findValW("v") / findValW("v") / 3.)));
...

  {
    double vev = 2 * findValW("MW") * findValW("SW") / findValW("EE"),
           MH = findValW("MH"), aQCD = alphaQCD(MH) / M_PI,
           LGGSM = lGGhSM(MH, aQCD, findValW("Mcp"), findValW("Mbp"),
                          findValW("Mtp"), vev),
           LAASM = lAAhSM(MH, aQCD, findValW("Mcp"), findValW("Mbp"),
                          findValW("Mtp"), vev);
  ...
  fprintf(f, " %12.4E   %12.4E   3    25     5    5 # higgs-b-b \n",
          SQR(1 - findValW("vh") * findValW("vh") / findValW("f") /
                      findValW("f") / 4.),
          0.);
  fprintf(f, " %12.4E   %12.4E   3    25     6    6 # higgs-top-top \n",
          SQR(1 + findValW("mtcorr") / 2 / findValW("f") / findValW("f") *
                      findValW("B00014")),
          0.);
