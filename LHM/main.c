...
int main(int argc, char **argv) {
...
#ifdef CONSTRAINTS
  {
    ...
    if (LspNlsp_LEP(&csLim))
      printf("LEP excluded by e+,e- -> DM q qbar  Cross section= %.2E\n",
             csLim);
  }
#endif
...
#include "../include/Lilith.inc"
      if (ndf) {
        printf("LILITH(DB%s:  -2*log(L): %.2f; -2*log(L_reference): %.2f; ndf: "
               "%d; p-value: %.2E \n",
...
#include "../include/SMODELS.inc"
...
#ifdef NEUTRINO
  {
    ...
    // ICE CUBE
    if (forSun)
      printf("IceCube22 exclusion confidence level = %.2%%E\n",
