...
static int Integrate(creal epsrel, creal epsabs, cint flags, ccount mineval,
                     ccount maxeval, ccount nnew, creal flatness,
  ...
  for (dim = 0; dim < ndim_; ++dim) {
    ...
    if (dim == 0) {
      ...
      Copy(b->grid, anchor->bounds[0].grid, NBINS);
  }

  Sample(nnew, anchor, anchor->w, anchor->w + nnew,
         anchor->w + (ndim_ + 1) * nnew, flags);
  df = anchor->df;
  ...
  for (nregions_ = 1;; ++nregions_) {
    ...
    Fluct(var[0], flatness, region->bounds, region->w, region->n, maxcomp,
    ...
    nnewL = IMax((minfluct == 0) ? nnew / 2
                                 : (count)(vLR[0].fluct / minfluct * nnew),
                 MINSAMPLES);
    nL = vLR[0].n + nnewL;
