...
static inline void SolveEqs(Cut *cutfirst, Cut *cutlast, creal *delta,
                            creal diff) {
...
static void Split(void *voidregion, int depth) {
  ...
  Region *const region = voidregion, *reg, *next;
...
repeat:
  ...
  for (div = 1; div <= 16; div *= 4) {
    ...
    if (lhssqnew <= lhssq) {
      ...
      if (fabs(gammanew - gamma) < GAMMATOL * gamma)
        break;
      gamma = gammanew;
