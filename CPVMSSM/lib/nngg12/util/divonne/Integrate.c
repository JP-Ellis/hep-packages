...
static int Integrate(creal epsrel, creal epsabs, cint flags, ccount mineval,
                     ccount maxeval, int key1, int key2, int key3,
                     ccount maxpass, creal maxchisq, creal mindeviation,
  ...
  if (VERBOSE > 1) {
    ...
    sprintf(s,
            ...
            "  border %g\n  maxchisq %g\n  mindeviation %g\n"
            "  ngiven %d\n  nextra %d\n",
            epsrel, epsabs, flags, mineval, maxeval, key1, key2, key3, maxpass,
            border_.lower, maxchisq, mindeviation, ngiven_, nextra_);
    Print(s);
  }
  ...
  for (iter = 1;; ++iter) {
    ...
    if (nneed < MAXPRIME) {
      ...
      if (n < nmin) {
        nmin = n;
        pass = 0;
      } else if (++pass > maxpass && n >= mineval)
        break;
    }
...
#define Var(s) Sq((s.err[comp] == 0) ? r->spread *s.weight : s.err[comp])
      ...
      for (comp = 0; comp < ncomp_; ++comp) {
        ...
        creal r2 =
            (s1 == 0) ? Sq(samples_[1].neff * samples_[0].weight) : s2 / s1;
        ...
        if (todo == 3) {
          ...
          creal r3 =
              (s2 == 0) ? Sq(samples_[2].neff * samples_[1].weight) : s3 / s2;
        ...
        if (chisq > EPS)
          chisq /= Max(chiden, NOTZERO);

#define Out(s) s.avg[comp], r->spread *s.weight, s.err[comp]

        if (VERBOSE > 2) {
          p += sprintf(p, "\n[%d] %g +- %g(%g)\n    %g +- %g(%g)", comp + 1,
                       Out(samples_[0]), Out(samples_[1]));
          if (todo == 3)
    ...
    if (VERBOSE > 2) {
      ...
      for (comp = 0; comp < ncomp_; ++comp)
        p += sprintf(p, "\n[%d] %g +- %g  \tchisq %g (%d df)", comp + 1,
                     integral[comp], error[comp], prob[comp], df);
