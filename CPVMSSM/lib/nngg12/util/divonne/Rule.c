...
static inline void RuleIni(Rule *rule) { rule->first = NULL; }
...
static void Rule13Alloc(Rule *rule) {
  static creal w[][nrules] = {
      ...
      {0, .1460984204026913, .1480693879765931, 0, 0},
      ...
      {.015625, -.1420416552759383, -.147279632923196, .003760268580063992,
       .02561989142123099}};

  static creal g[] = {.12585646717265545,
                      .3506966822267133,
                      .4795480315809981,
                      .4978005239276064,
                      .25,
  ...
  for (s = first; s <= last; ++s)
    for (r = 1; r < nrules - 1; ++r) {
      creal scale =
          (s->weight[r] == 0) ? 100 : -s->weight[r + 1] / s->weight[r];
      real sum = 0;
...
static void Rule9Alloc(Rule *rule) {
  static creal w[] = {-.0023611709677855117884, .11415390023857325268,
  ...
  static creal g[] = {.47795365790226950619, .20302858736911986780,
                      .44762735462617812882, .125, .34303789878087814570};
...
static void Rule7Alloc(Rule *rule) {
  static creal w[] = {.019417866674748388428,  -.40385257701150182546,
  ...
  static creal g[] = {.47795365790226950619, .20302858736911986780, .375,
...
static void SampleRule(cSamples *samples, cBounds *b, creal vol) {
  ...
  Set *first = samples->rule->first, *last = samples->rule->last, *s;
  real *errcoeff = samples->rule->errcoeff;
  count comp, rul, n;

  for (s = first; s <= last; ++s)
    if (s->n)
      x = ExpandFS(b, s->gen, x);
  ...
  for (comp = 0; comp < ncomp_; ++comp) {
    ...
    for (rul = 1; rul < nrules - 1; ++rul) {
      real maxerr = 0;
      for (s = first; s <= last; ++s)
        maxerr = Max(maxerr, fabs(sum[rul + 1] + s->scale[rul] * sum[rul]) *
                                 s->norm[rul]);
      sum[rul] = maxerr;
    }
