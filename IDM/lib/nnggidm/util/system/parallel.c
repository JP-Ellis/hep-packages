...
#define WORKER(core, s, ...)                                                   \
  fprintf(stderr, TERM_BLUE "SquaredME core %d: " s TERM_RESET, core,          \
          __VA_ARGS__)
#else
#define MASTER(s, ...)
#define WORKER(core, s, ...)
...
static inline int readsock(int fd, void *data, size_t n) {
  ...
  size_t remain = n;
  do
    got = recv(fd, data, remain, MSG_WAITALL);
...
static inline int writesock(int fd, const void *data, size_t n) {
  ...
  size_t remain = n;
  do
    got = send(fd, data, remain, MSG_WAITALL);
  while (got > 0 && (data += got, remain -= got) > 0);
  return got;
}
...
static inline void newcore(subroutine foo, const int flags) {
  ...
  assert(socketpair(AF_LOCAL, SOCK_STREAM, 0, fd) != -1 &&
         (pid = fork()) != -1);

  if (pid) {
    MASTER("forked core %d pid %d pipe %d(master) -> %d(worker) seq %llx", core,
           pid, fd[0], fd[1], mem_seq);
    close(fd[1]);
    child[core].fd = fd[0];
  ...
  for (;;) {
    ...
    if (!readsock(fd[1], mem.h, mem.he - mem.h))
      exit(0);
    WORKER(core, "seq %llx  new %llx", seq, mem_seq);
    seq ^= mem_seq;
    if (SEQ_ANGLE(seq)) {
      WORKER(core, "reading mem_angle(%p#%ld+%p#%ld)", mem.v, mem.ve - mem.v,
             mem.a, mem.ae - mem.a);
...
static inline int readycore(RealType *result) {
  ...
  for (; core < ncores; ++core)
    if (FD_ISSET(child[core].fd, &ready))
      break;
...
static inline void oldcore(const int core) {
  ...
  MASTER("sending mem_hel(%p#%ld) to core %d", mem.h, mem.he - mem.h, core);
  writesock(fd, mem.h, mem.he - mem.h);
  seq ^= mem_seq;
  if (SEQ_ANGLE(seq)) {
    MASTER("sending mem_angle(%p#%ld+%p#%ld) to core %d", mem.v, mem.ve - mem.v,
           mem.a, mem.ae - mem.a, core);
    writesock(fd, mem.v, mem.ve - mem.v);
    writesock(fd, mem.a, mem.ae - mem.a);
  }
  if (SEQ_S(seq)) {
    MASTER("sending mem_s(%p#%ld) to core %d", mem.s, mem.se - mem.s, core);
...
void sqmeprep_(char *h, char *he, char *v, char *ve, char *a, char *ae, char *s,
               char *se) {
#ifdef HAVE_FORK
  void sqmewait_(void);
...
void sqmeexec_(subroutine foo, RealType *result, const int *flags) {
  ...
  if (ncores >= MINCORES) {
    if (nlaunched < ncores)
      ...
      oldcore((nrunning < ncores) ? nrunning : readycore(result));
    ++nrunning;
  } else
#endif
    foo(result, flags);
