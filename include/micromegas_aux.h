...
#define RhoCrit100                                                             \
  10.537 /* A critical density of Universe                                     \
...
extern void displayPlotN(char *title, char *xName, double xMin, double xMax,
                         int Scale, int N, char **Y, int *Dim, double **f,
                         double **ff);
extern int displayPlot(char *title, char *xName, double xMin, double xMax,
                       int Scale, int N, ...);

#define displayFunc(title, Func, xMin, xMax, xName, Scale)                     \
  displayPlot(title, xName, xMin, xMax, Scale, 1, title, 0, Func, NULL)

// integration
extern double simpson(double (*func)(double), double a, double b, double eps,
                      int *err);
extern double simpson_arg(double (*func)(double, void *par), void *par,
                          double a, double b, double eps, int *err);
extern int simpson_err;
extern double gauss(double (*func)(double), double a, double b, int n);
extern double gauss_arg(double (*func)(double, void *par), void *par, double a,
                        double b, int n);
extern double gauss345_arg(double (*func)(double, void *), void *par, double a,
...
extern int stiff(int first, double xstart, double xend, int n, double *y,
                 double *yscal, double eps, double *htry,
                 void (*derivs)(double, double *, double *, double, double *,
                                double *));
extern int stifbs(int first, double xstart, double xend, int nv, double *y,
                  double *yscal, double eps, double *htry,
                  void (*derivs)(double, double *, double *, double, double *,
                                 double *));
...
extern int buildInterpolation(double (*Fun)(double), double x1, double x2,
                              double eps, double delt, int *N, double **xa,
                              double **ya);

extern void spline(double *x, double *y, int n, double *y2);
extern void splint(double *xa, double *ya, double *y2a, int n, double x,
                   double *y);
...
extern double K1to2(double m1, double m2, double m3, double s1, double s2,
                    double s3);
extern double Stat2(double P, double M, double m1, double m2, double eta1,
                    double eta2);
...
extern double (*sqme22)(int nsub, double GG, REAL *pvect, REAL *cb_coeff,
                        int *err_code);

extern int kin22(double PcmIn, REAL *pmass);
extern double kinematic_23(double Pcm, int i3, double M45, double cs1,
                           double cs2, double fi, REAL *pmass, REAL *pvect);
extern double kinematic_24(double Pcm, int i3, int i4, double M1, double M2,
...
#define NTOF(X)                                                                \
  extern forCalchep1 X;                                                        \
  double X(double ok) { return findValW(#X); }
...
extern int vPolar(int out1, int out2, int out3, double *left, double *right,
                  double *lng);
extern double plazmaWidth(char *process, double T);

extern double cs23MM(numout *cc, int nsub, double Pcm, int fast, int ii3,
                     double M45min, double M45max);

extern double cs23(numout *cc, int nsub, double Pcm, int ii3, int *err);

extern double amoeba(double *p, double *dp, int ndim, double (*f)(double *),
                     double eps, int *nCalls);

extern REAL *Qaddress;

extern double lGGhSM(double Mh, double aQCDh, double Mcp, double Mbp,
                     double Mtp, double vev);
extern double lAAhSM(double Mh, double aQCDh, double Mcp, double Mbp,
                     double Mtp, double vev);
