...
double usrfun_(char *name, int n_in, int n_out, double *pvect, char **pnames,
               int *pdg) {
...
extern void evolvepdfphotonm_(int *P, double *x, double *Q, double *f,
                              double *fph);
...
void initpdfm(int *P, int *nSet, double *xMin, double *xMax, double *qMin,
              double *qMax) {
  initpdfm_(P, nSet);
  getxmaxm_(P, nSet, xMax);
  getxminm_(P, nSet, xMin);
  getq2maxm_(P, nSet, qMax);
  *qMax = sqrt(fabs(*qMax));
  getq2minm_(P, nSet, qMin);
  *qMin = sqrt(fabs(*qMin));
...
int qnumbers(char *pname, int *spin2, int *charge3, int *cdim) {
  ...
  for (n = 0; n < nModelParticles; n++) {
    if (!strcmp(pname, ModelPrtcls[n].name)) {
      sign = 1;
      break;
    }
    if (!strcmp(pname, ModelPrtcls[n].aname)) {
      sign = -1;
      break;
    }
  }
  if (n == nModelParticles)
    return 0;

  if (spin2)
    *spin2 = ModelPrtcls[n].spin2;
  if (charge3)
    *charge3 = sign * ModelPrtcls[n].q3;
  if (cdim) {
    *cdim = ModelPrtcls[n].cdim;
    if (sign == -1 && (*cdim == 3 || *cdim == -3))
      (*cdim) *= -1;
  }
