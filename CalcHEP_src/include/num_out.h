...
extern int pinfAux_ext(int nsub, int nprtcl, int *spin2, int *color,
                       int *neutral, int *ndf);
extern char *varName_ext[];

extern double sqme_ext(int nsub, double GG, REAL *momenta, REAL *cb_coeff,
                       int *err);
...
extern int jobInState(int nProc, double P1, double P2, char *strf1,
                      char *strf2);
...
extern double jobVegas(int nSess, int nCalls, int clear, int *err_, double *dI,
                       double *chi2);
#endif
