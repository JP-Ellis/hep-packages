...
extern double usrfun(char *name, int nIn, int nOut, double *pvect, char **pName,
                     int *pCode);
/*
...
double usrfun(char *name, int nIn, int nOut, double *pvect, char **pName,
              int *pCode) {
  ...
  {                                // read p1&p2
    ...
    for (i = nIn; i < nIn + nOut; i++) {
      if (strcmp(p1, p2) == 0)
        ...
        j = nIn;
      for (; j < nIn + nOut; j++)
        if (strcmp(p1, pName[i]) == 0 &&
            strcmp(p2, pName[j]) == 0) // find position of particles
        {
          double *q1 = pvect + 4 * i, *q2 = pvect + 4 * j;
          double Et1 = sqrt(
              fabs(q1[0] * q1[0] -
                   q1[3] * q1[3])); // transvers energy of the first particle
          double Et2 = sqrt(
              fabs(q2[0] * q2[0] -
                   q2[3] * q2[3])); // transvers energy of the second particle
          sum += sqrt((Et1 + Et2) * (Et1 + Et2) -
                      (q1[1] + q2[1]) * (q1[1] + q2[1]) -
                      (q1[2] + q2[2]) * (q1[2] + q2[2])); // sqrt(E^2-PL^2)
        }
    }
  } else {
    printf("Not defined user function %s\n", name);
    exit(2);
  }
