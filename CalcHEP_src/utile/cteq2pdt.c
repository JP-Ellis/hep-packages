...
int main(int npar, char **parch) {
  ...
  if (p)
    p[0] = 0;
  for (i = 0; i < 4; i++)
    if (toupper(version[i]) != cteq[i]) {
      fprintf(stderr, "Non expected file format\n");
      return 1;
    }
  Ncteq = version[4] - '0';
  switch (Ncteq) {
  ...
  case 6:
    break;
  default: {
    fprintf(stderr, "Unknown version CTEQ%d \n", Ncteq);
    return 2;
  }
  }

  skipline(2);
  if (6 != scanf("%lf %lf %lf %*lf %*lf %*lf %lf %lf %lf", &ordrf, &nff,
                 &lambda, &Mc, &Mb, &Mt))
    goto errExit;
  nf = nff;
  ordr = ordrf;
  skipline(2);
  if (3 != scanf("%d %d %d", &nx, &nt, &NfMx))
    goto errExit;
