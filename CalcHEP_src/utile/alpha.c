...
int writeAlpha(FILE *f, int nf, int ordr, double lambda, int nfMx, double Mc,
               double Mb, double Mt, int N, double *q) {
  int nf3, nf4, nf5, nf6;
  double L3, L4, L5, L6;
  double al;
  int i, j;
  ...
  for (i = 0, j = 1; i < N; i++, j++) {
    ...
    double Q = q[i];
    if (Q < 1.4)
      al = alpha(nf3, ordr, L3, Q);
    else if (Q < 4.5)
      al = alpha(nf4, ordr, L4, Q);
    else if (Q < 175.)
      al = alpha(nf5, ordr, L5, Q);
    else
      al = alpha(nf6, ordr, L6, Q);
    fprintf(f, " %.5E", al);
    if (j == 10) {
      fprintf(f, "\n");
      j = 0;
