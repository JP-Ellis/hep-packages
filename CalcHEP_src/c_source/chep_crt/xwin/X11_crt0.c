...
static int read_ini(char *fname) {
  ...
  while (fgets(str, 195, ini) != NULL) {
    ...
    parm1[0] = 0;
    sscanf(str, "%s %[^\n]", name, parm1);
    while (strlen(parm1) && parm1[strlen(parm1) - 1] == ' ')
      parm1[strlen(parm1) - 1] = 0;
    if (!strcmp(name, "fontFamily")) {
      strcpy(BGI_DEFAULT_FONT, parm1);
      strcpy(BGI_DEFAULT_FONT, parm1);
    } else if (!strcmp(name, "fontWidth")) {
      sscanf(parm1, "%d", &wBest);
    } else if (!strcmp(name, "fontCurrent")) {
      strcpy(currentFont, parm1);
    } else if (!strcmp(name, "colors")) {
      if (strcmp(parm1, "off") == 0)
        bgi_maxcolor = 0;
...
static int write_ini(void) {
  ...
  if (bgi_maxcolor)
    fprintf(ini, "colors on\n");
  else
    fprintf(ini, "colors off\n");
  if (BeepOn)
    fprintf(ini, "sound  on\n");
  else
    fprintf(ini, "sound  off\n");
  fclose(ini);
  return 0;
}
...
static void setallpalette(void) {
  int i;
  int palette_colors[16] = {0,   1,   2,   3,   4,   5,   036, 7,
                            070, 071, 072, 073, 074, 075, 076, 077};
  if (!bgi_maxcolor) {
    for (i = 0; i < 8; i++)
      bgi_colors[i] = BlackPixel(display, screen);
    for (i = 8; i < 16; i++)
      bgi_colors[i] = WhitePixel(display, screen);
  } else {
    for (i = 1; i <= bgi_maxcolor; i++)
      setpalette(i, palette_colors[i]);
    for (i = bgi_maxcolor + 1; i < 15; i++)
      palette_colors[i] = palette_colors[i - bgi_maxcolor];
...
static int findFont(char *fontName, int dir) {
  ...
  allFonts =
      XListFontsWithInfo(display, BGI_DEFAULT_FONT, 3000, &nFonts, &info);
  if (!allFonts) {
  ...
  for (;;) {
    ...
    if (dir == 0)
      break;
    if (strcmp(currentFont, allFonts[n0]))
      break;
    if (dir > 0) {
      if (wBest < wMax)
        wBest++;
  ...
  if (dir && strcmp(currentFont, allFonts[n0]) == 0)
    return 1;
...
static int bgi_0to2(char *window_name, char *icon_file) {
  ...
  XInitThreads();
  if ((display = XOpenDisplay(dsp_name)) == NULL)
    return -3;
  ...
  if (strlen(currentFont))
    bgi_font = XLoadQueryFont(display, currentFont);
  if (!bgi_font && 0 == findFont(currentFont, 0))
    bgi_font = XLoadQueryFont(display, currentFont);
  text_font = bgi_font;
  ...
  if (bgi_maxcolor && depth >= 3) {
    ...
    bgi_maxcolor = 0;
  win = XCreateWindow(display, RootWindow(display, screen), 0, 0, bgi_maxx + 1,
                      bgi_maxy + 1, 3, depth, InputOutput, CopyFromParent, 0,
                      &setwinattr);

  {
    int hx, hy;
    unsigned w, h;
    if (XReadBitmapFile(display, win, icon_file, &w, &h, &icon_pixmap, &hx,
                        &hy) != BitmapSuccess)
      icon_pixmap = None;
  }
  ...
  XSetStandardProperties(display, win, window_name, window_name, icon_pixmap,
                         NULL, 0, &size_hints);
...
void crt0_puts(int xc, int yc, int color, int bkcolor, char *s) {
  ...
  if (isProportional) {
    ...
    XFillRectangle(display, win, gc, xc, yc, xw_fn_w * strlen(s), xw_fn_h);

    XSetForeground(display, gc, bgi_colors[color]);
    for (i = 0; i < strlen(s); i++)
      XDrawString(display, win, gc, xc + i * xw_fn_w, yc + text_font->ascent,
                  s + i, 1);
  } else {
    XSetForeground(display, gc, bgi_colors[color]);
    XSetBackground(display, gc, bgi_colors[bkcolor]);
    XDrawImageString(display, win, gc, xc, yc + text_font->ascent, s,
                     strlen(s));
  }
}
...
int crt0_keypressed(void) {
  ...
  while (
      XCheckMaskEvent(display, ExposureMask | StructureNotifyMask, &report)) {
    switch (report.type) {
    case Expose:

      if (xw_expose != NULL)
        (*xw_expose)(report.xexpose.x, report.xexpose.y, report.xexpose.width,
    ...
    case ConfigureNotify:
      ...
      if (xw_expose != NULL)
        (*xw_expose)(0, 0, bgi_maxx, bgi_maxy);
      sizeChanged = 1;
    }
  }
...
int crt0_inkey(void) {
  ...
  for (;;) {
    ...
    case KeyPress: {
      ...
      if (Ctrl_mem &&
          (keysym == XK_plus || keysym == XK_equal || keysym == XK_greater) &&
          wBest < wMax) {
        wBest++;
        if (0 == findFont(newFont, 1)) {
          setNewFont(newFont);
          printf(" Increased\n");
          write_ini();
        } else {
          printf("no larger font\n");
          crt0_beep();
        }
        break;
      }

      if (Ctrl_mem && (keysym == XK_minus || keysym == XK_less) &&
          wBest > wMin) {
        wBest--;
        if (0 == findFont(newFont, -1)) {
          setNewFont(newFont);
    ...
    case ButtonPress:
      ...
      if (report.xbutton.button == Button3)
        mouse_info.but3 = 1;
      if (report.xbutton.button == Button4)
        mouse_info.but4 = 1;
      if (report.xbutton.button == Button5)
        mouse_info.but5 = 1;
      return KB_MOUSE;
...
void sg_drawLine(int x1, int y1, int x2, int y2, int color, int thickness,
                 int style) {
  if (thickness == NormWidth)
    thickness = 0;
  else
  ...
  if (style == SolidLn)
    ...
    if (style == DashedLn) {
      ...
      dy /= 2 * N - 1;
      for (i = 0; i < N; i++)
        XDrawLine(display, win, gc, x1 + 2 * i * dx, y1 + 2 * i * dy,
                  x1 + (2 * i + 1) * dx, y1 + (2 * i + 1) * dy);
    } else {
      N *= 2;
      dx /= 4 * (N - 1) + 1;
      dy /= 4 * (N - 1) + 1;
      for (i = 0; i < N; i++)
        XDrawLine(display, win, gc, x1 + 4 * i * dx, y1 + 4 * i * dy,
                  x1 + (4 * i + 1) * dx, y1 + (4 * i + 1) * dy);
    }
  }
}
...
void sg_outText(int x, int y, int color, char *s) {
  if (!xw_graphic) {
    XSetFont(display, gc, bgi_font->fid);
    xw_graphic = 1;
  }
