...
static int bgi_x = 0, bgi_y = 0, bgi_maxx = 639, bgi_maxy = 479, CdX = 8,
           CdY = 19;
...
static void texline(int x1, int y1, int x2, int y2,
                    int arrowflag) /* by A.Pukhov */
{
  ...
  case SolidLn:
    switch (arrowflag) {
    case 1:
      f_printf(out_tex, "\\ArrowLine(%.1f,%.1f)(%.1f,%.1f) \n", x, y, fx, fy);
      break;
    case -1:
      f_printf(out_tex, "\\ArrowLine(%.1f,%.1f)(%.1f,%.1f) \n", fx, fy, x, y);
      break;
    case 0:
      f_printf(out_tex, "\\Line(%.1f,%.1f)(%.1f,%.1f) \n", x, y, fx, fy);
    }
    break;
  case DottedLn:
    switch (arrowflag) {
    case 1:
      f_printf(out_tex, "\\DashArrowLine(%.1f,%.1f)(%.1f,%.1f){%.1f} \n", x, y,
               fx, fy, dotsiz);
      break;
    case -1:
      f_printf(out_tex, "\\DashArrowLine(%.1f,%.1f)(%.1f,%.1f){%.1f} \n", fx,
      ...
      break;
    case 0:
      f_printf(out_tex, "\\DashLine(%.1f,%.1f)(%.1f,%.1f){%.1f}\n", x, y, fx,
               fy, dotsiz);
    }
...
{
  if (strcmp(font, "tiny") == 0)
    texCharScale = 0.672;
  else if (strcmp(font, "scriptsize") == 0)
    texCharScale = 0.782;
  else if (strcmp(font, "footnotesize") == 0)
    texCharScale = 0.837;
  else if (strcmp(font, "small") == 0)
    texCharScale = 0.927;
  else if (strcmp(font, "normalsize") == 0)
    texCharScale = 1.0;
...
static void addToGraphList(int type, int x1, int y1, int x2, int y2, int color,
                           int aux, char *text) /* by A.Pukhov */
{
  ...
  while (p) {
    ...
    switch (type) {
    case bar:
      del = (MIN(p->x1, p->x2) >= MIN(x1, x2) &&
             MAX(p->x1, p->x2) <= MAX(x1, x2) &&
             MIN(p->y1, p->y2) >= MIN(y1, y2) &&
             MAX(p->y1, p->y2) <= MAX(y1, y2));
      break;
    case t_xt:
      ...
      break;
    case line:
      del = p->type == line && p->x1 == x1 && p->y1 == y1 && p->x2 == x2 &&
            p->y2 == y2;
      break;
    case string:
      del = p->type == string && p->x1 == x1 && p->y1 == y1 &&
...
static void repeatGrCom(int x1, int y1, int x2, int y2,
                        int all) /* by A.Pukhov */
{
  ...
  while (p != NULL) {
    ...
    if (yy2 >= y1 && yy1 <= y2 && xx1 <= x2 && xx2 >= x1)
      switch (p->type) {
      case bar:
        sg_drawBox(p->x1, p->y1, p->x2, p->y2, p->color);
        break;
      case line:
        sg_drawLine(p->x1, p->y1, p->x2, p->y2, p->color, p->aux & 15,
                    p->aux / 16);
        break;
      case string:
        sg_outText(p->x1, p->y1, p->color, p->text);
        break;
      case t_xt:
        if (all)
...
void tg_clearviewport(void) {
  tg_bar(0, 0, bgi_vport.right - bgi_vport.left,
         bgi_vport.bottom - bgi_vport.top);
  bgi_x = 0;
  bgi_y = 0;
}
...
void tg_line(int x1, int y1, int x2, int y2) {
  if (texflag)
    ...
    y2 += bgi_vport.top;
    if (blind != 1) {
      sg_drawLine(x1, y1, x2, y2, fColor, bgi_line.thickness,
                  bgi_line.linestyle);
...
void tg_arrowline(int x1, int y1, int x2, int y2) {
  if (texflag)
    ...
    tg_line((int)xc, (int)yc, (int)(xc - 2 * dx - 2 * dy),
            (int)(yc - 2 * dy + 2 * dx));
    tg_line((int)xc, (int)yc, (int)(xc - 2 * dx + 2 * dy),
            (int)(yc - 2 * dy - 2 * dx));
...
void tg_linerel(int dx, int dy) {
  tg_line(bgi_x, bgi_y, bgi_x + dx, bgi_y + dy);
  bgi_x += dx;
  bgi_y += dy;
}

void tg_lineto(int x, int y) {
  tg_line(bgi_x, bgi_y, x, y);
  bgi_x = x;
  bgi_y = y;
}
...
int tg_textheight(char *textstring) {
  int dx, dy;
  if (texflag)
    return (int)(10 * texCharScale / texyscale);
  if (blind != 1)
    sg_textSize(textstring, &dx, &dy);
  else
    dy = 8;
...
void tg_outtextxy(int x, int y, char *textstring) {
  ...
  if (texflag)
    ...
    if (blind != 1) {
      ...
      addToGraphList(string, x, y, x + dx - 1, y - dy + 1, fColor, 0,
                     textstring);
    }
  }
}
           ...
           void (*xw_errorExit)(void)) /* by A.Pukhov */
{
  ...
  if (blind != 1) {
    ...
    if (err == 1) {
      printf(
          "You have launched CalcHEP compiled without X11 (/usr/include/X11 is "
          "empty)\n"
...
static void print0(int xc, int yc, int color, int bkcolor,
                   char *s) /* by A.Pukhov*/
{
  char buff[STRSIZ];
  int i, x, y, h1, w1;
  ...
  strcpy(buff, s);
  if (strlen(s) + xc / CdX > maxCol())
    buff[maxCol() - xc / CdX] = 0;
...
void put_text(void **dump) /* by A.Pukhov */
{
  ...
  while (p != NULL) {
    ...
    addToGraphList(t_xt, p->x1 * kx, p->y1 * ky, p->x2 * kx, p->y2 * ky,
                   p->color, p->aux, p->text);
    q = p;
    p = p->next;
    free(q->text);
    free(q);
  ...
  for (;;) {
    ...
    if (q) {
      if (blind != 1) {
        ...
        addToGraphList(t_xt, xc * CdX, yc * CdY, (xc + q) * CdX - 1,
                       (yc + 1) * CdY - 1, fColor, bColor, d);
      }
      xc += q;
    }
...
/* LaTeX   by A.Pukhov  */
double texX(double x) { return (x + texxshift) * texxscale; }

double texY(double y) { return (texymax1 - y) * texyscale; }
...
int escpressed(void) /* by A.Pukhov */
{
  ...
  if (crt0_keypressed())
    ...
    return 0;
  for (;;) {
    if ((key == KB_ESC) || (key == KB_BACKSP) || (key == 5))
      return 1;
    if (crt0_keypressed())
      key = crt0_inkey();
    else
