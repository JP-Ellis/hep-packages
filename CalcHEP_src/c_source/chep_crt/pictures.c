...
void pictures(int n_diag_tot, int sizeX, int sizeY, void (*pict)(int, int, int),
              char *commandStr, int (*command)(int, char), char *help) {
  /* to  sreen restore */
  void *prtscr;

/* up command line */
#define DNMAX 8
  int dnKey[DNMAX] = {KB_F1,   KB_F2,  KB_PAGEU, KB_PAGED,
                      KB_HOME, KB_END, '#',      KB_ESC};
...
redrawScreen:
  ...
  dx = 3;
  ndi_x = (maxx - 2 * x0 - dx) / (dx + sizeX);
  dy = 3;
  ndi_y = (maxy - 2 * y0 - dy) / (dy + sizeY);
  ...
  for (i = 0; i <= ndi_x; i++) {
    tg_line(x0 + i * lx, y0, x0 + i * lx, y0 + ndi_y * ly);
  }
  for (i = 0; i <= ndi_y; i++) {
    tg_line(x0, y0 + i * ly, x0 + ndi_x * lx, y0 + i * ly);
  }
  ...
  for (;;) {
    ...
    for (j = 0; j < ndi_y; j++)
      for (i = 0; i < ndi_x; i++) {
        if (redraw) {
          scrcolor(Black, White);
          tg_bar(x0 + i * lx + 1, y0 + j * ly + 1, x0 + i * lx + lx - 1,
                 y0 + j * ly + ly - 1);
          if (l <= n_diag_tot && redraw)
            (*pict)(l, x0 + i * lx + dx, y0 + j * ly + dy);
        }
        scrcolor(Black, White);
        tg_setlinestyle(SolidLn, ThickWidth);
        if (l == n) {
          tg_rectangle(x0 + i * lx + 2, y0 + j * ly + 2, x0 + i * lx + lx - 2,
                       y0 + j * ly + ly - 2);
          i0 = i;
          j0 = j;
        }
    ...
    if (key == KB_MOUSE && mouse_info.but1 == 2) {
      ...
      if (mouse_info.y <= cHeight) {
        i = 0;
        while (i < imax_up && upLen[i] < mouse_info.x)
          i++;
        if (i != imax_up)
          key = upKey[i];
      }
      ...
      /* down line */
        ...
        i = 0;
        while (i < imax_dn && dnLen[i] < mouse_info.x)
          i++;
        if (i != imax_dn)
          key = dnKey[i];
      }
    ...
    if (maxx != tg_getmaxx() || maxy != tg_getmaxy())
      goto redrawScreen;
