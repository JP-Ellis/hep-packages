...
ret_:
  ...
  /* mouse filter */
  if (mess == KB_MOUSE) {
    if ((mouse_info.but1 != 2) || (mouse_info.row < y1) ||
        (mouse_info.row > y2) || (mouse_info.col < x1) || (mouse_info.col > x2))
      goto ret_;
    if (mouse_info.row == y2) {
      if (mouse_info.col < (x1 + x2) / 2 /* -1*/)
        mess = 'Y';
      if (mouse_info.col > (x1 + x2) / 2 /*+1 */)
        mess = 'N';
    }
...
int informline(long curent, long total) {
  return infoLine((double)curent / (double)total);
}
...
void menu0(int col, int row, char *label, char *menstr, void (**funcKey)(int),
           char **funcKeyMess, void **hscr, int *kk) {
  ...
  if (funcKey == NULL)
    ...
    for (i = 0; i < 10; i++) {
      ...
      if (funcKey[i] && funcKeyMess[i]) {
        scrcolor(help_fg1, help_bg);
        print(" F%i-", i + 1);
        scrcolor(help_fg2, help_bg);
        print(funcKeyMess[i]);
      }
    }
    fkPos[10] = where_x();
  ...
  if (row + height + 1 > lastLine - 2)
    height = lastLine - 3 - row;
  lastpage = 1 + (strlen(menstr) / ncol - 1) / height;
  if (label[0] == 0 || row == 1) {
    if (*hscr == NULL)
      get_text(col, row, col + ncol + 1, row + 2, hscr);
  } else {
    char label_[STRSIZ];
    int shft, sz;
    if (*hscr == NULL)
      get_text(col, row - 1, col + ncol + 1, row + 2, hscr);
    for (i = 0; i < ncol + 2; i++)
      label_[i] = ' ';
    label_[ncol + 2] = 0;
    sz = strlen(label);
...
label_1:
  ...
  if (npage < lastpage)
    ...
    lastrow = (strlen(menstr) / ncol) % height;
  ...
  for (j = 1; j <= height; j++) {
    ...
    if (shift < strlen(menstr)) {
      print(fmt, menstr + shift);
      lastrow++;
    } else
      print(fmt, " ");
  }
  ...
  while (1) {
    ...
    if ((ink == KB_MOUSE) && (mouse_info.but1 == 2)) {
      if (mouse_info.row == lastLine) {
      ...
      } else if ((mouse_info.col < col) || (mouse_info.col > col + ncol + 1) ||
                 (mouse_info.row < row) || (mouse_info.row > row + height + 1))
        goto label_3;
      else {
        ...
        if (mousePos == 0) {
          if (col + ncol + 1 - mouse_info.col < 4)
            ink = KB_PAGEU;
          else if (mouse_info.col - col < 4)
            ink = KB_ESC;
        } else if (mousePos == height + 1)
          ink = KB_PAGED;
  ...
  label_4:
    switch (ink) {
      /*
              ...
              if (mouse_info.row == lastLine )
              for(i=0; i<10;i++)
                if ((mouse_info.col > fkPos[i]) && (mouse_info.col <
         fkPos[i+1])) {  if (i==9)ink='0'; else ink='1'+i; goto label_4;
                }
...
void menu1(int col, int row, char *label, char *menustr, char *help, void *hscr,
           int *kk) {
  void (*funcKey[11])(int);
  char *funcKeyMess[11];
  ...
  for (i = 0; i < 8; i++) {
    funcKey[2 + i] = f3_key[i];
    funcKeyMess[2 + i] = f3_mess[i];
  }
  ...
  do {
    ...
    if (key == KB_ENTER) {
      ...
      if (type < 0)
        ...
        switch (type) {
        case 'L':
          err = sscanf(buff, "%ld", L);
          break;
        case 'D':
          err = sscanf(buff, "%lf", D);
          break;
        case 'I':
          err = sscanf(buff, "%d", I);
          break;
      ...
      if (err <= 0)
        messanykey(10, 10, " incorrect number");
    }
...
int improveStr(char *str, char *mark, char *format, ...) {
  ...
  for (i = strlen(buff); i < strlen(mark); i++)
    sub[i] = ' ';
  return 0;
}
