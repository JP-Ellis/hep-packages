...
void showtext(int x1, int y1, int x2, int y2, char *headline, FILE *fileptr) {
  int key;
  void *pscr;
  int i, l;
  ...
  if (y2 <= y1 + 1)
    for (y2 = y1 + 2;
         y2 < maxRow() && readnewline(fileptr, width, &currentline); y2++)
  ...
  do {
    ...
    case KB_PAGEU:
      for (i = y1 + 1; i < y2; i++)
        if (currentline->pred != NULL)
          currentline = currentline->pred;
      break;
    ...
    key = inkey();
    /* mouse filter */
    if ((key == KB_MOUSE) && (mouse_info.but1 == 2) && (mouse_info.col >= x1) &&
        (mouse_info.col <= x2)) {
      if (x2 - mouse_info.col < 4) {
        if (mouse_info.row == y1)
          key = KB_PAGEU;
        else if (mouse_info.row == y2)
          key = KB_PAGED;
      }
      if ((mouse_info.row == y1) && (mouse_info.col - x1 < 3))
        key = KB_ESC;
      if ((mouse_info.row > y1) && (mouse_info.row < y2)) {
        if (mouse_info.row < (y1 + y2) / 2)
          key = KB_UP;
        else
          key = KB_DOWN;
      }
...
int readtable0(table *tab, FILE *txt) {
  ...
  if (txt == NULL)
    return 1;
  cleartab(tab);
  if (!fgets1(tab->mdlName, STRSIZ, txt))
    return 1;
  if (!fgets1(tab->headln, 70, txt))
    return 1;
  if (fgets1(tab->format, STRSIZ, txt) <= 0)
    return -2;
  len = strlen(tab->format);
  while (len && tab->format[len] != '|')
    len--;
  if (len == 0)
    return 1;
  tab->format[len + 1] = 0;
  ...
  while (fgets1(newline->line, len + 1, txt) && newline->line[0] != '=') {
    ...
    for (i = strlen(newline->line); i <= len; i++)
      newline->line[i] = ' ';
    lastln->next = newline;
    newline->next = NULL;
    newline->pred = lastln;
