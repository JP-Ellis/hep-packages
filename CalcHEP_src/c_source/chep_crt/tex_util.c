...
void texStart(char *f_name, char *upstr, char *charSize) {
  ...
  if (setTexCharSize(charSize))
    f_printf(out_tex, "\\%s", charSize);
  else
    f_printf(out_tex, "\\scriptsize");
...
int texmenu(int *pictureX, int *pictureY, char *letterSize) {
  ...
  for (;;) {
    ...
    case 0:
      return 0;
    case 1:
      correctInt(15, 20, "Picture X-size=", pictureX, 1);
      correctInt(15, 20, "Picture Y-size=", pictureY, 1);
      break;
    case 2:
      menu0(12, 12, "", sizestr, NULL, NULL, &pscr2, &l);
      if (l != 0) {
        put_text(&pscr2);
