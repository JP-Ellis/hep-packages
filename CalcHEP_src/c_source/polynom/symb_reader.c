...
symb_all rd_symb(char *s) {
  ...
  if (isdigit(s[0])) {
    ...
    for (i = 0; i < vardef->nvar; i++)
      if (strcmp(vardef->vars[i].name, s) == 0) {
        p->expr.p = plusone();
  ...
  if (strcmp(ch, "-") == 0) {
    *mm = symb_imult(*mm, 1, -1);
    ...
    for (; t; t = t->next) {
      ...
      s->g[0] = np ? np : 1;
      delpoly(&(s->tcoef->re));
      s->tcoef->re = copypoly(t->re);
      delpoly(&(s->tcoef->im));
      s->tcoef->im = copypoly(t->im);
      addSpin(&sum, s);
    }
    mm->type = spintp;
    deltensor(&(mm->expr.t));
...
static symb_all bact_(char ch, symb_all mm1, symb_all mm2) {
  ...
  case '/':
    ...
    sortie(60);
  case '+':
    *mm3 = symb_sum(*mm1, 0, *mm2, 0);
    break;
  case '*':
    *mm3 = symb_mult(*mm1, 0, *mm2, 0);
    break;
  case '^': {
...
static symb_all act_symb(char *name, int n, symb_all *args) {
  if (n == 1)
    return uact_(name, args[0]);
  if (name[0] == '-') {
    uact_(name, args[0]);
    strcpy(name, "+");
  }
  if (n == 2)
    return bact_(name[0], args[0], args[1]);
...
symb_data symb_read(char *s) {
  ...
  m = (symb_all)readExpression(s, (rdelement)rd_symb, (operation)act_symb,
                               NULL);
  if (m) {
    ret = *m;
    delunit(m);
