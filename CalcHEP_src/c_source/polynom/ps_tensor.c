...
static tensor multeps(char *e1, char *e2) {
  ...
  static struct epsdata {
    int contr[4];
    int sgn;
  } epsdata[24] = {{{0, 1, 2, 3}, 1},  {{1, 0, 2, 3}, -1}, {{1, 2, 0, 3}, 1},
  ...
  for (i1 = 0, i2 = 0, j1 = 0, j2 = 0;
       i1 < 4 && i2 < 4 && e1[i1] > 0 && e2[i2] > 0;) {
    if (e1[i1] == e2[i2]) {
      i1++;
      i2++;
      sgn = 1 & (sgn + i1 - i2);
  ...
  for (k = 0; k < epsnumb[L]; k++) {
    ...
    for (i1 = 0; i1 < L && t; i1++) {
      ...
      if (cc > 0)
        ...
        multtenspoly(&t, scalarmult(c, cc));
    }
...
Etens mult2Etens(Etens S1, Etens S2) {
  ...
  */
  ...
  if (f1 && f2)
    sumt = multtwotens(f1->tcoef, f2->tcoef);
  else
    sumt = NULL;
  ...
  for (s1 = S1; s1; s1 = s1->next)
    for (s2 = S2; s2; s2 = s2->next) {
      tensor t2, t1;
