...
symb_data symb_copy(symb_data S) {
  ...
  case numbertp:
  case polytp:
    ans.expr.p = copypoly(S.expr.p);
    break;
  case vectortp:
  case indextp:
  case tenstp:
    ans.expr.t = copytens(S.expr.t);
    break;
  case spintp:
    ans.expr.s = copySpin(S.expr.s);
    break;
  case etenstp:
    ans.expr.et = copyEtens(S.expr.et);
    break;
  }
  return ans;
}
...
symb_data symb_imult(symb_data S, int del, NUM_TYPE factor) {
  ...
  case numbertp:
  case polytp:
    multpolyint(&(ans.expr.p), factor);
    break;
  case vectortp:
  case tenstp:
    multtensint(&(ans.expr.t), factor);
    break;
  case spintp:
    multSpinInt(&(ans.expr.s), factor);
    break;
  case etenstp:
    multEtensInt(&ans.expr.et, factor);
    break;
  }
  return ans;
}
...
symb_data symb_mult(symb_data S1, int del1, symb_data S2, int del2) {
  ...
  if (S1.type == errortp || S2.type == errortp || S1.type == indextp ||
      S2.type == indextp)
    return ans;
  ...
  if (S1.type == S2.type) {
    ...
    case numbertp:
    case polytp:
      ans.expr.p = multtwopoly(S1.expr.p, S2.expr.p);
      break;
    case tenstp:
      ans.expr.t = multtwotens(S1.expr.t, S2.expr.t);
      break;
    case spintp:
      ans.expr.s = mult2Spin(S1.expr.s, S2.expr.s, 0);
      break;
    case etenstp:
      ans.expr.et = mult2Etens(S1.expr.et, S2.expr.et);
      break;
    default:
      return ans;
    }
  ...
  if (S2.type == vectortp || S2.type == spintp)
    return ans;
  ...
  case vectortp:
  case tenstp:
    multtenspoly(&ans.expr.t, S2.expr.p);
    break;
  case spintp:
    switch (S2.type) {
    case polytp:
      multSpinPoly(&ans.expr.s, S2.expr.p);
      break;
    case tenstp:
      multSpinTens(&ans.expr.s, S2.expr.t);
      break;
    }
    break;
  case etenstp:
    switch (S2.type) {
    case polytp:
      multEtensPoly(&ans.expr.et, S2.expr.p);
      break;
    case tenstp:
      multEtensTens(&ans.expr.et, S2.expr.t);
      break;
    }
    break;
  }
...
symb_data symb_typeUp(symb_data S, int del, int type) {
  ...
  case polytp:
    ...
    return ans;
  case tenstp:
    ans.expr.t = newtensor1();
    multtenspoly(&(ans.expr.t), S.expr.p);
    break;
  case spintp:
    ans.expr.s = spin1();
    if (S.type == tenstp)
      multSpinTens(&ans.expr.s, S.expr.t);
    else
      multSpinPoly(&ans.expr.s, S.expr.p);
    break;
  case etenstp:
    ans.expr.et = etens1();
    if (S.type == tenstp)
      multEtensTens(&ans.expr.et, S.expr.t);
    else
      multEtensPoly(&ans.expr.et, S.expr.p);
    break;
  }
...
static set symb_index(symb_data S) {
  ...
  switch (S.type) {
  case tenstp:
    for (i = 0, t = S.expr.t; i < maxIndex; i++)
      if (t->tens[i])
        set_add1(&index, i + 1);
...
symb_data symb_sum(symb_data S1, int del1, symb_data S2, int del2) {
  ...
  case numbertp:
  case polytp:
    sewpoly(&s1.expr.p, &s2.expr.p);
    break;
  case vectortp:
  case tenstp:
    sewtens(&s1.expr.t, &s2.expr.t);
    break;
  case spintp:
    addSpin(&s1.expr.s, s2.expr.s);
    break;
  case etenstp:
    addEtens(&s1.expr.et, s2.expr.et);
    break;
  }
