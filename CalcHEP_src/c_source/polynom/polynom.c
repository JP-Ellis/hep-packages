...
void makeNewGarbage(void) {
  ...
  for (q = garbage; brest; brest--, q = q->next)
    q->next = (poly)getmem_(symb_size);
  q->next = NULL;
...
void sewpoly(poly *p1, poly *p2) {
  ...
  *p2 = NULL;
  ...
  for (;;) {
    ...
    unsigned long *pw1 = m1->power, *pw2 = m2->power,
                  *pw_end = pw1 + monomLength;
...
void symb_start(int nvar, varinfo *Vars, int nSpin, int nIndex, int nMom) {
  ...
  for (i = 0; i < nMom * (nMom + 1) / 2; i++)
    contracts[i] = NULL;
#ifdef STRACE
  printf("vars position \n");
  for (i = 0; i < nvar; i++) {
    printf(" name= %s pos= %d maxdeg=  %lu zerodeg= %lu\n", Vars[i].name,
           Vars[i].wordpos, Vars[i].maxdeg, Vars[i].zerodeg);
  }
