...
void writetens(tensor p) {
  ...
  for (; p; p = p->next) {
    ...
    isMono = 0;
    if (p->re && p->re->next == NULL && p->im == NULL)
      isMono = 1;
    if (p->im && p->im->next == NULL && p->re == NULL)
      isMono = 1;
...
void writeEtens(Etens p) {
  ...
  for (; p; p = p->next) {
    if ((p->tcoef->next || ((p->tcoef->re && p->tcoef->im))) &&
        p->eps[0] != X_MARK) {
      if (plus)
        wrt_0("+(");
      else
...
void symb_print(char *txt, symb_data m) {
  ...
  case tenstp:
    ...
    break;
  case spintp:
    writespinor(m.expr.s);
    break;
  case etenstp:
    writeEtens(m.expr.et);
    break;
  default:
    if (m.type == vectortp) {
      p = m.expr.t;
  ...
  for (; p;) {
    ...
    for (i = 0; i < vardef->nvar; i++)
      if (strchr(vardef->vars[i].name, '.')) {
        deg =
            (q->power[vardef->vars[i].wordpos - 1] / vardef->vars[i].zerodeg) %
            vardef->vars[i].maxdeg;
        if (deg > 0) {
          q->power[vardef->vars[i].wordpos - 1] -=
              vardef->vars[i].zerodeg * deg;
          if (strlen(text))
            strcat(text, "*");
          strcat(text, vardef->vars[i].name);
          if (deg > 1)
            sprintf(text + strlen(text), "^%d", deg);
        }
      }
  ...
  for (; p; p = p->next) {
    ...
    for (i = 1; i <= maxIndex; i++) {
      ...
      if (s < i && s) {
        if (first)
          ...
          strcat(text, "*");
        if (s > 0)
          sprintf(text + strlen(text), "m%d.m%d", i, s);
        else
          sprintf(text + strlen(text), "p%d.m%d", -s, i);
      }
    }
    ...
    if (p->re) {
      ...
      for (i = 0; i < Nre; i++) {
        ...
        if (strlen(txtRe[i]) && strlen(text)) {
          sprintf(buff, "%s*%s", text, txtRe[i]);
          strcpy(txtRe[i], buff);
        } else if (strlen(text))
          strcpy(txtRe[i], text);
      }
    ...
    if (p->im) {
      ...
      for (i = 0; i < Nim; i++) {
        ...
        if (strlen(txtIm[i]) && strlen(text)) {
          sprintf(buff, "%s*i*%s", text, txtIm[i]);
          strcpy(txtIm[i], buff);
        } else if (strlen(txtIm[i]) == 0 && strlen(text) == 0)
          strcpy(txtIm[i], "i");
        else if (strlen(text))
          sprintf(txtIm[i], "%s*i", text);
        else if (strlen(txtIm[i])) {
          sprintf(buff, "i*%s", txtIm[i]);
          strcpy(txtIm[i], buff);
        }
      }
    }
  ...
  for (; p; p = p->next) {
    ...
    if (p->g5 || p->l) {
      ...
      for (i = 0; i < p->l; i++) {
        ...
        if (c > 0)
          sprintf(text + l, "G(m%d)", c);
        else
          sprintf(text + l, "G(p%d)", -c);
      }
    }
    char **txtT = NULL;
    poly *expT = NULL;
  ...
  for (; p; p = p->next) {
    ...
    if (p->eps[0] != X_MARK) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        l = strlen(text);
        if (c > 0)
          sprintf(text + l, "m%d,", c);
        else
          sprintf(text + l, "p%d,", -c);
      }
      text[strlen(text) - 1] = ')';
    }
...
int symb_expand(symb_data m, char ***txt, poly **exp) {
  ...
  case numbertp:
  case polytp:
    Nterm = expand_poly(m.expr.p, txt, exp);
    break;
  case tenstp:
    Nterm = expand_tensor(m.expr.t, txt, exp);
    break;
  case spintp:
    Nterm = expand_spinor(m.expr.s, txt, exp);
    break;
  case etenstp:
    Nterm = expand_Etens(m.expr.et, txt, exp);
    break;
  default:
    Nterm = 0;
    *txt = NULL;
