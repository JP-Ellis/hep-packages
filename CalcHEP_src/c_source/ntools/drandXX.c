...
static double float48 =
    1. / (((double)0x10000) * ((double)0x10000) * ((double)0x10000));
...
double drandXX(void) {
  ...
  Xlong = (top + (bot >> 16) + Along * Xshort + Ashort * Xlong +
           ((Along16 * Xlong))) &
          0xFFFFFFFF;
  Xshort = bot & FIRST16;
...
{  unsigned  long Xshort=rnd->Xshort, Xlong=rnd->Xlong, bot=Xshort*Ashort, top=
Clong+ (bot>>16); bot=(bot&FIRST16)+Cshort; Xlong=(top +
(bot>>16)+Along*Xshort+Ashort*Xlong+((Along16*Xlong)))&0xFFFFFFFF;
   Xshort=bot&FIRST16;
...
printf("sizeoflong=%d\n",sizeof(long));
  for(sum=0,i=0;i<N;i++)   if( fabs(erand48(seed16v )-drandXX())> 1.E-10)
printf("N=%d\n",i); printf("check OK\n");
}
*/
