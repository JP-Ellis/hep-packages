...
static void Local2Global(vegasGrid *vegPtr, int *Kg, int *Ng, double *XLOC,
                         double *XGLOB, double *JACOB, int *GRID_LOC) {
  int j, n;
  double xlj, xn, xn_;
...
vegasGrid *vegas_init(int dim, double (*fun)(double *, double), int nd) {
  ...
  if (vegPtr) {
    ...
    for (j = 0; j < dim; j++)
      for (i = 0; i <= nd; i++)
        vegPtr->x_grid[j][i] = i / (double)nd;
    vegPtr->evnCubes = 0;
    vegPtr->intCubes = 0;
    for (i = 0; i < dim; i++) {
      vegPtr->NgI[i] = vegPtr->NgE[i] = 0;
    }
    vegPtr->fMax = NULL;
    vegPtr->key = keyInit;
  }
...
static void *vegas_cycle(void *par_) {
  ...
  for (;;) {
    if (nCore)
      pthread_mutex_lock(&(par->key));
    if (par->end) {
      if (nCore)
        pthread_mutex_unlock(&(par->key));
      return result;
    }
    for (i = 0; i < dim; i++)
      Kg_[i] = par->Kg[i];
    ...
    for (i = 0; i < npg; i++) {
      ...
      if (fMax) {
        for (j = 0; j < dim; j++) {
          double xj = (Kg_[j] + xlocal[j]) / Ng[j];
          kg[j] = xj * ng[j];
        }
        for (j = 1, l = kg[0]; j < dim; j++)
          l = l * ng[j] + kg[j];
        f = fabs(f);
        if (nCore)
          pthread_mutex_lock(&(par->vegPtr->key));
        if (fMax[l] < f)
          fMax[l] = f;
        if (nCore)
          pthread_mutex_unlock(&(par->vegPtr->key));
      }
    }
    f2b = sqrt(f2b / npg);
    fb /= npg;
...
long vegas_int(vegasGrid *vegPtr, long ncall0, double alph, int nCore,
  ...
  if (alph == 0) {
    if (vegPtr->evnCubes && !vegPtr->fMax) {
      ...
      for (cC = 0; cC < vegPtr->evnCubes; cC++)
        vegPtr->fMax[cC] = 0;
    }
  } else if (vegPtr->fMax) {
    free(vegPtr->fMax);
  ...
  /*    - MAIN INTEGRATION LOOP */
  {
    ...
    if (nCore) {
      ...
      for (k = 0; k < nCore; k++)
        pthread_create(threads + k, NULL, vegas_cycle, &par);
    ...
    for (j = 0; j < dim; j++) /* REFINE GRID */
    {
      ...
      if (rc) {
        ...
        for (i = 1; i < Ndmx;) {
          ...
          for (; dr >= rc; i++) {
            dr -= rc;
            x[i] = xn - (xn - xo) * dr / r[k - 1];
          }
        }
        for (i = 1; i < Ndmx; i++)
          vegPtr->x_grid[j][i] = x[i];
...
static void *event_cycle(void *par_) {
  ...
  for (;;) {
    ...
    if (weight < 1 || par->recalc)
      weight = 1;
    weight *= sgn;
    stat->sumW += weight;
    stat->sumW2 += weight * weight;
    (par->out)(x, sgn * weight);
    if (par->cEvent >= par->nEvents) {
      par->end = 1;
      if (nCore)
        pthread_mutex_unlock(&par->key);
      continue;
    ...
    if (newMax > oldMax * gmax && par->recalc) {
      ...
      for (dN = 1; dN < Ntry * newMax / sum / gmax; dN++) {
        ...
        if (f > oldMax * gmax) {
          if (nRec >= nRecT)
            events = realloc(events, sizeof(*events) * (++nRecT));
          events[nRec].f = f;
          events[nRec].sgn = sgn;
          events[nRec].rnd = rc;
          for (i = 0; i < dim; i++)
      ...
      if (nCore)
        pthread_mutex_lock(&par->key);
      for (n = 0, k = 0; k < nRec; k++)
        if (events[k].f > oldMax && events[k].f > newMax * events[k].rnd &&
            par->cEvent < par->nEvents) {
          par->cEvent++;
    ...
    if (newMax > oldMax * gmax) {
      ...
      if (nCore)
        pthread_mutex_unlock(&(par->vegPtr->key));
      stat->nexc++;
      if (stat->rmax < newMax / oldMax)
        stat->rmax = newMax / oldMax;
    }
  }
}
...
long vegas_events(vegasGrid *vegPtr, long nEvents, double gmax,
                  void (*out)(double *, double), int recalc, int nCore,
                  event_stat *stat) {
  ...
  if (!smax)
    return -1;
  for (cCube = 1; cCube < nCubes; cCube++)
    smax[cCube] += smax[cCube - 1];
  par.nCore = nCore;
  par.vegPtr = vegPtr;
  par.nEvents = nEvents;
  par.recalc = recalc;
  ...
  if (nCore) {
    ...
    pthread_t control;
    for (i = 0; i < nCore; i++)
      pthread_create(threads + i, NULL, event_cycle, &par);
    event_control(&par);
    for (i = 0; i < nCore; i++) {
      ...
      if (stat) {
        ...
        if (stat->lmax < stat1->lmax)
          stat->lmax = stat1->lmax;
        if (stat->nexc < stat1->nexc)
          stat->nexc = stat1->nexc;
        if (stat->rmax < stat1->rmax)
  ...
  if (stat)
    stat->eff = par.cEvent / (double)(par.Ntry);
  return par.cEvent;
}
