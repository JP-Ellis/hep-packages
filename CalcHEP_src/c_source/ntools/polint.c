...
double polint3(double x, int dim, const double *xa, const double *ya) {
  if (dim < 4)
    return polintN(x, dim, xa, ya);
  if ((xa[0] < xa[dim - 1] && x <= xa[1]) ||
      (xa[0] > xa[dim - 1] && x >= xa[1]))
    return polintN(x, 3, xa, ya);

  if ((xa[0] < xa[dim - 1] && x >= xa[dim - 2]) ||
      (xa[0] > xa[dim - 1] && x <= xa[dim - 2]))
    return polintN(x, 3, xa + dim - 3, ya + dim - 3);

  int shift = leftXN(4, dim, xa, x);
  double x1 = xa[shift], x2 = xa[shift + 1], x3 = xa[shift + 2],
         x4 = xa[shift + 3];
  double y1 = ya[shift], y2 = ya[shift + 1], y3 = ya[shift + 2],
         y4 = ya[shift + 3];
  double h = x3 - x2;
  double res = y3 * (x - x2) / h - y2 * (x - x3) / h;
  double d = (y3 - y2) / h,
         d2 = y1 * (x2 - x3) / (x1 - x2) / (x1 - x3) +
              y2 * (2 * x2 - x3 - x1) / (x2 - x1) / (x2 - x3) +
              y3 * (x2 - x1) / (x3 - x1) / (x3 - x2),
         d3 = y2 * (x3 - x4) / (x2 - x3) / (x2 - x4) +
              y3 * (2 * x3 - x4 - x2) / (x3 - x2) / (x3 - x4) +
              y4 * (x3 - x2) / (x4 - x2) / (x4 - x3);

  return res + (x - x2) * (x - x3) *
                   ((d2 - d) * (x - x3) + (d3 - d) * (x - x2)) / h / h;
}
...
double polint1Exp(double x, int n, double *xa, double *ya) {
  ...
  if (y1 > 0 && y2 > 0)
    return pow(y1, 1 - alpha) * pow(y2, alpha);
