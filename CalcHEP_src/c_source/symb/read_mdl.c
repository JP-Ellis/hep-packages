...
static void errorMessage(char *fieldName, char *format, ...) {
  ...
  if (strcmp(dump, "*") == 0)
    sprintf(errorText,
            "Error in table '%s' line %d field '%s'\nposition %u: %s", tabName,
            nLine, fieldName, rderrpos, errmesstxt(rderrcode));
  else
    sprintf(errorText,
            "Error in table '%s' line %d field '%s'  position %u:\n  %s",
    ...
    for (k = 0; k < 1 + nv1 / (2 * b); k++) {
      ...
      for (i = 2 * k * b; i1 < e1 || i2 < e2; i++) {
        if (i2 == e2)
          ...
          vSorted[i] = vSorted_[i2++];
        else if (strcmp(modelvars[vSorted_[i1]].varname,
                        modelvars[vSorted_[i2]].varname) <= 0)
          vSorted[i] = vSorted_[i1++];
        else
          vSorted[i] = vSorted_[i2++];
      }
...
static void *act_33(char *ch, int n, void **args) {
  ...
  char math[] =
      " + - * / ^ if"
      " sqrt pow  exp log log10 sin cos tan asin acos atan sinh cosh tanh "
...
static int readvars(int check) {
  ...
  for (nLine = 1, ln = func_tab.strings; ln; nLine++, ln = ln->next) {
    ...
    trim(name);
    if (!name[0]) {
      errorMessage("Name", "Empty line in the table", "*");
      goto errExi1;
    }
    if (name[0] == '*') {
      name[0] = ' ';
  ...
  if (check) // duplicate name
    for (i = 1; i <= nmodelvar; i++)
      if (strcmp(modelvars[vSorted[i]].varname,
                 modelvars[vSorted[i - 1]].varname) == 0) {
        if (modelvars[vSorted[i]].func)
          tabName = func_tab.headln;
        else
          tabName = vars_tab.headln;
        nLine = modelvars[vSorted[i]].line;
        rderrpos = 1;
        errorMessage("Name", "duplicate name '%s'",
                     modelvars[vSorted[i]].varname);
        goto errExi1;
      }
  if (check)
    rd = rd_33;
  ...
  for (i = 1; i <= nmodelvar; i++)
    if (modelvars[i].func) {
      ...
      depQ = 0;
      aWidth = 0;
      if (readExpression(modelvars[i].func, rd, act_33, NULL) == &String)
        rderrcode = typemismatch;
      if (rderrcode && (rderrcode != unknownfunction)) {
        if (rderrcode == unknownidentifier) {
          ...
          if (!ch)
            errorMessage("Expression", "%s", "Unexpected end of line");
          else
            errorMessage("Expression", "Unexpected characted |%c|", ch);
        } else
          errorMessage("Expression", "*");
        goto errExi1;
      }

      if (ExtFunc && i > nCommonVars)
        nCommonVars = i;
      if (depQ && depQ1 > i)
        depQ1 = i;
      if (check && aWidth) {
  ...
  for (i = nmodelvar; i > nCommonVars; i--)
    if (modelvars[i].pub) {
...
static void clearLatexNames(void) {
  ...
  if (!prtclbase)
    return;
  for (j = 0; j < nparticles; j++) {
    if (!strchr("fcCtT", prtclbase[j].hlp))
      free(prtclbase[j].latex);
  }
}

static int isPrtclName(char *p) {
  return strlen(p) < P_NAME_SIZE - 2 && !strchr(p, ' ') && !strchr(p, ')') &&
         !strchr(p, ')') && !strchr(p, '%');
}
...
static int q3SM(int N) {
  ...
  case 4:
  case 6:
    return 2 * sgn; /*u quarks*/
  case 11:
  case 13:
  case 15:
    return -3 * sgn; /*electron*/
  case 12:
  case 14:
  case 16:
    return 0; /*neutrino*/
...
static int readparticles(int check, int ugForce) {
  ...
  while (ln != NULL) {
    ...
    sscanf(ss,
           "%[^|]%*c%[^|]%*c%[^|]%*c%[^|]%*c%[^|]%*c%[^|]%*c%[^|]%*c%[^|]%*c%[^"
           "|]%*c%[^|]%*c%[^|]",
           fullname, p1, p2, numtxt, s, massname, imassname, c, chlp, latex,
           latex_);
    trim(p1);
    trim(p2);
    trim(latex);
    ...
    if (check) {
      ...
      if ((itmp != 0) && (itmp != 1) && (itmp != 2) && (itmp != 3) &&
          (itmp != 4)) {
    ...
    if (1 != sscanf(numtxt, "%ld", &prtclbase[nparticles - 1].N))
      prtclbase[nparticles - 1].N = 0;

    if (strcmp(p1, p2) == 0)
      prtclbase[nparticles - 1].q3 = 0;
    else
      prtclbase[nparticles - 1].q3 = q3SM(prtclbase[nparticles - 1].N);

    trim(massname);
    if (strcmp(massname, "0") == 0) {
      if (prtclbase[nparticles - 1].spin == 3 ||
          prtclbase[nparticles - 1].spin == 4)
    ...
    if (imassname[0] == '!') {
      ...
      if (check) {
        ...
        if (findVarFast(imassname) > 0) {
          errorMessage("width",
                       "%s used variable, not accepted for automatic width",
                       imassname);
          return 0;
        }
        for (i = nSortedVar + 1; i <= nmodelvar; i++)
          if (strcmp(modelvars[i].varname, imassname) == 0) {
            errorMessage("width", "this identifier  '%s' already was used",
                         imassname);
            return 0;
          }
      }
    ...
    if (check) {
      ...
      if (((itmp != 1) && (itmp != 3) && (itmp != 8) && (itmp != 6)) ||
          ((itmp == 3) && (strcmp(p1, p2) == 0)) ||
          ((itmp == 6) && (strcmp(p1, p2) == 0))) {
    ...
    if (strchr(chlp, '*')) {
      ...
      if (ch) {
        ch[0] = ' ';
        prtclbase[nparticles - 1].nHerm = 1;
      }
    }
    ...
    if (check) {
      ...
      case '*':
        if (prtclbase[nparticles - 1].massidnt[0] == '0')
          ...
          errorMessage("aux", "for aux='*' zero width is  expected");
          return 0;
        }
        break;
      case 'G':
      ...
      if (prtclbase[nparticles - 1].N == 0 &&
          prtclbase[nparticles - 1].hlp != '*') {
        errorMessage("number", "Zero (or unreadable) PDG code.");
        return 0;
      }
      if (prtclbase[nparticles - 1].hlp != '*')
        for (i = 0; i < nparticles - 1; i++)
          if (abs(prtclbase[i].N) == abs(prtclbase[nparticles - 1].N)) {
            errorMessage("number", "duplicate PDG code.");
            return 0;
          }
    }
    ...
    if (strcmp(p1, p2) == 0)
      ...
      if (prtclbase[np1 - 1].cdim == 6)
        prtclbase[nparticles - 1].cdim = -6;
      if (prtclbase[np1 - 1].q3 == unknownQ3)
        prtclbase[nparticles - 1].q3 = unknownQ3;
      else
        prtclbase[nparticles - 1].q3 = -prtclbase[np1 - 1].q3;
      np2 = ghostaddition();
  ...
  if (ugForce)
    for (i = 1; i <= nparticles; i++)
      if (gaugep(i) && (!zeromass(i)))
        prtclbase[i - 1].hlp = ' ';

  for (i = 1; i <= nmodelvar; i++)
    if (modelvars[i].pwidth)
      modelvars[i].pwidth = ghostmother(modelvars[i].pwidth);

  int pnum = 0;
  for (i = 0; i < nparticles; i++)
    if (strchr("*fcCtT", prtclbase[i].hlp))
      prtclbase[i].pnum = 0;
    else {
      ...
      if (i + 1 <= anti) {
        ...
        if (anti != i + 1)
          prtclbase[anti - 1].pnum = prtclbase[i].pnum;
      }
    }
  for (i = 0; i < nparticles; i++)
    if (strchr("fcCtT", prtclbase[i].hlp))
...
static int testLgrgn(algvertptr lgrgn) {
  ...
  for (n = 0; n < vardef->nvar; n++) {
    ...
    err = findVarFast(vardef->vars[n].name);
    if (err < 0) {
      errorMessage("Lorentz part", "unknown variable '%s'",
                   vardef->vars[n].name);
      return 0;
    }
    if (strcmp(vardef->vars[n].name, strongconst) == 0) {
      errorMessage("Lorentz part", "illegal variable '%s'",
                   vardef->vars[n].name);
      return 0;
    }
  }
...
static int readlagrangian(int check, int ugForce) {
  ...
  for (ln = lgrng_tab.strings, nLine = 1; ln; ln = ln->next, nLine++) {
    ss = ln->line;
    sscanf(ss, "%[^|]%*c%[^|]%*c%[^|]%*c%[^|]", pPtr[0], pPtr[1], pPtr[2],
           pPtr[3]);
    for (i = 0; i < 4; i++)
      trim(pPtr[i]);
    if (pPtr[0][0] == '%')
    ...
    lgrgn1->next = lgrgn;
    lgrgn = lgrgn1;
    for (lgrgn->comcoef = ln->line, i = 0; i < 4; lgrgn->comcoef++)
      if (lgrgn->comcoef[0] == '|')
        i++;
    lgrgn->description = strchr(lgrgn->comcoef, '|') + 1;
    for (i = 0; i < 4; i++)
      lgrgn->fields[i] = f_copy[i];
    ...
    if (check) {
      ...
      spinorNumb = 0;
      for (mm = 0; ((mm < 4) && (lgrgn->fields[mm] != 0)); mm++) {
        if (prtclbase1[lgrgn->fields[mm]].spin & 1)
          spinorNumb++;
      }
    ...
    do {
      ...
      while (lgrgn2 != NULL) {
        if ((lgrgn1->fields[0] == lgrgn2->fields[0]) &&
          ...
          for (i = 0; i < 4; i++)
            if (lgrgn1->fields[i]) {
              strcat(sss, prtclbase1[lgrgn1->fields[i]].name);
              strcat(sss, " ");
            }
...
static int find3charge(void) {
  ...
  int photon = -1;
  for (i = 0; i < nparticles; i++)
    if (prtclbase[i].nHerm)
      prtclbase[i].q3 = 0;

  for (cont = 1; cont;) {
    cont = 0;
    for (i = 0; i < nparticles; i++)
      if (ghostmother(i + 1) == i + 1 && prtclbase[i].q3 == unknownQ3) {
        decaylink dec = prtclbase[i].top;
        for (; dec; dec = dec->next) {
          int j, ch, br;
          for (j = 0, ch = 0, br = 0; j < 3 && dec->part[j]; j++)
            if (prtclbase[dec->part[j] - 1].q3 == unknownQ3) {
              br = 1;
              break;
            } else
              ch += prtclbase[dec->part[j] - 1].q3;
          if (!br) {
            prtclbase[i].q3 = ch;
            cont = 1;
            if (i + 1 != prtclbase[i].anti)
              prtclbase[prtclbase[i].anti - 1].q3 = -prtclbase[i].q3;
            break;
          }
        }
      }
    ...
    if (cont == 0 && photon != -2) {
      if (photon == -1)
        for (photon = 0; photon < nparticles; photon++) {
          if (ghostmother(photon + 1) == photon + 1 &&
              prtclbase[photon].N == 22)
            break;
        }
      if (photon == nparticles)
        photon = -2;
      if (photon >= 0)
        for (i = 0; i < nparticles; i++)
          if (ghostmother(i + 1) == i + 1 && prtclbase[i].q3 == unknownQ3) {
            ...
            for (; dec; dec = dec->next) {
              if (dec->part[2] == 0 &&
                  ((dec->part[0] - 1 == photon && dec->part[1] - 1 == i) ||
                   (dec->part[1] - 1 == photon && dec->part[0] - 1 == i)))
                break;
            }
            if (dec == NULL) {
              prtclbase[i].q3 = 0;
  ...
  for (i = 0; i < nparticles; i++)
    if (prtclbase[i].hlp != '*' && prtclbase[i].q3 == unknownQ3) {
      ...
      if (im == i + 1) {
        sprintf(errorText,
                ...
                "For example: 2 for u-quark, -1 for d-quark.",
                prtclbase[i].name);
        if (blind)
          printf("ERROR:%s\n", errorText);
        else
        ...
        prtclbase[i].q3 = prtclbase[im - 1].q3;
    }
  return 0;
}
...
static int checkQ3(void) {
  ...
  for (; lgrgn1; lgrgn1 = lgrgn1->next, nLine++) {
    ...
    if (chTot) {
      ...
      for (i = 0; i < 4 && lgrgn1->fields[i]; i++)
        sprintf(txt + strlen(txt), "%s(%d) ",
                prtclbase[lgrgn1->fields[i] - 1].name,
                prtclbase[lgrgn1->fields[i] - 1].q3);
      nLine = 0;
      errorMessage("", txt);
      return 0;
    }
...
int loadModel(int check, int ugForce) {
  errorText[0] = 0;
  if (ldModelStatus & 1 == 0) {
    sprintf(errorText, "model files are not read\n");
    return 0;
  }

  if ((!check) && (ldModelStatus & 2) == 2 &&
      (ldModelStatus & 4) == 4 * ugForce)
    return 1;
  ldModelStatus = 1;
  ...
  if (!readparticles(check, ugForce)) {
    if (blind)
      sortie(125);
    else
      return 0;
  ...
  if (!readlagrangian(check, ugForce)) {
    if (blind)
      sortie(125);
    else
      return 0;
...
int makeVandP(int rd, char *path, int L, int mode, char *CalcHEP) {
  ...
  for (nLn = 0, i = 0; i < nparticles; i++)
    if (!strchr("*fcCtT", prtclbase[i].hlp) && i + 1 <= prtclbase[i].anti)
      nLn++;
  ...
  for (nLn = 0, i = 0; i < nparticles; i++)
    if (!strchr("*fcCtT", prtclbase[i].hlp)) {
      ...
      if (i + 1 == anti)
        ...
        fprintf(f, "\"%s\", ", prtclbase[anti - 1].name);
      fprintf(f, "%d, \"%s\",\"%s\",%d,%d,%d}\n", prtclbase[i].N,
              prtclbase[i].massidnt, prtclbase[i].imassidnt, prtclbase[i].spin,
              prtclbase[i].cdim, prtclbase[i].q3);
    }
  fprintf(f, "};\n");
  ...
  for (i = 1; i <= nmodelvar; i++)
    if (modelvars[i].func) {
      if (modelvars[i].pub || i <= nCommonVars)
        nFunc++;
    } else {
      if (strcmp(modelvars[i].varname, "i"))
        nVar++;
    }
  ...
  for (i = 0; i < nv0; i++) {
    ...
    else if (strcmp(modelvars[i].varname, "pi") == 0)
      sprintf(vararr[i].alias, "M_PI");
    else if (strcmp(modelvars[i].varname, "Sqrt2") == 0)
      sprintf(vararr[i].alias, "M_SQRT2");
  }
  ...
  for (first = 1, i10 = 0, i = nv0; i < nmodelvar; i++)
    if ((i <= nCommonVars || modelvars[i].pub == 1) && vararr[i].used) {
      if (first)
        first = 0;
      else
        fprintf(f, ",");
  ...
  for (first = 1, nv = 0, i10 = 0, i = 0; i <= nmodelvar; i++)
    if ((i <= nCommonVars || modelvars[i].pub == 1) && vararr[i].used)
  ...
  if (depQ1 <= nCommonVars) {
    fprintf(f,
            "     {if(iQ>=0 && VV[iQ]!=V[iQ]) goto FirstQ; else return 0;} \n");
  } else
    fprintf(f, "    return 0;\n");
  ...
  for (i = 1; i <= nmodelvar; i++)
    if ((i <= nCommonVars || modelvars[i].pub == 1) && vararr[i].used) {
      if (vararr[i].used && modelvars[i].func) {
        ...
        {
          char *ss =
              (char *)readExpression(modelvars[i].func, rd_c, act_c, free);
          fprintf(f, "   %s=%s;\n", vararr[i].alias, ss + 3);
          free(ss);
        }
        if (checkNaN)
  ...
  fprintf(f, "      for(i=0;i<nModelVars;i++) if(strcmp(varNames[i],\"Q\")==0) "
             "iQ=i;\n");
  fprintf(f, "   }\n");
  ...
  if (mode & 4) {
    ...
    for (i = nCommonVars + 1; i <= nmodelvar; i++)
      if (modelvars[i].func && !modelvars[i].pub) {
        char buff[STRSIZ], *c;
