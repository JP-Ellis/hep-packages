...
static void savepoly(poly p) {
  ...
  b = (char *)&(zero->num);
  if (vardef->nvar)
    e = (char *)&(zero->power[vardef->vars[vardef->nvar - 1].wordpos]);
  else
    e = (char *)&(zero->power[0]);
  width = e - b;

  while (p) {
    if (fwrite(&(p->num), width, 1, archiv) != 1)
      wAbort();
    p = p->next;
  }

  if (fwrite(&(zero->num), width, 1, archiv) != 1)
    wAbort();
}

void saveanaliticresult(poly rnum, poly factn, poly factd, vcsect vcs,
                        int ndiagr_abs, int nFile) {
  catrec cr;
  int i;
  int m;
  ...
  for (i = 0; i < denrno; i++) {
    ...
    m = 0;
    do
      FWRITE1(denom[i].momStr[m], archiv);
