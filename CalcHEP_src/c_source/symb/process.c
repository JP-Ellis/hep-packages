...
static void prtcllist(int key) {
  ...
  if (key == 0) {
    ...
    if (nFirst < 1)
      nFirst = 1;
    if (nTot - nFirst + 3 < 3 * tabMax)
      nFirst = 1 + 3 * ((nTot + 2) / 3) - 3 * tabMax;
...
static int input(int y0, char *hlp, char *directive, char *text, int cur,
                 int lim) {
  if (errTxt) {
    goto_xy(1, y0 + 1);
    scrcolor(FGmain, BGmain);
...
static int enter_h(int *y, char *name, int num, int scat) {
  ...
  if (j) {
    ...
    if (name[strlen(name) - 1] == '%') {
      if (!scat || nout || strcmp(prtclbase1[j].massidnt, "0") ||
        ...
        hadrons[num].polarized[0] = 1;
    } else
      hadrons[num].polarized[0] = 0;
    return 0;
  }
  ...
  for (i = 0; i < num; i++)
    if (!strcmp(hadrons[i].name, name)) {
      ...
      for (j = 0; j < hadrons[num].len; j++) {
        ...
        if (nout)
          hadrons[num].polarized[j] = 0;
        else
          hadrons[num].polarized[j] = hadrons[i].polarized[j];
      }
      return 0;
    }
  ...
  do {
    ...
    for (m = 0, hadrons[num].len = 0; items[m]; m++) {
      char name[100];
      if (hadrons[num].len >= 100) {
        errTxt = "too many partons";
        break;
      }
      sscanf(items[m], "%[^ ,]", name);
      locateinbase(name, &j);
      if (j == 0 || pseudop(j)) {
        errTxt = "This particle is absent in the model";
        break;
      }
      ...
      if (name[strlen(name) - 1] == '%') {
        if (!scat || nout || strcmp(prtclbase1[j].massidnt, "0") ||
          ...
          hadrons[num].polarized[hadrons[num].len] = 1;
      } else
        hadrons[num].polarized[hadrons[num].len] = 0;
...
static int restrict_p(int y0, int anti, char *mess, char *hlp, char *inputstr,
                      whohow liminsp) {
  int m, j, k;
  int ntot, ntotQ, forQ;
  ...
  do {
    for (r = 1; r != KB_ENTER;) {
      r = input(y0, hlp, mess, inputstr, errpos ? 1 + errpos - inputstr : 1,
                STRSIZ - 1);
      if (r == KB_ESC)
        return 1;
    }
...
label_1:
  ...
  redres = input(y0, "s_ent_1", "Enter  process: ", processch,
                 errpos ? errpos - processch + 1 : 1, SSTRLEN - 1);
  switch (redres) {
  case KB_F1: /*  Help  */
    show_help("s_ent_1");
    goto label_1;
  ...
  if (items[0] && items[1] && items[1] < arrpos)
    scat = 1;
  else
    scat = 0;
  ...
  for (i = 0, nin = 0, nout = 0, n_x = 0, curh = 0; items[i]; i++) {
    ...
    if (items[i] > arrpos && strlen(name) == 3 && name[1] == '*' &&
      ...
      if (items[i] > arrpos)
        ...
        nin++;
      if (nin > 2) {
        errTxt = "Too many incoming particles";
        break;
      }
      if (enter_h(&y0, name, curh, scat))
        break;
  ...
  for (i = 0; LimQ[i].who; i++)
    addlim(liminsp, LimQ[i].who, LimQ[i].how, 0);
  ...
  if (n_x &&
      restrict_p(y0, 0, "Exclude X-particles ", "s_ent_5", deloutch, limout))
    goto label_1;
