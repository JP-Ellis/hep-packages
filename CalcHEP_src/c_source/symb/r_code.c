...
static void emitfactors(void) {
  ...
  writeF(" AverFact:=1/%d$       %% Normalization factor of polarization "
         "average\n",
         c);
  /* ----- Fermion factor  --------- */
  for (c = 1, i = 0; i < vcs.sizel; i++)
    for (j = 0; j < vcs.valence[i]; j++) {
      pnum = vcs.vertlist[i][j].partcl;
      if (prtclbase1[pnum].spin & 1 && (IN_PRTCL & vcs.vertlist[i][j].prop))
        c *= -1;
    }
    ...
    for (k = 1; k <= vcs.valence[j - 1]; k++) {
      ...
      if (ln->moment > 0) {
        ...
        if (!(ln->prop & (IN_PRTCL | OUT_PRTCL)) && pseudop(ln->partcl))
          ...
          if (spin == 3)
            writeF("totFactor_:=totFactor_/(3*%s^2)$\n", mass);
          else if (spin == 4)
            writeF("totFactor_:=totFactor_/(6*%s^4)$\n", mass);
        }
      }
    }
      ...
      if (ln->moment > 0 && !(ln->prop & (IN_PRTCL | OUT_PRTCL))) {
        if (!pseudop(ln->partcl)) {
          ...
          if (denrno)
            writeF("*");
          denrno++;
          /*           if(!ttypepropag(j,k))*/ strcpy(
              width, prtclbase1[ln->partcl].imassidnt);
...
static char *subst(int ind, int arg) {
  ...
  if (gammaflag && fermvrt(v)) {
    ...
    sprintf(ans + strlen(ans), "%s", pexpr(-vcs.vertlist[v - 1][l - 1].moment));

  sprintf(ans + strlen(ans), ")/%s",
          prtclbase1[vcs.vertlist[v - 1][l - 1].partcl].massidnt);
  return ans;
}
...
static void fermprgemit(void) {
  ...
  for (v = 0; v < nloop; v++)
    for (l = 0; !fermloops[v].g5 && l < fermloops[v].len; l++)
      if (strstr(parsedverts[fermloops[v].vv[l] - 1], "G(ln,A)"))
        fermloops[v].g5 = 1;

  for (v = 0, numberg5 = 0; v < nloop; v++)
    if (fermloops[v].g5)
      numberg5++;
  ...
  for (lpcount = 0; lpcount < nloop; lpcount++) {
    ...
    for (nn = 0; nn < FL->len; nn++) {
      ...
      if (FL->spin[nn] == 1)
        propStr = fermPropagTxt(v, l, 1);
      else
        propStr = spin3_2_propagator(v, l, 1);
      if (FL->ind[nn][0])
        writeF("Index m%d;\n", FL->ind[nn][0]);
    ...
    for (v1 = 1; v1 < FL->len; v1++) {
      ...
      for (i = 0; i < FL->nint[v1]; i++) {
        ...
        if (s == 2)
          set_add1(&indexs, vcs.vertlist[v][l].lorentz);
      }
      if (FL->ind[v1 - 1][1])
        set_add1(&indexs, FL->ind[v1 - 1][1]);
      if (v1 == FL->len - 1 && FL->ind[v1][1])
        set_add1(&indexs, FL->ind[v1][1]);
      ...
      for (i = 0; i < FL->nint[v1]; i++) {
        ...
        if (s == 4 && prtclbase1[vcs.vertlist[v][l].partcl].hlp != 't') {
          ...
          writeF("tProp:=(%s)/(6*%s^4)$\n", txt,
                 prtclbase1[vcs.vertlist[v][l].partcl].massidnt);
          free(txt);
    ...
    if (numberg5 == 1 && FL->g5) {
      writeF(" Vrt_%d:=Sub(A=0*A,Vrt_%d)$\n"
  ...
  for (v1 = 0; v1 < 2 * maxvert; v1++)
    fermmap[v1] = vertmap[v1];
  writeF("%%\n");
  gammaflag = 0;
}
      ...
      if (prtclbase1[np].spin == 4 && prtclbase1[np].hlp != 't') {
        ...
        if (v < vv && vertmap[v] != vertmap[vv]) {
          ...
          count++;
          {
            ...
            writeF(
                " Vrt_%d:=(m%d.m%d*m%d.m%d+m%d.m%d*m%d.m%d-m%d.m%d*m%d.m%d)/2",
            ...
            writeF("-(m%d.m%d*P%d.m%d*P%d.m%d+m%d.m%d*P%d.m%d*P%d.m%d+"
                   ...
                   m1, n1, P, m2, P, n2, m2, n2, P, m1, P, n1, m1, n2, P, m2, P,
                   n1, m2, n1, P, m1, P, n2, mass);

            writeF("+(m%d.m%d+2*P%d.m%d*P%d.m%d/"
                   "%s^2)*(m%d.m%d+2*P%d.m%d*P%d.m%d/%s^2)/6$\n",
        ...
        if (P > 0 /*&& vertmap[v]!=vertmap[vv]*/) {
          ...
          count++;
          {
            ...
            if (P == 1)
              ...
              Paux = 1;
            writeF(
                " Vrt_%d:=m%d.m%d*p%d.p%d - p%d.m%d*p%d.m%d - p%d.m%d*p%d.m%d"
                " -i*lPolar%d*eps(p%d,p%d,m%d,m%d)$",
                count, m1, m2, P, Paux, P, m1, Paux, m2, P, m2, Paux, m1, P, P,
                Paux, m1, m2);
          }
        }
      }
...
void makeghostdiagr(int dnum, char *fname) {
  ...
  FREAD1(csd, diagrq);
  {
    ...
    if (vcs.clrnum == 0)
      ...
      generateghosts(&vcs, &gstlist);
      if (gstlist == NULL) {
        writeF("Diagrams of this type are absent\n");
        csd.status = 2;
        fseek(diagrq, dnum * sizeof(csdiagram), SEEK_SET);
        FWRITE1(csd, diagrq);
      } else {
