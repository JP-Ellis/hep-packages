...
static void revol(rmptr r) {
  ...
  while (m1 != NULL) {
    if (strcmp(m1->name, "i") == 0) {
      ...
      mult_s(&(r->n), &s);
      m1 = m->next;
    } else if (strcmp(m1->name, "Sqrt2") == 0) {
      m->next = m1->next;
      for (i = 0; i < m1->deg; i++)
        (r->d).c = (r->d).c * 2;
      s.c = 1;
...
char *rmonomtxt(r_monom r) {
  ...
  if (strcmp(sden, "1") != 0)
    sprintf(snum + strlen(snum), "/(%s)", sden);
  return snum;
}
...
void diagramsrfactors(hlpcsptr gst, s_listptr *s, rmptr *totf) {
  ...
  while (gst != NULL) {
    ...
    coloringvcs(gst);
    attachvertexes();
    rcoef = (rmptr)readExpression(vertexes[0].lgrnptr->comcoef, rd_rat, act_rat,
                                  NULL);
    for (i = 2; i <= vcs.sizet; i++) {
      rrcoef = (rmptr)readExpression(vertexes[i - 1].lgrnptr->comcoef, rd_rat,
                                     act_rat, NULL);
...
int rmonomGG(rmptr r) {
  ...
  for (v = r->n.v; v; v = v->next)
    if (strcmp(v->name, "GG") == 0) {
      po += v->deg;
      if (v_)
        v_->next = v->next;
      else
