...
static void *act_rcode(char *ch, int n, void **args) {
  ...
  if (n == 4 && !strcmp(ch, "eps")) {
    int l = 15 + strlen(args[0]) + strlen(args[1]) + strlen(args[2]) +
            strlen(args[3]);
  ...
  if (strlen(s) == 2 && s[1] > '0' && s[1] <= '9') {
    ...
    case 'P':
      ...
      if (num == maxMom) // for  parseOneVertex
      {
        ...
        strcpy(p, "M|(");
        for (int i = 1; i < maxMom; i++)
          sprintf(p + strlen(p), "-p%d", i);
        strcat(p, ")");
      } else {
        if (num > 0)
          sprintf(p, "M|p%d", num);
...
char *parseVertex(int v, int forReduce) {
  ...
  forR_code = forReduce;
  pstr = (char *)readExpression(vertexes[v].lgrnptr->description, rd_rcode,
                                act_rcode, free);
  if (rderrcode) {
    if (forReduce)
      outFileClose();
  ...
  for (i = 0; i < vcs.valence[v]; i++)
    rsubst[vertexes[v].subst[i] - 1] = i + 1;
  for (i = 0; i < vcs.valence[v]; i++)
    if (prtclbase1[vcs.vertlist[v][i].partcl].cdim != 1) {
      si[nc] = rsubst[i];
      cdi[nc++] = prtclbase1[vcs.vertlist[v][i].partcl].cdim;
    }

  if (nc == 3 && cdi[0] == cdi[1] && cdi[0] == cdi[2] &&
      (abs(cdi[0]) == 3 || cdi[0] == 8)) {
    if (si[0] > si[1])
      sgn *= -1;
    if (si[1] > si[2])
...
char *parseOneVertex(char *v, int *perm, int *field) {
  ...
  int spin[2] = {0, 0};
  for (i = 0, j = 0; i < val; i++)
    if (prtclbase1[field[i]].spin & 1)
      spin[j++] = i;

  if (!spin[0])
    r_reading0 = 0;
  else
    r_reading0 = (perm[spin[0]] > perm[spin[1]]);
  forR_code = 0;
...
char *fermPropagTxt(int v, int l, int forReduce) {
  ...
  if (aux == '*')
    ...
    sprintf(proptxt, "%sG(%sp%d)+%s", sgn, Ln, pa, mass);
    ...
    sprintf(proptxt, "%sG(%sp%d)*(1+2*Helicity%d*%s)", sgn, Ln, pa, pa, G5);
  else
    sprintf(proptxt, "%sG(%sp%d)*(1-2*Helicity%d*%s)", sgn, Ln, pa, pa, G5);
  return proptxt;
}
...
char *tPropagator6mass4(int v, int l, set *indexs) {
  ...
  if (indexs)
    *indexs = set_constr(m1, m2, n1, n2, _E);

  sprintf(txt, "3*(m%d.m%d*m%d.m%d+m%d.m%d*m%d.m%d-m%d.m%d*m%d.m%d)*%s^4", m1,
          n1, m2, n2, m1, n2, m2, n1, m1, m2, n1, n2, mass);
  ...
  sprintf(txt + strlen(txt),
          ...
          m1, n1, P, m2, P, n2, m2, n2, P, m1, P, n1, m1, n2, P, m2, P, n1, m2,
          n1, P, m1, P, n2, mass);
...
char *spin3_2_propagator(int v, int l, int forReduce) {
  ...
  sprintf(txt,
          ...
          ,
          sgn, Ln, P, mass, mass, m1, m2, P, m1, P, m2, Ln, m1, mass, sgn, P,
          m1, sgn, Ln, P, mass, Ln, m2, mass, sgn, P, m2
...
static symb_data tPropagator(int v, int l) {
  ...
  sprintf(txt,
          "(m1.m3*m2.m4+m1.m4*m2.m3-m1.m2*m3.m4)*3*%s^4"
          "-(m1.m3*p1.m2*p1.m4+m2.m4*p1.m1*p1.m3+m1.m4*p1.m2*p1.m3+m2.m3*p1.m1*"
          "p1.m4 )*3*%s^2"
