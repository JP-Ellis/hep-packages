...
void findReversVert(void) {
  ...
  for (lp = 0; lp < nloop; lp++) {
    ...
    for (k = 0; k < fl_len; k++) {
      ...
      v = fermloops[lp].vv[k];
      if (k > 0) {
        vin = fermloops[lp].vv[k - 1];
        lin = fermloops[lp].ll[k - 1];
      } else {
        vin = fermloops[lp].vv[fl_len - 1];
...
static void nextFerm(int *v, int *l) {
  ...
  *v = vcs.vertlist[*v][*l].link.vno;
  for (*l = 0;; (*l)++)
    if (*l != l1 && prtclbase1[vcs.vertlist[*v][*l].partcl].spin & 1)
      break;
}
...
static void findfermcycles(void) {
  ...
  for (v = 0, nloop = 0; v < vcs.sizet; v++)
    if (fermmap[v] == 0) {
      fermloopstp *FL = &(fermloops[nloop]);
      int vr, lr;
      for (l = vcs.valence[v] - 1; l >= 0; l--)
        if (a_fermionp(vcs.vertlist[v][l].partcl))
          break;
      if (l < 0)
        continue;
      count = 0;
      ...
      for (v1 = v; !fermmap[v1]; nextFerm(&v1, &l), count++) {
        ...
        if (strchr("LR", prtclbase1[vcs.vertlist[v1][l].partcl].hlp) ||
            PLR_PRTCL & vcs.vertlist[v1][l].prop)
          FL->g5 = 1;
        fermmap[v1] = nloop + 1;
                 ...
                 if (fermmap[v-1] == 0)
                 {
                    ...
                    if (l != 0)
                    {
                       ...
                       if (nl != 0)
                       {
                          for (l = l - 1; l >= 1; l--)
                             if (nl !=
           fermmap[vcs.vertlist[v-1][l-1].nextvert.vno-1]) goto label_1;
