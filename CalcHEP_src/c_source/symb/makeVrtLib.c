...
int main(int argv, char **argc) {
  ...
  //  printf("argv=%d\n",  argv);
  if (argv < 7 || argv > 8) {
    printf("6 or 7 arguments expected:  1) path To models 2) model number  3) "
           "label for library\n");
  ...
  CalcHEP[L - 16] = 0;
  if (sscanf(argc[2], "%d", &M) != 1) {
    printf("Second argument should be a number\n");
    return 1;
  }
  N = argv - 4;
  char *field[4];
  ...
  // printf("Model=%d Nf=%d [",M,N); for(i=0;i<N;i++) printf(" %s ",field[i]);
  // printf("]\n");
