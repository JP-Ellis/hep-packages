...
int bosonp(int p) { return (prtclbase[p - 1].spin % 2 == 0); }

int vectorp(int p) { return (prtclbase[p - 1].spin == 2); }

int zeromass(int p) { return (strcmp(prtclbase[p - 1].massidnt, "0") == 0); }

int photonp(int p) { return ((vectorp(p) && zeromass(p)) || pseudop(p)); }

int ghostp(int p) {
  return (prtclbase[p - 1].hlp == 'c' || prtclbase[p - 1].hlp == 'C' ||
          prtclbase[p - 1].hlp == 'f');
}

int gaugep(int j) { return (j != 0 && prtclbase[j - 1].hlp == 'G'); }
