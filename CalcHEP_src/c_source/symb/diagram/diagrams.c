...
void mkverts(decayDiagram diag,
             vampl *vlist) /*  transformation of diagr representation */
{
  int i, j, k, particle;
  ...
  for (i = 1, currVert = 0, newVert = 1, extNum = 1, restIn = nin - 1;
       currVert >= 0; i++) {
    particle = diag[i];
    if (particle) {
      if (particle < 0) {
        ...
        if (restIn) {
          restIn--;
          w->prop = IN_PRTCL;
          if (polarized(2, prtclbase1[w->partcl].anti))
            w->prop += PLR_PRTCL;
        } else
          w->prop = OUT_PRTCL;
        w->link.vno = nullvert;
...
void printDiagram(vampl *vlist) {
  ...
  for (i = 0; i < vlist->size; i++) {
    for (j = 0; j < MAXVALENCE && vlist->vertlist[i][j].partcl; j++) {
      ...
      printf("vertex=%d, line=%d, prtcl=%s  link(%d,%d) moment=%d,lorentz=%d "
             "prop=%d \n",
