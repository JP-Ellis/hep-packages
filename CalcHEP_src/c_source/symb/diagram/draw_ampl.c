...
static void super_outtextxy(int x, int y, char *name) {
  if (name[0] == '~') {
    ...
    if (strlen(name + 1) == 2)
      tg_outtextxy(x, y - cHeight / 2, "~~");
    else
      tg_outtextxy(x, y - cHeight / 2, "~");
  } else
    tg_outtextxy(x, y, name);
}

static void drawPropagator(int x1, int y1, int x2, int y2, int particle,
                           int virtual) {
  ...
  if (virtual) {
    ...
    super_outtextxy((x1 + x2) / 2 + xShift, (y1 + y2) / 2 - yShift,
                    prtclbase[particle - 1].name);
  }
}
...
static int see_taranov(vampl *v, vertlink *head, int fromlegno,
                       Pre_test pre_test, see_edge leg_test,
                       Pos_test pos_test) {
  int i, j, Iout;
  int vertno, inleg;
  int Ivalence;
  ...
  for (i = 0; i < Ivalence; i++) {
    ...
    if ((leg->prop & IN_PRTCL) || (leg->prop & OUT_PRTCL)) {
      ...
      if (1 ==
          (Iout = see_taranov(v, &leg->link, i, pre_test, leg_test, pos_test)))
        break;
    }
  }
...
static int out_count_pos(vampl *v, vertlink *head, int Iout, int fleg,
                         int prop) {
  int previous_vertex;
  int vertno;
  edgeinvert *ed;
  ...
  for (i = 0; i < linesize; i++) {
    ...
    for (j = 0; j < v->valence[vertno]; j++) {
      ...
      if ((v->vertlist[vertno][j].prop & OUT_PRTCL) == 0)
        see_taranov(v, &v->vertlist[vertno][j].link, 0, out_count_pre,
                    out_count_leg, out_count_pos);
      else
        outlegs[vertno]++;
    }
  }
...
static void decay_pre(vampl *v, vertlink *head, int dumm, int *legs_order) {
  ...
  if ((v->vertlist[vertno][legs_order[0]].prop & (IN_PRTCL | OUT_PRTCL)) == 0)
...
static int decay_pos(vampl *v, vertlink *head, int dum1, int dum2, int dum3) {
  ...
  if (direction[vertno] == FROM_LEFT) {
    ...
    if (direction[vertno] == FROM_DOWN)
      vertexPos[previous_vertex].y =
          myedgePos[nin + out_place - 1].y + quant / 2;
  }
  return 0;
}
...
static void getPositions(vampl *v, position *vertexPos, position *edgePos) {
        ...
        if ((v->vertlist[vertno][j].prop & OUT_PRTCL) == 0) {
          if (DIR == FROM_UP && ii == 1) {
            ...
            if (LASTDIR == FROM_LEFT)
              vertexPos[vertno].y =
                  myedgePos[nin + out_place - 1].y + 2 * quant / 2;
            if (LASTDIR == FROM_DOWN)
              vertexPos[vertno].y =
                  myedgePos[nin + out_place - 1].y + 3 * quant / 2;
  ...
  if (nout == 1) {
    int yshift = quant;
    for (i = 0; i < v->size; i++)
      vertexPos[i].y = vertexPos[i].y + quant / 2;
    for (i = 0; i < (nin + nout); i++)
    ...
    for (j = 0; j < diagr->valence[i]; j++) {
      ...
      if (v->link.vno == nullvert) {
        ...
        if (v->prop & IN_PRTCL) {
          ...
          tg_outtextxy(x + out.x, y + out.y,
                       prtclbase[prtclbase[v->partcl - 1].anti - 1].name);
        } else {
          if (mode & 1)
            tg_settextjustify(RightText, CenterText);
          else
