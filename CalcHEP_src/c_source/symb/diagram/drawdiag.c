...
static int kt, kl, kk, ks, nk, se, se1, ku, tpc, pext; /* From picture */
...
static void mouttextxy(int xm, int y) {
  if (texflag)
    tg_outtextxy(xm, y, prtclbase[np - 1].latex);
  else
    /*  if(prtclbase[np-1].name[0]=='~')
      {   tg_outtextxy(xm,y,prtclbase[np-1].name+1);
          if(!prtclbase[np-1].name[2])
      tg_outtextxy(xm,y-tg_textheight("H")/2,"~"); else
      tg_outtextxy(xm,y-tg_textheight("H")/2,"~~"); } else
    */
...
static void mline(int x1_, int y1, int x2_, int y2, int r, int np_, int mm) {
  int h, y, xc, yc;
...
pp:
  ...
  kk = kl - 2 - kt;
  if (tpc == 2)
    kn[se].e1 = prtclbase[kn[se].e1 - 1].anti;
...
aa : {
  ...
  if ((tpc > 1) && ((l == -se) || (l == -se1))) {
    if (tpc == 3)
      mline(x[0], *yl, x[2], *yl, 0, with1->e1, 0);
    m1 = with1->e2;
    m2 = with1->e3;
    m3 = 0;
    p1 = with1->n2;
...
aa:
  ...
  if (m1 > 0)
    mline(x[1], yz, x[2], *yl, -1, m1, 0);
  else
    mline(x[1], yz, x[2], yz, 1, kn[-m1].pt, 0);
  pext = p1;
  knot1(m1, &yz, yyl);
    ...
    yz += ys;
  {
    ...
    if ((tpc < 3) && (with1->e2 > 0))
      ...
      mline(x[1], *yl, x[1], yz, 0, with1->pt, 0);
    *yl = yz;
    m1 = with1->e1;
    p1 = with1->n1;
    m2 = with1->e2;
...
static void draw(void) {
  ...
  if (((tpc == 3) && (kl == elongl(0) + 3)))
    yl = y[8];
  else if (((tpc < 3) || ((kt > 0) && (ks == 0))) &&
           ((kn[0].ty == 0) || ((kn[0].e2 < 0) && (kn[0].e3 < 0)) ||
            ((tpc == 2) && (kn[0].e2 < 0) && (kn[-(kn[0].e2)].ty == 0)))) {
    yl = y[5];
    ku = 1;
  } else
    yl = y[7];
  ...
  if ((tpc == 2))
    if (((tpc == 2) && (kl == 4) && (kk == 1)) ||
      ...
      if (l < 0) {
        if (ku == 1)
          mline(x[1], yl, x[2], yl, 3, kn[-l].pt, 0);
        else
          mline(x[1], yl, x[2], yl, 1, kn[-l].pt, 0);
        knot1(l, &yl, &yyl);
      } else
        knot2(&yl, &yyl);
    }
  else if (((tpc == 3) && ((kt == 0) || (ks != 0)) && (kl != elongl(0) + 3)) ||
           ((tpc == 1) && (kn[0].e2 < 0) && (kn[-(kn[0].e2)].ty == 0))) {
...
void picture(int squared, void *buff, int x, int y) {
  ...
  if (squared) {
    upr = 2;
    filler(((csdiagram *)buff)->dgrm1);
  } else {
    upr = 1;
    filler(((adiagram *)buff)->dgrm0);
  }
  ...
  if (squared) {
    ...
    for (q = 0; q < ni; q++) {
      ...
      tg_line(bh + xn1 + 1 + 6 * dWidth, intar[q].y1, bh + xn2 + 4 * dWidth,
              intar[((csdiagram *)buff)->lnk[q] - 1].y2);
    }
  }
}
