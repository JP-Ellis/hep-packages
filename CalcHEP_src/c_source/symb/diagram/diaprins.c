...
typedef struct intsect {
  int y, xst, sp, selfvert, selfslot, aliasvert, aliasslot;
} intsect;
  ...
  res->scr[x - 1][y - 1] =
      prtclbase[diag.vertlist[ivr - 1][slt - 1].partcl - 1].spin % 2 == 0
          ? ((n == 1 && left) || (n == 2 && !left))
                ...
                : ((n == 2 && left) || (n == 1 && !left))
                      ? chrD9
                      : ((n == 3 && left) || (n == 4 && !left)) ? chrBF : chrDA
          : ((n == 1 && left) || (n == 2 && !left))
                ? chrC8
                : ((n == 2 && left) || (n == 1 && !left))
                      ? chrBC
                      : ((n == 3 && left) || (n == 4 && !left)) ? chrBB : chrC9;
...
static void hline(int xs, int xe, int y, int ivr, int slt) {
...

  {
    ...
    linech = (prtclbase[with1->partcl - 1].spin % 2) == 0 ? chrC4 : chrCD;
    ...
    if (left) {
      j = l > 1 ? k + 1 - l : k - 1;
      for (i = 1; i <= l; i++)
        res->scr[i + j - 1][y - 1 - 1] = nm[i - 1];
      if (((IN_PRTCL | OUT_PRTCL) & with1->prop) || debug) {
        if ((left ? 1 : 0) == (with1->moment > 0 ? 1 : 0))
          res->scr[k - 1 - 1][y + 1 - 1] = '-';
        res->scr[k - 1][y + 1 - 1] = chp;
        res->scr[k + 1 - 1][y + 1 - 1] = abs(with1->moment) > 9
                                             ? abs(with1->moment) + '7'
                                             : (abs(with1->moment) + '0');
        if (debug && with1->lorentz > 0)
          res->scr[k - 1 - 1][y - 1] = with1->lorentz > 9
                                           ? (with1->lorentz + '7')
                                           : (with1->lorentz + '0');
      }
    } else {
...
static void vline(int x, int ys, int ye, int ivr, int slt, int txt) {
...

  {
    ...
    linech = (prtclbase[with1->partcl - 1].spin % 2) == 0 ? chrB3 : chrBA;
    l = left ? prtclbase[with1->partcl - 1].anti : with1->partcl;
    ...
    if (txt) {
      if (left) {
        ...
        if (debug) {
          ...
          res->scr[j + x + 1 - 1][k - 1] = abs(with1->moment) > 9
                                               ? (abs(with1->moment) + '7')
                                               : (abs(with1->moment) + '0');
          if (with1->lorentz > 0)
            res->scr[x - 1][k + 1 - 1] = with1->lorentz > 9
                                             ? (with1->lorentz + '7')
                                             : (with1->lorentz + '0');
        }
      } else {
        ...
        if (debug) {
          ...
          res->scr[x - j - 1 - 1][k - 1] = abs(with1->moment) > 9
                                               ? (abs(with1->moment) + '7')
                                               : (abs(with1->moment) + '0');
          if (with1->lorentz > 0)
            res->scr[x - 1][k + 1 - 1] = (with1->lorentz > 9)
...
static void mline(int xs, int ys, int xe, int ye, int ivr, int slt, int hor) {
  int lg;
...
static int howin(int i) {
  ...
  for (k = 1; k <= diag.valence[i - 1]; k++)
    if (IN_PRTCL & diag.vertlist[i - 1][k - 1].prop)
      ++(r);
  return r;
...
static int wherein(int i) {
  ...
  k = diag.valence[i - 1];
  while (k > 0 && !(IN_PRTCL & diag.vertlist[i - 1][k - 1].prop))
    --(k);
  return k;
...
static void lot(int *ylo, int *yhi, int ivr, int slt, int hor) {
  ...
  *yhi = 0;
  {
    edgeinvert *with1 = &diag.vertlist[ivr - 1][slt - 1];
    if ((IN_PRTCL | OUT_PRTCL) & with1->prop)
      return;
...
static void painter(int xc, int yc, int ivr, int slt, int hor) {
  int tp, l, xcnt, ycnt, xx, yy, hh, ylo, yhi, start, ks, kk, pp;
  int flag;
  /*
  char buff[10];
  ...
  */
  ...
  tp = howin(ivr);
  ks = wherein(ivr);
  kk = ks == diag.valence[ivr - 1] ? diag.valence[ivr - 1] - 1
                                   : diag.valence[ivr - 1];
  ...
  if (tp == 2) {
    ...
    mline(xstart, ycnt, xcnt, ycnt, ivr, ks, 1);
  } else if (diag.valence[ivr - 1] == 4) {
    start = 1;
    if (!hor)
      cr(1, xcnt++, ycnt, ivr, slt);
  } else if (hor && (OUT_PRTCL & diag.vertlist[ivr - 1][1].prop) &&
  ...
  for (l = kk; l >= 2; l--) {
    ...
    if (!(IN_PRTCL & with1->prop)) {
      if (flag) {
        ...
        // start, ivr,l, start==2, ylo,yhi);
      } else
        flag = 1;
      xx = (OUT_PRTCL & with1->prop) ? xend : start == 3 ? xcnt : xcnt + xstep;
      {
...
static int lh(int y) {
  ...
  else if (is[jsect - 1][y - 1] == chrB3)
    is[jsect - 1][y - 1] = ssect ? chrC5 : chrD8;
  else if (is[jsect - 1][y - 1] == chrBA)
...
static void lv(void) {
  ...
  is[isect - 1][jsect - 1] =
      c == chsp ? (ssect ? chrB3 : chrBA)
                : c == chrC4 ? (ssect ? chrC5 : chrD7)
                             : c == chrCD ? (ssect ? chrD8 : chrCE) : c;
...
b:
  pp = sh < 0 ? resl.mxy - sh : resr.mxy + sh;
  if (resr.mxy > pp)
    pp = resr.mxy;
  if (resl.mxy > pp)
    pp = resl.mxy;
...
void writeAmDiagram(vampl *diag1, int label, char comment, FILE *outfile) {
  ...
  for (i = 0; i < diag.sizel; i++) {
    ...
    for (j = 0; j < diag.valence[i]; j++)
      diag.vertlist[i][j] = diag1->vertlist[i][j];
  }
  ...
  for (i = 0, i2 = diag1->size, j2 = 0; i < diag1->outno; i++) {
    ...
    diag.vertlist[i2][j2].partcl =
        prtclbase[diag.vertlist[i2][j2].partcl - 1].anti;
