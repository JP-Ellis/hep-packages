...
static int testin(int l, decayDiagram restmp) {
  ...
  for (j = 0; liminsp[j].who; j++)
    copyins[j] = liminsp[j].how;
  for (j = 0; limout[j].who; j++)
    copyout[j] = limout[j].how;
  ...
  if (l == ndecay) {
    ...
    for (i = 1; i <= 2 * (l - 1); i++) {
      ...
      if (np < 0) {
        np = -np - 1;
        pdg = prtclbase[np].N;
        if (pdg == 23 || abs(pdg) == 24)
          nVV++;
      }
    }

    if (nVV > ZWmax || nVV < ZWmin)
      return 0;
  }
...
static void conon(decayDiagram diagram, int k) {
  ...
  do {
    ...
    length = li - l;
    if (diagram[li] != 0) { /*  3 - particle  vertex  */
      k -= length;
      c = 0;
      do { /*  until c=1  */
        if (diagram[++li - 1] > 0)
...
static void decay(int n_part, int l) {
  ...
  decaylink pp;
  int pk, d, c;
  int m, ni, li, k, jk, ik, i1, i2, i3, i2_min, i2_max, i3_min, i3_max, i[3],
      j[3];
  ...
  if (l == 1) {
    /*      k = 1;
          while (inclp[k-1].who != 0 && inclp[k-1].who != n_part) k++;
            if (inclp[k-1].who == 0 && missingmass - prtclbase[n_part-1].mass
       < 1.0E-5) ref[n_part-1][0] = zero; else
    */
    ...
    for (i3 = i3_min; i3 <= i3_max; i3++) {
      ...
      for (i2 = i2_min; i2 <= i2_max; i2++) {
        ...
        while (k < ni) {
          d = i[j[k - 1] - 1] - i[j[k] - 1];
          if (d > 0 ||
              (d == 0 && pp->part[j[k - 1] - 1] < pp->part[j[k - 1] - 1])) {
            c = j[k - 1];
            j[k - 1] = j[k];
            j[k] = c;
            if (k == 1)
        ...
        /*  memory optimisation ?=>	if (l == ndecay && maxavail() < 20000)
         * clearref(); */
      label_101:;
      }      /* I2 FOR circl  */
    }        /*  I3 FOR circl  */
