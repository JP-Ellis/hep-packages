...
static int findParam(int X, int Y, int rdf, int shift, int tot, char *title,
                     int *pos) {
  int n;
  char *menu;
  char *hlp = "";
  ...
  for (n = 0; n < tot; n++)
    sprintf(menu + strlen(menu), " %-11.11s%12.4E", varNames[n + shift],
            (double)(varValues[n + shift]));
...
static int changeParam(int X, int Y) {
  ...
  for (n = 0, ch = 0, pos = 1; n >= -1;) {
    ...
    if (pos == 1) {
      ...
      if (correctDouble(20, 20, txt, &x, 1)) {
        varValues[n] = x;
        ch = 1;
      }
    }
        ...
        case 4:
          if (xMax > xMin && nPoints >= 3 && nPoints <= 150) {
            ...
            for (i = 0; i < nPoints; i++) {
              ...
              if (NaN) {
                char mess[100];
                sprintf(mess, "Can not evaluate constraints for %s=%G",
                        varNames[ni], x);
            ...
            if (!(NaN || Esc))
              plot_1(xMin, xMax, nPoints, f, NULL, "Plot", varNames[ni],
                     varNames[nc]);
...
static void writeSLHA(void) {
  ...
  for (i = 0; i < nModelParticles; i++) {
    ...
    fprintf(f,
            ...
            "%d # 0={ "
            "self-conjugated}\n#\n",
            ModelPrtcls[i].q3, ModelPrtcls[i].spin2 + 1, ModelPrtcls[i].cdim,
  ...
  for (i = 0; i < nModelParticles; i++) {
    ...
    double mass, width;
    char *name;
    if (strcmp(ModelPrtcls[i].mass, "0") == 0)
      continue;
    if (strcmp(ModelPrtcls[i].width, "0") == 0)
      continue;
...
static void show_spectrum(int X, int Y) {
  ...
  for (i = 0; i < nModelParticles; i++) {
    ...
    char *name = ModelPrtcls[i].name;
    if (!strcmp(mass, "0"))
      sprintf(menuP + strlen(menuP), " %-6.6s      Zero     ", name);
    else
      sprintf(menuP + strlen(menuP), " %-6.6s  %12.4E ", name, pMass(name));
  }
  ...
  while (mode) {
    ...
    if (mode == 1)
      ...
      fprintf(f, "Patricle %s(%s),  PDG = %d,  Mass= ", ModelPrtcls[pos].name,
              ModelPrtcls[pos].aname, ModelPrtcls[pos].NPDG);
      if (strcmp(mass, "0") == 0)
        fprintf(f, "Zero\n");
      else {
        double width;
...
static double L3 = 3.125347E-01, L4 = 2.763267E-01, L5 = 1.991586E-01,
              L6 = 8.449407E-02;
...
double alpha_2(double Q) {
  ...
  if (Q < MCHARM)
    return alpha(nf3, 3, L3, Q);
  else if (Q < MbMb)
    return alpha(nf4, 3, L4, Q);
  else if (Q < Mtp)
    return alpha(nf5, 3, L5, Q);
  else
    return alpha(nf6, 3, L6, Q);
}
