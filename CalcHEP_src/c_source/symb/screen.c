...
static void diag_stat(int type, int *n_sub, int *n_del, int *n_calc,
                      int *n_rest) {
...
cont:
  ...
  do {
    ...
    if (edit)
      strcat(tabMenu, "      RENAME   "
                      "   CHECK MODEL ");
    if (edit)
      menu1(56, 7, "", tabMenu, "s_mdl_e", &pscr, &n);
    else
    ...
    case 0:
      if (edited || renamed) {
        if (mess_y_n(15, 19, " Save corrections ?")) {
          if (ok || loadModel(1, 0) ||
              mess_y_n(15, 19, " Save incorrect models?")) {
          ...
          if (renamed) {
            ...
            for (; i < size - 1; i++)
              modelmenu[(n_model - 1) * size + 2 + i] = ' ';
            modelinfo();
          }
        } else {
          int mStat = ldModelStatus;
    ...
    case 5:
      edited =
          edittable(1, 1, &modelTab[n - 1], 1, tabhelp[n - 1], !edit) || edited;
      if (edited)
        ok = 0;
      break;
    case 6:
      if (correctStr(10, 17, "New name:", modelTab[0].mdlName, 29, 1)) {
        ...
        for (i = 1; i < 5; i++)
          strcpy(modelTab[i].mdlName, modelTab[0].mdlName);
        renamed = 1;
      }
      break;
    case 7:
      if (!loadModel(1, 0))
        ...
        ok = 1;
        messanykey(10, 15, "The model is Ok");
      }
      break;
    }
  } while (n);
...
void showheap(void) { /* goto_xy(60,24);  print("Memory= %lu",usedmemory);*/
}
...
void viewsqdiagr(void) {
  ...
  do {
    ...
    if (nsub != 0) {
      //      if(nin+nout<7)
      showgraphs(2);
      //        else  messanykey(10,15,"The editor does not work if the number
      //        of legs exceed 6.");
    }
    sq_diagramsinfo();
...
void viewfeyndiag(int del_mode) {
  ...
  do {
    if (subproc_f == 1)
      ...
      menu_f(9, 11,
             "NN        Subprocess                               Del   Rest ",
             MENUP_NAME, "s_proc", &pscr, &nsub);
    }
    if (nsub > 0) { /*showgraphs2(upr);*/
      showgraphs(upr);
      if (del_mode)
        diagramsinfo();
    }
...
void cleanResults(void) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (strcmp(namelist[i]->d_name, "aux") &&
        strcmp(namelist[i]->d_name, "..") && strcmp(namelist[i]->d_name, ".")) {
...
label_1:
  ...
  case 2:
    if (dirStat == 2) {
      char mess[] =
          "Can not clean dir 'results' because it contains the LOCK file";
      if (blind) {
        printf("%s\n", mess);
        sortie(102);
      } else {
        messanykey(3, 13, mess);
        break;
      }
    }
    ...
    while (1) {
      ...
      if (k == KB_ENTER) {
        ...
        if (rename("results", newname) == 0) {
          ...
          messanykey(10, 15, " Can't rename the directory");
      }
      put_text(&pscr3);
    }
  }
  ...
  while (kmenu) {
    ...
    */
    if (NcInfLimit) {
      ...
      if (NcInfCC == 1)
        improveStr(strmen, "OF2", "ON");
      else
        improveStr(strmen, "OF2", "OFF");
    }
    ...
    */
    if (noPict)
      improveStr(strmen, "OF3", "OFF");
    else
      improveStr(strmen, "OF3", "ON ");
    if (tWidths)
      improveStr(strmen, "OF4", "ON ");
    else
      improveStr(strmen, "OF4", "OFF");

    if (VWdecay)
      improveStr(strmen, "OF5", "ON ");
    else
      improveStr(strmen, "OF5", "OFF");
    ...
    case 4: {
      char mess[40];
      sprintf(mess, "There are %d processors. To use ",
              (int)sysconf(_SC_NPROCESSORS_ONLN));
      correctInt(20, 22, mess, &nPROCSS, 1);
    } break;
    case 5:
      NcInfLimit = !NcInfLimit;
      break;
    case 6:
      NcInfCC = !NcInfCC;
      break;
