...
static void *bact5(char ch, void *mm1, void *mm2) {
  ...
  case '/':
    if (dbl)
      sprintf(ans, "%c%c|%s/(double)(%s)", p_m, r_n, m1, m2);
    else if (m2[0] != '-')
      sprintf(ans, "%c%c|%s/%s", p_m, r_n, m1, m2);
    else {
      if (m1[0] == '-')
...
void *act_c(char *name, int n, void **args) {
  ...
  if (strcmp(name, "aWidth") == 0)
    name = "aWidth_ext";
  l = n + 10 + strlen(name);
  for (i = 0; i < n; i++)
    l += strlen((char *)args[i]);
  int prnf = 0;
  if (strcmp(name, "aPrintF") == 0 || strcmp(name, "printf") == 0) {
    l += 10 * (n - 1);
    prnf = 1;
  }
  ans = m_alloc(l);
  if (!strcmp(name, "if") && n == 3) {
    if (((char *)args[0])[1] == 'N' && ((char *)args[1])[1] == 'N')
      tp = 'N';

    sprintf(ans, "M%c|(%s>0 ? %s : %s)", tp, (char *)args[0] + 3,
            (char *)args[1] + 3, (char *)args[2] + 3);
  } else {
    if (prnf)
      tp = 'N';
    sprintf(ans, "M%c|%s(", tp, name);
    if (n)
      strcat(ans, (char *)args[0] + 3);
    for (i = 1; i < n; i++) {
      if (prnf && ((char *)args[i])[1] == 'R')
        sprintf(ans + strlen(ans), ",(double)(%s)", (char *)args[i] + 3);
      else
        sprintf(ans + strlen(ans), ",%s", (char *)args[i] + 3);
    }
    strcat(ans, ")");
  }
  return ans;
