...
void seekArchiv(long n) { fseek(archiv, n, SEEK_SET); }
...
void readDenominators(void) {
  ...
  for (i = 0; i < denrno; i++) {
    ...
    m = 0;
    do
      FREAD1(denom[i].momStr[m], archiv);
    ...
    if (ncalc != 0) {
      ...
      while (FREAD1(cr, catalog)) {
        if (cr.status == 1 && cr.nsub_ == nsub) {
          ...
          if (graphOn) {
            fseek(diagrq, (cr.ndiagr_ + recpos - 1) * sizeof(csdiagram),
                  SEEK_SET);
            FREAD1(csdiagr, diagrq);
            transfdiagr(&csdiagr, &vcs);
            diagramOutput(&vcs, &cr);
          } else
            diagramOutput(NULL, &cr);
          ++(count);
          if (informline(count, ntot))
            goto escexit;
        }
      }
    escexit:
