...
static void writeprocessname(int *prtclNum) {
  ...
  for (i = 1; i <= nin; i++)
    if (i == nin)
      ...
      writeF("\"%s\",", prtclbase1[prtclNum[i]].name);
...
static int modifyF(char *txt) {
  ...
  for (Br = strchr(txt, '('); Br; Br = strchr(Br + 1, '(')) {
    ...
    i++;
    Br[0] = 0;
    if (strcmp(Br + i, "sqrt") == 0 || strcmp(Br + i, "sin") == 0 ||
        strcmp(Br + i, "cos") == 0 || strcmp(Br + i, "tan") == 0 ||
        strcmp(Br + i, "asin") == 0 || strcmp(Br + i, "acos") == 0 ||
        strcmp(Br + i, "atan") == 0 || strcmp(Br + i, "exp") == 0 ||
        strcmp(Br + i, "log") == 0)
      Br[0] = '(';
    else if (strcmp(Br + i, "one") == 0)
      strcpy(Br + i, "1  ");
    else {
      extF = 1;
      Br[0] = '(';
