...
static void diagramForm(vcsect *vcs, catrec *cr) {
  ...
  for (i = 0; i < denrno; i++) {
    ...
    writeF("\n *propDen(%s,%s)", momStr, vararr[denom[i].mass].alias);
    if (abs(denom[i].power) != 1)
      writeF("^%d", abs(denom[i].power));
  }
  writeF(";\n");
  writeF("\n#call userWork{%d}\n", cr->ndiagr_);
}
...
void makeFormOutput(void) { makeOutput(startForm, diagramForm, endForm); }
