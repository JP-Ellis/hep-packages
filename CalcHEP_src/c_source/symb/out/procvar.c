...
int initvararray(int nsub, char key, polyvars *vardef_ext) {
  ...
  if (vardef_ext == NULL) {
        ...
        if (cr.status == 1) {
          if (!nsub || cr.nsub_ == nsub) {
            ...
            for (l = 0; l < vardef->nvar; l++)
              vararr[vardef->vars[l].num].used = 1;
            clearvardef();
            ...
            for (l = 0; l < denrno; l++) {
              if (denom[l].mass)
                vararr[denom[l].mass].used = 1;
              if (denom[l].width)
                vararr[denom[l].width].used = 1;
            }
            clearvardef();
          }
        }
    ...
    for (i = 0; i < vardef_ext->nvar; i++)
      vararr[vardef_ext->vars[i].num].used = 1;
  for (k = nmodelvar; k >= 0; k--)
    if (vararr[k].used && modelvars[k].func &&
        (key != 'c' || (k > nCommonVars) && modelvars[k].pub == 0))
  ...
  for (i = 1; i <= 2; i++) {
    ...
    case 'F':
    case 'M':
      sprintf(vararr[k].alias, "Helicity%d", i);
      break;
    case 'c':
      sprintf(vararr[k].alias, "Helicity[%d]", i - 1);
      break;
    case 'f':
      sprintf(vararr[k].alias, "Helicity(%d)", i);
      break;
  ...
  case 'R':
  case 'F':
    sprintf(vararr[k].alias, "(1/p1.p2)");
    break;
  case 'M':
    sprintf(vararr[k].alias, "(1/SC[p1,p2])");
    break;
  case 'c':
    sprintf(vararr[k].alias, "N_p1p2_");
    break;
  }
  vararr[k].used = 1;
  ...
  if (key == 'F' || key == 'R' || key == 'M')
    ...
    for (k = 0; k < 4; k++) {
      if (strcmp(modelvars[k].varname, "i") == 0)
        strcpy(vararr[k].alias, "I");
      else if (strcmp(modelvars[k].varname, "pi") == 0)
        strcpy(vararr[k].alias, "M_PI");
      else if (strcmp(modelvars[k].varname, "Sqrt2") == 0)
        strcpy(vararr[k].alias, "M_SQRT2");
      vararr[k].used = 0;
    }
    ...
    for (k = nCommonVars + 1; k < nmodelvar; k++)
      if (vararr[k].used && (modelvars[k].pub == 0)) {
