...
static void calc_nvars_nfunc(void) {
  int k;
  for (nvars = 0, nfunc = 0, k = 1; k <= nmodelvar; k++)
    if (vararr[k].used) {
      if (k > nCommonVars && modelvars[k].pub == 0)
        nfunc++;
      else
        nvars++;
    }
    ...
    for (i = 0; i < nin + nout; i++) {
      ...
      if (strcmp(mass, "0")) {
        for (k = 1; k <= nmodelvar; k++)
          if (strcmp(modelvars[k].varname, mass) == 0)
            break;
      } else
        k = 0;
      if (k > nmodelvar) {
        printf("mass %s is not defined\n", mass);
        exit(125);
      }
      inf->p_masspos[i] = k;
...
static void sortvars(void) {
  ...
  for (k = 4; k <= nmodelvar; k++)
    if (vararr[k].used && (k <= nCommonVars || modelvars[k].pub))
      writeF("\n,\"%s\"", modelvars[k].varname);
  for (k = 1 + nCommonVars; k <= nmodelvar; k++)
    if (vararr[k].used && !modelvars[k].pub)
      writeF("\n,\"%s\"", modelvars[k].varname);
}
...
static void writesubroutineinit(void) {
  ...
  for (l = nCommonVars + 1; l <= nmodelvar; l++) {
    if (vararr[l].used &&
        ((modelvars[l].func && modelvars[l].pub == 0) || modelvars[l].pwidth)) {
      int num;
      checkNaN = 0;
...
static void writeDenominators(deninforec *dendescript) {
  ...
  for (i = 0; i < dendescript->tot_den; i++) {
    ...
    if (dendescript->denarr[i].width == 0)
      numm += nden_w;
    else {
    ...
    if (dendescript->denarr[i].power == 2)
      writeF("*Q2[%d]", numm);
    else {
      if (dendescript->denarr[i].stype) {
      ...
      for (k = 1; k <= nden_s; k++) {
        ...
        for (i = 1; i <= dendescript->tot_den; i++)
          if (dendescript->denarr[i - 1].width &&
              dendescript->denarr[i - 1].stype &&
              k == dendescript->denarr[i - 1].order_num)
            addpr = 0;
        if (addpr)
          writeF("*Q0[%d]", k);
      }
    ...
    if (nden_t2) {
      ...
      for (k = nden_s + 1; k <= nden_w; k++) {
        ...
        for (i = 1; i <= dendescript->tot_den; i++)
          if (dendescript->denarr[i - 1].width &&
              (!dendescript->denarr[i - 1].stype) &&
              k == dendescript->denarr[i - 1].order_num + nden_s)
            addpr = 0;
        if (addpr)
          writeF("*Q0[%d]", k);
      }
...
static void calcColor(long diag) {
  ...
  if (cBasisPower && generateColorWeights(&csdiagr, cBasisPower, nC, cChains,
                                          cCoefN, cCoefD)) {
    int k;
    writeF(" if(cb_coeff)\n {\n");
    ...
    for (k = 0; k < cBasisPower; k++)
      if (cCoefN[k]) {
        ...
        writeF("  cb_coeff[%d] += (R*%d)/(%d); %s ", k, cCoefN[k], cCoefD[k],
               "/*");
        for (i = 0; i < nC; i++)
          if (cChains[4 * (nC * k + i)] == 2)
            writeF("(%d %d) ", cChains[4 * (nC * k + i) + 1],
                   cChains[4 * (nC * k + i) + 2]);
          else
            writeF("(%d %d %d) ", cChains[4 * (nC * k + i) + 1],
                   cChains[4 * (nC * k + i) + 2],
                   cChains[4 * (nC * k + i) + 3]);
        writeF("*/\n");
      }
...
static void onediagram(deninforec *dendescript) {
  ...
  if (deg1)
    writeF("REAL S[%d];", deg1);
  if (tmpNameMax)
    writeF("REAL tmp[%d];", tmpNameMax);

  fseek(outFile, 0, SEEK_END);
  tmpNameMax = 0;
  initdegnames();

  writeF("REAL F%d_ext(double "
         "GG,REAL*DP,REAL*Q0,COMPLEX*Q1,REAL*Q2,REAL*cb_coeff)\n{\n",
         diagrcount);
  ...
  writeF("R*=creal(Prop);\n");
  if (!noCChain)
    calcColor(cr.ndiagr_ + inftmp->firstdiagpos);
...
static int alldiagrams(FILE *fd, int nsub) {
  ...
  if (deg1)
    writeF("REAL S[%d];", deg1);
  if (tmpn1)
    writeF("REAL tmp[%d];", tmpn1);

  fseek(outFile, pos_c2, SEEK_SET);
  deg2 = cleardegnames();
  tmpn2 = tmpNameMax;
  if (deg2)
    writeF("REAL S[%d];", deg2);
  if (tmpn2)
    writeF("REAL tmp[%d];", tmpn2);
  fseek(outFile, 0, SEEK_END);
...
static void writesubprocess(int nsub, long firstDiag, long totDiag,
                            int *breaker) {
  denlist den_;
  int i;
  ...
  if (totDiag == 0) {
    ...
    writeF("REAL S%d_ext(double GG,  REAL * momenta, REAL*cb_coeff, "
           "double*Fmax, int * err)\n{",
           nsub);
    writeF("  return 0;\n}\n");
    outFileClose();
  ...
  if (sumDiag)
    writeF("static void C%d(REAL *);\n", nsub);
  else {
    writeF("extern FNN F%d_ext", firstDiag);
    for (i = 1; i < totDiag; i++)
      writeF(",F%d_ext", i + firstDiag);
    writeF(";\n");
  ...
  writeF("extern DNN S%d_ext;\n", nsub);
  writeF("REAL S%d_ext(double GG, REAL * momenta, REAL*cb_coeff, double*Fmax, "
         "int * err)\n{",
         nsub);
  writeF("REAL  ans=0;\n");
  ...
  if (nin + nout > 3) {
    ...
    writeF("REAL Q0[%d]; COMPLEX Q1[%d]; REAL Q2[%d];\n", nden_w + nden_0 + 1,
           nden_w + nden_0 + 1, nden_w + nden_0 + 1);
    ...
    if (sumDiag)
      ...
      writeF("  if(!momenta) {int i; for(i=0;i<%d;i++) "
             "Farr[i](GG,NULL,NULL,NULL,NULL,NULL); return "
             "0;}\n",
             totDiag);
    ...
    for (; den_; den_ = den_->next) {
      ...
      if (den_->width) {
        if (den_->stype)
          fprintf(outFile, "width[%d]=%s; ", i, vararr[den_->width].alias);
        else {
          i += nden_s;
          fprintf(outFile, "width[%d]=(twidth_ext)? %s : 0.; ", i,
                  vararr[den_->width].alias);
        }
      } else {
        i += nden_w;
      ...
      /*         fprintf(outFile,"
       * Q[%d]=mass[%d]*mass[%d]-sqrMom(nin_ext,\"",i,i,i);*/

      while (den_->momStr[m])
        fprintf(outFile, "\\%o", den_->momStr[m++]);
      fprintf(outFile, "\";\n");
      /*         fprintf(outFile,"\",momenta);\n");   */
    }
    writeF("*err=*err|prepDen(%d,nin_ext,BWrange_ext*BWrange_ext,mass,width,"
           "Qtxt,momenta,Q0,Q1,Q2);\n",
           nden_w + nden_0);
  } else {
    if (sumDiag)
      writeF(" if(momenta)\n {");
  ...
  if (sumDiag) {
    *breaker = alldiagrams(fd, nsub);
    ...
    writeF("{ REAL r=Farr[i](GG,DP,Q0,Q1,Q2,cb_coeff);\n"
           "  if(r>(*Fmax)) *Fmax=r;\n"
           "  ans+=r;\n"
           "}}\n"
           "return ans;\n}\n");
...
static void make_pinf(void) {
  ...
  writeF("int pinfAux_ext(int nsub,int "
         "nprtcl,int*spin2,int*color,int*neutral,int*ndf)\n{\n");
  /*   writeF("int n;\n"); */
  ...
  for (nsub = 1; nsub <= subproc_sq; nsub++) {
    ...
    for (i = 0; i < nin + nout; i++) {
      ...
      if (strcmp(prtclbase1[pos].massidnt, "0") == 0 && spin2 == 2)
        ndf *= 2;
      else if (strchr("LR", prtclbase1[pos].hlp) == 0)
        ndf *= (spin2 + 1);
      writeF("%d", ndf);
    }
    if (nsub == subproc_sq)
...
static void make_infbasis(void) {
  ...
  for (nsub = 1, inftmp = inf; nsub <= subproc_sq; nsub++) {
    for (i = 0; i < nin + nout; i++) {
      ...
      if (i < nin && pcolor[i] != 1 && pcolor[i] != 8)
        pcolor[i] *= -1;
    }
      ...
      for (i = 0; i < cBasisPower; i++) {
        ...
        for (j = 0; j < nC; j++) {
          writeF(" %d,%d,%d,%d ", cChains[4 * (i * nC + j)],
                 cChains[4 * (i * nC + j) + 1], cChains[4 * (i * nC + j) + 2],
                 cChains[4 * (i * nC + j) + 3]);
...
static int *makeCperm(int ntot, int *np, int *perm) {
  ...
  for (i = 0; i < ntot; i++) {
    ...
    if (i < nin && pc[i] != 1 && pc[i] != 8)
      pc[i] *= -1;
  }

  bPow = infCbases(ntot, pc, &nC, &chains);
  // printf(" Ibasis: "); for(i=0;i<bPow*nC*4;i++) printf("%d ",chains[i]);
  // printf("\n");
  ...
  if (bPow > 0) {
    ...
    memcpy(chains2, chains, nC * bPow * 4 * sizeof(int));
    {
      ...
      for (k = 0; k < 4 * nC * bPow; k++)
        if (k % 4 && chains2[k])
          chains2[k] = perm[chains2[k] - 1] + 1;
      // printf(" Pbasis: "); for(i=0;i<bPow*nC*4;i++) printf("%d ",chains2[i]);
      // printf("\n");
      ...
      for (l = 0; l < bPow; l++) {
        ...
        for (l2 = 0; l2 < bPow; l2++)
          if (memcmp(chains + 4 * nC * l2, c, 4 * nC * sizeof(int)) == 0) {
            res[l + 1] = l2 + 1;
            break;
          }

        if (l2 == bPow)
          fprintf(stderr, "Can not construct permutation\n");
      }
      // printf(" Sbasis: "); for(i=0;i<bPow*nC*4;i++) printf("%d ",chains2[i]);
      // printf("\n");
    }
    free(chains2);
  }
...
static void make_perm(void) {
  ...
  for (nsub = 1, inftmp = inf; nsub <= subproc_sq;
       nsub++, inftmp = inftmp->next) {
    int nP = 1;
    int k = 1;
    for (i = nin + 1; i < nin + nout; i++) {
      if (strcmp(inftmp->p_name[i], inftmp->p_name[i - 1]) == 0) {
        k++;
        nP *= k;
      } else
        k = 1;
  ...
  for (nsub = 1, inftmp = inf; nsub <= subproc_sq;
      ...
      ;
    {
      ...
      writeF("\n // ");
      for (i = 0; i < nin; i++)
        writeF("%s ", inftmp->p_name[i]);
      writeF("-> ");
      for (i = nin; i < nin + nout; i++)
        writeF("%s ", inftmp->p_name[i]);

      for (i = 0; i < nin + nout; i++)
        locateinbase(inftmp->p_name[i], np + i);
      for (i = 0; i < nin; i++) {
        used[i] = 1;
        buff[i] = i;
      }
  ...
  for (i = 0, cbPowMax = 0; i < ncPerm; i++)
    if (cbPowMax < cPerm[i][0])
      cbPowMax = cPerm[i][0];
  writeF("static int permC[%d][%d]={\n", pos, cbPowMax);

  for (i = 0; i < ncPerm; i++) {
    writeF("{");
    for (j = 1; j < cPerm[i][0]; j++)
      writeF("%d,", cPerm[i][j]);
    if (j == cPerm[i][0])
      writeF("%d", cPerm[i][j]);
    writeF("}");
...
static int c_prog_int(void) {
...

  {
    ...
    fprintf(
        ...
        "varName_ext,va_ext,"
        "%d, %d, %d, &pinf_ext, &pinfAux_ext, polarized_ext, &calcFunc_ext, "
        "&BWrange_ext,&twidth_ext,"
        "&gtwidth_ext,&gswidth_ext, &aWidth_ext, "
        "&sqme_ext,&den_info_ext,cb_ext};\n",
  ...
  for (nsub = 1, dfirst = 1; nsub <= subproc_sq; nsub++) {
    ...
    if (inftmp->tot != 0) /*  this subprocess IN archive  */
    {
      ...
      for (i = 0; i < nin; i++)
        if (abs(colors[i]) == 3 || abs(colors[i]) == 6)
          colors[i] *= -1;
      ...
      writesubprocess(nsub, dfirst, 0, NULL);
    inftmp = inftmp->next;
  }
...
int vert_code(polyvars *vardef_ext) {
  ...
  for (l = 0; l < vardef_ext->nvar; l++)
    if (!strchr(vardef_ext->vars[l].name, '.'))
