...
void wrt_menu(int menutype, int k, char *txt, int ndel, int ncalc, int nrest,
              long recpos) {
  if (menutype == 1) {
    ...
    fprintf(menuq, "%4d| %-44.44s|%5d|%5d|%5d|%-8ld", k, txt, ndel, ncalc,
            nrest, recpos);
  }
}

int rd_menu(int menutype, int k, char *txt, int *ndel, int *ncalc, int *nrest,
            long *recpos) {
  if (menutype == 1) {
    fseek(menup, (k - 1) * 71 + 2, SEEK_SET);
    if (5 !=
        fscanf(menup, "%d| %[^|]%*c%d|%d|%ld", &k, txt, ndel, nrest, recpos))
      return 0;
    *ncalc = 0;
  } else {
    fseek(menuq, (k - 1) * 77 + 2, SEEK_SET);
    if (6 != fscanf(menuq, "%4d| %[^|]%*c%d|%d|%d|%ld", &k, txt, ndel, ncalc,
                    nrest, recpos))
      return 0;
  }
  return 1;
...
static void *pCompile_cycle(int *input) {
  ...
  for (;;) {
    ...
    for (i = ibeg + 1; i <= iend; i++)
      sprintf(csrc + strlen(csrc), " f%d.c", i);
    strcpy(osrc, csrc);
    for (c = strstr(osrc, ".c"); c; c = strstr(c, ".c"))
      c[1] = 'o';
...
int pCompile(void) {
  ...
  while ((dp = readdir(dirPtr))) {
    ...
    if (c && c[2] == 0) {
      if (sscanf(dp->d_name, "f%d", &n) == 1 && n > fN)
        fN = n;
      if (sscanf(dp->d_name, "d%d", &n) == 1 && n > dN)
        dN = n;
    }
  }
  closedir(dirPtr);
  ...
  if (fN) {
    ...
    for (k = 0; k < nPROCSS; k++)
      num[k] = k + 1;
    for (k = 0; k < nPROCSS; k++)
      pthread_create(threads + k, NULL, pCompile_cycle, num + k);
    comp_control(NULL);
    for (k = 0; k < nPROCSS; k++)
  ...
  if (dN)
    ...
    csrc[0] = 0;
  if (access("service.c", R_OK) == 0)
    strcat(csrc, " service.c");
  if (access("sqme.c", R_OK) == 0)
    strcat(csrc, " sqme.c");
  if (access("VandP.c", R_OK) == 0)
    strcat(csrc, " VandP.c");
  strcpy(osrc, csrc);
  for (c = strstr(osrc, ".c"); c; c = strstr(c, ".c"))
    c[1] = 'o';
