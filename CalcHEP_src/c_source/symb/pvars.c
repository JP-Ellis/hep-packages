...
void unite_vardef(polyvars *vardef_s, polyvars *vardef) {
  ...
  for (nn = 0; nn < vardef->nvar; nn++) {
    ...
    while (n < vardef_s->nvar &&
           strcmp(vardef->vars[nn].name, vardef_s->vars[n].name))
      n++;
    if (n < vardef_s->nvar)
      vardef_s->vars[n].maxdeg =
...
void addvar(char *varname, int deg) {
  ...
  n = 0;
  while (n < vardef->nvar && strcmp(varname, vardef->vars[n].name))
    n++;
  if (n < vardef->nvar)
    vardef->vars[n].maxdeg += deg;
  else {
    increaseVars(vardef);
...
void sortvar() {
  ...
  for (i = 0; i < vardef->nvar; i++) /*  numeration  */
    ...
    } else if (!strcmp(vardef->vars[i].name, "Helicity1"))
      vardef->vars[i].num = nmodelvar + 2;
    else if (!strcmp(vardef->vars[i].name, "Helicity2"))
      vardef->vars[i].num = nmodelvar + 3;
    else if (!strcmp(vardef->vars[i].name, "N_p1p2_"))
      vardef->vars[i].num = nmodelvar + 4;
    else
      vardef->vars[i].num = modelVarPos(vardef->vars[i].name);

  for (i = 0; i < vardef->nvar; i++)
    if (!strcmp(vardef->vars[i].name, "i")) {
      vardef->nvar--;
      break;
    }
  for (; i < vardef->nvar; i++)
    vardef->vars[i] = vardef->vars[i + 1];

  if (vardef->nvar > 1) /*  Sorting  */
  {
    i = 1;
    while (i < vardef->nvar)
      if (vardef->vars[i - 1].num > vardef->vars[i].num)
        i++;
      else {
