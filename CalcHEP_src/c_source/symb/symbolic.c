...
static poly polyfactor(poly p) {
  ...
  m->next = NULL;
  if (!p)
    for (i = 0, m->num = 1; i < monomLength; i++)
      m->power[i] = 0;
  else {
    num1 = p->num;
    if (num1 < 0) {
...
static void firstvertexreading(void) {
  ...
  for (v = 0; v < vcs.sizet; v++) {
    void *agrs[2];
    m_ = (preres)readExpression(vertexes[v].lgrnptr->description, rd_pre,
                                act_pre, NULL);

    if (rderrcode)
      save_sos(rderrcode);
    if (m_->g5)
      fermloops[fermmap[v] - 1].g5 = 1;
    gamma_map[v] = m_->maxg;
    m_->g5 = 0;
    m_->maxg = 0;
    m_->indlist = 0;
  ...
  for (n = 0; n < vardef->nvar; n++)
    vardef->vars[n].maxdeg = m->varsdeg[n] + 1;
  maxmomdeg = m->degp;
}
...
static int findMaxIndex(void) {
  ...
  for (maxI = 0, v = 0; v < vcs.sizet; v++) {
    vln = vcs.valence[v];
    for (l = 0; l < vln; l++)
      maxI = MAX(maxI, vcs.vertlist[v][l].lorentz);
  }
  if (!maxI)
    maxI = 1;
    ...
    for (l = 0; l < vcs.valence[v]; l++) {
      ...
      if (L->moment > 0) {
        ...
        if (d == 2 && !set_in(L->lorentz, setmassindex) &&
            !(PLR_PRTCL & L->prop))
          d = 0;
        if (d == 4 && tolower(prtclbase1[L->partcl].hlp) == 't')
          d = 2;
        maxmomdeg += d;
...
static void addscmult(int nSpin) {
  ...
  for (n = 1; n < px; n++)
    for (m = 0; m < n; m++) {
      sprintf(name, "p%d.p%d", m + 1, n + 1);
      addvar(name, maxmomdeg);
    }
...
#ifdef STRACE
    writeF("\n VERTEX %d  %s\n type=%d \n", v, vertexes[v].lgrnptr->description,
           vert[v].type);
    symb_print("=>", vert[v]);
#endif
    free(pstr);
...
static symb_data fermpropag(int v, int l, int spin) {
  ...
  if (spin == 1)
    proptxt = fermPropagTxt(v, l, 0);
  else
    proptxt = spin3_2_propagator(v, l, 0);
  m = symb_read(proptxt);
  free(proptxt);
  return m;
}
    ...
    for (n = 1; n < Lp->len; n++) {
      ...
      for (i = 0; i < Lp->nint[n]; i++) {
        ...
        if (s == 2 && set_in(L->lorentz, setmassindex)) {
          symb_data Vk = symb_copy(V), m2 = masscalc(v, l, 2),
                    km = kmsubcalc(v, l);
...
static void multblocks(int v1, int v2, int saveEps) {
  ...
  if (onlineWrt) {
    ...
    for (i = 1; i <= 15; i++)
      if (set_in(i, ind1) && set_in(i, ind2))
...
static void transformfactor(rmptr *t_fact, poly mon, long del) {
  ...
  for (i = 0; i < vardef->nvar; i++) {
    deg = (mon->power[vardef->vars[i].wordpos - 1] / vardef->vars[i].zerodeg) %
  ...
  for (c = 1, v = 0; v < vcs.sizel; v++)
    for (l = 0; l < vcs.valence[v]; l++)
      if (IN_PRTCL & vcs.vertlist[v][l].prop) {
        ...
        switch (s) {
        case 1:
          if (!strchr("LR", prtclbase1[pnum].hlp))
            c *= 2;
          break;
        case 2:
          if (zeromass(pnum))
  ...
  for (c = 1, v = 0; v < vcs.sizel; v++)
    for (l = 0; l < vcs.valence[v]; l++)
      if (IN_PRTCL & vcs.vertlist[v][l].prop &&
          (prtclbase1[vcs.vertlist[v][l].partcl].spin & 1))
        c *= -1;
  for (v = 0; v < nloop; v++)
    c *= -4;
  factnum *= c;
  ...
  for (v = 0; v < vcs.sizet; v++)
    for (l = 0; l < vcs.valence[v]; l++)
      if (vcs.vertlist[v][l].moment > 0 &&
          strchr("LR", prtclbase1[vcs.vertlist[v][l].partcl].hlp))
        factdenum *= 2;
  ...
  for (v = 0; v < vcs.sizet; v++)
    for (l = 0; l < vcs.valence[v]; l++) {
      ...
      if (L->moment > 0 && strcmp(mass, "0")) {
        if (pseudop(L->partcl) ||
            (L->lorentz && set_in(L->lorentz, setmassindex0)))
...
static void formBlocks(void) {
  ...
  for (v = 0; v < 2 * maxvert; v++)
    vertmap[v] = fermmap[v];
  for (count = 0; count < nloop; count++)
    block[count] = fermres[count];
  ...
  for (v = 0; v < vcs.sizet; v++) {
    ...
    for (l = 0; l < vcs.valence[v]; l++) {
      ...
      if (lori &&
          (prtclbase1[np].spin != 4 || tolower(prtclbase1[np].hlp) == 't')) {
        if (v1 != v2) {
          vertinfo[v1].link[vertinfo[v1].vlnc++] = v2 + 1;
          set_add1(&(vertinfo[v1].ind), lori);
          if (tolower(prtclbase1[np].hlp) == 't')
            set_add1(&(vertinfo[v1].ind), lori - 1);
        }
        vertinfo[v1].weight += 2;
        if (set_in(lori, setmassindex))
          vertinfo[v1].weight++;
      }
    }
  }
  for (v = 0; v < vcs.sizet; v++)
    for (l = 0; l < vcs.valence[v]; l++) {
      ...
      if (prtclbase1[np].spin == 4 && tolower(prtclbase1[np].hlp) != 't') {
      ...
      } else if (prtclbase1[np].spin == 2 &&
                 PLR_PRTCL & vcs.vertlist[v][l].prop) {
        ...
        if (P > 0 /*&& vertmap[v]!=vertmap[vv]*/) {
          ...
          sprintf(txt,
                  "m%d.m%d "
                  "-N_p1p2_*(m%d.p%d*m%d.p%d+m%d.p%d*m%d.p%d-i*Helicity%d*eps("
                  "p%d,p%d,m%d,m%d))",
...
static int symbcalc(hlpcsptr ghst) {
  ...
  do {
    ...
    }
    {
      ...
      if (sgn != 1)
        block[0] = symb_imult(block[0], 1, sgn);
    }
  ...
  if (delImageryOne(t_fact))
    rnum = symb_mult(rnum, 1, symb_read("i"), 1);
  ...
  if (/*consLow*/ nin + nout <= 5)
    del_pp(&(rnum.expr.t->re), &mon, &del);
  else {
    mon = polyfactor(rnum.expr.t->re);
    del = 1;
  }

  if (rnum.expr.t->re == NULL)
    return 2;
...
static void writestatistic(unsigned noutmemtot, char *txt) {
  ...
  if (noutmemtot) {
    goto_xy(1, 13);
    print("%u Out of memory   ", noutmemtot);
  }
  goto_xy(17, 14);
  print("%4d", ndiagr);
  goto_xy(42, 14);
    ...
    for (ndiagr = 1; ndiagr <= naux; ndiagr++) {
      ...
      if (csd.status == 0) {
        ...
        if (csd.status == 1) {
          ...
          saveanaliticresult(rnum.expr.t->re, factn, factd, vcs,
                             nrecord + ndiagr - 1, ArcNum);
          newCodes = 1;
        }
        release_(&heap_beg);
        {
...
void calcWithFork(int np, int *diag, int fi) {
  ...
  for (k = 0; diag[k] >= 0; k++) {
    ...
    } else if (csd.status == 2)
      saveanaliticresult(NULL, NULL, NULL, vcs, diag[k], ArcNum);
    ...
    for (i = 1; i <= vardef->nvar; i++) {
      ...
      if (deg > 0) {
        if (first)
          ...
          strcat(txt, "*");
        if (deg > 1)
          sprintf(txt + strlen(txt), "pow(%s,%d)", vardef->vars[i - 1].name,
                  deg);
        else
          sprintf(txt + strlen(txt), "%s", vardef->vars[i - 1].name);
      }
    }
...
int writeVertexCode(char *pathToModels, int Model, int N, char **field_txt,
                    char *label) {
  int i;
  ...
  for (i = 0; i < N; i++) {
    ...
    field[i] = field_inp[i];
    if (field[i] == 0) {
      printf("can not find particle %s in the model\n", field_txt[i]);
      return i + 1;
    }
  }
  ...
  for (lgrgn1 = lgrgn; lgrgn1; lgrgn1 = lgrgn1->next) {
    if (field[0] == lgrgn1->fields[0] && field[1] == lgrgn1->fields[1] &&
        field[2] == lgrgn1->fields[2] && field[3] == lgrgn1->fields[3]) {
      rmptr rm = (rmptr)read_rmonom(lgrgn1->comcoef);
      int GGpower = rmonomGG(rm);
      int Ipower = rmonomI(rm);
      while (Ipower < 0)
        Ipower += 4;
      while (Ipower >= 4)
        Ipower -= 4;
      ...
      for (i = 0; i < vardef->nvar; i++)
        if (vardef->vars[i].name[2] == '.' &&
            vardef->vars[i].name[4] - '0' == N + 1)
          vardef->vars[i].name[4] = vardef->vars[i].name[1];
      ...
      for (int i = 0; i < 4; i++)
        aperm[lgrgn1->perm[i] - 1] = i;
      for (int i = 0; i < 4; i++)
        field_in_vert[i] = lgrgn1->fields[aperm[i]];

      for (i = 0; i < 4;
           i++) // set relation between field_in_vert and field_inp
      {
        for (int j = 0; j < 4; j++)
          if (field_in_vert[i] == field_inp[j]) {
            perm[i] = j + 1;
            field_inp[j] = -1;
            break;
          }
      ...
      char *pstr;
      if (!Ipower)
        pstr = parseOneVertex(lgrgn1->description, perm, field_in_vert);
      else {
        char *buff = malloc(strlen(lgrgn1->description) + 10);
        switch (Ipower) {
        case 1:
          sprintf(buff, "i*(%s)", lgrgn1->description);
          break;
        case 2:
          sprintf(buff, "-(%s)", lgrgn1->description);
          break;
        case 3:
          sprintf(buff, "-i*(%s)", lgrgn1->description);
          break;
      ...
      writeF(" double R=(");
      writepolyC(tcN.expr.p);
      writeF(")/(");
      writepolyC(tcD.expr.p);
      writeF(");\n");

      for (int i = 0; i < Nterm; i++) {
        writeF(" coeff_out[%d]=R*(", i);
        writepolyC(exp[i]);
        writeF(");\n");
      }
      writeF(" return 0;\n}\n");
      ...
      for (int i = 0; i < Nterm; i++) {
        if (i)
          fprintf(outFile, ",");
        if (strlen(txt[i]) > 0)
          fprintf(outFile, "\"%s\"", txt[i]);
        else
          fprintf(outFile, "\"1\"");
      }
      writeF("};\n");

      fprintf(outFile, "typedef struct  lVert\n{\n"
                       "    void * handle;\n"
                       "    void ** link;\n"
                       "    int init;\n"
                       "    int GGpower; int nVar; char **varNames; double "
                       "*varValues; int nTerms; char "
                       "**SymbVert;  int (*calcCoeff)(double*);\n"
                       "}  lVert;\n");

      fprintf(outFile, " extern lVert %s;\n", label);
      fprintf(outFile,
              " lVert %s={NULL,NULL,0,%d,%d, varName+1, V, %d, SymbVert, "
              "vertexCoeff};\n",
  ...
  fprintf(outFile,
          ...
          "    void ** link;\n"
          "    int init;\n"
          "    int GGpower; int nVar; char **varNames; double *varValues; int "
          "nTerms; char "
          "**SymbVert;  int (*calcCoeff)(double*);\n"
          "}  lVert;\n"
          "extern lVert %s;\n"
