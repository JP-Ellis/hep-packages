...
static int checkOut(int p, int v, int l) {
  ...
  if (vcs.vertlist[v][l].prop & IN_PRTCL)
    return -1;
  ...
  for (s = 0, i = 0; i < vcs.valence[vv]; i++)
    if (i != ll) {
...
void calcdenominators(vcsect vcs) {
  ...
  for (v = 1; v <= vcs.sizet; v++)
    for (l = 1; l <= vcs.valence[v - 1]; l++) {
      ...
      if (!(ln->moment < 0 || ((IN_PRTCL | OUT_PRTCL) & ln->prop) ||
            pseudop(ln->partcl))) {
        ...
        if (ttypepropag(v, l) && !tWidths)
          denom[denrno].width = 0;
        else
          denom[denrno].width =
              modelVarPos(prtclbase[ln->partcl - 1].imassidnt);
...
void denominatorStatistic(int nsub, int *n_swidth, int *n_twidth, int *n_0width,
    ...
    if (cr.status == 1) {
      if (cr.nsub_ == nsub) {
        ...
        for (i = 0; i < dendescript.tot_den; i++) {
          ...
          den_tmp = den_;
          while (den_tmp != NULL && (strcmp(denom[i].momStr, den_tmp->momStr) ||
          ...
          if (den_tmp == NULL) {
            ...
            if (denom[i].width) {
              if (den_tmp->stype)
                den_tmp->order_num = ++(*n_swidth);
              else
                den_tmp->order_num = ++(*n_twidth);
            } else
              den_tmp->order_num = ++(*n_0width);
          }
