...
int deletemodel(int n) {
  ...
  if (mess_y_n(56, 10, "Delete model?")) {
    ...
    if (access(to, R_OK) == 0) {
      sprintf(to, "rm -r %sProcesses/m%d", pathtouser, n);
      system(to);
    }

    for (; n < maxmodel; n++) {
      for (i = 0; i < 5; i++) {
        sprintf(from, "%smodels%c%s%d.mdl", pathtouser, f_slash, mdFls[i],
                n + 1);
        sprintf(to, "%smodels%c%s%d.mdl", pathtouser, f_slash, mdFls[i], n);
        rename(from, to);
      }
...
int makenewmodel(void) {
  ...
  for (;;) {
    ...
    while ((dp = readdir(dirPtr))) {
      char tail[100];
      int n;
      if (sscanf(dp->d_name, "vars%d.%s", &n, tail) == 2 &&
          strcmp(tail, "mdl") == 0) {
        for (i = 1; i < 4; i++) {
    ...
    if (nmdl == 0) {
      ...
      for (xmdl = 0; xmdl < len; xmdl++)
        if (nmdl == 1 || nmdl - 2 == xmdl) {
          int n;
          ...
          if (1 + xmdl + maxmodel > MAX_MODEL_NUM) {
            messanykey(10, 10, "Too many models");
            break;
          }
          for (i = 0; i < 5; i++) {
            int s;
    ...
    fillModelMenu();
    if (nmdl == 1) {
      messanykey(10, 16, "All models are added");
      put_text(&pscr);
      return 0;
    }
...
void clear_tmp(void) {
  ...
  char name[40];
  for (i = 1;; i++) {
    sprintf(name, ARCHIV_NAME, i);
    if (unlink(name))
      break;
  }
  for (i = 1;; i++) {
    sprintf(name, ARCHIV_NAME "2", i);
    if (unlink(name))
      break;
  }
  unlink(CATALOG_NAME);
