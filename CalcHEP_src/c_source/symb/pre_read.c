...
static void *bact(char ch, void *mm1, void *mm2) {
  ...
  case '.':
    ...
    m1->tp = m1->tp == vectortp && m2->tp == vectortp ? polytp : tenstp;
...
void *act_pre(char *name, int n, void **args) {
  ...
  if (n == 2)
    return bact(name[0], args[0], args[1]);
  if (n == 4 && !strcmp(name, "eps")) {
    if (!(args2[0] = bact('.', args[0], args[1])))
      return NULL;
    if (!(args2[1] = bact('.', args[2], args[3])))
      return NULL;
