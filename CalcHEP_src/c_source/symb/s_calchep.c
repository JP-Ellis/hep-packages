...
static int checkAuxDir(int nmod) {
  ...
  if (i != 5) {
    ...
    for (i = 0; i < 5; i++) {
      char command[100];
      sprintf(command, "cp models/%s%d.mdl results/aux/models/%s1.mdl",
              fList[i], nmod, fList[i]);
      system(command);
    }
  }
  if (!compDir) {
...
int main(int argc, char **argv) {
  ...
  void *pscr1 = NULL, *pscr2 = NULL, *pscr3 = NULL, *pscr4 = NULL,
       *pscr5 = NULL;
  ...
  int pid = 0;
  char LOCKtxt[] =
      "Directory 'results/' contains the .lock file created by n_calchep.\n"
      "To continue you may a)Close the n_calchep session, or b)Rename "
      "'results/',\n";
  ...
  outputDir = "results/";
  {
    ...
    if (err && blind == 0) {
      printf("Error:You have launched interactive session for version compiled "
             " without X11 "
             "library.\n");
      printf(" Presumably X11 development package is not installed in your "
             "computer.\n");
      printf(" In this case directory /usr/include/X11/ is empty.\n");
      printf("Options: a) Use blind session; b) Update Linux and recompile "
             "CalcHEP \n");
      printf("Name of needed package\n");
      printf("     libX11-devel      Fedora/Scientific "
             "Linux/CYGWIN/Darwin(MAC)\n");
...
label_10:           /*   Menu2(ModelMenu): */
  ...
  for (;;) {
    ...
    if (k1 == 0) {
      ...
      int err = 0;
      put_text(&pscr1);
      if (k1 != n_model || ldModelStatus == 0) {
        err = readModelFiles("./models", k1);
      }
      n_model = k1;
      if (err) {
        if (blind)
...
label_31:
  ...
  do {
    ...
    case 1:
      ...
      break;
    case 3: {
      FILE *f = fopen("results/list_prc.txt", "w");
  ...
  if (!squaring())
    goto label_30; /*  process is absent  */
    ...
    restart2:
      ...
      if (!nPROCSS || nin + nout < 4)
        ...
        while (nProc < nPROCSS2) {
          for (k = 0; k < nPROCSS; k++)
            if (pids[k] == 0 && K2 < nPROCSS2 && abort == 0) {
              int *kpipe = pipes + 2 * k;
              int i;
              while (K2 < nPROCSS2 && qd[K2][0] < 0) {
                K2++;
                nProc++;
              }
              if (K2 == nPROCSS2)
          ...
          for (procActive = 0, k = 0; k < nPROCSS; k++)
            if (pids[k]) {
              int one;
              if (waitpid(pids[k], NULL, WNOHANG) != 0) {
                ...
                totC += pow[k];
                close(pipes[2 * k]);
                pids[k] = 0; /*printf("End of %d\n",k);*/
              } else {
                procActive++;
                //              if(blind<=0 &&
                //              read(pipes[2*k],&one,sizeof(int))== sizeof(int))
                //              { totC++; pow[k]-=1;}
              }
            }
  ...
  for (;;) {
    ...
    if ((k5 == 1 || k5 == 2) && pid) {
      ...
      if (epid)
        ...
        messanykey(3, 10,
                   "This option is frozen while n_calchep runs or is compiled");
        continue;
      }
    }
    ...
    case 2:
      ...
      if (n_calchep_id) {
        if (nPROCSS) {
          ...
          if (access("./n_calchep", X_OK) == 0) {
            ...
            messanykey(15, 15, "n_calchep is not generated");
          chdir("../");
        } else {
          fflush(NULL);
          pid = fork();
          if (pid == 0) {
            if (chdir("results") == 0) {
              ...
              sprintf(
                  command,
                  "command=\"%s/sbin/make__n_calchep %d\"; xterm=`which "
                  "xterm`; if(test -n "
                  "\"$xterm\") then   $xterm -e $command; else $command; fi",
                  pathtocalchep, n_model);
              system(command);
              free(command);
              system("./n_calchep");
            }
      ...
      break;
    case 3:
      if (edittable(1, 1, &modelTab[4], 1, " ", 0)) {
        char fName[STRSIZ];
        sprintf(fName, "%smodels%c%s%d.mdl", pathtouser, f_slash, mdFls[4],
                n_model);
