...
static void getLeadingTerm(factor *f, int maxP, long *num, long *den) {
  ...
  if (f->len == 0 || maxP > f->len + f->powN - 1) {
    *num = 0;
    *den = 1;
  } else {
    *num = f->nc[f->len - 1];
...
void c_basis_coef(vampl *g, int pow, int nc, int *chains, long *num,
                  long *den) {
  int i, nv;
  ...
  for (i = 0; i < pow; i++) {
    ...
    for (maxP = 0, j = 0; j < nc; j++)
      if (chains[4 * (nc * i + j)] == 2)
        maxP++;
      else
        maxP += 2;
    ...
    if (NcInfLimit)
      getLeadingTerm(f, maxP, num + i, den + i);
    else
      fct_num_calc(f, 3, num + i, den + i);
...
int generateColorWeights(csdiagram *csdiagr, int cBasisPower, int nC,
                         int *cChains, long *cCoefN, long *cCoefD) {
  vcsect vcs;
...
static int recurGen(int k, int rev, int np, int nap, int *pos3, int *pos_3,
                    int *perm, int *used, int **w) {
  int nc = np + (nap - np) / 3;
  int pow = 0;
  int i, j0, j1, j2;
  ...
  } else if (k >= np) // construction of eps chains
  {
    ...
    for (j0 = 0; j0 < nap - 2; j0++)
      if (!used[j0]) {
        ...
        if (abs(pos_3[j0]) > prev[0]) {
          prev[1] = prev[2] = 0;
        }
        used[j0] = mark;
        for (j1 = j0 + 1; j1 < nap - 1; j1++)
          if (!used[j1] && (pos_3[j1] > 0 || used[j1 - 1]) &&
              abs(pos_3[j1]) >= prev[1]) {
            used[j1] = mark;
            if (abs(pos_3[j1]) > prev[1])
              prev[2] = 0;
            for (j2 = j1 + 1; j2 < nap; j2++)
              if (!used[j2] && (pos_3[j2] > 0 || used[j2 - 1]) &&
                  abs(pos_3[j2]) >= prev[2])
                if (abs(pos_3[j0]) != abs(pos_3[j1]) &&
                    abs(pos_3[j1]) != abs(pos_3[j2])) {
                  used[j2] = mark;
                  pow +=
                      recurGen(k + 1, rev, np, nap, pos3, pos_3, perm, used, w);
                  used[j2] = 0;
                }
            used[j1] = 0;
          }
...
int infCbases(int np, int *cweight, int *nc, int **chains) {
  ...
  if (nc_) {
    ...
    *chains = wrt;
    ...
    used = (int *)malloc((nc2 + 3 * nc3) * sizeof(int));
    for (i = 0; i < nc2 + 3 * nc3; i++)
      used[i] = 0;
    perm = (int *)malloc(nc2 * sizeof(int));
    if (rev)
      pow = recurGen(0, rev, n_3, n3, pos_3, pos3, perm, used, &wrt);
    else
      pow = recurGen(0, rev, n3, n_3, pos3, pos_3, perm, used, &wrt);
    if (pow_ < pow)
      printf("ERROR: pow_=%d pow=%d  - wrong memmory allocation\n", pow_, pow);
    *nc = nc_;
    free(perm);
    free(used);
  } else {
...
int main(int argv, char **argc) {
  ...
  for (i = 0; i < np; i++)
    if (1 != sscanf(argc[i + 1], "%d", cweight + i)) {
      printf("input mistyping\n");
      exit(2);
    }
  ...
  for (i = 0; i < pow; i++) {
    for (j = 0; j < nc; j++) {
      ...
      if (c[0] == 2)
        printf("( %2d %2d)", c[1], c[2]);
      if (abs(c[0]) == 3)
        printf("( %2d %2d %2d)", c[1], c[2], c[3]);
    }
    printf("\n");
  }
  return 0;
