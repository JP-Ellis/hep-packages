...
static int cerror(int n, char *s) { /*  - generate error message occur in color
                                       package - 08/01/90  */

  fprintf(stdout, "***** %s\n", s);
  if (n > CERRLEV1)
    fprintf(stderr, "Error  %u in  colorf.c \n", n), sortie(55);
  else if (n > CERRLEV2)
    sortie(56);
  return 0;
}
...
static void add_fct(factor *f, int sgn, int pow2, int powNm1, int powN,
                    int powNp1) {
  ...
  if (pow2 > f->pow2)
    sgn *= 1 << (pow2 - f->pow2); // pow2 matching
  else if (f->pow2 > pow2) {
  ...
  for (k = 1; k <= powNm1; k++)
    for (i = k; i >= 1; i--)
      s[i] -= s[i - 1];
  for (k = 1; k <= powNp1; k++)
    for (i = k + powNm1; i >= 1; i--)
      s[i] += s[i - 1];

  for (i = 0; i <= powNm1 + powNp1; i++)
    f->nc[i + powN] += s[i];
...
static int findv2(vtype vt, cgraph *cg,
                  int *n) { /* return True and number first vertex with type VT
                               in C-graph - 06/01/90 */
  int i = 0;
  for (i = 0; i < cg->mgl; i++)
    if (cg->vl[i].vt == vt) {
...
static int
findl2(int e1, int n, cgraph *cg,
       int *l) { /* - find number of vertex contane edge e1 - 08/01/90  */
                 /* 19/03/99 */
...
static void
wrcg(cgraph *cg) { /*  Write C-graph on standard device - 04/01/90 */
  int i;
  printf("Kr: {(%d*2^%d) N^%d (N-1)^%d  (N+1)^%d\n", cg->sgn, cg->pow2,
         cg->powN, cg->powNm1, cg->powNp1);
  for (i = 0; i < cg->mgl; i++)
    if (cg->vl[i].vt != zv) {
      printf("[%d/", i);
      switch (cg->vl[i].vt) {
      ...
      printf(" (%d,%d,%d)]", cg->vl[i].e[0], cg->vl[i].e[1], cg->vl[i].e[2]);
    } /* if */
  printf("}");

  i = testGraph(cg);
  if (i)
    printf(" error in vertex %d\n", i);
  else
    printf("ok\n");
}
//#endif
...
static int
getv2(cgraph *cg,
      int lbl) { /* return number of first free vertex in C-graph - 04/01/90 */
...
static cgraph *initcg(int nv, cvertex *vl) {
  ...
  cg->sgn = 1;
  cg->pow2 = cg->powN = cg->powNm1 = cg->powNp1 = 0;
  cg->en = 0;
  cg->gl = nv;
  cg->mgl = nv + 2;
  cg->next = NULL;
...
static cgraph *
addcg(cgraph **cg) { /*- add C-graph CG to weight structure - 16/10/99 -*/
...
static void
rem833_833(int n0, int n1,
           cgraph **c) { /* - remove gluon connected vertex n0 and n1 (see
                            fugure) from first C-graph - 08/01/90  */
  int l01, l02, l11, l12, n0_, n1_, i0_, i1_;
  cgraph *cg = *c;
#if (CDEBLEV > DEBLEV)
  printf("rem833_833(%d,%d)", n0, n1);
  wrcg(cg);
#endif
  l01 = cg->vl[n0].e[1];
  l02 = cg->vl[n0].e[2];
...
rem833_866(int n3, int n6,
  ...
  cg->vl[n3].vt = v633;
  cg->vl[n6].vt = V633;
  cg->vl[n3].e[0] = cg->vl[n62].e[l62];
  cg->vl[n6].e[0] = cg->vl[n61].e[l61];
  cg->vl[n3].e[1] = cg->vl[n31].e[l31];
  cg->vl[n6].e[1] = cg->vl[n32].e[l32];
  cg->vl[n3].e[2] = lg;
  cg->vl[n6].e[2] = lg;
...
static void rem866_Fabc(int n866, int n3g, cgraph **c) {
  ...
  N1 = getv2(cg, V633);
  cg->vl[N1].e[0] = cg->vl[n866].e[1];
  N2 = getv2(cg, v633);
  cg->vl[N2].e[0] = cg->vl[n866].e[2];
  ...
  cg->vl[n1].e[1] = cg->vl[N1].e[1];
  cg->vl[n2].e[2] = cg->vl[N2].e[1];
...
static int
istadpole2(int n,
           cgraph *cg) { /* return True if vertex n is teadpole - 08/01/90  */
  if (cg->vl[n].e[0] == cg->vl[n].e[1] || cg->vl[n].e[1] == cg->vl[n].e[2] ||
...
static void remtv(
    cgraph *pgl) { /* Remove transfered vertex from all C-graphs - 06/01/90 */
  ...
  while (findv2(t33, pgl, &n) || findv2(t88, pgl, &n) || findv2(t66, pgl, &n)) {
    ...
    if (n1 == n)
      ...
      pgl->vl[n1].e[l] = (vt0 == t88) ? pgl->vl[n].e[0] : pgl->vl[n].e[2];
    pgl->vl[n].vt = zv;
  }
#if (CDEBLEV > DEBLEV)
  wrcg(pgl);
...
factor *colorFactor(
    int nv,
    cvertex *vl) { /* - calculate color weight (two int n,d) - 08/01/90  */
  cgraph *cg;
  ...
  while (cg) {
    ...
    while (cg->gl && cg->sgn) {
      ...
      if (findv2(v333, cg, &n0)) {
        if (rem333(n0, &(cg)))
          continue;
      }

      wrcg(cg);
      cerror(251, "No rules to expend diagram");
    }
    pgl = cg;
    if (pgl->sgn)
      add_fct(f, pgl->sgn, pgl->pow2, pgl->powNm1, pgl->powN, pgl->powNp1);
    cg = cg->next;
    free(pgl->vl);
    free(pgl);
  }
...
static void rednd(long *n, long *d,
                  int b) { /* - reduce N and D with respect to B - 08/01/90  */
  if (b != 1)
...
void fct_print(factor *fct, char *s) {
  ...
  for (i = 0; i < fct->len; i++)
    if (fct->nc[i]) {
      ...
      if (i)
        sprintf(s + strlen(s), "N^%d", i);
    }
  sprintf(s + strlen(s), ")");

  if (fct->powN)
    sprintf(s + strlen(s), "*(N^%d)", fct->powN);
  if (fct->pow2 < 0)
    sprintf(s + strlen(s), "*2^%d", fct->pow2);
}
...
vtype typev(vert0 v, int valence) {
  ...
  for (n = 0; n < valence; n++)
    switch (prtclbase[v[n].partcl - 1].cdim) {
    case 8:
      color *= 2;
      break;
    case 3:
...
int t2k2(vcsect *g, int *nv, cvertex *vl) {
  ...
  *nv = 0;
      ...
      for (k = 0, ne = 0; ne < g->valence[i]; ne++) {
        ...
        if (dim != 1) {
          ...
          if (!l) {
            l = ++en;
            maptar[i][ne] = l;
            maptar[g->vertlist[i][ne].link.vno][g->vertlist[i][ne].link.edno] =
                l;
          }
  ...
  int j, n, sgn = 1;
  for (n = 0; n < g->sizel; n++)
    if (vl[n].vt == vFabc || vl[n].vt == v333 || vl[n].vt == V333)
      for (i = 0; i < g->valence[n] - 1; i++)
        if (prtclbase1[g->vertlist[n][i].partcl].cdim != 1)
          for (j = i + 1; j < g->valence[n]; j++)
            if (prtclbase1[g->vertlist[n][j].partcl].cdim != 1) {
              int ni = g->vertlist[n][i].link.vno,
                  nj = g->vertlist[n][j].link.vno;
              if (ni == nj &&
                  g->vertlist[n][i].link.edno > g->vertlist[n][j].link.edno)
                sgn *= -1;
            }
  return sgn;
}
