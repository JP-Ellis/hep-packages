...
void vertinlgr(int *vert_, int nvert, int *subst, algvertptr *lgr) {
  ...
  for (i = 0; i < 4; i++)
    vert[i] = vert_[i] ? prtclbase[vert_[i] - 1].anti : 0;
  ...
  for (q = j_lgrarray[nvert - 1];
       q != NULL &&
       (q->lgrnptr->fields[0] != vert[0] || q->lgrnptr->fields[1] != vert[1] ||
        q->lgrnptr->fields[2] != vert[2] || q->lgrnptr->fields[3] != vert[3]);
       q = q->next)
    ;
  ...
  for (i = 0; i < vcs_stat.sizet; i++) /*  mark subgraph vertex with '1'  */
    for (j = 0; j < vcs_stat.valence[i]; j++) {
      ...
      if (gaugep(p) && i < i1 &&
          !(PLR_PRTCL &
            vcs.vertlist[i][j]
                .prop)) { /*  check the  Ghost particles existence in vertex  */
...
static void skipup(int *v, hlpcsptr lpl) {
  ...
  j = 1;
  while (sklt[*v - 1][j - 1] != vertmrk[*v - 1] + 1)
    ++(j);
      ...
      if (sklt[i - 1][j - 1] == 1) /*   if (i,j) is't element of OCTOV .... */
      {
        ...
        if (i < i1) {
          ...
          *indpl = lpl;
          ...
          while (vertmrk[v1 - 1] > vertmrk[v2 - 1])
            skipup(&v2, lpl);
          while (vertmrk[v2 - 1] > vertmrk[v1 - 1])
            skipup(&v1, lpl);
          while (v1 != v2) {
            skipup(&v1, lpl);
            skipup(&v2, lpl);
...
static int find1(int *i, int *j, hlpcsptr tmp) {
  while (1) {
    if (tmp->hlpcs[*i - 1][*j - 1] == 1)
      return 1;
    else if (++(*j) > vcs_stat.valence[*i - 1]) {
      if (++(*i) > vcs_stat.sizet)
        return 0;
      *j = 1;
    }
    ...
    for (j = 0; j < vcs_stat.valence[i]; j++)
      if (vcs_stat.vertlist[i][j].nextvert.vno != nullvert) {
        ...
        if (i < i1) {
          ...
          mark = 0;
          if (gaugep(np) && !zeromass(np) &&
              prtclInVert(np + sbosonmark, i + 1) &&
              prtclInVert(prtclbase[np + sbosonmark - 1].anti, i1 + 1))
            mark += S_PERMIT;
          ...
          if ((prtclbase[np - 1].spin == 2) /*&& (prtclbase[np-1].cdim !=1)*/ &&
            ...
            if (prtclInVert(prtclbase[np + xbosonmark - 1].anti, i1 + 1) &&
                prtclInVert(np + xbosonmark, i + 1))
              mark += X_PERMIT;

            if (prtclInVert(prtclbase[np + ybosonmark - 1].anti, i1 + 1) &&
                prtclInVert(np + ybosonmark, i + 1))
              mark += Y_PERMIT;
          }
          first_mark[i][j] = mark;
...
static void preliminaryTest(hlpcsptr *alll) {
  ...
  while (c != NULL) {
    ...
    for (i = 0; i < vcs_stat.sizet && !del; i++) {
      for (j = 0; j < vcs_stat.valence[i]; j++)
        if (c->hlpcs[i][j] >= undefprtclmark)
          goto label_1;
      for (j = 0; j < vcs_stat.valence[i]; j++)
        vert[j] = vcs_stat.vertlist[i][j].partcl + c->hlpcs[i][j];
      for (j = vcs_stat.valence[i]; j < MAXVALENCE; j++)
        vert[j] = 0;
        ...
        case 2:
          ...
          if (/* ( prtclbase1[np].cdim != 1 ) && */
            ...
            for (tmp = alll; tmp; tmp = tmp->next)
              if (tmp->hlpcs[i][j] == xbosonmark ||
                  tmp->hlpcs[i][j] == ybosonmark) {
                lorCount++;
                break;
              }
          ind1 = lorCount;
    ...
    for (i = 0; i < vcs_stat.sizet; i++)
      for (j = 0; j < vcs_stat.valence[i]; j++) {
        ...
        if (i < i1) {
          ...
          if (prtclbase[np].spin == 2 && tmp->hlpcs[i][j] == vbosonmark)
            tmp->sgn *= -1;
        }
      }
  }
...
void ghostsForAmplitude(vampl *va, hlpcsptr *ghosts) {
  ...
  for (i = 0; i < vcs_stat.sizel; i++) {
    ...
    for (j = 0; j < MAXVALENCE; j++)
      vcs_stat.vertlist[i][j] = va->vertlist[i][j];
  }
