...
static int SMid[18] = {1,  2,  3,  4,  5,  6,  11, 12, 13,
                       14, 15, 16, 21, 22, 23, 24, 81, 83};
static double SMwidth[18] = {0, 0, 0,       1.03E-12, 4.5E-13,  1.59,
                             0, 0, 3.E-19,  0,        2.26E-12, 0,
                             0, 0, 2.49444, 2.08895,  0,        0};
static int SLHA = 0;
...
static eventfile_info *secondCopy(eventfile_info *new, eventfile_info *list) {
  ...
  for (; list; list = list->next)
    if (new->Nout == list->Nout) {
      for (i = 0; i < new->Nin + new->Nout; i++)
        if (new->PIDs[i] != list->PIDs[i])
          break;
      if (i >= new->Nin + new->Nout)
        return list;
    }
  return NULL;
}
...
int scandir_(char *dirname, int len) {
    ...
    if (sscanf(dp->d_name, "events_%d", &N) == 1) {
        ...
        if (All) {
          for (i = 0; i < 2; i++)
            if (All->inMom[i] != Finfo->inMom[i] ||
              ...
              fprintf(stderr,
                      "Error: different incoming states in\n"
                      "%s \n and  %s\n",
      ...
      if (Finfo->Nin == 1) {
        ...
        if (!D) {
          ...
          D->next = Decays;
          D->ID = Finfo->PIDs[0];
          if (SLHA && slhaDecayExists(abs(D->ID)) > 0)
            D->totWidth = slhaWidth(abs(D->ID));
          else {
            int i;
            for (i = 0; i < 18; i++)
              if (abs(D->ID) == SMid[i]) {
                D->totWidth = SMwidth[i];
                break;
              }
            if (i == 18) {
              printf("Total width of %d is not found\n", D->ID);
              D->totWidth = 0;
            }
          }
  ...
  if (All) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (All->inPID[i] != All->PIDs[i] && (fabs(All->inPID[i]) == 2212))
        m = 0.98;
      else
        m = All->pmass[i];
      p = All->inMom[i];
...
/* finds boost angle to accelerate p to p_; m - particle mass */
{
  ...
  return (p_ - p) *
         (p + p_ + (E * E_ + p * p_ + p * p + p_ * p_ + m * m) / (E_ + E)) /
         (p_ + E_) / (E + p);
}
...
static int momImproveS(int NP, double *p, int *stat, int *mothers, double m1,
                       int i1) {
  int i, k, err;
  ...
  if (mothers[2 * i1] < 3 || mothers[2 * i1 + 1] < 3)
    for (i = 0; i < NP; i++)
      inv[i] = 1;
  else {
    for (i = 0; i < NP; i++)
      if (mothers[2 * i] == mothers[2 * i1])
        inv[i] = 1;
      else
        inv[i] = 0;
    for (i = 0; i < NP; i++)
...
static int upEvent1(void) {
  ...
  if (readEvent(Finfo, &Nmom, mom, ch_clr, &Q, &alphaQCD, &w)) {
    E_.NUP = 0;
    return 0;
  }
  ...
  for (I = 2; I < N; I++) {
    for (j = 0; j < 2; j++)
      E_.MOTHUP[I][j] = j + 1;
    for (j = 0; j < 3; j++)
      E_.PUP[I][j] = mom[2 + 3 * (I - 2) + j];
    E_.ISTUP[I] = 1;
  }

  for (I = 0; I < N; I++)
    E_.PUP[I][3] = Ep(E_.PUP[I]);
    ...
    if (E_.ISTUP[I] == 1) {
      ...
      for (D = Decays; D; D = D->next)
        if (D->ID == E_.IDUP[I]) {
          ...
          for (i = 0; i < E_.NUP; i++)
            for (j = 0; j < 5; j++)
              MYPUP[i][j] = E_.PUP[i][j];

          for (err = 1; err;) {
            // printf("do\n");
            M_ = E_.PUP[I][4] *
                 (E_.PUP[I][4] +
                  D->totWidth * tan(M_PI * (0.5 - drand48()) * 0.99));
            if (M_ <= 0)
              continue;
            M_ = sqrt(M_);
            ...
            // printf("decay\n");
            if (E_.NUP == 3)
              err = momImproveS21(MYPUP[0], M_);
            else
              err = momImproveS(E_.NUP - 2, MYPUP[2], E_.ISTUP + 2,
                                &(E_.MOTHUP[2][0]), M_, I - 2);
            if (err)
              continue;

            for (k = 1; k <= Finfo->Nout; k++) {
              for (j = 0; j < 3; j++)
                E_.PUP[shift + k][j] = mom[3 * (k - 1) + j];
          ...
          for (i = 0; i < 2; i++)
            if (buff[i]) {
              ...
              {
                for (k = 2; k < 2 * (1 + Finfo->Nout); k++)
                  if (abs(buff[k]) == abs(buff[i]))
          ...
          for (i = 1; i <= Finfo->Nout; i++)
            for (j = 0; j < 2; j++)
              E_.ICOLUP[i + shift][j] = buff[2 * i + j];
...
void eventstat_(double *cs, int *nevents) {
  ...
  for (Finfo = All; Finfo; Finfo = Finfo->next) {
    ...
    for (i = 0; i < Finfo->Nout; i++)
      cs *= BrEff(Finfo->PIDs[i + 2]);
    Lu = Finfo->nEvents / cs;
    if (LuMin < 0)
      LuMin = Lu;
    else {
...
void writeInfo(void) {
  ...
  if (SLHA)
    printf("SLHA decay file is found\n");
  else
    printf("SLHA decay file is not found\n");
  printf("List of reactions:\n");
  if (!All)
    printf("            Empty\n");
  for (Finfo = All; Finfo; Finfo = Finfo->next) {
    printf(" Process  : %s %s -> %s", Finfo->pName[0], Finfo->pName[1],
           Finfo->pName[2]);
    for (i = 4; i <= Finfo->Nin + Finfo->Nout; i++)
      printf(",%s", Finfo->pName[i - 1]);
    printf("    File: %s\n", Finfo->fileName);
  ...
  if (!Decays)
    ...
    for (D = Decays; D; D = D->next) {
      ...
      for (Finfo = D->List; Finfo; Finfo = Finfo->next) {
        printf("           width=%.2E[GeV] N=%ld  Channel: %s -> %s", Finfo->cs,
               Finfo->nEvents, Finfo->pName[0], Finfo->pName[1]);
        for (i = 3; i <= Finfo->Nin + Finfo->Nout; i++)
          printf(",%s", Finfo->pName[i - 1]);
        printf("  File: %s\n", Finfo->fileName);
      }
    }
...
void printProcInfo(FILE *f) {
  ...
  for (k = 1; all; all = all->next, k++)
    if (all->Nin == 2) {
      int i;
      fprintf(f, "              <subprocess cutset_id=\"%d\" >\n", k);
      fprintf(f, "                <notation> ");
      fprintf(f, "%s(%d),%s(%d) -> ", all->pName[0], all->PIDs[0],
              all->pName[1], all->PIDs[1]);
      ...
      fprintf(f, " </notation>\n");
      fprintf(f,
              "                <crossSection unit=\"pb\"> %E </crossSection>\n",
              all->cs);
