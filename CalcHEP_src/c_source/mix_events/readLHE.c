  ...
  for (;;) {
    ...
    if (strcmp(buff, "<init>") == 0) {
      n = fscanf(eF, " %d %d %lf %lf %d %d %d %d %d %d %lf %lf %lf %d",
                 R_.IDBMUP, R_.IDBMUP + 1, R_.EBMUP, R_.EBMUP + 1, R_.PDFGUP,
                 R_.PDFGUP + 1, R_.PDFSUP, R_.PDFSUP + 1, &R_.IDWTUP, &R_.NPRUP,
                 R_.XSECUP, R_.XERRUP, R_.XMAXUP, R_.LPRUP);
      if (n == 14)
        return 0;
      else
        return 2;
...
int readevent_(void) {
  ...
  for (;;) {
    ...
    if (strcmp(buff, "<event>") == 0) {
      int I, J;
      n = fscanf(eF, "%d %d %lf %lf %lf %lf\n", &E_.NUP, &E_.IDPRUP, &E_.XWGTUP,
                 &E_.SCALUP, &E_.AQEDUP, &E_.AQCDUP);
      if (n != 6)
        return 1;
      for (I = 0; I < E_.NUP; I++) {
        n += fscanf(eF, " %8d %4d", E_.IDUP + I, E_.ISTUP + I);
        for (J = 0; J < 2; J++)
          n += fscanf(eF, " %d", E_.MOTHUP[I] + J);
        for (J = 0; J < 2; J++)
          n += fscanf(eF, " %d", E_.ICOLUP[I] + J);
        for (J = 0; J < 5; J++)
          n += fscanf(eF, " %lf", E_.PUP[I] + J);
