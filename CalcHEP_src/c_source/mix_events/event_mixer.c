...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    printf(
        "%s needs arguments: Luminosity[1/fb]  nEvents event_directories ...\n",
        argv[0]);
    exit(1);
  }

  if (sscanf(argv[1], "%lf", &Lumi) != 1) {
    printf("Can not read first argument. It should be luminosity in [1/fb] "
           "units.\n");
    exit(1);
  }
  ...
  for (II = 3; II < argc; II++)
    scandir_(argv[II], strlen(argv[II]));
  if (Lumi <= 0 && NEV <= 0) {
    writeInfo();
    exit(0);
  }
  ...
  if (Lumi > 0 && (NEV <= 0 || NEV > 1000 * Lumi * cs))
    NEV = 1000 * Lumi * cs;
  ...
  if (lun1) {
    ...
    fprintf(lun1, "    xsi:schemaLocation=\"http://mcdb.cern.ch/hepml/0.2/ "
                  "http://mcdb.cern.ch/hepml/0.2/hepml.xsd\">\n");
    fprintf(lun1, "    <files>\n");
    fprintf(lun1, "        <file>\n");
    ...
    //  fprintf(lun1,"            <checksum
    //  type=\"md5\">d7722af9e2886fa465e3f7b786c3e6e6</checksum>\n");
    ...
    fprintf(lun1, "            "
                  "<homepage>http://theory.sinp.msu.ru/~pukhov/calchep.html</"
                  "homepage>\n");
    fprintf(lun1, "            <description>\n");
    ...
    if (SLHA) {
      ...
      char name[50];
      double complex val;
      for (i = 1; allBlocks(i, 0, name, NULL, NULL, NULL); i++)
        if (strcmp(name, "MODELPARAMETERS") == 0) {
    ...
    //  fprintf(lun1,"                    <Lambda
    //  unit=\"GeV\">0.226200</Lambda>\n"); fprintf(lun1,"
    //  <NFlavours>6</NFlavours>\n"); fprintf(lun1,"
    //  <NLoopsAlphaS>2</NLoopsAlphaS>\n"); fprintf(lun1," </QCDCoupling>\n");
    ...
    if (SLHA) {
...

      {
        ...
        double complex val;
        int first = 1;
        for (i = 1; allBlocks(i, 0, blkName, NULL, NULL, NULL); i++)
          if (strcmp(blkName, "MASS") == 0) {
            for (j = 1; allBlocks(i, j, blkName, &len, key, &val); j++)
              if (len == 1) {
                // for(k=0;k<16;k++) if(key[0]==SM[k]) break;
                // if(k==16)
                //{
                if (first) {
                  fprintf(lun1, "BLOCK MASS\n");
                  first = 0;
                }
    ...
    fprintf(lun1, " %5d %5d %18.11E %18.11E %5d %5d %5d %5d %5d %5d\n",
            R_.IDBMUP[0], R_.IDBMUP[1], R_.EBMUP[0], R_.EBMUP[1], R_.PDFGUP[0],
            R_.PDFGUP[1], R_.PDFSUP[0], R_.PDFSUP[1], R_.IDWTUP, R_.NPRUP);

    fprintf(lun1, " %18.11E %18.11E %18.11E %3d\n", cs, R_.XERRUP[0],
            R_.XMAXUP[0], R_.LPRUP[0]);
    fprintf(lun1, "</init>\n");
  }
  ...
  for (N = 1; N <= NEV; N++) {
    while (upevnt_())
      ;
    if (E_.NUP == 0) {
      printf("Only %d events are generated\n", N - 1);
      break;
    }
    ...
    if (lun1) {
      ...
      fprintf(lun1, "%2d %4d %15.7E %15.7E %15.7E %15.7E\n", E_.NUP, E_.IDPRUP,
              E_.XWGTUP * cs, E_.SCALUP, E_.AQEDUP, E_.AQCDUP);

      for (II = 0; II < E_.NUP; II++) {
        double s;
        fprintf(lun1, " %8d %4d", E_.IDUP[II], E_.ISTUP[II]);
        for (J = 0; J < 2; J++)
          fprintf(lun1, " %4d", E_.MOTHUP[II][J]);
        for (J = 0; J < 2; J++)
          fprintf(lun1, " %4d", E_.ICOLUP[II][J]);
        for (J = 0; J < 4; J++)
          fprintf(lun1, " %18.11E", E_.PUP[II][J]);
        if (E_.ISTUP[II] == 2) {
          s = E_.PUP[II][3] * E_.PUP[II][3] - E_.PUP[II][0] * E_.PUP[II][0] -
              E_.PUP[II][1] * E_.PUP[II][1] - E_.PUP[II][2] * E_.PUP[II][2];
          if (s < 0)
            s = -sqrt(-s);
          else
            s = sqrt(s);
