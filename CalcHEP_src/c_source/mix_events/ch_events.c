...
void ch2pythColors(int *CC, int nin, int nout, int *ch_color, int *pth_color) {
  ...
  for (i = 0;; i++, (*CC) += 4)
    ...
    case 2: {
      ...
      int n = *CC;
      if (k1 < nin)
        addCol(n, 0, pth_color + 2 * k1);
      else
        addCol(n, 1, pth_color + 2 * k1);
      if (k2 < nin)
        addCol(n, 1, pth_color + 2 * k2);
      else
        addCol(n, 0, pth_color + 2 * k2);
    } break;
    ...
    case 3: {
      ...
      int n = *CC;
      if (k1 < nin)
        addCol(n + 1, 0, pth_color + 2 * k1);
      else
        addCol(n + 1, 1, pth_color + 2 * k1);
      if (k2 < nin)
        addCol(n + 2, 0, pth_color + 2 * k2);
      else
        addCol(n + 2, 1, pth_color + 2 * k2);
      if (k3 < nin)
        addCol(n + 3, 0, pth_color + 2 * k3);
      else
        addCol(n + 3, 1, pth_color + 2 * k3);
    } break;
...
static void Rot3D(double cn, double fi_1, double psi, int ntot, double *pvect) {
  ...
  double EPS[3][3] = {{0, -E[2], E[1]}, {E[2], 0, -E[0]}, {-E[1], E[0], 0}};
...
eventfile_info *initEventFile(char *fname) {
  ...
  F = fopen(fname, "r");
  if (F == NULL) {
    printf("Can not open event file %s\n", fname);
    exit(1);
  }
  ...
  for (fscanf(F, "%s", buff); !feof(F);) {
    if (strcmp(buff, "#Type") == 0) {
      ...
      if (Finfo->Nin == 2) {
        for (i = 0; i < 2; i++) {
          fscanf(F, " P%d_3=", &n);
          fscanf(F, "%lf", Finfo->inMom + n - 1);
        }
        for (i = 0; i < 2; i++) {
          fscanf(F, " StrFun%d=", &n);
          if (n > 0 && n <= 2)
            if (fscanf(F, "\"%[^\"]\" %d", Finfo->pdf[n - 1],
                       Finfo->inPID + n - 1) != 2)
              Finfo->inPID[n - 1] = 0;
        }
      }
      ...
      fscanf(F, "%*[^\n]%*c");
      for (i = 0; i < Finfo->Nin; i++)
        if (Finfo->inPID[i] == 0)
          Finfo->inPID[i] = Finfo->PIDs[i];
...
int readEvent(eventfile_info *Finfo, int *Nmom, double *mom, int *color,
              double *Qf, double *alphaQCD, double *w) {
  int n = 0;

  if (Finfo->F == NULL) {
    if (nFiles >= maxFileOpen)
      closeLast();
  ...
  for (;;) {
    if (Finfo->cEvent > Finfo->nEvents) {
      ...
      if (Finfo->Nin == 2) {
        ...
        fprintf(
            stderr,
            "Warning: File %s : no more events. Reread from the beginning. \n",
            Finfo->fileName);
        Finfo->firstRd = 0;
      }
    }
    ...
    for (n = 0; 1 == fscanf(Finfo->F, " %lf", mom + n); n++)
      ;
    *Nmom = n;
    if (2 != fscanf(Finfo->F, "| %lf %lf", Qf, alphaQCD))
      return 2;
    if (Finfo->Nin == 1 && Finfo->firstRd == 0)
      Rot3D(2 * (drand48() - 0.5), 2 * M_PI * drand48(), M_PI * drand48(),
            Finfo->Nout, mom);
    for (n = 0; 4 == fscanf(Finfo->F, " (%d %d %d %d)", color + n,
                            color + n + 1, color + n + 2, color + n + 3);
         n += 4)
      ;
    color[n] = 0;
    return 0;
