...
int main(int argc, char **argv) {
  ...
  double wCoeff;
  if (argc < 2) {
    printf("%s needs one argument: name of event file in CalcHEP format.\n",
           argv[0]);
    exit(1);
  }
  ...
  fprintf(F, "    xsi:schemaLocation=\"http://mcdb.cern.ch/hepml/0.2/ "
  ...
  if (Finfo->Nin == 2)
    fprintf(F, "            <crossSection unit=\"pb\">%.3E</crossSection>\n",
            Finfo->cs);
  else
    fprintf(F, "            <width unit=\"GeV\">%.3E</width>\n", Finfo->cs);
  ...
  fprintf(F, " %5d %5d %18.11E %18.11E %5d %5d %5d %5d %5d %5d\n",
          Finfo->inPID[0], Finfo->inPID[1], fabs(Finfo->inMom[0]),
          fabs(Finfo->inMom[1]), PDFGUP, PDFGUP, PDFSUP, PDFSUP, IDWTUP, NPRUP);
  ...
  for (N = 0; N < Finfo->nEvents; N++) {
    ...
    if (readEvent(Finfo, &Nmom, mom, clr, &Qf, &alphaQCD, &w))
      break;
    fprintf(F, "<event>\n");
    ...
    for (II = 0; II < Finfo->Nin + Finfo->Nout; II++) {
      ...
      for (J = 0; J < 2; J++)
        fprintf(F, " %4d", icol[2 * II + J]);
      if (II < Finfo->Nin) {
        if (Finfo->Nin == 2)
          P[2] = mom[II];
      } else
        for (J = 0; J < 3; J++)
          P[J] = mom[3 * II - 2 * Finfo->Nin + J];
      for (J = 0; J < 3; J++)
        P[3] += P[J] * P[J];
      P[3] = sqrt(P[3] + Finfo->pmass[II] * Finfo->pmass[II]);
      for (J = 0; J < 4; J++)
        fprintf(F, " %18.11E", P[J]);
      fprintf(F, " %18.11E", Finfo->pmass[II]);
      fprintf(F, " %11.4E %4.1f\n", 0., 9.);
#ifdef NotDone
      fprintf(F, " %8d %4d", E_.IDUP[II], E_.ISTUP[II]);
      for (J = 0; J < 2; J++)
        fprintf(F, " %4d", E_.MOTHUP[II][J]);
      for (J = 0; J < 2; J++)
        fprintf(F, " %4d", E_.ICOLUP[II][J]);
      for (J = 0; J < 4; J++)
        fprintf(F, " %18.11E", E_.PUP[II][J]);
      if (E_.ISTUP[II] == 2) {
