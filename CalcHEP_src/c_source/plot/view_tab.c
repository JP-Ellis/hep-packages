...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    printf("This function needs one argument: name of file which contains plot "
           "table\n");
    return 1;
  }
  F = fopen(argv[1], "r");
  ...
  while (n >= 0 && pathtocalchep[n] != f_slash)
    n--;
  pathtocalchep[n - 3] = 0;
  ...
  for (;;) {
    ...
    if (1 == sscanf(buff + 1, "%s", word)) {
      if (strcmp(word, "type") == 0)
        sscanf(buff + 1, "%*s %d", &ptype);
      else if (strcmp(word, "xMin") == 0)
        sscanf(buff + 1, "%*s %lf", &xMin);
      else if (strcmp(word, "xMax") == 0)
        sscanf(buff + 1, "%*s %lf", &xMax);
      else if (strcmp(word, "xScale") == 0)
        sscanf(buff + 1, "%*s %d", &xScale);
      else if (strcmp(word, "yMin") == 0)
        sscanf(buff + 1, "%*s %lf", &yMin);
      else if (strcmp(word, "yMax") == 0)
        sscanf(buff + 1, "%*s %lf", &yMax);
      else if (strcmp(word, "xDim") == 0) {
        n = 0;
        char *ch = strtok(buff, " ");
        ch = strtok(NULL, " ");
        while (ch) {
          xDim = realloc(xDim, (n + 1) * sizeof(int));
          sscanf(ch, "%d", xDim + n);
          ch = strtok(NULL, " ");
          if (xDim[n] > xDimMax)
            xDimMax = xDim[n];
      ...
      } else if (strcmp(word, "yDim") == 0)
        sscanf(buff + 1, "%*s %d", &yDim);
  ...
  if (yDim == 1) {
    ...
    for (ch = yName; ch = strstr(ch, "{c}"); ch += 3)
      N++;
    for (ch = yName; ch = strstr(ch, "{h}"); ch += 3)
      N++;
    if (N) {
      ...
      clearTypeAhead();
      plot_Nar(argv[1], procName, xMin, xMax, xName, xScale, N, xDim, fn, dn,
               Y);
      finish();
      return 0;
    }
  ...
  if (ptype) {
    ...
    for (i = 1; i < xDim[0] * yDim; i++)
      fscanf(F, "%lf  %lf", f + i, df + i);
    for (i = 1; i < xDim[0] * yDim; i++)
  ...
  clearTypeAhead();
  if (ptype == 2)
    plot_2D(xMin, xMax, xDim[0], yMin, yMax, yDim, f, df, procName, xName,
            yName);
  else {
