...
static void axisDesign(double xmin, double xmax, int islog, double *xfirst,
                       double *step, int *nsub) {
  double dx, dx0, dx100;
  int n, n0, n1;
  char xmintxt[50];
...
static void gminmax(double *f, double *df, int dim, double *ymin,
                    double *ymax) {
  int i;
  if (dim == 0)
    return;
...
static double xPhys(void) {
  if (logX)
    return pow(10., (mouse_info.x - X1) / xscale) * grafminmax.xmin;
  else
...
static double yPhys(void) {
  if (logY)
    ...
    return -(mouse_info.y - Y1) / yscale + grafminmax.ymin;
}
...
static int colList[] = {Black,     Blue,     Red,          Yellow,
                        Magenta,   Green,    Cyan,         Brown,
                        LightGray, DarkGray, LightBlue,    LightGreen,
                        LightCyan, LightRed, LightMagenta, White};

static char *colListTxt[] = {
...
static void gaxes(char *upstr, char *xstr, int N, char **Y) {
  ...
  if (logY && (ymin <= 0.0 || log10(ymax / ymin) < 1))
    logY = 0;
...
static void plot_spline(double xMin, double xMax, int dim, double *f) {
  ...
  if (logX)
    for (i = 0; i < dim; i++)
      xgrid[i] = log(xMin * pow(xMax / xMin, (i + 0.5) / (double)dim));
  else
    for (i = 0; i < dim; i++)
      xgrid[i] = xMin + (i + 0.5) * step;
  ...
  for (i = 1; i < sdim; i++) {
    ...
    if (logY) {
      ...
      if (!isfinite(y))
        y = exp(polint1(x1, dim, xgrid, fLn));
      if (!isfinite(y))
        y = polint3(x1, dim, xgrid, f);

      yy = exp(polint3(x2, dim, xgrid, fLn));
      if (!isfinite(yy))
        yy = exp(polint1(x2, dim, xgrid, fLn));
      if (!isfinite(yy))
...
static void plot_hist(double xMin, double xMax, int dim, double *f,
                      double *df) {
  double x, y, yp, ym;
  int i;
  ...
  for (i = 0; i < dim; i++) {
    ...
    if (y < ymax && y > ymin) {
      if (logX) {
        tg_line((int)dscx(xMin * pow(xMax / xMin, (double)i / dim)), (int)y,
                (int)dscx(xMin * pow(xMax / xMin, (double)(i + 1) / dim)),
                (int)y);
      ...
      if (yp < ym)
        tg_line((int)x, (int)ym, (int)x, (int)yp);
    }
  }
}

static int writetable1(char *filename, double xMin, double xMax, double yMin,
                       double yMax, char *x_str, char *upstr, int N, int *Dim,
                       double **f, double **df, char **Y) {
  FILE *outfile;
  int i, k;
  ...
  fprintf(outfile, "#xDim ");
  for (i = 0; i < N; i++)
    fprintf(outfile, " %d", Dim[i]);
  fprintf(outfile, "\n");
...
static void writeGNU(char *fname, double xMin, double xMax, double yMin,
                     double yMax, char *x_str, char *upstr, int N, int *Dim,
                     char *type, char **Y) {
  ...
  if (logX) {
    ...
    for (k = 0; k < N; k++)
      fprintf(outfile, " X%d(n)=xMin*(xMax/xMin)**((n+0.5)/%d)\n", k, Dim[k]);
  } else
    for (k = 0; k < N; k++)
      fprintf(outfile, " X%d(n)=xMin+(n+0.5)/%d*(xMax-xMin)\n", k, Dim[k]);
  ...
  for (i = 1, k = 0; k < N; k++, i++) {
    //      printf("type[%d]=%c\n",k,type[k]);
    if (type[k] == 'h') {
      fprintf(outfile, "'%s' using (X%d($0)):%d:%d  w error ", fname, k, i,
              i + 1);
      if (N > 1)
        fprintf(outfile, " ti '%s'", Y[k]);
      else
        fprintf(outfile, " noti");
      i++;
    } else {
      fprintf(outfile, "'%s' using (X%d($0)):%d w l ", fname, k, i);
      if (N > 1)
        fprintf(outfile, " ti '%s'", Y[k]);
      else
        fprintf(outfile, " noti");
      if (type[k] == 's')
        fprintf(outfile, " smooth csplines");
    }
    if (k < N - 1)
      fprintf(outfile, ", ");
    else
...
static void writePAW(char *fname, double xMin, double xMax, double yMin,
                     double yMax, char *x_str, char *upstr, int N, int *Dim,
                     char *type, char **Y) {
  char filename[100], buff[STRSIZ], command[100];
  FILE *outfile;
  ...
  if (logY)
    fprintf(outfile, "opt  logy\n");
  if (logX)
    fprintf(outfile, "opt  logx\n");
  fprintf(outfile, "* TITLE '%s'\n", upstr);
  ...
  for (k = 0; k < N; k++) {
    ...
    if (logX) {
      ...
      fprintf(outfile, " sigma X%d=ARRAY(%d,%G#%G)\n", k + 1, Dim[k],
              xMin + 0.5 * (xMax - xMin) / Dim[k],
              xMax - 0.5 * (xMax - xMin) / Dim[k]);

    fprintf(outfile, " vector/Create Y%d(%d)\n", k + 1, Dim[k]);
    if (type[k] == 'h')
      fprintf(outfile, " vector/Create dY%d(%d)\n", k + 1, Dim[k]);
  }
  ...
  for (k = 0; k < N; k++) {
    ...
    if (type[k] == 'h')
      fprintf(outfile, " set HCOL %d\n", k + 1);
    fprintf(outfile, " set plci %i ; set txci %i  \n", k + 1, k + 1);

    if (logY)
      yl = exp(log(yMax) - 0.05 * (k + 1) * log(yMax / yMin));
    else
      yl = yMax - 0.05 * (k + 1) * (yMax - yMin);
...
static void writeROOT(char *fname, double xMin, double xMax, double yMin,
                      double yMax, char *x_str, char *upstr, int N, int *Dim,
                      char *type, char **Y) {
  ...
  if (logX) {
    ...
    for (k = 0; k < N; k++)
      fprintf(outfile,
              " for(int i=0;i<%d;i++){ "
              "X%d[i]=xMin*pow(xMax/xMin,(i+0.5)/%d);dX%d[i]=X%d[i]*(pow(xMax/"
              "xMin,0.333/%d)-1);} \n",
              Dim[k], k, Dim[k], k, k, Dim[k]);
  } else
    for (k = 0; k < N; k++)
      fprintf(outfile,
              " for(int i=0;i<%d;i++){ X%d[i]=xMin+(i+0.5)*(xMax-xMin)/%d; "
              "dX%d[i]=(xMax-xMin)/3./%d;}\n",
              Dim[k], k, Dim[k], k, Dim[k]);

  if (logX)
    fprintf(outfile, " double Xtext = xMin*pow(xMax/xMin,0.3);\n");
  else
  ...
  //   fprintf(outfile, " for(int i=0;i<%d;i++)
  //   dX[i]=(xMax-xMin)/%d/3.;\n",dim,dim);
  ...
  for (k = 0; k < N; k++) {
    if (type[k] == 'h') {
      fprintf(
          outfile,
          "   TGraphErrors *gr%d =new TGraphErrors(%d,X%d,Y%d,dX%d,dY%d);\n", k,
          Dim[k], k, k, k, k);
      ...
      fprintf(outfile, "   gr%d->SetLineColor(%d);\n", k, k + 1);
      if (type[k] == 'l')
        fprintf(outfile, "   gr%d->Draw(\"L\");\n", k);
      else
        fprintf(outfile, "   gr%d->Draw(\"C\");\n", k);
    }
    if (N > 1 && Y[k][0]) {
      if (logY)
        fprintf(outfile, "   Ytext/=dYtext;\n");
      else
        fprintf(outfile, "  Ytext-=dYtext;\n");
      /*
              fprintf(outfile,"  TText *t%d = new
         TText(Xtext,Ytext,\"%s\");\n",k,Y[k]); fprintf(outfile,"
         t%d->SetTextColor(%d);\n",k,k+1); fprintf(outfile,"
         t%d->SetTextFont(42);\n",k); fprintf(outfile,"
         t%d->SetTextSize(0.03);\n",k); fprintf(outfile,"  t%d->Draw();\n",k);
      */
...
static void writePython(char *fname, double xMin, double xMax, double yMin,
                        double yMax, char *x_str, char *upstr, int N, int *Dim,
                        char *type, char **Y) {
  char filename[100], basename[100], buff[STRSIZ];
  FILE *outfile;
  ...
  for (k = 0; k < N; k++) {
    ...
    if (type[k] == 'h')
      fprintf(outfile, ", dy%d", k);
  }
  fprintf(outfile,
          " = np.genfromtxt('%s',dtype=float, comments='#',unpack=True)\n",
          fname);
  ...
  if (N == 1)
    fprintf(outfile, "plt.ylabel('%s',fontsize=16)\n", Y[0]);
  if (logX)
    fprintf(outfile, "plt.xscale('log')\n");
  else
    fprintf(outfile, "plt.xscale('linear')\n");
  if (logY)
    fprintf(outfile, "plt.yscale('log')\n");
  else
    fprintf(outfile, "plt.yscale('linear')\n");
  fprintf(outfile, "xmin=%E\n", xMin);
  fprintf(outfile, "xmax=%E\n", xMax);
  ...
  if (k != N) // interpalation is needed
  {
    if (logX) {
      ...
      fprintf(outfile, "xi=np.linspace(xmin, xmax,300)\n");
  }
  for (k = 0; k < N; k++) {
    if (logX) {
    ...
    if (type[k] == 'h') {
      if (logX) {
        ...
        fprintf(outfile,
                "plt.errorbar(x%d, y%d[0:%d], dy%d[0:%d], xerr=[errL,errR], "
                "fmt=None,label='%s')\n",
                k, k, Dim[k], k, Dim[k], Y[k]);
      } else
        fprintf(outfile,
                "plt.errorbar(x%d, y%d[0:%d], dy%d[0:%d], xerr=%e, "
                "fmt=None,label='%s')\n",
    ...
    } else if (type[k] == 's') // interpolation
    {
      ...
      if (logX) {
        if (logY) {
          fprintf(outfile,
                  "tck=interpolate.splrep(l%d, np.log(y%d[0:%d]),s=0)\n", k, k,
                  Dim[k]);
          fprintf(outfile,
                  "plt.plot(xi, np.exp(interpolate.splev(li, tck, der=0)), "
                  "alpha=0.7, label='%s', "
                  "linestyle='-')\n",
                  Y[k]);
        } else {
          fprintf(outfile, "tck=interpolate.splrep(l%d, y%d[0:%d],s=0)\n", k, k,
                  Dim[k]);
          fprintf(outfile,
                  "plt.plot(xi, interpolate.splev(li, tck, der=0), alpha=0.7, "
                  "label='%s', "
                  "linestyle='-')\n",
                  Y[k]);
        }
        ...
        if (logY) {
          fprintf(outfile,
                  "tck=interpolate.splrep(x%d, np.log(y%d[0:%d]),s=0)\n", k, k);
          fprintf(outfile,
                  "plt.plot(xi, np.exp(interpolate.splev(xi, tck, der=0)), "
                  "alpha=0.7, label='%s', "
                  "linestyle='-')\n",
                  Y[k]);
        } else {
          fprintf(outfile, "tck=interpolate.splrep(x%d, y%d[0:%d],s=0)\n", k,
                  k);
          fprintf(outfile,
                  "plt.plot(xi, interpolate.splev(xi, tck, der=0), alpha=0.7, "
                  "label='%s', "
                  "linestyle='-')\n",
                  Y[k]);
        }
      ...
      fprintf(outfile,
              "plt.plot(x%d, y%d[0:%d],alpha=0.7, label='%s', linestyle='-')\n",
              k, k, Dim[k], Y[k]);
    /*
            else if(type[k]=='s') // interpolation
            {  fprintf("fi = interpolate.interp1d(x%d, y%d)\n");
               ...
               if(logX)
               { fprintf(outfile,"lx=np.linspace(log(xmin)+dxi,
       log(xmax)-dxi,%d)\n",300); fprintf(outfile,"xi=np.exp(lx)\n"); } else
       fprintf(outfile,"xi=np.linspace(xmin+dx, xmax-dx,%d)\n",300);
               fprintf(outfile,"plt.plot(xi, fi(xi),alpha=0.7, label='%s',
       linestyle='-')\n",Y[k]);
            }

            if(N>1) fprintf(outfile,"plt.plot(x%d, y%d[0:%d],alpha=0.7,
       label='%s', linestyle='-')\n",k,k,Dim[k],Y[k]); else
       fprintf(outfile,"plt.plot(x%d, y%d[0:%d],alpha=0.7,
       linestyle='-')\n",k,k,Dim[k],Y[k]);
         }
    */
  }
  if (N > 1)
    fprintf(outfile, "plt.legend()\n");
...
static void writeROOT2D(char *fname, double xMin, double xMax, double yMin,
                        double yMax, int dimX, int dimY, char *x_str,
                        char *y_str, char *upstr) {
  char filename[100], basename[100], buff[STRSIZ];
  FILE *outfile;
  int i, k;
  char format[150] = {}, read[150] = {};
  ...
  fprintf(outfile, " if(buff[0]!='#')\n  {\n");
  fprintf(outfile,
          " sscanf(buff,\" %%lf %%lf \",&(data[i][j]),&(dData[i][j])); j++;\n");
  fprintf(outfile, "    }\n }\n");
  fprintf(outfile, " fclose(f);\n");

  fprintf(
      outfile,
      " TH2D * h2 = new TH2D(\"h2\",\"%s\",xDim,xMin,xMax,yDim,yMin,yMax);\n",
      upstr);

  fprintf(outfile, " for(int i=0;i<xDim;i++) for(j=0;j<yDim;j++) "
                   "h2->SetBinContent(i+1,j+1,data[i][j]);\n");
...
static int writetable2(char *baseF, double xMin, double xMax, int dimX,
                       double yMin, double yMax, int dimY, double *f,
...
static void writePAW2D(char *fname, double xMin, double xMax, double yMin,
                       double yMax, int dimX, int dimY, char *x_str,
                       char *y_str, char *upstr)
...
static void writeMath1(char *fname, double xMin, double xMax, int dim,
                       double *f, double *df, char *upstr, char *x_str,
                       char *y_str) {
  ...
  fprintf(outfile, "(If you move this file, then change the path and/or "
  ...
  fprintf(outfile, "dataCH={\n\t{%-12f,%-12f*2^%-2d}",
          xMin + (1.0 / 2.0) * (xMax - xMin) / dim, mant, expon);
  for (i = 1; i < dim; i++) {
    mant = frexp(f[i], &expon);
    fprintf(outfile, ",\n\t{%-12f,%-12f*2^%-2d}",
            xMin + (1.0 / 2.0 + i) * (xMax - xMin) / dim, mant, expon);
  }
  fprintf(outfile, "\n};\n\n");
  ...
  mant = frexp(f[0], &expon);
  fprintf(outfile, "histDataCH={\n\t{%-12f,%-12f*2^%-2d},{%-12f,%-12f*2^%-2d}",
          xMin, mant, expon, xMin + (xMax - xMin) / dim, mant, expon);
  for (i = 1; i < dim; i++) {
    mant = frexp(f[i], &expon);
    fprintf(outfile, ",\n\t{%-12f,%-12f*2^%-2d},{%-12f,%-12f*2^%-2d}",
            xMin + (xMax - xMin) / dim * i, mant, expon,
            xMin + (xMax - xMin) / dim * (i + 1.0), mant, expon);
  }
  fprintf(outfile, "\n};\n\n");
  ...
  if (df) {
    ...
    fprintf(outfile, "histUncCH={\n\t{%-12f,%-12f,%-12f*2^%-2d,%-12f*2^%-2d}",
            xMin, xMin + (xMax - xMin) / dim, mant, expon, mant2, expon2);
    for (i = 1; i < dim; i++) {
      mant = frexp(f[i] - df[i], &expon);
      mant2 = frexp(f[i] + df[i], &expon2);
      fprintf(outfile, ",\n\t{%-12f,%-12f,%-12f*2^%-2d,%-12f*2^%-2d}",
              xMin + (xMax - xMin) / dim * i,
              xMin + (xMax - xMin) / dim * (i + 1.0), mant, expon, mant2,
              expon2);
    }
    fprintf(outfile, "\n};\n\n");
  }
  ...
  fprintf(
      outfile,
      "histCH=ListPlot[histDataCH,Joined->True, Frame -> True, PlotRange -> "
      "Full, Axes->False, "
      "FrameLabel -> {\"%s\", \"%s\", \"%s\", \"\"}, PlotStyle->Black];\n\n",
      x_str, y_str, upstr);
  ...
  fprintf(outfile,
          "histCombCH=Show[{errHistCH, histCH}, Frame -> True, Axes -> False, "
          "FrameLabel -> "
          "{\"%s\", \"%s\", \"%s\", \"\"}, AspectRatio -> 2/3]\n\n",
          x_str, y_str, upstr);
...
static void writeMath2(double xMin, double xMax, int dimX, double yMin,
                       double yMax, int dimY, double *f, double *df,
                       char *upstr, char *x_str, char *y_str) {
  ...
  for (i = 0; i < dimX; i++)
    for (j = 0; j < dimY; j++) {
      ...
      if (i != 0 || j != 0)
        fprintf(outfile, ",");
      fprintf(outfile, "\n\t{%-12f,%-12f,%-12f*2^%-2d}",
              xMin + (1.0 / 2.0 + i) * (xMax - xMin) / dimX,
              yMin + (1.0 / 2.0 + j) * (yMax - yMin) / dimY, mant, expon);
    }
  fprintf(outfile, "\n};\n\n");
  ...
  fprintf(outfile,
          "histCH=ListContourPlot[dataCH, FrameLabel -> {\"%s\", \"%s\", "
          "\"%s\", \"\"}]\n\n",
          x_str, y_str, upstr);
...
int plot_Nar(char *file, char *title, double xMin, double xMax, char *xName,
             int xScale, int N, int *Dim, double **f, double **ff, char **Y) {
  void *prtscr;
...
#define MESS "Esc - exit; Mouse - position; Other keys - menu"
  ...
  for (i = 1; i < N; i++) {
    ...
    if (grafminmax.ymin > ymin)
      grafminmax.ymin = ymin;
    if (grafminmax.ymax < ymax)
      grafminmax.ymax = ymax;
  }
  for (i = 0; i < N; i++)
    if (ff[i] == NULL)
...
REDRAW:
  ...
  for (i = 0; i < N; i++) {
    fColor = colList[i];
    if (ff[i])
      plot_hist(xMin, xMax, Dim[i], f[i], ff[i]);
    else if (type[i] == 'l')
      plot_curve(xMin, xMax, Dim[i], f[i]);
    else
      plot_spline(xMin, xMax, Dim[i], f[i]);
  }
  ...
  for (;;) {
    ...
    if (mouseXY)
      ...
      for (k = 0; k < N; k++) {
        ...
        if (ff[k])
          print(" %.2E+/-%.1E ", f[k][i], ff[k][i]);
        else
          print(" %.2E ", f[k][i0] * (1 - al) + f[k][i0 + 1] * al);
      }
    }
  }
  ...
  do {
    ...
    sprintf(menustr,
            "%c Y-max = %-9.3G Y-min = %-9.3G Y-scale = %s"
            " Spline           "
            " Mouse: show XY   "
            " Save plot in file",
    ...
    case 4: {
      ...
      for (;;) {
        ...
        for (i = 0; i < N; i++) {
          if (type[i] == 'l')
            sprintf(splineMenu + strlen(splineMenu), " %-25.25s OFF", Y[i]);
          if (type[i] == 's')
            sprintf(splineMenu + strlen(splineMenu), " %-25.25s ON ", Y[i]);
        }
    ...
    contin:
      ...
      while (m) {
        ...
        switch (m) {
        case 1:
          writeROOT(baseF, xMin, xMax, grafminmax.ymin, grafminmax.ymax, xName,
                    title, N, Dim, type, Y);
          break;
        case 2:
          writePAW(baseF, xMin, xMax, grafminmax.ymin, grafminmax.ymax, xName,
                   title, N, Dim, type, Y);
          break;
        case 3:
          writeGNU(baseF, xMin, xMax, grafminmax.ymin, grafminmax.ymax, xName,
                   title, N, Dim, type, Y);
        case 4:
          writePython(baseF, xMin, xMax, grafminmax.ymin, grafminmax.ymax,
    ...
    case 0:
      ...
      if (logY && (grafminmax.ymin <= 0))
        grafminmax.ymin = grafminmax.ymax * 1E-4;

      if (logY && grafminmax.ymax / grafminmax.ymin <= 10)
        grafminmax.ymin = grafminmax.ymax / 10.;
...
int plot_N(char *title, double xMin, double xMax, char *xName, int xScale,
           int N, ...) {
  int err, k, i;
...
void plot_1(double xMin, double xMax, int dim, double *f, double *ff,
            char *title, char *xstr, char *ystr) {
  plot_Nar(NULL, title, xMin, xMax, xstr, 0, 1, &dim, &f, &ff, &ystr);
}

void plot_2D(double hMin1, double hMax1, int nBin1, double hMin2, double hMax2,
             int nBin2, double *f, double *df, char *upstr, char *xstr,
             char *ystr) {
...
REDRAW:
...

  {
    ...
    for (i = 0; i < nBin1; i++)
      for (j = 0; j < nBin2; j++) {
        ...
        double g2 =
            pow(fabs(f[i * nBin2 + j] - df[i * nBin2 + j]) / fmax, 0.5 * P);
        if (f[i * nBin2 + j] < df[i * nBin2 + j])
          g2 = 0;
        if (g && fabs((dscx(x - g * DX / 2) - dscx(x + g * DX / 2)) *
                      (dscy(y + g * DY / 2) - dscy(y - g * DY / 2))) > 1) {
          double dx = 0.9 * DX, dy = 0.9 * DY;
          bColor = Black;
          tg_bar(scx(x - g * dx / 2), scy(y + g * dy / 2), scx(x + g * dx / 2),
                 scy(y - g * dy / 2));

          fColor = Black;
          tg_line(scx(x - g * dx / 2) - 1, scy(y), scx(x - g1 * dx / 2),
                  scy(y));
          tg_line(scx(x + g * dx / 2) + 1, scy(y), scx(x + g1 * dx / 2),
                  scy(y));
          tg_line(scx(x), scy(y + g * dy / 2) + 1, scx(x),
                  scy(y + g1 * dy / 2));
          tg_line(scx(x), scy(y - g * dy / 2) - 1, scx(x),
                  scy(y - g1 * dy / 2));
  ...
  for (;;) {
    ...
    print(" X=%.3E Y=%.3E F=%.2E+/-%.1E  ", x, y, f[i * nBin2 + j],
          df[i * nBin2 + j]);
  }
  if (nCol0 != maxCol() && nRow0 != maxRow())
    goto REDRAW;
...
contin:
  do {
    ...
    case 0:
      goto REDRAW;
    case 1:
      correctDouble(33, 11, "S=F^(P), P= ", &P, 1);
      break;
    case 2: {
      char baseF[100];
      ...
      correctStr(10, 17, "File name: ", baseF, 29, 1);
      trim(baseF);
      writetable2(baseF, hMin1, hMax1, nBin1, hMin2, hMax2, nBin2, f, df, upstr,
                  xstr, ystr);
      char fileMenu[] = "\011"
                        " PAW     "
                        " ROOT    ";
      ...
      while (m) {
        ...
        switch (m) {
        case 1:
          writePAW2D(baseF, hMin1, hMax1, hMin2, hMax2, nBin1, nBin2, xstr,
                     ystr, upstr);
          break;
        case 2:
          writeROOT2D(baseF, hMin1, hMax1, hMin2, hMax2, nBin1, nBin2, xstr,
                      ystr, upstr);
          break;
          break;
        }
      ...
      break;
    case 3:
      writeMath2(hMin1, hMax1, nBin1, hMin2, hMax2, nBin2, f, df, upstr, xstr,
                 ystr);
      break;
    }
    }
