...
static int getNinNout(FILE *flow) {
  if (2 != fscanf(flow, "%d -> %d", &nin_int, &nout_int))
    return 1;
  return 0;
}
...
static int getNames(FILE *flow) {
  ...
  for (i = 0; i < nin_int + nout_int; i++) {
    if (2 != fscanf(flow, "%d(%[^)]%*c", p_codes_ + i, p_names_[i]))
      return 1;
    if (i == nin_int - 1)
      fscanf(flow, " -> ");
  }
...
int main(int argc, char **argv) {
  ...
  int Nmiss = 0;
  rw_paragraph rd_array[8] = {{"CalcHEP", NULL},
                              {"Type", getNinNout},
                              {"Initial_state", NULL},
                              {"PROCESS", getNames},
                              {"MASSES", getMasses},
  ...
  while (fscanf(stdin, "%lf", &weight) == 1) {
    N++;
    for (i = 0; i < 4 * nin_int; i++)
      pvect[i] = 0;
    if (nin_int == 2)
      fscanf(stdin, "%lf %lf", pvect + 3, pvect + 7);
      ...
      pvect[4 * i] = ENERGY(mass[i], pvect + 4 * i + 1);
    {
      ...
      for (; plist0; plist0 = plist0->next)
        z0[n0++] = calcPhysVal(key[0], plist0->pstr, pvect);
