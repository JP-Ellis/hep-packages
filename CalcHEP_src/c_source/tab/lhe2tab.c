...
static int skipHeadLine(FILE *flow) {
  fscanf(flow, "%*[^\n]\n");
  return -1;
}
static int readCS(FILE *flow) {
  fscanf(flow, "%lf", &totCS);
  return 0;
}
static int readNEvents(FILE *flow) {
...
int main(int argc, char **argv) {
  ...
  if (sscanf(argv[2], "%lf", &minX) != 1) {
    wrongParam(2);
    return 1;
  }
  if (sscanf(argv[3], "%lf", &maxX) != 1 || minX >= maxX) {
    wrongParam(3);
    return 1;
  }
  ...
  while (readevent_() == 0) {
    ...
    for (i = 0; i < E_.NUP; i++)
      if (E_.ISTUP[i] == 2) {
        double M = sqrt(
            fabs(E_.PUP[i][3] * E_.PUP[i][3] - E_.PUP[i][0] * E_.PUP[i][0] -
                 E_.PUP[i][1] * E_.PUP[i][1] - E_.PUP[i][2] * E_.PUP[i][2]));
        for (k = 0; k < 4; k++) {
          double dif = E_.PUP[i][k];
          int j;
          for (j = i + 1; j < E_.NUP; j++)
            if (E_.MOTHUP[j][0] == i + 1)
              dif -= E_.PUP[j][k];
          dif = fabs(dif) / M;
          if (dif > qmiss[k])
            qmiss[k] = dif;
    ...
    for (i = 0; i < nout_int; i++) {
      ...
      for (k = 0; k < 3; k++)
        pp[k + 1] = E_.PUP[outPos[i]][k];
      p_masses_[nin_int + i] = E_.PUP[outPos[i]][4];
      p_codes_[nin_int + i] = E_.IDUP[outPos[i]];
    }

    for (i = 0; i < nout_int + nin_int; i++)
      sprintf(p_names_[i], "%d", p_codes_[i]);
    ...
    if (checkPhysValN(argv[1], key, &plist)) {
      ...
      for (; plist; plist = plist->next)
        z0[n0++] = calcPhysVal(key[0], plist->pstr, pvect);
      cleanPVlist(plist0);
      switch (key[1]) {
      case '^':
        for (k = 1; k < n0; k++)
...

  {
    ...
    */
    xUnit(key[0], xunits);
    if (nin_int == 2)
      sprintf(yname, "Diff. cross section [pb/%s]", xunits);
    else
      sprintf(yname, "Diff. width [GeV/%s]", xunits);
    fprintf(stdout, "\n");
