...
static void Rot3D(double cn0, double fi, double psi, int ntot, REAL *pvect) {
  ...
  REAL EPS[3][3] = {{0, -E[2], E[1]}, {E[2], 0, -E[0]}, {-E[1], E[0], 0}};
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++)
      ROT[i][j] = (cn2 - 1) * P[i][j] + sn2 * EPS[i][j];
...
int mkmom(double *x, double *tfact, double *xp1, double *xp2, REAL *pvect) {
  ...
  *tfact = tfact0;
  /* **   MOMENTS */
  if (nout_int == 1) {
    REAL s = pm[0] + pm[1], d = pm[0] - pm[1],
         p = sqrt((pm[2] - s) * (pm[2] + s) * (pm[2] - d) * (pm[2] + d)) /
             (2 * pm[2]);
    ...
    if (pm[0] > 0) {
      REAL y = log(pm[0] / (e1 + p));
      if (y2 < y)
        y2 = y;
    }
    if (pm[1] > 0) {
      REAL y = -log(pm[1] / (e2 + p));
      if (y1 > y)
        y1 = y;
    }
    for (i = 0; i < 12; i++)
    ...
    if (sf_num[0] || sf_num[1]) {
      ...
      if (xx < al) {
        ...
        if (xx == al)
          ...
          xx = (1 - xx) / (1 - al);
        stilda = stop - pow(xx * pow(stop - ssmin, bes) +
                                (1 - xx) * pow(stop - ssmax, bes),
                            1 / bes);
      ...
      if (sf_num[0] && sf_num[1]) {
        ...
        if (beta[0] < 1 && beta[1] < 1) {
          ...
          *tfact *= pow(divy_(y1), beta[0] - 1) * pow(divy_(y2), beta[1] - 1) /
          ...
          if (bes < 1)
            *tfact *= pow(divy_(yy), 1 - bes);
          else
            *tfact *= bes * pow(yy, bes - 1);
        } else if (beta[0] < 1) {
          psy1 = pow(xx, 1 / beta[0]);
          y1 = yy * psy1;
          y2 = yy * (1 - psy1);
  ...
  for (k = 0; k < nin_int; ++k)
    pvFill(pm[k], pIn[k], k + 1, pvect);

  if (nin_int == 2)
    *tfact /= 4 * pcmtilda * rstilda;
  else {
    rstilda = pm[0];
    *tfact /= rstilda * 2;
    ytilda = 0;
  ...
  for (i = 0; i < nout1; ++i) {
    ...
    for (k = 0; k < 2; ++k) {
      if (kinmtc_1[i].lvout[k][1]) {
        ...
        if (nmscut[i][k])
          rancor_(&smin, &smax, 0., 1., nmscut[i][k]);
  ...
  for (i = 0; i < nout1; ++i) /*  MAIN CYCLE */
  {
    ...
    for (k = 0; k < 2; ++k) {
      ...
      if (ncM)
        rancor_(&cosmin, &cosmax, hsum[k], d__1, ncM);
      if (ncT)
        rancor_t(&cosmax, hsum[k], d__1, Emax[k],
                 pm[sph_inf[i][ns___].lvpole[0] - 1], pcmtilda, amass[i][k],
                 invcut_1[ncT - 1].cvmin);
      if (cosmin >= cosmax) {
        *tfact = 0;
        return 0;
      }
    }
    regfun_(sph_inf[i][ns___].itypep, nsing, singar, cosmin, cosmax, xcos,
            &parcos, &fct);
    ...
    for (ns = 0; ns < i__2; ++ns)
      if (ns != ns___) {
        ...
        for (k = 0; k < 2; ++k) {
          ...
          if (cosmin >= parcos || parcos >= cosmax) {
            *tfact = 0;
            return 0;
          }
        }
        regfct_(sph_inf[i][ns].itypep, nsing, singar, cosmin, cosmax, parcos,
                &fct);
  ...
  if (nin_int == 2) {
    ...
    *xp2 = sf_num[1] ? x2 : 1;
    ...
    Rot3D(2 * (drandXX() - 0.5), drandXX() * 2 * M_PI, drandXX() * M_PI,
          nout_int, pvect + 4);
  ...
  for (i = 0; i < (nin_int + nout_int) * 4; i++)
    if (!isfinite(pvect[i])) {
      *tfact = 0;
      return 0;
    }
...
int imkmom(double P1, double P2) {
  ...
  for (i = 0; i < nout1; ++i)
    for (k = 0; k < 2; ++k) {
      if (kinmtc_1[i].lvout[k][1])
        ...
        nvout[i][k] = kinmtc_1[i].lvout[k][0];
    }
  ...
  for (i = 1; i < nout1; ++i) {
    ...
    if (!nvin[i]) {
      fprintf(stderr, "Error in kinematics \n");
      sortie(52);
    }
  }
  ...
  for (i = 0; i < nout1; ++i)
    for (k = 0; k < 2; ++k) {
      ...
      for (j = 0; (pn = kinmtc_1[i].lvout[k][j]); j++)
        ss += pm[pn - 1];
      summas[i][k] = ss;
    }
  ...
  if (nin_int == 2) {
    ...
    if (sf_num[0] && m1 < sf_mass[0])
      m1 = sf_mass[0];
    if (sf_num[1] && m2 < sf_mass[1])
      m1 = sf_mass[1];

    incomkin(m1, m2, P1, P2, &sqrt_S, &pcm, &rapidity);
    ssmin = pm[0] + pm[1];
    if (ssmin < summas[0][0] + summas[0][1])
      ssmin = summas[0][0] + summas[0][1];
    ssmin *= ssmin;
    ...
    if (sf_num[0] && sf_num[1]) {
      ...
      sbot = m2 * m2;
      stop = sbot + 2 * pcm * (pcm + sqrt(pcm * pcm + sbot));
    } else if (sf_num[1]) {
      sbot = m1 * m1;
      stop = sbot + 2 * pcm * (pcm + sqrt(pcm * pcm + sbot));
    } else
      ...
      if (i == 0)
        ...
        if (lvbuf[0] == 0)
          ...
          if (ns == nsph[i] && ns < 10) {
            ...
            if (spole_(invreg_1[l].lvinvr))
              sph_inf[i][ns].itypep = -2;
            else
              sph_inf[i][ns].itypep = -1;
  ...
  for (i = 0; i < nout1; ++i) {
    ...
    for (ns = 0; ns < nsph[i]; ++ns) {
      int nwe = 0;
      for (k = 0; k < 2; ++k)
        for (l = sph_inf[i][ns].ncsreg[k]; l; l = invreg_1[l - 1].nextrg)
          ++nwe;

      if (sph_inf[i][ns].itypep >= 0)
        ++nwe;
      sph_inf[i][ns].sph_we = nwe;
      wesum += nwe;
    }
    for (ns = 0; ns < nsph[i]; ++ns)
      sph_inf[i][ns].sph_we /= wesum;
  }

#ifdef DEBUG
  for (i = 0; i < nout1; ++i) {
    printf("Decay number %d     nmscut= (%d,%d) nmsreg = (%d,%d)\n", i,
           nmscut[i][0], nmscut[i][1], nmsreg[i][0], nmsreg[i][1]);

    {
      int l, c;
      printf("kinematics= (");
      for (l = 0; c = kinmtc_1[i].lvin[l]; l++)
        printf("%d", c);
      printf(")->(");
      for (l = 0; c = kinmtc_1[i].lvout[0][l]; l++)
        printf("%d", c);
      printf(")+(");
      for (l = 0; c = kinmtc_1[i].lvout[1][l]; l++)
        printf("%d", c);
      printf(")\n");
    }
    ...
    for (ns = 0; ns < nsph[i]; ++ns) {
      ...
      printf("   Sphere number = %d  weight=%f type=%d \n", ns,
             sph_inf[i][ns].sph_we, sph_inf[i][ns].itypep);
      printf("     pole vector(");
      for (k = 0; c = sph_inf[i][ns].lvpole[k]; k++)
        printf("%d", c);
      printf(")\n");
