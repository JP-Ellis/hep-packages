...
int prepDen(int nden, int nin, double BWrange2, REAL *dmass, REAL *dwidth,
            char **Qtxt, REAL *momenta, REAL *Q0, COMPLEX *Q1, REAL *Q2) {
  ...
  for (i = 1; i <= nden; i++) {
    ...
    int sgn = 0;
    if (loopFF__) {
      if (strcmp(Qtxt[i], "\1\2") == 0)
        sgn = 1;
      else if (strcmp(Qtxt[i], "\1\4") == 0)
        sgn = -1;
    }
    ...
    if (dwidth[i]) {
      ...
      if (cabs(Q1[i]) < 100 * s0max)
        err = 2;
      //     if(cabs(Q1[i]) < 100*s0max)   printf("i=%d s0max=%E cabs(Q1[i]=%E
      //     eps=%E E=%E\n",i,
      //     (double)s0max,(double)cabs(Q1[i]),(double)computer_eps,
