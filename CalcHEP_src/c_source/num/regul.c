...
static int addreg_(char *lv, double rgmasw, double rgwdtw, int nndeg) {
  ...
  for (; invreg_1[nreg].lvinvr[0]; nreg++) {
    if (eqvect_(lv, invreg_1[nreg].lvinvr))
      lastrg = nreg;
  }
  if (nreg >= 199)
    return 0;
...
int fillRegArray(void) {
  ...
  while (ln != NULL) {
    ...
    sscanf(ln->line, "%[^|]%*c%[^|]%*c%[^|]%*c%d", invStr + 1, massStr,
           widthStr, &power);
    ...
    if (!checkPhysVal(invStr, &charKey, lv)) {
      sprintf(errorText,
              " Error in  regularization table line %d .\n"
              " Wrong field 'Momentum' .",
              lineNum);
      goto errorExit;
    }
...
int wrtreg_(FILE *nchan) {
  fprintf(nchan, "\n");
  writetable0(&regTab, nchan);
  return 0;
}
int rdrreg_(FILE *nchan) {
  fscanf(nchan, "\n");
  return readtable0(&regTab, nchan);
}

int getreg_(int *nsing, sing_struct *singar, double shift, double fmult,
            int n) {
  double d__1;
  ...
  for (; n && *nsing < 100; n = invreg_1[n - 1].nextrg) {
    ...
    singar[*nsing - 1].width =
        (d__1 = invreg_1[n - 1].rgmass * invreg_1[n - 1].rgwdth * fmult,
         ABS(d__1));
