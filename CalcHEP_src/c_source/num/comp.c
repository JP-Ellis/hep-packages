...
int wrtcomp_(FILE *nchan) {
  fprintf(nchan, "\n");
  writetable0(&compTab, nchan);
  return 0;
}
...
int fillCompositeArray(void) {
  ...
  for (ln = compTab.strings; ln; ln = ln->next) {
    ...
    for (j = 0; chB; chB = strchr(chB, ','), j++) {
      ...
      if (found == 0)
        for (k = 0; k < nModelParticles; k++)
          if (strcmp(pname, ModelPrtcls[k].name) == 0)
            found++;
      if (found == 0)
        for (k = 0; k < nModelParticles; k++)
          if (strcmp(pname, ModelPrtcls[k].aname) == 0)
            found++;
      if (found == 0) {
        sprintf(fieldName, "Cannot find %s in model.", pname);
        goto errorExit;
      }
      // Check that pname is not a duplicate.
      for (l = 0; l < nCompParts[nComps - 1]; l++)
        if (strcmp(compParts[nComps - 1][l], pname) == 0)
          new = 0;
      if (new) {
        sprintf(compParts[nComps - 1][i++], "%s", pname);
        nCompParts[nComps - 1]++;
      } else {
        sprintf(fieldName, "%s is included more than once.", pname);
        goto errorExit;
      }
    }
  }
