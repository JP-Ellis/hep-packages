...
static FILE *fout;
static double (*scaleCC_)(int nsub, REAL *,
                          double (*calcPV)(char, char *, double *), double *,
...
static void *act_num_(char *ch, int n, void **args) {
  ...
  case '*':
    ...
    return p;
  case '/':
    p = malloc(strlen(p1) + strlen(p2) + 15);
  ...
  default:
    switch (n) {
    case 1:
      if (!strcmp(ch, "sqrt") || !strcmp(ch, "sin") || !strcmp(ch, "cos") ||
          !strcmp(ch, "tan") || !strcmp(ch, "asin") || !strcmp(ch, "acos") ||
          !strcmp(ch, "atan") || !strcmp(ch, "exp") || !strcmp(ch, "log") ||
          !strcmp(ch, "fabs")) {
        p = malloc(strlen(p1) + 10);
        sprintf(p, "%s(%s)", ch, p1);
        return p;
    ...
    if (strchr("ACDEJKMPSTUYNWZ", ch[0]) &&
        (ch[1] == 0 || (ch[2] == 0 && (ch[1] == '_' || ch[1] == '`')) ||
      ...
      sprintf(buff, "%s(", ch);
      for (i = 0; i < n; i++)
        sprintf(buff + strlen(buff), "%s,", (char *)args[i]);
      buff[strlen(buff) - 1] = ')';
      int Nsub_mem = Nsub;
      ...
      for (Nsub = 1; Nsub <= nprc_int; Nsub++) {
        ...
        char key[3];
        physValRec *pList, *p;
        if (!checkPhysValN(buff, key, &pList))
          return NULL;
        fprintf(fout, "char * plist[]={");
        for (p = pList; p; p = p->next) {
          fprintf(fout, "\"");
          for (i = 0; i < strlen(p->pstr); i++)
            fprintf(fout, "\\%d", p->pstr[i]);
          fprintf(fout, "\",");
        }
        fprintf(fout, "NULL};\n");
        cleanPVlist(pList);
        fprintf(fout,
                "     for(i=1,ss=calcPV('%c',plist[0],pvect); plist[i];i++) ",
                key[0]);
        switch (key[1]) {
        case 0:
          fprintf(fout, " ss+=calcPV('%c',plist[i],pvect);\n", key[0]);
          break;
        case '_':
          fprintf(fout, "{ s1=calcPV('%c',plist[i],pvect); if(s1<ss) ss=s1;} ",
                  key[0]);
          break;
        case '`':
          fprintf(fout, "{ s1=calcPV('%c',plist[i],pvect); if(s1>ss) ss=s1;} ",
...
int initScales(char *sR, char *sF1, char *sF2, char *sS, char *mess) {
  ...
  if (!fout) {
    if (mess)
      sprintf(mess, "can't open file scale.c for writing");
    return -1;
  }
  NX = 0;
  ...
  fprintf(fout, "extern void ScaleCC(int nsub, REAL*, double "
                "(*calcPV)(char,char*,double*), "
                "double*,double*,double*,double*,double *);\n");
  fprintf(fout,
          "void ScaleCC(int nsub,REAL*modelVal, double "
          "(*calcPV)(char,char*,double*), double "
          "*pvect,double *%s, double *%s,double *%s, double*%s)\n",
  ...
  sprintf(command, ". %s/FlagsForSh; $CC $CFLAGS $SHARED -o scale.so scale.c",
          rootDir);
  system(command);
  free(command);
  ...
  if (!scaleLib) {
    if (mess)
      sprintf(mess, "Can't load shared library for QCD scale");
    return -3;
  }
  scaleCC_ = dlsym(scaleLib, "ScaleCC");
  if (!scaleCC_) {
    if (mess)
      sprintf(mess, "Problem in library scale.so ");
    return -4;
  }
  return 0;
}

void Scale(int nsub, double *pv, double *qR, double *qF1, double *qF2,
           double *qS) {
  double q = 91.187;
