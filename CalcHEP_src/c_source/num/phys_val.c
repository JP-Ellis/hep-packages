...
double calcPhysVal(char key, char *lv, double *V) {
  ...
  case 'P': {
    ...
    // printf("%E  %E %E %E\n", (s*pp[0] - V[np1]*p2 ) /(sqrt(p2) * mtot * pcm),
    // p2,mtot,pcm);
  ...
  case 'U':
    return usrfun(lv, nin_int, nout_int, V, p_names, p_codes);
  }
  return 0;
}
...
int checkPhysVal(char *name, char *key, char *plist) {
  ...
  *key = toupper(*key);
  if (strchr("ACEDFJKMPSTUYNZW", *key) == NULL)
    return 0;
  ...
  if (strchr("CAJDFK", *key) != NULL && strlen(plist) != 2)
    return 0;
  if (strchr("P", *key) != NULL && strlen(plist) < 2)
    return 0;

  //  if(strchr("MS",*key)!=NULL && strlen(plist)<1)  return 0;
  if (strchr("S", *key) != NULL && strlen(plist) < 1)
    return 0;
  ...
  if (strchr("MNYZ", *key) && !spole_(plist))
    return 0;
...
int checkPhysValN(char *name, char *key, physValRec **pLists) {
  ...
  *key = toupper(*chB);
  ...
  if (!strchr("ACDEFJKMPTUYNWZ", *key)) {
    sprintf(errorText, "wrong key '%c'", *key);
    return 0;
  }
  chB++;

  if (*key == 'E' && nin_int == 2 &&
      (strcmp(chB, "1") == 0 || strcmp(chB, "2") == 0)) {
    *pLists = malloc(sizeof(physValRec));
    (*pLists)->next = NULL;
    (*pLists)->pstr[0] = chB[0] - '0';
    (*pLists)->pstr[1] = 0;
    return 1;
  }

  if (*key == 'M' && nin_int == 2 &&
      (strcmp(chB, "12") == 0 || strcmp(chB, "21") == 0)) {
    *pLists = malloc(sizeof(physValRec));
    (*pLists)->next = NULL;
  ...
  if (*key == 'U') {
    if (strlen(chB) > 29) {
      sprintf(errorText, "Too long name for U-function ");
      return 0;
    }
    *pLists = malloc(sizeof(physValRec));
    (*pLists)->next = NULL;
  ...
  if (*chB != '(') {
    sprintf(errorText, " bracket '(' expected");
    return 0;
  }
  chE = strchr(chB, ')');
  if (!chE) {
  ...
  for (; chE[0]; chE++)
    if (chE[0] != ' ') {
      sprintf(errorText, " only one term expected");
      return 0;
    }
  ...
  for (ln = 0; chB; chB = strchr(chB, ','), ln++) {
    ...
    k = 0;
    for (i = 0; i < nComps; i++)
      if (strcmp(pname, compName[i]) == 0)
        isComp = i;
    if (isComp > -1)
      for (j = 0; j < nCompParts[isComp]; j++)
        for (i = nin_int + 1; i <= nin_int + nout_int; i++) {
          if (strcmp(compParts[isComp][j], pinf_int(Nsub, i, NULL, NULL)) ==
              0) {
            pnum[ln][k] = i;
            k++;
            pnum[ln][k] = 0;
    ...
    if (!k) {
      ...
      if (found == 0)
        for (i = 0; i < nModelParticles; i++)
          if (strcmp(pname, ModelPrtcls[i].name) == 0)
            found++;
      if (found == 0)
        for (i = 0; i < nModelParticles; i++)
          if (strcmp(pname, ModelPrtcls[i].aname) == 0)
            found++;
      if (found == 0) {
        sprintf(errorText, "'%s' - not a particle or partice set", pname);
        return 0;
      }
    }
  }
  ...
  if (*key == 'P' && ln < 2) {
    sprintf(errorText, "P() value needs 2 arguments ");
    return 0;
  }
  if (strchr("CA", *key) && ((ln == 1 && nin_int == 1) || ln > 2)) {
    printf(errorText, "wrong number of arguments for C() or A() ");
    return 0;
  }

  if (nin_int == 1) {
    if (strchr("DFTYNKZW", *key)) {
      printf(errorText,
             "'D/F/K/T/Y/N/Z/W'  can not be used for decay processes");
      return 0;
    }
  }
