...
static double alpha(int nf, int odr, double lambda, double dscale) {
  ...
  if (odr == 1)
    ...
    return alpha0 *
           (1 - 2 * b1 * log(rl) / (b0 * b0 * rl) +
            d__2 * d__2 * (d__4 * d__4 + b2 * b0 / (8 * b1 * b1) - 1.25));
  else {
    fprintf(stderr, "Can not evaluate alpha in so large oder (%d).\n", odr);
    exit(1);
  }
}
...
static double findLambda(int nf, int odr, double alpha0, double M) {
  ...
  while ((a1 = alpha(nf, odr, l1, M) - alpha0) > 0)
    l1 *= 0.7;

  while ((a2 = alpha(nf, odr, l2, M) - alpha0) < 0)
    l2 *= 1.3;
...
static void init_alpha(void) {
  ...
  if (nf5 == 5) {
    if (MbMb < 1.5 * L5) {
      ...
      if (alphaOrder == 3)
        al *= (1. + (11. / 72.) * pow(al / M_PI, 2.));
      L4 = findLambda(4, alphaOrder, al, MbMb);
      nf4 = 4;
      ;
    }
  } else {
  ...
  if (nf4 == 4) {
    if (MCHARM < 1.5 * L4) {
      ...
      if (alphaOrder == 3)
        al *= (1. + (11. / 72.) * pow(al / M_PI, 2.));
      L3 = findLambda(3, alphaOrder, al, MCHARM);
      nf3 = 3;
    }
  } else {
    nf3 = nf;
...
static double alpha_0(double Q) {
  ...
  if (Q < MCHARM)
    return alpha(nf3, alphaOrder, L3, Q);
  else if (Q < MbMb)
    return alpha(nf4, alphaOrder, L4, Q);
  else if (Q < Mtp)
    return alpha(nf5, alphaOrder, L5, Q);
  else
    return alpha(nf6, alphaOrder, L6, Q);
}
...
L10 : {
  ...
  if (alphaPDF) {
    ...
    if (k)
      improveStr(strmen, "OFF", "pdf%d", k);
  }
  improveStr(strmen, "ZZZZ", "%.4f", alphaMZ);
  improveStr(strmen, "NF", "%d", alphaNF);

  if (alphaOrder == 1)
    improveStr(strmen, "NNLO", "%-.4s", "LO");
  else if (alphaOrder == 2)
    improveStr(strmen, "NNLO", "%-.4s", "NLO");
  else
    alphaOrder = 3;
  ...
  case 1: {
    ...
    if (k)
      alphaPDF = k - 1;
    if (alphaPDF && !sf_alpha[alphaPDF - 1])
      messanykey(20, 20, "WARNING! This pdf  does not define alphaQCD ");
  } break;
  case 2: {
    double alphaMZ_old = alphaMZ;
    if (correctDouble(3, 15, "Enter new value ", &alphaMZ, 1))
      returnCode = 1;
    if (alphaMZ > 0 && alphaMZ < 0.3)
      returnCode = 1;
    else {
      alphaMZ = alphaMZ_old;
  ...
  case 3: {
    ...
    if (correctInt(3, 15, "Enter new value ", &alphaNF, 1)) {
      if (alphaNF <= 6 && alphaNF >= 3)
        returnCode = 1;
      else {
        messanykey(5, 15, "NF out of range");
        alphaNF = NF_old;
      }
    }
  } break;
  ...
  case 5:
    correctDouble(3, 15, "Enter new value ", &MbMb, 1);
    break;
  case 6:
    correctDouble(3, 15, "Enter new value ", &Mtp, 1);
    break;
  case 7: {
    void *screen;
    int i;
    ...
    if (correctDouble(40, 15, "Q_min=", &qMin, 0) && qMin >= 0.5 &&
        ...
        correctInt(33, 17, "number of points=", &nPoints, 0) && nPoints > 3 &&
        nPoints <= 150) {
      ...
      switch (alphaOrder) {
      case 1:
        sprintf(Y[0], "MSbar LO");
        break;
      case 2:
        sprintf(Y[0], "MSbar NLO");
        break;
      case 3:
        sprintf(Y[0], "MSbar NNLO");
        break;
      default:
        sprintf(Y[0], "MSbar");
      }
      ...
      for (i = 0; i < N; i++)
        dim[i] = nPoints;
      //            printf("N=%d Y[0]=%s\n Y[1]=%s\n Y[2]=%s\n",
      //            N,Y[0],Y[1],Y[2]); plot_Nar(NULL, "Alpha(Q)",  log10(qMin),
      //            log10(qMax),"log10(Q/GeV)", nPoints, N, f,ff,Y);
  ...
  case 8: {
    ...
    do {
      ...
      goto_xy(2, 12);
      print("Renorm. scale: ");
      if (str_redact(Rscale_str, npos, 60) == KB_ENTER)
        returnCode = 1;
      goto_xy(2, 12);
      clr_eol();
  ...
  case 9: {
    ...
    do {
      ...
      goto_xy(2, 12);
      print("Fct1.scale: ");
      if (str_redact(F1scale_str, npos, 60) == KB_ENTER)
        returnCode = 1;
      goto_xy(2, 12);
      clr_eol();
      rc = initScales(Rscale_str, F1scale_str, F2scale_str, Sscale_str, mess);
  ...
  case 10: {
    ...
    do {
      ...
      goto_xy(2, 12);
      print("Fct1.scale: ");
      if (str_redact(F2scale_str, npos, 60) == KB_ENTER)
        returnCode = 1;
      goto_xy(2, 12);
      clr_eol();
      rc = initScales(Rscale_str, F1scale_str, F2scale_str, Sscale_str, mess);
  ...
  case 11: {
    ...
    do {
      char mess[200];
      goto_xy(2, 12);
      print("Shworing scale: ");
      if (str_redact(Sscale_str, npos, 60) == KB_ENTER)
        returnCode = 1;
      goto_xy(2, 12);
      clr_eol();
      rc = initScales(Rscale_str, F1scale_str, F2scale_str, Sscale_str, mess);
...
void i_Scales(void) {
  ...
  if (nin_int == 1)
    strcpy(Rscale_str, "M1");
  else
    strcpy(Rscale_str, "M12");
