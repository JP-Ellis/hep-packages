...
static double singi_(int ityp, double pole, double width, int deg, double x) {
  double dx = x - pole;
...
static double singi1_(int ityp, double pole, double width, int deg, double x,
                      double xmin) {
  if (ityp == 1) {
  L1:
    if (deg == 2)
...
static void regfun_0_(int factOnly, int itype, int nsing, sing_struct *singar,
                      double xmin, double xmax, double xx, double *xout,
                      double *factor) {
  double delt0 = 1e-13;
      ...
      if (singar[i - 1].pos >= xmin && singar[i - 1].pos <= xmax) {
        ...
        if (singar[i - 1].pos - delt <= xmin) {
          ...
          for (ll = 1; ll <= nsing; ++ll) {
            printf("%d %f %f %d\n", ll, singar[ll - 1].pos,
                   singar[ll - 1].width, singar[ll - 1].power);
          }
  ...
  for (i = 1; i <= nsing; ++i) {
    ai[i] = singi_(ityp, singar[i - 1].pos, singar[i - 1].width,
                   singar[i - 1].power, xmin);
    bi[i] = singi_(ityp, singar[i - 1].pos, singar[i - 1].width,
                   singar[i - 1].power, xmax);
  }
  ...
  if (!factOnly) {
    ...
    if (tint0 > tintxx)
      ...
      *xout = singi1_(ityp, singar[nn - 1].pos, singar[nn - 1].width,
                      singar[nn - 1].power, tintxx * (bi[nn] - ai[nn]) + ai[nn],
                      xmin);
    }
    if (*xout > xmax)
      *xout = xmax;
  ...
  *factor = rnorm_0;
  for (i = 1; i <= nsing; ++i) {
    *factor += sing_(ityp, singar[i - 1].pos, singar[i - 1].width,
                     singar[i - 1].power, *xout) /
               (bi[i] - ai[i]);
  }
  *factor = (tint0 + nsing) / *factor;
} /* regfun_ */

void regfun_(int itype, int nsing, sing_struct *singar, double xmin,
             double xmax, double xx, double *xout, double *factor) {
  regfun_0_(0, itype, nsing, singar, xmin, xmax, xx, xout, factor);
}

void regfct_(int itype, int nsing, sing_struct *singar, double xmin,
             double xmax, double xout, double *factor) {
