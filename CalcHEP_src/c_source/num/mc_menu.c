...
static void f7_prog(int mode) {
  ...
  for (;;) {
    static double xMin = 1E-5, xMax = 1.0, q0 = 91.187, qMin = 1.5, qMax = 1.E4,
                  x0 = 0.1;
    ...
    if (LOG)
      improveStr(strmen, "LOG", "ON");
    else
      improveStr(strmen, "LOG", "OFF");
    ...
    if (!sf_num[1])
      on[1] = 0;
    if (sf_num[0] == 0 && sf_num[1] == 0)
      return;

    if (both)
      improveStr(strmen, "BOTH", "ON");
    else
      improveStr(strmen, "BOTH", "OFF");
    ...
    case 11: {
      ...
      if (z1 >= 0 && (!LOG || (z2 / z1 > 10)) && z2 > z1 && nPoints >= 3 &&
          nPoints <= 150) {
        int l, i;
        ...
        for (l = 0; l < 2; l++)
          if (on[l]) {
            ...
            sprintf(p_name[l] + strlen(p_name[l]), "(%s)",
                    pinf_int(Nsub, l + 1, NULL, NULL));
            //              sprintf(p_name[l],"pdf%d(%s)",
            //              l+1,pinf_int(Nsub,l+1,NULL,NULL) );
            for (i = 0; i < nPoints; i++) {
              double x = x0, q = q0, z, al;
              al = (i + 0.5) / (double)nPoints;
              if (LOG)
                z = pow(z1, 1 - al) * pow(z2, al);
              else
                z = z1 + al * (z2 - z1);
        ...
        switch (pos) {
        case 9:
          sprintf(title + strlen(title), " x*F(x,Q=%.2E)", q0);
          break;
        case 10:
          sprintf(title + strlen(title), " F(x,Q= %.2E)", q0);
          break;
        case 11:
          sprintf(title + strlen(title), " F(x=%.2E,Q)", x0);
          break;
        }
        if (pos == 11)
          xName = "Q";
          ...
          xName = "x";
        if (on[0] && on[1])
          plot_N(title, z1, z2, xName, LOG, 2, nPoints, f[0], NULL, p_name[0],
                 nPoints, f[1], NULL, p_name[1]);
        else {
          if (on[0])
            l = 0;
          else
...
static int in_setting(void) {
  ...
  for (;;) {
    ...
    if (is_polarized(1, Nsub))
      improveStr(strmen, "FirstPol", "Helicity of  first particle   %.3G",
                 (double)Helicity[0]);
    else
      improveStr(strmen, "FirstPol", "First  particle unpolarized");
    if (is_polarized(2, Nsub))
      improveStr(strmen, "SecondPol", "Helicity of second particle   %.3G",
                 (double)Helicity[1]);
    else
      improveStr(strmen, "SecondPol", "Second particle unpolarized");

    strFunName(1, sf_txt);
    improveStr(strmen, "First", "%-45.45s", sf_txt);
    strFunName(2, sf_txt);
    improveStr(strmen, "Second", "%-45.45s", sf_txt);
    ...
    case 3:
      ...
      break;
    case 4:
      correctDouble(50, 12, "Enter new value ", &inP2, 1);
      returnCode = returnCode | 1;
      break;
    case 5:
      if (is_polarized(1, Nsub)) {
        ...
        sprintf(txt, "Enter new value [%.1f,%.1f] :", -(spin2 / 2.),
                (spin2 / 2.));
...
L1 : {
  ...
  case 0:
    return 0;
  case 1:
    correctDouble(40, 18, "Breit-Wigner range = ", BWrange_int, 1);
    if (*BWrange_int < 0)
      *BWrange_int = -*BWrange_int;
    if (*BWrange_int > 999.9)
      *BWrange_int = 999.9;
    break;
  case 2:
    *twidth_int = !*twidth_int;
    break;
  case 3:
    *gtwidth_int = !*gtwidth_int;
    break;
  case 4:
    *gswidth_int = !*gswidth_int;
    break;
  case 5:
    useSLHAwidth = !useSLHAwidth;
    break;
  }
  goto L1;
} /* w_men__ */
...
static int checkEnergy(void) {
  ...
  if (nin_int == 1) {
    ...
    if (sf_num[0] && sf_mass[0] > m1)
      m1 = sf_mass[0];
    if (sf_num[1] && sf_mass[1] > m2)
      m2 = sf_mass[1];
...
static void infor(void) {
  ...
  if (first) {
    ...
    print(" #IT %s Error[%%]  nCall    Eff.  chi^2",
          nin_int == 2 ? "Cross section[pb]" : "   Width[GeV]    ");
  }
  if (integral.old) {
    print("(continue)");
    if (integral.n_it > 0 && first) {
      ...
      print(" < >   %12.4E %10.2E %8d %7.7s %-7.1G", integral.In,
...
static int mappingMenu(void) {
  ...
  for (;;) {
    ...
    case 2:
      do
        r = r | (2 * edittable(1, 4, &regTab, 1, "n_reg", 0));
      while (fillRegArray());
      break;
    }
  }
...
int monte_carlo_menu(void) {
  ...
  if (nout_int != 2)
    menutxt[menutxt[0] * 10 + 1] = 0;
  if (nin_int == 1)
    improveStr(menutxt, "Easy", "Total width");
  else
    improveStr(menutxt, "Easy", "1D integration");
  ...
  for (;;) {
    ...
    if (mode == 1 || mode == 2 || mode == 3 || mode == 5 || mode == 7)
      f3_key[7] = NULL;
    ...
    case 7:
      do
        r = r | (3 * edittable(1, 4, &compTab, 1, "n_comp", 0));
      while (fillCompositeArray());
      break;
    case 8:
      do
    ...
    case 10:
      if (nout_int == 1 && !sf_num[0] && !sf_num[1]) {
        if (blind)
          return 1;
        messanykey(15, 15,
                   "Phase space integration for 2->1 processes\n needs "
    ...
    case 11:
      if (mode == 11) {
        ...
        if (blind)
          return 3;
        messanykey(15, 15, "Can not evaluate regularization paremeters");
        break;
      }
