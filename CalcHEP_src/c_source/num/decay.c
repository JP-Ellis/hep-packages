...
void decay_0(int nvin, REAL amm1, REAL amm2, double *factor, double *Emax,
             iDecay *memDecay, REAL *V) {
  REAL summ, diffm;
  ...
  if (vv < 0)
    printf("decay0: vv=%e m1=%e m2=%e\n", vv, amm1, amm2);
  if (vv < 0) {
    vv = 0;
    fprintf(stderr, "decay_0: negative incoming mass\n");
  }
  v0 = sqrt(vv);
  e0v = 1 / v0;
  /* Computing 2nd power */
  ...
  if (v0 < summ) {
    pout = 0;
    fprintf(stderr, "decay_0: virtual decay\n");
  } else
    pout = sqrt((v0 - summ) * (v0 + summ) * (v0 - diffm) * (v0 + diffm)) /
           (v0 * 2);
...
void decay_2(int nvout1, double *parcos, iDecay *memDecay, REAL *V) {
  ...
  *parcos =
      (po * vv - vp * vo) / sqrt((pp * vv - vp * vp) * (oo * vv - vo * vo));
}

void decay_3(int nvath, REAL parcos, REAL parfi, int nvout1, int nvout2,
             iDecay *memDecay, REAL *V) {
  int i;
  ...
  /*
     if((am1!=0 && vdot4(nvout1, nvout1,V)<0)||(am2!=0 && vdot4(nvout2,
     nvout2,V)<0 )  ) {  printf("nvout1^2=%E am1=%E\n",vdot4(nvout1,
     nvout1,V),am1); printf("nvout2^2=%E am2=%E\n",vdot4(nvout2, nvout2,V),am2);
        printf("v0=%E\n", v0);
        printf("pout=%e\n", pout);
        printf(" in={%e %e %e
     %e}\n",V[0+4*n0-4],V[1+4*n0-4],V[2+4*n0-4],V[3+4*n0-4]); printf(" out={%e
     ...
     V[0+4*n1-4],V[1+4*n1-4],V[2+4*n1-4],V[3+4*n1-4]); printf(" azim V= {%e %e
