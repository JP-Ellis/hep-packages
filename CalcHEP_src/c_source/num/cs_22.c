...
static void calccoef(void) {
  ...
  for (i = 0; i < 4; i++)
    pinf_int(Nsub, i + 1, pmass + i, NULL);
...
static double cross_section(double x) {
  ...
  r = sqme_int(Nsub, GG, pvect4, NULL, &err_code) *
      usrFF(2, 2, pvect4_, p_names, p_codes);
  if (err_code != 0)
    return 0;
  return r * totcoef;
}
...
static void drawgraph(void) {
  ...
  calccoef();
  if (err_code) {
    messanykey(10, 10, "Error in kinematics");
    return;
  }
  ...
  do {
    if (correctInt(56, 8, "Number of points=", &n, 1)) {
      if (n < 3)
        messanykey(56, 8, "Too few points");
      if (n > 201)
        messanykey(56, 8, "Too many points");
    } else
      return;
  } while (n < 3 || n > 201);
  ...
  plot_1(cos1, cos2, n, f, NULL, procname, "cos(p1,p3)",
...
static double totcs(void) {
  ...
  calccoef();
  if (err_code == 0)
    int_val = gauss345(cross_section, cos1, cos2, eps, &err_code);
  return int_val;
}
...
int cs_numcalc(double Pcm) {
  ...
  sprintf(procname, "%s,%s ->%s,%s", pinf_int(k, 1, NULL, NULL),
          pinf_int(k, 2, NULL, NULL), pinf_int(k, 3, NULL, NULL),
          pinf_int(k, 4, NULL, NULL));
  ...
  do {
    ...
    case 2:
      ...
      break;
    case 3:
      recalc = correctDouble(15, 10, "Min[cos(p1,p3)]=", &cos1, 1);
      break;
    case 4:
      recalc = correctDouble(15, 10, "Max[cos(p1,p3)]=", &cos2, 1);
      break;
    case 5:
      if (err_code > 1)
        errormessage();
    ...
    case 6:
      ...
      break;
    case 7:
      paramdependence(vtotcs, procname, "v*sigma[pb]");
      break;
    } /*  switch  */
    if (k > 0)
      writeinformation();
