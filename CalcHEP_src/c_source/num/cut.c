...
static void cutInit(void) {
  int i;
  for (i = 0; i < nCuts; i++) {
    cleanPVlist(invcut_1[i].pLists);
    invcut_1[i].pLists = NULL;
  }
  nCuts = 0;
}

static int addcut(int Mirr, char *title, int minonw, int maxonw, double cvminw,
                  double cvmaxw) {
  int ok;
  if (nCuts >= 60)
    return 2;
...
int wrtcut_(FILE *nchan) {
  fprintf(nchan, "\n");
  writetable0(&cutTab, nchan);
  return 0;
}
int rdrcut_(FILE *nchan) {
  fscanf(nchan, "\n");
...
int fillCutArray(void) {
  ...
  for (ln = cutTab.strings; ln; ln = ln->next) {
    ...
    maxStr[0] = 0;
    lineNum++;
    sscanf(ln->line, "%[^|]%*c%[^|]%*c%[^|]%*c%[^|]", auxStr, cutStr, minStr,
           maxStr);
    trim(auxStr);
    trim(cutStr);
    trim(minStr);
    ...
    minOn = strlen(minStr);
    if (minOn) {
      if (calcExpression(minStr, rd_num, &min_))
        goto errorExit;
    }
    ...
    maxOn = strlen(maxStr);
    if (maxOn) {
      if (calcExpression(maxStr, rd_num, &max_))
        goto errorExit;
    }
    ...
    if ((minOn || maxOn) && aux != 0) {
      ...
      case 2:
        ...
        goto errorExit;
      case 3:
        strcpy(fieldName, "Too long cut identifier");
        goto errorExit;
      case 4:
        strcpy(fieldName, "Error in parameter");
        goto errorExit;
      }
    }
  }
...
void rancor_t(double *cosmax, double hsum, double fmult, double Ecm, double mq,
              double pcmtilda, double mp1, double ptMin) {
  ...
  if (Ecm * mq > Eq * sqrt(mt2))
    Ecm = Eq * sqrt(mt2) / mq;

  Sinv = mq * mq + mp1 * mp1 -
         2 * (Ecm * Ecm * mq * mq + pcmtilda * pcmtilda * mt2) /
...
double calcCutFactor(double *V) {
  ...
  for (i = 0; i < nCuts; i++) {
      ...
      default:
        ...
        if (invcut_1[i].aux == -1) {
          if (invcut_1[i].minon && invcut_1[i].maxon) {
            if (val >= invcut_1[i].cvmin && val <= invcut_1[i].cvmax)
              return 0;
          } else if (invcut_1[i].minon) {
            if (val >= invcut_1[i].cvmin)
              return 0;
          } else if (val <= invcut_1[i].cvmax)
            return 0;
        }
      }
    }
    if (invcut_1[i].key[1]) {
      if (invcut_1[i].aux == 1) {
        if (invcut_1[i].minon && (valM < invcut_1[i].cvmin))
          return 0;
        if (invcut_1[i].maxon && (valM > invcut_1[i].cvmax))
          return 0;
      } else if (invcut_1[i].aux == -1) {
        if (invcut_1[i].minon && invcut_1[i].maxon) {
          if (valM >= invcut_1[i].cvmin && valM <= invcut_1[i].cvmax)
            return 0;
        } else if (invcut_1[i].minon) {
          if (valM >= invcut_1[i].cvmin)
            return 0;
        } else if (valM <= invcut_1[i].cvmax)
          return 0;
      }
    }
  }
