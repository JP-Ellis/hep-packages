...
static void f5_key_prog(int x) {
  ...
  while (kmenu) {
    ...
    if (regenEvents)
      improveStr(strmen, "OF_", "OFF");
    else
      improveStr(strmen, "OF_", "ON");
...
static void f8_key_prog(int x) {
  static char FUNC[75] =
      "2*2  % Press ESC to finish, F1 for help, ENTER to calculate";
  int npos = 1;
  void *pscr;
...
int main(int argc, char **argv) {
...

  {
    ...
    if (!calchepDir)
      calchepDir = interface_ext.CALCHEP;
  ...
  goto_xy(10, 10);
  print("Calculation of constraints.  Please, be patient.");
  escpressed();
  n_comphep();
  finish();
  sortie(0);
