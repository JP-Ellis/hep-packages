...
void paramdependence(r_func ff, char *procname, char *resultname) {
  ...
  char name[20];
  REAL *vPos, memprm;
  if (!selectParam(54, 11, "Choose parameter", NULL, nin_int == 2, 0, 1, 0,
                   &vPos, name, &mPos))
    return;
...
label1:
  ...
  if (!correctDouble(55, 14, txt, &minprm, 0))
    return;
...
label4:
  ...
  strcpy(txt, name);
  if (err_code <= 1 && Esc == 0)
    plot_1(minprm, maxprm, npoints, f, NULL, procname, txt, resultname);
  *vPos = memprm;
  checkParam();
}
