...
static void exportParam(void) {
  ...
  if (link == NULL) {
    ...
    for (i = 0; i < nvar_int; i++) {
      for (j = 0; j < nModelVars + nModelFunc; j++)
        if (strcmp(varNames[j], varName_int[i + 1]) == 0) {
          link[i] = varValues + j;
          break;
        }
    }
  ...
  for (i = 0; i < nvar_int; i++)
    if (link[i])
      va_int[i + 1] = *link[i];
    else if (strcmp("i", varName_int[i + 1]))
      printf(" '%s' not found\n", varName_int[i + 1]);
  cleanDecayTable();
}
...
static void param_menu(int sqtrS_on, int rd_on, int vars_on, int func_on,
                       int *polar, char **strmen) {
  int k, pos, npos;
  npos = 0;
  if (sqtrS_on) {
    npos++;
    for (k = 0; k < 2; k++)
      if (is_polarized(k + 1, Nsub)) {
        polar[k] = 1;
        npos++;
      } else
        polar[k] = 0;
  ...
  *strmen = malloc(24 * npos + 2);
  ...
  for (k = 0; k < 2; k++)
    if (polar[k]) {
      sprintf((*strmen) + pos, " Helicity%d  %-12.4g", k + 1,
              (double)Helicity[k]);
      pos += 24;
    }
  if (sqtrS_on) {
    sprintf((*strmen) + pos, "  Pcm       %-12.4g", (double)Pcm22);
    pos += 24;
  }
  if (rd_on) {
  ...
  if (vars_on)
    for (k = 0; k < nModelVars; k++) {
      sprintf((*strmen) + pos, " %-11s%12.4E", varNames[k],
              (double)varValues[k]);
      pos += 24;
    }

  if (func_on)
    for (k = nModelVars; k < nModelVars + nModelFunc; k++) {
      sprintf((*strmen) + pos, " %-11s%12.4E", varNames[k],
              (double)varValues[k]);
      pos += 24;
    }
...
int selectParam(int x, int y, char *mess, void **pscrPtr, int sqtrS_on,
                int rd_on, int vars_on, int func_on, REAL **varPos,
                char *varName, int *mPos) {
  char *strmen;
  void *pscr;
  int polar[2];
  int position = 1;
  ...
  if (strmen) {
    ...
    position = *mPos;
    if (position)
      sscanf(strmen + 1 + (position - 1) * strmen[0], "%s", varName);
    free(strmen);
    if (position == 0) {
      if (pscrPtr)
        *pscrPtr = NULL;
    ...
    if (func_on && sqtrS_on == 0 && rd_on == 0 && vars_on == 0) {
      position--;
      if (!position) {
        varPos = NULL;
        return 1;
...
int change_parameter(int x, int y, int for22) {
  ...
  for (i = 0; i < nModelVars; i++)
    va_mem[i] = varValues[i];

  for (err = 1; err;) {
    for (; selectParam(x, y, "Change parameter", &pscr, for22, 1, 1, 0, &vPos,
                       name, &mPos);) {
      if (vPos == varValues - 1) {
        FILE *f;
        ...
        if (!findCalcHEPfile(fName))
          continue;
        if (stat(fName, &buf) || !(S_ISREG(buf.st_mode))) {
          messanykey(10, 17, "Not a regular file");
          continue;
        }
        f = fopen(fName, "r");
        if (f == NULL) {
          messanykey(10, 17, "Can not open file");
          continue;
        }
        for (;;) {
          char name[20];
    ...
    if (returnCode) {
      ...
      print("Be patient:");
      goto_xy(x, y + 3);
      print("Calculation of constraints");
      escpressed();
      err = checkParam();
      if (Warnings)
        messanykey(5, 10, Warnings);
      put_text(&pscr);
    } else
      err = 0;
    if (err && mess_y_n(10, 10, "Restore previous parameter set?")) {
      for (i = 0; i <= nModelVars; i++)
        varValues[i] = va_mem[i];
      Pcm22 = Pcm_mem;
    }
  }
  free(va_mem);
...
void show_depend(int x, int y) {
  ...
  if (!nModelFunc) {
    messanykey(5, 10, "There are no public constraints in this model");
    return;
  }
  REAL *allfunc = (REAL *)malloc(sizeof(REAL) * nModelFunc);
  for (i = 0; i < nModelFunc; i++)
    allfunc[i] = varValues[1 + nModelVars + i];
  ...
  for (;;) {
    ...
    if (!selectParam(x, y + 1, "Display dependence", &pscr1, 0, 0, 0, 1, &fPos,
                     name1, &mPos))
      return;
    ...
    if (mPos == 1) {
      ...
      for (int i = 0; i < nModelFunc; i++)
        fprintf(f, " %10.10s %E\n", varNames[i + nModelVars],
                varValues[i + nModelVars]);
      fclose(f);
      continue;
    }
    ...
    for (;;) {
      ...
      if (!selectParam(x, y + 5, "on parameter", &pscr2, 0, 0, 1, 0, &vPos,
                       name2, &mPos_))
        break;
      val2 = *vPos;
          ...
          if (xMax > xMin && nPoints >= 3 && nPoints <= 150) {
            ...
            for (i = 0; i < nPoints; i++) {
              ...
              *vPos = x;
              ...
              if (NaN) {
                char mess[100];
                sprintf(mess, "Can not evaluate constraints for %s=%G", name2,
                        x);
            ...
            *vPos = val2;
            for (i = 0; i < nModelFunc; i++)
              varValues[1 + nModelVars + i] = allfunc[i];

            if (!(NaN || Esc))
              plot_1(xMin, xMax, nPoints, f, NULL, "Parameter dependence",
                     name2, name1);
