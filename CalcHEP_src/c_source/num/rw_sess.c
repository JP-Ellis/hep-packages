...
static int writeIntegral(FILE *f) {
  fprintf(f, " %.17E %.17E %.17E %d %d %ld %d %d", integral.s0, integral.s1,
          integral.s2, integral.n_it, integral.old, integral.nCallTot,
          integral.freeze, integral.tp);
  return 0;
}

static int readIntegral(FILE *f) {
  fscanf(f, " %lf %lf %lf %d %d %ld %d %d", &(integral.s0), &(integral.s1),
         &(integral.s2), &(integral.n_it), &(integral.old),
         &(integral.nCallTot), &(integral.freeze), &(integral.tp));
  return 0;
}

static int wnsess_(FILE *mode) {
  fprintf(mode, "%d", nSess);
  return 0;
}
static int rnsess_(FILE *mode) {
  fscanf(mode, "%d", &nSess);
  return 0;
}
...
static int w_VVdecays(FILE *mode) {
  fprintf(mode, " %d\n", VWdecay);
  return 0;
}

static int r_VVdecays(FILE *mode) {
  fscanf(mode, " %d", &VWdecay);
  VZdecay = VWdecay;
  return 0;
}
...
int is_polarized(int k, int nsub) {
  char name[20];
  if (nin_int == 1 || k > 2 || k < 1)
    return 0;
  sprintf(name, ",%s,", pinf_int(nsub, k, NULL, NULL));
  if (strstr(polarized_int[k], name))
    return 1;
  else
    return 0;
}
...
int wrtprc_(void) /* write process string */
{
  ...
  for (i = 0; i < nin_int + nout_int; i++)
    p_names[i] = pinf_int(Nsub, i + 1, NULL, p_codes + i);
...
int w_sess__(FILE *mode_) {
  ...
  rw_paragraph wrt_array[NITEMS] = {
      {"Subprocess", w_prc__},         {"Session_number", wnsess_},
      {"Initial_state", w_in__},       {"Physical_Parameters", w_mdl__},
      {"Breit-Wigner", w_widths},      {"VVdecays", w_VVdecays},
      {"alphaQCD", w_alphaQCD},        {"QCDscales", w_Scales},
      {"Composites", wrtcomp_},        {"Cuts", wrtcut_},
      {"Parallelization", wrtNcores},  {"Distributions", wrt_hist},
      {"Kinematical_scheme", wrtkin_}, {"Regularization", wrtreg_},
      {"Vegas_calls", w_mc__},         {"Vegas_integral", writeIntegral},
      {"Events", saveEventSettings},   {"Random", saveRandom},
  ...
  if (mode_ == NULL) {
    ...
    mode = fopen(fname, "w");
    if (mode) {
      writeParagraphs(mode, 9, wrt_array + 2);
      fclose(mode);
    }
  } else
    writeParagraphs(mode, 10, wrt_array);
...
int r_sess__(FILE *mode_) {
  ...
  rw_paragraph rd_array[NITEMS] = {
      {"Subprocess", r_prc__},          {"Session_number", rnsess_},
      {"Initial_state", r_in__},        {"Physical_Parameters", r_mdl__},
      {"Breit-Wigner", r_widths},       {"VVdecays", r_VVdecays},
      {"alphaQCD", r_alphaQCD},         {"QCDscales", r_Scales},
      {"Composites", rdrcomp_},         {"Cuts", rdrcut_},
      {"Parallelization", rdNcores},    {"Distributions", rdr_hist},
      {"Regularization", rdrreg_},      {"Kinematical_scheme", rdrkin_},
      {"Vegas_integral", readIntegral}, {"Vegas_calls", r_mc__},
      {"Events", readEventSettings},    {"Random", readRandom},
