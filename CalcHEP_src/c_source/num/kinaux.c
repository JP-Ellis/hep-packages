...
void lvmirr_(char *lv) {
  ...
  strcpy(buff, lv);
  for (i1 = 1; i1 <= nin_int + nout_int; i1++) {
    if (!strchr(buff, i1))
      lv[i2++] = i1;
  }
  lv[i2] = 0;
...
void coninv_(char *lv) {
  ...
  if (2 * strlen(lv) < nin_int + nout_int)
    return;
  if ((2 * strlen(lv) == nin_int + nout_int) && lv[0] == 1)
    return;
...
static int isknown(char *lv, int level) {
  ...
  if (eqvect_(lv, kinmtc_1[level - 1].lvin))
    return 0;
  ...
  for (n = 0; n < len; n++) {
    ...
    for (k = 0; k < 2; k++)
      for (j = 0; j < level - 1; j++) {
        if (kinmtc_1[j].lvout[k][1] == 0 && kinmtc_1[j].lvout[k][0] == lv[n])
          icmp = 0;
      }

    for (k = 1; k <= nin_int; ++k)
      if (k == lv[n] || lv[n] == numbx)
        icmp = 0;
    if (icmp)
      return 0;
  }
...
void sngpos_(char *lv, int *ndec, int *nclust, char *lvaux) {
  ...
  for (i = 0; i < nout_int - 1; ++i) {
    for (k = 0; k < 2; k++) {
      ...
      if (!ierr[k])
        ierr[k] = !isknown(lvaux_cp[k], i + 1);
      if (!ierr[k])
        ln[k] = strlen(kinmtc_1[i].lvout[k]) + strlen(lvaux_cp[k]);
    }
    if (!(ierr[0] && ierr[1])) {
      *ndec = i + 1;
      if ((!ierr[0]) && (!ierr[1]))
