...
void fillHists(double w, double *V) {
  ...
  for (; hists; hists = hists->next) {
    ...
    if (!w || !p)
      continue;
    for (n0 = 0; p; p = p->next, n0++)
      z0[n0] = calcPhysVal(hists->key[0][0], p->pstr, V);
    ...
    if (hists->key[1][0] == '0') {
      for (k = 0; k < n0; k++) {
        double id =
            300 * (z0[k] - hists->hMin[0]) / (hists->hMax[0] - hists->hMin[0]);
        if (!isfinite(id) || id < 0 || id >= 300)
          continue;
        i = id;
        hists->f[i] += w;
        hists->ff[i] += w * w;
    ...
    if (!p)
      continue;
    for (n1 = 0; p; p = p->next, n1++)
      z1[n1] = calcPhysVal(hists->key[1][0], p->pstr, V);
    ...
    for (k0 = 0; k0 < n0; k0++) {
      double di0 =
          30 * (z0[k0] - hists->hMin[0]) / (hists->hMax[0] - hists->hMin[0]);
      if (!isfinite(di0) || di0 < 0 || di0 >= 30)
        continue;
      i0 = di0;
      for (k1 = 0; k1 < n1; k1++) {
        double di1 =
            30 * (z1[k1] - hists->hMin[1]) / (hists->hMax[1] - hists->hMin[1]);
        if (!isfinite(di1) || di1 < 0 || di1 >= 30)
          continue;
        i1 = di1;
        hists->f[30 * i0 + i1] += w;
...
int correctHistList(void) {
  ...
  for (ln = histTab.strings, lineNum = 1; ln; ln = ln->next, lineNum++) {
    ...
    for (hptr = histPtr; hptr; hptr = hptr->next)
      if (hptr->mother == NULL && strcmp(hptr->title[0], histStr[0]) == 0 &&
        ...
        if (strlen(histStr[1]))
          ok = ok & checkPhysValN(histStr[1], hptr->key[1], hptr->pList + 1);
        else {
          strcpy(hptr->key[1], "0");
          hptr->pList[1] = NULL;
        }
    ...
    for (hists = histPtr; hists; hists = hists->next)
      if (hists->mother == rec) {
        ...
        for (i = 0; i < 2; i++) {
          ...
          fprintf(iprt, " hMin=%-12E hMax=%-12E", hists->hMin[i],
                  hists->hMax[i]);
        }
        if (strcmp(hists->key[1], "0") == 0)
          Nprint = 300;
        else
          Nprint = 900;
        fprintf(iprt, "\n  f: ");
  ...
  for (rec = histTab.strings; rec; rec = rec->next) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (nR == 0)
        strcpy(hist->title[i], "");
      fscanf(iprt, "\" ");
      if (strlen(hist->title[i]))
        checkPhysValN(hist->title[i], hist->key[i], hist->pList + i);
      else {
        strcpy(hist->key[i], "0");
        hist->pList[i] = NULL;
      }
...
int wrt_hist2(FILE *fi, char *comment) {
  ...
  char minRec1[200], minRec2[200], maxRec1[200], maxRec2[200], nameRec1[200],
      nameRec2[200];
  if (comment)
    fprintf(fi, "%s\n", comment);
  else
  ...
  for (ll = histTab.strings; ll; ll = ll->next) {
    ...
    sscanf(ll->line, "%[^|]|%[^|]|%[^|]|%[^|]|%[^|]|%[^|]", nameRec1, minRec1,
           maxRec1, nameRec2, minRec2, maxRec2);
...
int add_hist(FILE *f, char **procname) {
  ...
  if (strcmp(histTab.format, histTab2.format)) {
    int l[6], l2[6];
    char *ch, *ch2;
    for (i = 0, ch = histTab.format, ch2 = histTab2.format; i < 6;
         i++, ch++, ch2++) {
      ch = strchr(ch, '|');
      l[i] = ch - histTab.format;
      ch2 = strchr(ch2, '|');
      l2[i] = ch2 - histTab2.format;
    }
  ...
  for (r2 = histPtr2; r2; r2 = r2->next)
    for (r = histPtr; r; r = r->next)
      if (r->mother && strcmp2(r->mother->line, r2->mother->line) == 0) {
        ...
        if (n1) {
          ...
          if (n2)
            for (i = 0; i < Ntot; i++) {
              r2->ff[i] =
                  (n / n1) * (n / n1) * (r->ff[i] - r->f[i] * r->f[i] / n1) +
        ...
        r2->nPoints = n;
        if (r->mother->next)
          r->mother->next->pred = r->mother->pred;
        if (r->mother->pred)
          r->mother->pred->next = r->mother->next;
        else
          histTab.strings = r->mother->next;
        free(r->mother);
        r->mother = NULL;
        break;
      }
  ...
  qLl = histTab.strings;
  if (qLl) {
    for (; qLl->next; qLl = qLl->next)
      continue;
    qLl->next = histTab2.strings;
    histTab2.strings = histTab.strings;
    for (histPtr_ = histPtr; histPtr_->next; histPtr_ = histPtr_->next)
      continue;
...
void showHist(int X, int Y, char *title) {
  ...
  for (ln = histTab.strings, width1 = 0, width2 = 0; ln;
       ln = ln->next, npos++) {
  ...
  for (ln = histTab.strings; ln; ln = ln->next) {
    ...
    trim(histStr2);
    if (width2)
      sprintf(menutxt + strlen(menutxt), " %-*.*s| %-*.*s", width1, width1,
              histStr1, width2, width2, histStr2);
    else
      sprintf(menutxt + strlen(menutxt), " %-*.*s  ", width1, width1, histStr1);
  }
      ...
      if (hist) {
        ...
        if (hist->nPoints == 0)
          ...
          while ((nBin1 = nBinMenu(X, Y + 4))) {
            ...
            plot_1(hist->hMin[0], hist->hMax[0], nBin1, f, df, title, xname,
                   yname);
          }
        else
          while (nBinMenu2(X, Y + 4, &nBin1, &nBin2)) {
            ...
            int i, j;
            coeff = nBin1 * nBin2 /
                    (hist->nPoints * (hist->hMax[0] - hist->hMin[0]) *
            ...
            for (i = 0; i < nBin1; i++)
              for (j = 0; j < nBin2; j++) {
                ...
                f[pos] = 0;
                df[pos] = 0;
                for (k = 0; k < 30 / nBin1; k++)
                  for (l = 0; l < 30 / nBin2; l++) {
                    f[pos] +=
                        coeff *
                        hist->f[30 * (i * 30 / nBin1 + k) + j * 30 / nBin2 + l];
                    df[pos] += coeff * coeff *
                               hist->ff[30 * (i * 30 / nBin1 + k) +
                                        j * 30 / nBin2 + l];
                  }
            ...
            xUnit(hist->key[0][0], units);
            if (units[0])
              sprintf(xname + strlen(xname), "[%s]", units);
            xUnit(hist->key[1][0], units);
            if (units[0])
              sprintf(yname + strlen(yname), "[%s]", units);

            plot_2D(hist->hMin[0], hist->hMax[0], nBin1, hist->hMin[1],
                    hist->hMax[1], nBin2, f, df, title, xname, yname);
          }
      }
    }
    }
