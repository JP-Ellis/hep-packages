  ...
  if (totwidth > 0.) {
    ...
    for (i = 0, xcount = 5, ycount = 7; i < nprc_int; i++, ycount++)
      if (widths[sort[i]] > 0) {
        ...
        scrcolor(FGmain, BGmain);
        if (Branchings)
          print("%8.2E%%", 100 * widths[sort[i]] / totwidth);
        else
          print("%9.3E", widths[sort[i]]);
      }
    free(sort);
  }
  scrcolor(FGmain, BGmain);
  ...
  for (nsub = 1; nsub <= nprc_int; nsub++) {
    ...
    if (strcmp(pinf_int(nsub, 1, &m1, &N1), inParticle) == 0) {
      if (first) {
        ...
        if (Q)
          GG = sqrt(4 * M_PI * alpha_2(*Q));
        else
          GG = sqrt(4 * M_PI * alpha_2(m1));
        first = 0;
      }
      ...
      if (m1 <= m2 + m3)
        ...
        widths[nsub - 1] =
            totcoef * sqme_int(nsub, GG, pvect3, NULL, &err_code);
        if (err_code != 0) {
          errormessage();
          widths[nsub - 1] = 0;
          err_code = 0;
        ...
        if (nsubSel == nsub)
          selChan = widths[nsub - 1];
      }
    }
  }
  if (nsubSel) {
    if (width12)
      return selChan / width12;
    else
      return 0;
  }
  return width12;
...
static void *selectChan(void) {
  ...
  for (nsub = 1, ntot = 1; nsub <= nprc_int; nsub++)
    if (strcmp(pinf_int(nsub, 1, NULL, NULL), inParticle) == 0) {
      sprintf(menutxt + 1 + 15 * ntot++, " BR(%-3.3s , %-3.3s) ",
              pinf_int(nsub, 2, NULL, NULL), pinf_int(nsub, 3, NULL, NULL));
    }

  for (ntot = 0; ntot == 0;)
    menu1(56, 7, "Select", menutxt, "", &pscr, &ntot);
  if (ntot == 0)
    return NULL;
  ntot--;
  if (ntot == 0)
...
void decay12(void) {
  ...
  for (k = 1; k;) {
    ...
    if (EffQmass)
      improveStr(strmen, "Free ", "M1");
    if (!Branch)
      improveStr(strmen, "Branchings", "Partial widths");
    ...
    case 1: {
      if (strlen(mlist) > L + 2) {
        ...
        if (k)
          sscanf(mlist + (k - 1) * L + 1, "%s", inParticle);
        put_text(&pscr2);
      }
    } break;
    case 2:
    ...
    case 6: {
      ...
      if (nsubSel == 0)
        ...
        sprintf(proc, " BR(%s ->  %s %s)", inParticle,
                pinf_int(nsubSel, 2, NULL, NULL),
                pinf_int(nsubSel, 3, NULL, NULL));
        dimInfo[0] = 0;
      }
