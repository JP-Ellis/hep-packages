...
double divy_(double y) {
  if (fabs(y) > 0.01)
    ...
    return 1 - y * (1 - y * (1 - y * (1 - y / 5) / 4) / 3) / 2;
} /* divy_ */
...
double dinter_(double x, int n, double *xi, double *yi) {
  ...
  if (x <= xi[2]) {
    ...
    x3 = xi[i + 1];
    x4 = xi[i + 2];
    return yi[i - 1] * (x - x2) * (x - x3) * (x - x4) /
               ((x1 - x2) * (x1 - x3) * (x1 - x4)) +
           yi[i] * (x - x1) * (x - x3) * (x - x4) /
               ((x2 - x1) * (x2 - x3) * (x2 - x4)) +
           yi[i + 1] * (x - x1) * (x - x2) * (x - x4) /
               ((x3 - x1) * (x3 - x2) * (x3 - x4)) +
           yi[i + 2] * (x - x1) * (x - x2) * (x - x3) /
               ((x4 - x1) * (x4 - x2) * (x4 - x3));
  }
} /* dinter_ */
...
static double conv_f(double x) {
  ...
  if (x == 0.) {
    ...
    z1 = 0.5 * pow(x, 1 / conv_b1), z2 = 0.5 * pow(x, 1 / conv_b2);
  }
  return (*conv_f1)(conv_x * z1) * (*conv_f2)(conv_x * (1 - z1)) *
             pow(1 - z1, conv_b2 - 1) * conv_c1 +
         (*conv_f2)(conv_x * z2) * (*conv_f1)(conv_x * (1 - z2)) *
             pow(1 - z2, conv_b1 - 1) * conv_c2;
}

double convol_(double (*f1)(double), double (*f2)(double), double b1, double b2,
               double x, double eps) {
  /*
