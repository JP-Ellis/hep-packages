...
static int readLnC(int size, char *buff) {
  int i;
  if (fortranreadline_(&_N_, buff, size - 1))
    return -1;
  i = size - 1;
  buff[i--] = 0;
  for (; i >= 0 && buff[i] == ' '; i--)
...
double slhawidth_(int *pNum) {
  double res = slhaWidth(*pNum);
  ferror_.ferror = FError;
  return res;
}

double slhabranch_(int *pNum, int *N, int *nCh) {
  double res = slhaBranch(*pNum, *N, nCh);
  ferror_.ferror = FError;
  return res;
}
...
double complex cslhaval3_(char *Block, double *Q, int *k1, int *k2, int *k3,
                          int len) {
  char c_name[200];
  double complex res;
  fName2c(Block, c_name, len);
...
int allqnumbers_(int *i, int *pdg, int *eQ3, int *spinDim, int *cDim,
                 int *neutral) {
  int k;
  int ret = allQnumbers(*i, pdg, eQ3, spinDim, cDim, neutral);
  strncpy(slhacomment_.txt, slhaComment, 100);
  for (k = strlen(slhaComment); k < 100; k++)
    slhacomment_.txt[k] = ' ';

  return ret;
}

int allblocks_(int *i, int *j, char *name, int *Len, int *key,
               double complex *val, int l) {
  char c_name[40];
...
int alldecays_(int *i, int *j, int *pdg, int *Len, int *decay, double *width,
               double *br) {
  int k, ret;
...
int aprintf4_(char *format, double *x1, double *x2, double *x3, double *x4,
              int len) {
  char c_name[1000];
  fName2c(format, c_name, len);
...
int aprintf5_(char *format, double *x1, double *x2, double *x3, double *x4,
              double *x5, int len) {
  char c_name[1000];
  fName2c(format, c_name, len);
...
double slhavalformat_(char *Block, double *Q, char *format, int len1,
                      int len2) {
...
int slhastrformat_(char *Block, char *format, char *fRes, int len1, int len2,
                   int len3) {
...
int rjacobi_(double *a, int *n, double *d, double *v) {
  return rJacobi(a, *n, d, v);
}

int rjacobia_(double *a, int *n, double *d, double *u, double *v) {
  return rJacobiA(a, *n, d, u, v);
}
int cjacobih_(double complex *a, int *n, double *d, double complex *v) {
  return cJacobiH(a, *n, d, v);
}
int cjacobis_(double complex *a, int *n, double *d, double complex *v) {
  return cJacobiS(a, *n, d, v);
}
int cjacobia_(double complex *a, int *n, double *d, double complex *u,
              double complex *v) {
...
int cdiagonalh4_(aList10(double complex *)) {
  return cDiagonalH(4, aList10(*));
}
int cdiagonalh5_(aList15(double complex *)) {
  return cDiagonalH(5, aList15(*));
}
...
int cdiagonals4_(aList10(double complex *)) {
  return cDiagonalS(4, aList10(*));
}
int cdiagonals5_(aList15(double complex *)) {
  return cDiagonalS(5, aList15(*));
}
...
int cdiagonala4_(aList16(double complex *)) {
  return cDiagonalA(4, aList16(*));
}
int cdiagonala5_(aList25(double complex *)) {
  return cDiagonalA(5, aList25(*));
}
...
double remixmatrix_(int *id, int *i, int *j) {
  return creal(cMixMatrix(*id, *i, *j));
}
double remixmatrixu_(int *id, int *i, int *j) {
  return creal(cMixMatrixU(*id, *i, *j));
}
double immixmatrix_(int *id, int *i, int *j) {
  return cimag(cMixMatrix(*id, *i, *j));
}
double immixmatrixu_(int *id, int *i, int *j) {
  return cimag(cMixMatrixU(*id, *i, *j));
}
