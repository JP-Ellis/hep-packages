...
static void extendData(int id, int nDim, int std) {
  ...
  if (std == 1 || std == 2) {
    ...
    if (rU[id] == NULL && std == 2)
      rU[id] = malloc(sizeof(REAL) * nDim * nDim);
  }
  ...
  if (std == 3 || std == 4) {
    ...
    if (cU[id] == NULL && std == 4)
      cU[id] = malloc(sizeof(COMPLEX) * nDim * nDim);
  }
}
...
int rDiagonal(int nDim, ...) {
  ...
  for (i = 0; i < nDim; i++)
    for (j = i; j < nDim; j++)
      MassM[sMT(i, j)] = va_arg(ap, REAL);
  // for(i=0;i<nDim;i++)for(j=i;j<nDim;j++) printf("%d %d %E\n",i,j,
  // (doubble)MassM[sMT(i,j)]);
  va_end(ap);
...
int cDiagonalH(int nDim, ...) {
  ...
  for (i = 0; i < nDim; i++)
    for (j = i; j < nDim; j++)
      MassM[sMT(i, j)] = va_arg(ap, COMPLEX);
  va_end(ap);
...
int cDiagonalA(int nDim, ...) {
  ...
  for (i = 0; i < nDim; i++)
    for (j = 0; j < nDim; j++)
      MassM[i * nDim + j] = va_arg(ap, COMPLEX);
  va_end(ap);
...
REAL MixMatrix(int id, int i, int j) {
  if (id > idMax || i < 1 || i > dim[id] || j < 1 || j > dim[id]) {
    FError = 1;
    return 0;
  }
...
REAL MixMatrixU(int id, int i, int j) {
  if (id > idMax || i < 1 || i > dim[id] || j < 1 || j > dim[id] || !rU[id]) {
    FError = 1;
    return 0;
  }
...
COMPLEX cMixMatrixU(int id, int i, int j) {
  if (id > idMax || i < 1 || i > dim[id] || j < 1 || j > dim[id] || !cU[id]) {
    FError = 1;
    return 0;
  }
