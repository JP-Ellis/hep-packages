...
void cleanSLHAdata(void) {
  ...
  while (qNumberList) {
    qNumberStr *qnb = qNumberList;
    qNumberList = qNumberList->next;
    free(qnb->txt);
    free(qnb);
  }
...
static void addWarning(char *txt) {
  if (Warnings)
    Warnings = (char *)realloc(Warnings, strlen(Warnings) + strlen(txt) + 4);
  else {
    Warnings = (char *)malloc(strlen(txt) + 2);
    Warnings[0] = 0;
  }
...
static int readLine(int size, char *buff) {
  ...
  for (;;) {
    ...
    for (i = 0; buff[i] && buff[i] == ' '; i++)
      ;
    if (buff[i] != 0 && buff[i] != '#') {
      if (_end_ && buff + i == strstr(buff + i, _end_))
        return -1;
...
int slhaBasicReader(int mode, int (*getLnPar)(int, char *), int *anydate,
                    char *end) {
  ...
  *anydate = 0;
  ...
  for (;;) {
    ...
    for (L = 0; L < 6 && buff[L] && toupper(buff[L]) == block[L]; L++)
      continue;
    if (L == 6) {
      int i;
        ...
        *anydate = 1;
        if (m16) {
          ...
          if (sscanf(buff + 6, "%*s %d", &val) != 1) {
            sprintf(wTxt, "SLHAreader: line %d No pdg code for Qnumbers",
                    nLine);
            addWarning(wTxt);
            if (m2) {
              cleanSLHAdata();
          ...
          for (;;) {
            ...
            case 4:
              ...
              break;
            default: {
              sprintf(wTxt, "SLHAreader: line %d: unexpected key for QNUMBERS",
                      nLine);
              addWarning(wTxt);
            }
            }
      ...
      {
        blockStr *newBlock;
        if (sscanf(buff + 6, "%*s %s", rest) == EOF)
          scale = -1;
        else if (sscanf(buff + 6, "%*s %*s %lf %s", &scale, rest) != 1) {
          sprintf(wTxt, "SLHAreader: line %d: Unexpected BLOCK specification",
                  nLine);
          addWarning(wTxt);
          if (m2) {
            cleanSLHAdata();
        ...
        if (m8)
          ...
          for (;;) {
            ...
            if (c[0] == ')') {
              for (; c > buff && c[0] != '('; c--)
                ;
              if (2 != sscanf(c, "( %Lf , %Lf)", &re, &im))
                err = 1;
            } else {
              char rest[StrLn];
              for (; c > buff && c[0] != ' '; c--)
                ;
              if (1 != sscanf(c, "%Lf%s", &re, rest))
                err = 1;
            }
            if (err)
              continue;
#ifdef OLD
            {
              sprintf(wTxt, "SLHAreader: line %d: Unexpected last token ",
                      nLine);
              addWarning(wTxt);
              if (m2) {
                cleanSLHAdata();
            ...
            for (c = strtok(buff, " "); c && nkey < KeyMLn;
                 c = strtok(NULL, " "), nkey++) {
              if (1 != sscanf(c, "%d%s", keys + nkey, rest)) {
                err = 1;
                break;
              }
            }
...
#ifdef OLD
            {
              sprintf(wTxt, "SLHAreader: line %d: Unexpected %d token", nLine,
                      nkey + 1);
              addWarning(wTxt);
              if (m2) {
                cleanSLHAdata();
    ...
    /* DECAY */
    {
      ...
      for (L = 0; L < 6 && buff[L] && toupper(buff[L]) == decay[L]; L++)
        continue;
      if (L != 6) {
      ...
      if (sscanf(buff + 6, "%d %lf %s", &pNum, &pWidth, rest) != 2) {
        sprintf(wTxt, "SLHAreader line %d : Unexpected DECAY specification",
                nLine);
        addWarning(wTxt);
        if (m2) {
          cleanSLHAdata();
      ...
      *anydate = 1;
      if (m4 || pNum == 23 || abs(pNum) == 24) {
        ...
        for (;;) {
          ...
          c = strtok(buff, " ");
          if (c)
            rdn = sscanf(c, "%lf%s", &(dr->Br), end);
          else
            rdn = 0;
          if (rdn == 1) {
            c = strtok(NULL, " ");
            if (c)
              rdn = sscanf(c, "%d%s", &(dr->nkey), end);
            else
              rdn = 0;
          }

          if (rdn == 1 && dr->nkey <= DecLen)
            for (i = 0, c = strtok(NULL, " "); i < dr->nkey && c && rdn == 1;
                 i++, c = strtok(NULL, " ")) {
...
static long double complex *slhaValAddress(char *Block, int nKey, int *keys) {
  ...
  while (blck && strcmp(BLOCK, blck->name))
    blck = blck->next;
  if (!blck)
    return NULL;
  dr = blck->dataList;
...
static long double complex cslhaVal_(char *Block, double Q, int nKey,
                                     va_list ap) {
  int keys[KeyMLn];
  int i;
  char BLOCK[BlckLn];
  ...
  if (strlen(Block) > BlckLn - 1) {
    FError = 1;
    return 0;
  }

  for (i = 0; i < nKey; i++)
    keys[i] = va_arg(ap, int);
    ...
    if (strcmp(BLOCK, blck->name) == 0) {
      ...
      if (blck->scale < 0) {
        if (found[0])
          ...
          pos = 0;
      } else if (blck->scale < Q) {
        if (found[1] && scale[1] > blck->scale)
          continue;
        else
          pos = 1;
      } else {
        if (found[2] && scale[2] < blck->scale)
          continue;
        else
          pos = 2;
      }
  ...
  if (found[2] == 0)
    return val[1];
  return (val[1] * log(scale[2] / Q) + val[2] * log(Q / scale[1])) /
         log(scale[2] / scale[1]);
}
...
int slhaDecayExists(int pNum) {
  decayStr *decay = decayList;
  for (; decay; decay = decay->next)
    if (abs(decay->pNum) == abs(pNum)) {
      decayRec *dr = decay->dataList;
      int n;
      for (n = 0; dr; dr = dr->next, n++)
        continue;
      return n;
    }
  return -1;
...
int allQnumbers(int i, int *pdg, int *eQ3, int *spinDim, int *cDim, int *anti) {
  ...
  if (i <= 0)
    return 0;
  for (i1 = 0, qNmb = qNumberList; qNmb; qNmb = qNmb->next)
    i1++;
  if (i1 < i)
    return 0;
  for (i1 -= i, qNmb = qNumberList; i1; i1--)
    qNmb = qNmb->next;
  if (pdg)
    *pdg = qNmb->pdg;
  if (spinDim)
...
int allBlocks(int i, int j, char *name, int *keyLen, int *key,
              double complex *val) {
  ...
  if (i <= 0 || j < 0)
    return 0;
  for (i1 = 0, bList = blockList; bList; bList = bList->next)
    i1++;
  if (i > i1)
    return 0;
  for (i1 -= i, bList = blockList; i1; i1--)
    bList = bList->next;

  if (name)
    strcpy(name, bList->name);
  if (j == 0) {
    if (keyLen)
      *keyLen = 0;
    if (val)
      *val = bList->scale;
    slhaComment = bList->txt;
    return 1;
  }

  for (j1 = 0, bRec = bList->dataList; bRec; bRec = bRec->next)
    j1++;
  if (j > j1)
    return 0;
  for (j1 -= j, bRec = bList->dataList; j1; j1--)
    bRec = bRec->next;
  if (keyLen)
    *keyLen = bRec->nkey;
  if (key)
    for (k = 0; k < bRec->nkey; k++)
      key[k] = bRec->keys[k];
  if (val)
    *val = bRec->val;
...
int allDecays(int i, int j, int *pdg, int *decayLen, int *decay, double *width,
              double *br) {
  ...
  if (i <= 0 || j < 0)
    return 0;
  for (i1 = 0, dList = decayList; dList; dList = dList->next)
    i1++;
  if (i > i1)
    return 0;
  for (i1 -= i, dList = decayList; i1; i1--)
    dList = dList->next;
  if (pdg)
    *pdg = dList->pNum;
  if (width)
  ...
  if (j == 0) {
    if (decayLen)
      *decayLen = 0;
    slhaComment = dList->txt;
    return 1;
  }

  for (j1 = 0, dRec = dList->dataList; dRec; dRec = dRec->next)
    j1++;
  if (j > j1)
    return 0;
  for (j1 -= j, dRec = dList->dataList; j1; j1--)
    dRec = dRec->next;
  if (decayLen)
    *decayLen = dRec->nkey;
  if (decay)
    for (k = 0; k < dRec->nkey; k++)
      decay[k] = dRec->pNum[k];
  if (br)
    *br = dRec->Br;
...
#define NEW
  ...
  for (k = 1;; k++) {
    ...
    if (0 == allBlocks(k, 0, name, &len, key, &val))
      break;
    ...
    for (l = 1;; l++) {
      if (0 == allBlocks(k, l, name, &len, key, &val))
        break;
      for (i = 0; i < len; i++)
        fprintf(f, " %3d", key[i]);
      if (cimagl(val) == 0)
        fprintf(f, "  %16LE ", creall(val));
      else
        fprintf(f, "  (%16LE,%16LE) ", creall(rec->val), cimagl(rec->val));
      fprintf(f, " # %s\n", slhaComment);
    }
  }
  ...
  for (k = 1;; k++) {
    ...
    if (0 == allQnumbers(k, &pdg, &eQ3, &spinDim, &cDim, &anti))
      break;
  ...
  for (k = 1;; k++) {
    ...
    if (0 == allDecays(k, 0, &pdg, &len, out, &width, &br))
      break;
    fprintf(f, "DECAY %d %E # %s\n", pdg, width, slhaComment);
    for (l = 1;; l++) {
      if (0 == allDecays(k, l, &pdg, &len, out, &width, &br))
        break;
...
int blockExists(char *block) {
  ...
  for (blockStr *blck = blockList; blck; blck = blck->next)
    if (strcmp(Block, blck->name) == 0)
      return 1;
  return 0;
}
