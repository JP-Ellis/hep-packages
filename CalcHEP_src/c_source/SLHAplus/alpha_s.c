...
static double lambda[7] = {
    0, 0, 0, 3.274942E-01, 2.888141E-01, 2.095375E-01, 8.929726E-02};
...
static double alpha3(double Q, double lambda, int nf) {
  double b0 = (11 - (2. / 3.) * nf) / 4, b1 = (102 - (38. / 3.) * nf) / 16,
  ...
  double d1 = b1 / (b0 * b0t), d2 = b2 / (b0 * b0t * b0t),
         d3 = b3 / (b0 * b0t * b0t * b0 * t);

  // http://pdg.lbl.gov/2016/reviews/rpp2016-rev-qcd.pdf
  return (M_PI / b0t) *
         (1 - d1 * Lt + d1 * d1 * (Lt * Lt - Lt - 1) + d2 -
          d1 * d1 * d1 * (Lt * Lt * Lt - 2.5 * Lt * Lt - 2 * Lt + 0.5) -
          3 * d1 * d2 * Lt + 0.5 * d3);
}
...
static double findLambda(int nf, double alpha, double M) {
  ...
  while ((a2 = alpha3(M, l2, nf) - alpha) < 0)
    l2 *= 1.2;
  l1 = l2 * 0.8;
  while ((a1 = alpha3(M, l1, nf) - alpha) > 0)
    l1 *= 0.8;
...
double poleQmass(double mm, double alpha, int nf) {
  double zeta2 = 1.6439, c1 = 4. / (3.), c2 = 13.4434 - 1.0414 * (nf - 1),
         c3 = 190.595 - (nf - 1) * (26.655 - (nf - 1) * 0.6527);
  double a = alpha / M_PI;

  if (nf == 5)
    c2 += zeta2 * (1.61 / 4.62); /* (Mc+Ms)/Mb contribution */
  else if (nf == 6)
    c2 += zeta2 * (6.23 / 175); /* (Mc+Ms+Mb)/Mt contribution */
...
double initQCD(double MZalphaS, double McMc, double MbMb, double MtP) {
  ...
  if (MtP < poleQmass(91.187, MZalphaS, 6))
    return -1;
...
double alphaQCD(double Q) {
  if (notInitialized)
    initQCD(0.1184, 1.2, 4.23, 173.07);
  if (Q < qLim)
    Q = qLim;
  if (Q < qMin)
    return 1;
  return alpha3(Q, lambda[NF(Q)], NF(Q));
}
...
static double m_fact(int nf, double alpha1, double alpha2) {
  ...
  {
    double xx = pow(2 * alpha2 / k, k) * (1 + alpha2 * (c1 + alpha2 * c2)) /
...
double MqRun(double mass2GeV, double Q) { return runningMass(mass2GeV, 2., Q); }
...
double MtRun(double Q) { return runningMass(qMass[6], qMass[6], Q); }

static double DeltaQCD(
    double Q) // M. Spira, Fortsch. Phys. 46 (1998) 203 [arXiv:hep-ph/9705337]
{
  double a = alphaQCD(Q) / M_PI;
  int nf = NF(Q);
  double res, res1;

  return a * (5.67 + a * ((35.94 - 1.36 * nf) +
                          a * (164.14 - nf * (25.77 - nf * 0.259))));
}

double MqEff(double mass2GeV, double Q) {
  return MqRun(mass2GeV, Q) * sqrt(1 + DeltaQCD(Q));
}

double McEff(double Q) { return McRun(Q) * sqrt(1 + DeltaQCD(Q)); }
double MbEff(double Q) {
  double m = MbRun(Q) * sqrt(1 + DeltaQCD(Q));
  if (m > MbPole)
    return MbPole;
  else
    return m;
