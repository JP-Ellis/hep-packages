...
static double xMin[2] = {0, 0}, xMax[2] = {0, 0}, qMin[2] = {1, 1},
              qMax[2] = {1E10, 1E10};
...
static double alpha_lha1(double q) {
  ...
  int one = 1;
  if (nPROCSS > 1 && VERS < 6)
    pthread_mutex_lock(&alpha_key);
  al = alphaspdfm(&one, &q);
  if (nPROCSS > 1 && VERS < 6)
    pthread_mutex_unlock(&alpha_key);
  return al;
}
...
static int initLHA(void) {
  ...
  if (VERS == 5) {
    ...
    if (!getpdfsetlist)
      getpdfsetlist = dlsym(h, "_lhapdf_getpdfsetlist_");
    if (!getpdfsetlist) {
      VERS = -1;
      return 0;
    }
...
void n_lha(int i, char *name) {
  ...
  if (strlen(fname[i1]))
    sprintf(name, "LHA:%s:%d:%d", fname[i1], setNum[i1], sgn[i1]);
  else
    strcpy(name, "LHA:");
}
...
int init_lha(int i, double *be, double *mass) {
  ...
  *be = 1;
  if (i == 1)
    sf_alpha[0] = alpha_lha1;
  else
    sf_alpha[1] = alpha_lha2;
  for (k = 0; k < 12; k++)
    if (parton[k] == N) {
      pnum[i - 1] = N;
      return 1;
...
static char *lhaMenu(void) {
  ...
  menutxt[1] = 0;
  for (i = 0; i < N; i++) {
    sprintf(menutxt + 1 + (width + 1) * i, " %-*.*s", width, width,
            namelist[i]->d_name);
    free(namelist[i]);
  }
  free(namelist);
...
int r_lha(int i, char *name) {
  ...
  if (!initLHA())
    return 0;
  if (3 != sscanf(name, "LHA:%[^:]:%d:%d", txt + 1, setNum + i1, sgn + i1))
    return 0;
  if (abs(sgn[i1]) != 1)
    return 0;
  men = lhaMenu();
...
int m_lha(int i, int *pString) {
  ...
  for (; n0 != 0 && n0 != 3;) {
    ...
    numberpdfm(&i, &nMax);
    if (nMax > 1)
      improveStr(strmen0, "Set = 0", "Set = %d [0,%d]", setNum[i1], nMax);
    else
      improveStr(strmen0, "Set = 0", "Set = 0 (only)");

    if (sgn[i1] < 0)
      improveStr(strmen0, "Proton", "%s", "antiProton");
...
double c_lha(int i, double x, double q) {
  ...
  if (q > qMax[i1])
    q = qMax[i1];
  if (nPROCSS > 1 && VERS < 6)
    pthread_mutex_lock(&strfun_key);
  evolvePDFm(i, x, q, f);
  if (nPROCSS > 1 && VERS < 6)
    pthread_mutex_unlock(&strfun_key);
