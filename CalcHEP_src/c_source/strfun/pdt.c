...
static double cubicInt1D(double x, double *X, double *F) {
  if (x < X[0] && F[0] > 0 && F[1] > 0)
    return pow(F[0], (x - X[1]) / (X[0] - X[1])) *
           pow(F[1], (X[0] - x) / (X[0] - X[1]));
  if (x > X[3] && F[2] > 0 && F[3] > 0)
    return pow(F[2], (x - X[3]) / (X[2] - X[3])) *
           pow(F[3], (X[2] - x) / (X[2] - X[3]));
  //   return polint(x,4,X,F);

  if (x <= X[1])
    return polint(x, 3, X, F);
  if (x >= X[2])
    return polint(x, 3, X + 1, F + 1);

  double d0 = (F[1] - F[0]) / (X[1] - X[0]), d1 = (F[2] - F[1]) / (X[2] - X[1]),
         d2 = (F[3] - F[2]) / (X[3] - X[2]);

  double dL =
      d0 * (X[2] - X[1]) / (X[2] - X[0]) + d1 * (X[1] - X[0]) / (X[2] - X[0]);
  double dR =
      d1 * (X[3] - X[2]) / (X[3] - X[1]) + d2 * (X[2] - X[1]) / (X[3] - X[1]);
  double dx = X[2] - X[1];
  ...
  return F[1] * (1 - 3 * t2 + 2 * t3) + F[2] * t2 * (3 - 2 * t) +
         dL * t * (1 - t) * (1 - t) + dR * t2 * (t - 1);
}
...
static double biCubicLogXQ(double x, double q, pdtStr *W) {
  ...
  for (i = 0; i < 4; i++)
    f[i] = cubicInt1D(logX, W->lx_grid + px, W->strfun + W->nx * (pq + i) + px);
  if (q > 1)
    return cubicInt1D(logQ, W->lq_grid + pq, f);
  else {
    for (i = 0; i < 4; i++)
      f[i] /= W->q_grid[pq + i] * W->q_grid[pq + i];
    return cubicInt1D(logQ, W->lq_grid + pq, f) * q * q;
  }
}

static double qSplineCteq6(double x, double *xa, double *ya) {
  double s12 = xa[0] - xa[1], s13 = xa[0] - xa[2], s23 = xa[1] - xa[2],
         s24 = xa[1] - xa[3], s34 = xa[2] - xa[3];
  double sy2 = x - xa[1], sy3 = x - xa[2];

  double c1 = s13 / s23, c2 = s12 / s23, c3 = s34 / s23, c4 = s24 / s23;

  double cxx = sy2 * sy3 / (s12 * s34 - (s12 + s13) * (s24 + s34));
  double c5 = (s34 * sy2 - (s24 + s34) * sy3) * cxx / s12,
         c6 = ((s12 + s13) * sy2 - s12 * sy3) * cxx / s34;
  double res =

      (c5 * (ya[0] - ya[1] * c1 + ya[2] * c2) +
       c6 * (ya[3] + ya[1] * c3 - ya[2] * c4) + ya[1] * sy3 - ya[2] * sy2) /
      s23;
  return res;
}
...
double int_cteq6(double x, double q, pdtStr *W) {
  ...
  if (xExt == 0)
    ...
    for (i = 0; i < 4; i++) {
      for (j = 1; j < 4; j++)
        ftmp[j] = pow(x_grid[j], 2 / 0.3) * W->strfun[W->nx * (pq + i) + j];
      tmp[i] = polint(x3, 4, x_grid, ftmp) / (x * x);
    }
  }

  if (qExt)
    return polint(loglogQ, 4, q_grid + pq, tmp);
  else
    return qSplineCteq6(loglogQ, q_grid + pq, tmp);
}
...
void freePdtData(pdtStr *data) {
  ...
  if (data->q_grid_aux) {
    free(data->q_grid_aux);
    data->q_grid_aux = NULL;
  }
  if (data->x_grid_aux) {
    free(data->x_grid_aux);
...
int getPdtData(char *file, int n_parton, pdtStr *data) {
  ...
  while (1 == fscanf(f, "%c", &c))
    if (c == '#') {
      ...
      if (!strcmp(buff, "Index")) {
        if (1 != fscanf(f, "%d", &data->index))
          goto errexit;
      } else if (!strcmp(buff, "Set")) {
        if (1 != fscanf(f, "%d", &data->set))
          goto errexit;
        ...
        for (i = 0; i < nq; i++) {
          if (fscanf(f, "%lf", data->q_grid + i) != 1)
            goto errexit;
          if (i) {
            if (data->q_grid[i - 1] >= data->q_grid[i])
              goto errexit;
          } else if (data->q_grid[0] <= 0)
            goto errexit;
        }
      } else if (!strcmp(buff, "Interpolation")) {
        fscanf(f, "%s", buff);
        if (0 == strcmp(buff, "biCubicLogXQ"))
          data->interpolation = &biCubicLogXQ;
        else if (0 == strcmp(buff, "CTEQ6")) {
          data->interpolation = &int_cteq6;
        } else
          printf("Inknown interpolation '%s', biCubicLogXQ  is used instead\n",
                 buff);
        ...
        if (fscanf(f, "%lf", &(data->q_threshold)) != 1)
          goto errexit;
  ...
  if (data->q_grid) {
    ...
    for (i = 0; i < nq; i++)
      if (data->q_grid[i] < data->q_threshold)
        data->qt0 = i + 1;

    data->lq_grid = (double *)malloc(sizeof(double) * nq);

    for (i = 0; i < nq; i++)
      data->lq_grid[i] = log(data->q_grid[i]);
  }
  ...
  if (data->x_grid) {
    ...
    for (i = 0; i < nx; i++)
      data->lx_grid[i] = log(data->x_grid[i]);
  }
  ...
  if (data->interpolation == &int_cteq6) {
    ...
    if (data->x_grid[0] == 0)
      ...
      data->x_grid_aux[0] = pow(data->x_grid[0], 0.3);
    for (i = 1; i < nx; i++)
      data->x_grid_aux[i] = pow(data->x_grid[i], 0.3);

    data->q_grid_aux = (double *)malloc(sizeof(double) * nq);
    for (i = 0; i < nq; i++)
      data->q_grid_aux[i] = log(data->lq_grid[i] - log(0.22));
  }
...
long makePdtList(char *file, pdtList **list) {
  ...
  for (L = 0; !feof(f);) {
    ...
    char ch;
    L = 0;
    if (2 != fscanf(f, " \"%[^\"]%*c %ld => ", dName, &mother))
      break;
    for (pos = 1, ch = 0; ch != '#';) {
      if (1 == fscanf(f, " %d ", &N)) {
        partons_[L] = N;
