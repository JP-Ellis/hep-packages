...
static double b_h(double etax) {
  ...
  if (etax <= 0.)
    return g[0] * gammai_(2, b_ng);
  g[1] = pow(etax, R3) / 1.354117939426404 / 2.;
  g[2] = pow(etax, 2. * R3) / 6.;

  for (n = 1; n < 4; n++)
    s0 += g[n - 1] * gammai_(n + 1, b_ng);
...
static double cfbeam(double x) {
  if (x <= 0.)
    ...
    if (etax > 50.)
      return 0;
    return (1 + x * (b_cips - 1)) * pow(k / x, R3) * exp(-etax) *
           b_h(etax * pow(1 + x * (b_cips - 1), 3));
  }
}

static double cfbeamLog(double y) {
...
static double cfisr(double x) {
  return coeff *
         (x * x + 1 -
          beta * (log(x) * (3. * x * x + 1.) / 2. + (1. - x) * (1. - x)) / 2.) /
         2;
}

static double cfisrLog(double y) {
  return cfisr(exp(-y)) * pow(divy_(y), beta - 1);
}

int p_isr__(int *pNum) {
  if (abs(pNum[0]) == 11 && pNum[1] == 0)
    return 1;
  else
    return 0;
}
...
void n_isr__(int i, char *name) {
  char *c;
  if (scale <= 1.)
    sprintf(name, "Qisr=%.2E*sqrtS,Beamstr:", scale);
  else
    sprintf(name, "Qisr=%.2E[GeV],Beamstr:", scale);

  if (bOn)
    sprintf(name + strlen(name), "%.1f,%.3f,%.1E", xy_nm, z_mm, qTot);
  else
    strcat(name, " OFF");
  c = strstr(name, "E+0");
  if (c)
...
static void calc_params(void) {
  ...
  if (bOn) {
    ...
    b_ips = 5 * ALPHA * qTot * sqrt_S /
            (12 * EM * EM * EM * z_mm * (xy_nm * 1.E-6) * mmToGeV * mmToGeV);
    if (b_ips >= 1)
      b_cips = sqrt(1 + pow(b_ips, 0.666666666666666666));
    else
      b_cips = 1;
    b_ng = b_ncl / b_cips;
  }
...
int i_isr__(int ii, double *be, double *mass) {
  ...
  *mass = 5.11E-4;
  ...
  for (i = 0; i < NPOINTS; ++i)
    xi[i] = (double)(i) / NPOINTS;
  for (i = NPOINTS; i < NPOINTS + 4; i++)
    xi[i] = 1 - (1 - xi[i - 1]) * 0.5;
  if (bOn)
    iRest = b_ng - simpson(norm_test, 0., 1., 1.E-8, NULL);
    ...
    if (bOn) {
      ...
      yi[i] = (yi[i] * iRest +
               pow(1 - x, B2) * pow(divy_(lx), 1 - beta - B2) *
                   convol_(cfisrLog, cfbeamLog, beta, B2, lx, EPS)) /
              b_ng;
    }
  }
//#define CHECK_NORN
#ifdef CHECK_NORN
  if (bOn)
    printf("Check of Beamstralung normalization  delta(1-x)*%E  + "
           "%E(Beamstralung) =1?\n",
           (1 - exp(-b_ng)) / b_ng,
...
double c_isr__(int i, double x, double q) {
  x = pow(1 - x, R3);
  return dinter_(x, NPOINTS + 4, xi, yi);
}
...
int m_isr__(int i, int *pString) {
  ...
  for (;;) {
    ...
    if (scale <= 1)
      improveStr(strmen, "XXX", "%.2E*sqrtS", scale);
    else
      improveStr(strmen, "XXX", "%.0fGeV", scale);

    calc_params();
    if (bOn) {
      if (b_cips < 1)
        improveStr(strmen, "_gamma", "_cl");
    ...
    case 1:
      correctDouble(25, 16, "Enter value:( <=1 in part of sqrt(S); >1 in GeV) ",
                    &scale, 1);
      break;
    case 2:
      bOn = !bOn;
    ...
    case 9: {
      ...
      for (i = 0; i < nPoints; i++) {
        ...
        if (mode == 8)
          f[i] = cfbeam(x) / pow(1 - x, 2 * R3) / b_ng;
        else
          f[i] = cfbeam(x) / b_ng;
      }
      {
        ...
        if (mode == 8)
          ...
          sprintf(mess, "F(x)*(1-x)^(2/3)");
        plot_1(xMin + dx, xMax - dx, nPoints, f, NULL, "Beamstrahlung", "x",
               mess);
      }
      put_text(&screen);
