...
static double pt_min[2] = {0.1,
                           0.1}; /* Minimal Pt cut [GeV] of outgoing protons*/
...
int p_epp__(int *pNum) {
  if (pNum[0] == 22 && pNum[1] == 0)
    return 1;
  else
    return 0;
}

void n_epp__(int i, char *name) {
  sprintf(name, "Proton.Photon(m=%.10G Ch=%d Q=%.8G Pt>%.9G)", xin[i - 1],
          charge[i - 1], q2_max[i - 1], pt_min[i - 1]);
}

int r_epp__(int i, char *name) {
  double xin__, q2_max__, pt_min__;
  int charge__;
  if (4 != sscanf(name,
                  "Proton.Photon(m=%lf%*[^=]%*c%d%*[^=]%*c%lf%*[^>]%*c%lf)",
                  &xin__, &charge__, &q2_max__, &pt_min__))
    goto L10;
  ...
  if (q2_max__ <= 0.)
    goto L10;
  if (pt_min__ < 0. || pt_min__ > Maxpt_min)
    goto L10;
  i--;
  xin[i] = xin__;
...
int m_epp__(int i, int *pString) {
  ...
  for (;;) {
    ...
    case 2:
      ...
      break;
    case 3:
      correctDouble(40, 16, "Enter new value ", q2_max + i, 1);
      break;
    case 4:
      do {
        correctDouble(40, 16, "Enter new value ", pt_min + i, 1);
...
static double phi(double x, double qq) {
  ...
  f += (1 - b) * y /
       (4 * qq * qq1 * qq1 *
        qq1); // here  sign  is differnt  with Budnev et al and corresponds to
              // realization by Muriel Vander Donckt. Investigation by Richard
              // Ruiz revealed that Budnev et al have mistake in this place.
