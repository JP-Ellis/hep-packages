...
static struct {
  ...
  double (*val)(int, double, double);
    ...
    {p_epa__, n_epa__, r_epa__, m_epa__, mc_epa__, i_epa__, c_epa__},
...
/*
Interface with   earch family of structure functions is presented by 7 auxiliary
functions included in struct strFun.
...
int readName(int i1, char*name)  reads 'name' of structure function set for
incoming particle i1 and performs corresponding initialization if the name
corresponds to member of family.

int menu(int i1,int*list) allows initialization of structure function set by
means of menu.

fullName(int i1, char*name)
   writes full name of  initialized structure function for i1-th particle. Thus
name should be recognized by the 'readName' routine.

int init(int i1, &beta, &mass) perfoms final initialization for given parton.
    mass - is mass of incoming physical particle. It actually is universal
number for the set. The beta parameter characterizes singularity of distribution
at x=1. Namely the behavior (1-x)^(beta-1) is expected.

double val(int i1, double x, double Q)
     returns value of parton distribution corresponding to dx measure.

int mc_code(int i1) writes PDG code for i1-th  incoming physical particle. It
needs to event file.
...
void strFunName(int i, char *mess) {
  if (sf_num[i - 1])
    strFun[sf_num[i - 1] - 1].fullName(i, mess);
  else
    strcpy(mess, "OFF");
}
...
int sf_menu(int i1) {
  ...
  if (!k) {
    messanykey(15, 15,
               "Structure functions for this particle\n"
               "are not known\n");
    return 0;
  }
...
double strfun_(int i, double x, double q) {
  ...
  if (!isfinite(ff)) {
    if (nnan[i - 1] == 0)
      printf("Distribution function of %d^th particle returns NAN   at x=%E "
             "Q=%E\n",
             i, x, q);
    if (nnan[i - 1] == 100)
      printf("More than 100 times distribution function of %d^th particle "
             "returns NAN\n",
             i);
    nnan[i - 1]++;
    ff = 0;
...
static int loadStrFun(char *name1, char *name2) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (l = 0; l < MAXFUN; l++)
      if (strFun[l].myParticle(allInP(i + 1)) &&
          strFun[l].readName(i + 1, sf_txt[i])) {
        sf_num[i] = l + 1;
        break;
      }
  }
...
int rd_sf__(FILE *mode) {
  ...
  if (1 != fscanf(mode, " StrFun1=\"%[^\"]", sf_txt[0])) {
    err = 1;
    strcpy(sf_txt[0], "OFF");
  } else
    trim(sf_txt[0]);
  ...
  if (1 != fscanf(mode, " StrFun2=\"%[^\"]", sf_txt[1])) {
    err += 2;
    strcpy(sf_txt[1], "OFF");
  } else
    trim(sf_txt[1]);
  ...
  if (nin_int == 2 && (err || loadStrFun(sf_txt[0], sf_txt[1]))) {
    if (blind) {
      printf("File 'session.dat':Error in stucture function specification\n");
      exit(2);
    } else
      messanykey(10, 12,
                 "File 'session.dat':Error in stucture function specification");
    return 1;
  }
  return 0;
}
