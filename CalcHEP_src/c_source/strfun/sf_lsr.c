...
int r_lsr__(int i, char *name) {
  if (strcmp(name, "Laser photons") != 0)
    return 0;
  else
    return 1;
}

int i_lsr__(int i, double *be, double *mass) {
  *be = 1.;
  *mass = 0.;
  return 1;
}
...
double c_lsr__(int i, double x, double q) {
  ...
  if (first) {
    double d__2 = x0 + 1.;
    first = 0;
    rnorma = (1. - 4. / x0 - 8. / (x0 * x0)) * log(x0 + 1.) + .5 + 8. / x0 -
             1. / (d__2 * d__2 * 2.);
  }
  if (x > xmax)
    return 0.;
