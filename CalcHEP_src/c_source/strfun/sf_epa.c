...
int r_epa__(int i, char *name) {
  ...
  double v;
  i--;
  if (2 != sscanf(name, "Equiv.Photon(particle= %s |Q|max= %[^)]", pname,
                  q_max_txt[i]))
    return 0;
  trim(q_max_txt[i]);
  ...
  if (strcmp(pname, "mu^-") == 0)
    pc[i] = -2;
  else if (strcmp(pname, "e^-") == 0)
    pc[i] = -1;
  else if (strcmp(pname, "e^+") == 0)
    pc[i] = 1;
  else if (strcmp(pname, "mu^+") == 0)
    pc[i] = 2;
  else
    return 0;
  return 1;
...
int m_epa__(int i, int *pString) {
  ...
  for (;;) {
    ...
    case 1:
      ...
      break;
    case 2:
      correctStr(40, 16, "Enter new value ", q_max_txt[i], 19, 1);
      break;
    }
  }
  return 1;
