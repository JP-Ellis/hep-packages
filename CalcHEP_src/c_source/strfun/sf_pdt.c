...
int p_pdt(int *pNum) {
  pdtList *L;
  findAllPDT();
  for (L = allPDT; L; L = L->next)
    if (checkPartons(pNum, L))
      return 1;
  return 0;
}
...
void n_pdt(int i, char *name) {
  i--;
  if (curPDT[i])
    sprintf(name, "PDT:%s", curPDT[i]->name);
  else
    strcpy(name, "PDT:");
}
...
int r_pdt(int i, char *name) {
  ...
  if (name != strstr(name, "PDT"))
    return 0;
  findAllPDT();
  for (p = allPDT; p; p = p->next)
    if (strcmp(name + 4, p->name) == 0)
      break;
...
int init_pdt(int i, double *be, double *mass) {
  ...
  for (k = 0; curPDT[i]->partons[k]; k++)
    if (curPDT[i]->partons[k] == pNum) {
      pos[i] = curPDT[i]->items[k] - 1;
      if (pdtData[i][0]->alpha) {
        sf_alpha[i] = i == 0 ? alpha_pdt1 : alpha_pdt2;
      }
      if (pdtData[i][pos[i]]->pow1 < 0)
        *be = pdtData[i][pos[i]]->pow1 + 1;
      else
        *be = 1.;
  ...
  if (pNumAux == 0)
    ...
    for (k = 0; curPDT[i]->partons[k]; k++)
      if (curPDT[i]->partons[k] == pNumAux) {
        posAux[i] = curPDT[i]->items[k] - 1;
        break;
      }
  return 1;
}
...
int m_pdt(int i, int *pString) {
  ...
  for (list_ = allPDT; list_; list_ = list_->next)
    if (checkPartons(pString, list_))
      k++;
  ...
  for (list_ = allPDT; list_; list_ = list_->next)
    if (checkPartons(pString, list_))
      sprintf(strmen + strlen(strmen), " %-*.*s", WIDTH - 1, WIDTH - 1,
              list_->name);
  k = 0;
  ...
  if (k == 0)
    return 0;
  k--;
  for (list_ = allPDT;; list_ = list_->next)
    if (checkPartons(pString, list_)) {
      if (k == 0)
        break;
      else
...
double c_pdt(int i, double x, double q) {
  ...
  if (pdtData[i][pos[i]]->pow0)
    r *= pow(x, pdtData[i][pos[i]]->pow0);
  if (pdtData[i][pos[i]]->pow1 > 0)
    r *= pow(1 - x, pdtData[i][pos[i]]->pow1);
  if (r < 0)
    r = 0;
  return r;
}
