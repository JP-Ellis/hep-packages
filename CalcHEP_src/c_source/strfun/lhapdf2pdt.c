...
int main(int argc, char **argv) {
  if (argc < 2) {
    printf("One argument is expected,  name  of directory with  LHAPDF "
           "distribution.\n");
  ...
  if (argc == 2)
    strcat(fdat, "0000.dat");
  else {
    int i;
    for (i = 0; i < 4 - strlen(argv[2]); i++)
      strcat(fdat, "0");
    strcat(fdat, argv[2]);
    strcat(fdat, ".dat");
  ...
  while (1 == fscanf(Finf, "%[^:]:", key)) {
    //     printf("key='%s'\n", key);
    if (0 == strcmp(key, "SetIndex"))
      fscanf(Finf, "%d", &Index);
    else if (0 == strcmp(key, "Reference"))
      fscanf(Finf, "%[^\n]", Ref);
    else if (0 == strcmp(key, "Format")) {
      fscanf(Finf, "%s", Format);
      if (0 != strcmp(Format, "lhagrid1")) {
    ...
    } else if (0 == strcmp(key, "Particle"))
      fscanf(Finf, " %d", &Particle);
    char c;
    do
      fscanf(Finf, "%c", &c);
    while (c != '\n');
  ...
  for (i = 0; i < Xdim * Qdim * Pdim; i++)
    if (1 != fscanf(Fdat, " %lf ", buff + i)) {
  ...
  char *Pstr = (char *)malloc(Pdim * 4);
  Pstr[0] = 0;
  char *aPstr = (char *)malloc(Pdim * 4);
  aPstr[0] = 0;
  int pk;
  for (ip = 0, pk = 0; ip < Pdim; ip++) {
    ...
    for (iq = 0; iq < Qdim; iq++) {
      double d;
      for (ix = 0; ix < Xdim; ix++) {
        fprintf(Fout, " %.4E", buff[ip + iq * Pdim + ix * Pdim * Qdim]);
      }
      fprintf(Fout, "\n");
    }
  }
  fseek(Fout, posB, SEEK_SET);
  fprintf(Fout, "%s", Pstr);
  fseek(Fout, posE, SEEK_SET);
  fprintf(Fout, "%s", aPstr);
