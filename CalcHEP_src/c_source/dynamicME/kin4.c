...
double decayPcm(double am0, double am1, double am2) {
  ...
  if (am0 < summ)
    return 0;
  return sqrt((am0 - summ) * (am0 + summ) * (am0 - diffm) * (am0 + diffm)) /
         (am0 * 2);
}
...
static double ME(double m0, double m1, double m2) {
  return m2 * m2 * m1 * m1 +
         (m0 * m0 - m2 * m2 - m1 * m1) * (m0 * m0 - m2 * m2 - m1 * m1) / 8;
}
...
static double intDecay2_(double x) {
  if (x <= 0 || x >= 1)
    return 0;
  return intDecay2(y21 + x * x * x * (4 - 3 * x) * (y22 - y21)) * 12 * x * x *
         (1 - x) * (y22 - y21);
}
...
static double intDecay1(double y1) {
  ...
  if (nGauss)
    return gauss(intDecay2_, 0, 1, nGauss);
  else
    return simpson(intDecay2_, 0, 1, 1.E-3, NULL);
  //   return  simpson(intDecay2, atan(-m2_p/w2_), atan(
  //   ((m0_-m1_)*(m0_-m1_)-m2_p*m2_p)/(m2_p*w2_)), 1.E-3);
}

static double intDecay1_(double x) {
  if (x <= 0 || x >= 1)
    return 0;
  return intDecay1(y11 + x * x * x * (4 - 3 * x) * (y12 - y11)) * 12 * x * x *
         (1 - x) * (y12 - y11);
}
...
double decayPcmW(double m0, double m1, double m2, double w1, double w2, int N) {
  ...
  if (w1 == 0 && w2 == 0)
    ...
    if (nGauss)
      return gauss(intDecay2_, 0, 1, nGauss) / M_PI / ME(m0, m1, m2);
    else
      return simpson(intDecay2_, 0, 1, 1.E-3, NULL) / M_PI / ME(m0, m1, m2);
  } else if (w2 == 0) {
    if (m2 > m0)
      return 0;
    m1_ = m2;
    ...
    m2_p = m2;
    y11 = atan(-m1 / w1);
    y12 = atan((m0 * m0 - m1 * m1) / (m1 * w1));

    if (nGauss)
      return gauss(intDecay1_, 0, 1, nGauss) / (M_PI * M_PI) / ME(m0, m1, m2);
    else
      return simpson(intDecay1_, 0, 1, 1E-3, NULL) / (M_PI * M_PI) /
             ME(m0, m1, m2);
  }
}
...
static double kinematic_1_3(REAL *pmass, int i3, double m12, double xcos,
                            REAL *P) {
  double factor;
  ...
  E1 = sqrt(m1 * m1 + pout * pout);
  E2 = sqrt(m2 * m2 + pout * pout);
  P13 = xcos * pout;
  P23 = -P13;
  P12 = xsin * pout;
  P22 = -P12;

  P[4 * i1] = chY * E1 + shY * P13;
  P[4 * i2] = chY * E2 + shY * P23;
  P[4 * i1 + 3] = shY * E1 + chY * P13;
  P[4 * i2 + 3] = shY * E2 + chY * P23;
  P[4 * i1 + 2] = P12;
  P[4 * i2 + 2] = P22;
...
static double kinematic_1_4(REAL *pmass, double xm1, double xm2, double xcos1,
                            double xcos2, double fi2, REAL *P) {
  ...
  M1 = pmass[1] + pmass[2] +
       xm1 * (pmass[0] - pmass[1] - pmass[2] - pmass[3] - pmass[4]);
  M2 = pmass[3] + pmass[4] + xm2 * (pmass[0] - M1 - pmass[3] - pmass[4]);

  factor *= (pmass[0] - pmass[1] - pmass[2] - pmass[3] - pmass[4]) *
            (pmass[0] - M1 - pmass[3] - pmass[4]);
  Pcm = decayPcm(pmass[0], M1, M2);
  factor *= 4 * M_PI * Pcm / pmass[0];
  p1cm = decayPcm(M1, pmass[1], pmass[2]);
  p2cm = decayPcm(M2, pmass[3], pmass[4]);
  factor *= 2 * M_PI * p1cm * p2cm;

  P[0] = pmass[0];
  P[1] = P[2] = P[3] = 0;

  P[4 + 0] = sqrt(pmass[1] * pmass[1] + p1cm * p1cm);
  P[8 + 0] = sqrt(pmass[2] * pmass[2] + p1cm * p1cm);
  P[4 + 1] = 0;
  P[4 + 2] = p1cm * sqrt(1 - xcos1 * xcos1);
  P[4 + 3] = p1cm * xcos1;

  P[12 + 0] = sqrt(pmass[3] * pmass[3] + p2cm * p2cm);
  P[16 + 0] = sqrt(pmass[4] * pmass[4] + p2cm * p2cm);
  ...
  */
  for (i = 0; i < 5; i++) {
    double m;
    m = sqrt(fabs(P[4 * i] * P[4 * i] - P[4 * i + 1] * P[4 * i + 1] -
                  P[4 * i + 2] * P[4 * i + 2] - P[4 * i + 3] * P[4 * i + 3]));
    if (fabs(m - pmass[i]) > pmass[0] * 1.E-5) {
      printf("wrong mass %d (%E != %E) \n", i, m, pmass[i]);
      exit(33);
    }
  }
  return factor;
...
double (*sqme)(int nsub, double GG, REAL *pvect, REAL *cb_coeff,
               int *err_code) = NULL;
...
static double dWidthdM(double M) {
  ...
  M_ = M;
  r = simpson(dWidthdCos, -1., 1., 1.E-4, NULL);
...
static double width13(numout *cc, int nsub, int *err) {
  ...
  if (cc->SC)
    ...
    GG = sqrt(4 * M_PI * alphaQCD(Pmass[0]));
  *err = 0;
  i3_ = 1;
...
static double wInt14(double *x, double w) {
  ...
  int err_code = 0;
  double res;
  res = kinematic_1_4(Pmass, x[0], x[1], 2 * (x[2] - 0.5), 2 * (x[3] - 0.5),
                      2 * M_PI * x[4], pvect);
  if (res == 0)
    return 0;
...
static double width14(numout *cc, int *err) {
  ...
  *err = 0;
  ...
  for (i = 0; i < 5; i++)
    cc->interface->pinf(1, 1 + i, Pmass + i, NULL);
  if (cc->SC)
    GG = *(cc->SC);
  else
    GG = sqrt(4 * M_PI * alphaQCD(Pmass[0]));
...
int pname2lib(char *pname, char *libname) {
  ...
  n = pTabPos(buff);
  if (!n) {
    printf("Wrong particle name '''%s'''\n", pname);
    libname[0] = 0;
    return 1;
  }
  if (p) {
    if (n > 0)
      sprintf(libname, "pp%d", n);
    else
      sprintf(libname, "ap%d", -n);
  } else {
    if (n > 0)
      sprintf(libname, "p%d", n);
    else
      sprintf(libname, "a%d", -n);
  }
  return 0;
}
...
void massFilter(double M, txtList *List) {
  ...
  while (lold) {
    ...
    for (; ch; ch = strchr(ch, ',')) {
      ...
      } else if (abs(ModelPrtcls[abs(n) - 1].NPDG) == 6)
        dM = MtEff(0);
      else
        dM = fabs(*(varAddress(nm)));
      Msum += dM;
    }
    lnext = lold->next;
    if (M > Msum) {
  ...
  for (; ch1 && err == 0; pos++) {
    ...
    if (strcmp(ch1 + 1, "*x") && strcmp(ch1 + 1, "*X")) {
      ...
      if (err)
        return pos;
      strcat(lib, bufflib);
    } else if (1 != sscanf(ch1, "%d", &nX))
      return pos;
    ch1 = strtok(NULL, " ,");
  }
  if (nX)
    sprintf(lib + strlen(lib), "x%d", nX);
  free(process_);
  return 0;
}
...
void process2Mass(char *process, double *mass) {
  ...
  ch1 = strtok(process_, " ,");
  for (i = 0; ch1;
       i++, ch1 = strtok(NULL, " ,")) { /*if(ch1==ch) strcat(lib,"_");*/
    mass[i] = pMass(ch1);
  }
  free(process_);
}
...
double pWidth2(numout *cc, int nsub) {
  ...
  if (nsub < 1 || nsub > ntot || nin != 1 || nout != 2)
    return 0;
  ...
  if (m1 > m2 + m3) {
    ...
    double ms = m2 + m3;
    double pRestOut =
        sqrt((m1 * m1 - ms * ms) * (m1 * m1 - md * md)) / (2 * m1);
...
double decay2Info(char *pname, FILE *f) {
  ...
  for (wtot = 0, i = 1; i <= ntot; i++) {
    ...
    if (w != 0) {
      wtot += w;
      if (f)
        fprintf(f, "%3.3s %3.3s  %.2E\n", dname[1], dname[2], w);
    }
  }
  if (f)
    fprintf(f, " Total width %.2E GeV\n", wtot);
  return wtot;
}
...
static int chOpen(numout *cc, int k) {
  ...
  char *name[3];
  for (j = 0; j < 3; j++)
    name[j] = cc->interface->pinf(k, j + 1, NULL, pdg + j);
  s = pMass(name[0]);
  for (j = 1; j < 3; j++)
    s -= pMass(name[j]);
...
numout *xVtoxll(int Nin, int Nout, char **name, int *pdg, int lV, double *wV,
                double *br) {
  int i, err;
  char *e_ = NULL, *E_ = NULL, *ne_ = NULL, *Ne_ = NULL, *m_ = NULL, *M_ = NULL,
       *nm_ = NULL, *Nm_ = NULL, *W_ = NULL, *Z_ = NULL;
  ...
  if (pdg[lV] != 23 && abs(pdg[lV]) != 24)
    return NULL;

  // if(Nin==2)printf("%s %s -> %s %s\n", name[0], name[1],name[2],name[3]);

  for (i = 0; i < nModelParticles &&
              !(e_ && ne_ && m_ && nm_ && e_ && Ne_ && M_ && Nm_ && W_ && Z_);
       i++)
    switch (ModelPrtcls[i].NPDG) {
    case 11:
      e_ = ModelPrtcls[i].name;
      E_ = ModelPrtcls[i].aname;
      break;
    case -11:
      e_ = ModelPrtcls[i].aname;
      E_ = ModelPrtcls[i].name;
      break;
    case 12:
      ne_ = ModelPrtcls[i].name;
      Ne_ = ModelPrtcls[i].aname;
      break;
    case -12:
      ne_ = ModelPrtcls[i].aname;
      Ne_ = ModelPrtcls[i].name;
      break;
    case 13:
      m_ = ModelPrtcls[i].name;
      M_ = ModelPrtcls[i].aname;
      break;
    case -13:
      m_ = ModelPrtcls[i].aname;
      M_ = ModelPrtcls[i].name;
      break;
    case 14:
      nm_ = ModelPrtcls[i].name;
      Nm_ = ModelPrtcls[i].aname;
      break;
    case -14:
      nm_ = ModelPrtcls[i].aname;
      Nm_ = ModelPrtcls[i].name;
      break;
    case 24:
      W_ = ModelPrtcls[i].name;
      break;
    case -24:
      W_ = ModelPrtcls[i].aname;
      break;
    case 23:
      Z_ = ModelPrtcls[i].name;
      break;
    }

  if (!(e_ && ne_ && m_ && nm_ && e_ && Ne_ && M_ && Nm_ && W_ && Z_))
    return NULL;

  {
    ...
    ww = pWidth(W_, &wDlist);
    wz = pWidth(Z_, &zDlist);
    sprintf(txt, "%s,%s", E_, ne_);
    wBrE = findBr(wDlist, txt);
    sprintf(txt, "%s,%s", M_, nm_);
    wBrM = findBr(wDlist, txt);
    sprintf(txt, "%s,%s", ne_, Ne_);
    zBrEn = findBr(zDlist, txt);
    sprintf(txt, "%s,%s", nm_, Nm_);
    zBrMn = findBr(zDlist, txt);
  }

  sprintf(processX3, "%s", name[0]);
  if (Nin > 1)
    sprintf(processX3 + strlen(processX3), ",%s", name[1]);
  sprintf(processX3 + strlen(processX3), "->%s,", name[lV_]);
  c = processX3 + strlen(processX3);

  if (abs(pdg[lV_]) == 11 || abs(pdg[lV_]) == 12)
    switch (pdg[lV]) {
    case -24:
      sprintf(c, "%s,%s", m_, Nm_);
      *wV = ww;
      *br = wBrM;
      break;
    case 24:
  ...
  if (Nin == 2)
    sprintf(exclude, "%s", "%Z+W<1");
  else {
    if (pdg[lV] == 23)
      sprintf(exclude, "%s<1", Z_);
    else
      sprintf(exclude, "%s<1", W_);
  }
  strcat(plib13, "V");
  ...
  if (pN != 23 && abs(pN) != 24) {
    ...
    for (k = 1; k <= ntot; k++)
      if (!chOpen(cc, k)) {
        ...
        int vd[3] = {0, 0, 0};
        for (i = 0; i < 3; i++)
          name[i] = cc->interface->pinf(k, i + 1, m + i, pdg + i);
        if (no22 && m[0] <= m[1] + m[2])
          continue;
        ...
        for (i = 1; i < 3; i++)
          vd[i] = (abs(pdg[i]) == 24 && VWdecay) || (pdg[i] == 23 && VZdecay);
        ...
        if (l > 0 && l < 3) {
          ...
          if ((pdg[l_] == 23 || abs(pdg[l_]) == 24) && m[l] < m[l_]) {
            l = l_;
            l_ = 3 - l;
          }
          ...
          if (cc13) {
            ...
            *(cc13->interface->BWrange) = 20;
            for (i3_ = 1; i3_ < 4; i3_++)
              if (strcmp(cc13->interface->pinf(1, i3_ + 1, NULL, NULL),
                         name[l_]) == 0)
                break;
            for (i = 0; i < 4; i++)
              cc13->interface->pinf(1, 1 + i, Pmass + i, NULL);
            sqme = cc13->interface->sqme;
            nsub_stat = 1;
            {
              double Mmin, Mmax;
              Mmax = Pmass[0] - Pmass[i3_];
              for (Mmin = 0, j = 1; j < 4; j++)
                if (j != i3_)
                  Mmin += Pmass[j];
              w = simpson(dWidthdM, Mmin * 1.00001, Mmax * 0.9999, 1.E-3, NULL);

              //             if(findVal(ModelPrtcls[abs(pTabPos(name[l]))-1].width,&wVt))
              //             K=1;else K=wVt/wV;
              w /= brV;
            }
            ...
            if (vd[l_]) {
              ...
              w *= decayPcmW(m[0], m[l], m[l_], wV, w2, 0) /
                   decayPcmW(m[0], m[l], m[l_], wV, 0, 0);
              if (pdg[l_] == pdg[l])
                w /= 2;
            }
...
static txtList conBrList(txtList BrList) {
  ...
  for (; BrList; BrList = BrList->next) {
    ...
    if (inCode[0] > 0)
      sprintf(buff, "%E  %s -> ", br, ModelPrtcls[inCode[0] - 1].aname);
    else
      sprintf(buff, "%E  %s -> ", br, ModelPrtcls[-inCode[0] - 1].name);
    for (i = 0; outCode[i]; i++) {
      if (i)
        strcat(buff, ",");
      if (outCode[i] > 0)
        strcat(buff, ModelPrtcls[outCode[i] - 1].aname);
      else
        strcat(buff, ModelPrtcls[-outCode[i] - 1].name);
    }
    ...
    if (useSLHAwidth)
      for (i = 1; allDecays(i, 0, &pdg, &Len, decay, &width, &br); i++) {
        if (abs(pdg) == abs(pdg0)) {
          ...
          for (j = 1; allDecays(i, j, &pdg, &Len, decay, &width, &br); j++)
            if (br > 0) {
              int k;
              char *ch;
              l = malloc(sizeof(txtListStr));
              ...
              ch = pdg2name(pdg);
              if (ch)
                sprintf(l->txt, "%E  %s -> ", br, ch);
              else
                sprintf(l->txt, "%E  #%d -> ", br, pdg);
              ch = pdg2name(decay[0]);
              if (ch)
                sprintf(l->txt + strlen(l->txt), "%s", ch);
              else
                sprintf(l->txt + strlen(l->txt), "#%d", decay[0]);
              for (k = 1; k < Len; k++) {
                ch = pdg2name(decay[k]);
                if (ch)
                  sprintf(l->txt + strlen(l->txt), ", %s", ch);
                else
                  sprintf(l->txt + strlen(l->txt), ", #%d", decay[k]);
              }
              L = l;
            }
          sortWidthTxtList(L);
...
static int pListEq(char *txt1, char *txt2) {
  ...
  strcpy(buff, txt1);
  while ((ch = strchr(buff, ',')))
    ch[0] = ' ';

  n1 = sscanf(buff, "%s %s %s %s %s %s %s %s %s %s", rd1[0], rd1[1], rd1[2],
              rd1[3], rd1[4], rd1[5], rd1[6], rd1[7], rd1[8], rd1[9]);

  strcpy(buff, txt2);
  while ((ch = strchr(buff, ',')))
    ch[0] = ' ';

  n2 = sscanf(buff, "%s %s %s %s %s %s %s %s %s %s", rd2[0], rd2[1], rd2[2],
              rd2[3], rd2[4], rd2[5], rd2[6], rd2[7], rd2[8], rd2[9]);

  if (n1 != n2)
    return 0;
  for (i1 = 0; i1 < n1; i1++) {
    for (i2 = 0; i2 < n2; i2++)
      if (strcmp(rd1[i1], rd2[i2]) == 0) {
        rd2[i2][0] = 0;
        break;
      }
    if (i2 == n2)
...
int procInfo2(numout *cc, int nsub, char **name, REAL *mass) {
  ...
  if (nsub < 1 || nsub > cc->interface->nprc)
    return 2;
  ...
  if (mass) {
    ...
    for (i = 0; i < ntot; i++)
      cc->interface->pinf(nsub, i + 1, mass + i, NULL);
  }
  return 0;
}
...
int passParameters(numout *cc) {
  ...
  for (i = 1; i <= cc->interface->nvar; i++)
    if (cc->link[i] && ((REAL *)(cc->link[i]) - varValues) < *currentVarPtr)
      cc->interface->va[i] = *(cc->link[i]);
    else {
      printf("Value of variable  '%s' needed for calculation of '%s' is not "
             "known yet\n",
  ...
  if (err > 0) {
    printf("cannot calculate constraint %s\n", cc->interface->varName[err]);
    return 1;
  }
  return 0;
}
          ...
          if (v[k]) {
            ...
            for (; LV; LV = LV->next) {
              ...
              fprintf(f, "   %e  3  %d  %d  %d # %s,%s->%s\n", br * brV,
                      id[1 - k], pNum(name1), pNum(name2), N[1 - k], N[k], chD);
            }
            if (id[0] == id[1])
              break;
          }
