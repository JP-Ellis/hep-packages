...
int getDynamicVP(void) {
  ...
  if (access(SO, X_OK & R_OK) || checkMtime(SO)) // [re]compile VandP.so
  {
    ...
    sprintf(command,
            ...
            " . $CALCHEP/FlagsForSh;"
            " . ./EXTLIBsh;"
            "$CC $CFLAGS $SHARED -o so_generated/VandP.so VandP.c "
            "$CALCHEP/include/VandPgate.c $EXTLIB "
            " $CALCHEP/lib/dummy.a  $CALCHEP/lib/dynamic_me.a    "
            "$CALCHEP/lib/libSLHAplus.a "
            "$CALCHEP/lib/ntools.a $CALCHEP/lib/serv.a  -lm -lpthread -ldl",
            rootDir, compDir);
    err = system(command);
    // printf("%s\n", command);
    free(command);
  ...
  if (VandP == NULL)
    VandP = dlopen(SO, RTLD_NOW);
  if (!VandP) {
    printf("dlopen problem with %s\n", SO);
    messanykey(10, 10, dlerror());
    free(SO);
    return 2;
  }
  {
    ...
    char **libDir_ = dlsym(VandP, "libDir");
    if (libDir_)
      *libDir_ = libDir;
    char **modelDir_ = dlsym(VandP, "modelDir");
    if (modelDir_)
      *modelDir_ = modelDir;
    char **compDir_ = dlsym(VandP, "compDir");
    if (compDir_)
      *compDir_ = compDir;
    char **calchepDir_ = dlsym(VandP, "calchepDir");
    if (calchepDir_)
      *calchepDir_ = calchepDir;
    int *modelNum_ = dlsym(VandP, "modelNum");
    if (modelNum_)
      *modelNum_ = 1;

    if (!nModelParticles_ || !ModelPrtcls_ || !nModelVars_ || !nModelFunc_ ||
        !varNames_ || !varValues_ || !calcMainFunc_) {
      messanykey(10, 10, "Can not find symbols");
      free(SO);
      return 3;
    }
