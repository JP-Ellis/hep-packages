...
#ifdef __hpux
  void *addr;
  if (shl_findsym((shl_t *)&handle, name, TYPE_UNDEFINED, &addr))
    return NULL;
  else
    return addr;
#else
...
static numout *loadLib(void *handle, char *lib) {
  ...
  if (!cc->interface || cc->interface->nprc == 0) {
    free(cc);
    return NULL;
  } else {
    int i;
    ...
    for (i = 1; i <= cc->interface->nvar; i++) {
      ...
      if (strcmp(name, "Q") == 0)
        cc->Q = cc->interface->va + i;
      else if (strcmp(name, "SC") == 0)
        cc->SC = cc->interface->va + i;
    }
...
numout *getMEcode(int twidth, int Gauge, char *Process, char *excludeVirtual,
  ...
  if (Gauge)
    sprintf(lib_, "%s_u", lib);
  else
    strcpy(lib_, lib);
  if (twidth)
    strcat(lib_, "_t");
  for (test = allProc; test; test = test->next)
    if (strcmp(lib_, test->libname) == 0) {
  ...
  if (Process)
    Len += strlen(Process);
  if (excludeVirtual)
    Len += strlen(excludeVirtual);
  if (excludeOut)
    Len += strlen(excludeOut);
  command = malloc(Len);

  sprintf(proclibf, "%s/%s.so", libDir, lib_);

  if (access(proclibf, R_OK) == 0 && checkMtime(proclibf) == 0)
  ...
  if (!handle) {
    ...
    if (!handle) {
      ...
      if (excludeVirtual)
        sprintf(command + strlen(command), " \"%s\"", excludeVirtual);
      else
        sprintf(command + strlen(command), " \"\"");
      if (excludeOut)
        sprintf(command + strlen(command), " \"%s\"", excludeOut);
...
lVert *getVertex(char *n1, char *n2, char *n3, char *n4) {
  ...
  for (test = allVert; test; test = test->next)
    if (strcmp(lib, test->libname) == 0) {
      if (test->vv->nTerms == 0)
        return NULL;
      else
        return test->vv;
  ...
  if (!handle) {
    ...
    for (i = 0; i < 4; i++)
      if (fields[i])
        sprintf(vert_txt + strlen(vert_txt), " %s", fields[i]);
...
int getNumCoeff(lVert *vv, double *coeff) {
  int i;
  if (!vv) {
    printf("Call of  getNumCoeff with NULL argument. Program terminated\n");
    exit(1);
  }

  for (i = 0; i < vv->nVar; i++)
    if (vv->link[i] && ((REAL *)(vv->link[i]) - varValues) < *currentVarPtr)
      vv->varValues[i + 1] = *((REAL *)(vv->link[i]));
    else {
...
txtList makeProcList(char **InNames, char **OutNames, int nx) {
  ...
  */
  ...
  if (access(fnameG, R_OK) || checkMtime(fnameG)) {
    char *command =
        malloc(strlen(compDir) + strlen(calchepDir) + strlen(libDir) + 200);
...
int HiggsLambdas(double Q, char *Higgs, double complex *lAA,
                 double complex *lGG, double complex *lAA5,
                 double complex *lGG5) {
  if (lGG)
    *lGG = 0;
  if (lGG5)
  ...
  for (i = 0; i < nModelParticles; i++)
    if (strcmp(ModelPrtcls[i].name, Higgs) == 0)
      break;
  if (i == nModelParticles) {
    printf(" %s - no such particles\n", Higgs);
    return 0;
  }
  if (ModelPrtcls[i].spin2 != 0 || ModelPrtcls[i].cdim != 1 ||
      ModelPrtcls[i].q3 != 0) {
    ...
    if (strcmp(Xp, antiParticle(Xm)) == 0) {
      ...
      if ((charge3 != 0 || cdim != 1) && mX > 0) {
        ...
        for (k = 0; k < xxh->nTerms; k++) {
          ...
          switch (spin2) {
          case 0:
            if (strcmp(xxh->SymbVert[k], "1") == 0)
              addFF = 1;
            break;
          case 1:
            if (strcmp(xxh->SymbVert[k], "1") == 0)
              addFF = 1;
            else if (strcmp(xxh->SymbVert[k], "G5*i") == 0)
              addFA = 1;
            break;
          case 2:
            if (strcmp(xxh->SymbVert[k], "m2.m1") == 0)
              addFF = 1;
            break;
          }
          if (addFF) {
            if (lGG && cdim != 1)
              *lGG += hGGeven(Q, a, 1, spin2, cdim, (REAL)mXp,
                              (REAL)(coeff[k] / mN));
            if (lAA && charge3)
              *lAA += hAAeven(Q, a, 1, spin2, cdim, (REAL)mXp,
                              (REAL)(coeff[k] / mN)) *
                      charge3 * charge3 / 9.;
          }
          if (addFA) {
            if (lGG5 && cdim != 1)
              *lGG5 += 0.5 * hGGodd(Q, a, 1, spin2, cdim, (REAL)mXp,
                                    (REAL)(coeff[k] / mN));
            if (lAA5 && charge3)
              *lAA5 += 0.5 *
                       hAAodd(Q, a, 1, spin2, cdim, (REAL)mXp,
                              (REAL)(coeff[k] / mN)) *
                       charge3 * charge3 / 9.;
          }
        }
...
double complex lAAhiggs(double Q, char *hName) {
  double complex lAA;
  if (HiggsLambdas(Q, hName, &lAA, NULL, NULL, NULL))
    return lAA;
  else {
    FError = 1;
    return 0;
...
double complex lGGhiggs(double Q, char *hName) {
  double complex lGG;
  if (HiggsLambdas(Q, hName, NULL, &lGG, NULL, NULL))
    return lGG;
  else {
    FError = 1;
    return 0;
...
double complex lAA5higgs(double Q, char *hName) {
  double complex lAA5;
  if (HiggsLambdas(Q, hName, NULL, NULL, &lAA5, NULL))
    return lAA5;
  else {
    FError = 1;
    return 0;
...
double complex lGG5higgs(double Q, char *hName) {
  double complex lGG5;
  if (HiggsLambdas(Q, hName, NULL, NULL, NULL, &lGG5))
    return lGG5;
  else {
    FError = 1;
    return 0;
