...
int pTabPos(char *name) {
  ...
  for (i = 0; i < nModelParticles; i++) {
    if (!strcmp(name, ModelPrtcls[i].name))
      return i + 1;
    if (!strcmp(name, ModelPrtcls[i].aname))
      return -(i + 1);
  }
  return 0;
}
...
char *pdg2name(int pdg) {
  ...
  for (i = 0; i < nModelParticles; i++) {
    if (ModelPrtcls[i].NPDG == pdg)
      return ModelPrtcls[i].name;
    else {
      if (ModelPrtcls[i].NPDG == -pdg)
        return ModelPrtcls[i].aname;
    }
  }
  return NULL;
}
...
char *antiParticle(char *name) {
  int i;
  for (i = 0; i < nModelParticles; i++) {
    if (strcmp(ModelPrtcls[i].name, name) == 0)
      return ModelPrtcls[i].aname;
    if (strcmp(ModelPrtcls[i].aname, name) == 0)
      return ModelPrtcls[i].name;
  }
  return NULL;
}
...
int pNum(char *name) {
  ...
  if (n > 0)
    return ModelPrtcls[abs(n) - 1].NPDG;
  else
    return -ModelPrtcls[abs(n) - 1].NPDG;
}
...
int qNumbers(char *pname, int *spin2, int *charge3, int *cdim) {
  ...
  int pdg;
  if (!n) {
    if (pname[0] == '#' && sscanf(pname + 1, "%d", &pdg) == 1)
      return pdg;
    else
      return 0;
  }
  ...
  if (spin2)
    *spin2 = ModelPrtcls[n - 1].spin2;
  if (charge3)
    *charge3 = sign * ModelPrtcls[n - 1].q3;
