...
void copyfile(char *namefrom, char *nameto) {
  ...
  for (;;)
    if (fgets(s, STRSIZ, filefrom) != NULL)
      f_printf(fileto, s);
    else {
      fclose(fileto);
      fclose(filefrom);
...
static char *view_Dir(char *dirName) {
  ...
  if (!dirPtr) {
    messanykey(10, 10, "Wrong directory name");
    return NULL;
  }
    ...
    if (strcmp(dp->d_name, ".") && strcmp(dp->d_name, "..")) {
      ...
      if (stat(longname, &buf) == 0) {
        if (S_ISDIR(buf.st_mode)) {
          ...
          for (i = 0; (i < strlen(dp->d_name)) && (i < W - 1); i++)
            menustr[k++] = dp->d_name[i];
          menustr[k++] = '/';
          for (; i < W - 1; i++)
            menustr[k++] = ' ';
        } else {
          menustr1 = realloc(menustr1, k1 + W + 5);
          for (i = 0; (i < strlen(dp->d_name)) && (i < W); i++)
            menustr1[k1++] = dp->d_name[i];
          for (; i < W; i++)
            menustr1[k1++] = ' ';
        }
      }
...
int findCalcHEPfile(char *name) {
  ...
  for (;;) {
    ...
    if (key == KB_ENTER) {
      ...
      if (!fName_) {
        messanykey(10, 17, "Can not interpret beginning of path");
        continue;
      }
      if (strlen(fName) > 1 && strcmp(fName + strlen(fName) - 2, "/*"))
        break;
