...
static void *rd(char *name) {
  ...
  if (!res) {
    if (!rderrcode)
      rderrcode = cannotread;
    rderrpos = count - strlen(name) + 1;
  }
  if (rderrcode) {
    return NULL;
  }
...
static void *readSmplTerm(void) {
  ...
  if (len > ILEN) {
    ...
    sprintf(tmp, "%*.*s", len, len, source + m1);
  ...
  if (isdigit(tmp[0]) || tmp[0] == '"' || source[count] != '(')
    return rd(tmp);
  ...
  do {
    ...
    if (source[count] == ')')
      break;
    if (n == MAXFUN) {
      rderrcode = toomanyagruments;
      break;
    }
    if (!(tmp_arg[n] = readSum()))
      break;
    n++;
  } while (source[count] == ',');
  if (!rderrcode) {
  ...
  if (!rderrcode && !(s_res = act(tmp, n, tmp_arg)))
    rderrpos = m1 + 1;
  return s_res;
} /*  ReadSmplTerm  */
...
static void *readSum() {
  ...
  if (sgn == '-' || sgn == '+')
    ...
    sgn = '+';
  if (!(s_res = readMonom(3)))
    return NULL;

  if (sgn == '-' && !(s_res = act("-", 1, &s_res))) {
    rderrpos = sgn_pos;
    return NULL;
  }
  tmp_arg[0] = s_res;
...
char *errmesstxt(int errcode) {
  ...
  case toolongidentifier:
    return "Too long identifier/number";
  case toomanyagruments:
    return "Too many arguments";
  case cannotread:
    return "Undefined identifier";
  case cannotevaluate:
    return "Can not evaluate function";
  case unknownidentifier:
    return "Unknown identifier";
  case unexpectedoperation:
    return "Unexpected operator/function";
  ...
  case naninoperation:
    return "NAN in operation";
  case toolargenumber:
    return "Too large or float number";
  case indexuncompatibility:
    return "Index uncompatibility";
