...
int treeMSSM(void) {
  ...
  double QSUSY =
      sqrt(slhaVal("MASS", 91, 1, 1000006) * slhaVal("MASS", 91, 1, 2000006));
  ...
  double GF =
      (EE * EE) / pow(2 * SW * MW, 2) / Sqrt2; //  Fermi constant/ 1.166379E-5;
  ...
  double MSne = sqrt(c2b * (MW * MW) / 2 + (CW * CW) * Ml1 * Ml1) /
                CW; // e-sNeutrino mass;
  double MSnm = sqrt(c2b * (MW * MW) / 2 + (CW * CW) * Ml2 * Ml2) /
                CW; // mu-sneutrino mass;
  double MSeL = sqrt(-c2b * ((CW * CW) - (SW * SW)) * (MW * MW) / 2 +
                     (CW * CW) * Ml1 * Ml1) /
                CW; // 1st selectron mass;
  double MSeR = sqrt(-(SW * SW) * c2b * (MW * MW) + (CW * CW) * (Mr1 * Mr1)) /
                CW; // 2nd selectron mass;
  double MSmL = sqrt(-c2b * ((CW * CW) - (SW * SW)) * (MW * MW) / 2 +
                     (CW * CW) * Ml2 * Ml2) /
                CW; // 1st smuon mass;
  double MSmR = sqrt(-(SW * SW) * c2b * (MW * MW) + (CW * CW) * Mr2 * Mr2) /
                CW; // 2nd smuon mass;
  double MSuL = sqrt((Mq1 * Mq1) + (MW * MW) / (CW * CW) *
                                       (1. / 2. - 2. / 3. * (SW * SW)) * c2b);
  double MScL = sqrt((Mq2 * Mq2) + (MW * MW) / (CW * CW) *
                                       (1. / 2. - 2. / 3. * (SW * SW)) * c2b);
  double MSuR =
      sqrt((Mu1 * Mu1) + (MW * MW) / (CW * CW) * (2. / 3. * (SW * SW)) * c2b);
  double MScR =
      sqrt((Mu2 * Mu2) + (MW * MW) / (CW * CW) * (2. / 3. * (SW * SW)) * c2b);
  double MSdL = sqrt((Mq1 * Mq1) - (MW * MW) / (CW * CW) *
                                       (1. / 2. - 1. / 3. * (SW * SW)) * c2b);
  double MSsL = sqrt((Mq2 * Mq2) - (MW * MW) / (CW * CW) *
                                       (1. / 2. - 1. / 3. * (SW * SW)) * c2b);
  double MSdR =
      sqrt((Mu1 * Mu1) + (MW * MW) / (CW * CW) * (-1. / 3. * (SW * SW)) * c2b);
  double MSsR =
      sqrt((Mu2 * Mu2) + (MW * MW) / (CW * CW) * (-1. / 3. * (SW * SW)) * c2b);
  double MSnl = sqrt(c2b * (MW * MW) / 2 + (CW * CW) * (Ml3 * Ml3)) /
                CW; // tau-sneutrino mass;
  double lDiag = rDiagonal(
      2, Ml3 * Ml3 + Ml * Ml - (1. / 2. - (SW * SW)) * MZ * MZ * c2b,
      Ml * (Al - mu * tb), Mr3 * Mr3 + Ml * Ml - (SW * SW) * MZ * MZ * c2b);
  ...
  double QSUSY_ = pow((Mt11_ + (Mtp * Mtp)) * (Mt22_ + (Mtp * Mtp)) -
                          Mt12_ * Mt12_ * (Mtp * Mtp),
                      0.25);
  double MtMM = MtRun(QSUSY_);

  double tDiag =
      rDiagonal(2, Mt11_ + (MtMM * MtMM), MtMM * Mt12_, Mt22_ + MtMM * MtMM);
  ...
  double bDiag =
      rDiagonal(2,
                (Mq3 * Mq3) + MbMM * MbMM -
                    (1. / 2. - 1. / 3. * (SW * SW)) * (MZ * MZ) * c2b,
                MbMM * (Ab - mu * tb),
                Md3 * Md3 + MbMM * MbMM - 1 / 3 * (SW * SW) * (MZ * MZ) * c2b);
  ...
  for (int i = 1; i <= 4; i++)
    for (int j = 1; j <= 4; j++)
      Zn[i][j] = slhaVal("Nmix", QSUSY, 2, i, j);

  double MG1_ = Zn[1][1] * MNE[1] * Zn[1][1] + Zn[2][1] * MNE[2] * Zn[2][1] +
                Zn[3][1] * MNE[3] * Zn[3][1] + Zn[4][1] * MNE[4] * Zn[4][1];
  double MG2_ = Zn[1][2] * MNE[1] * Zn[1][2] + Zn[2][2] * MNE[2] * Zn[2][2] +
                Zn[3][2] * MNE[3] * Zn[3][2] + Zn[4][2] * MNE[4] * Zn[4][2];
  // printf("MG1=%E MG1_=%E MG2=%E MG2_=%E\n", MG1,MG1_,MG2,MG2_);
  MG1 = MG1_;
  MG2 = MG2_;

  double NeDiag = rDiagonal(4, MG1, zero, -MZ * SW * cb, MZ * SW * sb, MG2,
                            MZ * CW * cb, -MZ * CW * sb, zero, -mu, zero);
  ...
  double calcL =
      calcLambdas(tb, MH3, mu, QSUSY, Mtp, MSG, MC2, Mq3, Mu3, Md3, Ab, At);
  double la1 = Lambda1();
  double la2 = Lambda2();
  double la3 = Lambda3();
  double la4 = Lambda4();
  ...
  double la7 = -Lambda7();
  double MHc =
      sqrt((MH3 * MH3) + 2 / (EE * EE) * (MW * MW) * (SW * SW) * (la5 - la4));
  double Mhh11 =
      (VEV * VEV) * ((cb * cb) * la1 + (sb * sb) * la5 - 2 * cb * la6 * sb) +
      (MH3 * MH3) * (sb * sb);
  double Mhh12 = (VEV * VEV) * (sb * cb * (la3 + la4) - la6 * (cb * cb) -
                                la7 * (sb * sb)) -
                 (MH3 * MH3) * cb * sb;
  double Mhh22 =
      (VEV * VEV) * ((sb * sb) * la2 + (cb * cb) * la5 - 2 * sb * cb * la7) +
      (MH3 * MH3) * (cb * cb);
...
int treeEwsbMSSM(void) {
  ...
  //  double
  //  QSUSY=sqrt(findValW("MASS",91,1,1000006)*findValW("MASS",91,1,2000006));
  ...
  double calcL = calcLambdas(tb, MH3, mu, sqrt(MSt1 * MSt2), Mtp, MSG, MC2, Mq3,
                             Mu3, Md3, Ab, At);
  double la1 = Lambda1();
  double la2 = Lambda2();
  double la3 = Lambda3();
