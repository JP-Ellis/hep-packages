...
*/
{
  ...
  switch (number) {
  case 0: {
    if ((var < 1.01) && (var > 0.99))
      result = -1;
    else
      result = -2 / (pow((1 - var), 4)) *
               (1 - 6 * var + 3 * pow(var, 2) + 2 * pow(var, 3) -
                6 * pow(var, 2) * log(var));
    return (result);
    break;
  }
  case 2: {
    if ((var < 1.01) && (var > 0.99))
      ...
      result =
          3 / (2 * pow((1 - var), 3)) * (1 - pow(var, 2) + 2 * var * log(var));
    return (result);
    break;
  }
  case 1: {
    if ((var < 1.01) && (var > 0.99))
      ...
      result =
          2 / (pow((1 - var), 4)) *
          (2 + 3 * var - 6 * pow(var, 2) + pow(var, 3) + 6 * var * log(var));
    return (result);
    break;
  }
  case 3: {
    if ((var < 1.01) && (var > 0.99))
      ...
      result = -3 / (2 * pow((1 - var), 3)) *
               (3 - 4 * var + pow(var, 2) + 2 * log(var));
    return (result);
    break;
  }
  }
...
static double calcgmuon(int csboson, double mmuon, double mbosino,
                        double msfermion, double coeff1, double coeff2)
...

{
  ...
  result = mmuon / (16 * pow(M_PI, 2)) *
           (mmuon / (12 * pow(msfermion, 2)) * coeff1 * integr(csboson, x) +
            (2 * mbosino) / (3 * pow(msfermion, 2)) * coeff2 *
                integr(csboson + 2, x));
  return (result);
}
...
double gmuon_(void) {
  ...
  */
  ...
  double thw, cb, cw, sw, ymu, g1, masssneutr, g2, gmuo = 0, beta, Mz, Mw, Mmu,
                                                   e, tbeta;
  ...
  findVal("Zn11", &Nconj.e[0][0].r);
  findVal("Zn12", &Nconj.e[1][0].r);
  findVal("Zn13", &Nconj.e[2][0].r);
  findVal("Zn14", &Nconj.e[3][0].r);
  findVal("Zn21", &Nconj.e[0][1].r);
  findVal("Zn22", &Nconj.e[1][1].r);
  findVal("Zn23", &Nconj.e[2][1].r);
  findVal("Zn24", &Nconj.e[3][1].r);
  findVal("Zn31", &Nconj.e[0][2].r);
  findVal("Zn32", &Nconj.e[1][2].r);
  findVal("Zn33", &Nconj.e[2][2].r);
  findVal("Zn34", &Nconj.e[3][2].r);
  findVal("Zn41", &Nconj.e[0][3].r);
  findVal("Zn42", &Nconj.e[1][3].r);
  findVal("Zn43", &Nconj.e[2][3].r);
  findVal("Zn44", &Nconj.e[3][3].r);

  findVal("Zu11", &Uconj.e[0][0].r);
  findVal("Zu12", &Uconj.e[1][0].r);
  findVal("Zu21", &Uconj.e[0][1].r);
  findVal("Zu22", &Uconj.e[1][1].r);

  findVal("Zv11", &Vcomp.e[0][0].r);
  findVal("Zv12", &Vcomp.e[0][1].r);
  findVal("Zv21", &Vcomp.e[1][0].r);
  findVal("Zv22", &Vcomp.e[1][1].r);

  {
    double MSmuLL, MSmuRR, MSmuLR, Am, mu, MSmuth;

    MSmuLL = findValW("MSmL");
    MSmuLL *= MSmuLL;
    MSmuRR = findValW("MSmR");
    MSmuRR *= MSmuRR;
    Am = findValW("Am");
    mu = findValW("mu");
    ...
    massesmu.e[0].r = sqrt(
        (MSmuLL + MSmuRR -
         sqrt((MSmuLL - MSmuRR) * (MSmuLL - MSmuRR) + 4 * MSmuLR * MSmuLR)) /
        2);
    massesmu.e[1].r = sqrt(
        (MSmuLL + MSmuRR +
         sqrt((MSmuLL - MSmuRR) * (MSmuLL - MSmuRR) + 4 * MSmuLR * MSmuLR)) /
  ...
  for (i = 0; i < 4; i = i + 1)
    for (j = 0; j < 2; j = j + 1) {
      nc1 = somme(prodscal(sqrt(2) * g1, prod(N.e[i][0], X.e[j][1])),
                  prodscal(ymu, prod(N.e[i][2], X.e[j][0])));
      nc2 = diff(prodscal(1 / sqrt(2), prod(somme(prodscal(g2, N.e[i][1]),
                                                  prodscal(g1, N.e[i][0])),
                                            conjug(X.e[j][0]))),
                 prodscal(ymu, prod(N.e[i][2], conjug(X.e[j][1]))));
      ...
      gmuo = gmuo +
             calcgmuon(0, Mmu, massen.e[i].r, massesmu.e[j].r, coeff1, coeff2);
    }
