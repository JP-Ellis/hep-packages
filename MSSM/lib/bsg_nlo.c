...
/*
void calc_eps(struct read_param_tag* param, double *eps_b, double *eps_bp,
double *eps_tps); int read_prm(struct read_param_tag* param);
*/
...
#define ckmf                                                                   \
  0.9613 /* ckmf= abs(Vts*Vtb/Vcb)^2, ckmf=0.9613 corresponds in the           \
            Wolfenstein parametrisation to A=0.808, lamda=0.2253,                                                     \
            rhobar=0.132, etabar=0.341 , PDG 2010 */
#define lam2 0.12
#define mbs 50.           /* mb/ms*/
...
static double H2(double x, double y) {
  if (x - y < 0.0001)
    x += .0001;
  return x * log(x) / (1.0 - x) / (x - y) + y * log(y) / (1.0 - y) / (y - x);
}
#ifdef DEBUG

static double fga1(double x) {
  double x1 = x - 1.0;
  return (7.0 - 5.0 * x - 8.0 * x * x) / 36.0 / x1 / x1 / x1 +
         x * (3.0 * x - 2.0) / 6.0 / x1 / x1 / x1 / x1 * log(x);
}

static double fga2(double x) {
  double x1 = x - 1.0;
  return (3.0 - 5.0 * x) / 6.0 / x1 / x1 +
         (3.0 * x - 2.0) / 3.0 / x1 / x1 / x1 * log(x);
}

static double fg1(double x) {
  double x1 = x - 1.0;
  return (2.0 + 5.0 * x - x * x) / 12.0 / x1 / x1 / x1 -
         x / 2 / x1 / x1 / x1 / x1 * log(x);
}

static double fg2(double x) {
  double x1 = x - 1.0;
  return (3.0 - x) / 2.0 / x1 / x1 - log(x) / x1 / x1 / x1;
}

static double fga3(double x) {
  return (1.0 - x) * fga1(x) - x * fga2(x) / 2.0 - 23.0 / 36.0;
}

static double fg3(double x) {
  return (1.0 - x) * fg1(x) - x * fg2(x) / 2.0 - 1.0 / 3.0;
}

/*  B0 is really B0(0, x,y) and x=m^2/q^2 with q some scale*/
static double B0(double x, double y) {
  return 1 + .5 * ((x + y) / (x - y) * log(y / x) - log(x * y));
}
static double B1(double x, double y) {
  return .5 * (.5 + 1. / (1. - y / x) + log(y / x) / (1 - y / x) / (1 - y / x) -
               log(y));
}
#endif
...
static double Li2(double z) {
  ...
  if (z > -1) {
    return z + z * z / 4. + z * z * z / 9. + z4 / 16. + z4 * z / 25. +
           z4 * z * z / 36. + z4 * z * z * z / 49. + z8 / 64. + z8 * z / 81. +
           z8 * z * z / 100. + z8 * z * z * z / 121. + z8 * z4 / 144. +
           z8 * z4 * z / 13. / 13. + z8 * z4 * z * z / 14. / 14. +
           z8 * z4 * z * z * z / 225. + z8 * z8 / 16. / 16.;
  } else {
    return -.5 * log(-x) * log(-x) - M_PI * M_PI / 6. -
           (x + x * x / 4. + x * x * x / 9. + x4 / 16. + x4 * x / 25. +
            x4 * x * x / 36. + x4 * x * x * x / 49. + x8 / 64. + x8 * x / 81. +
            x8 * x * x / 100. + x8 * x * x * x / 121. + x8 * x4 / 144. +
            x8 * x4 * x / 13. / 13. + x8 * x4 * x * x / 14. / 14. +
            x8 * x4 * x * x * x / 225. + x8 * x8 / 16. / 16.);
  }
}
...
static double F81(double x) {
  double x1 = (x - 1);
  return x * (2. + 5. * x - x * x) / 8. / x1 / x1 / x1 -
         3. * x * x / 4. / x1 / x1 / x1 / x1 * log(x);
}
static double F72(double x) {
  double x1 = (x - 1);
  return x * (3. - 5. * x) / 12. / x1 / x1 +
         x * (3. * x - 2.) / 6. / x1 / x1 / x1 * log(x);
}
...
static double G71(double y) {
  double y1 = (y - 1);
  return 4 * (-3 + 7 * y - 2 * y * y) * Li2(1. - 1. / y) +
         (8. - 14. * y - 3. * y * y) * log(y) * log(y) / y1 +
         2 * (-3 - y + 12 * y * y - 2 * y * y * y) * log(y) / y1 +
         3. * (7. - 13. * y + 2 * y * y);
}
...
static double G72(double y) {
  ...
  return y * (18. - 37. * y + 8. * y * y) * Li2(1. - 1. / y) +
         ...
         (-50 + 251 * y - 174 * y * y - 192 * y * y * y + 21 * y * y * y * y) *
             log(y) / 9. / (y - 1.) +
...
static double G7H(double y, double tb) {
  double y1 = (y - 1);
  return -4. * y * G71(y) / 9. / (y1 * y1 * y1) +
         2 * y * G72(y) / (y1 * y1 * y1 * y1) / 9. / tb / tb;
}

static double D71(double y) {
  double y1 = (y - 1);
  return y1 * (21. - 47. * y + 8. * y * y) +
         2. * (-8. + 14. * y + 3. * y * y) * log(y);
}
...
static double EH(double y, double tb) {
  double y1 = (y - 1);
  return y * (y1 * (16. - 29. * y + 7. * y * y) + 6. * (3. * y - 2.) * log(y)) /
         36. / (y1 * y1 * y1 * y1) / tb / tb;
}

static double G81(double y) {
  double y1 = (y - 1);
  return .5 * (-36 + 25 * y - 17 * y * y) * Li2(1. - 1. / y) +
         (19 + 17 * y) * log(y) * log(y) / y1 +
         .25 * (-3. - 187. * y + 12. * y * y - 14. * y * y * y) * log(y) / y1 +
         3. * (143. - 44. * y + 29. * y * y) / 8.;
}
...
static double G8H(double y, double tb) {
  double y1 = (y - 1);
  return -y * G81(y) / 3. / (y1 * y1 * y1) +
         y * G82(y) / (y1 * y1 * y1 * y1) / 6. / tb / tb;
}
...
static double D82(double y) {
  double y1 = (y - 1);
  return y1 * (-38. - 261. * y + 18. * y * y - 7. * y * y * y) / 6. +
         y * (31. + 17. * y) * log(y);
}
...
static void c78_su(struct read_param_tag *param, double *, double *, double,
                   double);
static double yt, yb;
...
static double f78(double z) {
  return 8 *
         (Li2(1. - z) - M_PI * M_PI / 6. - z * log(z) + 9. * z / 4. -
          z * z / 4. + z * z * z / 12.) /
         9.;
}

static double f88(double z, double b) {
  return (4. * Li2(1. - z) - 2. * M_PI * M_PI / 3. - z * (2. + z) * log(z) +
          8. * log(1. - z) + 7. * z + 3. * z * z - 2. * z * z * z / 3. -
          2. * (2. * z + z * z + 4. * log(1 - z)) * log(b)) /
         27.;
}
...
double deltaMb(void) {
  ...
  double eta_s;
  double c70_w, c80_w, c20, c2_tum double c71Hw, c81Hw, c71_w_sm, c81_w_sm;
  double c80_mu, c70_mu, c71_w, c81_w, c71_sum, c81_tum, c71_wmu;
  double c7em_mu, cem8_mu, cem2_mu;
  double kap, rer2, rer8, r7, double K77, K27, K78, K22, K88, K28, kslem_mub;
  ...
  g1m = 404. / 3. - 200. / 9.;
  del = .9;
  /* Set the renormalisation scale mub, this value can be changed between
   * Mbpole/2 to 2 MbPole*/
  mub = MbPole;
  /* Set the renormalisation scale mubar, this value is the one relevant for
   * semileptonic B decays, can be changed between Mbpole/2 to 2 MbPole*/
  mubar = MbPole;
  ...
  delmt = alpsmt * g0m * (g1m / g0m - beta1 / beta0) * (alpsmw / alpsmt - 1.0) /
          8.0 / beta0 / M_PI;

  mtmw = mtp * (1 - 4 * alpsmt / 3. / M_PI) * pow(alpsmw / alpsmt, 12. / 23.) *
         (1. + delmt);
  mtmw2 = mtp * mtp * (1 - 8. * alpsmt / 3. / M_PI) *
          pow(alpsmw / alpsmt, 24. / 23.) * (1. + 2. * delmt);
...
double deltaMs(void) {
  ...
  eps_s = -2.0 / 3.0 * alpssu / M_PI * (param.mu - findValW("Ad") / param.tb) /
          mglu * H2(mLMSG * mLMSG, mRMSG * mRMSG);
  if (fabs(param.mu * param.mu / param.m2 / param.m2 - 1) < .01) {
    eps_s += alphew / param.sw / param.sw / 4.0 / M_PI *
             (-param.mu * param.m2) *
             (MSuL * MSuL / (param.m2 * param.m2 - MSuL * MSuL) /
                  (param.mu * param.mu - MSuL * MSuL) *
                  (log(MSuL * MSuL / param.m2 / param.m2) +
                   (param.m2 * param.m2 / MSuL / MSuL - 1.0)) +
              MSdL * MSdL / (param.m2 * param.m2 - MSdL * MSdL) /
                  (param.mu * param.mu - MSdL * MSdL) *
                  (log(MSdL * MSdL / param.m2 / param.m2) +
                   (param.m2 * param.m2 / MSdL / MSdL - 1.0)) /
                  2.);
  } else {
    eps_s += alphew / param.sw / param.sw / 4.0 / M_PI * (param.mu * param.m2) *
             (H2(param.m2 * param.m2 / MSuL / MSuL,
                 param.mu * param.mu / MSuL / MSuL) /
                  MSuL / MSuL +
              H2(param.m2 * param.m2 / MSdL / MSdL,
                 param.mu * param.mu / MSdL / MSdL) /
                  MSdL / MSdL / 2.);
  }
...
double deltaMl(void) {
  ...
  eps_l = -alphew / (1 - param.sw * param.sw) / 4.0 / M_PI *
          (param.mu / param.m1) * H2(msl11, msl21);
  //    printf("eps_l_m1=%.3e\n",eps_l*param.tb);
  eps_l +=
      -alphew / param.sw / param.sw / 4.0 / M_PI * (param.mu * param.m2) *
      H2(param.m2 * param.m2 / MSnu / MSnu, param.mu * param.mu / MSnu / MSnu) /
      MSnu / MSnu;
  //   printf("eps_l_tot=%.3e\n",eps_l*param.tb);
  //    printf("m2=%.3e mu=%.3e tb=%.3e h2=%.3e
...
double bsgnlo_(double *SMbsg) {
  ...
  c7_hp_mw =
      pow(eta_h, 16.0 / 21.0) * c70Hw +
      8. / 3. * (pow(eta_h, 2.0 / 3.0) - pow(eta_h, 16.0 / 21.0)) * c80Hw;
  ...
  /* Chargino contribution at mw scale , see  Degrassi et al., JHEP12(2000) 009
   * Eq.3.5*/

  c7_chi_mw =
      pow(eta_s, 16.0 / 21.0) * c7_su +
      8. / 3. * (pow(eta_s, 2.0 / 3.0) - pow(eta_s, 16.0 / 21.0)) * c8_su;
  ...
  c2_sum = 626126.0 / 272277.0 * pow(eta_wmu, 14.0 / 23.0) -
           ...
           0.6494 * pow(eta_wmu, 0.4086) - 0.0380 * pow(eta_wmu, -0.423) -
           0.0186 * pow(eta_wmu, -0.8994) - 0.0057 * pow(eta_wmu, 0.1456);

  c2_tum = -.9135 * pow(eta_wmu, .4086) + .0873 * pow(eta_wmu, (-.4230)) -
           .0571 * pow(eta_wmu, (-.8994)) + .0209 * pow(eta_wmu, .1456);

  /* Runnning from mw to mub  : SM+HIGGS+SUSY*/
  c80_mu = (c80_w + 313063. / 363036.) * pow(eta_wmu, (14. / 23.)) + c2_tum;
  c70_mu = c70_w * pow(eta_wmu, (16. / 23.)) +
           8. * (pow(eta_wmu, (14. / 23.)) - pow(eta_wmu, (16. / 23.))) *
               c80_w / 3. +
           c2_sum;

  /* Runnning from mw to mub  : SM only */
  c80sm_mu = (c80smw + 313063. / 363036.) * pow(eta_wmu, (14. / 23.)) + c2_tum;
  c70sm_mu = c70smw * pow(eta_wmu, (16. / 23.)) +
             8. * (pow(eta_wmu, (14. / 23.)) - pow(eta_wmu, (16. / 23.))) *
                 c80smw / 3. +
             c2_sum;
  ...
  x = xtw;
  c71_w = (-16. * x * x * x * x - 122. * x * x * x + 80. * x * x - 8. * x) *
              Li2(1. - 1. / x) / 9. / pow((x - 1.), 4.) +
          (6. * x * x * x * x + 46. * x * x * x - 28 * x * x) * log(x) *
              log(x) / 3. / pow((x - 1.), 5.);
  c71_w += (-102. * pow(x, 5.) - 588. * x * x * x * x - 2262. * x * x * x +
            3244. * x * x - 1364. * x + 208.) *
           log(x) / 81. / pow((x - 1), 5.);
  c71_w += (1646. * x * x * x * x + 12205. * x * x * x - 10740. * x * x +
            2509. * x - 436.) /
           486. / pow((x - 1.), 4.);

  c71_w_sm = c71_w;

  c81_w = (-4 * x * x * x * x + 40. * x * x * x + 41. * x * x + x) *
              Li2(1. - 1. / x) / 6. / pow((x - 1.), 4.) +
          (-17. * x * x * x - 31. * x * x) * log(x) * log(x) / 2. /
              pow((x - 1.), 5);
  c81_w += (-210. * pow(x, 5.) + 1086. * x * x * x * x + 4893. * x * x * x +
            2857. * x * x - 1994. * x + 280.) *
           log(x) / 216. / pow((x - 1.), 5.);
  c81_w += (737. * x * x * x * x - 14102. * x * x * x - 28209. * x * x +
            610. * x - 508.) /
           1296. / pow((x - 1.), 4.);
  c81_w_sm = c81_w;
  ...
  c71Hw =
      G7H(ytw, param.tb) +
      D7H(ytw, param.tb) * log(param.MW * param.MW / param.Mhc / param.Mhc) -
      4 * EH(ytw, param.tb) / 9;
  c81Hw =
      G8H(ytw, param.tb) +
      D8H(ytw, param.tb) * log(param.MW * param.MW / param.Mhc / param.Mhc) -
      EH(ytw, param.tb) / 6;
  ...
  c71_sum =
      (4661194 / 816831. * eta_wmu * E7(xtw) - 17.3023 + 14.8088 * eta_wmu) *
      ...
      (-1.9043 * eta_wmu * E7(xtw) + 2.0040 - 2.9347 * eta_wmu) *
          pow(eta_wmu, 0.4086) +
      (-.1008 * eta_wmu * E7(xtw) + .7476 + .3971 * eta_wmu) *
          pow(eta_wmu, -0.423) +
      (.1216 * eta_wmu * E7(xtw) - .5385 + .1600 * eta_wmu) *
          pow(eta_wmu, -0.8994) +
      (.0183 * eta_wmu * E7(xtw) + .0914 + 0.0225 * eta_wmu) *
          pow(eta_wmu, 0.1456);
  ...
  c71sm_wmu =
      c71_w_sm * pow(eta_wmu, 39 / 23.) +
      8. * (pow(eta_wmu, 37 / 23.) - pow(eta_wmu, 39 / 23.)) * c81_w / 3. +
      c71_sum + c81_tum * c80smw +
      37208. * (pow(eta_wmu, 39 / 23.) - pow(eta_wmu, 16 / 23.)) * c70smw /
          4761.;
  /* end SM*/
  ...
  c71_wmu =
      c71_w * pow(eta_wmu, 39 / 23.) +
      8. * (pow(eta_wmu, 37 / 23.) - pow(eta_wmu, 39 / 23.)) * c81_w / 3. +
      c71_sum + c81_tum * c80_w +
      37208. * (pow(eta_wmu, 39 / 23.) - pow(eta_wmu, 16 / 23.)) * c70_w /
          4761.;
  ...
  c7em_mu =
      (32 / 75. * pow(eta_wmu, -9 / 23.) - 40 / 69. * pow(eta_wmu, -7 / 23.) +
       88 / 575. * pow(eta_wmu, 16 / 23.)) *
          c70_w +
      cem8_mu * c80_w + cem2_mu;
  ...
  g27 = 416. / 81.;
  g87 = -32. / 9.;
  S_mub = exp(-2. * alps(mub, param.MZ, alpsmz) * log(del) *
              (log(del) + 7. / 2.) / 3. / M_PI);

  K77 = S_mub * (1 +
                 alps(mub, param.MZ, alpsmz) *
                     (r7 + ga77 * log(MbPole / mub) - 16 / 3.) / 2. / M_PI +
                 6. * (pow((1 - z0), 4) / f(z0) - 1) * lam2 / MbPole / MbPole) +
        alps(mub, param.MZ, alpsmz) * f77(del) / M_PI +
        S_mub * alps(mubar, param.MZ, alpsmz) * kap1 / M_PI / 2.;
  ...
  */
  ...
  K27 = S_mub * (alps(mub, param.MZ, alpsmz) *
                     (rer2 + g27 * log(MbPole / mub)) / 2. / M_PI -
                 lam2 / 9. / mcPole / mcPole) +
        alps(mub, param.MZ, alpsmz) * (f27(z1)) / M_PI;

  K78 = S_mub * alps(mub, param.MZ, alpsmz) * (rer8 + g87 * log(MbPole / mub)) /
            2. / M_PI +
  ...
  Knlo =
      K22 * c20 * c20 + K77 * c70sm_mu * c70sm_mu + K88 * c80sm_mu * c80sm_mu +
      K27 * c20 * c70sm_mu + K28 * c20 * c80sm_mu + K78 * c70sm_mu * c80sm_mu +
      S_mub * alps(mub, param.MZ, alpsmz) * c71sm_wmu * c70sm_mu / 2. / M_PI +
      S_mub * alphem *
          (2 * c7em_mu * c70sm_mu - c70sm_mu * c70sm_mu * kslem_mub) /
          alps(mub, param.MZ, alpsmz);
  if (SMbsg)
    *SMbsg = facbsg * Knlo;

  //	 printf("SM: bsg_lo=%.5e Knlo=%.5e
  //bsg_nlo=%.4e\n",bsg_lo_sm,Knlo,bsg_nlo);

  /* SM+Higgs+SUSY */

  bsg_lo = facbsg * (c70_mu * c70_mu);
  Knlo = K22 * c20 * c20 + K77 * c70_mu * c70_mu + K88 * c80_mu * c80_mu +
         K27 * c20 * c70_mu + K28 * c20 * c80_mu + K78 * c70_mu * c80_mu +
...
static double F73(double x) {
  double x1 = (x - 1);
  return (5. - 7. * x) / 6. / x1 / x1 +
         x * (3. * x - 2.) / 3. / x1 / x1 / x1 * log(x);
}
...
void calc_eps(struct read_param_tag *param, double *eps_b, double *eps_bp,
  ...
  /* Add the full corrections to eps_b, does not include the terms subdominant
   * in tanbeta*/

  /*	eps_b=-2.0/3.0*alpssu/M_PI*(mu-Ab/tb)/mglu*
                          H2(msb[1]*msb[1]/mglu/mglu,msb[2]*msb[2]/mglu/mglu);
  */
  *eps_b = -2.0 / 3.0 * alpssu / M_PI * (param->mu - param->Ab / param->tb) /
           param->mglu *
           H2(param->msb[1] * param->msb[1] / param->mglu / param->mglu,
              param->msb[2] * param->msb[2] / param->mglu / param->mglu);
  ...
  *eps_b +=
      -yt * yt / 16.0 / M_PI / M_PI * (param->At - param->mu / param->tb) *
      (param->U[1][2] * param->V[1][2] / param->mc[1] *
           H2(param->mst[1] * param->mst[1] / param->mc[1] / param->mc[1],
              param->mst[2] * param->mst[2] / param->mc[1] / param->mc[1]) +
       param->U[2][2] * param->V[2][2] / param->mc[2] *
           H2(param->mst[1] * param->mst[1] / param->mc[2] / param->mc[2],
              param->mst[2] * param->mst[2] / param->mc[2] / param->mc[2]));
  ...
  */
  ...
  if (fabs(param->mu * param->mu / param->m2 / param->m2 - 1) < .01) {
    *eps_b +=
        alphew / param->sw / param->sw / 4.0 / M_PI * (-param->mu * param->m2) *
        (CT * CT * param->mst[1] * param->mst[1] /
             (param->m2 * param->m2 - param->mst[1] * param->mst[1]) /
             (param->mu * param->mu - param->mst[1] * param->mst[1]) *
             (log(param->mst[1] * param->mst[1] / param->m2 / param->m2) +
              (param->m2 * param->m2 / param->mst[1] / param->mst[1] - 1.0)) +
         ST * ST * param->mst[2] * param->mst[2] /
             (param->m2 * param->m2 - param->mst[2] * param->mst[2]) /
             (param->mu * param->mu - param->mst[2] * param->mst[2]) *
             (log(param->mst[2] * param->mst[2] / param->m2 / param->m2) +
              (param->m2 * param->m2 / param->mst[2] / param->mst[2] - 1.0)) +
         CB * CB * param->msb[1] * param->msb[1] /
             (param->m2 * param->m2 - param->msb[1] * param->msb[1]) /
             (param->mu * param->mu - param->msb[1] * param->msb[1]) *
             (log(param->msb[1] * param->msb[1] / param->m2 / param->m2) +
              (param->m2 * param->m2 / param->msb[1] / param->msb[1] - 1.0)) /
             2. +
         SB * SB * param->msb[2] * param->msb[2] /
             (param->m2 * param->m2 - param->msb[2] * param->msb[2]) /
             (param->mu * param->mu - param->msb[2] * param->msb[2]) *
             (log(param->msb[2] * param->msb[2] / param->m2 / param->m2) +
              (param->m2 * param->m2 / param->msb[2] / param->msb[2] - 1.0)) /
             2.);
  } else {
    *eps_b += alphew / param->sw / param->sw / 4.0 / M_PI *
              (param->mu * param->m2) *
              ((CT * CT *
                    H2(param->m2 * param->m2 / param->mst[1] / param->mst[1],
                       param->mu * param->mu / param->mst[1] / param->mst[1]) /
                    param->mst[1] / param->mst[1] +
                ST * ST *
                    H2(param->m2 * param->m2 / param->mst[2] / param->mst[2],
                       param->mu * param->mu / param->mst[2] / param->mst[2]) /
                    param->mst[2] / param->mst[2]) +
               CB * CB *
                   H2(param->m2 * param->m2 / param->msb[1] / param->msb[1],
                      param->mu * param->mu / param->msb[1] / param->msb[1]) /
                   param->msb[1] / param->msb[1] / 2. +
               SB * SB *
                   H2(param->m2 * param->m2 / param->msb[2] / param->msb[2],
                      param->mu * param->mu / param->msb[2] / param->msb[2]) /
                   param->msb[2] / param->msb[2] / 2.);
  }
  // printf("eps_b ewk=%.3e\n",eps_b);
  ...
  /*	yb=yb/(1.0+eps_b*tb);*/
  *eps_bp = -2.0 / 3.0 * alpssu / M_PI * (param->mu - param->Ab / param->tb) /
            param->mglu *
            (CT * CT * CB * CB *
                 H2(param->mst[1] * param->mst[1] / param->mglu / param->mglu,
                    param->msb[2] * param->msb[2] / param->mglu / param->mglu) +
             CT * CT * SB * SB *
                 H2(param->mst[1] * param->mst[1] / param->mglu / param->mglu,
                    param->msb[1] * param->msb[1] / param->mglu / param->mglu) +
             ST * ST * CB * CB *
                 H2(param->mst[2] * param->mst[2] / param->mglu / param->mglu,
                    param->msb[2] * param->msb[2] / param->mglu / param->mglu) +
             ST * ST * SB * SB *
                 H2(param->mst[2] * param->mst[2] / param->mglu / param->mglu,
                    param->msb[1] * param->msb[1] / param->mglu / param->mglu));
  ...
  for (i = 1; i <= 4; i++)
    *eps_bp +=
        yt * yt / 16.0 / M_PI / M_PI * param->N[i][4] * param->N[i][3] *
        (param->At - param->mu / param->tb) / param->mn[i] *
        (CT * CT * CB * CB *
             H2(param->mst[2] * param->mst[2] / param->mn[i] / param->mn[i],
                param->msb[1] * param->msb[1] / param->mn[i] / param->mn[i]) +
         CT * CT * SB * SB *
             H2(param->mst[2] * param->mst[2] / param->mn[i] / param->mn[i],
                param->msb[2] * param->msb[2] / param->mn[i] / param->mn[i]) +
         ST * ST * CB * CB *
             H2(param->mst[1] * param->mst[1] / param->mn[i] / param->mn[i],
                param->msb[1] * param->msb[1] / param->mn[i] / param->mn[i]) +
         ST * ST * SB * SB *
             H2(param->mst[1] * param->mst[1] / param->mn[i] / param->mn[i],
                param->msb[2] * param->msb[2] / param->mn[i] / param->mn[i]));
  ...
  if (fabs(param->mu * param->mu / param->m2 / param->m2 - 1) < .01) {
    ...
    *eps_bp += alphew / param->sw / param->sw / 4.0 / M_PI *
               (param->mu * param->m2) *
               ((CT * CT *
                     H2(param->m2 * param->m2 / param->mst[1] / param->mst[1],
                        param->mu * param->mu / param->mst[1] / param->mst[1]) /
                ...
                (CB * CB *
                     H2(param->m2 * param->m2 / param->msb[1] / param->msb[1],
                        param->mu * param->mu / param->msb[1] / param->msb[1]) /
                     param->msb[1] / param->msb[1] +
                 SB * SB *
                     H2(param->m2 * param->m2 / param->msb[2] / param->msb[2],
                        param->mu * param->mu / param->msb[2] / param->msb[2]) /
                     param->msb[2] / param->msb[2]));
  }
  ...
  *eps_tps =
      -2.0 / 3.0 * alpssu / M_PI * (param->mu + param->At / param->tb) /
      param->mglu *
      (CT * CT *
           H2(param->mst[2] * param->mst[2] / param->mglu / param->mglu,
              param->mss[1] * param->mss[1] / param->mglu / param->mglu) +
       ST * ST *
           H2(param->mst[1] * param->mst[1] / param->mglu / param->mglu,
              param->mss[1] * param->mss[1] / param->mglu / param->mglu));
  ...
  for (i = 1; i <= 4; i++)
    *eps_tps +=
        yb * yb / 16.0 / M_PI / M_PI * param->N[i][4] * param->N[i][3] *
        (param->mu / param->tb) / param->mn[i] *
        (CT * CT * CB * CB *
             H2(param->mst[1] * param->mst[1] / param->mn[i] / param->mn[i],
                param->msb[2] * param->msb[2] / param->mn[i] / param->mn[i]) +
         CT * CT * SB * SB *
...
/* Leading order chargino contribution contains the leading higher order effect
 * assumes that
 * sparticles have mass  1 TeV with possibly lighter charginos and Higgs*/

static void c78_su(struct read_param_tag *param, double *c7_su, double *c8_su,
                   double eps_b, double mtsu) {
#ifdef DEBUG
  double t1, t2;
  double eta, bet0, k1;
#endif
  ...
  {
    t += 2.0 / 3.0 * param->MW * param->MW / param->msq1 / param->msq1 *
         param->V[a][1] * param->V[a][1] *
         F71(param->msq1 * param->msq1 / param->mc[a] / param->mc[a]);

    t -= 2.0 / 3.0 *
         (CT * param->V[a][1] -
          ST * param->V[a][2] * mtsu / sq2 / sb / param->MW) *
         (CT * param->V[a][1] -
          ST * param->V[a][2] * mtsu / sq2 / sb / param->MW) *
         param->MW * param->MW / param->mst[1] / param->mst[1] *
         F71(param->mst[1] * param->mst[1] / param->mc[a] / param->mc[a]);

    t -= 2.0 / 3.0 *
         (ST * param->V[a][1] +
          CT * param->V[a][2] * mtsu / sq2 / sb / param->MW) *
         (ST * param->V[a][1] +
          CT * param->V[a][2] * mtsu / sq2 / sb / param->MW) *
         param->MW * param->MW / param->mst[2] / param->mst[2] *
         F71(param->mst[2] * param->mst[2] / param->mc[a] / param->mc[a]);

    t += k / cb * param->U[a][2] * param->V[a][1] * param->MW / sq2 /
         param->mc[a] *
         (F73(param->msq1 * param->msq1 / param->mc[a] / param->mc[a]) -
          CT * CT *
              F73(param->mst[1] * param->mst[1] / param->mc[a] / param->mc[a]) -
          ST * ST *
              F73(param->mst[2] * param->mst[2] / param->mc[a] / param->mc[a]));
    t += k / cb * CT * ST * param->U[a][2] * param->V[a][2] * mtsu / 2.0 / sb /
         param->mc[a] *
         (F73(param->mst[1] * param->mst[1] / param->mc[a] / param->mc[a]) -
          F73(param->mst[2] * param->mst[2] / param->mc[a] / param->mc[a]));
  }
  ...
  *c7_su = t;
  ...
  {
    t8 += 2.0 / 3.0 * param->MW * param->MW / param->msq1 / param->msq1 *
          param->V[a][1] * param->V[a][1] *
          F81(param->msq1 * param->msq1 / param->mc[a] / param->mc[a]);
    t8 -= 2.0 / 3.0 *
          (CT * param->V[a][1] -
           ST * param->V[a][2] * mtsu / sq2 / sb / param->MW) *
          (CT * param->V[a][1] -
    ...
    t8 +=
        k / cb * param->U[a][2] * param->V[a][1] * param->MW / sq2 /
        param->mc[a] *
        (F83(param->msq1 * param->msq1 / param->mc[a] / param->mc[a]) -
         CT * CT *
             F83(param->mst[1] * param->mst[1] / param->mc[a] / param->mc[a]) -
         ST * ST *
             F83(param->mst[2] * param->mst[2] / param->mc[a] / param->mc[a]));

    t8 += k / cb * ST * CT * param->U[a][2] * param->V[a][2] * mtsu / 2.0 / sb /
          param->mc[a] *
          (F83(param->mst[1] * param->mst[1] / param->mc[a] / param->mc[a]) -
           F83(param->mst[2] * param->mst[2] / param->mc[a] / param->mc[a]));
  }
...
*/
  ...
  */
  ...
  //  printf("alphasMZ =%.3e \n",param->alphS_MZ);
  {
    ...
    param->msq1 = sqrt(
        MSuL * MSuL - (MZ * MZ * (0.5 - 2 / 3. * SW * SW) * (1 - 2 * sb * sb)));
  }
