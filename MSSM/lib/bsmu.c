...
static double D2(double x, double y) {
  if (fabs(y - 1.) < .01) {
    return 1.0 / (x - y) * (x * log(x) / (1.0 - x) + 1.);
  }
  if (fabs(x - y) < .0001) {
    return 1.0 / (1. - x) * (1. + log(x) / (1.0 - x));
  }
...
static double D1(double x, double y, double z) {
  if (fabs(z - 1) < .0001) {
    return (x * log(x) / (1.0 - x) / (x - y) / (x - z) +
            y * log(y) / (1.0 - y) / (y - x) / (y - z) -
            1. / (z - y) / (z - x));
  }
  return (x * log(x) / (1.0 - x) / (x - y) / (x - z) +
          y * log(y) / (1.0 - y) / (y - x) / (y - z) +
...
double bsmumu_(void) {
  ...
  /*	printf("betamu %.3E %.3E  \n",betamu,facbsmu);
     ...
     \n",Cbox_S,Cpen_S,Cep_S,Cs_susy,CsHc,Yxt); printf("Box,pen,epP %.3E %.3E
     %.3E
  ...
  */
  ...
  bsmu = facbsmu *
         (betamu * betamu * (Cs_susy * Cs_susy) * MBs * MBs * MBs * MBs / 16. /
              param.MW / param.MW / param.MW / param.MW +
          (MBs * MBs / 4. / param.MW / param.MW * Cp_susy - 2. * Yxt) *
              (MBs * MBs / 4. / param.MW / param.MW * Cp_susy - 2. * Yxt));
  /* for test with keps_b factor*/
  bsmuk =
      facbsmu * (betamu * betamu * (csk * csk) * MBs * MBs * MBs * MBs / 16. /
                     param.MW / param.MW / param.MW / param.MW +
                 (MBs * MBs / 4. / param.MW / param.MW * cpk - 2. * Yxt) *
                     (MBs * MBs / 4. / param.MW / param.MW * cpk - 2. * Yxt));
...
static void Cep_sp(struct read_param_tag *param, double *Cep_S, double *Cep_P,
                   double mtmt)

{
  ...
  for (a = 1; a <= 2; a++) {
    ...
    t += param->mc[a] * ST * CT * mtmt * param->V[a][2] * param->U[a][2] *
         (D3(yst1) - D3(yst2));
    t += -param->mc[a] * sq2 * param->MW * param->V[a][1] * param->U[a][2] *
         (CT * CT * D3(yst1) + ST * ST * D3(yst2) -
          D3(param->msq1 * param->msq1 / param->mc[a] / param->mc[a]));
  }

  *Cep_S = t * param->tb * param->tb * param->tb /
           (param->Mhc * param->Mhc - param->MW * param->MW);

  *Cep_P = -t * param->tb * param->tb * param->tb /
           (param->Mhc * param->Mhc - param->MW * param->MW);
}

static void Cbox_sp(struct read_param_tag *param, double *Cbox_S,
                    double *Cbox_P, double mtmt)

{
  ...
  for (i = 1; i <= 2; i++) {
    ...
    for (j = 1; j <= 2; j++) {
      ...
      f1 = (yst1 * param->U[j][2] * param->V[i][1] +
            param->mc[j] / param->mc[i] * param->U[i][2] * param->V[j][1]) *
           D1(xli, yst1, zji);
      f2 = (yst2 * param->U[j][2] * param->V[i][1] +
            param->mc[j] / param->mc[i] * param->U[i][2] * param->V[j][1]) *
           D1(xli, yst2, zji);
      fq = (ysq * param->U[j][2] * param->V[i][1] +
            param->mc[j] / param->mc[i] * param->U[i][2] * param->V[j][1]) *
           D1(xli, ysq, zji);

      t += param->U[j][2] / param->mc[i] / param->mc[i] * ST * CT * mtmt *
           param->V[i][2] * (f1 - f2);
      t += -param->U[j][2] / param->mc[i] / param->mc[i] * sq2 * param->MW *
           param->V[i][1] * (CT * CT * f1 + ST * ST * f2 - fq);
      f1p = (yst1 * param->U[j][2] * param->V[i][1] -
             param->mc[j] / param->mc[i] * param->U[i][2] * param->V[j][1]) *
            D1(xli, yst1, zji);
      f2p = (yst2 * param->U[j][2] * param->V[i][1] -
             param->mc[j] / param->mc[i] * param->U[i][2] * param->V[j][1]) *
            D1(xli, yst2, zji);
      fqp = (ysq * param->U[j][2] * param->V[i][1] -
             param->mc[j] / param->mc[i] * param->U[i][2] * param->V[j][1]) *
            D1(xli, ysq, zji);
      tp += param->U[j][2] / param->mc[i] / param->mc[i] * ST * CT * mtmt *
            param->V[i][2] * (f1p - f2p);
      tp += -param->U[j][2] / param->mc[i] / param->mc[i] * sq2 * param->MW *
            param->V[i][1] * (CT * CT * f1p + ST * ST * f2p - fqp);
    }
  }
  /*printf("Box %.3E %.3E %.3E \n",zji,t,tp);
  printf("Box %.3E %.3E %.3E \n",f1,f2,fq);
...
static void Cpen_sp(struct read_param_tag *param, double *Cpen_S,
                    double *Cpen_P, double mtmt) {
  int a, i, j;
  ...
  for (i = 1; i <= 2; i++) {
    for (j = 1; j <= 2; j++) {
      ...
      g1 = (yst1 * param->U[j][2] * param->V[i][1] +
            param->mc[i] / param->mc[j] * param->U[i][2] * param->V[j][1]) *
           D2(yst1, zij);
      g2 = (yst2 * param->U[j][2] * param->V[i][1] +
            param->mc[i] / param->mc[j] * param->U[i][2] * param->V[j][1]) *
           D2(yst2, zij);
      gq = (ysq * param->U[j][2] * param->V[i][1] +
            param->mc[i] / param->mc[j] * param->U[i][2] * param->V[j][1]) *
           D2(ysq, zij);

      t += -param->U[j][2] * ST * CT * mtmt * param->V[i][2] * (g1 - g2);

      t += param->U[j][2] * sq2 * param->MW * param->V[i][1] *
           (CT * CT * g1 + ST * ST * g2 - gq);

      g1p = (yst1 * param->U[j][2] * param->V[i][1] -
             param->mc[i] / param->mc[j] * param->U[i][2] * param->V[j][1]) *
            D2(yst1, zij);
      g2p = (yst2 * param->U[j][2] * param->V[i][1] -
             param->mc[i] / param->mc[j] * param->U[i][2] * param->V[j][1]) *
            D2(yst2, zij);
      gqp = (ysq * param->U[j][2] * param->V[i][1] -
             param->mc[i] / param->mc[j] * param->U[i][2] * param->V[j][1]) *
            D2(ysq, zij);

      tp += -param->U[j][2] * ST * CT * mtmt * param->V[i][2] * (g1p - g2p);
      tp += param->U[j][2] * sq2 * param->MW * param->V[i][1] *
            (CT * CT * g1p + ST * ST * g2p - gqp);
    }
  }
  ...
  for (a = 1; a <= 2; a++) {
    yst1 = param->mst[1] * param->mst[1] / param->mc[a] / param->mc[a];
    yst2 = param->mst[2] * param->mst[2] / param->mc[a] / param->mc[a];
    ts2 += param->U[a][2] / param->mc[a] *
           (-(sq2 * param->MW * param->V[a][1] * CT * CT -
              mtmt * param->V[a][2] * ST * CT) *
            D4(yst1));
    ts2 += param->U[a][2] / param->mc[a] *
           (+(sq2 * param->MW * param->V[a][1] * ST * ST +
              mtmt * param->V[a][2] * ST * CT) *
            D4(yst2));
    ts2 += param->U[a][2] / param->mc[a] *
           (sq2 * param->MW * param->V[a][1] * (CT * CT - ST * ST) *
            D2(yst1, yst2));
  }
  ...
  for (a = 1; a <= 2; a++) {
    yst1 = param->mst[1] * param->mst[1] / param->mc[a] / param->mc[a];
    yst2 = param->mst[2] * param->mst[2] / param->mc[a] / param->mc[a];
    tp2 += mtmt * mtmt * param->mu * param->U[a][2] * param->V[a][2] /
           param->mc[a] * D2(yst1, yst2);
  }

  *Cpen_S = param->tb * param->tb /
            (param->Mhc * param->Mhc - param->MW * param->MW) *
            (sq2 * param->MW * t + 2. * CT * ST * mtmt * param->mu * ts2);
  *Cpen_P = -param->tb * param->tb /
            (param->Mhc * param->Mhc - param->MW * param->MW) *
            (sq2 * param->MW * tp + tp2);
}
