...
/* Main program */ int main(int argv, char **argc) {
  FILE *f;
  ...
  /* Local variables */
  static double v, q0;
  extern /* Subroutine */ int subh1_hdec__(
      double *, double *, double *, double *, double *, double *, double *,
      double *, double *, double *, double *, double *, double *, double *,
      double *, double *, double *, double *, double *);
  static double ca, ad, cb, cf, ma, qb, sa, sb, au, pi, ft, xb, mu, qt, xt, ad0,
      au0, dmb, mch, bet, rmb, amu, mhp, err, hmp, rmt, amh12, amh22, albp,
      qcdl, amdr, tanb, alsp, altp, amsq, mglu, amur, zero, mtop, amdl0, mchi0,
      susy, amdr0, amul0, amur0;
  ...
  char *fin, *fout;
  if (argv < 2) {
    printf("%s  needs 2 arguments:\n, 1) slha input file 2) output file for "
           "lambdas\n",
           argc[0]);
    exit(1);
  }
  ...
  /*<        >*/
  susy =
      sqrt(slhaVal("MASS", zero, 1, c_b28) * slhaVal("MASS", zero, 1, c_b30));
  ...
  d__5 = sb;
  d__6 = amu;
  amh12 = d__1 * d__1 * (d__2 * d__2) -
          d__3 * d__3 / 2 * (d__4 * d__4 - d__5 * d__5) - d__6 * d__6;
  ...
  d__1 = qb;
  d__2 = q0;
  ad0 = ad + (cf * alsp * mglu + altp * 3 / 2 * au + albp / 4 * ad) *
                 log(d__1 * d__1 / (d__2 * d__2));
  /*<        >*/
  d__1 = amsq;
  d__2 = mglu;
  d__3 = qb;
  d__4 = q0;
  amdl0 = sqrt(d__1 * d__1 +
               (-cf * alsp * (d__2 * d__2) + (altp * xt + albp * xb) / 4) *
  ...
  d__1 = qt;
  d__2 = q0;
  au0 = au + (cf * alsp * mglu + altp / 4 * au + albp * 3 / 2 * ad) *
                 log(d__1 * d__1 / (d__2 * d__2));
  /*<        >*/
  d__1 = amsq;
  d__2 = mglu;
  ...
  amur0 = sqrt(d__1 * d__1 + (-cf * alsp * (d__2 * d__2) + altp * xt / 4) *
                                 log(d__3 * d__3 / (d__4 * d__4)));
  /*<        >*/
  subh1_hdec__(&ma, &tanb, &amul0, &amdl0, &amur0, &amdr0, &mtop, &au0, &ad0,
  ...
  fprintf(f, "BLOCK LAMBDAS\n");
  /*<       write(22,fmt='(1x,I3,1PE12.4)') 1,Lambda1 >*/
  fprintf(f, " 1 %E\n", hself_hdec__1.lambda1);
  fprintf(f, " 2 %E\n", hself_hdec__1.lambda2);
  fprintf(f, " 3 %E\n", hself_hdec__1.lambda3);
  fprintf(f, " 4 %E\n", hself_hdec__1.lambda4);
  fprintf(f, " 5 %E\n", hself_hdec__1.lambda5);
  fprintf(f, " 6 %E\n", hself_hdec__1.lambda6);
  fprintf(f, " 7 %E\n", hself_hdec__1.lambda7);
  /*<       close(22) >*/
  fclose(f);
  /*<       END >*/
  return 0;
...
/*<        >*/
/* Subroutine */ int
subh1_hdec__(double *ma, double *tanb, double *mul, double *mdl, double *mur,
             double *md, double *mtop, double *au, double *ad, double *mu,
             double *mchi0, double *mhp, double *hmp, double *mch, double *sa,
             double *ca, double *tanba, double *mglu, double *deltamb) {
  /* System generated locals */
  double d__1, d__2, d__3, d__4, d__5, d__6, d__7, d__8, d__9, d__10, d__11,
      d__12, d__13, d__14;

  /* Builtin functions */
  double atan(double), sqrt(double), log(double), pow_dd(double *, double *),
      asin(double), sin(double), cos(double);

  /* Local variables */
  static double deltal3p4, dlambdap2, cos2alpha, sin2alpha, v;
  extern /* Subroutine */ int gfun_hdec__(
      double *, double *, double *, double *, double *, double *, double *,
      double *, double *, double *, double *, double *, double *, double *,
      double *, double *, double *, double *, double *);
  static double g1, g2, g3, m2[4] /* was [2][2] */, hd, mb, td, pi, cw, hu, mw,
      tp, mz, tu, sw, m2p[4] /* was [2][2] */, tdl, tdp, tpd, tul, mch2, mh2p,
      hm2p, mchi, cosb, sinb, tdpd, tglu, dlam1, dlam2, dlam3, dlam4, dlam5,
      dlam6, dlam7, cos2b, trm2p, alpha, tchar, sqbma, tanbt;
  static double rmtop;
  static double alpha1, alpha2, alpha3, detm2p, alpha3z, mssusy, deltal12,
      dlambda1, dlambda2, dlambda3, dlambda4, deltam112, deltam122, deltam222;
  ...
  *tanba = *tanb;
  ...
  if (*ma > *mtop) {
    ...
    d__8 = *ma;
    d__9 = *mtop;
    *tanba = *tanb * (1. - .09375 / (d__1 * d__1) *
                               (d__2 * d__2 / (d__3 * d__3) / (d__4 * d__4) -
                                d__5 * d__5 / (d__6 * d__6) / (d__7 * d__7)) *
                               log(d__8 * d__8 / (d__9 * d__9)));
  }
  ...
  if (*mdl > *md) {
    ...
    d__2 = g1;
    d__3 = hd;
    dlambda1 = .0625f / (d__1 * d__1) * (d__2 * d__2) * (d__3 * d__3) * tpd;
  }
  /*<        >*/
  if (*mdl < *md || *mdl == *md) {
    d__1 = pi;
    ...
    d__3 = g1;
    d__4 = g2;
    dlambda1 = .09375f / (d__1 * d__1) * (d__2 * d__2) *
               (d__3 * d__3 / 3.f + d__4 * d__4) * tpd;
  }
  ...
  if (*mul < *mur || *mul == *mur) {
    ...
    d__3 = g1;
    d__4 = g2;
    dlambda2 = .09375f / (d__1 * d__1) * (d__2 * d__2) *
               (-(d__3 * d__3) / 3.f + d__4 * d__4) * tp;
  }
  /* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
  ...
  if (*mglu < *mur || *mglu < *mul) {
    ...
    if (*mul > *mur && *mglu > *mur) {
      ...
      d__5 = tglu;
      dlambdap2 =
          -4.f / (d__1 * d__1) * (d__3 * d__3) * (d__4 * d__4 - d__5 * d__5);
      /*<         endif >*/
    }
  ...
  if (*mdl > *md) {
    ...
    d__2 = g1;
    d__3 = hd;
    dlambda3 = -.03125f / (d__1 * d__1) * (d__2 * d__2) * (d__3 * d__3) * tpd;
  }
  /*<        >*/
  if (*mdl < *md || *mdl == *md) {
    d__1 = pi;
    ...
    d__3 = g2;
    d__4 = g1;
    dlambda3 = .046875f / (d__1 * d__1) * (d__2 * d__2) *
               (d__3 * d__3 - d__4 * d__4 / 3.f) * tpd;
  }
  /*<        >*/
  if (*mul > *mur) {
    d__1 = pi;
    d__2 = g1;
    d__3 = hu;
    dlambda3 -= .0625f / (d__1 * d__1) * (d__2 * d__2) * (d__3 * d__3) * tp;
  }
  /*<        >*/
  if (*mul < *mur || *mul == *mur) {
    d__1 = pi;
  ...
  if (*mul < *mur) {
    ...
    d__2 = g2;
    d__3 = hu;
    dlambda4 = -.09375f / (d__1 * d__1) * (d__2 * d__2) * (d__3 * d__3) * tp;
  }
  /*<        >*/
  if (*mdl < *md) {
    d__1 = pi;
    d__2 = g2;
    d__3 = hd;
    dlambda4 -= .09375f / (d__1 * d__1) * (d__2 * d__2) * (d__3 * d__3) * tpd;
  }
  ...
  d__14 = pi;
  hself_hdec__1.lambda1 =
      (d__1 * d__1 + d__2 * d__2) / 4.f *
          (1.f - d__3 * d__3 * 3.f * (tpd + tdpd) / 8.f / (d__4 * d__4)) +
      pow(hd, c_b83) * 3.f / 16.f / (d__5 * d__5) * tpd *
          ((d__6 * d__6 * 3.f / 2.f + d__7 * d__7 / 2.f - d__8 * d__8 * 8.f) *
               (tpd + tdpd * 2.f) / 16.f / (d__9 * d__9) +
           1.f) +
      pow(hd, c_b83) * 3.f / 8.f / (d__10 * d__10) * tdpd *
          ((d__11 * d__11 * 3.f / 2.f + d__12 * d__12 / 2.f -
            d__13 * d__13 * 8.f) *
               tdpd / 16.f / (d__14 * d__14) +
           1.f) +
      dlambda1;
  ...
  hself_hdec__1.lambda2 =
      (d__1 * d__1 + d__2 * d__2) / 4.f *
          (1.f - d__3 * d__3 * 3.f * (tp + tdp) / 8.f / (d__4 * d__4)) +
      pow(hu, c_b83) * 3.f / 16.f / (d__5 * d__5) * tp *
          ((d__6 * d__6 * 3.f / 2.f + d__7 * d__7 / 2.f - d__8 * d__8 * 8.f) *
               (tp + tdp * 2.f) / 16.f / (d__9 * d__9) +
           1.f) +
      pow(hu, c_b83) * 3.f / 8.f / (d__10 * d__10) * tdp *
          ((d__11 * d__11 * 3.f / 2.f + d__12 * d__12 / 2.f -
            d__13 * d__13 * 8.f) *
               tdp / 16.f / (d__14 * d__14) +
           1.f) +
      dlambda2 + dlambdap2;
  ...
  hself_hdec__1.lambda3 =
      (d__1 * d__1 - d__2 * d__2) / 4.f *
          (1.f - d__3 * d__3 * 3.f * (tp + tdp) / 16.f / (d__4 * d__4) -
           d__5 * d__5 * 3.f * (tpd + tdpd) / 16.f / (d__6 * d__6)) +
      dlambda3;
  ...
  hself_hdec__1.lambda4 =
      -(d__1 * d__1) / 2.f *
          (1.f - d__2 * d__2 * 3.f * (tp + tdp) / 16.f / (d__3 * d__3) -
           d__4 * d__4 * 3.f * (tpd + tdpd) / 16.f / (d__5 * d__5)) +
      dlambda4;
  ...
  deltal12 = (.140625f / (d__1 * d__1) * (d__2 * d__2) +
              .026041666666666668f / (d__3 * d__3) * (d__4 * d__4)) *
             tchar;
  /*< 	DELTA >*/
  d__1 = pi;
  /* Computing 4th power */
  ...
  deltal3p4 = (.046875f / (d__1 * d__1) * (d__2 * d__2) +
               .036458333333333336f / (d__3 * d__3) * (d__4 * d__4) +
               .125f / (d__5 * d__5) * (d__6 * d__6) * (d__7 * d__7)) *
              tchar;
  ...
  d__4 = g1;
  d__5 = g2;
  dlam3 = deltam122 / 2. / (d__1 * d__1) / sinb / cosb *
          (d__2 * d__2 - d__3 * d__3) / (d__4 * d__4 + d__5 * d__5);
  /*<        >*/
  d__1 = v;
  d__2 = g2;
  d__3 = g1;
  ...
  hself_hdec__1.lambda1 += dlam1;
  /*<         LAMBDA2 = LAMBDA2+DLAM2 >*/
  hself_hdec__1.lambda2 += dlam2;
  /*<         LAMBDA3 = LAMBDA3+DLAM3 >*/
  hself_hdec__1.lambda3 += dlam3;
  /*<         LAMBDA4 = LAMBDA4+DLAM4 >*/
  hself_hdec__1.lambda4 += dlam4;
/* --END OF EXTENSION */
/*<  3790	CONTINUE >*/
L3790:
  ...
  hself_hdec__1.lambda5 += dlam5;
  /*<       LAMBDA6 = LAMBDA6+DLAM6 >*/
  hself_hdec__1.lambda6 += dlam6;
  /*<       LAMBDA7 = LAMBDA7+DLAM7 >*/
  hself_hdec__1.lambda7 += dlam7;
  /*<        >*/
  d__1 = v;
  d__2 = cosb;
  d__3 = sinb;
  ...
  d__5 = sinb;
  m2[0] = d__1 * d__1 * 2.f *
              (hself_hdec__1.lambda1 * (d__2 * d__2) +
               hself_hdec__1.lambda6 * 2.f * cosb * sinb +
               hself_hdec__1.lambda5 * (d__3 * d__3)) +
          d__4 * d__4 * (d__5 * d__5);
  /*<        >*/
  d__1 = v;
  d__2 = cosb;
  ...
  d__5 = cosb;
  m2[3] = d__1 * d__1 * 2.f *
              (hself_hdec__1.lambda5 * (d__2 * d__2) +
               hself_hdec__1.lambda7 * 2.f * cosb * sinb +
               hself_hdec__1.lambda2 * (d__3 * d__3)) +
          d__4 * d__4 * (d__5 * d__5);
  /*<        >*/
  d__1 = v;
  d__2 = cosb;
  ...
  d__4 = *ma;
  m2[2] = d__1 * d__1 * 2.f *
              (hself_hdec__1.lambda6 * (d__2 * d__2) +
               (hself_hdec__1.lambda3 + hself_hdec__1.lambda4) * cosb * sinb +
               hself_hdec__1.lambda7 * (d__3 * d__3)) -
          d__4 * d__4 * sinb * cosb;
  /*<       M2(2,1) = M2(1,2) >*/
  m2[1] = m2[2];
  /*<       M2P(1,1) = M2(1,1) >*/
  ...
  d__2 = v;
  mch2 = d__1 * d__1 +
         (hself_hdec__1.lambda5 - hself_hdec__1.lambda4) * (d__2 * d__2);
  /* !!!!!!!!!!!!!!!!!!! */
  /*<       MCH=DSQRT(MCH2) >*/
  *mch = sqrt(mch2);
  /*<       HMP = DSQRT(HM2P)  >*/
...
/*<        >*/
/* Subroutine */ int gfun_hdec__(double *ma, double *tanb, double *mul,
                                 double *mdl, double *mur, double *md,
                                 double *mtop, double *at, double *ab,
                                 double *mu, double *mglu, double *dlam1,
                                 double *dlam2, double *dlam3, double *dlam4,
                                 double *dlam5, double *dlam6, double *dlam7,
                                 double *deltamb) {
  /* System generated locals */
  double d__1, d__2, d__3, d__4, d__5, d__6, d__7, d__8, d__9, d__10, d__11,
      d__12, d__13, d__14, d__15, d__16;

  /* Builtin functions */
  double sqrt(double), atan(double), log(double), pow_dd(double *, double *);

  /* Local variables */
  static double v, g1, g2, g3, hb, g32, al[4] /* was [2][2] */, mb, tb;
  extern /* Subroutine */ int delmb_hdec__(
      double *, double *, double *, double *, double *, double *, double *,
      double *, double *, double *, double *, double *, double *, double *,
      double *, double *, double *);
  static double td, pi, cw, ht, vi, mw, tq, mz, tt, tu, sw, bb2, f1b, f2b, h1b,
      h2b, al1, al2, h1i, md2, h2i, bt2, f1t, f2t, h1t, h2t,
      vh1[4] /* was [2][2] */, vh2[4] /* was [2][2] */, msb, tqd, mst, stw,
      mdl2, mul2, mur2, facb, fact, cosb, sinb, alst, htst, mbot2, mbot4, sbot1,
      sbot2, bt2st, mtop2, mtop4, stop1, stop2, tanba, cosba, cosbb, sinba,
      sinbb, cosbt, sbot12, sbot22, sinbt;
  static double stop12, stop22, dlam1b, rmtop, dlam2b, dlam3b, dlam4b;
  static double alpha1, alpha2, alpha3, dlam5b, dlam6b, dlam7b, dlam1t, dlam2t,
      dlam3t, dlam4t, dlam5t, dlam6t, dlam7t, tanbsb, factor, tanbst, msusyb,
      alpha3z, msusyt, deltamt;
  ...
  d__2 = hb;
  d__3 = pi * 4.f;
  bt2 = -(g32 * 8.f - d__1 * d__1 * 9.f / 2.f - d__2 * d__2 / 2.f) /
        (d__3 * d__3);
  /*< 	BB2 = -(8.*G32 - 9.*HB**2/2. - HT**2/2.)/(4.*PI)**2 >*/
  d__1 = hb;
  d__2 = ht;
  d__3 = pi * 4.f;
  bb2 = -(g32 * 8.f - d__1 * d__1 * 9.f / 2.f - d__2 * d__2 / 2.f) /
        (d__3 * d__3);
  /*<         AL2 = 3./8./PI**2*HT**2 >*/
  d__1 = pi;
  d__2 = ht;
  ...
  if (*ma > *mtop) {
    ...
    d__3 = *mtop;
    d__4 = *ma;
    vi = v * (.09375f / (d__1 * d__1) * (d__2 * d__2) *
                  log(d__3 * d__3 / (d__4 * d__4)) +
              1.f);
    /*<         H1I = VI*COSBA >*/
    h1i = vi * cosba;
    ...
    d__4 = *ma;
    d__5 = msusyt;
    d__1 = .375f / (d__2 * d__2) * (d__3 * d__3) *
               log(d__4 * d__4 / (d__5 * d__5)) +
           1.f;
  ...
  cosbb = sinbb / tanbsb;
  /*<        >*/
  delmb_hdec__(ma, tanb, mul, mdl, mur, md, at, ab, mu, mglu, mtop, &deltamt,
               deltamb, &stop12, &stop22, &sbot12, &sbot22);
  ...
  mtop4 = pow(rmtop, c_b83) * (bt2 * 2.f * tt + 1.f - al2 * tt - deltamt * 4.f);
  /*     * /(1.+deltamt)**4. */
  /*<        >*/
  d__1 = *deltamb + 1.f;
  mbot4 = pow(mb, c_b83) * (bb2 * 2.f * tb + 1.f - al1 * tb) / pow(d__1, c_b83);
  /*<         MTOP2 = DSQRT(MTOP4) >*/
  mtop2 = sqrt(mtop4);
  /*<         MBOT2 = DSQRT(MBOT4) >*/
  mbot2 = sqrt(mbot4);
  ...
  /*< 	F1T=( >*/
  f1t =
      (mul2 - mur2) / (stop12 - stop22) * (.5f - stw * 1.3333333333333333f) *
          log(stop1 / stop2) +
      (.5f - stw * .66666666666666663f) * log(stop1 * stop2 / (mul2 + mtop2)) +
      stw * .66666666666666663f * log(stop1 * stop2 / (mur2 + mtop2));
  /*< 	F1B=( >*/
  f1b =
      (mdl2 - md2) / (sbot12 - sbot22) * (stw * .66666666666666663f - .5f) *
          log(sbot1 / sbot2) +
      (stw * .33333333333333331f - .5f) * log(sbot1 * sbot2 / (mdl2 + mbot2)) -
      stw * .33333333333333331f * log(sbot1 * sbot2 / (md2 + mbot2));
  /*< 	F2T=1 >*/
  f2t =
      1 / (stop12 - stop22) *
      (log(stop12 / stop22) * -.5f +
       (stw * 1.3333333333333333f - .5f) * (mul2 - mur2) / (stop12 - stop22) *
           (2. - (stop12 + stop22) / (stop12 - stop22) * log(stop12 / stop22)));
  /*< 	F2B=1 >*/
  f2b =
      1 / (sbot12 - sbot22) *
      (log(sbot12 / sbot22) * .5f +
       (stw * -.66666666666666663f + .5f) * (mdl2 - md2) / (sbot12 - sbot22) *
  ...
  d__9 = cosbt;
  dlam1t =
      mtop4 / (d__1 * d__1) * (d__2 * d__2) *
          (2. - (stop12 + stop22) / (stop12 - stop22) * log(stop12 / stop22)) -
      d__6 * d__6 * mtop2 * (d__7 * d__7) / (d__8 * d__8) * f2t / (d__9 * d__9);
  /*<        >*/
  /* Computing 4th power */
  ...
  dlam1b =
      mbot4 / (d__1 * d__1) *
          (log(d__2 * d__2 * (d__3 * d__3) / (mdl2 + mbot2) / (md2 + mbot2)) +
           d__4 * d__4 * 2 / (d__5 * d__5 - d__6 * d__6) *
               log(d__7 * d__7 / (d__8 * d__8))) +
      mbot4 / (d__9 * d__9) * (d__10 * d__10) *
          (2. - (sbot12 + sbot22) / (sbot12 - sbot22) * log(sbot12 / sbot22)) +
      d__14 * d__14 * (mbot2 * 2 * f1b - mbot2 * (d__15 * d__15) * f2b) /
          (d__16 * d__16);
  /*<        >*/
  /* Computing 4th power */
  d__1 = sinbt, d__1 *= d__1;
  ...
  dlam2t =
      mtop4 / (d__1 * d__1) *
          (log(d__2 * d__2 * (d__3 * d__3) / (mul2 + mtop2) / (mur2 + mtop2)) +
           d__4 * d__4 * 2 / (d__5 * d__5 - d__6 * d__6) *
               log(d__7 * d__7 / (d__8 * d__8))) +
      mtop4 / (d__9 * d__9) * (d__10 * d__10) *
          (2. - (stop12 + stop22) / (stop12 - stop22) * log(stop12 / stop22)) +
      d__14 * d__14 * (mtop2 * -2 * f1t + mtop2 * (d__15 * d__15) * f2t) /
          (d__16 * d__16);
  /*<        >*/
  /* Computing 4th power */
  d__1 = cosbb, d__1 *= d__1;
  ...
  d__9 = sinbb;
  dlam2b =
      mbot4 / (d__1 * d__1) * (d__2 * d__2) / (d__3 * d__3) *
          (2. - (sbot12 + sbot22) / (sbot12 - sbot22) * log(sbot12 / sbot22)) +
      d__6 * d__6 * mbot2 * (d__7 * d__7) * (d__8 * d__8) * f2b / (d__9 * d__9);
  /*<        >*/
  /* Computing 4th power */
  d__1 = sinbt, d__1 *= d__1;
  d__2 = *mu;
  ...
  d__11 = *at;
  d__12 = *mu;
  dlam3t = mtop4 / (d__1 * d__1) * (d__2 * d__2) / (d__3 * d__3 - d__4 * d__4) *
               (log(d__5 * d__5 / (d__6 * d__6)) / 2. +
                d__7 * d__7 / (d__8 * d__8 - d__9 * d__9) *
                    (2. - (stop12 + stop22) / (stop12 - stop22) *
                              log(stop12 / stop22))) +
           d__10 * d__10 *
               (mtop2 / tanbst * f1t -
                mtop2 * (d__11 * d__11 - d__12 * d__12) / tanbst / 2.f * f2t) /
               sinbt / cosbt / 2;
  ...
  d__11 = *ab;
  d__12 = *mu;
  dlam3b = mbot4 / (d__1 * d__1) * (d__2 * d__2) / (d__3 * d__3 - d__4 * d__4) *
               (log(d__5 * d__5 / (d__6 * d__6)) / 2. +
                d__7 * d__7 / (d__8 * d__8 - d__9 * d__9) *
                    (2. - (sbot12 + sbot22) / (sbot12 - sbot22) *
                              log(sbot12 / sbot22))) +
           d__10 * d__10 *
               (-mbot2 * tanbsb * f1b +
                mbot2 * (d__11 * d__11 - d__12 * d__12) * tanbsb / 2.f * f2b) /
               sinbb / cosbb / 2;
  /*<        >*/
  /* Computing 4th power */
  d__1 = sinbt, d__1 *= d__1;
  ...
  d__11 = *at;
  d__12 = *mu;
  dlam4t = mtop4 / (d__1 * d__1) * (d__2 * d__2) / (d__3 * d__3 - d__4 * d__4) *
               (log(d__5 * d__5 / (d__6 * d__6)) / 2. +
                d__7 * d__7 / (d__8 * d__8 - d__9 * d__9) *
                    (2. - (stop12 + stop22) / (stop12 - stop22) *
                              log(stop12 / stop22))) +
  ...
  d__11 = *ab;
  d__12 = *mu;
  dlam4b = mbot4 / (d__1 * d__1) * (d__2 * d__2) / (d__3 * d__3 - d__4 * d__4) *
               (log(d__5 * d__5 / (d__6 * d__6)) / 2. +
                d__7 * d__7 / (d__8 * d__8 - d__9 * d__9) *
                    (2. - (sbot12 + sbot22) / (sbot12 - sbot22) *
                              log(sbot12 / sbot22))) +
  ...
  dlam5t = mtop4 / (d__1 * d__1) * (d__2 * d__2 * (d__3 * d__3)) /
           (d__4 * d__4) *
           (2. - (stop12 + stop22) / (stop12 - stop22) * log(stop12 / stop22));
  /*<        >*/
  /* Computing 4th power */
  ...
  d__6 = sbot2;
  d__4 = d__5 * d__5 - d__6 * d__6;
  dlam5b = mbot4 / (d__1 * d__1) * (d__2 * d__2 * (d__3 * d__3)) /
           (d__4 * d__4) *
           (2. - (sbot12 + sbot22) / (sbot12 - sbot22) * log(sbot12 / sbot22));
  /*<        >*/
  /* Computing 4th power */
  ...
  d__6 = mz;
  dlam6t =
      mtop4 / (d__1 * d__1) * (-(d__2 * (d__2 * d__2)) * *at) / (d__3 * d__3) *
          (2. - (stop12 + stop22) / (stop12 - stop22) * log(stop12 / stop22)) +
      d__6 * d__6 * mtop2 * *mu * *at / tanbst * f2t / (sinbt * 2 * cosbt);
  /*<        >*/
  /* Computing 4th power */
  d__1 = cosbb, d__1 *= d__1;
  d__2 = sbot1;
  ...
  d__10 = mz;
  dlam6b =
      mbot4 / (d__1 * d__1) * *mu * *ab *
          (-1. / (d__2 * d__2 - d__3 * d__3) *
               log(d__4 * d__4 / (d__5 * d__5)) -
           d__6 * d__6 / (d__7 * d__7) *
               (2. -
                (sbot12 + sbot22) / (sbot12 - sbot22) * log(sbot12 / sbot22))) -
      d__10 * d__10 * (-mbot2 * *ab * *mu * tanbsb * f2b) / (sinbb * 2 * cosbb);
  /*<        >*/
  /* Computing 4th power */
  d__1 = sinbt, d__1 *= d__1;
  d__2 = stop1;
  ...
  d__10 = mz;
  dlam7t =
      mtop4 / (d__1 * d__1) * *mu * *at *
          (-1. / (d__2 * d__2 - d__3 * d__3) *
               log(d__4 * d__4 / (d__5 * d__5)) -
           d__6 * d__6 / (d__7 * d__7) *
               (2. -
                (stop12 + stop22) / (stop12 - stop22) * log(stop12 / stop22))) -
      d__10 * d__10 * mtop2 * *at * *mu / tanbst * f2t / (sinbt * 2 * cosbt);
  /*<        >*/
  /* Computing 4th power */
  d__1 = cosbb, d__1 *= d__1;
  /* Computing 3rd power */
  ...
  d__6 = mz;
  dlam7b =
      mbot4 / (d__1 * d__1) * (-(d__2 * (d__2 * d__2)) * *ab) / (d__3 * d__3) *
          (2. - (sbot12 + sbot22) / (sbot12 - sbot22) * log(sbot12 / sbot22)) -
      d__6 * d__6 * mbot2 * *mu * *ab * tanbsb * f2b / (sinbb * 2 * cosbb);
  ...
  *dlam1 = fact * dlam1t * (1.f - al1 * tt) + facb * dlam1b * (1.f - al1 * tb);
  /*<         DLAM2 = FACT*DLAM2T*(1.-AL2*TT) + FACB*DLAM2B*(1.-AL2*TB) >*/
  *dlam2 = fact * dlam2t * (1.f - al2 * tt) + facb * dlam2b * (1.f - al2 * tb);
  /*<        >*/
  *dlam3 = fact * dlam3t * (1.f - (al1 + al2) / 2 * tt) +
           facb * dlam3b * (1.f - (al1 + al2) / 2 * tb);
  /*<        >*/
  *dlam4 = fact * dlam4t * (1.f - (al1 + al2) / 2 * tt) +
           facb * dlam4b * (1.f - (al1 + al2) / 2 * tb);
  /*<        >*/
  *dlam5 = fact * dlam5t * (1.f - (al1 + al2) / 2 * tt) +
           facb * dlam5b * (1.f - (al1 + al2) / 2 * tb);
  /*<        >*/
  *dlam6 = fact * dlam6t * (1.f - (al1 * 3 + al2) / 4 * tt) +
           facb * dlam6b * (1.f - (al1 * 3 + al2) / 4 * tb);
  /*<        >*/
  *dlam7 = fact * dlam7t * (1.f - (al1 + al2 * 3) / 4 * tt) +
           facb * dlam7b * (1.f - (al1 + al2 * 3) / 4 * tb);
  /*<         FACTOR = 1.D0 >*/
  factor = 1.;
  /*<         DLAM1 = DLAM1 * FACTOR >*/
  *dlam1 *= factor;
...
/*<        >*/
/* Subroutine */ int
delmb_hdec__(double *ma, double *tanb, double *mul, double *mdl, double *mur,
             double *md, double *at, double *ab, double *mu, double *mglu,
             double *mtop, double *deltamt, double *deltamb, double *stop12,
             double *stop22, double *sbot12, double *sbot22) {
  /* System generated locals */
  double d__1, d__2, d__3, d__4, d__5, d__6, d__7, d__8, d__9, d__10, d__11;
  ...
  /* Local variables */
  static double v, g1, g2, g3, hb, g32, mb, tb, pi, cw, ht, vi, mw, mz, tt, sw,
      bb2, h1b, h2b, al1, al2, h1i, md2, h2i, bt2, h1t, h2t, msb, mst, mdl2,
      mul2, mur2, cosb, sinb, alst, htst, mbot2, mbot4, sbot1, sbot2, bt2st,
      mtop2, mtop4, stop1, stop2, tanba, cosba, cosbb, sinba, sinbb, cosbt,
      sinbt;
  static double rmtop;
  ...
  *deltamb = 0.;
  /*<         mtop4 = rmtop**4.*(1.+2.*bt2*tt- al2*tt - 4.*deltamt) >*/
  mtop4 =
      pow(rmtop, c_b83) * (bt2 * 2.f * tt + 1.f - al2 * tt - *deltamt * 4.f);
  /*     * /(1.+deltamt)**4. */
  /*<        >*/
  d__1 = *deltamb + 1.f;
  mbot4 = pow(mb, c_b83) * (bb2 * 2.f * tb + 1.f - al1 * tb) / pow(d__1, c_b83);
  ...
  d__9 = h1t;
  d__10 = h2t;
  d__6 = (d__7 * d__7 - d__8 * d__8 * 5.f / 3.f) / 4.f *
             (d__9 * d__9 - d__10 * d__10) +
         mul2 - mur2;
  d__11 = *at - *mu / tanbst;
  d__5 = d__6 * d__6 * .25f + mtop2 * (d__11 * d__11);
  *stop12 = (mul2 + mur2) * .5f + mtop2 +
            (d__1 * d__1 + d__2 * d__2) * .125f * (d__3 * d__3 - d__4 * d__4) +
            pow(d__5, c_b98);
  /*<        >*/
  d__1 = g2;
  d__2 = g1;
  ...
  *stop22 = (mul2 + mur2) * .5f + mtop2 +
  ...
  d__10 = h2b;
  d__6 =
      (d__7 * d__7 / 3.f - d__8 * d__8) / 4.f * (d__9 * d__9 - d__10 * d__10) +
      mdl2 - md2;
  d__11 = *ab - *mu * tanbsb;
  ...
  *deltamb = alpha3 * -2 / 3.f / pi * *mglu * (*ab - *mu * *tanb) *
                 t_hdec__(&sbot1, &sbot2, mglu) +
             d__1 * d__1 / (d__2 * d__2) * (*at - *mu / *tanb) * *mu * *tanb *
                 t_hdec__(&stop1, &stop2, mu);
  /*<        >*/
...
double t_hdec__(double *x, double *y, double *z__) {
  /* System generated locals */
  double ret_val, d__1, d__2, d__3, d__4, d__5, d__6, d__7, d__8, d__9, d__10,
  ...
  d__18 = *z__;
  ret_val =
      (d__1 * d__1 * (d__2 * d__2) * log(d__3 * d__3 / (d__4 * d__4)) +
       d__5 * d__5 * (d__6 * d__6) * log(d__7 * d__7 / (d__8 * d__8)) +
       d__9 * d__9 * (d__10 * d__10) * log(d__11 * d__11 / (d__12 * d__12))) /
      ((d__13 * d__13 - d__14 * d__14) * (d__15 * d__15 - d__16 * d__16) *
       (d__17 * d__17 - d__18 * d__18));
  /*<       return >*/
  return ret_val;
  /*<       end >*/
} /* t_hdec__ */
