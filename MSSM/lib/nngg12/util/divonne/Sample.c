...
static inline void SamplesIni(Samples *samples) { MEM(samples) = NULL; }
...
static count SamplesLookup(Samples *samples, int key, ccount nwant, ccount nmax,
  ...
  if (key == 13 && ndim_ == 2) {
    ...
    if (n < 40)
      n *= nwant;
    samples->sampler = (key < 0) ? SampleSobol : (n = n / 2 + 1, SampleKorobov);
...
#include "KorobovCoeff.c"
  ...
  if (samples->sampler == SampleKorobov) {
    ...
    while (i = IMin(IDim(i), max), n > (p = prime[i + 1]) || n <= prime[i]) {
...
static real Sample(creal *x0) {
  ...
  for (dim = 0; dim < ndim_; ++dim) {
    ...
    real dx;
    if ((dx = x1 - border_.lower) < 0 || (dx = x1 - border_.upper) > 0)
      dist += Sq(dx);
  }
  ...
  if (nextra > 1)
    f += dist * (f - ftmp[selectedcomp_ + ncomp_]);
