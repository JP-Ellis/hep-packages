...
#define HAVESAMPLES (flags & 2)
  ...
  if (SPLICE) {
    ...
    region->next = new;
    VecCopy(new->bounds, region->bounds);
    region = new;
  }
  ...
  if (!HAVESAMPLES) {
    ...
    n = ngiven_;
    if (nextra_)
      n += SampleExtra(region->bounds);

    for (; n; --n) {
      for (dim = 0; dim < ndim_; ++dim) {
        Bounds *b = &region->bounds[dim];
        if (x[dim] < b->lower || x[dim] > b->upper)
          goto skip;
      }
      for (comp = 0; comp < ncomp_; ++comp) {
        Extrema *e = &extrema[comp];
  ...
  if (!HAVESAMPLES) {
    if (samples->weight * r->spread < r->err ||
        r->spread < totals_[maxcomp].secondspread)
      region->depth = 0;
    if (region->depth == 0) {
      for (comp = 0; comp < ncomp_; ++comp) {
