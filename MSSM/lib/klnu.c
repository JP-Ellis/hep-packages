...
/* New physics contribution to  Ratio R_l23 in MSSM with main tanbeta NLO
contributions This is the ratio of 2body/3body Kaon decay as defined in M.
Antonelli et al (FlaviaNet Working group) arXiv:0801.1817
*/
...
double dtaunu_(double *dmunu) {
  ...
  xHp = mDs * mDs / mHp / mHp;
  lnu =
      GF * GF / 8. / M_PI * Vcs * Vcs * fDs * fDs * TauDs * mDs *
      (1 +
       xHp / (1 + msc) * (1 - msc * param.tb * param.tb / (1 + deltaMc()))) *
      (1 + xHp / (1 + msc) * (1 - msc * param.tb * param.tb / (1 + deltaMc())));

  lmu = lnu * Mm * Mm * (1 - Mm * Mm / mDs / mDs) * (1 - Mm * Mm / mDs / mDs);
  lnu *= (1 - findValW("Ml") * findValW("Ml") / mDs / mDs) *
