...
static double findValW(char *name) {
  int i;
  for (i = 0; i < powInfo; i++)
    if (strcmp(nameInfo[i].name, name) == 0)
      return nameInfo[i].val;
    ...
    if (strcmp(buff, "BLOCK") == 0) {
      ...
      if (fscanf(f, "%[^\n]", rest)) {
        ...
        if (c)
          c[0] = 0;
        c = strchr(rest, '=');
        if (c && 1 == sscanf(c + 1, "%lf", &val))
          assignValW("QSUSY", val);
      }
      fscanf(f, "%*c");
      ...
      if (strcmp(buff, "MODSEL") == 0) {
        for (; nfscanf(f, &n1, NULL, &val) == 2;)
          if (n1 == 1)
            assignValW("model", val);
      } else if (strcmp(buff, "SMINPUTS") == 0) {
        for (; nfscanf(f, &n1, NULL, &val) == 2;) {
          switch (n1) {
          case 1:
            assignValW("alfEMZ", 1 / val);
            break;
          case 2:
            assignValW("GF", val);
            break;
          case 3:
            assignValW("alfSMZ", val);
            break;
          case 4:
            assignValW("MZ", val);
            break;
        ...
        while (2 == nfscanf(f, &n1, NULL, &val))
          ...
          case 2:
            switch (n1) {
            case 1:
              assignValW("Lambda", val);
              break;
            case 2:
              assignValW("Mmess", val);
              break;
            case 3:
              assignValW("tb", val);
              break;
        ...
        while (2 == nfscanf(f, &n1, NULL, &val))
          ...
          case 21:
            if (val > 0)
              assignValW("gMHd", sqrt(val));
            else
              assignValW("gMHd", -sqrt(-val));
            break;
          case 22:
            if (val > 0)
              assignValW("gMHu", sqrt(val));
            else
              assignValW("gMHu", -sqrt(-val));
            break;
          case 23:
