...
static void addSU_ALGO(char *fname, int *ichoice) {
  ...
  fprintf(
      f,
      "   2    %d  # 2-loop RGE \n"
      "   3    %d   # 1: g_1(gut) = g_2(gut) consistently calculated from "
      "input\n"
      ...
      "   9    %d   # Final spectrum accuracy: 1 -> 1%% acc.; 2 -> 0.01 %% "
      "acc.\n"
      "   10   %d   # Higgs boson masses: One-loop  + dominant DSVZ 2-loop\n"
      "   11   %d   # RUNNING DRbar Higgs masses at loop-level at mZ\n",
      ichoice[2], ichoice[3], ichoice[4], ichoice[6], ichoice[7], ichoice[8],
      ichoice[9], ichoice[10], ichoice[11]);
...
double suspectSUGRAc(double tb, double gMG1, double gMG2, double gMG3,
  ...
  if (sugraLesH(FIN, tb, gMG1, gMG2, gMG3, gAl, gAt, gAb, sgn, gMHu, gMHd, gMl2,
...
double suspectSUGRAnuhc(double tb, double gMG1, double gMG2, double gMG3,
  ...
  if (sugraHiggsLesH(FIN, tb, gMG1, gMG2, gMG3, gAl, gAt, gAb, gMl2, gMl3, gMr2,
...
double suspectEwsbMSSMc(double tb, double MG1, double MG2, double MG3,
