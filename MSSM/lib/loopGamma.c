...
int loopGamma(double *csAZ, double *csAA) {
  ...
  *csAA = 0, *csAZ = 0;
  ...
  if (access(buff, X_OK)) {
    char buf[2000];
    sprintf(buf,
            "dir=%s/../lib/nngg12;  which  gmake; if(test $? -eq 0) then  "
            "gmake -C $dir; else make "
            "-C $dir; fi",
            WORK);
    system(buf);
  }
  ...
  if (err)
    return err;
  f = fopen(FIN, "a");
  if (slhaDecayExists(36) < 0)
    slhaDecayPrint("H3", 0, f);
  if (slhaDecayExists(25) < 0)
    slhaDecayPrint("h", 0, f);
  if (slhaDecayExists(35) < 0)
    slhaDecayPrint("H", 0, f);
  fclose(f);
  if (!access(FOUT, R_OK))
    unlink(FOUT);

  sprintf(buff + strlen(buff), " %s %s  %E %E %E %E ", FIN, FOUT,
          findValW("MW"), findValW("SW"), findValW("EE"), findValW("Mb"));
  err = System(buff);
