...
int MSSMDDtest(int loop, double *pA0, double *pA5, double *nA0, double *nA5) {
  double ApB[2][7], AmB[2][7], MI[2][2][7], NL[5], T3Q[2], EQ[2], mq[7],
      mqSM[7], msq[2][7], Aq[7];
  ...
  *pA0 = 0, *pA5 = 0, *nA0 = 0, *nA5 = 0;
  ...
  for (i = 1; i <= 4; i++) {
    sprintf(buffName, "Zn1%d", i);
    NL[i] = findValW(buffName);
  }
  ...
  for (IQ = 1; IQ <= 3; IQ++) {
    double f =
        sqrt(3.) * 0.5 * (SQ(NL[4]) - SQ(NL[3])) * SQ(G / 2 / MW) * T3Q[IQ & 1];
    *pA5 += f * wV5P__[IQ - 1];
    *nA5 += f * wV5N__[IQ - 1];
  }
  *pA5 /= sqrt(3);
  *nA5 /= sqrt(3);
  ...
  /****** light quarks-squarks SI contribution (plus heavy squarks at tree
   * level)*/

  for (IQ = 1; IQ <= (loop ? 3 : 6); IQ++)
    for (II = 0; II < 2; II++) {
      double g, f, D, D2;
      qcdNLO = qcdNLOs = 1;
      if (QCDcorrections) {
  ...
  if (loop)
    for (IQ = 4; IQ <= 6; IQ++)
      for (II = 0; II < 2; II++) {
        ...
        bd = AmB[II][IQ] * mqSM[IQ] * LintIk(1, msq[II][IQ], mqSM[IQ], mne) *
             3 / 8.;
        ...
        *nA0 += f * MN * wS0N__[IQ - 1] * qcdNLO;
        ...
        if (Twist2On) {
          double Ampl2;
          Ampl2 = parton_alpha(mqSM[IQ]) / (12 * M_PI) *
                  (b2s + mne * (b1s + b1d) / 2) * parton_x(21, MqPole[IQ]);
  ...
  for (IQ = 1; IQ <= 6; IQ++) {
    ...
    if (IQ & 1) {
      ...
      fh = o1o1h * E * sa * mq[IQ] * (1 - dMq * ca * cb / sa / sb) /
           (2 * MW * cb * SW);
      fH = -o1o1H * E * ca * mq[IQ] * (1 + dMq * sa * cb / ca / sb) /
           (2 * MW * cb * SW);
    } else {
      fh = -o1o1h * E * ca * mq[IQ] / (2 * MW * sb * SW);
      fH = -o1o1H * E * sa * mq[IQ] / (2 * MW * sb * SW);
    }
    *pA0 += 0.5 * (fh / (mh * mh) + fH / (mH * mH)) / mqSM[IQ] * MN *
            wS0P__[IQ - 1] * qcdNLO;
    *nA0 += 0.5 * (fh / (mh * mh) + fH / (mH * mH)) / mqSM[IQ] * MN *
            wS0N__[IQ - 1] * qcdNLO;
  }
...
#define D 1
  ...
  for (IQ = 1; IQ <= 6; IQ++)
    for (II = 0; II < 2; II++) {
      ...
      for (fh = 0, fH = 0, i = 0; i < 2; i++)
        for (j = 0; j < 2; j++) {
          double dSQMh, dSQMH;
          double b = -T3Q[IQ & 1] + 0.5, t = T3Q[IQ & 1] + 0.5;
          if (i == j) {
            dSQMh = (ca * t - sa * b) * 3 * SQ(mq[IQ] * CW / MW) / SW;
            dSQMH = (sa * t + ca * b) * 3 * SQ(mq[IQ] * CW / MW) / SW;
          } else {
            dSQMh =
                ((ca * Aq[IQ] + mu * sa) * t - (sa * Aq[IQ] + mu * ca) * b) *
                1.5 * mq[IQ] * SQ(CW / MW) / SW;
            dSQMH =
                ((sa * Aq[IQ] - mu * ca) * t + (ca * Aq[IQ] - mu * sa) * b) *
                1.5 * mq[IQ] * SQ(CW / MW) / SW;
          }
      ...
      fH *= o1o1H * E * MW / (3 * CW * CW);
      *pA0 += (fh / (mh * mh) + fH / (mH * mH)) /
              (2 * msq[II][IQ] * msq[II][IQ]) * MN * wS0P__[5] / 8 * qcdNLOs;
      *nA0 += (fh / (mh * mh) + fH / (mH * mH)) /
              (2 * msq[II][IQ] * msq[II][IQ]) * MN * wS0N__[5] / 8 * qcdNLOs;
    }
}
