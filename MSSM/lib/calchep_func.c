...
/* =====  end of header part ========= */
/*
double saveSM(double MbMb,double Mtp,double SW,double alfSMZ,double
alfEMZ,double MZ,double Ml)
{
  double Qmin;
...
double findGIparam(double ok) {
  ...
  for (i = 0; i < 4; i++)
    for (j = 0; j < 4; j++)
      Z[j][i] = slhaVal("NMIX", QSUSY, 2, i + 1, j + 1);
  ...
  mZero = 0;
  if (mZero < fabs(MM[0][1]))
    mZero = fabs(MM[0][1]);
  if (mZero < fabs(MM[2][2]))
    mZero = fabs(MM[2][2]);
  if (mZero < fabs(MM[3][3]))
    mZero = fabs(MM[3][3]);
