...
/* choose 'suspect','softSusy','spheno'*/
...
int main(int argc, char **argv) {
  ...
  */
  ...
  {
    double m0, mhf, a0, tb;
    double gMG1, gMG2, gMG3, gAl, gAt, gAb, sgn, gMHu, gMHd, gMl2, gMl3, gMr2,
        gMr3, gMq2, gMq3, gMu2, gMu3, gMd2, gMd3;
    ...
    if (argc < 5) {
      ...
      if (argc > 5)
        ...
        sgn = 1;
      if (argc > 6) {
        sscanf(argv[6], "%lf", &Mtp);
        assignValW("Mtp", Mtp);
      }
      if (argc > 7) {
        sscanf(argv[7], "%lf", &MbMb);
        assignValW("MbMb", MbMb);
      }
      if (argc > 8) {
        sscanf(argv[8], "%lf", &alfSMZ);
    ...
    err = SUGRAMODEL(RGE)(tb, gMG1, gMG2, gMG3, gAl, gAt, gAb, sgn, gMHu, gMHd,
                          gMl2, gMl3, gMr2, gMr3, gMq2, gMq3, gMu2, gMu3, gMd2,
                          gMd3);
  }
#elif defined(SUGRANUH)
  {
    double m0, mhf, a0, tb;
    double gMG1, gMG2, gMG3, gAl, gAt, gAb, gMl2, gMl3, gMr2, gMr3, gMq2, gMq3,
        gMu2, gMu3, gMd2, gMd3, mu, MA;
    ...
    if (argc < 7) {
      ...
      sscanf(argv[6], "%lf", &MA);
      if (argc > 7) {
        sscanf(argv[7], "%lf", &Mtp);
        assignValW("Mtp", Mtp);
      }
      if (argc > 8) {
        sscanf(argv[8], "%lf", &MbMb);
        assignValW("MbMb", MbMb);
      }
      if (argc > 9) {
        sscanf(argv[9], "%lf", &alfSMZ);
        assignValW("alfSMZ", alfSMZ);
      }
    }
    ...
    err = SUGRANUHMODEL(RGE)(tb, gMG1, gMG2, gMG3, gAl, gAt, gAb, gMl2, gMl3,
                             gMr2, gMr3, gMq2, gMq3, gMu2, gMu3, gMd2, gMd3, mu,
                             MA);
  }
#elif defined(AMSB)
  {
    ...
    if (argc < 4) {
      ...
      sscanf(argv[3], "%lf", &tb);
      if (argc > 4)
        sscanf(argv[4], "%lf", &sgn);
      else
        sgn = 1;
      if (argc > 5) {
        sscanf(argv[5], "%lf", &Mtp);
        assignValW("Mtp", Mtp);
      }
      if (argc > 6) {
        sscanf(argv[6], "%lf", &MbMb);
        assignValW("MbMb", MbMb);
      }
      if (argc > 7) {
        sscanf(argv[7], "%lf", &alfSMZ);
        assignValW("alfSMZ", alfSMZ);
      }
    }
...
#elif defined(EWSB)
  {
    ...
    if (argc < 2) {
      printf("The program needs one argument:the name of file with MSSM "
             "parameters.\n"
  ...
  /*
    ...
    Mbp(), 4.23*(1+4/3*alphaQCD(4.23)/M_PI), alphaQCD(4.23)); exit(0);
  */
  ...
  if (strcmp(cdmName, "~o1"))
    printf(" ~o1 is not CDM\n");
  else
    o1Contents(stdout);
...
#ifdef CONSTRAINTS
  {
    ...
    printf("deltartho=%.2E\n", deltarho());
    printf("gmuon=%.2E\n", gmuon());
    printf("bsgnlo=%.2E ", bsgnlo(&SMbsg));
    printf("( SM %.2E )\n", SMbsg);

    printf("bsmumu=%.2E\n", bsmumu());
    printf("btaunu=%.2E\n", btaunu());

    printf("dtaunu=%.2E  ", dtaunu(&dmunu));
    printf("dmunu=%.2E\n", dmunu);
    printf("Rl23=%.3E\n", Rl23());
    if (Zinvisible())
      printf("Excluded by Z->invisible\n");
    if (LspNlsp_LEP(&csLim))
      printf(
          "Excluded by LEP  by e+,e- -> DM q qbar. Cross section =%.2E [pb] \n",
          csLim);

    if (masslimits() == 0)
      printf("MassLimits OK\n");
...
#ifdef SUPERISO
  {
    ...
    if (err == 0) {
      ...
      printf("  BR(b->s gamma)                     %.3E  (%.3E)\n",
             slhaValFormat("FOBS", 0.,
                           "    5    1  %lf    0     2     3    22        "),
             slhaValFormat("FOBSSM", 0.,
                           "    5    1  %lf    0     2     3    22        "));
      ...
      printf("  BR(B+->D0 tau nu)                  %.3E  (%.3E)\n",
             slhaValFormat("FOBS", 0.,
                           "  521    1  %lf    0     3   421   -15    16  "),
             slhaValFormat("FOBSSM", 0.,
                           "  521    1  %lf    0     3   421   -15    16  "));
      printf("  BR(B+->D0 tau nu)/BR(B+-> D0 e nu) %.3E  (%.3E)\n",
             slhaValFormat("FOBS", 0.,
                           "  521   11  %lf    0     3   421   -15    16  "),
             slhaValFormat("FOBSSM", 0.,
                           "  521   11  %lf    0     3   421   -15    16  "));
...
#include "../include/hBandS.inc"
    ...
    for (int i = 0; i < 3; i++)
      printf("  id= %d  result = %d  obsratio=%.2E  channel= %s \n", HB_id[i],
             HB_result[i], HB_obsratio[i], HB_chan[i]);
#endif
#ifdef HIGGSSIGNALS
...
#include "../include/Lilith.inc"
      if (ndf) {
        printf("LILITH(DB%s):  -2*log(L): %.2f; -2*log(L_reference): %.2f; "
               "ndf: %d; p-value: %.2E \n",
               Lilith_version, m2logL, m2logL_reference, ndf, pvalue);
      }
    } else
      printf("LILITH: there is no Higgs candidate\n");
  }
#endif
...
#include "../include/SMODELS.inc"
...
#ifdef INDIRECT_DETECTION
  {
    int err, i;
    double Emin = 1,
           SMmev = 320; /*Energy cut in GeV and solar potential in MV*/
    double sigmaV;
    char txt[100];
    ...
    */
    ...
    /* Returns sigma*v in cm^3/sec.     SpX - calculated spectra of
       annihilation. Use SpectdNdE(E, SpX) to calculate energy distribution in
       1/GeV units.
    ...
    */

    {
      double fi = 0.1,
             dfi = M_PI /
                   180.; /* angle of sight and 1/2 of cone angle in [rad] */
      ...
      sprintf(
          txt,
          "Photon flux for angle of sight %.2f[rad] and cone angle %.2f[rad]",
          fi, 2 * dfi);
...

    {
      ...
      if (SMmev > 0)
        solarModulation(SMmev, 0.0005, FluxE, FluxE);
#ifdef SHOWPLOTS
      displayPlot("positron flux [cm^2 s sr GeV]^{-1}", "E[GeV]", Emin, Mcdm, 0,
                  1, "", 0, SpectdNdE, FluxE);
#endif
...

    {
      ...
      if (SMmev > 0)
        solarModulation(SMmev, 1, FluxP, FluxP);
#ifdef SHOWPLOTS
      displayPlot("antiproton flux [cm^2 s sr GeV]^{-1}", "E[GeV]", Emin, Mcdm,
                  0, 1, "", 0, SpectdNdE, FluxP);
#endif
      printf(
...
#ifdef LoopGAMMA
  {
    ...
    double fi = 0., dfi = M_PI / 180.; /* fi angle of sight[rad], dfi  1/2 of
                                          cone angle in [rad] */
    ...
    if (loopGamma(&vcs_gz, &vcs_gg) == 0) {
      ...
      printf("E=%.2E[GeV]  vcs(A,A)= %.2E[cm^3/s], flux=%.2E[cm^2 s]^{-1}\n",
             Mcdm, vcs_gg, 2 * gammaFlux(fi, dfi, vcs_gg));
    }
...
#ifdef RESET_FORMFACTORS
  {
    ...
    */
    ...
    //                    To restore default form factors of  current version
    //                    call
...
#ifdef CDM_NUCLEUS
  {
...
#ifdef SHOWPLOTS
    displayPlot("Distribution of recoil energy of 73Ge", "E[KeV]", 0, 200, 0, 1,
                "dN/dE", 0, dNdERecoil, dNdE);
#endif
    ...
    printf("Number of events in 10 - 50 KeV region=%.2E /day/kg\n",
           cutRecoilResult(dNdE, 10, 50));
#ifdef SHOWPLOTS
    displayPlot("Distribution of recoil energy of 131Xe", "E[KeV]", 0, 200, 0,
                1, "dN/dE", 0, dNdERecoil, dNdE);
#endif
    ...
    printf("Number of events in 10 - 50 KeV region=%.2E /day/kg\n",
           cutRecoilResult(dNdE, 10, 50));
#ifdef SHOWPLOTS
    displayPlot("Distribution of recoil energy of 23Na", "E[KeV]", 0, 200, 0, 1,
                "dN/dE", 0, dNdERecoil, dNdE);
#endif
    ...
    printf("Number of events in 10 - 50 KeV region=%.2E /day/kg\n",
           cutRecoilResult(dNdE, 10, 50));
#ifdef SHOWPLOTS
    displayPlot("Distribution of recoil energy of 127I", "E[KeV]", 0, 200, 0, 1,
                "dN/dE", 0, dNdERecoil, dNdE);
#endif
  }
#endif
...
#ifdef SHOWPLOTS
    displayPlot("neutrino fluxes [1/Year/km^2/GeV]", "E[GeV]", Emin, Mcdm, 0, 2,
                "dnu/dE", 0, SpectdNdE, nu, "dnu_bar/dE", 0, SpectdNdE, nu_bar);
#endif

    printf(
        " E>%.1E GeV neutrino/anti-neutrino fluxes   %.2E/%.2E [1/Year/km^2]\n",
        Emin, spectrInfo(Emin, nu, NULL), spectrInfo(Emin, nu_bar, NULL));
    //  ICE CUBE
    if (forSun)
...
#ifdef SHOWPLOTS
    displayPlot("Upward muons[1/Year/km^2/GeV]", "E", Emin, Mcdm / 2, 0, 1,
                "mu", 0, SpectdNdE, mu);
#endif

    printf(" E>%.1E GeV Upward muon flux    %.2E [1/Year/km^2]\n", Emin,
           spectrInfo(Emin, mu, NULL));

    /* Contained events */
    muonContained(nu, nu_bar, 1., mu);
#ifdef SHOWPLOTS
    displayPlot("Contained  muons[1/Year/km^3/GeV]", "E", Emin, Mcdm, 0, 1, "",
                0, SpectdNdE, mu);
#endif
  ...
  system("rm -f HB.* HS.* hb.* hs.*  debug_channels.txt debug_predratio.txt  "
         "Key.dat");
  system("rm -f Lilith_*   particles.py*");
  system("rm -f  smodels.in  smodels.log  smodels.out  summary.*; rm -rf  "
         "__pycache__ ");
#endif
