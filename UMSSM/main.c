...
int main(int argc, char **argv) {
  ...
  if (err) {
    printf("An error occurred running umssmtools.\n");
    return 1;
  }
  slhaWarnings(stdout);
...
#ifdef STABLE_NLSP
  {
    ...
    if (charge3 || cdim != 1) {
      ...
      printf("NLSP is '%s' with spin=%d/2, electric charge %d/3 and the mass "
             "difference with the LSP "
             "is %f GeV\n",
             nextOdd(1, &NLSPMass), spin2, charge3, MNLSP - Mcdm);
      if (pWidth(nextOdd(1, &NLSPMass), &L) == 0) {
        printf("WARNING: the NLSP '%s' is stable\n", nextOdd(1, &NLSPMass));
      }
    }
  }
#endif
...
#ifdef CONSTRAINTS
  {
    ...
    printf("RD  = BR[B+ -> D  tau+ nu_tau]/BR[B+ -> D  l+ nu_l] = %.2E (%.2E , "
           "%.2E) \n",
           constr0, constrM, constrP);
    ...
    if (masslimits() == 0)
      printf("LEP limits OK\n");
    if (Zprimelimits() == 0) {
      printf("LHC limits on new Zprime OK\n");
    }
    if (Zinvisible())
...
#include "../include/SMODELS.inc"
...
#ifdef SHOWPLOTS
      displaySpectrum("antiproton flux [cm^2 s sr GeV]^{-1}", "E[GeV]", Emin,
                      Mcdm, 0, 1, "flux", 0, SpectdNdE, FluxP);
#endif
      printf(
...
#ifdef CDM_NUCLEON
  {
    ...
    printf(" proton  SI %.3E [%.3E] SD %.3E [%.3E]\n",
           SCcoeff * pA0[0] * pA0[0], SCcoeff * pA0[1] * pA0[1],
           3 * SCcoeff * pA5[0] * pA5[0], 3 * SCcoeff * pA5[1] * pA5[1]);
    printf(" neutron SI %.3E [%.3E] SD %.3E [%.3E]\n",
           SCcoeff * nA0[0] * nA0[0], SCcoeff * nA0[1] * nA0[1],
           3 * SCcoeff * nA5[0] * nA5[0], 3 * SCcoeff * nA5[1] * nA5[1]);
  }
#endif
...
#ifdef NEUTRINO
  {
    ...
    displayPlot("neutrino fluxes [1/Year/km^2/GeV]", "E[GeV]", Emin, Mcdm, 0, 2,
                "nu", 0, SpectdNdE, nu, "nu_bar", 0, SpectdNdE, nu_bar);
#endif
...
#ifdef SHOWPLOTS
    displayPlot("Upward muons[1/Year/km^2/GeV]", "E[GeV]", Emin, Mcdm / 2, 0, 1,
                "mu", 0, SpectdNdE, mu);
#endif
...
#ifdef SHOWPLOTS
    displayPlot("Contained  muons[1/Year/km^3/GeV]", "E[GeV]", Emin, Mcdm, 0, 1,
                "mu", 0, SpectdNdE, mu);
#endif
  ...
  system("rm -f Key.dat SM_decay.dat HB.* hb.stdout HS.* hs.stdout UMSSM* "
         "debug* Lilith*  smodels.in  "
         "smodels.log  smodels.out  summary.*  particles.py*");
  printf("\n====================================\n");
#endif
