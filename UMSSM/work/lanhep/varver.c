...
static Term dif_term(Term t) {
  ...
  if (CompoundName(t) == A_SQRT && CompoundArity(t) == 1) {
    ...
    if (is_compound(d1) && CompoundName(d1) == OPR_MLT &&
      ...
      FreeAtomic(d1);
      return MakeCompound1(OPR_MINUS, MakeCompound2(OPR_DIV, d2, t));
    }
    return MakeCompound1(
        OPR_MINUS,
        MakeCompound2(OPR_DIV, d1, MakeCompound2(OPR_MLT, NewInteger(2), t)));
  }
  ...
  if (CompoundName(t) == OPR_POW && CompoundArity(t) == 2) {
    ...
    return MakeCompound2(
        OPR_MLT, d1,
        MakeCompound2(OPR_POW, CompoundArg1(t), NewInteger(ppow - 1)));
  }
  ...
  if (CompoundName(t) == OPR_MLT && CompoundArity(t) == 2) {
    ...
    return MakeCompound2(OPR_PLUS, MakeCompound2(OPR_MLT, CompoundArg1(t), d2),
  ...
  if (CompoundName(t) == OPR_DIV && CompoundArity(t) == 2) {
    ...
    if (d1 == 0)
      return MakeCompound1(
          OPR_MINUS,
          MakeCompound2(
              OPR_DIV, MakeCompound2(OPR_MLT, d2, CompoundArg1(t)),
    ...
    return MakeCompound2(
        OPR_DIV,
        MakeCompound2(OPR_MINUS, MakeCompound2(OPR_MLT, d1, CompoundArg2(t)),
...
Term VarVer(Term t, Term ind) {
  ...
  for (l1 = l; !is_empty_list(l1); l1 = ListTail(l1)) {
    ...
    alg2_red_cos(a2);
    alg2_red_orth(a2);
    if (CompoundArg2(a2) == NewInteger(0) || is_empty_list(CompoundArgN(a2, 5)))
      continue;
...
extern int kill_gamma_pm, opTriHeu;
  ...
  for (l = ml; l; l = ListTail(l)) {
    ...
    for (l1 = CompoundArgN(ListFirst(l), 3); l1; l1 = ListTail(l1)) {
      ...
      if (CompoundName(tt) == OPR_SPECIAL && CompoundArg1(tt) == A_GAMMA5)
        tg5++;
      if (CompoundName(tt) == A_MOMENT &&
          CompoundArg1(tt) == NewInteger(mmmpos))
        tm++;
    }
  ...
  for (l = ml; l; l = ListTail(l)) {
    ...
    for (l1 = CompoundArg2(ListFirst(l)); l1; l1 = ListTail(l1))
      if (CompoundArg1(ListFirst(l1)) == a1) {
        SetCompoundArg(ListFirst(l1), 1, a2);
        ii++;
      }
  }
...
static int set_ppl(void) {
  ...
  for (l = vlist; !is_empty_list(l); l = ListTail(l)) {
    ...
    while (!is_empty_list(l1)) {
      ...
      printf(" ");
#endif
      while (!is_empty_list(l1) && remcc(CompoundArg1(ListFirst(l1))) == app)
        l1 = ListTail(l1);
    }
#ifdef DBG_PPL
    puts("");
...
Term ProcMkProc(Term t, Term ind) {
  ...
  if (dec == 0)
    ...
    sprintf(pname, "%s__%s%s", AtomValue(prt[0]), AtomValue(prt[2]),
            AtomValue(prt[3]));
  ...
  if (dec == 0)
    fprintf(f, "process = {prt[\"%s\"],prt[\"%s\"]} ->", AtomValue(prt[0]),
            AtomValue(prt[1]));
  else
    fprintf(f, "process = {prt[\"%s\"]} ->", AtomValue(prt[0]));
  ...
  if (color[0] && color[1] && color[2] && color[3]) {
    int glu = 1, gno = 1;
    if (GetAtomProperty(prt[0], A_ANTI) != prt[1])
      glu = 0;
    if (spin[0] == 0 && spin[1] == 0 && spin[2] == 0 && spin[3] == 0)
      gno = 0;
    if (glu)
  ...
  fprintf(
      f,
      "then echo Output directory is not created | tee -a scan.log && exit;\n");
  fprintf(f, "fi\n\n");
  ...
  */
  ...
  fprintf(f, "if test ! -f run  ;\n");
  fprintf(f,
          "then echo Run file is not created | tee -a ../scan.log && exit;\n");
  fprintf(f, "fi\n");
  ...
  fprintf(f, "cd ..\n");
  //	fprintf(f,"grep \"|    1000.000\" scan_%s/ru*.01000*/* >>
  //scan.log\n\n\n", 				pname);
