...
void o1Contents(FILE *f) {
  fprintf(f,
          ...
          "%+.3f*singlino "
          "%+.3f*binoprime\n",
          findValW("Zn11"), findValW("Zn12"), findValW("Zn13"),
          findValW("Zn14"), findValW("Zn15"), findValW("Zn16"));
}
...
// Random number generator : first don't forget to initialize the pseudo random
// number generator that rand() uses, for instance with srand(time(NULL));
// randpar("x=name of a given variable of the model (or
// not)",x_start,x_max,precision,sign,logarithmic scan,experimental contraint);
// examples : M1p=randpar("M1p",0,20000,2,1,0,0);
// Mtp=randpar("Mtp",173.34,1,3,1,0,1); msd=randpar("",27.5,0.3,3,1);

// caution when scanning on mu and Alda with negative values allowed (flag
// sign=1) : start using randpar for mu and then use it for Alda since they must
// get the same sign.

double randpar(char *xx, double start, double amp, int pwr, int sign,
               int logmod, int expval) {
  double XX;
  double sgn = 0;
  int FloorAmp;
  ...
  if (sign == 0)
    ...
    printf("**** ERROR **** Fifth must be either 0 or 1.\n");
    exit(1);
  }
  if (!strcmp("Alda", xx)) {
    if (findValW("mu") >= 0) {
  ...
  if (logmod == 0) {
    ...
    if (expval == 0) {
      XX = sgn * (start + (rand() % (abs(FloorAmp) + 1)) * pow(10, -pwr));
    } else if (expval == 1) {
      XX = start + sgn * ((rand() % (abs(FloorAmp) + 1)) * pow(10, -pwr));
    } else {
      printf("**** ERROR **** Seventh argument must be either 0 or 1.\n");
      exit(1);
    }
  } else if (logmod == 1) {
    ...
    printf("**** ERROR **** Sixth argument must be either 0 or 1.\n");
    exit(1);
  }
...
static int runTools(char *cmd, char *fout, int mode) {
  ...
  sprintf(command,
          "lPath=%s;EXPCON_PATH=$lPath/UMSSMTools/EXPCON;export "
          "EXPCON_PATH;$lPath/%s",
          LPATH, cmd);
  printf("\n*****************************************\n");
...
int umssmtools(int PDG_LSP) {
  ...
  // First change the value of "keys haHpmtosfer" into the LanHEP definition of
  // the UMSSM and then reproduce the modified model files Then :
  /* for h -> ~uL ~UL : */
