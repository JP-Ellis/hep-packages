...
//#define OMEGA  /* Calculate relic density and display contribution of
//individual channels */
...
int main(int argc, char **argv) {
  ...
  /*
    ...
    spin2,Mcdm1); if(charge3) printf("Dark Matter has electric charge
    ...
    mass=%.2E\n",CDM2,spin2,Mcdm2); if(charge3) printf("Dark Matter has electric
    charge %d/3\n",charge3); if(cdim!=1) printf("Dark Matter is a color
    particle\n");
    }
  */
...
#include "../include/SMODELS.inc"
...
#ifdef FREEZEIN
  {
    ...
    if (err)
      printf("darkOmegaFi error code = %d\n", err);
    else
      printf("Freeze-in Omega h^2 =%.3E\n", omegaFi);
  ...
  if (!CDM1 || !CDM2) {
    ...
    displayPlot("nu flux from Sun [1/Year/km^2/GeV]", "E[GeV]", Emin, Mcdm, 0,
                1, "nu", 0, SpectdNdE, nu, "nu_bar", 0, SpectdNdE, nu_bar);
#endif
    {
...
#ifdef CLEAN
  system("rm -f HB.* HB.* hb.* hs.*  debug_channels.txt debug_predratio.txt  "
         "Key.dat");
