...
static inline real SignSample(real *x) { return sign_ * Sample(x); }
...
static inline void LinearSolve(ccount n, creal *hessian, creal *grad, real *p) {
  int i, j;
  real dir;
...
static void RenormalizeCholesky(ccount n, real *hessian, real *z, real alpha) {
  count i, j;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (i + 1 >= n || gammanew < 0 ||
        (gammanew < 1 && gamma > DBL_MAX * gammanew))
      return;
...
static void UpdateCholesky(ccount n, real *hessian, real *z, real *p) {
  int i, j;
  real gamma = 0;
...
static inline void BFGS(ccount n, real *hessian, creal *gnew, creal *g, real *p,
                        creal dx) {
  real y[NDIM], c;
  count i, j;
...
static void Gradient(ccount nfree, ccount *ifree, cBounds *b, real *x, creal y,
                     real *grad) {
  count i;
...
static Point LineSearch(ccount nfree, ccount *ifree, creal *p, creal *xini,
                        real fini, real *x, real step, creal range, creal grad,
  ...
  if (step > 0 && range > tol2 && grad <= 0) {
    ...
    for (first = true;; first = false) {
      ...
      if (cur.f <= min.f) {
        ...
        if (cur.f <= w.f || w.dx == 0)
          v = w, w = cur;
        else if (cur.f <= v.f || v.dx == 0 || v.dx == w.dx)
          v = cur;
      }

      if (distmin + b.dx <= xtol)
        break;
      if (min.f < fini && a.f - min.f <= fabs(a.dx) * maxgrad &&
          (fabs(distmin - range) > tol || maxstep < b.dx))
        break;

      mid = .5 * (a.dx + b.dx);
      if (fabs(mid) <= tol2 - .5 * (b.dx - a.dx))
        break;
      ...
      if (fabs(end) > tol) {
        ...
        q = fabs(q);
        r = end;
        if (step != b1 || b.dx <= maxstep)
          end = step;
      }

      if (distmin == a.dx)
        step = mid;
      else if (b.dx > maxstep)
        step = (step < b.dx) ? -4 * a.dx : maxstep;
      else {
        ...
        num /= -den;
        step =
            (num < 1) ? .5 * den * sqrt(num) : 5 / 11. * den * (.1 + 1 / num);
      }
...
static real LocalSearch(ccount nfree, ccount *ifree, cBounds *b, creal *x,
                        creal fx, real *z) {
...
static real FindMinimum(cBounds *b, real *xmin, real fmin) {
  ...
  for (local = 0; local < 2; ++local) {
    ...
    if (resample)
      fini = fmin = SignSample(xmin);

    if (nfree == 0)
      goto releasebounds;

    Gradient(nfree, ifree, b, xmin, fmin, gfree);
    if (local || Length(nfree, gfree) > GTOL)
      break;
  ...
  while (neval_ <= maxeval) {
    ...
    if (nfree > 0) {
      ...
      low = LineSearch(nfree, ifree, p, xmin, fmin, tmp, Min(minstep, 1),
                       Min(minstep, 100), Dot(nfree, gfree, p), RTEPS / pleneps,
                       DELTA / pleneps, .2);
      ...
      if (low.dx > 0) {
        ...
        if (fabs(low.dx - minstep) < QEPS * minstep)
          goto fixbound;

        fdiff = fini - fmin;
        fini = fmin;
        if (fdiff > (1 + fabs(fmin)) * FTOL ||
            low.dx * plen > (1 + Length(ndim_, xmin)) * FTOL)
          continue;
      }
    }
