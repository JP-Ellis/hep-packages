/*
        f77290.c
                ...
                Sept 97, last modified 1 Nov 05 th
*/
...
const char *types[] = {
    ...
    "double complex",
#endif
    "double precision", "integer", "real", "character", "complex", "logical"};

const char *units[] = {"function", "subroutine", "block data", "program"};
...
int isnumber(const int c1, const int c2, const int c3, const int c4) {
  int i2 = isdigit(c2), i3 = isdigit(c3);

  if (i3)
    return i2 || ((c2 | 0x21) == 'e' && isdigit(c1)) ||
           ((c2 == '+' || c2 == '-') && (c1 | 0x21) == 'e');
  if (i2)
    return (c3 | 0x21) == 'e' && (c4 == '+' || c4 == '-' || isdigit(c4));
  return (c2 | 0x21) == 'e' && isdigit(c1) && (c3 == '+' || c3 == '-') &&
         isdigit(c4);
}

#ifdef QPRECISION
void typereplace(const char *s, char *d, const char *from, const char *to) {
  int i;
  char *p, s2[200];
...
int main(int argc, char **argv) {
  ...
  *funcname = fnstack;
  while (!feof(f)) {
    ...
    *p = 0;
    ...
    if (strcmp(s, "end") == 0 || strncmp(s, "end function", 12) == 0 ||
      ...
      if (fnp == funcname) {
        *d = 0;
        fprintf(stderr, "warning: superfluous END statement in line %d\n", lnr);
      } else {
    ...
    if ((p = strstr(s, "goto")) || (p = strstr(s, "go to")))
      if ((i = strtol(p + 5, &d2, 10)))
        *gp++ = i;
#ifdef QPRECISION
    if (strncmp(s, "parameter", 9) == 0)
      param = 1;
    if (param) {
      p = d;
      while ((p = strchr(p, '=')))
    ...
    /* statement functions _are_ a problem if they
       ...
       therefore:  */
             ...
             (strncmp(s, "absc(", 5) == 0 || strncmp(s, "absr(", 5) == 0 ||
              strncmp(s, "norm(", 5) == 0)) {
  ...
  for (new = start; new; new = new->next) {
    ...
    if (strcasecmp(new->s, "enddo") == 0 || strcasecmp(new->s, "end do") == 0)
      indent -= 2;
    new->indent += indent;
    if (new->label) {
      for (i = new->label; i; i /= 10)
        --new->indent;
      if (--new->indent <= 0)
        *s = 0;
      else {
