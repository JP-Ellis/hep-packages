    ...
    for (l = m2_i; l; l = ListTail(l)) {
      ...
      SetCompoundArg(a1, 4, l1);
      {
        ...
        c_mlt(IntegerValue(CompoundArg1(m2_b2)),
              ...
              IntegerValue(CompoundArg2(ListFirst(l))), &n, &d);
...
Term ProcLet(Term t, Term ind) {
  ...
  if (CompoundName(t1) == OPR_EQSIGN && is_atom(CompoundArg1(t1)) &&
      ...
      (CompoundArg2(t1) == A_GAMMA5 ||
       GetAtomProperty(CompoundArg2(t1), A_GAMMA5))) {
  ...
  if (is_compound(nm) &&
      (CompoundName(nm) == OPR_USCORE || CompoundName(nm) == OPR_CARET))
  ...
  if (transf_fl == 0) {
    ...
    } else if (only_prm(CompoundArg1(t)))
...
void alg1_opt_let(Term a1) {
  ...
  rpt2:
    ...
    if (ListLength(CompoundArg1(f1)) == 1 &&
        ListLength(CompoundArg1(f1)) == 1) {
