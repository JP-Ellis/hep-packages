...
static Term sub_ind_wild(Term w, int ino, int i) {
  ...
  if (is_empty_list(al)) {
    ...
    return MakeCompound2(A_ALG1, AppendFirst(NewList(), ret), CopyTerm(il));
  }
...
static List sub_w_2(Term m1, Atomic ind, int ci1, int i1, int ci2, int i2,
                    int i) {
  Term w1, w2;
  List wl1, wl2, ret;
  MTHOPTDECL
  ...
  for (l1 = nind; l1; l1 = ListTail(l1)) {
    ...
    for (l2 = al; l2; l2 = ListTail(l2)) {
      ...
      for (c = 1, l3 = CompoundArgN(m1, 3); l3; c++, l3 = ListTail(l3)) {
        for (i = 1, l4 = CompoundArg1(ListFirst(l3)); l4;
             i++, l4 = ListTail(l4))
