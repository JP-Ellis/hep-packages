...
static Term cc_particle(Term t1, List *ind) {
  ...
  if (!is_empty_list(*ind) &&
      CompoundName(CompoundArg1(ListFirst(*ind))) == A_LORENTZ) {
    Term tt, in1, in2;
...
cnt:
  ...
  if (!is_empty_list(rl)) {
    ...
    rl = CopyTerm(rl);
    if (is_empty_list(ind)) {
      SetCompoundArg(ret, 3,
                     AppendFirst(NewList(), MakeCompound2(ttype, rl, t)));
...
Term ExpandTerm(Term t) {
  ...
  puts("\n");*/
  if (is_compound(t) &&
      (CompoundName(t) == OPR_USCORE || CompoundName(t) == OPR_CARET)) {
    t = SplitIndices(t, &il);
    if (t == 0)
      longjmp(alg1_jmp_buf, 1);
  }
  ...
  if (is_atom(t) || (is_compound(t) && CompoundName(t) == A_FBRACET) ||
      ...
      (is_compound(t) &&
       (CompoundName(t) == A_SIN || CompoundName(t) == A_COS) &&
  ...
  if (opSetGpm && is_compound(t) && CompoundArity(t) == 2 &&
      CompoundArg1(t) == NewInteger(1) && is_atom(CompoundArg2(t)) &&
      (CompoundArg2(t) == A_GAMMA5 ||
       GetAtomProperty(CompoundArg2(t), A_GAMMA5)) &&
