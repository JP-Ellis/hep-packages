  ...
  */
  ...
  while (!is_empty_list(l)) {
    ...
    if (CompoundName(t) == OPR_SPECIAL && CompoundArg1(t) == A_MOMENT) {
      ...
      t2 = MakeCompound2(A_MOMENT, CompoundArg1(t1), ConsumeCompoundArg(t, 2));
...
Term alg2_mterm_to_2(Term t) {
  ...
  */
  ...
  while (!is_empty_list(l)) {
    ...
    printf("Error: fail to process %s in 1->2 (", AtomValue(CompoundName(t1)));
    WriteTerm(t1);
    puts("");
    FreeAtomic(t1);
    longjmp(alg2_jmp_buf, 1);
...
static void a2_symm_1(Term a2, int ppos, int pno) {
  ...
  while (next_perm((int *)rpl, pno)) {
    ...
    for (i = 1; i <= pno; i++)
      rm = AppendLast(
          rm,
          MakeCompound2(OPR_RARROW, NewInteger(ppos - 1 + i),
    ...
    for (i = 1; i <= pno; i++)
      for (j = 0; j < pindno; j++)
        ri = AppendLast(
            ri, MakeCompound2(
                    OPR_RARROW, ListNth(pind[i - 1], j + 1),
    ...
    for (l2 = l1; l2; l2 = ListTail(l2))
      for (l3 = CompoundArgN(ListFirst(l2), 3); l3; l3 = ListTail(l3))
        if (CompoundArg1(ListFirst(l3)) == A_DELTA) {
          ...
          la2 = IntegerValue(ListFirst(ListTail(CompoundArg2(ListFirst(l3)))));
          if (la1 > la2) {
            ChangeList(CompoundArg2(ListFirst(l3)), NewInteger(la2));
            ChangeList(ListTail(CompoundArg2(ListFirst(l3))), NewInteger(la1));
          }
        }
    ...
    if (ListLength(CompoundArg1(a2)) == 2 && !FAOutput)
      for (l2 = l1; l2; l2 = ListTail(l2)) {
        ...
        if (s == -1)
          SetCompoundArg(CompoundArg1(ListFirst(l2)), 1,
...
void alg2_symmetrize(Term a2) {
  ...
  for (l = CompoundArg1(a2); l && ListTail(l); l = ListTail(l)) {
    ...
    if (CompoundArg1(ListFirst(l)) == CompoundArg1(ListFirst(ListTail(l)))) {
      ...
      ps1 = ps;
      while (ListTail(l) &&
             CompoundArg1(ListFirst(l)) == CompoundArg1(ListFirst(ListTail(l))))
        l = ListTail(l), ps++;
      sz = ps - ps1;
      epl = AppendLast(
          epl, MakeCompound2(OPR_DIV, NewInteger(ps1), NewInteger(sz + 1)));
    }
  }
