...
static int mcmp(Term m1, Term m2) {
  ...
  if (ListLength(l1) < ListLength(l2))
    return -1;
  return strcmp(AtomValue(ListFirst(l1)), AtomValue(ListFirst(l2)));
}
...
static int prt_spin(Atom p) {
  ...
  if (is_empty_list(prop) ||
      CompoundName(CompoundArg1(ListFirst(prop))) != A_LORENTZ)
    return 0;
...
static Atom remake_sp(List m3) {
  ...
  if (mom2 == 0 && ga)
    ...
    re = NewAtom("p1^mu*p1^nu", 0);
  if (ag == A_GAMMA5) {
    sprintf(cbuf, "%s*gamma5", AtomValue(re));
    re = NewAtom(cbuf, 0);
  }
  if (ag == A_GAMMAP) {
    sprintf(cbuf, "%s*(1+gamma5)/2", AtomValue(re));
    re = NewAtom(cbuf, 0);
  }
  if (ag == A_GAMMAM) {
    sprintf(cbuf, "%s*(1-gamma5)/2", AtomValue(re));
...
rpt2:
  ...
  */
  ...
  /*	WriteTerm(CompoundArg1(a2));printf(" base ");
          WriteTerm(b1);printf("/");WriteTerm(b2);printf(";
     sp=%d\n",prt_spin(b1)); puts("");
  */
...
static void look_for_orth2(List prt, List al, int c, int s, FILE *f, Label,
                           Label);
...
Term ProcCheckMasses(Term t, Term ind) {
  ...
  if (is_compound(t)) {
    ...
    for (i = 1; i <= CompoundArity(t); i++) {
      ...
      if (is_compound(a) && CompoundName(a) == OPR_EQSIGN &&
          CompoundArity(a) == 2 && CompoundArg1(a) == NewAtom("RedDiag", 0) &&
          is_atom(CompoundArg2(a))) {
        Atom v;
        Term prop;
        v = CompoundArg2(a);
      ...
      ErrorInfo(709);
      printf("CheckMasses: invalid argument '");
      WriteTerm(a);
      puts("'.");
      return 0;
    }
  ...
  for (l = all_lagr; l; l = ListTail(l)) {
    ...
    */
    ...
    if (is_empty_list(l1))
      masses = AppendLast(
          masses,
          MakeCompound2(OPR_DIV, b1 == b2 ? MakeList1(b1) : MakeList2(b1, b2),
                        MakeList1(a2)));
  }
      ...
      if (!correct_mt(a2, bn, &tmass)) {
        ...
        if (is_atom(tmass)) {
          ...
          if (tmass == 0)
            ...
            fprintf(outf,
                    "This (derived) particle should not have mass term\n");
        }
      }
...
static List get_expr2(List ml, int i, int j, int fixel, int is_c, Label ola,
                      Label ola2) {
  List l1, l2, l3;
  List res = 0;
  ...
  for (l1 = ml; l1; l1 = ListTail(l1)) {
    ...
    if (p1 == 0 || p2 == 0) {
      printf("Error in determining chargino-like mass matrix: missing mixing "
             "element\n");
      return NewAtom("???", 0);
    }
  ...
  */
  ...
  if (is_c == 0 && 2 * ListLength(al) != prtlen * (prtlen + 1))
    fprintf(f, "%d particles, expecting %d vertices, found %d.\n", prtlen,
            prtlen * (prtlen + 1) / 2, ListLength(al));

  if (is_c != 0 && ListLength(al) != prtlen * prtlen)
    fprintf(f, "%d particles, expecting %d vertices, found %d.\n", prtlen,
            prtlen * prtlen, ListLength(al));
    ...
    for (l1 = al; l1; l1 = ListTail(l1)) {
      if (CompoundArg1(CompoundArg1(ListFirst(l1))) == p1 &&
        ...
        for (l2 = CompoundArgN(ListFirst(l1), 4); l2; l2 = ListTail(l2))
          for (l3 = CompoundArg2(ListFirst(l2)); l3; l3 = ListTail(l3)) {
            ...
            if ((prp1 = GetAtomProperty(p, A_ORTH_MATR)) &&
                CompoundArg1(prp1) != orth_red_diag && !ListMember(oeused, p))
        ...
        for (l2 = oeused; l2; l2 = ListTail(l2)) {
          ...
          if (!ListMember(e1used, i))
            e1used = AppendLast(e1used, i);
          if (!ListMember(e2used, j))
            e2used = AppendLast(e2used, j);
        }
  ...
  for (l1 = al; l1; l1 = ListTail(l1)) {
    ...
    for (i = 1; i <= prtlen; i++)
      for (j = i; j <= prtlen; j++) {
        Term thexpr;
        thexpr = get_expr(CompoundArgN(ListFirst(l1), 4), prtp[i], prtp[j],
                          fixel, is_c);
        if (mmatr[i][j] == 0)
          mmatr[i][j] = thexpr;
        else {
  ...
  for (i = 1; i <= prtlen; i++) {
    ...
    for (j = 1; j <= prtlen; j++)
      fprintf(f, " %s ",
              fixel == 2 ? AtomValue(herm_matr_el(ola, prtp[i], prtp[j]))
  ...
  */
  ...
  for (l1 = al; l1; l1 = ListTail(l1)) {
    ...
    for (i = 1; i <= prtlen; i++)
      for (j = 1; j <= prtlen; j++) {
        Term thexpr;
        thexpr = get_expr2(CompoundArgN(ListFirst(l1), 4), prtp[i], prtp[j],
                           fixel, is_c, ola, ola2);
        if (mmatr[i][j] == 0)
          mmatr[i][j] = thexpr;
        else {
...
static int correct_kt(Term m2, int spi) {
  ...
  if ((spi == 0 || spi == 3) && CompoundArg1(CompoundArg1(m2)) != NewInteger(1))
    return 0;
  if ((spi != 0 && spi != 3) &&
      CompoundArg1(CompoundArg1(m2)) != NewInteger(-1))
    return 0;
