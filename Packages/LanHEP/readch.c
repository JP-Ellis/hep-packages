...
static int read_prt(int no, Atom dir) {
  ...
  while (cbuf[curl] == ' ')
    curl++;
  if (cbuf[curl] == 'n' || cbuf[curl] == 'N' || cbuf[curl] == 'p' ||
      cbuf[curl] == 'P')
    ca = 1;
  curl = 0;
  ...
  while (fgets(cbuf, 1250, f)) {
    ...
    if (aux)
      ...
      case 'G':
        ...
        break;
      case 'L':
        SetCompoundArg(prt, 7, A_LEFT);
        break;
      case 'R':
        SetCompoundArg(prt, 7, A_RIGHT);
        break;
      default:
    ...
    */
    if (col && AtomValue(col)[0] == '3')
      SetCompoundArg(prt, 8,
                     MakeList1(MakeCompound2(A_COLOR, NewAtom("c3", 0),
                                             NewAtom("c3b", 0))));

    if (col && AtomValue(col)[0] == '8')
      SetCompoundArg(prt, 8,
                     MakeList1(MakeCompound2(A_COLOR, NewAtom("c8", 0),
                                             NewAtom("c8", 0))));
    ...
    /*		printf("%10s|%4s|%4s|%3s|%5s|%5s|%3s|%2s|%10s|%10s\n",AtomValue(name),AtomValue(pname),
                                    AtomValue(apname),
                                    AtomValue(spa), AtomValue(mass),
       AtomValue(width), AtomValue(col), AtomValue(aux), AtomValue(lt),
       AtomValue(alt));
    */
  }
...
static List l_mlt(List t1, List t2) {
  ...
  for (l1 = t1; l1; l1 = ListTail(l1))
    for (l2 = t2; l2; l2 = ListTail(l2)) {
      ...
      ret = AppendLast(
          ret, MakeCompound2(OPR_MLT, NewInteger(i1 * i2), ConcatList(q1, q2)));
    }
...
static List e_2_l(Term t) {
  ...
  if (is_atom(t) && strcmp(AtomValue(t), "G5") == 0)
    return MakeList1(
        MakeCompound2(OPR_MLT, NewInteger(1), MakeList1(A_GAMMA5)));

  if (is_atom(t))
    return MakeList1(MakeCompound2(OPR_MLT, NewInteger(1), MakeList1(t)));
  ...
  if (opSetGpm && CompoundArity(t) == 2 && CompoundArg1(t) == NewInteger(1) &&
    ...
    if (CompoundName(t) == OPR_PLUS)
      return MakeList1(
          MakeCompound2(OPR_MLT, NewInteger(2), MakeList1(A_GAMMAP)));
    else
      return MakeList1(
          MakeCompound2(OPR_MLT, NewInteger(2), MakeList1(A_GAMMAM)));
  }
  ...
  if (CompoundName(t) == OPR_CARET && CompoundArity(t) == 2) {
    ...
    for (l1 = t1; l1; l1 = ListTail(l1))
      for (l2 = t2; l2; l2 = ListTail(l2)) {
        ...
        ret = AppendLast(
            ret, MakeCompound2(OPR_MLT, NewInteger(i1 * i2),
                               MakeList1(MakeCompound2(A_POINT, q1, q2))));
      }
    FreeAtomic(t1);
    FreeAtomic(t2);
    return ret;
...
static Term c_mlt(Term m1, Term m2) {
  ...
  SetCompoundArg(ret, 1,
                 NewInteger(IntegerValue(CompoundArg1(m1)) *
                            IntegerValue(CompoundArg1(m2))));
  SetCompoundArg(ret, 2,
                 NewInteger(IntegerValue(CompoundArg2(m1)) *
                            IntegerValue(CompoundArg2(m2))));

  SetCompoundArg(
      ret, 3, ConcatList(ConsumeCompoundArg(m1, 3), ConsumeCompoundArg(m2, 3)));
  SetCompoundArg(
      ret, 4, ConcatList(ConsumeCompoundArg(m1, 4), ConsumeCompoundArg(m2, 4)));
...
static Term mk_a1(Term cf, List lp, Term colobj, Label fei1, Label fei2,
  ...
  for (l1 = lp; l1; l1 = ListTail(l1)) {
    ...
    SetCompoundArg(cf1, 1,
                   NewInteger(IntegerValue(CompoundArg1(cf1)) *
                              IntegerValue(CompoundArg1(lp1))));
    ...
    for (l3 = CompoundArg2(lp1); l3; l3 = ListTail(l3)) {
      ...
      if (cuo == A_GAMMA5 || cuo == A_GAMMAP || cuo == A_GAMMAM) {
        ...
        List li;
        t1 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(1), NewInteger(0)), fei11);
        t2 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(0), NewInteger(1)), 0);
        if (lf)
      ...
      if (is_compound(cuo) && CompoundName(cuo) == A_GAMMA) {
        ...
        t2 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(0), NewInteger(1)), 0);
        t3 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(2), NewInteger(2)), lind);
        if (lf)
          SetCompoundArg(lf, 2, fei11);
        lf = t2;
        fei11 = NewLabel();
...

      {
        ...
        t1 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(2), NewInteger(2)), li1);
        t2 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(2), NewInteger(2)), li2);

        l2 = AppendLast(l2,
                        MakeCompound2(OPR_SPECIAL, MakeList2(t1, t2), A_DELTA));
      }
    }
    ...
    if (fei11) {
      if (lf)
        ...
        t1 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(1), NewInteger(0)), fei11);
        t2 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(0), NewInteger(1)), fei2);
        l2 = AppendLast(l2,
                        MakeCompound2(OPR_SPECIAL, MakeList2(t1, t2), A_DELTA));
      }
    }
    ...
    for (l3 = prtl; l3; l3 = ListTail(l3)) {
      ...
      if (pind[pcn]) {
        Term t1;
        t1 = MakeCompound2(
            A_I, MakeCompound2(A_LORENTZ, NewInteger(2), NewInteger(2)),
            pind[pcn]);
        l2 =
            AppendLast(l2, MakeCompound2(OPR_SPECIAL, MakeList1(t1), A_MOMENT));
      }
      l2 = AppendLast(l2, ps);
    }
  ...
  while (fgets(cbuf, 12500, f)) {
    ...
    for (pos1 = 1; pos1 <= prtno; pos1++) {
      ...
      for (pos2 = 1; pos2 <= ll; pos2++) {
        ...
        if (CompoundName(it) == A_LORENTZ &&
            CompoundArg1(it) == NewInteger(2)) {
          char cbuf[4];
          if (pos2 == 1)
        ...
        if (CompoundName(it) == A_COLOR) {
          ...
          coi[pos1] = la;
          cot[pos1] =
              IntegerValue(CompoundArg1(GetAtomProperty(prt[pos1], A_COLOR)));
        }
        if (la == 0) {
...

    {
      ...
      if (ctypes[1] == 0 && ctypes[2] == 0 && ctypes[3] == 0)
        ...
        colobj = MakeCompound2(
            OPR_SPECIAL,
            MakeList2(MakeCompound2(A_I, c1, i1), MakeCompound2(A_I, c2, i2)),
            A_DELTA);
        ...
        colobj = MakeCompound2(OPR_SPECIAL,
                               MakeList3(MakeCompound2(A_I, c1, i1),
                                         MakeCompound2(A_I, c2, i2),
        ...
        colobj = MakeCompound2(OPR_SPECIAL,
                               MakeList3(MakeCompound2(A_I, c[0], i[0]),
                                         MakeCompound2(A_I, c[1], i[1]),
