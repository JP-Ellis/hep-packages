...
int VerbMode = 0;
int VerbT = 0;
time_t tm_lt = 0, tm_12 = 0, tm_sl = 0, tm_sw = 0, tm_a2 = 0, tm_add = 0,
       tm_rl = 0, tm_wr = 0;
time_t tm_rsc = 0;
...
void ProcessTerm(Term t) {
  ...
  if (is_compound(t) && CompoundName(t) == OPR_LOCAL) {
    ...
    for (l1 = la; l1; l1 = ListTail(l1))
      l_ali = AppendLast(
          l_ali, InterfSetAlias(MakeCompound1(OPR_ALIAS, ListFirst(l1)), 0));
    RemoveList(la);
    return;
  }
  ...
  if (!is_compound(t) ||
      (CompoundName(t) != OPR_ALIAS && CompoundName(t) != OPR_IN &&
       CompoundName(t) != OPR_WHERE))
    t = ProcessAlias(t);
  ...
  if (procc == 0) {
    if (!IsTermInput())
      printf("File \"%s\", line %d: ", CurrentInputFile(), CurrentInputLine());
  ...
  if (is_compound(t) &&
      ((VerbMode && CompoundName(t) == OPR_LTERM) || VerbMode == 3)) {
    char sment[1024];
    sWriteTerm(sment, t);
    sment[55] = '.';
    sment[56] = '.';
...
int main(int argc, char **argv, char **env) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (strcmp(argv[i], "-c4") == 0 || strcmp(argv[i], "-co") == 0 ||
    ...
    if (strcmp(argv[i], "-calchep") == 0 || strcmp(argv[i], "-CalcHEP") == 0 ||
        strcmp(argv[i], "-ca") == 0) {
      setoutput(2);
      continue;
    }
    ...
    if (strcmp(argv[i], "-feynarts") == 0 ||
        strcmp(argv[i], "-FeynArts") == 0 || strcmp(argv[i], "-fa8") == 0 ||
        strcmp(argv[i], "-fa") == 0) {
      setoutput(3);
      continue;
    }
  ...
  if (CompOutput == 0 && CalcOutput == 0 && TexOutput == 0 && FAOutput == 0 &&
      UFOutput == 0) {
  ...
  if (InputFile != NULL)
    ...
    printf("Welcome to LanHEP                                Version 3.1.9  "
           "(Nov 27 2013)\n");
    /*
  ...
  */
  ...
  if (ModelNumber != 0 || InputFile != NULL) {
    ...
    if (!TexOutput && !FAOutput && C_F_WIDTH < longest_cfline) {
      printf(
          "Error: 'Common Factor' field longer than maximum of %d symbols.\n",
          C_F_WIDTH);
    ...
    if (!TexOutput && !FAOutput && P_D_WIDTH < longest_pdline) {
      if (P_D_WIDTH) {
        printf("Error: 'Parameter expression' field for '%s' is longer than "
               "maximum of %d symbols.\n",
        ...
        printf("Longest parameter expression is %d symbols for '%s'.\n",
               longest_pdline, AtomValue(llparam));
    }
