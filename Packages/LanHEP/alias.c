...
Term InterfSetAlias(Term t, Term ind) {
  ...
  if (!is_compound(tt) || CompoundArity(tt) != 2 ||
    ...
    ErrorInfo(212);
    WriteTerm(tt);
    printf(" : wrong argument in call to SetAlias\n");
    FreeAtomic(tt);
    return 0;
  }
...
Term ali_uni(Term t, Term s, List *lu) {
  ...
  if (is_compound(t) && is_compound(s) &&
      CompoundArity(s) == CompoundArity(t)) {
    int ac, i;
    ac = CompoundArity(s);
    for (i = 1; i <= ac; i++)
  ...
  if (is_compound(t) && CompoundArity(t) == 1 &&
    ...
    for (l = li; l; l = ListTail(l)) {
      ...
      if (CompoundName(t) == A_LORENTZ && CompoundName(sp) == A_LORENTZ) {
        if (CompoundArg1(sp) == NewInteger(1) &&
            CompoundArg2(sp) == NewInteger(0))
          lp = AppendLast(lp, OPR_SPINOR);
        else if (CompoundArg1(sp) == NewInteger(0) &&
                 CompoundArg2(sp) == NewInteger(1))
          lp = AppendLast(lp, A_CSPINOR);
        else if (CompoundArg1(sp) == NewInteger(2) &&
                 CompoundArg2(sp) == NewInteger(2))
          lp = AppendLast(lp, OPR_VECTOR);
        else if (CompoundArg1(sp) == NewInteger(0) &&
                 CompoundArg2(sp) == NewInteger(0))
          lp = AppendLast(lp, NewAtom("spinor2,", 0));
        else
