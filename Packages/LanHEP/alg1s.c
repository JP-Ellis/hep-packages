...
static int acmp(Atom a, Atom b) { return strcmp(AtomValue(a), AtomValue(b)); }
...
static Term mk_im_f(int apdim, int apcol, Term inw, Term inc, Term inv1,
                    Term inv2) {
  Term a, m;
  List al = 0;
  int i;
...
static void mk_im_f_2(int apdim, int apcol, Term inw1, Term inc1, Term inw2,
                      Term inc2, Term *ap1, Term *ap2) {
  Term a, m;
  List al1 = 0, al2 = 0;
  int i;
...
Term ProcDFDFC(Term t, Term ind) {
  ...
  for (l1 = CompoundArg1(t); l1; l1 = ListTail(l1))
    for (l2 = CompoundArg1(t1); l2; l2 = ListTail(l2)) {
      ...
      if (opSplitCol1 == -1 && alg1_cw_mono(m1) == 2 && alg1_cw_mono(m2) == 2)
        continue;
      ...
      SetCompoundArg(m, 3,
                     ConcatList(CopyTerm(CompoundArgN(m1, 3)),
                                CopyTerm(CompoundArgN(m2, 3))));
      SetCompoundArg(m, 4,
                     ConcatList(CopyTerm(CompoundArgN(m1, 4)),
