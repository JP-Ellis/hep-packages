...
int tri_heu(List m2l) {
  ...
  if (ListLength(lu) == 2 && IntegerValue(ListFirst(lu)) > 0) {
    ...
    if (tri_divs(ListNth(tri_si_co_list, a1), &ft, &l1, &l2, m2l)) {
    ...
    if (tri_divf(ListNth(tri_si_co_list, a1), &l1, &l2, m2l)) {
      List l1_r, l1_l, l2_r, l2_l;
      Term m2;
      if (red_1(a1, l1, &l1_l, &l1_r) == 0)
        return 0;
  ...
  if (ListLength(lu) == 2 && IntegerValue(ListFirst(lu)) < 0) {
    ...
    if (tri_divf(ListNth(tri_si_co_list, a1), &l2, &l1, m2l)) {
      List l1_r, l1_l, l2_r, l2_l;
      Term m2;
...
xyz1:
  ...
  for (l = lu; l; l = ListTail(l))
    m2l = tri_cvt(
        m2l, CompoundArg1(ListNth(tri_si_co_list, IntegerValue(ListFirst(l)))),
...
xyz11:
  ...
  for (l = lu; l; l = ListTail(l))
    m2l = tri_cvt(
        m2l, CompoundArg2(ListNth(tri_si_co_list, IntegerValue(ListFirst(l)))),
...
xyz1:
  ...
  if (!opUndefAngleComb) {
    ...
    if (ListLength(l1) > 2 || opNoDummies)
      ...
      SetCompoundArg(
          ListFirst(ListTail(l3)), 1,
          NewInteger(IntegerValue(CompoundArg1(ListFirst(ListTail(l3)))) / cf));
...
static Term m2_p_to_e(Term pt) {
  ...
  if (is_list(CompoundArg1(pt)))
    return MakeCompound2(OPR_POW, tri_m2_l_to_e(CompoundArg1(pt)),
                         CompoundArg2(pt));
