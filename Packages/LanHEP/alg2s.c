  ...
  if (vtype == 4) {
    ...
    do {
      ...
      while (!is_empty_list(l2)) {
        ...
        a1 = ListFirst(l1);
        a2 = ListFirst(l2);
        if (strcmp(AtomValue(CompoundArg1(a1)), AtomValue(CompoundArg1(a2))) ==
            1) {
          ChangeList(l1, a2);
          ChangeList(l2, a1);
          flag = 1;
  ...
  */
  ...
  while (lab0 != lab2) {
    ...
    while (!is_empty_list(l1)) {
      Term y1;
      y1 = ListFirst(l1);
      if (CompoundName(y1) == OPR_SPECIAL && CompoundArg1(y1) == A_GAMMA &&
...

{
  ...
  Atom o1, o2;
  {
    ...
    if ((pr1 == 0 && pr2 == 0) || (pr1 != 0 && pr2 != 0))
      ...
      bnm = pr1;
      cnm = CallFunction(
          MakeCompound1(A_CC, CompoundArg1(ListFirst(ListTail(prt)))), 0);
      if (bnm == CompoundArg1(ListFirst(ListTail(prt))))
        mfl = 1;
    } else {
      typ = 2;
      bnm = pr2;
      cnm = CallFunction(MakeCompound1(A_CC, CompoundArg1(ListFirst(prt))), 0);
  ...
  if (mfl) {
    ...
    /*		prp=GetAtomProperty(bnm,PROP_TYPE);
                    if(CompoundArg1(prp)!=CompoundArg2(prp))
                            {
                            printf("Warning: vertex with 2 equivalent Dirac
       spinors '"); WriteTerm(bnm); puts("'");
                            }*/
  ...
  if (typ == 3 || (typ == 1 && strcmp(AtomValue(bnm), AtomValue(o2)) > 0) ||
    ...
    if (is_empty_list(gl))
      ...
      if (CompoundArg1(ListFirst(ListTail(gl))) == A_GAMMA) {
        gsf = 1;
        inv_gamma(gl);
      } else if (CompoundArg1(ListFirst(ListTail(gl))) == A_GAMMA5)
        gsf = 1;
      else {
        gsf = -1;
...
int alg2_updown(List prt, List *spec) {
  ...
  if (sigma == 0) {
    ...
    */
    /*		if(pbar[0]+pbar[1]!=1 && bname[0]!=baname[0] &&
       bname[1]!=baname[1])
                            {
                            ErrorInfo(916);
                            printf("wrong combination of 2-component spinors in
       vertex "); WriteVertex(prt); puts(""); return 0;
                            }
    */
    ilab[0] = NewLabel();
    ilab[1] = NewLabel();
