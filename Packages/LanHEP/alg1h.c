    ...
    */
    ...
    if (!is_atom(aa) || (at && !is_atom(at))) {
      ...
      if (!remove_rc) {
        ...
        Term f = MakeCompound1(OPR_LET,
                               MakeCompound2(OPR_EQSIGN, aa, NewInteger(0)));
        CallFunction(f, 0);
        if (at) {
          Term f = MakeCompound1(OPR_LET,
                                 MakeCompound2(OPR_EQSIGN, at, NewInteger(0)));
          CallFunction(f, 0);
        }
      }
    }
...
List a1l_rem_inf(List l) {
  ...
  if (ch) {
    ...
    for (l1 = l; l1; l1 = ListTail(l1))
      if (ListFirst(l1)) {
        if (ml2 == 0) {
          ml2 = AppendLast(ml2, ListFirst(l1));
          mle = ml2;
        } else {
  ...
  for (l = CompoundArg1(a1); l; l = ListTail(l)) {
    ...
    */
    if (gpm == A_GAMMAM) {
      ...
      tu = MakeCompound2(
          IntegerValue(CompoundArg1(ListFirst(l))) > 0 ? OPR_PLUS : OPR_MINUS,
          tu, MakeCompound2(OPR_DIV, mm, NewInteger(2)));
    } else {
      ...
      td = MakeCompound2(
          IntegerValue(CompoundArg1(ListFirst(l))) > 0 ? OPR_PLUS : OPR_MINUS,
          td, MakeCompound2(OPR_DIV, mm, NewInteger(2)));
    }
  }
...
Term ProcTransform(Term t, Term ind) {
  ...
  if (is_parameter(t) && GetAtomProperty(t, OPR_MASS)) {
    ...
    if (CompoundArg2(t1) == 0)
      SetCompoundArg(t1, 2,
                     MakeCompound1(OPR_MASS, MakeCompound2(OPR_DIV, prt, t)));
    return 0;
  }
  ...
  if (is_particle(t, 0)) {
    ...
    if (CompoundName(prp) == OPR_PARTICLE &&
        CompoundArg1(prp) != CompoundArg2(prp) && t == CompoundArg2(prp))
      return 0;
    ...
    for (l = CompoundArg1(t1); l; l = ListTail(l)) {
      ...
      if (ListLength(inff) != 1 || ListLength(fi) != 1 ||
        ...
        if (ListLength(inff)) {
          WarningInfo(0);
          printf("non-standard rc for field %s\n", AtomValue(t));
        }
        continue;
      }
      ...
      if (sp == 0)
        ...
        tp = OPR_VECTOR;
      else if (ListFirst(gpm) == A_GAMMAM)
        tp = A_LEFT;
      else
        tp = A_RIGHT;

      tp = MakeCompound1(
          tp, MakeCompound2(OPR_RARROW, MakeCompound2(OPR_DIV, t, mass1),
  ...
  cnt:
    for (l2 = ml; l2; l2 = ListTail(l2)) {
      if (CompoundArg2(ListFirst(l2)) == A_DELTA) {
        ...
        if (CompoundName(ListFirst(CompoundArg1(ListFirst(l2)))) == OPR_WILD)
          cno = IntegerValue(
              CompoundArgN(ListFirst(CompoundArg1(ListFirst(l2))), 3));
        else
          cno = 0;
        ...
        if (in1 == in2) {
          ...
          mm = ListFirst(l1);
          if (CompoundName(r1) == A_LORENTZ && CompoundName(r2) == A_LORENTZ &&
  ...
  for (l = all_infi; l; l = ListTail(l)) {
    ...
    if (is_compound(t) && CompoundName(t) == OPR_SCALAR) {
      ...
      if (prt1 == prt2)
        ...
        fprintf(f,
                "1/(%s^2-%s^2) ReTilde[SelfEnergy[prt[\"%s\"] -> prt[\"%s\"], "
                "%s]]\n",
    ...
    if (is_compound(t) && CompoundName(t) == OPR_VECTOR) {
      ...
      if (prt1 == prt2)
        fprintf(
            f, "VSESign ReTilde[DSelfEnergy[prt[\"%s\"] -> prt[\"%s\"], %s]]\n",
            AtomValue(prt1), AtomValue(prt2), mas1 ? AtomValue(mas1) : "0");
      else if (mas1 == 0)
        fprintf(f,
                "2 VSESign /%s^2 ReTilde[SelfEnergy[prt[\"%s\"] -> "
                "prt[\"%s\"], %s]]\n",
                AtomValue(mas2), AtomValue(prt1), AtomValue(prt2),
                AtomValue(mas2));
      else
        fprintf(f,
                "-2 VSESign /%s^2 ReTilde[SelfEnergy[prt[\"%s\"] -> "
    ...
    if (is_compound(t) &&
        (CompoundName(t) == A_LEFT || CompoundName(t) == A_RIGHT)) {
      Term t1 = CompoundArg1(t);
      Atom prt1 = CompoundArg1(CompoundArg1(t1));
      Atom mas1 = CompoundArg2(CompoundArg1(t1));
      Atom prt2 = CompoundArg1(CompoundArg2(t1));
      ...
      if (prt1 == prt2) {
        ...
        if (tp == 'L') {
          ...
          fprintf(f,
                  "%s RVectorCoeff[sff] + %s LVectorCoeff[sff] + 2 "
                  "LScalarCoeff[sff] ] ]\n",
