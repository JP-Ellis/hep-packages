...
List all_prtc_list(void) { return all_particles; }
...
static void trigg1_color(Atom name, List ind) {
  ...
  while (!is_empty_list(l)) {
    ...
    if (r != 0) {
      SetAtomProperty(name, A_COLOR,
                      MakeCompound2(OPR_DIV, NewInteger(r), NewInteger(i)));
      r = -100;
    }
    i++;
    l = ListTail(l);
...
static void trigg_gsb(Term prt, List ind, int anti) {
  ...
  if (CompoundArgN(prt, 7) != A_GAUGE ||
      CompoundArgN(prt, 4) != NewInteger(2) || CompoundArgN(prt, 5) == 0)
    return;
...
static void trigg_tens(Term prt, List ind, int anti) {
  ...
  sprintf(mk_g_buf, "%s.t", AtomValue(a));
  ca = NewAtom(mk_g_buf, 0);
  sprintf(mk_g_buf, "%s.t", AtomValue(aa));
  caa = NewAtom(mk_g_buf, 0);
  ii = CopyTerm(ind);
