...
static Term alg1_df_df(Term m, int pos, List il) {
  ...
  for (l1 = obj, l2 = il; l1; l1 = ListTail(l1), l2 = ListTail(l2))
    nl = AppendLast(
        nl, MakeCompound2(OPR_SPECIAL,
                          MakeList2(ListFirst(l1), CopyTerm(ListFirst(l2))),
                          A_DELTA));
...
Term ProcUED5th(Term t, Term ind) {
  ...
  for (t1 = t; t1; t1 = ListTail(t1)) {
    ...
    Term f, l;
    List il1 = 0, il2;
    if (!is_compound(v) || CompoundArity(v) != 2 || !is_atom(CompoundArg1(v))) {
      ErrorInfo(0);
      printf("ued_5th: wrong expression ");
      WriteTerm(v);
      puts("");
      continue;
    }
    ...
    if (f != A_MOMENT && !is_particle(f, &il1) && !is_let(f, &il1)) {
      ErrorInfo(0);
      printf("ued_5th: is not a particle: ");
      WriteTerm(f);
      puts("");
      continue;
    }
    ...
    ali = MakeCompound1(
        OPR_ALIAS,
        MakeCompound2(OPR_EQSIGN, MakeCompound2(OPR_CARET, f, NewInteger(5)),
                      l));
    CallFunction(ali, 0);
    if (il1 && ListLength(il1) == 1) {
      ali = MakeCompound1(
          OPR_ALIAS, MakeCompound2(OPR_EQSIGN,
                                   MakeCompound2(OPR_CARET, f,
                                                 MakeCompound2(OPR_CARET, A_I,
                                                               NewInteger(5))),
  ...
  for (l1 = a1; l1; l1 = ListTail(l1)) {
    ...
    for (l2 = pl, i1 = 1; l2; l2 = ListTail(l2), i1++) {
      ...
      if (is_particle(CompoundArg2(ListFirst(l2)), 0))
        continue;
      if (GetAtomProperty(CompoundArg2(ListFirst(l2)), OPR_SCALAR) ==
          NewInteger(0))
        continue;
      ...
      if (CompoundName(CompoundArg1(ListFirst(l3))) != A_LORENTZ ||
        ...
        ErrorInfo(0);
        printf("ued_5th: '%s' is not vector field.\n",
      ...
      if (l3) {
        ...
        SetCompoundArg(mlc, 1, NewInteger(-IntegerValue(CompoundArg1(mlc))));
        ...
        if (CompoundArg2(ListFirst(l3)) == A_GAMMA) {
          ...
          SetCompoundArg(mlc, 2, NewInteger(-IntegerValue(CompoundArg2(mlc))));
          SetCompoundArg(mlc, 1, NewInteger(-IntegerValue(CompoundArg1(mlc))));
        } else {
          Term prp;
          prp = GetAtomProperty(CompoundArg2(ListFirst(l3)), A_DERIV5);
    ...
    for (l2 = pl; l2; l2 = ListTail(l2)) {
      if (CompoundArg2(ListFirst(l2)) == A_DERIV5) {
        ...
        if (l3 == 0) {
          ...
          WarningInfo(0);
          puts("deriv5 does not followed by field.");
  ...
  rpt:
          ...
          nl = AppendLast(
              ...
              MakeCompound1(
                  ...
                  AppendFirst(
                      CopyTerm(r),
                      MakeCompound1(
                          A_COS, NewInteger(IntegerValue(CompoundArg1(t1)) -
          ...
          nl = AppendLast(
              nl, MakeCompound1(
                      ...
                      AppendFirst(
                          r, MakeCompound1(
                                 A_COS,
                                 NewInteger(IntegerValue(CompoundArg1(t1)) +
                                            IntegerValue(CompoundArg1(t2)))))));
        }
          ...
          nl = AppendLast(
              ...
              MakeCompound1(
                  ...
                  AppendFirst(
                      CopyTerm(r),
                      MakeCompound1(
                          A_SIN, NewInteger(IntegerValue(CompoundArg1(t1)) -
                                            IntegerValue(CompoundArg1(t2)))))));
          nl = AppendLast(
              nl, MakeCompound1(
                      plus,
    ...
    if (nf == 0)
      ...
      int n = IntegerValue(CompoundArg1(ListFirst(l1))),
          d = IntegerValue(CompoundArg2(ListFirst(l1))), gc;
      n *= nf;
      d *= df;
      gc = gcf(n, d);
      n /= gc;
