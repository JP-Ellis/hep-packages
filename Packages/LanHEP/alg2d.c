...
void alg2_common_t(Term a2) {
  ...
  if (FAOutput) {
    for (l1 = CompoundArg1(a2); l1; l1 = ListTail(l1))
      if (CompoundName(CompoundArg2(ListFirst(l1))) == OPR_VECTOR ||
        ...
        vil = AppendLast(vil,
                         ListFirst(CompoundArg1(CompoundArg2(ListFirst(l1)))));
  }
  ...
  while (!is_empty_list(l1)) {
    ...
    if (FAOutput) {
      ...
      if (vil && CompoundName(spt) == OPR_SPECIAL &&
          CompoundArg1(spt) == A_DELTA) {
        l2 = CompoundArg2(spt);
...
void alg2_common_s(Term a2) {
  ...
  while (!is_empty_list(l)) {
    if (IntegerValue(CompoundArg2(ListFirst(l))) == 0 ||
        ...
        (FAOutput &&
         GetAtomProperty(CompoundArg1(ListFirst(l)), A_INFINITESIMAL))) {
      la = CutFromList(la, l);
      l = la;
    } else
      l = ListTail(l);
  ...
  while (!is_empty_list(l)) {
    ...
    while (!is_empty_list(l2)) {
      ...
      l1 = AppendLast(
          l1, MakeCompound2(OPR_POW, CompoundArg1(t2),
...
void alg2_common_n(Term a2) {
  ...
  SetCompoundArg(a2, 2,
                 MakeCompound2(OPR_DIV, NewInteger(cnum), NewInteger(cden)));
  l = m2l;
  l1 = nl;
