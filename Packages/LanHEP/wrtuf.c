      ...
      if (IntegerValue(CompoundArgN(p, 4)) == 2 &&
        ...
        if (v == 0 || CompoundArg2(v) == NewInteger(1))
          ...
          fprintf(f, "              charge = %ld/%ld,\n",
                  IntegerValue(CompoundArg1(v)), IntegerValue(CompoundArg2(v)));
  ...
  for (l1 = l; l1; l1 = ListTail(l1)) {
    ...
    */
...

    {
      ...
      for (l3 = CompoundArg1(ListFirst(l2)), i = 0; l3;
           l3 = ListTail(l3), i++) {
        Atom a = ListFirst(l3);
        if (a == OPR_SCALAR) {
          cbuf[i] = 'S';
        ...
        lv2[i] = AppendLast(lv2[i], ListFirst(l2)), hasz = 2;
      {
        for (l3 = lc; l3; l3 = ListTail(l3))
          if (EqualTerms(ListFirst(l3), CompoundArgN(ListFirst(l2), 3)))
            break;
        if (!l3)
          lc = AppendLast(lc, CompoundArgN(ListFirst(l2), 3));
      }
    }
    ...
    for (i = 0; i < lvl; i++)
      for (l2 = lv[i]; l2; l2 = ListTail(l2)) {
        ...
        if (ListLength(CompoundArg2(ListFirst(l2))) != 1)
          ...
          sscanf(
              AtomValue(CompoundArg1(ListFirst(CompoundArg2(ListFirst(l2))))) +
                  1,
              "%d", &no);
  ...
  for (l = ten; l; l = ListTail(l)) {
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l)), A_COLOR) ==
      ...
      if (il2 >= 5 && l2 && ListFirst(CompoundArg2(ListFirst(l2))) == in2) {
        ...
        sno += fprintf(of, "T(%d,%d,-1)*T(%d,-1,%d)", il3 + 1, il1 + 1, il4 + 1,
                       il2 + 1);
        if (sno > 60 && ListTail(l) &&
      ...
      if (il3 >= 5 && l2 && ListNth(CompoundArg2(ListFirst(l2)), 3) == in3) {
        ...
        sno += fprintf(of, "T(-1,%d,%d)*T(-1,%d,%d)", il1 + 1, il2 + 1, il3 + 1,
                       il4 + 1);
        if (sno > 60 && ListTail(l) &&
            (ListTail(l) != l2 || ListTail(ListTail(l)))) {
          fprintf(of, "*\n\t\t");
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l)), A_COLOR) == A_COLOR_F) {
      ...
      if (l2) {
        ...
        sno += fprintf(of, "f(%d,%d,-1)*f(%d,%d,-1)", il1 + 1, il2 + 1, il3 + 1,
                       il4 + 1);
        if (sno > 60 && ListTail(l)) {
          fprintf(of, "*\n\t\t");
          sno = 15;
      ...
      if (l2) {
        ...
        sno += fprintf(of, "f(%d,%d,-1)*d(%d,%d,-1)", il1 + 1, il2 + 1, il3 + 1,
                       il4 + 1);
        if (sno > 60 && ListTail(l)) {
          fprintf(of, "*\n\t\t");
          sno = 15;
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l)), A_COLOR) == A_COLOR_D) {
      ...
      if (l2) {
        ...
        sno += fprintf(of, "d(%d,%d,-1)*f(%d,%d,-1)", il1 + 1, il2 + 1, il3 + 1,
                       il4 + 1);
        if (sno > 60 && ListTail(l)) {
          fprintf(of, "*\n\t\t");
          sno = 15;
      ...
      if (l2) {
        ...
        sno += fprintf(of, "d(%d,%d,-1)*d(%d,%d,-1)", il1 + 1, il2 + 1, il3 + 1,
                       il4 + 1);
        if (sno > 60 && ListTail(l)) {
          fprintf(of, "*\n\t\t");
          sno = 15;
...
void UF_write_gen(void) {
  ...
  for (l1 = gkl; l1; l1 = ListTail(l1)) {
    ...
    for (l2 = CompoundArg1(ListFirst(l1)), i = 0; l2; l2 = ListTail(l2), i++) {
      ...
      if (a == OPR_SCALAR) {
        cbuf[i] = 'S';
        spl = AppendLast(spl, NewInteger(1));
      } else if (a == OPR_SPINOR) {
        cbuf[i] = 'F';
        spl = AppendLast(spl, NewInteger(2));
      } else if (a == OPR_VECTOR) {
        cbuf[i] = 'V';
        spl = AppendLast(spl, NewInteger(3));
      } else if (a == A_GRASS) {
        cbuf[i] = 'U';
        spl = AppendLast(spl, NewInteger(-1));
      } else {
        ...
        if (CompoundArity(ListFirst(l3)) == 4) {
          ...
          for (l4 = 1; l4 <= 4; l4++) {
            if (AtomValue(CompoundArgN(ListFirst(l3), l4))[0] == 'l')
              ...
              lp = AppendLast(
                  lp, MakeCompound2(OPR_DIV, NewInteger(-lin), NewInteger(l5)));
              lin++;
            }
            if (l4 < 4)
              fprintf(f, ",");
          ...
          fprintf(f, " P(%d,%s)*P(%d,%s)", -lin, AtomValue(a1) + 3, -lin,
                  AtomValue(a2) + 3);
          lin++;
        } else {
        ...
        for (l3 = nc; l3; l3 = ListTail(l3)) {
          if (ListFirst(l3) == A_GAMMAP)
            ...
            if (AtomValue(a)[0] == 'l')
              ...
              fprintf(f, "P(%d,%d)*Gamma(%s,", -lin, -lin, AtomValue(a) + 3);
              lin++;
            } else {
              puts("Internal error wrtuf03");
            }
          } else {
            puts("Internal error wrtuf04");
          }
          if (su == 0)
            fprintf(f, "1,");
          else {
