  ...
  case 'c': {
    ...
    if (arity == 2) {
      ...
      if (GetOperator(oper, OP_INFIX, &assoc, &prior) != 0) {
        ...
        if (assoc == OP_XFX || assoc == OP_XFY)
          prior1++;
        if (assoc == OP_XFX || assoc == OP_YFX)
          prior2++;
        if (prior1 > prior || (AlwaysBracets && prior1 > 1))
          retval += printf("(");
        retval += WriteTerm(CompoundArg1(T));
    ...
    for (arg = 1; arg <= arity; arg++) {
      ...
      if (iarg && ListMember(iarg, NewInteger(arg))) {
        if (is_integer(CompoundArgN(T, arg)))
          retval += sprintf(string + retval, "%ld",
                            IntegerValue(CompoundArgN(T, arg)));
        else {
...
static void dump_term(Term t, int marg) {
  ...
  case 'f':
    ...
    return;
  case 'u':
    printf("%s/%d\n", AtomValue(FunctorName(t)), FunctorArity(t));
    return;
  case 'e':
    printf("_\n");
    return;
