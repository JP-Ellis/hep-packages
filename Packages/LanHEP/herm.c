...
static int ghost_factor(List *prt) {
  ...
  do {
    ...
    while (!is_empty_list(l2)) {
      ...
      a1 = ListFirst(l1);
      a2 = ListFirst(l2);
      n1 = AtomValue(CompoundArg1(a1));
      while (n1[1])
        n1++;
      n2 = AtomValue(CompoundArg1(a2));
      while (n2[1])
        n2++;
      if (n1[0] == 'c' && n2[0] == 'C')
        spr = 1;
        ...
        spr = strcmp(AtomValue(CompoundArg1(a1)), AtomValue(CompoundArg1(a2)));
      if (spr == 0)
        return 0;
      if (spr > 0) {
        ChangeList(l1, a2);
    ...
    for (l = CompoundArgN(ListFirst(ml), 3); l; l = ListTail(l)) {
      if (CompoundName(ListFirst(l)) == A_MOMENT)
        SetCompoundArg(ListFirst(l), 1,
                       NewInteger(ListMember(ord, CompoundArg1(ListFirst(l)))));
    }
  }
}
...
static void fix_spin(List ord, List prt, List ml) {
  int cc_done;
  if (CompoundName(CompoundArg2(ListFirst(prt))) != OPR_SPINOR &&
      CompoundName(CompoundArg2(ListFirst(prt))) != OPR_SPINOR3)
    return;
  ...
  for (l = ml; l; l = ListTail(l)) {
    ...
    for (l1 = CompoundArg2(ListFirst(l)); l1; l1 = ListTail(l1))
      if ((an = GetAtomProperty(CompoundArg1(ListFirst(l1)), A_ANTI))) {
        SetCompoundArg(ListFirst(l1), 1, an);
        cprm = 1;
      }
...
static void compare_vertices(Term a2, Term a2cg, Term a2cf) {
  ...
  */
  ...
  printf("                        ");
  WriteVertex(CompoundArg1(a2cf));
  puts("\n");

  /*	DumpList(CompoundArgN(a2cf,5));
          DumpList(CompoundArgN(a2cg,5));
  */
  for (l1 = o, l2 = CompoundArgN(a2, 5); l1;
...
Term ProcHermiticity(Term arg, Term ind) {
  ...
  if (is_compound(arg)) {
    ...
    for (lagr = all_lagr; lagr; lagr = ListTail(lagr)) {
      ...
      for (ml = CompoundArgN(ListFirst(lagr), 5); ml; ml = ListTail(ml)) {
        ...
        SetCompoundArg(ListFirst(ml), 1,
                       MakeCompound2(OPR_DIV, ii, NewInteger(1)));
      }
    }
  } else
  ...
  for (lagr = all_lagr; lagr; lagr = ListTail(lagr)) {
    ...
    for (l = prt; l; l = ListTail(l)) {
      ...
      if (CompoundName(pr) == OPR_PARTICLE && CompoundArgN(pr, 7) == OPR_MLT)
        cf = 1;
      if (CompoundName(pr) == OPR_FIELD && CompoundArg2(pr) == NewInteger(5))
        cf = 1;
    }
    ...
    {
      List p1, p2;
      for (p1 = CompoundArg1(a2), p2 = CompoundArg1(a2c); p1 && p2;
           p1 = ListTail(p1), p2 = ListTail(p2))
        if (CompoundArg1(ListFirst(p1)) != CompoundArg1(ListFirst(p2)))
          break;
      if (p1 == 0) {
        FreeAtomic(a2c);
        FreeAtomic(ord);
