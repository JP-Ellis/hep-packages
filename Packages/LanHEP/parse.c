...
static Term read_term(Atomic *stop) {
  ...
  while (1) {
    ...
    } else /* waiting for end/xfx/xf */
    {
      ...
      sprintf(parse_err_mess, "operator expected %s <HERE> %s.", AtomValue(l_a),
              AtomValue(c_a));
  ...
  */
  ...
  while (l1 != 0) {
    if (IntegerValue(CompoundArgN(ListFirst(l1), 3)) == minprior) {
      ...
      oper_assoc = CompoundArgN(ListFirst(l1), 4);
      lop = l1;
      if (oper_assoc == OP_XFY || oper_assoc == OP_XFX || oper_assoc == OP_FX ||
          oper_assoc == OP_FY)
        break;
    }
  ...
  if (err != 0) {
    ...
    if (IsTermInput() == 0) {
      if (fline == lline)
        ...
        printf("File \"%s\", lines %d - %d : ", CurrentInputFile(), fline,
               lline);
    }
