...
static int twl_1(FILE *f, Term a2) {
  ...
  while (!is_empty_list(l2)) {
    ...
    while (!is_empty_list(l)) {
      ...
      if (ii > ino) {
        ...
        puts("");
        WriteTerm(CompoundArgN(a2, 4));
        puts("");
        WriteTerm(ListFirst(CompoundArgN(a2, 5)));
        puts("");
        WriteTerm(ListFirst(ListTail(CompoundArgN(a2, 5))));
        puts("");
        goto aaa;
      }
      if (ii == ino) {
  ...
  while (!is_empty_list(l2)) {
    ...
    if (CompoundName(sp) == A_MOMENT) {
      fprintf(
          f, "p_%ld^%s ", IntegerValue(CompoundArg1(sp)),
    ...
    if (CompoundArg1(sp) == A_DELTA) {
      ...
      if (ListMember(vind, NewInteger(n1)) && ListMember(vind, NewInteger(n2)))
        fprintf(f, "g^{%s %s} ", AtomValue(ListNth(ind, n1)),
                AtomValue(ListNth(ind, n2)));
      else
        fprintf(f, "\\delta_{%s %s} ", AtomValue(ListNth(ind, n1)),
                AtomValue(ListNth(ind, n2)));
      silcnt += 2;
      goto cnt8;
    }
  ...
  s1:
    ...
    while (!is_empty_list(l2)) {
      ...
      if (CompoundName(sp) == A_MOMENT) {
        fprintf(f, "p_%ld^%s ", IntegerValue(CompoundArg1(sp)),
                AtomValue(
                    ListNth(ind1, IntegerValue(ListFirst(CompoundArg2(sp))))));
        silcnt++;
        goto cnt6;
      }
      if (CompoundArg1(sp) == A_GAMMA) {
        fprintf(f, "\\gamma_{%s %s}^%s ",
                AtomValue(
                    ListNth(ind1, IntegerValue(ListNth(CompoundArg2(sp), 1)))),
                AtomValue(
                    ListNth(ind1, IntegerValue(ListNth(CompoundArg2(sp), 2)))),
                AtomValue(
                    ListNth(ind1, IntegerValue(ListNth(CompoundArg2(sp), 3)))));
        silcnt += 2;
        goto cnt6;
      }
      if (CompoundArg1(sp) == A_GAMMA5) {
      ...
      if (CompoundArg1(sp) == A_DELTA) {
        ...
        n1 = IntegerValue(ListNth(CompoundArg2(sp), 1));
        n2 = IntegerValue(ListNth(CompoundArg2(sp), 2));
        if (ListMember(vind1, NewInteger(n1)) &&
            ListMember(vind1, NewInteger(n2)))
          fprintf(f, "g^{%s %s} ", AtomValue(ListNth(ind1, n1)),
                  AtomValue(ListNth(ind1, n2)));
        else
          fprintf(f, "\\delta_{%s %s} ", AtomValue(ListNth(ind1, n1)),
                  AtomValue(ListNth(ind1, n2)));
        silcnt += 2;
        goto cnt6;
      }
      if (CompoundArg1(sp) == A_EPS_V) {
        fprintf(f, "\\varepsilon_{%s %s %s %s} ",
                ...
                AtomValue(
                    ListNth(ind1, IntegerValue(ListNth(CompoundArg2(sp), 3)))),
                AtomValue(
                    ListNth(ind1, IntegerValue(ListNth(CompoundArg2(sp), 4)))));
        silcnt += 3;
        goto cnt6;
      }
      ...
      }
      {
        ...
        for (iii = 1; iii <= ListLength(CompoundArg2(sp)); iii++)
          fprintf(f, "%s ",
                  AtomValue(ListNth(
                      ind1, IntegerValue(ListNth(CompoundArg2(sp), iii)))));
    ...
    if (silcnt >= TEX_spec_in_line && ListTail(l1) &&
      ...
      fprintf(f,
              "Variational derivative of Lagrangian by fields \\\\ \\hline\n");
      fprintf(f, " & $");
    }
    if (silcnt >= TEX_spec_in_line && ListTail(l1)) {
      silcnt = 0;
...
void tex_write_lagr(List l, FILE *f) {
  ...
  while (!is_empty_list(l1)) {
    ...
    if ((ListLength(CompoundArg1(a2)) > 2 || write_all_vertices) &&
        ListLength(CompoundArg1(a2)) <= TEX_max_pno && CompoundArgN(a2, 5)) {
...
void tex_wrt_2vrt(FILE *f, Term a2) {
  ...
  cnt1:
    ...
    while(!is_empty_list(l1))
            {
            fprintf(f,"%s
    ",AtomValue(ListNth(ind,IntegerValue(ListFirst(l1))))); l1=ListTail(l1);
            }
    fprintf(f,"}$ & ");*/
    l = ListTail(l);
  }
    ...
    while (!is_empty_list(l2)) {
      ...
      if (CompoundArg1(sp) == A_EPS_V) {
        fprintf(
            ...
            AtomValue(ListNth(ind, IntegerValue(ListNth(CompoundArg2(sp), 3)))),
            AtomValue(
                ListNth(ind, IntegerValue(ListNth(CompoundArg2(sp), 4)))));
        goto cnt6;
      }
