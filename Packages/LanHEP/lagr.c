...
static void plt_2(Term t, List nind) {
  ...
  sprintf(regbuf, "ProcLTerm: alg2_add, %d terms", (int)ListLength(t1));
  RegisterLine(regbuf);
...
void FAWriteLagr(int fno) {
  ...
  if (UFOutput)
    printf("\nStatic files for UFO model can be found in mdl/ufo-static/ of "
           "LanHEP source "
           "direcory\n\n");
  FreeAtomic(l);
      ...
      for (l2 = pl; l2; l2 = ListTail(l2))
        if (CompoundArg1(ListFirst(l2)) == CompoundArg1(ListFirst(l1))) {
          if (CompoundArg2(ListFirst(l1)) == CompoundArg2(ListFirst(l2)))
            ...
            SetCompoundArg(
                ListFirst(l2), 2,
                NewInteger(IntegerValue(CompoundArg2(ListFirst(l2))) -
    ...
    */
    ...
    if (sim_te) {
      ...
      for (i1 = sim_te; i1; i1 = ListTail(i1)) {
        ...
        SetCompoundArg(m2, 1, NewInteger(c_n_f));
        SetCompoundArg(
            m2, 2, AppendFirst(c_s_f, ConsumeCompoundArg(ListFirst(i2), 1)));
    ...
    for (lm = CompoundArgN(a2, 5); lm; lm = ListTail(lm)) {
      ...
      for (l3 = CompoundArgN(m2, 3); l3; l3 = ListTail(l3)) {
        ...
        if (l4)
          SetCompoundArg(
              ListFirst(l4), 2,
              NewInteger(IntegerValue(CompoundArg2(ListFirst(l4))) + 1));
        else
          l2 = AppendLast(l2,
...
static int chk_sel(List sel, List prt, int typ, int afl) {
  ...
  while (!is_empty_list(l)) {
    ...
    if ((CompoundName(prp) == OPR_PARTICLE &&
        ...
        (CompoundName(prp) == OPR_FIELD && CompoundArg2(prp) == NewInteger(5)))
      cprp = 1;
