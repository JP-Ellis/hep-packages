...
typedef signed int Atom, Integer, Float, Atomic, Functor, Term, Compound, List,
    Label;
...
#define LIST_pointer(l)                                                        \
  (LISTS_buffers[(l & 0xfff0000) / 0x10000] + (l & 0xffff))
#define ListFirst(l) (LIST_pointer(l)->d.value)
#define ListTail(l) (LIST_pointer(l)->d.nextl)
#define ListConcat(l1, l2)                                                     \
  (LIST_pointer(l1)->d.nextl = l2, LIST_pointer(l1)->d.nextp = LIST_pointer(l2))
...
#define CompoundFunctor(c)                                                     \
  ((COMPO_buffers[(c & 0xfff0000) / 0x10000] + (c & 0xffff))->data.a[0])
#define CompoundArg1(c)                                                        \
  ((COMPO_buffers[(c & 0xfff0000) / 0x10000] + (c & 0xffff))->data.a[1])
#define CompoundArg2(c)                                                        \
  ((COMPO_buffers[(c & 0xfff0000) / 0x10000] + (c & 0xffff))->data.a[2])
...
int is_atomic(Term), is_compound(Term), is_functor(Atomic), is_atom(Atomic),
    is_float(Atomic), is_integer(Atomic);
