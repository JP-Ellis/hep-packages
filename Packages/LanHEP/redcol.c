...
static void split_m2(Term m, Term *m1, Term *m2, int *anti, int spin) {
  ...
  *m2 = MakeCompound(A_MTERM, 3);
  ...
  while (!is_empty_list(l)) {
    ...
    if (i1)
      co1 = AppendLast(co1,
                       MakeCompound2(OPR_POW, CompoundArg1(t), NewInteger(i1)));
    if (i2)
      co2 = AppendLast(co2,
                       MakeCompound2(OPR_POW, CompoundArg1(t), NewInteger(i2)));
    l = ListTail(l);
  }
  ...
  SetCompoundArg(*m1, 1,
                 MakeCompound2(OPR_DIV, NewInteger(n1), NewInteger(d1)));
  SetCompoundArg(*m2, 1,
                 MakeCompound2(OPR_DIV, NewInteger(n2), NewInteger(d2)));
}

static int check_charge(List pl1, List prt1, List osp1, Term cs1, List pl2,
                        List prt2, List osp2, Term cs2, List fi) {
  List l, l1;
  if (osp1 || osp2) {
...
static List color_reduce_2(Term a2, List pl, List pil, List csp, List osp) {
  ...
  */
  ...
  if ((i1 == A_COLOR_LAMBDA && co1 != 3) ||
      (i2 == A_COLOR_LAMBDA && co2 != 3)) {
    printf("Internal error in vertex ");
    WriteVertex(CompoundArg1(a2));
    printf(" (rc24).\n");
    return AppendFirst(NewList(), a2);
...
cnt4:
  ...
  */
  ...
  if (opNeutrC8 == 0 && ctype == 3 && anti) {
    warnc8++;
    if (warnc8 == 5)
      puts("More problematic vertices follow...");
    if (warnc8 < 5) {
      printf("Warning: ");
      WriteVertex(CompoundArg1(a2));
  ...
  l2 = mk_im_field(ctype, ListLength(i1) - 1,
                   (ctype == 3 && opNeutrC8) ? 0 : anti, prt);
  ...
  case 0:
    co2 = MakeCompound2(OPR_SCALAR, co1, CopyTerm(i1));
    break;
  case 1:
    co2 = MakeCompound2(OPR_VECTOR, co1, CopyTerm(i1));
    break;
  case 2:
    co2 = MakeCompound2(OPR_TENSOR, co1, CopyTerm(i1));
    break;
  }
  ...
  switch (ListLength(i1) - 1) {
  case 0:
    co2 = MakeCompound2(OPR_SCALAR, co1, i1);
    break;
  case 1:
    co2 = MakeCompound2(OPR_VECTOR, co1, i1);
    break;
  case 2:
    co2 = MakeCompound2(OPR_TENSOR, co1, i1);
    break;
  }
...
static List color_reduce_0(Term a2, List pl, List pil, List osp) {
  ...
  if (ListLength(i1) != ListLength(i2) || ListLength(i1) > 2) {
    printf("Internal error in vertex ");
    WriteVertex(CompoundArg1(a2));
    printf(" (rc04).\n");
    return AppendFirst(NewList(), a2);
  ...
  case 0:
    co2 = MakeCompound2(OPR_SCALAR, co1, NewList());
    break;
  case 1:
    co2 = MakeCompound2(OPR_VECTOR, co1, CopyTerm(i1));
    break;
...
List color_reduce(Term a2) {
  ...
  while (!is_empty_list(l1)) {
    ...
    if (CompoundName(t1) == OPR_SPECIAL &&
        GetAtomProperty(CompoundArg1(t1), A_COLOR) != 0) {
