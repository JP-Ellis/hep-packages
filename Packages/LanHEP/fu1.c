...
static void set_keys(Term k) {
  ...
  for (k1 = k; k1; k1 = ListTail(k1)) {
    ...
    List l;
    k2 = ListFirst(k1);
    if (!is_compound(k2) || CompoundArity(k2) != 2 ||
        CompoundName(k2) != OPR_EQSIGN) {
      ErrorInfo(206);
...
Term ProcessModel(Term t, Term ind) {
  ...
  if (is_compound(arg) && FunctorName(CompoundFunctor(arg)) == OPR_DIV &&
      FunctorArity(CompoundFunctor(arg)) == 2 && is_atom(CompoundArg1(arg)) &&
...
static void p_clear(Term t1) {
  ...
  if (is_compound(t1) && CompoundName(t1) == OPR_MINUS &&
    ...
    if (f) {
      ...
      printf("clear ");
      WriteTerm(t1);
      puts(": second particle not found.");
      return;
    }
    for (l = lc; l; l = ListTail(l))
      ClearParameter(ListFirst(l));
    FreeAtomic(lc);
    FreeAtomic(t1);
    return;
  }
...
Term ProcOption(Term t, Term i) {
  ...
  if (strcmp(s, "SmartAngleComb") == 0) {
    if (!is_integer(val) || IntegerValue(val) < 0 || IntegerValue(val) > 4) {
      ErrorInfo(415);
