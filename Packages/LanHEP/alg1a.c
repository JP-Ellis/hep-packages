...
static List check_list(List l) {
  ...
  for (l2 = ListFirst(l); l2; l2 = ListTail(l2)) {
    ...
    if (is_list(CompoundArg2(t))) {
      if (ll == 0)
        ...
        ErrorInfo(0);
        puts("where: list length is different.");
        return l;
      }
    }
  }
  ...
  for (i = 1; i <= ll; i++) {
    ...
    for (l2 = ListFirst(l); l2; l2 = ListTail(l2)) {
      Term t, t2;
      t = ListFirst(l2);
      t2 = MakeCompound2(OPR_EQSIGN, CompoundArg1(t),
                         is_list(CompoundArg2(t)) ? ListNth(CompoundArg2(t), i)
                                                  : CompoundArg2(t));
...
void proc_hash(Term t) {
  if (is_compound(t) &&
      (CompoundName(t) == OPR_PARAMETER || CompoundName(t) == OPR_LET) &&
      has_where(t))
    return;
...
Term namely_w_t(Term t) {
  ...
  while (!is_empty_list(l2)) {
    ...
    while (!is_empty_list(ll)) {
      lai = AppendLast(
          lai, InterfSetAlias(MakeCompound1(OPR_ALIAS, ListFirst(ll)), 0));
      ll = ListTail(ll);
    }
