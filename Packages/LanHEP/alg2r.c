...
static Atom mk_eps(Atom p1, Atom p2, Atom p3, Atom p4) {
  sprintf(mb_buf, "eps(%s,%s,%s,%s)", AtomValue(p1), AtomValue(p2),
          AtomValue(p3), AtomValue(p4));
...
static int corr_mpg(Term m2) {
  ...
  for (l1 = CompoundArgN(m2, 3); l1; l1 = ListTail(l1)) {
    ...
    if (a[0] == 'm' || a[0] == 'M' || a[0] == 'p') {
      char buf[28];
      sprintf(buf, "(G(%2.2s)*G(%2.2s)+G(%2.2s)*G(%2.2s))", a, a + 3, a + 3, a);
...
static void m_reduce(Term m2, List pl, int pis, int fr) {
  ...
  /*
     ...
     IntegerValue(ferm_ind[0]), IntegerValue(ferm_ind[1]));
  */
  ...
  while (!is_empty_list(l)) {
    ...
    if (CompoundName(t) == A_MOMENT) {
      ...
      while (!is_empty_list(l2)) {
        ...
        if (CompoundName(t1) == OPR_VECTOR &&
            ListFirst(CompoundArg1(t1)) == mi) {
          l1 =
              AppendLast(l1, mk_cnvl(1, IntegerValue(CompoundArg1(t)), 0, ppp));
          goto cnt;
        }
        if (CompoundName(t1) == OPR_TENSOR &&
            ListFirst(CompoundArg1(t1)) == mi) {
          l1 =
              AppendLast(l1, mk_cnvl(1, IntegerValue(CompoundArg1(t)), 0, ppp));
          goto cnt;
        ...
        if (CompoundName(t1) == OPR_TENSOR &&
            ListNth(CompoundArg1(t1), 2) == mi) {
          l1 =
              AppendLast(l1, mk_cnvl(1, IntegerValue(CompoundArg1(t)), 2, ppp));
          goto cnt;
        }
        if (CompoundName(t1) == OPR_SPINOR3 &&
            ListNth(CompoundArg1(t1), 2) == mi) {
          l1 =
              AppendLast(l1, mk_cnvl(1, IntegerValue(CompoundArg1(t)), 0, ppp));
          goto cnt;
      ...
      while (!is_empty_list(l2)) {
        Term t1;
        t1 = ListFirst(l2);
        if (CompoundName(t1) == A_MOMENT && ListFirst(CompoundArg2(t1)) == mi) {
          l1 = AppendLast(l1, mk_cnvl(1, IntegerValue(CompoundArg1(t)), 1,
    ...
    if (CompoundName(t) == OPR_SPECIAL && CompoundArg1(t) == A_EPS_V) {
      ...
      il = CompoundArg2(t);
      l1 = AppendLast(l1, mk_eps(ListNth(il, 1), ListNth(il, 2), ListNth(il, 3),
                                 ListNth(il, 4)));
      goto cnt;
    }
    ...
    if (CompoundName(t) == OPR_PARAMETER) {
      ...
      pl = AppendLast(pl,
                      MakeCompound2(OPR_POW, CompoundArg1(t), NewInteger(1)));
