...
int ProcClass(Term cl, Term ind) {
  ...
  if (!is_compound(t) || CompoundArity(t) != 2 ||
      CompoundName(t) != OPR_EQSIGN || !is_atom(CompoundArg1(t)) ||
      !is_list(CompoundArg2(t)) || CompoundArg2(t) == 0) {
    ErrorInfo(325);
  ...
  t = AppendLast(
      CopyTerm(t),
      MakeCompound2(A_I,
                    MakeCompound2(OPR_WILD, NewInteger(ListLength(clmem)),
                                  NewInteger(ListLength(clmem))),
                    0));
  ...
  SetAtomProperty(clnm, PROP_TYPE,
                  MakeCompound1(OPR_CLASS, NewInteger(cls_no)));
  ...
  for (l1 = clmem, i = 1; l1; l1 = ListTail(l1), i++) {
    ...
    SetAtomProperty(
        ListFirst(l1), OPR_CLASS,
        MakeCompound2(OPR_CLASS, NewInteger(cls_no), NewInteger(i)));
  }

  t = GetAtomProperty(ListFirst(clmem), A_ANTI2);
  if (t)
    t = GetAtomProperty(t, A_ANTI);
  if (t == 0)
    t = GetAtomProperty(ListFirst(clmem), A_ANTI);
  for (i = 1; i < cls_no; i++)
    if (ListFirst(cls_membs[i]) == t) {
      ...
      for (l1 = clmem, l2 = cls_membs[i]; l1;
           l1 = ListTail(l1), l2 = ListTail(l2)) {
        Term ta2;
        ta2 = GetAtomProperty(ListFirst(l1), A_ANTI2);
        if (ta2)
          ta2 = GetAtomProperty(ta2, A_ANTI);
        if (ta2 == 0)
...
void cls_set_em(void) {
  ...
  for (i = 1; i < cls_no; i++) {
    ...
    for (l = ListTail(cls_membs[i]); l; l = ListTail(l))
      if (!EqualTerms(GetAtomProperty(ListFirst(cls_membs[i]), A_EM_CHARGE),
                      ch)) {
...
void cls_wrt_nms(FILE *f, List cpml) {
  ...
  for (i = 1; i < cls_no; i++) {
    ...
    if (CompoundArg1(fp) != CompoundArg2(fp) &&
        CompoundArg2(fp) == cls_names[i])
      continue;
    ...
    for (j = 1, l1 = cls_membs[i]; l1; l1 = ListTail(l1), j++) {
      ...
      fprintf(f, "%s[%d] = %s;\n", AtomValue(ms), j,
              CompoundArgN(prp, 5) ? AtomValue(CompoundArgN(prp, 5)) : "0");
      mmatr = AppendLast(
          mmatr,
          MakeCompound2(
              OPR_COLON, AppendFirst(0, NewInteger(j)),
              AppendFirst(
                  0, MakeCompound3(
                         A_MTERM, NewInteger(1),
                         AppendFirst(0, MakeCompound2(OPR_POW,
                                                      CompoundArgN(prp, 5)
                                                          ? CompoundArgN(prp, 5)
                                                          : NewAtom("0", 0),
                                                      NewInteger(1))),
    ...
    SetAtomProperty(ms, OPR_CLASS,
                    AppendFirst(0, NewInteger(ListLength(cls_membs[i]))));
    if (mtr_visi)
...
static void add_elem(List *el, List prt, List ml) {
  ...
  for (l1 = ml; l1; l1 = ListTail(l1)) {
    ...
    if (l2 == 0)
      lt =
          AppendLast(lt, MakeCompound2(OPR_COLON, CopyTerm(pl), MakeList1(m2)));
    else
...
static Term mka(List bl, List el, List cl, List il) {
  ...
  for (l = il; l; l = ListTail(l))
    p += sprintf(cbuf + p, (el || ListTail(l)) ? "%ld," : "%ld)",
                 IntegerValue(ListFirst(l)));
  for (l = el; l; l = ListTail(l))
    p += sprintf(cbuf + p, ListTail(l) ? "%ld," : "%ld)",
                 IntegerValue(ListFirst(l)));
  if (spec_symm == 1)
    p += sprintf(cbuf + p, ")");
  l = CopyTerm(bl);
  ...
  m = MakeCompound3(A_MTERM, NewInteger(1),
                    MakeList1(MakeCompound2(OPR_POW, a, NewInteger(1))), 0);
...
static void symm_mtr(List *mtr, List cls) {
  ...
  i2 = 0;
  for (i = 1; i <= n - 1; i++)
    if (ListNth(cls, i) == ListNth(cls, i + 1))
      i2++;
  if (i2 == 0)
    return;
  ...
  for (l1 = cls, i1 = 1; ListTail(l1); l1 = ListTail(l1), i1++) {
    ...
    for (l2 = *mtr; l2; l2 = ListTail(l2)) {
      ...
      if (spec_symm == 2 &&
          IntegerValue(ListFirst(il)) > IntegerValue(ListNth(il, 2))) {
        List l4;
        for (l4 = CompoundArg2(ListFirst(l2)); l4; l4 = ListTail(l4))
          SetCompoundArg(
              ListFirst(l4), 1,
              NewInteger(-IntegerValue(CompoundArg1(ListFirst(l4)))));
      }
  ...
  *sl = ConsumeCompoundArg(a2, 3);
  ...
  for (l1 = mtr_known; l1; l1 = ListTail(l1))
    if (EqualTerms(CompoundArg2(ListFirst(l1)), mtr)) {
      if (EqualTerms(GetAtomProperty(CompoundArg1(ListFirst(l1)), OPR_CLASS),
                     mdim))
        break;
    }
  if (l1) {
  ...
  for (l = CompoundArgN(a2, 5); l; l = ListTail(l)) {
    ...
    if (ga) {
      ...
      if (g5 == 0 || g5t != A_GAMMA5)
        SetCompoundArg(CompoundArg1(ListFirst(l)), 1,
                       NewInteger(-IntegerValue(
                           CompoundArg1(CompoundArg1(ListFirst(l))))));
      if (g5t == A_GAMMAP)
...
void setcls_1(Term a2, List a2l) {
  ...
  for (l1 = CompoundArg1(a2), i = 0; l1; l1 = ListTail(l1), i++) {
    ...
    if (prp) {
      ...
      matr_dim = AppendLast(
          matr_dim,
          NewInteger(ListLength(cls_membs[IntegerValue(CompoundArg1(prp))])));
...

  {
    ...
    if (cf == 4 && CompoundName(CompoundArg2(ListNth(pl, 1))) == OPR_SCALAR &&
      ...
      if (is_col[0] && is_col[1] && is_col[2] && is_col[3])
        sym2 = 1;
               ...
               prts[0] != prts[2] && prts[0] != prts[3] && prts[1] != prts[2] &&
               prts[1] != prts[3]) {
        sym2 = 1;
        slp = 1;
      }
...

  {
    ...
    if ((cf == 2 || cf == 3) && (pno == 2 || pno == 3) &&
        ...
        CompoundName(CompoundArg2(ListNth(pl, 2))) == OPR_SPINOR && is_cls[0] &&
        is_cls[1] && prts[0] == prts[1]) {
      sym2 = 2;
  ...
  for (l1 = CompoundArg1(a2), i = 0; l1; l1 = ListTail(l1), i++) {
    ...
    for (l2 = CompoundArg1(CompoundArg2(ListFirst(l1))); l2;
         l2 = ListTail(l2)) {
  ...
  for (l1 = elli; l1; l1 = ListTail(l1)) {
    ...
    if (ListLength(CompoundArg1(a2)) == 3) {
      ...
      if (CompoundName(CompoundArg2(ListNth(pl, 1))) == OPR_SCALAR &&
        ...
        spec_symm = 0;
        List tl1 = tl;
        if (ListLength(tl1) == 2 && (CompoundName(ListFirst(tl1)) == OPR_POW ||
                                     CompoundName(ListFirst(tl1)) == OPR_CLASS))
          tl1 = ListTail(tl1);
        if (ListLength(tl1) != 1 || CompoundName(ListFirst(tl1)) != A_MOMENT) {
    ...
    if (opclsred) {
      for (i = 1; i < ListLength(matr_cls); i++) {
        if (!sym2 &&
            ((i > 1 && ListNth(matr_cls, i - 1) == ListNth(matr_cls, i)) ||
        ...
        for (j = i + 1; j <= ListLength(matr_cls); j++) {
          ...
          if (!sym2 && (ListNth(matr_cls, j - 1) == ListNth(matr_cls, j) ||
                        (j < ListLength(matr_cls) &&
                         ListNth(matr_cls, j) == ListNth(matr_cls, j + 1))))
            continue;
      ...
      if (i2) {
        mtr = MakeCompound2(OPR_SPECIAL, A_DELTA,
                            MakeList2(ListNth(l2, i2), ListNth(l2, j2)));
      ...
      if (i1) {
        mtr = MakeCompound2(OPR_SPECIAL, A_DELTA,
                            MakeList2(ListNth(l2, i1), ListNth(l2, j1)));
        tl = AppendLast(tl, mtr);
        for (l3 = CompoundArg2(ListFirst(l1)); l3; l3 = ListTail(l3)) {
          List il = ConsumeCompoundArg(ListFirst(l3), 1);
    ...
    mtr = mk_mtr(ConsumeCompoundArg(ListFirst(l1), 2), &nc, &sl, CopyTerm(mds),
                 mcs);

    if (mtr == A_DELTA && ListLength(l2s) == 1) {
      if (!i1) {
        WarningInfo(0);
        puts("cls 1 dim delta");
        mtr = MakeCompound2(OPR_PARAMETER, mtr,
                            MakeList2(ListFirst(l2s), ListFirst(l2s)));
      } else
        mtr = 0;
    } else
...
List cls_real_matr(void) {
  ...
  for (l = mtr_known; l; l = ListTail(l)) {
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l)), A_INFINITESIMAL))
      continue;
    if (!hasi)
      ret = AppendLast(ret, CompoundArg1(ListFirst(l)));
  }
  return ret;
}
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l1)), A_INFINITESIMAL) == 0 &&
      ...
      for (l2 = CompoundArg2(ListFirst(l1)); l2; l2 = ListTail(l2)) {
        ...
        for (l3 = CompoundArg1(ListFirst(l2)); l3; l3 = ListTail(l3))
          p += fprintf(f, "%ld%c", IntegerValue(ListFirst(l3)),
                       ListTail(l3) ? ',' : ')');
        p += fprintf(f, "=");

        for (l3 = CopyTerm(CompoundArg2(ListFirst(l2))); l3;
             l3 = ListTail(l3)) {
          Term t3;
          List l4;
          ...
          for (l4 = CompoundArg2(ListFirst(l3)); l4; l4 = ListTail(l4)) {
            ...
            if (CompoundArg1(pp) == A_I)
              SetCompoundArg(pp, 1, fa_i);
...
static void wrt_expr(FILE *of, Term num, List sym, List ten) {
  ...
  NoQuotes = 1;
  if (is_integer(num) ||
      (is_compound(num) && IntegerValue(CompoundArg2(num)) == 1)) {
    int n = IntegerValue(is_integer(num) ? num : CompoundArg1(num));
    if (n == -1 && ((sym && IntegerValue(CompoundArg2(ListFirst(sym))) > 0) ||
                    (ten && (!sym))))
      sno += fprintf(of, "- "), f = 0;
