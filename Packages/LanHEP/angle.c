...
static List add_fct(List m_l, Term p, Term m2) {
  ...
  while (!is_empty_list(l1)) {
    ...
    if (EqualTerms(CompoundArg1(ListFirst(l1)), p) && EqualTerms(tt, tf)) {
      List ml;
  ...
  return AppendLast(m_l, MakeCompound2(OPR_DIV, p, AppendLast(NewList(), m2)));
}
...
static void hl2m2(List hhl) {
  ...
  for (l1 = s_l, l2 = u_l; l1; l1 = ListTail(l1), l2 = ListTail(l2)) {
    ...
    if (nco) {
      sco = CopyTerm(CompoundArg2(ListFirst(l1)));
      ChangeList(l2, NewInteger(IntegerValue(ListFirst(l2)) + 1));
      break;
    }
  }
  ...
  for (l1 = sco; l1; l1 = ListTail(l1)) {
    ...
    if (m22)
      SetCompoundArg(
          m2, 2,
          ConcatList(ConsumeCompoundArg(ListFirst(l1), 2), CopyTerm(m22)));
    else
      SetCompoundArg(m2, 2, ConsumeCompoundArg(ListFirst(l1), 2));
  ...
  if (is_list(sco)) {
    ...
    m2 = MakeCompound(A_MTERM, 3);
    SetCompoundArg(m2, 1, NewInteger(nco * IntegerValue(CompoundArg2(sco))));
    ...
    ChangeList(hhl, m2);
    m2 = MakeCompound(A_MTERM, 3);
    SetCompoundArg(m2, 1, NewInteger(nco * IntegerValue(CompoundArgN(sco, 4))));
...
static void l_set(List l, int *nco, Term *sco) {
  ...
  *nco = gcf_list(l);
  ...
  SetCompoundArg(
      m2, 2, AppendLast(NewList(), MakeCompound2(OPR_POW, np, NewInteger(1))));
  s_l = AppendFirst(s_l, MakeCompound2(OPR_DIV, l, AppendLast(NewList(), m2)));
...
static void l_fit(List l1, List l2, int *nco) {
  ...
  while (!is_empty_list(l1)) {
    ...
    if (i2 != ic * i1)
      return;
    if (!EqualTerms(CompoundArg2(ListFirst(l1)), CompoundArg2(ListFirst(l2))))
      return;
    l1 = ListTail(l1);
    l2 = ListTail(l2);
  }
    ...
    if (!is_empty_list(ll)) {
      ...
      if (IntegerValue(CompoundArg1(ListFirst(ll))) < 0) {
        ...
        if (AtomValue(CompoundName(CompoundArg2(a)))[0] == 's')
          SetCompoundArg(
              CompoundArg1(a), 1,
              NewInteger(-IntegerValue(CompoundArg1(CompoundArg1(a)))));
      }
...
static List tr_mlt1(Term a1, Term a2) {
  ...
  for (ll = l2; ll; ll = ListTail(ll)) {
    ...
    for (ll1 = ls; ll1; ll1 = ListTail(ll1)) {
      if (CompoundArg2(ListFirst(ll1)) == aa) {
        ...
        if (n == 0)
          ...
          SetCompoundArg(ListFirst(ll1), 1, NewInteger(n));
        break;
      }
    }
    ...
    if (is_empty_list(ll1)) {
      ...
      SetCompoundArg(q, 1, NewInteger(-IntegerValue(CompoundArg1(q))));
      ld = AppendLast(ld, q);
    }
  }
  ...
  if (f1[0] == 's' && f2[0] == 'c') {
    ...
    res = NewList();
    if (!is_empty_list(ls))
      res = AppendLast(res, MakeCompound2(OPR_MLT,
...
static void to_sums(List l) {
  ...
  for (l1 = l; l1; l1 = ListTail(l1)) {
    ...
    for (l2 = CompoundArg2(ListFirst(l1)); l2; l2 = ListTail(l2)) {
      Term cuf;
      int pw;
      cuf = GetAtomProperty(CompoundArg1(ListFirst(l2)), A_TRIG_FU);
      pw = IntegerValue(CompoundArg2(ListFirst(l2)));
      cuf = CopyTerm(cuf);
      cuf = MakeCompound2(
          OPR_MLT, MakeCompound2(OPR_DIV, NewInteger(1), NewInteger(1)), cuf);
      for (; pw > 1; pw--)
    ...
    if (is_empty_list(lcuf)) {
      res = tr_add(
          res, MakeCompound2(OPR_MLT,
                             MakeCompound2(OPR_DIV, CompoundArg1(ListFirst(l1)),
                                           NewInteger(1)),
      ...
      for (l2 = res1; l2; l2 = ListTail(l2)) {
        Term a1;
        a1 = ListFirst(l2);
        SetCompoundArg(CompoundArg1(a1), 1,
                       NewInteger(IntegerValue(CompoundArg1(CompoundArg1(a1))) *
                                  IntegerValue(CompoundArg1(ListFirst(l1)))));
        res = tr_add(res, a1);
      }
      RemoveList(lcuf);
