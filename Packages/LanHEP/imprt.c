...
List mk_im_field(int col, int spin, int ch, List hint) {
  ...
  spp = NewAtom(nnbuf, 0);
  used_fields =
      AppendLast(used_fields,
                 MakeCompound2(OPR_MINUS, spp, MakeCompound2(OPR_DIV, n, an)));

  if (verb_imprt) {
    printf("Intermediate field %s/%s for vertex ", AtomValue(n), AtomValue(an));
    WriteVertex(hint);
    printf(".\n");
    printf("hint= %d ", checked_hint);
    WriteTerm(hint);
    puts("");
  }
...
Term ProcImPrt(Term t, Term ind) {
  ...
  if (is_compound(t) && CompoundArity(t) > 2 && is_compound(CompoundArg1(t)) &&
      CompoundName(CompoundArg1(t)) == OPR_DIV) {
    Term r;
    r = CompoundArg1(t);
