...
extern int opEvalVrt, EvalVrt, NoColors, opAbbrVrt, allow_sym_div,
    write_all_vertices;
extern int FAver;
...
static int pcmp(Term mm1, Term mm2) {
  ...
  while (is_float(CompoundArg1(ListFirst(m1))) ||
         CompoundArg1(ListFirst(m1)) == A_I)
    m1 = ListTail(m1);
  while (is_float(CompoundArg1(ListFirst(m2))) ||
         CompoundArg1(ListFirst(m2)) == A_I)
    m2 = ListTail(m2);
...
void FA_write_lagr(List l, FILE *f) {
  ...
  for (l1 = all_prtc_list(); l1; l1 = ListTail(l1)) {
    ...
    if (col)
      col = IntegerValue(CompoundArg1(col));
    ...
    if (CompoundName(p) == OPR_PARTICLE) {
      ...
      if (IntegerValue(CompoundArgN(p, 4)) == 2 &&
        ...
        if (!firstprt)
          ...
          firstprt = 0;
        fprintf(f, "  U[%d] ==  { (* ", nu);
        fprintf(f, "%s/%s", AtomValue(CompoundArg1(g)),
        ...
        fprintf(f, "\tMass -> %s,\n",
                CompoundArgN(p, 5) ? AtomValue(CompoundArgN(p, 5)) : "0");
        ...
        if (CompoundArg1(p) != CompoundArg2(p)) {
          ...
          if (!firstprt)
            ...
            firstprt = 0;
          fprintf(f, "  U[%d] ==  { (* ", nu);
          fprintf(f, "%s/%s", AtomValue(CompoundArg1(ag)),
    ...
    if (CompoundName(p) == OPR_FIELD) {
      ...
      if (CompoundArg2(p) == NewInteger(1) && CompoundArgN(bpp, 5)) {
        ...
        if (col && NoColors)
          fprintf(f, "\tMatrixTraceFactor -> %d,\n", col == 1 ? 3 : 8);
        fprintf(f, "\tMass -> %s,\n",
                CompoundArgN(bpp, 5) ? AtomValue(CompoundArgN(bpp, 5)) : "0");
        if (defined_em_charge()) {
  ...
  */
  ...
  for (l1 = l; l1; l1 = ListTail(l1)) {
    ...
    for (l2 = CompoundArg1(a2), i = 0; l2; l2 = ListTail(l2), i++) {
      ...
      if (a == OPR_SCALAR &&
          GetAtomProperty(CompoundArg1(ListFirst(l2)), A_GRASS))
        a = A_GRASS;
      lp = AppendLast(lp, a);
    }
    ...
    for (l2 = CompoundArg1(a2), i = 0; l2; l2 = ListTail(l2), i++) {
      ...
      if (GetAtomProperty(CompoundArg1(ListFirst(l2)), OPR_CLASS)) {
        List l4;
        for (l4 = CompoundArg1(CompoundArg2(ListFirst(l2))); l4 && ListTail(l4);
             l4 = ListTail(l4))
          ;
    ...
    for (l2 = CompoundArg1(a2), i = 0; l2; l2 = ListTail(l2), i++) {
      ...
      if (fai == 0)
        printf("Internal error - miss no for %s\n",
               AtomValue(CompoundArg1(ListFirst(l2))));
      else
    ...
    for (l2 = CompoundArgN(a2, 5); l2; l2 = ListTail(l2)) {
      ...
      for (l3 = lp, i = 0; lp; l3 = ListTail(l3), i++)
        if (EqualTerms(ls, ListFirst(l3)))
          break;
      if (l3 == 0 || i >= lvl) {
        puts("Internal error wrtfa06");
        continue;
      }
      for (l3 = CompoundArg2(ListFirst(l2)); l3; l3 = ListTail(l3))
        if ((pr =
                 GetAtomProperty(CompoundArg1(ListFirst(l3)), A_INFINITESIMAL)))
      ...
      for (l3 = CompoundArgN(ListFirst(l2), 3); l3; l3 = ListTail(l3))
        if ((is_atom(CompoundArg1(ListFirst(l3))) &&
             (pr = GetAtomProperty(CompoundArg1(ListFirst(l3)),
                                   A_INFINITESIMAL))))
          io += 1;
      if (io == 1)
    ...
    for (i = 0; i < lvl; i++) {
      ...
      if (lv[i] == 0)
        ...
        for (l2 = lv[i]; l2; l2 = ListTail(l2)) {
          ...
          wrt_expr(f, CompoundArg1(m2), CompoundArg2(m2), CompoundArgN(m2, 3));
          if (ListTail(l2) &&
...
static Term conv_lor(List pl, Term m2, int eff) {
  ...
  if (ili[0] == 0) {
    ...
    for (i = 1; i <= 4; i++) {
      sprintf(cbuf, "li%d", i);
      ili[i - 1] = NewAtom(cbuf, 0);
      sprintf(cbuf, "mom%d", i);
      imom[i - 1] = NewAtom(cbuf, 0);
    }
  }
      ...
      if (CompoundName(ListFirst(l)) == OPR_SPECIAL &&
        ...
        if (CompoundArg1(ListFirst(l)) == A_GAMMA) {
          ...
          for (l2 = CompoundArgN(m2, 3); l2; l2 = ListTail(l2))
            if (ListFirst(l2) && CompoundName(ListFirst(l2)) == A_MOMENT &&
  ...
  for (l = CompoundArgN(m2, 3); l; l = ListTail(l)) {
    ...
    if (CompoundArg1(ListFirst(l)) == A_EPS_V) {
      ...
      int j;
      List lj, l2;
      for (j = 0, lj = CompoundArg2(ListFirst(l)); j < 4;
           j++, lj = ListTail(lj)) {
        in[j] = 0;
        for (i = 0; i < 4; i++)
        ...
        for (l2 = CompoundArgN(m2, 3); l2; l2 = ListTail(l2))
          if (CompoundName(ListFirst(l2)) == A_MOMENT &&
              ListFirst(lj) == ListFirst(CompoundArg2(ListFirst(l2))))
            break;
        if (l2)
    ...
    if (CompoundName(ListFirst(l)) == A_MOMENT) {
      ...
      for (l2 = ListTail(l); l2; l2 = ListTail(l2)) {
        ...
        if (CompoundName(ListFirst(l2)) == A_MOMENT &&
          ...
          if (p1 < p2)
            cc = AppendLast(cc,
                            MakeCompound2(A_DELTA, imom[p1 - 1], imom[p2 - 1]));
          else
            cc = AppendLast(cc,
                            MakeCompound2(A_DELTA, imom[p2 - 1], imom[p1 - 1]));
          if (eff) {
            FreeAtomic(ListFirst(l));
            ChangeList(l, 0);
            FreeAtomic(ListFirst(l2));
...
static void wrt_expr(FILE *of, Term num, List sym, List ten) {
  ...
  for (l = ten; l; l = ListTail(l)) {
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l)), A_COLOR) ==
      ...
      if (il2 >= 5 && l2 && ListFirst(CompoundArg2(ListFirst(l2))) == in2) {
        ...
        if (i == 4) {
          ...
          puts(": color structure error(2)");
          il3 = IntegerValue(in3);
        }
        for (i = 0; i < 4; i++)
          if (in4 == pind[i])
            break;
        ...
        if (i == 4) {
          ...
          WriteTerm(ListFirst(l2));
          puts(": color structure error(2)");
          il4 = IntegerValue(in4);
        }
        sno += fprintf(of, "SUNT[c%d, c%d, c%d, c%d] ", il3 + 1, il4 + 1,
                       il1 + 1, il2 + 1);
        if (sno > 60 && ListTail(l) &&
            (ListTail(l) != l2 || ListTail(ListTail(l)))) {
          fprintf(of, "*\n\t\t");
          sno = 15;
        }
      ...
      if (il3 >= 5 && l2 && ListNth(CompoundArg2(ListFirst(l2)), 3) == in3) {
        ...
        sno += fprintf(of, "SUNTSum[c%d, c%d, c%d, c%d] ", il3 + 1, il4 + 1,
                       il1 + 1, il2 + 1);
        if (sno > 60 && ListTail(l) &&
            (ListTail(l) != l2 || ListTail(ListTail(l)))) {
          fprintf(of, "*\n\t\t");
      ...
      WriteVertex(cv);
      WriteTerm(ListFirst(l));
      puts(": color structure error");
      il2 = IntegerValue(in2);
      sno += fprintf(of, "SUNT[c%d, c%d, c%d] ", il3 + 1, il1 + 1, il2 + 1);
      if (sno > 60 && ListTail(l)) {
        fprintf(of, "*\n\t\t");
        sno = 15;
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l)), A_COLOR) == A_COLOR_F) {
      Integer in1, in2, in3, in4;
      int i, il1, il2, il3, il4;
      List l2;
      in1 = ListFirst(CompoundArg2(ListFirst(l)));
      ...
      if (i == 4) {
        WriteVertex(cv);
        WriteTerm(ListFirst(l));
        puts(": color structure error(1)");
        il1 = IntegerValue(in1);
      }
      for (i = 0; i < 4; i++)
        if (in2 == pind[i])
          break;
      ...
      for (l2 = ListTail(l); l2; l2 = ListTail(l2))
        if (GetAtomProperty(CompoundArg1(ListFirst(l2)), A_COLOR) ==
                A_COLOR_F &&
            ListNth(CompoundArg2(ListFirst(l2)), 3) == in3)
          break;
      if (l2) {
        in3 = ListFirst(CompoundArg2(ListFirst(l2)));
        in4 = ListFirst(ListTail(CompoundArg2(ListFirst(l2))));
        ...
        sno += fprintf(of, "SUNF[c%d, c%d, c%d, c%d] ", il1 + 1, il2 + 1,
                       il3 + 1, il4 + 1);
    ...
    if (GetAtomProperty(CompoundArg1(ListFirst(l)), A_COLOR) == A_COLOR_D) {
      Integer in1, in2, in3, in4;
      int i, il1, il2, il3, il4;
      List l2;
      in1 = ListFirst(CompoundArg2(ListFirst(l)));
      ...
      if (l2) {
        ...
        sno += fprintf(of, "SUNDF[c%d, c%d, c%d, c%d] ", il1 + 1, il2 + 1,
                       il3 + 1, il4 + 1);
        if (sno > 60 && ListTail(l) && (ListTail(l) != l2 || ListTail(l2))) {
          fprintf(of, "*\n\t\t");
          sno = 15;
      ...
      if (l2) {
        ...
        sno += fprintf(of, "SUND[c%d, c%d, c%d, c%d] ", il1 + 1, il2 + 1,
                       il3 + 1, il4 + 1);
        if (sno > 60 && ListTail(l) && (ListTail(l) != l2 || ListTail(l2))) {
          fprintf(of, "*\n\t\t");
          sno = 15;
  ...
  for (l1 = gkl; l1; l1 = ListTail(l1)) {
    ...
    for (l2 = CompoundArg2(ListFirst(l1)); l2; l2 = ListTail(l2)) {
      ...
      for (l3 = cc; l3; l3 = ListTail(l3)) {
        Atom a1 = CompoundArg1(ListFirst(l3)), a2 = CompoundArg2(ListFirst(l3));
        if (CompoundArity(ListFirst(l3)) == 4)
          fprintf(f, " Epsilon[%s, %s, %s, %s]", AtomValue(a1), AtomValue(a2),
                  AtomValue(CompoundArgN(ListFirst(l3), 3)),
                  AtomValue(CompoundArgN(ListFirst(l3), 4)));
        else if (AtomValue(a1)[0] == 'l' && AtomValue(a2)[0] == 'l')
          ...
          printf("Internal error wrtfa02; ");
          WriteTerm(ListFirst(l3));
          puts("");
        }
      }
      if (nc) {
        ...
        for (l3 = nc; l3; l3 = ListTail(l3)) {
          if (ListFirst(l3) == A_GAMMAP)
            fprintf(f, "ChiralityProjector[+1]");
          else if (ListFirst(l3) == A_GAMMAM)
            fprintf(f, "ChiralityProjector[-1]");
            ...
            if (AtomValue(a)[0] == 'l')
              ...
              puts("Internal error wrtfa03");
            }
          } else {
            puts("Internal error wrtfa04");
          }
          if (ListTail(l3))
            fprintf(f, ", ");
        }
  ...
  for (l = CompoundArgN(a2, 5); l; l = ListTail(l)) {
    ...
    for (l1 = CompoundArgN(m, 3); l1; l1 = ListTail(l1)) {
      ...
      if (CompoundName(s) == A_MOMENT)
        mom = s;
      if (CompoundName(s) == OPR_SPECIAL &&
          (CompoundArg1(s) == A_GAMMAP || CompoundArg1(s) == A_GAMMAM))
        gpm = CompoundArg1(s);
    }
