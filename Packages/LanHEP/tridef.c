...
void tri_reg_prm(Atom p, Term val) {
  ...
  if (p1) {
    ...
    ns =
        MakeCompound2(OPR_EQSIGN, ns, MakeCompound2(OPR_POW, p, NewInteger(2)));
  ...
  if (is_compound(val) && CompoundArity(val) == 2 &&
      CompoundName(val) == OPR_DIV && is_atom(CompoundArg1(val)) &&
      is_atom(CompoundArg2(val))) {
    Atom s, c;
    s = CompoundArg1(val);
    c = CompoundArg2(val);
    for (l = tri_si_co_list; l; l = ListTail(l))
...
xyz:
  ...
  if (ListLength(gte_used) == 2) {
    ...
    for (l = tri_si_co_list; l; l = ListTail(l))
      if ((c1 == CompoundArg1(ListFirst(l)) &&
           s1 == CompoundArg2(ListFirst(l))) ||
          (s1 == CompoundArg1(ListFirst(l)) &&
           c1 == CompoundArg2(ListFirst(l))))
        break;
    if (is_empty_list(l)) {
      FreeAtomic(val);
      return;
    ...
    if ((CompoundArg1(ListFirst(l1)) != c1 ||
  ...
  */
  ...
  if (is_empty_list(l)) {
    ...
    if (type > 0)
      ...
      s = -1, type = -type;
    SetCompoundArg(t, type + 3,
                   MakeCompound1(s == 1 ? OPR_PLUS : OPR_MINUS, p));
    if (CompoundArgN(p1, 7) && CompoundArgN(p2, 7) &&
        !GetAtomProperty(p, A_TEXNAME)) {
      char buf[32];
      Atom nn = 0;
      if (type == 1 && s == 1) {
...
Term ProcAngle(Term t, Term ind) {
  ...
  for (l1 = t; l1; l1 = ListTail(l1)) {
    ...
    if (!is_compound(t1) || CompoundArity(t1) != 2 ||
      ...
      ErrorInfo(584);
      printf("angle: illegal construction '");
      WriteTerm(t1);
      puts("'");
      return 0;
    }
