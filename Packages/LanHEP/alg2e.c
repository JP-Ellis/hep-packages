...
static int alg2_red_orth_11(List *lm, List l1, Term prop1, Term prop2) {
  ...
  } else if (CompoundArgN(prop1, 4) == CompoundArgN(prop2, 4)) {
    type = 2;
...
static List orth3_mkprod(Term matr, int dim, int tp1, int k1, int k2, Atom a3) {
  List ret = 0;
  int i;
  for (i = 1; i <= dim; i++) {
    List m = 0;
...
static int alg2_red_orth_4(List *lm, List l1, Term *props, Term *names) {
  ...
  if (names[0] == names[3]) {
    if (rows[0] == cols[0]) {
      if (orth4_fit(lm, m1, par, 1,
                    orth4_mkprod(matr, dim, 1, 1, rows[0], rows[0], rows[0],
                                 rows[0])))
        return 1;

      if (orth4_fit(lm, m1, par, 1,
                    orth4_mkprod(matr, dim, 2, 2, rows[0], rows[0], rows[0],
                                 rows[0])))
        return 1;
      FreeAtomic(m1);
      FreeAtomic(par);
  ...
  if (names[0] == names[1] && names[1] != names[2] && names[2] == names[3]) {
    if (orth4_fit(
            lm, m1, par, 1,
            orth4_mkprod(matr, dim, 1, 1, cols[0], cols[0], cols[2], cols[2])))
      return 1;
    if (orth4_fit(
            lm, m1, par, 1,
            orth4_mkprod(matr, dim, 2, 2, rows[0], rows[0], rows[2], rows[2])))
      return 1;

    if (orth4_fit(
            lm, m1, par, 0,
            orth4_mkprod(matr, dim, 1, 1, cols[0], cols[2], cols[0], cols[2])))
      return 1;
    if (orth4_fit(
            lm, m1, par, 0,
            orth4_mkprod(matr, dim, 2, 2, rows[0], rows[2], rows[0], rows[2])))
      return 1;
  }

  if (orth4_fit(
          lm, m1, par, cols[0] == cols[1] && cols[2] == cols[3],
          orth4_mkprod(matr, dim, 1, 1, cols[0], cols[1], cols[2], cols[3])))
    return 1;
  if (orth4_fit(
          lm, m1, par, cols[0] == cols[2] && cols[1] == cols[3],
          orth4_mkprod(matr, dim, 1, 1, cols[0], cols[2], cols[1], cols[3])))
    return 1;
  if (orth4_fit(
          lm, m1, par, cols[0] == cols[3] && cols[1] == cols[2],
          orth4_mkprod(matr, dim, 1, 1, cols[0], cols[3], cols[1], cols[2])))
    return 1;

  if (orth4_fit(
          lm, m1, par, rows[0] == rows[1] && rows[2] == rows[3],
          orth4_mkprod(matr, dim, 2, 2, rows[0], rows[1], rows[2], rows[3])))
    return 1;
  if (orth4_fit(
          lm, m1, par, rows[0] == rows[2] && rows[1] == rows[3],
          orth4_mkprod(matr, dim, 2, 2, rows[0], rows[2], rows[1], rows[3])))
    return 1;
  if (orth4_fit(
          lm, m1, par, rows[0] == rows[3] && rows[1] == rows[2],
    ...
    */
    ...
    if (orth_red_diag != ListFirst(l0)) {
      for (l = lm; l; l = ListTail(l)) {
        ...
        for (l1 = CompoundArg2(ListFirst(l)); l1; l1 = ListTail(l1))
          if ((ss =
                   GetAtomProperty(CompoundArg1(ListFirst(l1)), A_ORTH_MATR)) &&
              CompoundArg1(ss) == ListFirst(l0))
            compl = 0;
      }
  ...
  bgn:
    ...
    while (!is_empty_list(l)) {
      ...
      if (no == 2 && sn[0] == sn[1] && alg2_red_herm_2(&lm, l, ss[0]))
        goto bgn;
      if (no == 2 && sn[0] != sn[1] &&
          CompoundArg2(ss[0]) == CompoundArg2(ss[1]) &&
...
setprp2:
  ...
  hermi_matrices =
      AppendLast(hermi_matrices, MakeCompound3(OPR_DIV, lab, tr, t1));
    ...
    while (!is_empty_list(lp)) {
      ...
      if (ss && ipw > 1) {
        if (ipw == 2) {
          ...
          SetCompoundArg(m11, 1, NewInteger(-IntegerValue(CompoundArg1(m11))));
          l3 = rcos_add(l3, m11);
          cfl = 1;
          break;
        }
        ...
        if (ipw == 6) {
          ...
          SetCompoundArg(m13, 1, NewInteger(-IntegerValue(CompoundArg1(m13))));
          l3 = rcos_add(l3, m11);
          l3 = rcos_add(l3, m12);
          l3 = rcos_add(l3, m13);
          cfl = 1;
