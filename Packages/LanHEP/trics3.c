...
static int tri_find_pure_ab(List *ml, Atom s1, Atom s2, Atom c1, Atom c2) {
  ...
  if (IntegerValue(CompoundArg1(m2_2)) != -comf ||
      (comf != comf1 && comf != -comf1))
    return 0;
  ...
  */
  ...
  SetCompoundArg(
      m2_1, 2,
      MakeList2(MakeCompound2(OPR_POW, tp < 3 ? s1 : c1, NewInteger(1)),
  ...
  SetCompoundArg(
      m2_2, 2,
      MakeList2(MakeCompound2(OPR_POW, tp < 3 ? c1 : s1, NewInteger(1)),
  ...
  SetCompoundArg(
      m2, 2,
      MakeList1(MakeCompound2(OPR_POW, tri_dummy_prm(l2), NewInteger(1))));
...
static List find_3_mono_pm(List ml, Atom sp, Atom sm, Atom cp, Atom cm,
                           int type) {
  /* 1 --- sa*cb**3 + ca*sb**3  *
   * 2 --- sa*cb**3 - ca*sb**3  *
   * 3 --- ca*cb**3 + sa*sb**3  *
...
static List find_3_mono_ab(List ml, Atom s1, Atom s2, Atom c1, Atom c2,
                           int type) {
  /* 1 --- sa*cb**3 + ca*sb**3  *
   * 2 --- sa*cb**3 - ca*sb**3  *
   * 3 --- ca*cb**3 + sa*sb**3  *
...
static void find_3_cnf_ab(List ll, int type, int *cnf, int *fno, List *ml) {
  List l;
  ...
  if (has_pm) {
    ...
    if (bfit < 5) {
      ...
      */
      ...
      SetCompoundArg(
          m2, 2,
          MakeList1(MakeCompound2(OPR_POW, tri_dummy_prm(l2), NewInteger(2))));
