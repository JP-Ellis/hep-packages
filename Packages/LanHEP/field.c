...
static char *props[] = {"fullname", "name",  "aname",   "spin2",
                        "mass",     "width", "echarge", "echarge3",
                        "color",    "aux",   "texname", "atexname"};
...
int is_particle(Atom p, List *ind) {
  ...
  if (!(is_compound(prt) &&
        (CompoundName(prt) == OPR_PARTICLE || CompoundName(prt) == OPR_FIELD)))
    return 0;
  if (ind != NULL)
...
static int set_particle_specs(Term t, Term sv, int pflag, int first) {
  ...
  if (is_compound(t) && FunctorName(CompoundFunctor(t)) == OPR_COMMA) {
    ...
    FreeAtomic(t);
    if (set_particle_specs(t1, sv, pflag, first) == 0)
      ret = 0;
    if (set_particle_specs(t2, sv, pflag, 0) == 0)
      ret = 0;
    return ret;
  }
  ...
  if (is_compound(t) && strcmp(AtomValue(CompoundName(t)), "echarge") == 0) {
    ...
    if (is_integer(t1))
      ...
      if (!is_compound(t1) || CompoundName(t1) != OPR_DIV ||
          !is_integer(CompoundArg1(t1)) || !is_integer(CompoundArg2(t1)))
    ...
    if (p1 == p2 && n) {
      WarningInfo(900);
      printf("particle %s identical to antiparticle with nonzero charge\n",
             AtomValue(p1));
    }
    return 1;
  }
...
static void ParticleList(int spin) {
  ...
  while (!is_empty_list(li)) {
    ...
    if (spin == -1 || spin == IntegerValue(CompoundArgN(t, 4))) {
      ...
      printf("%s/%s\t", AtomValue(CompoundArg1(t)), AtomValue(CompoundArg2(t)));
...
static void texWritePrt(int fno) {
  ...
  if (defined_em_charge()) {
    ...
    fprintf(f, "P & aP & Name & Spin & Mass & Width & Color &  Comment \\\\ "
               "\\hline\n");
  }
  li = particles;
  while (!is_empty_list(li)) {
    ...
    if (IntegerValue(CompoundArgN(ttt, 4)) % 2)
      sp = fprintf(f, "$%ld/2$", IntegerValue(CompoundArgN(ttt, 4)));
    else
...
void WriteParticles(int fno, char *name) {
  ...
  fprintf(f, "%s\n Particles \n", name);
  if (pformat == 0) {
    fprintf(f, " Full  name  | P | aP|2*spin| mass |width |color|aux|>      "
               "LaTeX(A)      <|>     "
               "LaTeX(A+)      <|\n");
  } else {
    ...
    for (li = particles; li; li = ListTail(li)) {
      ...
      for (l = pformat; l; l = ListTail(l)) {
        ...
        case 9:
          ...
          if (CompoundArgN(ttt, 7) == A_LEFT)
            sp = fprintf(f, "L");
          else if (CompoundArgN(ttt, 7) == A_RIGHT)
            sp = fprintf(f, "R");
          else if (CompoundArgN(ttt, 7) == A_GAUGE)
            sp = fprintf(f, "G");
          else if (CompoundArgN(ttt, 7) == OPR_MLT)
            sp = fprintf(f, "*");
          else if (CompoundArgN(ttt, 7) != 0)
        ...
        case 11: {
          ...
          if (a1) {
            ...
            sp = fprintf(f, "\\bar{%s}", AtomValue(a2));
          WriteBlank(f, w - sp);
          fprintf(f, "|");
        } break;
        case 12: {
          ...
          if (prop)
            ...
            printf("Warning: property '%s' is not defined for particle '%s'\n",
        ...
        if (w - sp < 0) {
          printf("Warning: Particle table: Property %s for particle %s is too "
                 "wide, make the title "
                 "wider.\n",
                 AtomValue(tp), AtomValue(CompoundArg1(ttt)));
        }
...
Term ProcPrtcFormat(Term t, Term ind) {
  ...
  for (l = fl; l; l = ListTail(l)) {
    ...
    if (is_compound(t) && CompoundArity(t) == 2 &&
        CompoundName(t) == OPR_COLON) {
      n = CompoundArg1(t);
      tt = CompoundArg2(t);
      if (!is_atom(n) || !is_atom(tt))
        goto err;
...
Term ProcPrtcProp(Term t, Term ind) {
  ...
  for (l = t; l; l = ListTail(l)) {
    ...
    if (i != PNO && i != 4 && i != 5 && i != 9) {
      ErrorInfo(999);
      printf(
          "prtcproperty: %s can not be set in this way, use different name.\n",
          props[i]);
      continue;
    }
    ...
    for (l1 = t1; l1; l1 = ListTail(l1)) {
      ...
      if (!is_compound(t2) || CompoundName(t2) != OPR_EQSIGN ||
        ...
        if (i == 4 || i == 5 || i == 9) {
          ...
          SetAtomProperty(CompoundArg1(t2), aprp, ConsumeCompoundArg(t2, 2));
      }
    }
    FreeAtomic(t1);
  }
