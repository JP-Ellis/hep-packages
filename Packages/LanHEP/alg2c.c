...
void alg2_eval_more(Term a2) {
  ...
  for (l0 = ctf; l0; l0 = ListTail(l0)) {
    ...
    if (GetAtomProperty(newprm, A_ANTI)) {
      ...
      evll[j] = AppendLast(
          evll[j],
          MakeCompound2(OPR_EQSIGN, GetAtomProperty(newprm, A_ANTI), l1cc));
    }
...
static Atom abbr_find(List pl) {
  ...
  a = NewAtom(cbuf, 0);
  prmli[i] =
      AppendLast(prmli[i], MakeCompound2(OPR_EQSIGN, a, (plsv = CopyTerm(pl))));
    ...
    if (UFOutput == 0) {
      ...
      for (l = ListTail(pl); l; l = ListTail(l))
        t = MakeCompound2(
            ...
            t,
            l_2_t(ConsumeCompoundArg(ListFirst(l), 2),
      ...
      for (l = ListTail(pl); l; l = ListTail(l)) {
        ...
        t = MakeCompound2(
            IntegerValue(CompoundArg1(r)) > 0 ? OPR_PLUS : OPR_MINUS, t,
            l_2_t(ConsumeCompoundArg(ListFirst(l), 2),
                  IntegerValue(CompoundArg1(r)),
  ...
  while (ListLength(pl) > opAbbrVrt) {
    ...
    for (i = 0; i < opAbbrVrt; i++) {
      t = MakeCompound2(
          IntegerValue(CompoundArg1(ListFirst(pl))) > 0 ? OPR_PLUS : OPR_MINUS,
          t,
          l_2_t(ConsumeCompoundArg(ListFirst(pl), 2),
                IntegerValue(CompoundArg1(ListFirst(pl))), 1));
      pl = CutFromList(pl, pl);
    }
  ...
  for (l = pl; l; l = ListTail(l))
    t = MakeCompound2(
        IntegerValue(CompoundArg1(ListFirst(l))) > 0 ? OPR_PLUS : OPR_MINUS, t,
        l_2_t(ConsumeCompoundArg(ListFirst(l), 2),
              IntegerValue(CompoundArg1(ListFirst(l))), 1));
  FreeAtomic(pl);
  if (csf)
...
void alg2_abbr_vrt(Term a2) {
  ...
  if (ListLength(l1) == 1 && UFOutput != 1) {
    ...
    if (l2 == 0 ||
        (ListLength(l2) == 1 && CompoundArg2(ListFirst(l2)) == NewInteger(1)) ||
        (ListLength(l2) == 1 && CompoundArg1(ListFirst(l2)) == A_I &&
  ...
  while (!is_empty_list(l1)) {
    ...
    while (l2) {
      if (EqualTerms(CompoundArgN(m2, 3), CompoundArgN(ListFirst(l2), 3))) {
        pl = AppendLast(
            pl, MakeCompound2(OPR_MLT, ConsumeCompoundArg(ListFirst(l2), 1),
    ...
    if (UFOutput != 1 && ListLength(pl) == 1 &&
        (CompoundArg2(ListFirst(pl)) == 0 ||
         (ListLength(CompoundArg2(ListFirst(pl))) == 1 &&
          CompoundArg2(ListFirst(CompoundArg2(ListFirst(pl)))) ==
              NewInteger(1)))) {
  ...
  if (has_inf)
    for (l1 = nml; l1; l1 = ListTail(l1)) {
      Term m2 = ListFirst(l1);
      if (CompoundArgN(m2, 3) &&
          CompoundName(ListFirst(CompoundArgN(m2, 3))) == OPR_PARAMETER) {
        Atom prm;
  ...
  if (has_i)
    for (l1 = nml; l1; l1 = ListTail(l1)) {
      Term m2 = ListFirst(l1);
      if (CompoundArgN(m2, 3) &&
          CompoundArg1(ListFirst(CompoundArgN(m2, 3))) == A_I) {
        l2 = ConsumeCompoundArg(m2, 3);
        l2 = CutFromList(l2, l2);
        SetCompoundArg(m2, 3, l2);
        l2 = ConsumeCompoundArg(m2, 2);
...
void alg2_eval_vrtn(Term a2) {
  ...
  for (l1 = ml; l1; l1 = ListTail(l1)) {
    ...
    SetCompoundArg(
        ListFirst(l1), 2,
        MakeList1(MakeCompound2(OPR_POW, NewFloat(v), NewInteger(1))));
  }
rpt:
  for (l1 = ml; l1; l1 = ListTail(l1))
    if (ListFirst(l1) == 0) {
