...
Term Alg1to2(Term t) {
  ...
  while (!is_empty_list(t)) {
    ...
    RedSqrt2(y);
    if (TexOutput || FAOutput)
      auxt = alg2_rem_lambdaf(y);
    else
      auxt = 0;
      ...
      if (t == A_GAMMAM || t == A_GAMMAP) {
        ...
        for (l22 = ListTail(l2); l22; l22 = ListTail(l22)) {
          if (EqualTerms(ListFirst(l2), ListFirst(l22))) {
            ...
            for (l33 = CompoundArgN(ListFirst(l2), 3); l33;
                 l33 = ListTail(l33)) {
              if (l33 == l3)
                continue;
...
rpt1:
    ...
    for (l3 = CompoundArgN(ListFirst(l2), 3); l3; l3 = ListTail(l3)) {
      ...
      if (t == A_GAMMAP) {
        ...
        SetCompoundArg(ListFirst(l2), 1,
                       NewInteger(-IntegerValue(CompoundArg1(ListFirst(l2)))));
  ...
  if (pmf && FAOutput == 0 && TexOutput == 0) {
    SetCompoundArg(
        CompoundArg2(a2), 2,
        NewInteger(2 * IntegerValue(CompoundArg2(CompoundArg2(a2)))));
    for (l2 = l1; l2; l2 = ListTail(l2)) {
      List l3;
      pmf = 0;
      for (l3 = CompoundArgN(ListFirst(l2), 3); l3; l3 = ListTail(l3)) {
      ...
      if (pmf == 0)
        SetCompoundArg(
            ListFirst(l2), 1,
            NewInteger(2 * IntegerValue(CompoundArg1(ListFirst(l2)))));
    }
  }
  ...
  if (FAOutput) {
    for (l2 = l1; l2; l2 = ListTail(l2)) {
      ...
      if (g && !gpm) {
        ...
        ChangeList(ListTail(CompoundArg2(ListFirst(g))), NewInteger(maxi + 1));
