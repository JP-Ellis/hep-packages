...
Atomic A_RBRA, A_RCET, A_FBRA, A_FCET, A_QBRA, A_QCET, A_POINT, A_COMMA, A_SECO,
    A_QUOTE, A_RQUOTE, A_DQUOTE, A_QBRACET, A_FBRACET;
...
Atom OPR_SCALAR, OPR_VECTOR, OPR_SPINOR, OPR_TENSOR, OPR_SPINOR3, OPR_MASS,
    OPR_WIDTH, OPR_WILD, OPR_READ, OPR_EDIT, OPR_FUNCTION, OPR_CLASS;
Atom OPR_LET, OPR_LTERM, OPR_MODEL, OPR_CLEAR, OPR_SPECIAL;
Atom OP_PREFIX, OP_INFIX, OP_POSTFIX;
Atom A_COLOR, A_COLOR_LAMBDA, A_COLOR_F, A_COLOR_D, A_LEFT, A_RIGHT, A_GAUGE,
    A_GHOST, A_END, A_S_COLOR, A_COLOR_EPS, A_BRST_TRANSF, A_BRSTI_TRANSF,
    A_MOMENT_S, A_DERIV_S, A_DERIV, A_DERIV5, A_MOMENT_E, A_EPS_V;
Atom PROP_TYPE, PROP_INDEX;
Atom A_I, A_LORENTZ, A_MTERM, A_ALG1, A_ALG2, A_MOMENT, A_SQRT, A_COS, A_SIN,
    A_DELTA, A_GAMMA, A_GAMMA5, A_GAMMAP, A_GAMMAM, A_CURT, A_LHA, A_SQRT2,
    A_CC, A_GRASS, A_ANTI, A_ANTI2, A_GLUON, A_GG, A_VEV, A_ORTH_MATR,
    A_HERM_MATR, A_HERMC, A_CHNAME, A_TEXNAME, A_TEXLENGTH, A_EM_CHARGE,
    A_CSPINOR, A_FANUM, A_SYMM, OPR_LOCAL, A_SET_ANGLE, A_TRIG_FU,
    A_INFINITESIMAL, A_DUMMY_PRM, A_KEEP_LETS;
...
Atom A_SIN, A_COS, A_TAN, A_ASIN, A_ACOS, A_ATAN, A_ATAN2, A_EXP, A_FABS, A_LOG,
    A_IF, A_EXT_FUNC, A_DFDFC, A_EE, A_GG, A_INTEGER;
...
void InitAtoms1(void) {
  ...
  OP_YFX = NewAtom("yfx", 0);
  OP_XFY = NewAtom("xfy", 0);
...
Atom NewAtom(char *s, int len) {
  ...
  ATOM_count++;
  i = 0;
  while (i < 256 && ATOM_buffers[i] != NULL &&
         ATOM_buffill[i] + len + sizeof(List) > 0xffff)
    i++;
  if (i == 256) {
    puts("Internal error (too much atoms or memory lack).");
    exit(0);
  }
  if (ATOM_buffers[i] == NULL) {
    ATOM_buffers[i] = (char *)malloc(0x10000);
    if (ATOM_buffers[i] == NULL) {
      puts("Internal error (memory lack for atoms).");
      exit(0);
    }
  }
  {
    List *tmp;
    tmp = (List *)(ATOM_buffers[i] + ATOM_buffill[i]);
    *tmp = NewList();
  }
...
void SetAtomProperty(Atom a, Atom type, Term value) {
  ...
  bpos = a & 0xffff;
  if (ATOM_buffers[bno] == NULL) {
    puts("Internal error (illegal atom).");
    exit(0);
  }
...
Term GetAtomProperty(Atom a, Atom type) {
  ...
  bpos = a & 0xffff;
  if (ATOM_buffers[bno] == NULL) {
    puts("Internal error (illegal atom) (GetAtomProp).");
    exit(0);
  }
  lp = (List *)(ATOM_buffers[bno] + bpos);
  ll = *lp;
...
Term SetProperty(Term t, Term ind) {
  if (!is_compound(t) || CompoundArity(t) != 3 || !is_atom(CompoundArg1(t)) ||
      !is_atom(CompoundArg2(t))) {
    ErrorInfo(390);
    puts("bad arguments in SetProperty statement");
  }
  SetAtomProperty(CompoundArg1(t), CompoundArg2(t), ConsumeCompoundArg(t, 3));
  FreeAtomic(t);
  return 0;
}
