...
Term ProcGenGauge(Term t, Term ind) { return 0; }
...
static int ord_ind(List pind, Term a1, Atom p) {
  ...
  if (ListLength(l1) != ListLength(pind)) {
    ErrorInfo(702);
    printf(
        "brst_transform: expression for particle %s with different indices\n",
        AtomValue(p));
    return 0;
  }
...
Term ProcBRSTTransf(Term t, Term ind) {
  ...
  for (t1 = t; t1; t1 = ListTail(t1)) {
    ...
    if (!is_compound(t2) || CompoundArity(t2) != 2 ||
        CompoundName(t2) != OPR_RARROW) {
      ErrorInfo(701);
...
Term alg1_proc_brst(Term a1, Atom tp) {
  ...
  for (l3 = l1; l3; l3 = ListTail(l3)) {
    ...
    for (l6 = l4; l6; l6 = ListTail(l6)) {
      ...
      r = ListFirst(l6);
      l5 = AppendLast(l5, r);
      if (CompoundName(r) == OPR_SPECIAL && CompoundArg2(r) == A_MOMENT)
        mm = r;
