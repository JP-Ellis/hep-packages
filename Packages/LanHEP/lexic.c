...
static void flushbuff(void) {
  if (currec == -1) {
    puts("Internal error (no open file)");
    exit(0);
  }
...
int SetInputFile(char *name) {
  ...
  if (isatty(fileno(rrecs[currec].file))) {
    rrecs[currec].diag = 1;
  }
  return 1;
}
...
int IsTermInput(void) { return rrecs[currec].term; }
...
int CurrentInputLine(void) { return rrecs[currec].line; }
