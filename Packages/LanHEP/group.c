...
Term SpecToRepr(Term t) {
  ...
  if (is_compound(t) && CompoundName(t) == OPR_MINUS && CompoundArity(t) == 1 &&
      CompoundArg1(t) == OPR_SPINOR) {
    FreeAtomic(t);
    return MakeCompound2(A_LORENTZ, NewInteger(0), NewInteger(1));
  }
  if (!is_compound(t) || CompoundArity(t) != 1) {
    ErrorInfo(144);
    printf("Error: bad representation spec '");
    WriteTerm(t);
    printf("'\n");
    return 0;
  }
  ...
  if (IntegerValue(CompoundArg2(gspec)) == 0) { /*   U(1) processing */
    if (is_integer(arg))
  ...
  if (ListLength(arg) != IntegerValue(CompoundArgN(gspec, 3))) {
    ErrorInfo(150);
    printf("wrong length of repres spec ");
    WriteTerm(arg);
    puts("");
    return 0;
  }
...
static void proc_repres(Term t) {
  ...
  if (!is_compound(t) || CompoundArity(t) != 2 ||
    ...
    ErrorInfo(152);
    printf("bad form of representation '");
    WriteTerm(t);
    printf("'\n");
    FreeAtomic(t);
    return;
  ...
  if (!is_atom(gname) || (gspec = GetAtomProperty(gname, OPR_GROUP)) == 0) {
    ErrorInfo(153);
    printf("bad group name '");
    WriteTerm(gname);
    printf("' in 'repres' call\n");
    return;
  }
