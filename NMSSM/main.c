...
int main(int argc, char **argv) {
...
#ifdef SUGRA
  {
    ...
    if (argc < 7) {
      ...
      if (argc > 7)
        sscanf(argv[7], "%lf", &sgn);
      else
        sgn = 1;
      if (argc > 8)
        sscanf(argv[8], "%lf", &aLambda);
      else
        aLambda = a0;

      if (argc > 9) {
        sscanf(argv[9], "%lf", &Mtp);
        assignValW("Mtp", Mtp);
      }
      if (argc > 10) {
        sscanf(argv[10], "%lf", &MbMb);
        assignValW("MbMb", MbMb);
      }
      if (argc > 11) {
        sscanf(argv[11], "%lf", &alfSMZ);
        assignValW("alfSMZ", alfSMZ);
      }
    }

    err = nmssmSUGRA(m0, mhf, a0, tb, sgn, Lambda, aLambda, aKappa, mXiF, mXiS,
                     muP, msP, m3h);
  }
#elif defined(EWSB)
  {
...
#ifdef CONSTRAINTS
  {
    ...
    if (LspNlsp_LEP(&csLim))
      printf("LEP excluded by e+,e- -> DM q qbar cross section=%.2E pb \n",
             csLim);
  }
#endif
...
#include "../include/SMODELS.inc"
...
#ifdef INDIRECT_DETECTION
  {
    ...
    */
    ...
    {
      double fi = 0.1,
             dfi = 0.05; /* angle of sight and 1/2 of cone angle in [rad] */
...
#ifdef NEUTRINO
  {
    ...
    if (err == 0) {
#ifdef SHOWPLOTS
      displayPlot("neutrino fluxes [1/Year/km^2/GeV]", "E[GeV]", Emin, Mcdm, 0,
                  2, "nu", 0, SpectdNdE, nu, "nu_bar", 0, SpectdNdE, nu_bar);
#endif

      printf(" E>%.1E GeV neutrino/anti-neutrino fluxes   %.2E/%.2E "
             "[1/Year/km^2]\n",
...
#ifdef SHOWPLOTS
      displayPlot("Upward muons[1/Year/km^2/GeV]", "E[GeV]", Emin, Mcdm / 2, 0,
                  1, "flux", 0, SpectdNdE, mu);
#endif
...
#ifdef SHOWPLOTS
      displayPlot("Contained  muons[1/Year/km^3/GeV]", "E[GeV]", Emin, Mcdm, 0,
                  1, "flux", 0, SpectdNdE, mu);
#endif
