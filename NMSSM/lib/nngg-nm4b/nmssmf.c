...
extern double neut1_(double *MZ, double *MW, double *SW, double *MG1,
                     double *MG2, double *vv, double *tb, double *hk,
                     double *hl, double *xv);
...
double higs1_(double *swf, double *mwf, double *eef, double *sbf, double *cbf,
              double *hkf, double *hlf, double *xvevf, double *hksf,
  ...
  neum[0][0] = 1.0 / cw / cw * mw * mw * cb * cb + hl * hls / cb * sb * xvev +
               hk * hl / cb * sb * xvev * xvev;
  neum[1][1] = 1.0 / cw / cw * mw * mw * sb * sb + hl * hls / sb * cb * xvev +
               hk * hl / sb * cb * xvev * xvev +
               2.0 * vev * vev * sb * sb * dlh2;
  neum[2][2] = 4 * hk * hk * xvev * xvev +
               vev * vev * hl * hls / xvev * cb * sb + hk * hks * xvev;

  neum[0][1] = neum[1][0] = -1.0 / cw / cw * mw * mw * cb * sb +
                            2.0 * vev * vev * cb * sb * hl * hl -
                            xvev * hl * (hls + xvev * hk);
  neum[0][2] = neum[2][0] = vev * (2.0 * hl * hl * xvev * cb -
                                   2 * hk * hl * xvev * sb - hl * hls * sb);

  neum[1][2] = neum[2][1] = vev * (2.0 * hl * hl * xvev * sb -
                                   2 * hk * hl * xvev * cb - hl * hls * cb);
...
double higp1_(double *swf, double *mwf, double *eef, double *sbf, double *cbf,
              double *hkf, double *hlf, double *xvevf, double *hksf,
              double *hlsf) {
  double sw;
  double mw;
  ...
  neum[2][2] = 4 * vev * vev * hk * hl * cb * sb - 3 * hk * hks * xvev +
               vev * vev * hl * hls / xvev * cb * sb;
  ...
  while (f) {
    ...
    for (i = 0; i < 2; i++)
      if (fabs(hipr[i]) < 0.1 ||
          (fabs(hipr[i]) > 0.1 && fabs(hipr[i + 1]) > 0.1 &&
           hipr[i] > hipr[i + 1])) {
        double tmp;
        f = 1;
        tmp = hipr[i];
...
/*    procedure                                                          c*/
/*ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc*/
static doublereal d_sign(x, y) doublereal *x, *y;
{
...
/* Subroutine */ static int dia_(n, a, am, z__) integer *n;
doublereal *a, *am, *z__;
{
  static integer ierr, i__, i, j;
  static doublereal w[1024];
  static doublereal ac[1024] /* was [32][32] */,
      zc[1024] /* was [32][
32] */;
...
/* Subroutine */ static int eisch1_(nm, n, ar, ai, wr, zr, zi, ierr,
                                    work) integer *nm,
    *n;
...
{
  /* System generated locals */
  integer ar_dim1, ar_offset, ai_dim1, ai_offset, zr_dim1, zr_offset, zi_dim1,
      zi_offset, i__1, i__2;
...
/* Subroutine */ static int htridi_(nm, n, ar, ai, d__, e, e2, tau) integer *nm,
    *n;
doublereal *ar, *ai, *d__, *e, *e2, *tau;
{
  ...
  for (ii = 1; ii <= i__1; ++ii) {
    ...
    for (k = 1; k <= i__2; ++k) {
      /* L120: */
      scale = scale + (d__1 = ar[i__ + k * ar_dim1], abs(d__1)) +
  ...
  L140:
    ...
    for (k = 1; k <= i__2; ++k) {
      ...
      h__ = h__ + ar[i__ + k * ar_dim1] * ar[i__ + k * ar_dim1] +
            ai[i__ + k * ai_dim1] * ai[i__ + k * ai_dim1];
      /* L150: */
    }
    ...
    tau[(l << 1) + 1] = (ai[i__ + l * ai_dim1] * tau[(i__ << 1) + 2] -
                         ar[i__ + l * ar_dim1] * tau[(i__ << 1) + 1]) /
                        f;
    si = (ar[i__ + l * ar_dim1] * tau[(i__ << 1) + 2] +
          ai[i__ + l * ai_dim1] * tau[(i__ << 1) + 1]) /
         f;
    h__ += f * g;
    g = g / f + 1.;
  ...
  L170:
    ...
    for (j = 1; j <= i__2; ++j) {
      ...
      i__3 = j;
      for (k = 1; k <= i__3; ++k) {
        g = g + ar[j + k * ar_dim1] * ar[i__ + k * ar_dim1] +
            ai[j + k * ai_dim1] * ai[i__ + k * ai_dim1];
        gi = gi - ar[j + k * ar_dim1] * ai[i__ + k * ai_dim1] +
             ai[j + k * ai_dim1] * ar[i__ + k * ar_dim1];
        /* L180: */
      }
      jp1 = j + 1;
      if (l < jp1) {
      ...
      i__3 = l;
      for (k = jp1; k <= i__3; ++k) {
        g = g + ar[k + j * ar_dim1] * ar[i__ + k * ar_dim1] -
            ai[k + j * ai_dim1] * ai[i__ + k * ai_dim1];
        gi = gi - ar[k + j * ar_dim1] * ai[i__ + k * ai_dim1] -
             ai[k + j * ai_dim1] * ar[i__ + k * ar_dim1];
        /* L200: */
      }
    L220:
      e[j] = g / h__;
      tau[(j << 1) + 2] = gi / h__;
      f = f + e[j] * ar[i__ + j * ar_dim1] -
          tau[(j << 1) + 2] * ai[i__ + j * ai_dim1];
      /* L240: */
    }
    hh = f / (h__ + h__);
    i__2 = l;
    for (j = 1; j <= i__2; ++j) {
      ...
      for (k = 1; k <= i__3; ++k) {
        ...
        ai[j + k * ai_dim1] = ai[j + k * ai_dim1] - f * tau[(k << 1) + 2] -
                              g * ai[i__ + k * ai_dim1] - fi * e[k] -
                              gi * ar[i__ + k * ar_dim1];
        /* L260: */
      }
    }
...
/* Subroutine */ static int htribk_(nm, n, ar, ai, tau, m, zr, zi) integer *nm,
    *n;
...
doublereal *zr, *zi;
{
  ...
  for (i__ = 2; i__ <= i__2; ++i__) {
    ...
    for (j = 1; j <= i__1; ++j) {
      ...
      i__3 = l;
      for (k = 1; k <= i__3; ++k) {
        s = s + ar[i__ + k * ar_dim1] * zr[k + j * zr_dim1] -
            ai[i__ + k * ai_dim1] * zi[k + j * zi_dim1];
        si = si + ar[i__ + k * ar_dim1] * zi[k + j * zi_dim1] +
             ai[i__ + k * ai_dim1] * zr[k + j * zr_dim1];
        /* L110: */
      }
      s /= h__;
      si /= h__;
      i__3 = l;
      for (k = 1; k <= i__3; ++k) {
        zr[k + j * zr_dim1] = zr[k + j * zr_dim1] - s * ar[i__ + k * ar_dim1] -
                              si * ai[i__ + k * ai_dim1];
        zi[k + j * zi_dim1] = zi[k + j * zi_dim1] - si * ar[i__ + k * ar_dim1] +
                              s * ai[i__ + k * ai_dim1];
        /* L120: */
      }
      /* L130: */
...
/* Subroutine */ static int tql2_(nm, n, d__, e, z__, ierr) integer *nm, *n;
  ...
  L130:
    ...
    L160:
      ...
      for (k = 1; k <= i__3; ++k) {
        ...
        z__[k + (i__ + 1) * z_dim1] = s * z__[k + i__ * z_dim1] + c__ * h__;
